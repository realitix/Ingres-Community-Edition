...
static DU_STATUS duve_validate_table( DUVE_CB *duve_cb, 
				char *dbname,
				char *tablename,
				char *ownername,
				i4   *table_id,
...
**	12-apr-2011 (stial01) m1577
**	    verifydb -otable unsupported for VectorWise tables
**	    Issue legal warning after validating table arg
*/
...
char		*tablename;
{
    ...
    if (duve_cb->duve_user[0] == '\0')
	tbl_owner = duve_cb->duveowner;
    else
	tbl_owner = duve_cb->duve_user;
    ...
    */
    ...
    /* validate table arg, this might uppercase table name for fips */
    if ( duve_validate_table( duve_cb, dbname, tbl_name, tbl_owner,
		&table_id, &index_id) != DUVE_YES)
	return ( (DU_STATUS) DUVE_BAD);

    /* Issue legal warning */
    if ( duve_issue_legal_warning( duve_cb ) != DUVE_YES)
	return ( (DU_STATUS) DUVE_BAD);
    ...
    exec sql select relstat into :relstat from iirelation
	where relid = :tbl_name and relowner = :tbl_owner;
	...
	*/
	...
	select constraint_name, constraint_type from iiconstraints
		where table_name = :tbl_name and schema_name = :tbl_owner;
	    ...
	    */
	    ...
	    if (ask == DUVE_YES)
	    {
	        cmd = STpolycat( (i4) 7, "alter table ", tbl_owner, ".",
...

/*{
** Name: duve_validate_table
**
** Description:
**
**	This routine validates the table name specified for check/patch table
**
** Inputs:
**      duve_cb				verifydb control block
...
**      duve_cb				verifydb control block
**      owner				table owner
**      table_id			table base id (reltid)
**      index_id			table index id (reltidx)
** 
**	Returns:
**	    DUVE_YES			success
**	    DUVE_BAD			error
...
**
** Side Effects:
**	    The table name may get converted to uppercase if fips db 
**
** History:
**	12-apr-2011 (stial01)
**	    Initial Creation
...
{
    exec sql begin declare section;
	char	*tbl_name = tablename;
	char	*tbl_owner = tableowner;
	int 	reltid = 0;
	int	reltidx = 0;
	int	relspec = 0;
	int	relstat = 0;
    ...
    /* uppercase table name if this installation is FIPS */
    exec sql select dbmsinfo('db_name_case') into :fipsbuf;
    if (STcompare(fipsbuf, ERx("UPPER")) == DU_IDENTICAL)
	CVupper(tbl_name);

    /* get the tableid, table relstat */
    exec sql select reltid, relstat, relspec 
	into :reltid, :relstat, :relspec from iirelation
	where relid = :tbl_name and relowner = :tbl_owner;
	...
	duve_talk(DUVE_MODESENS, duve_cb, E_DU5025_NO_SUCH_TABLE, 4,
			  0, tablename, 0, tableowner);
        return ( (DU_STATUS) DUVE_BAD);
    }

    if (relspec > DB_STDING_STORE_MAX)
    {
	/* currently only supported for ingres tables */
	duve_talk( DUVE_MODESENS, duve_cb, E_DU503F_VERIFY_UNSUPPORTED, 0);
	duve_close(duve_cb, TRUE);
	return ( (DU_STATUS) DUVE_BAD);
    }
    ...
    {
	/* only report mode is valid for s_concur catalog, index */
