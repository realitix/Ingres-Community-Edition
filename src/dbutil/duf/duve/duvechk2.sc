...
**	    Replace DB_xxx values with equivalent PSQ_MODEs.
**/
...
DUVE_CB		*duve_cb;
{
	...
	*/
	if (   dep_offset == DU_INVALID
	    && dpnd_tbl.dtype != PSQ_VIEW
	    && dpnd_tbl.dtype != PSQ_INDEX)
	{
	...
	*/
	...
	else
	{
		...
		*/
	    ...
	    else if (   dpnd_tbl.itype == PSQ_DB_TABLE
	             || dpnd_tbl.itype == PSQ_VIEW
	             || dpnd_tbl.itype == PSQ_INDEX)
	    {
		/* 
	...
	*/
	if (dep->du_dflags & DU_MISSING_INDEP_OBJ)
	{
		...
		*/
	    ...
	    else if (dpnd_tbl.dtype == PSQ_RULE) 
	    { 
		i4         tbl_offset;
	    ...
	    else if (dpnd_tbl.dtype == PSQ_CREDBP)
	    { 
	    ...
	    else if (dpnd_tbl.dtype == PSQ_CONS)
	    { 
		i4         tbl_offset;
...
DUVE_CB            *duve_cb;
{
	...
	if (cnt == 0)
	{
		...
		*/
		...
		drop_obj->duve_obj_type            = PSQ_EVENT;
...
DUVE_CB            *duve_cb;
{
	...
	if (cnt == 0)
	{
		...
		*/
		...
		drop_obj->duve_obj_type            = PSQ_CREDBP;
		drop_obj->duve_secondary_id	   = 0;
	...
	*/
	...
	if (base == DU_INVALID)
	{
	    ...
	    ** so drop from iiprotect */
	    ...
	    {
		case DBOB_TABLE:
		    mode = PSQ_DB_TABLE; break;
		case DBOB_VIEW:
		    mode = PSQ_VIEW;  break;
		case DBOB_DBPROC:
		    mode = PSQ_CREDBP;   break;
		case DBOB_SEQUENCE:
		    mode = PSQ_CSEQUENCE; break;
		default:
		    mode = PSQ_EVENT; break;
	    }
		...
		*/
		...
		if (!satisfied)
		{
		    ...
		    */
		    ...
		    if (mode == PSQ_DB_TABLE || mode == PSQ_VIEW)
		    {
...
**          constant PSQ_REG_LINK used to be DB_REGISTER; it was changed to
...
#define		IS_NULL		-1
    ...
    */
	    ...
	    || (qry_tbl.mode != mode))
	{
		...
		** be null when qry_tbl.mode is PSQ_VIEW (17).
		*/
	...
	*/
	...
	if ( (qry_tbl.mode != PSQ_VIEW) && (qry_tbl.mode != PSQ_PROT) &&
	     (qry_tbl.mode != PSQ_0_CRT_LINK) && (qry_tbl.mode != PSQ_REG_LINK) &&
	     (qry_tbl.mode != PSQ_REG_IMPORT) &&
	     (qry_tbl.mode != PSQ_CALARM) &&
	     (qry_tbl.mode != PSQ_INTEG) && (qry_tbl.mode != PSQ_CREDBP ) &&
	     (qry_tbl.mode != PSQ_RULE) && (qry_tbl.mode != PSQ_EVENT) )
	{
	    if (duve_banner( DUVE_IIQRYTEXT, 2, duve_cb) == DUVE_BAD) 
		return ( (DU_STATUS) DUVE_BAD);
...
DUVE_CB            *duve_cb;
{
	...
	*/
	     ...
	     (tree_tbl.treeid2 != tree.treeid.db_tre_low_time) )
	{
	    ...
	    */
	    ...
	    else if (tree.treemode == PSQ_RULE)
	    {
		testid = 6;
	...
	if ( (tree_tbl.treemode != PSQ_VIEW) && 
	     (tree_tbl.treemode != PSQ_PROT) &&
	     (tree_tbl.treemode != PSQ_0_CRT_LINK) &&
	     (tree_tbl.treemode != PSQ_REG_LINK) && 
	     (tree_tbl.treemode != PSQ_REG_IMPORT) &&
	     (tree_tbl.treemode != PSQ_RULE) &&
	     (tree_tbl.treemode != PSQ_EVENT) &&
	     (tree_tbl.treemode != PSQ_INTEG) )
	{
    ...
    */
	...
	EXEC SQL delete from iitree where 
	    ...
	    treeid2 = :id2;
	if (duve_cb->duvetree[dropcnt].treemode == PSQ_EVENT)
	{
	    /* Drop from iievent */
	    exec sql delete from iievent
	      where event_ibase = :tid and event_idx = :tidx;
    	}
	if (duve_cb->duvetree[dropcnt].treemode == PSQ_RULE)
	{
	    /* Drop from iirule */
       ...
       DUVE_CB		*duve_cb)
{
	...
	*/
	    ...
	    || obj_type 		!= prv_tbl.d_obj_type)
	{
		...
		** exist 
		...
		if (obj_type == PSQ_VIEW || obj_type == PSQ_CREDBP)
		{
	...
	*/
	if (   prv_tbl.d_obj_type == PSQ_PROT
	    || (prv_tbl.d_obj_type == PSQ_VIEW && sel_perms_on_view)
	    || (prv_tbl.d_obj_type == PSQ_CREDBP && perms_on_dbp)
	   )
	{
	...
	if (!satisfied)
	{
	    ...
	    */
	    if (prv_tbl.d_obj_type == PSQ_VIEW || prv_tbl.d_obj_type == PSQ_PROT)
	    {
	        /*
		...
		*/
	    ...
	    else if (prv_tbl.d_obj_type == PSQ_CREDBP)
	    { 
		DUVE_DROP_OBJ		*drop_obj;
		...
		*/
	    ...
	    else if (prv_tbl.d_obj_type == PSQ_CONS)
	    { 
	    	DU_STATUS	indep_offset;
		...
		*/
		     ...
		     indep++, indep_offset++)
		{
		    ...
		    ** were not PSQ_CONS, but things may change, so let's check)
		    */
...
**      27-Jan-2010 (hanal04) Bug 124948
...
**	    PSQ_VIEW - mark in duverel as dropped
**	    PSQ_PROT - delete tuple from iiprotect
**	    PSQ_RULE - delete tuple from iirule
**	    PSQ_INTEG - delete tuple from iiintegrity
**
**
** Inputs:
