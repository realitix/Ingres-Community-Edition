...
namespace Ingres.ProviderInternals
{
	...
	**	31-oct-11 (thoda04) Bug 125909
	**	    Primary key info not returned from Vectorwise.
	*/
			...
			loadDbCaps()
		{
			...
			*/
			...
			if ((value = dbCaps.getDbCap(DBMS_DBCAP_DBMS_TYPE)) != null)
			{
				...
				is_ingres = value.Equals(DBMS_TYPE_INGRES);
				//is_ingres is true if the DBMS is an "INGRES" server
				if (is_ingres == false)
				{
					// Test if DBMS_TYPE might be a variation
					// on "INGRES" like a "INGRES_VECTORWISE" server
					if (value.Length > DBMS_TYPE_INGRES.Length)
						is_ingres = (value.StartsWith(DBMS_TYPE_INGRES+"_")  ||
						             value.Contains("VECTORWISE"));
				}
			}
