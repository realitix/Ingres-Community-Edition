...
namespace Ingres.ProviderInternals
{
	...
	**	    Add support for running on 64-bit Windows.
	*/
		...
		*/
			...
			ConnectStringConfig nv)
		{
			...
			*/
			...
			// Locate the Ingres.Support.dll that holds our XASwitch support.
			// Build the full path to the Ingres.Support.dll Resource Manager.
			// On 64-bit systems, the MSDTC is always 64-bit process,
			// even if the application is running in a 32-bit WOW process 
			System.Reflection.Assembly assembly =
				System.Reflection.Assembly.GetExecutingAssembly();

			string dll = assembly.Location.Replace("Ingres.Client", "Ingres.Support");
			dll = dll.Replace(@"\GAC_MSIL\",
				is64BitOperatingSystem ? @"\GAC_64\" : @"\GAC_32\");
			// dll should now be something like
			//    C:\Windows\assembly\GAC_64\Ingres.Support\
			//        2.1.0.0__1234567890123456\Ingres.Support.dll
...
//						throw;
		...
		*/
		...
		static private bool is64BitOperatingSystem
		{
			get
			{
				// get the machine's (not process') x86 or AMD64 architecture
				...
				if (processArchitecture == null ||
					processArchitecture.StartsWith(
						"x86", StringComparison.InvariantCultureIgnoreCase))
					return false;
