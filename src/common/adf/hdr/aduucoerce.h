...
FUNC_EXTERN DB_STATUS adu_getconverter(char *converter, bool force);
...
FUNC_EXTERN DB_STATUS adu_map_check_validity(
			ADU_MAP_STATETABLE  *validitytable,
    			u_i4                *bval);
FUNC_EXTERN DB_STATUS adu_col_init(void);
FUNC_EXTERN DB_STATUS adu_map_init(void);
#define ADU_MAP_GET_UNITOCHAR(UniBase,unicode) \
	(UniBase)[((*unicode)&0xFFFF) >> 8][(*unicode)&0xFF];
