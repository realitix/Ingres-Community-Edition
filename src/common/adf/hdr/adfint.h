...
{
    char	    adi_opname[ADF_MAXNAME];  /* An operation name */
...
#define	    ADZ_NOFIIDX	    ((i4) -1)	/* Signifies that there are no	    */
    ...
    i4		    adi_opflag;		/* Flag (defines in adf.h) */
}   ADI_OPRATION;
...
**          Add adi_dtid_touni to identify the unicode equivalent type.
*/
...
typedef struct _ADI_DATATYPE
{
    ...
    DB_DT_ID	    adi_dtfamily;	/* type ID of family to which this 
					** type belongs */
    DB_DT_ID        adi_dtid_touni;     /* Target Datatype id when converting
                                        ** to unicode.
                                        */
...
#define		ADI_DBLBYTE	0x0008  /* server is running using double
					** byte charater set */
    ADU_MAP_INFO   *Adu_maptbl;		/* Mapping table for local 
					** to unicode */
...
FUNC_EXTERN DB_STATUS adu_managed_readmap(char *charset, ADU_MAP_INFO **map_p);
FUNC_EXTERN DB_STATUS adu_managed_deletemap(ADU_MAP_INFO **map_p);
