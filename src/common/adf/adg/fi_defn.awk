function rights(){
	copyright = "Copyright (c) 2007,2008,2009,2010 Actian Corporation"
}
#
#
#
...
#	    coercions.
#	11-may-2011 (dougi)
#	    Add "flags" parm for operator descriptions (for window funcs).
#	19-Jul-2011 (kiria01) b125564
#	    Complete last change initialising the trailing record and
#	    quietening the compiler.
#	20-Oct-2011 (kiria01) b125878
#	    Allow i1,i2,i4,f4 to be marked as explicit coercions too.
#	01-Nov-2011 (kiria01) b125852
#	    Allow ingresdate to be marked as an explicit coercion.
#
...
# Defaults
	...
	max_fi_no = -1;
	ign_cast_ops["ADI_F4_OP"] = 1
	ign_cast_ops["ADI_I1_OP"] = 1
	ign_cast_ops["ADI_I2_OP"] = 1
	ign_cast_ops["ADI_I4_OP"] = 1
	ign_cast_ops["ADI_INGRESDATE_OP"] = 1
	if (h_file != "")
	{
	    #
...
$1 == "alias" || $1 == "sqlalias" || $1 == "quelalias" {
	...
	if (op_file != "")
	{
		...
		systems",ADO_"cur_op_root"_CNT,ADZ_"cur_op_root"_FIIDX,"opflags"},">opfile
	}
	next
}
...
$1 == "result" {
	...
	if (expl_coercion) {
		...
		if (cur_op in ign_cast_ops) {
			;
		} else if (result in cast_ops) {
...
END {
	...
	if (op_file != "")
	{
	    print "{ {\"\"}, ADI_NOOP, -1, -1, 0, 0, 0, ADI_NOFI, 0}">opfile
