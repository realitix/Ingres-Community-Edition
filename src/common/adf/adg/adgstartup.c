...
**	    Interface change for managed mapping tables and collation data for
**	    sharing - add initialisation hooks for adu_col_init and adu_map_init.
**	    Interface change for adu_getconverter.
**/
...
**          Protype cleanup.
**      26-jul-2011 (horda03) B125758
**          Include conversion tables on VMS.
*/
...
i4		    c2secure)
{
    ...
    */
    ...
    {
        Adf_globs = scb;
	if (db_stat = adu_col_init())
	    return db_stat;
	if (db_stat = adu_map_init())
	    return db_stat;
    }
...
/*  scb->adf_owner = ?????  -- Don't set owner; ADF doesn't own this struct */
    ...
    /* ------------------------------------------------------ */
    if ((db_stat = adu_getconverter (converter, FALSE)) == E_DB_OK)
...
"Ad0_opinit: Invalid name <%t> for operator addition %d. (zero based)\n",
	    ...
	    MEcopy(no->fod_name, ADF_MAXNAME, op->adi_opname);
