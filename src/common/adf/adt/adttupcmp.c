...
**	05-May-2011 (kiria01) SIR 124690
**	    Minor problem fixed where UCS_BASIC could drop into adugcmp()
**	    if the database had user collation enabled!
*/
i4
adt_compare(
ADF_CB		    *adf_scb,
...
#define	ADT_GT	(i4)1
    ...
    *status = E_DB_OK;
	...
	/* It is possible that a UTF8 db has also got -lcollation set so
	** we need to check for this here. Otherwise, UCS_BASIC columns
	** will be passed to adugcmp() and it will compare using the chosen
	** collation and not UCS_BASIC! */
	if (adf_scb->adf_collation && atr->collID != DB_UCS_BASIC_COLL)
