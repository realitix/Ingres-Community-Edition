/*
** Copyright (c) 2001, 2008, 2010 Actian Corporation
*/
...
**	    Fixed a couple of nasty buffer overruns due to testing for buffer
**	    end with equality!
**      17-Jun-2012 (hanal04) SIR 126680
**          Add support for "LIKE WITH|WITHOUT DIACRITICAL". Ensure we don't
**          bolt on overflow characters to the pattern built if they
**          are diacriticals and we have "WITHOUT DIACRITICAL".
*/
       ...
       i4	opts)
{
    ...
    */
    ...
    if ( opts & LikeCE || sizece > (i4)sizeof(local_list) )
    {
...
**	28-Mar-2011 (kiria01) 125223
**	    Reduced the need for allocated vm for strings less than 1024 chars
**	    with NFC decomposition in normalisation. Combined the remaining vm
**	    requests for NFC to use 1 contiguous allocation.
**	    Also dropped the ZERO of the VM as not needed.
*/
DB_STATUS
	...
	DB_DATA_VALUE	*dst_dv)
{
    ...
    i4			nfc_dstlen = 0;
    i4			bytes_copied = 0;
    UCS2		_tmpbuf[ADF_UNI_TMPLEN_UCS2*2];
   ...
   /* If collation table is not initialize bail out now */
    ...
    /* output data is empty, so get max length from data-value */
    ...
    */
    ...
    /*
    ** FIXME: The following process would be better done a character
    ** at a time directly to the output buffer. At the moment it does
    ** the NFD expansion first and then if needed the NFC afterwards.
    ** In the worst case for NFC means that an intermediate store be
    ** taken of the full NFD form which could be upto about 4 times
    ** the input size. If done char for char, the only extra memory
    ** would be a small stack buffer to cope with the largest character
    ** expansion from decomposition.
    ** Similarly for adu_utf8_unorm which ought to have the substance
    ** of this routine expanded directly for UTF8 without intermediates
    ** of UCS2/UTF-16.
    */
    srcend = (u_i2 *)((char *)src + srclen);
	...
	if (cetable[*srcchar].flags & CE_HASCDECOMP)
	{
		...
		if ((adf_scb->adf_uninorm_flag == AD_UNINORM_NFC) &&
			...
			(i4)sizeof(UCS2) * 4)) > nfc_dstlen))
		{
		    nfc_dstlen += 1024;
		    if (nfc_dstlen < (i4)sizeof(_tmpbuf))
		    {
			tmpbuf = _tmpbuf;
			nfc_dstlen = sizeof(_tmpbuf);
		    }
		    else
		    {
			/* Result buffer is too small to use for
			** decomposition and so was stack temp. Allocate one. */		    			
			tmpbuf = (UCS2 *) MEreqmem (0, nfc_dstlen, FALSE, &db_stat);
		    }
    ...
    */
    if (adf_scb->adf_uninorm_flag == AD_UNINORM_NFC)
    {
      ...
      /* Allocate nfc conversion buffer & comblist working buffer */
      nfcbuf = (UCS2 *) MEreqmem (0, sizeof(UCS2) * (dstlen+inlen), FALSE, &db_stat);
      nfcbufptr = nfcbuf;
      comblist = nfcbuf + dstlen;
...
**      18-Jun-2010 (horda03) b123926
**          Swap order of dst_dv and src_dv in adu_long_unorm.
**	10-Mar-2011 (kiria01) m1721
**	    Account for pfx len properly or 0 length string constants
**	    fail with -string_truncation=fail set.
*/
DB_STATUS
	...
	DB_DATA_VALUE	*dst_dv)
{
    ...
    /* We will be making this an NVCHR so add pfx */
    templen += DB_CNTSIZE;
	...
	if (cetable[*sptr].flags & CE_HASMATCH)
	{
	    ...
	    */
		...
		for (cevalue = 0; 
		     cevalue < ceover->num_values; cevalue++)
		{
                    if(ceover->oce[cevalue].ce[0] != 0 ||
                        (pat_flags & AD_PAT_DIACRIT_OFF) == 0)
                    {
		        /* add to CE list */
		        for (i = 0; i < max_levels; i++)
		        {
...
no_match:
	if (cetable[*sptr].flags & CE_HASMATCH)
	{
	    if (cetable[*sptr].flags & CE_HASOVERFLOW)
	    {
                i4	i;
		ADU_CEOVERFLOW	*ceover = cetable[*sptr].overflow;
                for (i = 0; i < ceover->num_values; i++)
                {
                    if(ceover->oce[i].ce[0] != 0 ||
                       (pat_flags & AD_PAT_DIACRIT_OFF) == 0)
                    {
		        ce += max_levels;
                    }
