...
**	    Allow adu_long_to_cstr() to use a scalar input
**	    so that the long temporary could be avoided.
**	    Also fixed adu_rtree_cmp() error where the
**	    results were incorrect due to the presence of
**	    semi-colons following 'if's that made the if
**	    block unconditional!
**	20-Jul-2011 (kiria01) b125574
**	    Correct NULL handling. Uniformally, handle NULL value returns
**	    as though ADE_0CXI_IGNORE were active for all functions. This
**	    then avoids leaving stale NULL bits set in the output that would
**	    mess up later function calls using the same output data locations.
**	26-Jul-2011 (troal01) b125597
**	    Instead of checkign the COLUMN's datatype we should be checking the
**	    GEOS Geometry's datatype. Since a column can be of a generic GEOMETRY
**	    type this could've lead to cases where the geometry was actually a
**	    valid type yet it wouldn't be recognized.
*/
...
** Also support scalar forms directly as this will avoid needless text->lob->text
** conversions.
*/
static DB_STATUS
adu_long_to_cstr(
...
i4 *length)
{
	...
	{
	    /* We were passed a varying scalar - use it.
	    ** Memory is already allocated */
	    memcpy(result, (char*)dv_in->db_data + sizeof(i2), *length);
	    result[*length] = EOS;
	    *output = result;
	    return E_DB_OK;
	}
...
DB_DATA_VALUE   *rdv)
{
     ...
     * NOTE: This is how PostGIS handles it
     */
    if(GEOSGeomTypeId_r(handle, linestring) != GEOS_LINESTRING)
    {
        if (rdv->db_datatype >= 0)
	    return adu_error(adf_scb, E_AD1012_NULL_TO_NONNULL, 0);
        ADF_SETNULL_MACRO(rdv);
...
DB_DATA_VALUE   *rdv)
{
     ...
     * NOTE: This is how PostGIS handles it
     */
    type_id = GEOSGeomTypeId_r(handle, curve);

    if(type_id != GEOS_LINESTRING &&
            type_id != GEOS_MULTILINESTRING)
    {
        if (rdv->db_datatype >= 0)
	    return adu_error(adf_scb, E_AD1012_NULL_TO_NONNULL, 0);
        ADF_SETNULL_MACRO(rdv);
...
DB_DATA_VALUE   *rdv)
{
     ...
     */
    if(type_id == GEOS_MULTILINESTRING)
    {
...
DB_DATA_VALUE   *rdv)
{
    ...
    if(GEOSGeomTypeId_r(handle, curve) != GEOS_LINESTRING)
    {
        return (adu_error(adf_scb, E_AD5600_GEOSPATIAL_USER, 2, 0,
...
DB_DATA_VALUE    *rdv)
{
    ...
    type_id = GEOSGeomTypeId_r(handle, surface);
    if(type_id != GEOS_POLYGON &&
       type_id != GEOS_MULTIPOLYGON)
    {
        /*
...
DB_DATA_VALUE    *rdv)
{
    ...
    if(GEOSGeomTypeId_r(handle, polygon) != GEOS_POLYGON)
    {
        return (adu_error(adf_scb, E_AD5600_GEOSPATIAL_USER, 2, 0,
...
DB_DATA_VALUE    *rdv)
{
     ...
     * NOTE: This is how PostGIS handles it
     */
    type_id = GEOSGeomTypeId_r(handle, polygon);
...
DB_DATA_VALUE    *rdv)
{
     ...
     * NOTE: This is how PostGIS handles it
     */
    type_id = GEOSGeomTypeId_r(handle, geomcoll);
    if(type_id != GEOS_GEOMETRYCOLLECTION && type_id != GEOS_MULTIPOINT &&
    		type_id != GEOS_MULTIPOLYGON && type_id != GEOS_MULTILINESTRING)
    {
        if (rdv->db_datatype >= 0)
	    return adu_error(adf_scb, E_AD1012_NULL_TO_NONNULL, 0);
        ADF_SETNULL_MACRO(rdv);
...
DB_DATA_VALUE   *rdv)
{
     ...
     * NOTE: This is how PostGIS handles it
     */
    type_id = GEOSGeomTypeId_r(handle, geometry);
    if(type_id != GEOS_LINESTRING &&
       type_id != GEOS_MULTILINESTRING)
    {
        if (rdv->db_datatype >= 0)
