...
**                             the real work.
**          adu_ascii_3arg() - Support the three argument version of string
**                             functions. Similar to adu_ascii_2arg but with
**                             control over the truncation checks.
...

/*
** Name: adu_ascii_3arg() - Convert a data value to character string data value.
**
** Description:
**      Support the three argument version of string functions CHAR(), VARCHAR(),
**      TEXT() and C().  Simply call adu_ascii() to do the real work.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
...
**	    .db_length			Its length.
**	    .db_data			Ptr to the data to be converted into
**      dv2                             DB_DATA_VALUE describing length to set
**					result to.
**	    .db_datatype		Should be DB_INT_TYPE
**	    .db_length			Its size.
**	    .db_data			Ptr to the integer value
**      dv3                             DB_DATA_VALUE describing Ingres datatype
**					to be converted.
**	    .db_datatype		Should be DB_BOO_TYPE
**	    .db_data			Ptr to the data to be converted into
**					a character string.
**	rdv				DB_DATA_VALUE describing character
**					string result.
