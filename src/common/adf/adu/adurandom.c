...
**
**	    adu_srand(i)  - Set random seed
**	    adu_rand([i]) - Optionally set random seed, then return next
**				random float
...

/*
** Name: adu_srand()	- ADF function instance to set the random seed
**
** Description:
**      This function calls FPsrand & MHsrand2 to set the random seed.
**
** Inputs:
...
**      dv1                             DB_DATA_VALUE describing the integer
**					operand containg the seed.
**	    .db_datatype		
**	    .db_length			
**	    .db_data			Ptr to data.
**
...
**					which returns 0 after the call.
**	    .db_data			Ptr to the integer result.
**
**	Returns:
...
DB_DATA_VALUE   *rdv)
{
    PTR			data = dv1->db_data;
    i4			seed, slen, rlen;
    DB_STATUS		db_stat;

    /* Verify the parm & result are integers. */
    if (abs(dv1->db_datatype) != DB_INT_TYPE || rdv->db_datatype != DB_INT_TYPE)
    {
	...
	slen = dv1->db_length-1;
    else slen = dv1->db_length;

    /* Get the seed value. */
    switch (slen)
    {
	case 1:
	    seed = I1_CHECK_MACRO(*(i1 *)data);
	    break;
	case 2:
	    seed = *(i2 *)data;
	    break;
	...
	rlen = rdv->db_length-1;
    else rlen = rdv->db_length;
    data = rdv->db_data;
...

/*
** Name: adu_srand_randomf()	- ADF function instance to set the random seed
**	AND subsequently do a "randomf()
**
** Description:
**      This function calls FPsrand & MHsrand2 to set the random seed then
...
**
** Outputs:
**	rdv				DB_DATA_VALUE describing the float
**					which returns the random number.
**	    .db_data			Ptr to the float result.
**
**	Returns:
**	    E_AD0000_OK			Completed successfully.
**          E_AD9999_INTERNAL_ERROR	If the data types are unexpected
...
DB_DATA_VALUE   *rdv)
{
    ...
    /* Verify the parm is an integer & result is float. */
    if (abs(dv1->db_datatype) != DB_INT_TYPE || rdv->db_datatype != DB_FLT_TYPE)
    {
       db_stat = adu_error(adf_scb, E_AD9999_INTERNAL_ERROR, 0);
       return(db_stat);
    }
    ...
    /* Compute and return the next random value. */
    d = FPrand();
