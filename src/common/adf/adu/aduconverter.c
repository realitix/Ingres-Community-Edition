...
**			chars in size.
**	force -		If true, use the passed value instead of reading
**			II_CHARSET
** Output:
...
**	    Interface change for adu_getconverter to allow for forcing
**	    the charset choice to that specified and the caching of the
**	    alias map data so that it is not continually re-read.
**      11-Oct-2011 (hanal04) Bug 125813
**          Hosted systems may host multiple installations with different
**          II_CHARSET settings. It is therefore not safe to use the
**          hosts locale to establish which unimap file to use. Use the
**          The II_CHARSET value and only fall back on the locale if we still
**          do not have a match. Note that the CM_LOC_MAP Localemap in
**          cmgetcp.c is not a complete list. For example, there is no way
**          to pick up ca-cp943c-shiftjis-jp-2004 from locale settings.
**          If full client transliteration is implemented then it would
**          make sense to once again use the locale ahead of the II_CHARSET
**          to establish the unimap file used. If this is done the
**          Localemap[] will need to be updated to include missing enties
**          like those required for shiftjis.
*/
STATUS
adu_getconverter(
    char	*converter, bool force)
{
    STATUS		stat;
    char		*tptr;
    char		*env = 0;
    ...
    static ADU_ALIAS_MAPPING   *aliasmapping;
    static ADU_ALIAS_DATA	*aliasdata;	
    static char 		*datasize;
    ...
    if (force)
	/* character set we want to set is being passed in */
       strcpy(pcs, converter); 
    else
    {
	...
	stat = CM_getcharset(pcs); /* platform character set */
    }
    if (!aliasmapping || !aliasdata || !datasize)
    {
    ...
    if(!force)
    {
        ...
        CMget_charset_name(chset);
        if (STcasecmp(chset, "UTF8") != 0)
        { 
            /* search maptable for env  */ 
            adu_csnormalize (chset, STlength(chset), norm_pcs);
