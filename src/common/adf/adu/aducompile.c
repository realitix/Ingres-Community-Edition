...
**	    Added wordbreakproperty.txt inclusion for support of titlecase.
**	    This aspect is using the 6.0.0 datafile naming.
**	    Added SIclose calls for tidiness.
*/
static	ADUUCETAB *
makeutable(
	   char		*desfile)
{
    ...
    i4			num_rvals = 0;
    ADUUCETAB		*epointer = &w.utab; /* entries first */
    /* then instructions */    
    char		*ipointer; 
    SIZE_TYPE		*varsize;
    char		*tptr;
...
/* Reset the table pointers */
    ...
    /**********************************************************/
    /*
    ** open wordbreakproperty.txt file.
    ** <code>;	       <class> # <category>	  <comment>
    ** <code>..<code>; <class> # <category> [<n>] <comment>
    */
    ...
    LOfaddpath(&loc, "lp32", &loc);
#endif /* hybrid */
    LOfstfile("wordbreakproperty.txt", &loc);
    if (SIopen(&loc, "r", &dfp) != OK)
    {
	SIprintf("aducompile: Unable to open wordbreakproperty.txt file \"%s\".\n", loc.string );
        return NULL;
    }
...
values must be semi-colon separated, ABORTING\n", lno);
	...
	*separator++ = EOS;
	while(*separator && CMspace(separator))
	    separator++;
	if (!STcasecmp(separator, "ALetter")) flagval = CE_ALetter;
	else if (!STcasecmp(separator, "CR")) /*SKIP*/;
	else if (!STcasecmp(separator, "Extend")) flagval = CE_Extend;
	else if (!STcasecmp(separator, "ExtendNumLet")) flagval = CE_ExtendNumLet;
	else if (!STcasecmp(separator, "Format")) flagval = CE_Format;
	else if (!STcasecmp(separator, "Katakana")) flagval = CE_Katakana;
	else if (!STcasecmp(separator, "LF")) /*SKIP*/;
	else if (!STcasecmp(separator, "MidLetter")) flagval = CE_MidLetter;
	else if (!STcasecmp(separator, "MidNum")) flagval = CE_MidNum;
	else if (!STcasecmp(separator, "MidNumLet")) flagval = CE_MidNumLet;
	else if (!STcasecmp(separator, "Newline")) flagval = CE_Newline;
	else if (!STcasecmp(separator, "Numeric")) flagval = CE_Numeric;
	else
	{
	    SIprintf("Syntax error on character database line %d, wordbreak class "
		    "is invalid (%s), ABORTING\n", lno, separator);
	    tab = NULL;
	    break;
	}
	STtrmwhite(buf);
	...
	else
	{
	    *separator++ = EOS;
		...
		break;
    	    ...
    	    if (cval > 0xFFFF || cval2 > 0xFFFF)
		continue;

	}
	/* Set the flag */
	while (cval <= cval2)
  	    epointer[cval++].flags |= flagval;
    }
    SIclose(dfp);
