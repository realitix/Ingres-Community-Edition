...
**
**        This routine will also accept an exponential float string as
**      input so that copy allows the same coercion as an insert. The input
**      string format below is checked if we have determined that we do not
**      have a valid money string:
**
...
**       7-Mar-2011 (hanal04) Bug 125119
**          Updated to allow exponential floats to be copied into money
**          columns.
*/
		...
		CMtolower(currency_mask, &ctmp2);
		    ...
		    CMnext(p);
                ...
                else
                {
                    if(got_number)
                    {
                        /* Last resort, check for exponent case 123.456E-2 */
                        p = pexp; /* reset p to 1st character found */
                        l = lexp; /* reset l to length from 1st char found */

                        /* Check case insensitive for 'e-' or 'e+' */
                        CMtolower(p, &ctmp1);
		        CMbytedec(l, p);
                        CMnext(p);
                        if(ctmp1 == 'e' && ((*p == '-') || (*p == '+')))
		            ...
		            CMbytedec(l, p);
                            ...
                            /* Copy remaining characters to see if we have
                            ** a number.
                            */
                            while(l)
                            {
                            ...
                            *ptemp = EOS;
                            ...
                            if((CVal((PTR)&temp, &exponent) == OK) &&
                               (inlen <= DB_MAXSTRING))
                            {
                                /* We have an exponential float string.
                                ** copy it to a null terminated buffer so
                                ** we can convert to float and then into money
                                */
                                ptemp = temp;
                                memcpy(ptemp, start, inlen);
                                *(ptemp + inlen) = EOS;

                                if (CVaf(ptemp, decimal, &double_number) == OK)
                                {
                                    DB_DATA_VALUE	num_dv;

                                    num_dv.db_data = (PTR)&double_number;
                                    num_dv.db_datatype = DB_FLT_TYPE;
                                    num_dv.db_length = sizeof(double_number);
                                    num_dv.db_prec = 0;
                                    num_dv.db_collID = -1;
 
                                    return(adu_numtomny(adf_scb,
                                                          &num_dv,
                                                          mny_dv));
                                }
                            }
