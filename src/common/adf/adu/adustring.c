/*
** Copyright (c) 1986, 2008, 2009, 2010, 2011 Actian Corporation
*/
...
**	adu_20osubstr	--  Substr ( char, int )
**	adu_21osubstrlen --  Substr ( char, int, int )
...
**	    Added missing init for .pop_user_arg
**	09-Aug-2011 (kiria01) b125634
**	    Added adu_20osubstr & adu_21osubstrlen to support
...
register DB_DATA_VALUE	*rdv)
{
	...
	switch (dv1->db_datatype)
	{
	  ...
	  case DB_LVCH_TYPE:
	  {
		...
		/* otherwise if non-UTF8, drop down to DB_LBYTE_TYPE etc. case. */ 
	  }
	  case DB_LNVCHR_TYPE:
	  case DB_LBYTE_TYPE:
	  case DB_GEOM_TYPE:
...
DB_DATA_VALUE     *rdv)
{
    ...
    if (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
      return (ad0_utf8_casetranslate(adf_scb, dv1, rdv, ADU_CASEMAP_LOWER));
...
**	    Work with i8.
**	09-Aug-2011 (kiria01) b125634
**	    Change call to adu_strextract to reflect change in start position
**	    support.
*/
...
DB_DATA_VALUE          *rdv)
{
    ...
    db_stat = adu_strextract(adf_scb, dv1, -len, len, rdv);
...
**	    Added missing DB_N[V]CHR_TYPE.
**	23-Sep-2011 (kiria01) b125619
**	    Correct byte/vbyte handling to pass the binary data when it can
**	    with no translation.
*/
...
DB_DATA_VALUE	    *rdv)
{
    ...
    switch (dv1->db_datatype)
    {
      ...
      case DB_TXT_TYPE:
      case DB_LTXT_TYPE:
	dv1_len = ((DB_TEXT_STRING*)dv1_str_addr)->db_t_count;
	dv1_str_addr = ((DB_TEXT_STRING*)dv1_str_addr)->db_t_text;
	break;

      case DB_NVCHR_TYPE:
      case DB_PAT_TYPE:
	dv1_len = ((DB_NVCHR_STRING*)dv1_str_addr)->count * sizeof(UCS2);
	dv1_str_addr = (PTR)((DB_NVCHR_STRING*)dv1_str_addr)->element_array;
	break;

      case DB_VBIT_TYPE:
      {	i4 bits;
	dv1_len = ((DB_BIT_STRING*)dv1_str_addr)->db_b_count;
	dv1_str_addr = ((DB_BIT_STRING*)dv1_str_addr)->db_b_bits;
	if (bits = (dv1_len & (BITSPERBYTE-1)))
	{
	    dv1_str_addr[dv1_len/BITSPERBYTE] &= ((1<<bits)-1);
	    dv1_len = dv1_len/BITSPERBYTE+1;
	}
	else
	    dv1_len /= BITSPERBYTE;
	break;
...
**      First written.
**  28-Jul-2011 (kiria01) b125530
**	Added missing truncation support.
*/
DB_STATUS
adu_3alltobyte(
ADF_CB		    *adf_scb,
...
DB_DATA_VALUE	    *rdv)
{
    ...
    */
    ...
    /* Set it */
    if (rdv2.db_length > r4len)
	rdv2.db_length = r4len;

    /* Ignore string truncations */
    adf_scb->adf_strtrunc_opt = ADF_IGN_STRTRUNC;

    /* Now call the worker routine */
    status = adu_2alltobyte(adf_scb, dv1, &rdv2);

    if (rdv2.db_length != rdv->db_length)
	memset(rdv2.db_data+rdv2.db_length, 0, rdv->db_length-r4len);
...

/*{
** Name: adu_3alltobyte3() - Any type to byte/varbyte conversion
**
**  Description:
**	A wrapper routine to limit the result lenght of the adu_2alltobyte() above
**      is used to capture the 'all' version of the Byte(expr,len) function call.
...
**      dv2                             Ptr to DB_INT_VALUE containing desired
**					length of the output data.
**	dv3				Ptr to DB_DATA_VALUE of BOOL parameter.
**					Set true means raise error on truncation
**					otherwise just warn.
** Outputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.  If an
...
DB_DATA_VALUE     *rdv)
{
    ...
    if (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
      return (ad0_utf8_casetranslate(adf_scb, dv1, rdv, ADU_CASEMAP_UPPER));
...
DB_DATA_VALUE	    *rdv)
{
    ...
    /* ***** FIXME this is silly, better would be uld_struct_name() and
    ** CVupper, except that uld_struct_name is buried in a backend
    ** directory!  not worth dealing with at the moment
    */
    switch (relspec)
    {
      case 3:
...
**	    Set the dn_status flags correctly reflecting the
**	    date portion!
*/
...
DB_DATA_VALUE     *rdv)
{
    ...
    d.dn_status  = AD_DN_ABSOLUTE | AD_DN_TIMESPEC |
		AD_DN_YEARSPEC | AD_DN_MONTHSPEC | AD_DN_DAYSPEC; 
...
**	    Add missing init of db_collID
**	09-Aug-2011 (kiria01) b125634
**	    Correct the long->non-long output variables adjusting for
**	    lengths that would artificially truncate long_nvarchar.
*/
DB_STATUS
adu_lvch_move(
ADF_CB             *adf_scb,
...
DB_DATA_VALUE	   *dv_out)
{
	...
	else if ((outbits & AD_PERIPHERAL) == 0)
	{
		...
		case DB_NVCHR_TYPE:
    		...
    		case DB_BYTE_TYPE:
		    res_dv->db_length = dv_out->db_length;
		    if (adf_scb->adf_uninorm_flag == AD_UNINORM_NFD)
			res_dv->db_length *= 4 * sizeof(UCS2);
		    else
	    		res_dv->db_length *= sizeof(UCS2);
		    if (res_dv->db_length > DB_GW4_MAXSTRING)
			res_dv->db_length = DB_GW4_MAXSTRING;
		    res_dv->db_length += DB_CNTSIZE;
		    break;
...
**	16-February-2011 (frima01) Bug 125050
**	    Removing optimization for a64.win to avoid stack corruption.
**
*/
#if defined (a64_win)
#pragma optimize("",off)
...

/*
** Name: adu_20osubstr() - Return the substring from a string. Oracle variant
**
** Description:
**	This file contains an implementation of 
**	SUBSTR(char, int).  It is based on the Oracle implemention which is
**	similar to ANSI SUBSTRING except that -ve start-position is counted
**	from the right.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
...
**  History:
**	09-Aug-2011 (kiria01) b125634
**	    Created from adu_20substr for substr(s,p) support
*/
...
DB_DATA_VALUE	*rdv)
{
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect,ADF_000_STRFI_ENTRY,&dum1,&dum2))
    {
        TRdisplay("adu_20osubstr: enter with args:\n");
        adu_prdv(dv1);
        adu_prdv(dv2);
        adu_prdv(rdv);
    }
	...
	break;
      default:
	return (adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 2, 0, "osubstr start length"));
    }
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_001_STRFI_EXIT,&dum1,&dum2))
    {
        TRdisplay("adu_20osubstr: returning DB_DATA_VALUE:\n");
        adu_prdv(rdv);
    }
#endif
...

/*
** Name: adu_21osubstrlen() - Return the substr from a string. Oracle varient
**
** Description:
**	This file contains an implementation of the Oracle scalar function
**	SUBSTR(char, int, int).  It is similar to ANSI except that -ve start
**	counts from the right and if length is -ve, the result is a NULL.
**	NOTE: Unlike ANSI SUBSTRING, this function needs to do its own NULL
**	handling.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
...
**  History:
**	09-Aug-2011 (kiria01) b125634
**	    Created from adu_21substrlen for substr(s,p,l) support
*/
...
DB_DATA_VALUE	*rdv)
{
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect,ADF_000_STRFI_ENTRY,&dum1,&dum2))
    {
        TRdisplay("adu_21osubstrlen: enter with args:\n");
        adu_prdv(dv1);
        adu_prdv(dv2);
        adu_prdv(dv3);
        adu_prdv(rdv);
	...
	case 8:
	    ...
	    break;
	default:
	    return (adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 2, 0, "osubstrlen start length"));
    }
	...
	case 8:
	    ...
	    break;
	default:
	    return (adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 2, 0, "osubstrlen for length"));
    }
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_001_STRFI_EXIT,&dum1,&dum2))
    {
        TRdisplay("adu_21osubstrlen: returning DB_DATA_VALUE:\n");
        adu_prdv(rdv);
    }
#endif
...
**	09-Aug-2011 (kiria01) b125634
**	    Interface tweek to align with negative start position handling
**	    of SUBSTR.
*/
...
DB_DATA_VALUE	*rdv)
{
    ...
    **  RIGHT(s, l)         -> STREXTRACT(s, -l, l)
    **  SUBSTRING(s, p)     -> STREXTRACT(s, p, LEN(s)-p+1)	[p>0]
    **  SUBSTRING(s, p, l)  -> STREXTRACT(s, p, l)		[p>0]
    **  CHAREXTRACT(s, p)   -> STREXTRACT(s, p, 1)
    **	SUBSTR(s, p)	    -> STREXTRACT(s, p, LEN(s)-abs(p)+1)
    **	SUBSTR(s, p, l)	    -> STREXTRACT(s, p, l)
    */
	...
	while (forlen-- > 0 &&  src < endsrc && dest + CMbytecnt(src) <= enddest)
	   CMcpyinc(src, dest);
    }
	...
	if (forlen > 0)
	{
	    memcpy(dest0, src + startpos, forlen);
	    dest += forlen;
	}
    }
...

/*
** Name: adu_varbyte_3arg() - Convert a data value to byte data value.
**
** Description:
**      Support the two argument version of string functions VARBYTE
**      Simply call adu_2alltobyte to do the real work.
...
**  History:
**     28-Jul-2011 (kiria01) b125530 
**         Created to provide control over string truncation
*/
DB_STATUS
adu_varbyte_3arg(
ADF_CB			*adf_scb,
...
**					ADU_CASEMAP_UPPER or
**					ADU_CASEMAP_TITLE or
**					ADU_CASEMAP_LOWER
** Outputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.  If an
...
i4		  flag)
{
    ...
    else if (flag == ADU_CASEMAP_LOWER)
    {
    ...
    else if (flag == ADU_CASEMAP_TITLE)
    {
      if ((db_stat = adu_nvchr_totitle(adf_scb, &temp_dv1, &temp_dv2)) != E_DB_OK)
      {
        if (memexpanded)
	{
	  MEfree (temp_ptr1);
...

/*
** Name: adu_str_initcap() - Convert all words to init cap 
**
** Description:
**	This routine converts all the words to have initial caps. Any
**	non-alphanumeric is deemed a separator.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.
...
DB_DATA_VALUE     *rdv)
{
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect,ADF_000_STRFI_ENTRY,&dum1,&dum2))
    {
        TRdisplay("adu_str_initcap: enter with args:\n");
        adu_prdv(dv1);
        adu_prdv(rdv);
    }
#endif
    ...
    if (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
      return (ad0_utf8_casetranslate(adf_scb, dv1, rdv, ADU_CASEMAP_TITLE));
	...
	if (initc)
	{
	    if (CMlower(p))
	    {
		/* Initial is wrong case - upper */
		CMtoupper(p, p);
		initc = FALSE;
	    }
	    else if (CMupper(p) || CMdigit(p))
	    {
		/* Initial is already upper or number */
		initc = FALSE;
	    }
	}
	else if(CMupper(p))
	{
	    /* Non-initial cap - drop case */
	    CMtolower(p, p);
	}
	else if (!CMlower(p) && !CMdigit(p))
	{
	    /* Potentially upcase next letter */
	    initc = TRUE;
	}
	CMnext(p);
    }
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_001_STRFI_EXIT,&dum1,&dum2))
    {
        TRdisplay("adu_str_initcap: returning DB_DATA_VALUE:\n");
        adu_prdv(rdv);
    }
#endif
...

/*
** Name: adu_str_repeat() - Repeat passed string 'n' times
**
** Description:
**	This routine concatenates 'n' copies of the source text.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.
...
**	    .db_length			Its length.
**	    .db_data			Ptr to string to be lowercased.
**      dv2                             Ptr to DB_DATA_VALUE source.
**	    .db_datatype		Its type.
**	    .db_length			Its length.
**	    .db_data			Ptr to string to be lowercased.
**	rdv				Ptr to destination DB_DATA_VALUE.
...
DB_DATA_VALUE     *rdv)
{
    DB_STATUS	db_stat;
    i4		i, rept, size1;
    char	*p1, *rp, *psave, *end;
#ifdef xDEBUG
    if (ult_check_macro(&Adf_globs->Adf_trvect,ADF_000_STRFI_ENTRY,&dum1,&dum2))
    {
        TRdisplay("adu_str_repeat: enter with args:\n");
        adu_prdv(dv1);
        adu_prdv(dv2);
        adu_prdv(rdv);
    }
    ...
    {
      case 1:
	rept = I1_CHECK_MACRO(*(i1 *) dv1->db_data);
	break;
      case 2:
	rept = *((i2 *)dv2->db_data);
	break;
    ...
    rp = psave;
    end = rp + rdv->db_length - sizeof(i2);
    if (rdv->db_datatype < 0)
	end--;

    for (i = 0; i < rept && rp < end; i++)
    {
	if (end - rp < size1)
	    size1 = end - rp;
	memcpy(rp, p1, size1);
    ...
    if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_001_STRFI_EXIT,&dum1,&dum2))
    {
        TRdisplay("adu_str_repeat: returning DB_DATA_VALUE:\n");
        adu_prdv(rdv);
    }
#endif
