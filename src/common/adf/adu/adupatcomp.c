/*
** Copyright (c) 2008, 2010 Actian Corporation
**
*/
	...
	const UCS2	*end)
{
	...
	*lit = NULL;
	...
	if (db_stat = adu_pat_MakeCElen(ctx->adf_scb, l, end, &len,
                                        (u_i4)ctx->patdata->patdata.flags))
	    return db_stat;
...
#undef _DEFINE
	...
	*p++ = ',';
	...
	{
	    STprintf(p, ",RSVD_UNUSED=%x", patdata->patdata.flags & AD_PAT2_RSVD_UNUSED);
	    p += STlength(p);
	}
	*p++ = '\n';
	*p++ = EOS;
...
**          Initial creation.
**      22-apr-2011 (huazh01)
**          MEfill 'pat_set_stk' to zero before using it. (b125296)
*/
	...
	AD_PAT_SEA_CTX	*sea_ctx)
{
    ...
    */
	...
	if (handle_set)
	{
	    ...
	    if (!*setbuf)
	    {
                MEfill(sizeof(PAT_SET), (u_char) 0, (PTR)&pat_set_stk);
...
ch_common:	    /* If ANY set character is non-ASCII then we
		    ...
		    */
			...
			u_i4 ce_len = 0;
			UCS2 *cep = ce;
			if ((db_stat = adu_pat_MakeCElen(adf_scb, p, p+1, &ce_len, pat_flags)))
			{
			    if ((*setbuf)->lim == PAT_SET_STK_DIM)
				sea_ctx->setbuf = NULL;
			    return db_stat;
