...
**	    Added adu_13lvch_osubstr & adu_14lvch_osubstrlen to support
**	    SUBSTR(long s,p[,l]) correctly.
**	    Corrected the behaviour of SUBSTRING(long s FROM p FOR l) when p
**	    is -ve.
**/

/*{
...
**	    cpn for adu_redeem.
**	09-Aug-2011 (kiria01) b125634
**	    Change interpretation of negative start position & its impact on
**	    length.
*/
DB_STATUS
		...
		DB_DATA_VALUE *dv_out)
{
	...
	{
	    case 1:
		startpos =  *(i1 *) dv_pos->db_data;
		break;
	    case 2:
		startpos =  *(i2 *) dv_pos->db_data;
		break;
	    case 4:
		startpos =  *(i4 *) dv_pos->db_data;
		break;
	    case 8:
	    {
	...
	else switch (dv_forlen->db_length)
	{
	    case 1:
		forlen = *(i1 *) dv_forlen->db_data;
		break;
	    case 2:
		forlen = *(i2 *) dv_forlen->db_data;
		break;
	    case 4:
		forlen = *(i4 *) dv_forlen->db_data;
		break;
	    case 8:
	    {
		i8 i8temp  = *(i8 *)dv_forlen->db_data;
	...
	*/
	if (forlen < 0)
	    return (adu_error(scb, E_AD2093_BAD_LEN_FOR_SUBSTR, 0));
	...
	if ((inp_cpn->per_length0 != 0) || (inp_cpn->per_length1 != 0))
	{
	    ...
	    seek_offset = 0;
	    seek_segno = 0;
	    if (!dv_forlen || inp_cpn->per_length1 > forlen)
	    {
		/* Try to skip reading segments we don't need  */
	    ...
	    if (work->adw_adc.adc_longs[ADW_L_SUBSTR_POS] < 0 ||
			forlen == 0)	/* Curtail seek if result has 0 length */
		work->adw_adc.adc_longs[ADW_L_SUBSTR_POS] = 0;

	    work->adw_shared.shd_l1_check = 0;
	    work->adw_shared.shd_l0_check = 0;

	    work->adw_adc.adc_longs[ADW_L_SUBSTR_CNT] = forlen;
	    work->adw_adc.adc_longs[ADW_L_CPNBYTES] = 0;
...
/*
** Name: adu_13lvch_osubstr() - Return the substring from a string.
**
** Description:
**	This file contains an implementation of  the Oracle scalar function
**	SUBSTR(char, int).  It is based on the Oracle varient of ANSI
...
** History:
**	09-Aug-2011 (kiria01) b125634
**	    Created from adu_13lvch_substr for substr(s,p) support
**
*/
DB_STATUS
adu_13lvch_osubstr(
	...
	DB_DATA_VALUE	*dv_out)
{
    return adu_14lvch_osubstrlen(scb, dv_in, dv_pos, NULL, dv_work, dv_out);
}

/*
** Name: adu_14lvch_osubstrlen() - Return the substring from a string. Oracle varient
**
** Description:
**	This file contains an implementation of  the Oracle scalar function
**	SUBSTR(char, int, int).  It is based on the Oracle varient of
**	SUBSTRING(char FROM int FOR int) in ISO/IEC FDIS 9075-2:1999
**
**	NOTE: Unlike SUBSTRING, SUBSTR with a negative length must return NULL.
**
** Inputs:
**      scb                              ADF session control block.
**      dv_in                            Ptr to DB_DATA_VALUE describing
...
** History:
**	09-Aug-2011 (kiria01) b125634
**	    Created from adu_14lvch_substrlen for substr(long s,p,l) support
*/
DB_STATUS
adu_14lvch_osubstrlen(
	ADF_CB		*scb,
	DB_DATA_VALUE	*dv_in,
	DB_DATA_VALUE	*dv_pos,
	DB_DATA_VALUE	*dv_forlen,
	...
	DB_DATA_VALUE	*rdv)
{
    DB_STATUS		status = E_DB_OK;
    ADP_PERIPHERAL	*inp_cpn = (ADP_PERIPHERAL *) dv_in->db_data;
    ADP_PERIPHERAL	*out_cpn = (ADP_PERIPHERAL *) rdv->db_data;
    ADP_LO_WKSP		*work = (ADP_LO_WKSP *) dv_work->db_data;
    i4			startpos;
    i4			seek_offset;
    i4			seek_segno;
    ...
    i4			forlen;
    ADP_POP_CB		*inpop;
    DB_DATA_VALUE	tdv_in, tdv_pos, tdv_forlen, tdv_work, trdv;
	...
	tdv_in = *dv_in;
	dv_in = &tdv_in;
	dv_in->db_datatype = -dv_in->db_datatype;
	dv_in->db_length--;
    }
    if (dv_pos->db_datatype < 0)
    {
	if (ADI_ISNULL_MACRO(dv_pos))
	{
	    ADF_SETNULL_MACRO(rdv);
	    return E_DB_OK;
	}
	tdv_pos = *dv_pos;
	dv_pos = &tdv_pos;
	dv_pos->db_datatype = -dv_pos->db_datatype;
	dv_pos->db_length--;
    }
    if (dv_forlen && dv_forlen->db_datatype < 0)
    {
	if (ADI_ISNULL_MACRO(dv_forlen))
	{
	    ADF_SETNULL_MACRO(rdv);
	    return E_DB_OK;
	}
	tdv_forlen = *dv_forlen;
	dv_forlen = &tdv_forlen;
	dv_forlen->db_datatype = -dv_forlen->db_datatype;
	dv_forlen->db_length--;
    }
    for (;;)
    {
	...
	{
	    return adu_error(scb, E_AD1012_NULL_TO_NONNULL, 0);
	}
	...
	if ((inp_cpn->per_length0 != 0) || (inp_cpn->per_length1 != 0))
	{
	    ...
	    /* Unlike SUBSTRING, SUBSTR with a negative offset is measured from end
	    ** of stream. */
	    if (startpos < 0)
	    {
		DB_DATA_VALUE count_dv;
		i4 chlen = 0;
		count_dv.db_data = (PTR)&chlen;
		count_dv.db_length = sizeof(chlen);
		count_dv.db_datatype = DB_INT_TYPE;
		count_dv.db_prec = 0;
		count_dv.db_collID = DB_NOCOLLATION;
		if (status = adu_7strlength(scb, dv_in, &count_dv))
		    break;
		/* We now have the character length & can adjust start
		** position accordingly */
	    ...
	    /* Now, need to ignore all char's before that */
	    work->adw_adc.adc_longs[ADW_L_SUBSTR_POS] = startpos;
	    ...
	    {
		/* Try to skip reading segments we don't need  */
		status = adu_opz_skip(scb, dv_in, work, startpos, 
				&seek_offset, &seek_segno);
		if (seek_offset)
		{
		    work->adw_adc.adc_longs[ADW_L_SUBSTR_POS] -= seek_offset;
...
**      04-aug-2006 (stial01)
**         Created.
**      02-jun-2011 (maspa05) b123897
**         made non-static so visible to adu_sc930prtdataval
*/
DB_STATUS
