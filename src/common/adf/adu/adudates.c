/*
** Copyright (c) 1986, 2001, 2008, 2009, 2011 Actian Corporation
*/
...
**	    Protype cleanup.
**      5-Jun-2010 (martinb) SIR: 125220
**          Add ISO4T, ISO4TC
**          Outputs.
**              ISO4TC displays yyyymmddThh:mm:ss.
**              ISO4T  displays yyyymmddThhmmss.
**          Inputs:
**              ISO4TC accepts same inputs as ISO4 with the addition that time
**                     components may be seprated from date components with a
**                     'T'.
**                     ie. both yyyymmddThh:mm:ss and yyyymmdd hh:mm:ss are
**                     acceptable.
**
**                     Furthermore, if the 'T' is specified then the time
**                     component may be specified as either the standard 
**                     hh:mm:ss or hhmmss.
**              ISO4T  as per iso4TC 
**	31-Aug-2011 (kiria01) b125709
**	    Add millisecond() support for extraction and truncation
**	10-Nov-2011 (kiria01) b125945
**	    Pulled out AD_NEWDTNTRNL related parts into internal
**	    header. Localized and simplified the TZ handling.
*/

...
static DB_DATA_VALUE millisecond_spec = {"millisecond", 11, DB_CHA_TYPE, 0, 0};
...
/*
** The index to Adu_dformat_prefix is the date type (DB_ISO4_DFMT etc) as 
** defined in common|hdr|hdr|iicommon.h
*/
static const char Adu_dformat_prefix[] =
{
	...
	'X',		/* ANSI style */
	'I',		/* ISO4T, same as ISO for input */
	'I'		/* ISO4TC, same as ISO for input */
};
...
**					converted.
**	date_dv				DB_DATA_VALUE describing the output dv.
**	    .db_datatype		Its datatype. This should be one of the
**					datetime related datatypes OR, if update_dv
**					is set, could be DB_ALL_TYPE.
**	    .db_length			Its length.
**	    .db_data			A pointer to the 
**	strict				If TRUE, limits parsing to match output
**					datatype otherwise, allows relaxed parse.
**					If output dt is ALL, this will restrict
**					the parse toi ANSI forms to allow DATE()
**					on literals to produce the implied DT.
...
**          Add 'DD MMM YYYY' as a valid date input format. Note that 
**          only the first two spaces should be treated like delimiters
**          and a pre-scan is required to ensure the first two spaces
**          of other date/time inputs are NOT treated as delimiters.
**	28-Jun-2011 (kiria01)
**	    Fix compiler warning about CMdigit(p++)
**	19-Jul-2011 (kiria01) b125564
**	    Correct end of loop checks to avoid reading past end of buffer.
**       4-Oct-2011 (frima01) Bug 125810
**          Prevent buffer overrun caused by change for Bug 125397.
**	17-Oct-2011 (kiria01) b125852
**	    Allow an input type of DB_ALL_TYPE to be used with update_dv.
**	    This allows the datatype to be completely set by inspection
**	    and not just variation in classes and precision.
**	03-Nov-2011 (kiria01) b125919, m2836
**	    In untyped literal (wild) ANSI mode, treat INGRES intervals as
**	    the INTDS or INTYM equivalents.
*/
...
#define T_DIGITS	2
    ...
    ** 
    ** perform a quick pre-parse to see if 'dd mmm yyyy' inputs with
    ** space as a delimiter is present. If so treat the first two space
    ** as delimiters in the main parse loop.
    */
    ...
    while (p < dbuf_end)
    {
        ...
        else if(sp_stage == A_MONTH)
        {
            if(!CMalpha(p) || (mon_chs > 2))
                break;
            else
            {
                ch = *p;
                ...
                if(p < dbuf_end && *p == ' ')
                {
                    ...
                    month[mon_chs] = EOS;
                    d = adu_monthname_lookup(month,mon_chs);
                    if (d == NULL)
                        break;
	...
	ch = *p;
        ...
        {
        case 'T':
            /* This can be used to delimit ISO4T and ISO4TC date and time 
            ** fields. If one of those date formats is in effect then we'll 
            ** use the 'T' in the timespec.
            */
            if(p < dbuf_end - 1)
            {
                chn = *(p + 1);
            ...
            if (((chn >= '0' && chn <= '9')) &&
                (adf_scb->adf_dfmt == DB_ISO4T_DFMT
             || adf_scb->adf_dfmt == DB_ISO4TC_DFMT))
            {
	        if (timespec == NULL)
		    timespec = datespec;
	        time = TRUE; /* There must be a following time field */
	        isempty = FALSE;
	        expect_month = FALSE;
                *datespec++ = ch;
	        ++p;
		...
		*datespec++ = 'n';
	    ...
	    else if (CMwhite(p))
	    {
                ...
                {
                    isempty = FALSE;
                    expect_month = !time; /* e.g. d month y */
                    *datespec++ = ch;
                    ++p;
                    sp_as_delim--;
                    break;
	...
	*/
	if ((datespec - specbuf) >= ( (i4)sizeof(specbuf)-1 ))
	{
	    goto adu_0parsedate_error;
	}
    }
    *datespec = EOS;
	...
	if (wild)
	{
	    date_dv->db_datatype = DB_DTE_TYPE;
	    date_dv->db_length = ADF_DTE_LEN;
	}
	else if (flags & DT_ANSI)
	    goto adu_0parsedate_error;
...
#   endif
	...
	*/
	      ...
	      || ((ingfmt->dt_flags & DT_TYP) && (flags & DT_ANSI) == 0) ) )
	{
	    if (wild)
	    {
		if (ingfmt->dt_flags & DT_TYP)
		{
		    /* In this ambiguous case we leave it as a string */
		    return E_DB_OK;
		}
		date_dv->db_datatype = DB_DTE_TYPE;
		date_dv->db_length = ADF_DTE_LEN;
	...
	else if (!strict || (flags&DT_ANSI))
	{
	    ...
	    */
	    if (ansifmt != NULL && (flags & DT_ANSI) && !wild)
	    {
		...
		*/
		...
		if (wild)
		{
		    switch(date_dv->db_datatype = ansifmt->dttype)		
		    {
		    case DB_ADTE_TYPE:	date_dv->db_length = ADF_ADATE_LEN; break;
		    case DB_TSWO_TYPE:	date_dv->db_length = ADF_TSTMP_LEN; break;
		    case DB_TSW_TYPE:	date_dv->db_length = ADF_TSTMP_LEN; break;
		    case DB_TSTMP_TYPE: date_dv->db_length = ADF_TSTMP_LEN; break;
		    case DB_INYM_TYPE:	date_dv->db_length = ADF_INTYM_LEN; break;
		    case DB_INDS_TYPE:	date_dv->db_length = ADF_INTDS_LEN; break;
		    case DB_DTE_TYPE:	date_dv->db_length = ADF_DTE_LEN; break;
		    case DB_TMWO_TYPE:
		    case DB_TMW_TYPE:
		    case DB_TME_TYPE:
			date_dv->db_length = ADF_TIME_LEN;
			/* Also need to set NO_DATE flag */
			flags |= DT_NODATE;
			break;
		    default:
			return E_DB_ERROR;
	...
	if (dt_interp != NULL)
	{
	    ...
	    */
		...
		if (ansifmt->dt_flags & DT_I)
		    dv2.dv_datetype = DV_LENGTH;
		ansists = ad0_dtcvrt(adf_scb, ansifmt->dt_interp,
			&d_valp, &d_sizp, &dv2, prec, &now);
		/* Disable INTERVAL YEAR recognition if in an INGRESDATE
		** context. Otherwise this can lead to confusing results
		** in legacy forms. */
		if (dv2.dv_status == DV_YEAR &&
			dv2.dv_datetype == DV_LENGTH && 
			date_dv->db_datatype == DB_DTE_TYPE &&
			!wild)
		    ansists = -1;
		...
		** only year can be 4 chars */
		...
		/* Check the numeric dates vs interval_ytom */
		else if (d_val_index == 1 && CMupper(&ingfmt->dt_interp[0]))
		{
		    switch (d_val_size[0])
		    {
		    case 1:
		    case 2: /* N & NN can't be INGRES */
			ingsts = -1;
			break;
		    case 3:
			if (ingfmt->dt_interp[0] != 'J')
			{
			    /* Only J format can be MDD */
			    ingsts = -1;
			    break;
			}
			/*FALLTHROUGH*/
		    case 4:
			if (flags & DT_ANSI ||
			    (ingfmt->dt_interp[0] == 'D'
			    ? /*DDMM*/	((d_val_array[0]-1) % 100 > 11 ||
					  d_val_array[0] > 3112)
			    : /*MMDD*/	((d_val_array[0]-1) % 100 > 30 ||
					  d_val_array[0] > 1231)))
			{
			    ingsts = -1;
			    break;
			...
			/*FALLTHROUGH*/
		    default: /* Can't be ANSI as year intervals cannot exceed 9999 */
			ansists = -1;
			if (wild)
			{
			    date_dv->db_datatype = DB_DTE_TYPE;
			...
			(ansifmt->dt_flags & DT_I) == 0)
		{
		    ...
		    ** user has not passed a 4 digit year so complain */
			...
			d_val_index > 2 && (
			    ansifmt->dt_interp[0] == 'y' && d_val_size[0] != 4 ||
			    ansifmt->dt_interp[1] == 'm' && d_val_size[1] != 2 ||
			    ansifmt->dt_interp[2] == 'd' && d_val_size[2] != 2 ||
			    d_val_index > 5 && (   
				ansifmt->dt_interp[3] == 'h' && d_val_size[3] != 2 ||
				ansifmt->dt_interp[4] == 'i' && d_val_size[4] != 2 ||
				ansifmt->dt_interp[5] == 's' && d_val_size[5] != 2
				)
			    )
			)
		{
		    /* This cannot be ansi form after all */
		    ansists = -1;
		    date_dv->db_datatype = DB_DTE_TYPE;
		    date_dv->db_length = ADF_DTE_LEN;
		}
	    ...
	    {
		/* Drop back to ingresdate if not ANSI unless prec
		** is specified - then leave as ANSI */
		if (prec == 0 &&
			(flags & DT_ANSI) == 0 && time < 0 &&
			timespec == &specbuf[SPEC_PREFIX_LEN])
		{
		    time = -time;
		    if (dv != &dv1)
	...
	*/
	...
	if ((flags & (DT_INTV|DT_ANSI)) == DT_ANSI && !wild ||
	    ad0_dtintvlok(p, time) ||
	    ad0_dtcvrt(adf_scb, p, &d_valp, &d_sizp, dv, prec, &now))
	{
	    goto adu_0parsedate_error;
	}
	else if (wild)
	{
	    if ((flags & DT_ANSI) &&
		(dv->dv_status & (DV_YEAR | DV_MONTH)) == 0)
	    {
		/* We have an ANSI context with an INGREDATE form
		** interval that does not include year or month so
		** we can safely pretend that this is INDS without
		** loss of generality. */
		date_dv->db_datatype = DB_INDS_TYPE;
		date_dv->db_length = ADF_INTDS_LEN;
	    }
	    else if ((flags & DT_ANSI) &&
		(dv->dv_status & (DV_DAY | DV_HOUR | DV_MINUTE |
				DV_SECOND | DV_NANOSECOND)) == 0)
	    {
		/* We have an ANSI context with an INGREDATE form
		** interval that does not include sub-month values so
		** we can safely pretend that this is INYM without
		** loss of generality. */
		date_dv->db_datatype = DB_INYM_TYPE;
		date_dv->db_length = ADF_INTYM_LEN;
	    }
	    else
	    {
		date_dv->db_datatype = DB_DTE_TYPE;
...
**
** History:
**	7-Sep-2010 (martinb) SIR: 125220
**	Allow for ISO4T and ISO4TC types which may have a leading 'T' in the 
**      timespec.
*/
...
char *f)
{
    ...
    ** n:n[:n[.n]][a][t]
    ** Tn[:n:n[.n]][a][t] ie. Thhmmss[.cs][a][t] or Thh:mm:ss[.cs][a][t]
    ** where n is a number, a is am/pm and t is timezone
    */
    if (MEcmp("Tn:n", f, 4) == 0)
    {
        f++;              /* Skip the leading 'T'...we'll leave that as a flag
                          ** for later. Processing will now fall through to 
                          ** standard processing of 'n:n'
                          */


    }
    else if (MEcmp("Tn", f, 2) == 0)
    {
        f++;               /* Skip the leading 'T'...we'll leave that as a flag
                           ** for later.
                           */
        *f++ = 'H';        /* replace first n with 'H'. this is used to imply 
                           ** a 6 character hhmmss field.
                           */
        if (MEcmp(".n", f, 2) == 0)
        {
            *++f = 'l';    /* So you'd get 'TH.l' */
            f++;
        }
    }
    else if (MEcmp("n:n", f, 3) == 0)
    {
    ...
    /* And now complete the processing checking for the [a][t] stuff */
    if (*f == 'a')
        f++;
    if (*f == 't')
        f++;
...
**      07-Sep-2010 (martinb) SIR: 125220
**          Allow for ISO4T and ISO4TC templates which *may* have formats:
**          TH[.l][a][t] or Th:i:s[.l][a][t]
*/ 
...
i4		*now)
{
	...
	i4	*sizep = *pdsizp;
	i2	status;
        i4      hhmmss; /* Used in ISO4T and ISO4TC */
#       ifdef xDEBUG
	if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_003_DATEFI_TRACE,
		&dum1, &dum2))
...
#       endif
    ...
    if (*p == 'T')
    {
        p++; /* Skip it, I'll assume its all kosher from here! */
    }
    for ( ; *p != EOS; p++)
    {
	status = 0;
	switch (*p)
	{
	  ...
	  case 'p':
	    ...
	    break;
          ...
          case 'H':
            /* This is hhmmss used in ISO4T and ISO4TC format.
            ** To avoid ambiguity, this must be exactly 6 characters long.
            ** For example a 5 character field could be interpreted as either:
            **     0hmmss, hh0mss or hhmm0s.
            ** Note that the colons in the normal absolute time of hh:mm:ss
            ** effectively solve that problem of interpretation.
            */
	    s = *sizep;
    	    if (s != 6)
		return (1); /* Error */
            hhmmss = *ip;
            dv->dv_second = hhmmss % 100;
            hhmmss = (i4 )hhmmss / 100;
	    dv->dv_minute = hhmmss % 100;
	    dv->dv_hour = (i4 )hhmmss / 100;
	    status = DV_SECOND; /* Should be sufficient for this! */
            break;
...
**	05-Feb-2010 (kiria01)
**	    Switch DATE('now') and DATE('today') to be CHKVARFI based on
**	    AD_CNST_FOR_X100 and not simply AD_CNST_DSUPLMT
**	12-Mar-2010 (kschendel) SIR 123275
**	    Caller can figure out ansi type easier than we can, pass it in.
**	28-Jun-2011 (kiria01) m2139
**	    Add pure_time - don't pick up avoidable non-const penalties.
*/
...
#define	    AD_TV_NORMMON		   1
    ...
    dn->dn_status2 |= AD_DN2_IN_LCLTZ;  /* Default to LCLTZ until we know better */
	...
	dn->dn_nsecond  = cur_tv.tm_nsec;
        ...
        /* adjust for timezone */
        /* now is already GMT don't adjust for tzoffset */
...
#   endif
	...
	{
	   /* The nonsensical TIME WITH [LOCAL] TIME ZONEs require a date to
	   ** TZ off of so will need YMD fetching. */
	   if (dn->dn_dttype == DB_TMWO_TYPE)
		pure_time = TRUE;
	   else
...
#       endif
	...
	*/
    ...
    else if (datetype == DV_ABSOLUTE && !pure_time)
    {
	/* Initialize current time vector for use in defaults */
	if (!ctv_init && (db_stat = ad0_1cur_init(adf_scb, &cur_tv, 
						now, highprec, &tzoff)))
...
**          interval string.
**	28-Mar-2011 (kiria01) 125223
**	    Removed the buffer overrun converting to Unicode and in addition
**	    dropped the need to alloc vm to do so.
*/
...
DB_DATA_VALUE	    *str_dv)
{
    ...
    char		formatbuf[50];
    static const char	adtformat[] = "%04d-%02d-%02d"; 
    static const char	atmformat[] = "%02d:%02d:%02d.%0";
    static const char	atzdformat[] = "%02d:%02d";
    static const char	aptzdformat[] = "+%02d:%02d";
    static const char	antzdformat[] = "-%02d:%02d";
    i4			mod_str_len;
    i4 			cnt = 0;
...
#       endif
        ...
        /* Get the date form into 'local' TZ. For dates that
	** have an existng TZ element, that TZ will be the 'local'
	** TZ used. */
	if (db_stat = ADU_NEWDT_PEND_DATE(adf_scb, nd))
	    return (db_stat);
	if (db_stat = ADU_NEWDT_TO_LCLTZ(adf_scb, nd))
	    return (db_stat);
	...
	else
	{
		...
		i4 ms = ns / AD_29DTE_NSPERMS;
                ...
                switch (adf_scb->adf_dfmt)
                {
                  ...
                  case DB_ISO4T_DFMT:
                    STprintf(temp, "%04d%02d%02dT%02d%02d%02d.%03d",
                        (i4)nd->dn_year,
                        (i4)nd->dn_month, (i4)nd->dn_day, hr, mn, sc, ms);
                    break;
            ...
            else
            {
                switch (adf_scb->adf_dfmt)
                {
                  ...
                  case DB_ISO4T_DFMT:
                    STprintf(temp, "%04d%02d%02dT%02d%02d%02d",
                        (i4)nd->dn_year,
                        (i4)nd->dn_month, (i4)nd->dn_day, hr, mn, sc);
                    break;
...
#       endif
    ...
    /* now we can return as result */
    if (str_dv->db_datatype == DB_NCHR_TYPE ||
	str_dv->db_datatype == DB_NVCHR_TYPE)
    {
	DB_DATA_VALUE str2_dv;
	str2_dv.db_datatype = DB_CHA_TYPE;
	str2_dv.db_data = temp;
	str2_dv.db_collID = DB_UNSET_COLL;
	str2_dv.db_length = STlength(temp);
	return adu_nvchr_coerce(adf_scb,  &str2_dv, str_dv);
    }
    else
...
DB_DATA_VALUE		*gmt_dv)
{
    ...
    if (db_stat = ADU_NEWDT_PEND_DATE(adf_scb, &d))
	return db_stat;
    if (db_stat = ADU_NEWDT_TO_GMT(adf_scb, &d))
	return db_stat;
    if (d.dn_status2 & AD_DN2_NEED_NORM)
...
DB_DATA_VALUE	    *dow_dv)
{
    ...
    if (nd->dn_status & AD_DN_LENGTH ||
	(nd->dn_status2 & (AD_DN2_NO_DATE|AD_DN2_DATE_DEFAULT)) ==
			AD_DN2_NO_DATE)
    {
...
**	    Only apply AD_DN2_IN_LCLTZ if delta has a time component.
*/
...
#       endif
	...
	** determination. */
	if (a->dn_status & AD_DN_ABSOLUTE && ~a->dn_status & AD_DN2_IN_LCLTZ)
	{
	    DB_STATUS db_stat;
	    if (db_stat = ADU_NEWDT_PEND_DATE(adf_scb, a))
		return (db_stat);
	    if (db_stat = ADU_NEWDT_TO_LCLTZ(adf_scb, a))
		return db_stat;
	    adu_1normldate(a);
	    tza = TRUE;
	}
	...
	if (tza)
	{
	    if (db_stat = ADU_NEWDT_TO_GMT(adf_scb, a))
		return db_stat;
	}
        if (a->dn_status & AD_DN_ABSOLUTE)
        {
...
AD_NEWDTNTRNL   *b)
{
	...
	** otherwise be delayed to adu_7from_dtntrnl call
	...
	{
	    if (a->dn_status2 & AD_DN2_NO_DATE)
		db_stat = ADU_NEWDT_PEND_DATE(adf_scb, a);
	    else
		db_stat = ADU_NEWDT_PEND_DATE(adf_scb, b);
	    if (db_stat)
		return db_stat;
	}

	/*
        ** If only one element is in in local timzone and
	** the other GMT - adjust the local to GMT.
	** INGRESDATES without a time portion (AD_DN_TIMESPEC
	** unset) and ANSIDATE will be treated as local with
        ** time set as midnight current time.
        */
	if ((a->dn_status2 ^ b->dn_status2) & AD_DN2_IN_LCLTZ)
	{
	    if (a->dn_status2 & AD_DN2_IN_LCLTZ)
		db_stat = ADU_NEWDT_TO_GMT(adf_scb, a);
	    else
		db_stat = ADU_NEWDT_TO_GMT(adf_scb, b);
	    if (db_stat)
		return db_stat;
	}
	if (a->dn_status2 & AD_DN2_NEED_NORM)
	    adu_1normldate(a);
	if (b->dn_status2 & AD_DN2_NEED_NORM)
	    adu_1normldate(b);

	/* Now we will be comparing GMT to GMT or local to local
	** (including date only dates too) */
	memset(&nd, 0, sizeof(nd)); 
	nd.dn_day = adu_3day_of_date(a) - adu_3day_of_date(b);

        /* subtract time portion (if any) */
        nd.dn_seconds = a->dn_seconds - b->dn_seconds;
        nd.dn_nsecond = a->dn_nsecond - b->dn_nsecond;

        nd.dn_status  = ((astat | bstat) & AD_DN_TIMESPEC) |
				AD_DN_LENGTH | AD_DN_DAYSPEC;

        /* Finally normalize time and days */
        adu_2normldint(&nd);
	/* Drop to common processing */
    }
    else if (astat & AD_DN_ABSOLUTE)
    {
	/* Keep this simple - negate the subtrahend and then ADD.
	** Do this via the temporary so that we don't change the
	** value of 'b'. */
	nd = *b; 
	if (db_stat = ad0_dte_negdtntrnl(adf_scb, &nd))
	    return db_stat;
	return (add_date(adf_scb, rdv, a, &nd));
    }
    else
    {
	memset(&nd, 0, sizeof(nd)); 
	nd.dn_year =	a->dn_year -	b->dn_year;
	nd.dn_month =	a->dn_month -	b->dn_month;
	nd.dn_day =	a->dn_day -	b->dn_day;
	nd.dn_seconds =	a->dn_seconds -	b->dn_seconds;
	nd.dn_nsecond =	a->dn_nsecond -	b->dn_nsecond;
        /*
        ...
        **  Fix of bug #1828 - Jeff Lichtman
        */
        nd.dn_status = a->dn_status | b->dn_status &
                (AD_DN_YEARSPEC | AD_DN_MONTHSPEC |
                 AD_DN_DAYSPEC | AD_DN_TIMESPEC);

	nd.dn_status &= ~AD_DN_ABSOLUTE;
	nd.dn_status |=  AD_DN_LENGTH;

        adu_2normldint(&nd);

        /* check if exceeded max. interval allowed */

        if (nd.dn_year > MAX_INV_YR || nd.dn_year < MIN_INV_YR)
        {
            return (adu_error(adf_scb, (i4) E_AD505A_DATEYROVFLO, 2,
			  (i4)sizeof(nd.dn_year),
			  (i4 *) &nd.dn_year));
        }
    }
...

/*
** Name: adu_date_abs() - ADF interface to abs(intervals)
**
** Description:
**	  adu_date_abs() supports deriving the absolute value of an interval
**	The results are the obvious ones:
...
DB_DATA_VALUE  *rdv)
{
    ...
    case DB_INDS_TYPE:
    	if (db_stat = adu_6to_dtntrnl (adf_scb, dv1, &dn))
	    return db_stat;
	/* Make sure we are dealing with and interval! */
	if (~dn.dn_status & AD_DN_LENGTH)
	    break;
	/* Normalise to ensure that all signs are consistent */
	adu_2normldint(&dn);
	if (dn.dn_year < 0 ||
            dn.dn_month < 0 ||
            dn.dn_day < 0 ||
...
DB_DATA_VALUE	*hg_dv)
{
    ...
    ** ordering.
    */
    if (db_stat = ADU_NEWDT_TO_GMT(adf_scb, datep))
	return db_stat;
    if (datep->dn_status2 & AD_DN2_NEED_NORM)
...
**	31-Aug-2011 (kiria01) b125709
**	    Allow milli/micro/nanosecond to be extracted()
*/
	    ...
	    * (f8)AD_40DTE_SECPERDAY
        ...
        case 'Y':
            *res = secs / (AD_40DTE_SECPERDAY * AD_14DTE_DAYPERYEAR);
            break;
        case 'm':
            *res = (datep->dn_seconds / 1000000)*1000000;
            break;
        case 'x':
            *res = (datep->dn_seconds / 1000)*1000;
            break;
        case 'X':
            *res = datep->dn_seconds;
            break;
        default:
...
**      10-Jun-2011 (coomi01) Bug 125405
**          Do not ignore timezone component in the truncation 
**          calculation for the larger inter_spec scale. It is there,
**          and will be applied in the call to adu_7from_dtntrnl() 
**          as we exit throwing our calculation out.
**      24-Jun-2011 (coomi01) Bug 125479
**          Need to apply the above to day, week, month, quater and year too.
**      08-Aug-2011 (coomi01) Bug 125627
**          Exclude ingres date data type from the previous change.
**	31-Aug-2011 (kiria01) b125709
**	    Add milli/micro/nanosecond
*/
...
DB_DATA_VALUE       *date_result)
{
    ...
    else if ((datep->dn_status & AD_DN_ABSOLUTE) == 0)
    {
    ...
    switch (dateclass)
    {
      ...
      case 'm':
        ...
        datep->dn_nsecond /= 1000000;
        datep->dn_nsecond *= 1000000;
        break;
      case 'x':
        /* truncate microseconds */
        datep->dn_nsecond /= 1000;
        datep->dn_nsecond *= 1000;
        break;
      case 'X':
        /* truncate nanoseconds - nothing to do! */
        break;
      case 'I':
        /* truncate minutes */
	if (db_stat = ADU_NEWDT_TO_LCLTZ(adf_scb, datep))
	    return db_stat;
	if (datep->dn_status2 & AD_DN2_NEED_NORM)
	    adu_1normldate(datep);
	...
	adjustment = datep->dn_seconds / AD_39DTE_ISECPERHOUR;
	datep->dn_seconds = adjustment * AD_39DTE_ISECPERHOUR;
	datep->dn_nsecond = 0;
        break;

      case 'D':
        /* truncate days */
	if (db_stat = ADU_NEWDT_PEND_DATE(adf_scb, datep))
	    return db_stat;
	if (db_stat = ADU_NEWDT_TO_LCLTZ(adf_scb, datep))
	    return db_stat;
	if (datep->dn_status2 & AD_DN2_NEED_NORM)
...
**	31-Aug-2011 (kiria01) b125709
**	    Add milli/micro/nanosecond and epoch
*/
...
DB_DATA_VALUE       *result)
{
	...
	*/
	...
	if (datep->dn_status & (AD_DN_ABSOLUTE | AD_DN_TIMESPEC) ==
		(AD_DN_ABSOLUTE | AD_DN_TIMESPEC))
	{
	    /* Don't apply any pending date for timezone calculation */
	...
	switch (dateclass)
	{
	  ...
	  case 'e': /* return epoch */
	    component = adu_5time_of_date(datep);
	    break;

	  case 'E': /* return dayofweek */
	    component = ((adu_3llday_of_date(datep->dn_year, datep->dn_month,
				datep->dn_day) - 2) % 7) + 1;
	    break;

	  case 'y': /* return dayofyear */
	    component = adu_llday_of_year(datep->dn_year, datep->dn_month,
				datep->dn_day);
	    break;

	  case 'm':
	    /* extract millisecoonds */
	    component = datep->dn_nsecond / 1000000;
	    break;
...
/*{
** Name: adu_dextract_decimal()   - extract component of date and return as decimal.
**
** Description:
**	This is the internal implementation of the date_part() SQL/QUEL
**	function.
**	
**      Definition of function: 
**          adu_dextract_decimal(adf_scb, date_result, inter_spec, date_spec) 
**        
**      This function returns an decimal which represents the extracted date
**      component from <date_spec> specified by the <inter_spec>.
**
**      Example 1, 
**	    adu_dextract_decimal(adf_scb, "month", date("23-oct-1985 12:33:00"))
**      would return the decimal 10.0
**
**      Example 2, 
**	    adu_dextract_decimal(adf_scb, "day", date("23-oct-1985 12:33:00"))
**      would return the decimal 23.0
**
**      Example 3, 
**	    adu_dextract_decimal(adf_scb, "second", date("23-oct-1985 12:33:25.12345"))
**      would return the decimal 25.12345
**        
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.
...
**		.adf_errmsgp		Pointer to the formatted error message.
**      result                          Ptr to location at which to return
**					the decimal value of the component
**	Returns:
**	      The following DB_STATUS codes may be returned:
**	    E_DB_OK, E_DB_WARN, E_DB_ERROR, E_DB_SEVERE, E_DB_FATAL
**
...
**          Create a decimal variant of extract from date, adu_dextract() above.
**          However, the seconds variation also includes nanosecond detail.
**	31-Aug-2011 (kiria01) b125709
**	    Add milli/micro/nanosecond and epoch
*/
DB_STATUS
adu_dextract_decimal(
ADF_CB              *adf_scb,
DB_DATA_VALUE	    *inter_spec,
DB_DATA_VALUE       *date_spec,
DB_DATA_VALUE       *result)
{
    ...
    register AD_NEWDTNTRNL  *datep;
    AD_NEWDTNTRNL	dateval;
    f8			fcomponent = 0.0; /* contains extracted date component */
    AD_NEWDTNTRNL       ndatev;
	...
	*/
	...
	switch (dateclass)
	{
	  case 'S':
	    ...
	    /* Where a decimal result is required, 
	    ** an f8 serves as temp workspace with
	    ** enough precision to hold the result
	    */
	    fcomponent = (f8)(datep->dn_seconds % AD_10DTE_ISECPERMIN)
		+ ((f8)datep->dn_nsecond)/AD_50DTE_NSPERS;
	    break;

	  case 'I':
	    /* extract minutes */
	    fcomponent= (f8)((datep->dn_seconds/AD_10DTE_ISECPERMIN) % AD_11DTE_IMINPERHOUR);
	    break;

	  case 'H':
	    /* extract hours */
	    fcomponent = (f8)(datep->dn_seconds/AD_11DTE_IMINPERHOUR/
						AD_10DTE_ISECPERMIN);
	    break;

	  case 'D':
	    /* extract days */
	    fcomponent = (f8)(datep->dn_day);
	    break;

	  case 'M':
	    /* extract months */
	    fcomponent = (f8)(datep->dn_month);
	    break;

	  case 'Y':
	    /* extract years */
	    fcomponent = (f8)(datep->dn_year);
	    break;

	  case 'Q':
	    /* extract quarters */
	    fcomponent = (f8)(((datep->dn_month-1) / AD_18DTE_IMONPERQUAR) + 1);
	    break;
	  ...
	  case 'W':
		...
		** return the week number otherwise where monday begins the week
		*/
		fcomponent = (f8)((daynumber + AD_15DTE_IDAYPERWEEK - dayofweek)
				    / AD_15DTE_IDAYPERWEEK);
	    }
	    break;
	  ...
	  case 'w':	/* ISO 6801 week */	
		...
		fcomponent = (f8)component;
	    }
	    break;

	  case 'e': /* return epoch */
	    fcomponent = (f8)adu_5time_of_date(datep);
	    break;

	  case 'E': /* return dayofweek */
	    fcomponent = ((adu_3llday_of_date(datep->dn_year, datep->dn_month,
				datep->dn_day) - 2) % 7) + 1;
	    break;

	  case 'y': /* return dayofyear */
	    fcomponent = adu_llday_of_year(datep->dn_year, datep->dn_month,
				datep->dn_day);
	    break;

	  case 'm':
	    /* extract millisecoonds */
	    fcomponent = (f8)(datep->dn_nsecond / 1000000);
	    break;

	  case 'x':
	    /* extract microsecoonds */
	    fcomponent = (f8)(datep->dn_nsecond / 1000);
	    break;

	  case 'X':
	    /* extract nanosecoonds */
	    fcomponent = (f8)(datep->dn_nsecond);
	    break;

	  case 'R':
	    /* extract timezone_hour */
	    fcomponent = (f8)(AD_TZ_SIGNNEW(datep)*AD_TZ_HOURSNEW(datep));
	    break;

	  case 'T':
	    /* extract timezone_minute */
	    fcomponent = (f8)(AD_TZ_SIGNNEW(datep)*AD_TZ_MINUTESNEW(datep));
	    break;
    ...
    /* 
    ** Finally convert the floating point component into 
    ** a decimal of appropriate scale and precision.
    */
    CVfpk(fcomponent,
	  (i4)DB_P_DECODE_MACRO(result->db_prec),
	  (i4)DB_S_DECODE_MACRO(result->db_prec),
	  (PTR)result->db_data);
...

/*{
** Name: adu_millisecond_part() - stub to drive adu_dextract() to extract 
**	'millisecond'
**
** Description:
**	This is the function instance called for the millisecond() function. It 
**	simply redirects the call to adu_dextract() with "millisecond" as
**	the unit to extract.
**        
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
...
DB_DATA_VALUE       *result)
{
    ...
    /* This is a simple matter of redirecting the call with "inter_spec" 
    ** chosen from constant values. */
    return(adu_dextract(adf_scb, &millisecond_spec, date_spec, result));
}
...
/*{
** Name: adu_21ansi_alltodt_with_precision - converts a valid
**	  text input to internal storage format for ANSI datetime data type
**
** Description:
**      This routine is a wrapper for adu_21ansi_strtodt() above
**       in a situation where an output precision has also been specified.
**
** Inputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.
...
**					array should be used in the inter-
**					pretation of the date string.
**	all_dv				DB_DATA_VALUE describing the input data
**	    .db_datatype		Its datatype.
**	    .db_length			Its length.
**	    .db_data			If the datatype is character, this is
**					a pointer to the string to be converted.
...
**		[.db_t_text]		Ptr to the "text" string to be
**					converted.
**     precision                        DB_DATA_VALUE describing the precision
**                                      as an i2.
** Outputs:
**	adf_scb				Pointer to an ADF session control block.
**	    .adf_errcb			ADF_ERROR struct.  If an
...
** History:
**    
**      04-Jul-2011 (coomi01) b125396
**            Created.
*/
DB_STATUS
adu_21ansi_alltodt_with_precision(
    ADF_CB          *adf_scb,
    DB_DATA_VALUE   *all_dv,
    DB_DATA_VALUE   *precision,
    DB_DATA_VALUE   *result_dv
    )
{
    return adu_0parsedate(adf_scb, all_dv, result_dv, FALSE, FALSE);
}
...
DB_DATA_VALUE		*pOutDv)
{
	...
	*/
    ...
    if (db_stat = ADU_NEWDT_PEND_DATE(adf_scb, pInternalDate))
	return (db_stat);

    if (db_stat = ADU_NEWDT_TO_GMT(adf_scb, pInternalDate))
	return (db_stat);

    if (pInternalDate->dn_status2 & AD_DN2_NEED_NORM)
    {
