...
** 	adu_managed_readmap - Reads and processes a coercion map table.  
** 	adu_readmap - Legacy interface. 
** 	adu_initmap - Initializes the server map table in memory
** 	adu_managed_deletemap - Cleans up the memory allocated for mapping tables. 
** 	adu_deletemap - Legacy interface. 
**
**  History:
**	04-Jan-2004 (gupsh01)
**	    Created.
...
**	    Interface change for managed mapping tables enable char mapping table
**	    sharing with adu_managed_readmap and adu_managed_deletemap.
...
/*
** Adu_mapdb - managed cache of charmaps
*/
typedef struct _ADU_MAPPING_DB
{
    struct _ADU_MAPPING_DB	*next;
    ADU_MAP_INFO		*map;
    u_i4			ref_count;
    char			charset[30+1];
} ADU_MAPPING_DB;

static ADU_MAPPING_DB *Adf_mapdb;
#ifdef OS_THREADS_USED
   static CS_SYNCH Adf_mapdb_mtx;
#  define LOCK_MAPDB_MTX        CS_synch_lock(&Adf_mapdb_mtx)
#  define UNLOCK_MAPDB_MTX      CS_synch_unlock(&Adf_mapdb_mtx)
#  define INIT_MAPDB_MTX        CS_synch_init(&Adf_mapdb_mtx)
#  define JUST_UNLOCK_MAPDB_MTX CS_synch_unlock(&Adf_mapdb_mtx)

#else
#  define LOCK_MAPDB_MTX        OK
#  define UNLOCK_MAPDB_MTX      OK
#  define INIT_MAPDB_MTX        OK
#  define JUST_UNLOCK_MAPDB_MTX
#endif
static u_i4 Adf_mapdb_base_cnt = 0;
static bool Adf_mapdb_initd = FALSE;
...
DB_STATUS adu_map_init(void)
{
    Adf_mapdb_initd = TRUE;
    return INIT_MAPDB_MTX;
}
...
{
    return adu_managed_readmap(charset, &Adf_globs->Adu_maptbl);
}
    ...
    ADU_MAP_INFO	**map_p)
{
    ...
    /* Make a quick scan of charset in the db & use if found */
    if (stat = LOCK_MAPDB_MTX)
	return stat;
    for (p = &Adf_mapdb; q = *p; p = &(*p)->next)
    {
	if (!STcompare(charset, q->charset))
	{
	    q->ref_count++;
	    *map_p = q->map;
	    return UNLOCK_MAPDB_MTX;
	}
    }
    ...
    for ( ;(bytes_read < (i4)(header->validity_size + 
    ...
    if (bytes_read < (i4)(header->validity_size +
                      header->assignment_size + 
                      sizeof(ADU_MAP_HEADER)))
    {
    ...
    */
    stat =  adu_initmap (header, validities, assignments, map_p);
    
    /* Add to mapping db */
    if (!stat && *map_p)
    {
	q = (ADU_MAPPING_DB *)(MEreqmem(0,
                  sizeof(ADU_MAPPING_DB), FALSE, &stat));
	q->next = Adf_mapdb;
	strncpy(q->charset, charset, sizeof(q->charset));
	q->charset[sizeof(q->charset)-1];
	q->map = *map_p;
	q->ref_count = 2;
	Adf_mapdb = q;
    }
...
adu_deletemap(void)
{
    return adu_managed_deletemap(&Adf_globs->Adu_maptbl);
}

DB_STATUS
adu_managed_deletemap(ADU_MAP_INFO **map_p)
{
    DB_STATUS		status;
    ADU_MAP_HEADER 	*header;
    ...
    if (!*map_p)
	return E_DB_OK;
    /* Look for it in mapping db */
    if (Adf_mapdb_initd)
    {
	if (status = LOCK_MAPDB_MTX)
	    return status;
	for (p = &Adf_mapdb; q = *p; p = &(*p)->next)
	{
	    if (q->map == *map_p)
	    {
		/* Found - check if its freeable */
		if (--q->ref_count > Adf_mapdb_base_cnt)
		    return UNLOCK_MAPDB_MTX;
		*p = q->next;
		MEfree ((char *)q);
		break;
    ...
    header = (*map_p)->header;
    charmapptr = (*map_p)->charmapptr;
    unimapptr = (*map_p)->unimapptr;
    validity_tbl = (*map_p)->validity_tbl;
