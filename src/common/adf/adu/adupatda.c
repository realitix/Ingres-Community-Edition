/*
** Copyright (c) 2009, 2010 Actian Corporation
**
*/
...
**      10-Oct-2010 (coomi01) b125828
**          Adjust size of MEreqmem for da_ctx->mem to cater for boundary case.
*/
DB_STATUS
adu_patda_init(
	ADF_CB		*adf_scb,
...
loc_common: da_ctx->cpn_dv.db_data = (PTR) &cpn;
	...
	*/
	...
	da_ctx->mem = p = MEreqmem(0, seg_multr*(len + DB_CNTSIZE + 2), FALSE, &db_stat);
	if (db_stat)
	{
	    return adu_error(adf_scb, da_ctx->pop_cb.pop_error.err_code, 0);
	}
...
adu_patda_get(AD_PAT_DA_CTX *da_ctx)
{
	...
	*/
	...
	if (db_stat = adu_pat_MakeCElen(da_ctx->adf_scb, s, s+len/sizeof(UCS2),
			&da_ctx->sea_ctx->buflen, pat_flags))
	    return db_stat;
