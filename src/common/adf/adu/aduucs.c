/*
** Copyright (c) 2001, 2009, 2011 Actian Corporation
*/
...
**      13-Oct-2011 (horda03) b125801
**          CS_SYNCH only defined in OS_THREAD builds. Added macros to init, lock and
**          unlock the mutex without the need to litter the code with too many
**          #ifdef OS_THREADS_USED.
*/

/*
** ADU_UCOL_DB - Header block for collation managing
**
**	One of these blocks will be present preceeding each ADUUCETAB.
**	It's purpose is to allown these ADUUCETAB sets to be shared and managed.
**	The aduucolinit will allocate the extra space when it loads a collation set
**	and returns the address of the adjoining ADUUCETAB.
**	This allows aduucolinit() to scan the ucol db to see if the collation data
**	has previously been requested. If found, the reference count is incremented
**	and the address returned.
**	The routine aduucolterm() scans for the block to be released and decrements
**	the reference count. If the count goes to zero, the vartbl block and the actual
**	fixed block are really freed.
*/
typedef struct _ADU_UCOL_DB
{
    struct _ADU_UCOL_DB	*next;
    i4			ref_count;
    char		colname[DB_MAXNAME+1];
    void		*vartbl;
} ADU_UCOL_DB;

static ADU_UCOL_DB *Adf_ucol_db = NULL;
#ifdef OS_THREADS_USED
   static CS_SYNCH Adf_ucoldb_mtx;
#  define LOCK_UCOLDB_MTX        CS_synch_lock(&Adf_ucoldb_mtx)
#  define UNLOCK_UCOLDB_MTX      CS_synch_unlock(&Adf_ucoldb_mtx)
#  define INIT_UCOLDB_MTX        CS_synch_init(&Adf_ucoldb_mtx)
#  define JUST_UNLOCK_UCOLDB_MTX CS_synch_unlock(&Adf_ucoldb_mtx)

#else
#  define LOCK_UCOLDB_MTX        OK
#  define UNLOCK_UCOLDB_MTX      OK
#  define INIT_UCOLDB_MTX        OK
#  define JUST_UNLOCK_UCOLDB_MTX
#endif
static u_i4 Adf_ucoldb_base_cnt = 0;
static bool Adf_ucoldb_initd = FALSE;

DB_STATUS adu_col_init(void)
{
    Adf_ucoldb_initd = TRUE;
    return INIT_UCOLDB_MTX;
}
	    ...
	    CL_ERR_DESC	*syserr)
{
    ...
    if (stat = LOCK_UCOLDB_MTX)
	return stat;
    for (p = &Adf_ucol_db; *p; p = &(*p)->next)
    {
	if (!strcmp(colname, (*p)->colname))
	{
	    (*p)->ref_count++;
	    *tbl = (ADUUCETAB*)((*p)+1);
	    return UNLOCK_UCOLDB_MTX;
	}
    }
    ...
    *tbl = (ADUUCETAB*)NULL;
    ...
    dbent = (ADU_UCOL_DB *)MEreqmem(0, ENTRY_SIZE+sizeof(ADU_UCOL_DB), TRUE, &stat);
    if (dbent == NULL || stat != OK)
    {
	 _VOID_ CMclose_col(syserr, CM_COLLATION_LOC);
	JUST_UNLOCK_UCOLDB_MTX;
	return (FAIL);
    }
    *tbl = tbase = (ADUUCETAB *)(dbent+1);
    tptr = (char*)tbase;
    dbent->next = Adf_ucol_db;
    dbent->ref_count = 2;
    strcpy(dbent->colname, colname);
	...
	else if (bytes_read <= ENTRY_SIZE + COL_BLOCK)
	{
		...
		ipointer = MEreqmem(0, (u_i4)*varsize + (u_i4)*recomb_tbl_size, 
				TRUE, &stat);
...
aduucolterm(ADUUCETAB **tbl)
{
	...
	{
	    ADU_UCOL_DB	*dbent = (*p);
	    if (*tbl == (ADUUCETAB*)((*p)+1))
	    {
		ADU_UCOL_DB *dbent = *p;
		if (--dbent->ref_count > Adf_ucoldb_base_cnt)
		    break;
		*p = dbent->next;
		MEfree(dbent->vartbl);
		MEfree(dbent);
