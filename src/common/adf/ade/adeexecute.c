/*
** Copyright (c) 2004, 2008, 2010, 2011 Actian Corporation
*/
#include    <compat.h>
#include    <gl.h>
#include    <cv.h>
...
static VOID	   adf_printf( char *str, ...);
#endif
...
**	    (b123074)
**	08-Feb-2010 (kiria01)
**	    Set the AD_CNST_FOR_X100 flag for the username related functions
...
**	    Added ADFI_1476_ASSERT_I, ADFI_1476_ASSERT_S and ADE_GETSTATUS
**	30-Jan-2011 (kiria01) SIR 124690
**	    Added missing collation propagation and initialisation
**	10-Oct-2011 (smeke01) b125834
**	    Added overflow/underflow check for integer arithmetic.
*/

...
/* {@fix_me@} */
	  ...
	  case ADE_GETSTATUS:
	    /* Set the BOOL result from the flags setting */
	    *((u_char *)data[0]) =
			cx_value == ADE_TRUE ? TRUE : FALSE;
	    break;
...
do_adccompare:
		...
		** here to execute "slow" path version of ADE_COMPARE. */
		...
		dv[0].db_collID = oprP[0]->opr_collID;
		dv[0].db_length = oprP[0]->opr_len;
		dv[0].db_data   = data[0];
		dv[1].db_prec   = oprP[1]->opr_prec;
		dv[1].db_collID = oprP[1]->opr_collID;
...
utf8_compare:
	 ...
	 case ADE_DBDV:
	 {
	    ...
	    /*	    oprP[1]->opr_collID length for db_collID		    */
	    ...
	    dbv->db_collID = oprP[1]->opr_collID;
	    dbv->db_data = data[1];
	    break;
	 }
	 ...
	 case ADE_PNAME:
	 {
	  ...
	  case ADE_REDEEM:
	  {
	    ...
	    dv[2].db_collID	  = oprP[2]->opr_collID;
...
*******************************************************************************/
	  ...
	  case ADFI_527_DEC_DIV_F:
	    ...
	    if (oprP[0]->opr_len == 8)
	    {
		/* MHi8add checks for i8 overflow/underflow */
		*(i8 *)data[0] = tempbi;
	    }
	    else
	    if (oprP[0]->opr_len == 4)
	    {
		if(tempbi > MAXI4 || tempbi < MINI4)
		     EXsignal(EXINTOVF,  0);
		*(i4 *)data[0] = tempbi;
	    }
	    else
	    if (oprP[0]->opr_len == 2)
	    {
		if(tempbi > MAXI2 || tempbi < MINI2)
		     EXsignal(EXINTOVF,  0);
		*(i2 *)data[0] = tempbi;
	    }
	    else
	    {
		if(tempbi > MAXI1 || tempbi < MINI1)
		     EXsignal(EXINTOVF,  0);
		*(i1 *)data[0] = tempbi;
	    }
            break;
	    ...
	    if (oprP[0]->opr_len == 8)
	    {
		/* MHi8sub checks for i8 overflow/underflow */
		*(i8 *)data[0] = tempbi;
	    }
	    else
	    if (oprP[0]->opr_len == 4)
		...
		*(i1 *)data[0] = tempbi;
	    ...
	    if (oprP[0]->opr_len == 8)
	    {
		/* MHi8mul checks for i8 overflow/underflow */
		*(i8 *)data[0] = tempbi;
	    }
	    else
	    if (oprP[0]->opr_len == 4)
	  ...
	  case ADFI_1476_ASSERT_I:
	  case ADFI_1477_ASSERT_S:
	  {
	    if (abs(oprP[2]->opr_dt) != DB_VCH_TYPE &&
		    abs(oprP[2]->opr_dt) != DB_INT_TYPE)
		return adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 2,
				0, "ASSERT p2 dt");
	    if (oprP[1]->opr_dt < 0 && ((data[1])[oprP[1]->opr_len - 1] & ADF_NVL_BIT) ||
		(data[1])[0] == FALSE)
	    {
		i4 len = oprP[2]->opr_len, err = 0;
		if (oprP[2]->opr_dt < 0)
		{
		    len--;
		    if ((data[2])[len] & ADF_NVL_BIT)
			return (adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 2,
				0, "ASSERT p2 NULL"));
		}
		if (abs(oprP[2]->opr_dt) == DB_VCH_TYPE)
		{
		    return db_stat = adu_error(adf_scb, E_AD106C_ASSERTION_FAILURE, 2,
				*((u_i2*)(data[2])), data[2]+sizeof(u_i2));
		}
		else
		{
		    if (len == (i4)sizeof(i4)) err = *(i4*)(data[2]);
		    else if (len == (i4)sizeof(i2)) err = *(i2*)(data[2]);
		    else if (len == (i4)sizeof(i1)) err = *(i1*)(data[2]);
		    else if (len == (i4)sizeof(i8)) err = *(i8*)(data[2]);
		    return db_stat = adu_error(adf_scb, err, 0);
		}
	    }
	    /* As lenspec ADI_O1 is used, the output length should be used */
	    MEcopy(data[1], oprP[0]->opr_len, data[0]);
...
**	10-dec-2010 (gupsh01) Sir 124685
**	    Prototype fixup.
*/
static VOID
adf_printf( char *str, ...)
{
    ...
    va_start(ap, str);
    SIdofrmt(CMCPYCHAR, cbuf, str, ap);
    va_end(ap);
