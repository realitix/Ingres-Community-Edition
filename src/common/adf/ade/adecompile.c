...
**	    Use ADE_OPS_ENUM type for instruction to aid debuging.
*/

/*
...
static  VOID	    ad0_setqconst_flag(ADE_OPS_ENUM icode,
				      i4   *qconst_flag);    
#ifdef ADE_BACKEND
...
**	    E_AD5514_BAD_DTID_GETSTATUS	ADE_GETSTATUS returning wrong type.
**	    E_AD5515_BAD_DTLEN_GETSTATUS    ADE_GETSTATUS returning wrong len
**
**	Exceptions:
**	    none
**
...
**	    Added dt_family fixup code as per adu_ficoerce_full() to correct
**	    the adi_fidesc returned structure.
**	15-Jul-2011 (kiria01) b125530
**	    Allow ADE_MECOPY with non-null inputs and nullable output to generate
**	    npi instruction ADE_6CXI_CLR_OP0NULL. This allows us to avoid calling
**	    coercion routines when all we have to change is the nullability.
*/
...
#define	    AD_NULLABLE           1  /* output is nullabe */
	...
	case ADE_GETSTATUS:
	    if (ade_nops != 1)
		status = E_AD5502_WRONG_NUM_OPRS;
	    else if (ade_ops[0].opr_dt == DB_BOO_TYPE)
	    {
		if (ade_ops[0].opr_len != DB_BOO_LEN)
		    status = E_AD5515_BAD_DTLEN_GETSTATUS;
	    }
	    else if (ade_ops[0].opr_dt != -DB_BOO_TYPE)
		status = E_AD5514_BAD_DTID_GETSTATUS;
	    else if (ade_ops[0].opr_len != DB_BOO_LEN+1)
		status = E_AD5515_BAD_DTLEN_GETSTATUS;
	    break;
	...
	case ADE_MECOPY:
	    /* If MECOPY from non-nullable to nullable, set npi to
	    ** ADE_1CXI_SET_SKIP. It will later be switched to be
	    ** ADE_6CXI_CLR_OP0NULL correctly adjusting for non-null
	    ** input. */
	    if (ade_icode == ADE_MECOPY &&
		    ade_ops[0].opr_dt < 0 &&
		    ade_ops[1].opr_dt > 0)
		npi = ADE_1CXI_SET_SKIP;
	    /*FALLTHROUGH*/
	...
	default:    /* Not a CX special instruction ... is it a f.i. ID? */
		...
		return (db_stat);    /* If not a legal instruction code,
				    ...
				    */
	    /* Trouble now is that the fdesc returned, whilst matching the icode,
	    ** might originally, when first chosen, have been fixed up for dt_family
	    ** processing. We need to pass through the same logic again but split out
	    ** coercions as these have slightly differing rules. */
	    if (fdesc->adi_fitype != ADI_COERCION)
	    {
		ADI_RSLV_BLK adi_rslv_blk;
		i2 need = 0;
		i4 i;
		adi_rslv_blk.adi_num_dts = fdesc->adi_numargs;
		for (i = 0; i <= fdesc->adi_numargs; i++)
		    adi_rslv_blk.adi_dt[i] =
			i == 0 ? fdesc->adi_dtresult : fdesc->adi_dt[i-1];
		if (db_stat = adi_need_dtfamily_resolve(adf_scb, fdesc, &adi_rslv_blk, &need))
		    return db_stat;
		if (need == 1)
		{
		    if (db_stat = adi_dtfamily_resolve(adf_scb, fdesc, 
						&lclfdesc, &adi_rslv_blk))
			return db_stat;
		    fdesc = &lclfdesc;
		}
	    }
	    ...
	    {
		if (db_stat = adi_coerce_dtfamily_resolve(adf_scb, fdesc,
			ade_ops[1].opr_dt, ade_ops[0].opr_dt, &lclfdesc))
		    return db_stat;
		fdesc = &lclfdesc;
	    }
...
**	    Tweak above by making result a void *.  It can point to
