...
**          If ADF_TYPE_TO_UNICODE is set generate the Unicode equivalent name
**          for the supplied datatype. char -> nchar for example.
**       9-Jun-2011 (hanal04) SIR 125274
**          Check for NULL adf_scb before trying to look for a unicode
**          equivalent. STAR may pass in a NULL adf_scb and cause a SEGV.
*/
...
ADI_DT_NAME         *adi_dname)
{
    ...
    if(adf_scb && (adf_scb->adf_misc_flags & ADF_TYPE_TO_UNICODE))
    {
        for(n=1; n <= ADI_MXDTS; n++)
        {
            ...
            {
                if (Adf_globs->Adi_datatypes[n].adi_dtid_touni != bdt)
                {
                    bdt = Adf_globs->Adi_datatypes[n].adi_dtid_touni;
                    break;
                }
                else
                {
