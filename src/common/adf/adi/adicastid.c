/*
** Copyright (c) 2002, Actian Corporation
*/
...
**	    Correct operator logic for spatials
**	15-Aug-2011 (kiria01) b125511
**	    Handle nullable types too!
**	17-Aug-2011 (kiria01) b125511
**	    Adjust lengths checked.
**	    Also return the potential second param value.
*/
	...
	i4		*par)
{
	...
	*adi_oid = ADI_BOO_OP;
    ...
    case DB_INT_TYPE:
	if (adi_castdv->db_length - len_adj == 4)
	    *adi_oid = ADI_I4_OP;
	else if (adi_castdv->db_length - len_adj == 2)
	    *adi_oid = ADI_I2_OP;
	else if (adi_castdv->db_length - len_adj == 8)
	    *adi_oid = ADI_I8_OP;
	else if (adi_castdv->db_length - len_adj == 1)
	    *adi_oid = ADI_I1_OP;
	break;

    case DB_FLT_TYPE:
	if (adi_castdv->db_length - len_adj == 8)
	    *adi_oid = ADI_F8_OP;
	else if (adi_castdv->db_length - len_adj == 4)
	    *adi_oid = ADI_F4_OP;
	break;

    case DB_CHR_TYPE:
	*adi_oid = ADI_ASCII_OP;
	*par = adi_castdv->db_length - len_adj;
	break;
	...
	*adi_oid = ADI_INYM_OP;
    ...
    case DB_INDS_TYPE:
	*adi_oid = ADI_INDS_OP;
	/* FIXME: INDS probably should support precision too */ 
	break;

    case DB_TXT_TYPE:
	*adi_oid = ADI_TEXT_OP;
	*par = adi_castdv->db_length - len_adj - DB_CNTSIZE;
	break;
	...
	*par = (adi_castdv->db_length - len_adj) / sizeof(UCS2);
	break;

    case DB_NVCHR_TYPE:
	*adi_oid = ADI_NVCHAR_OP;
	*par = (adi_castdv->db_length - len_adj - DB_CNTSIZE) / sizeof(UCS2);
	break;
	...
	*adi_oid = ADI_GEOM_OP;
    ...
    case DB_POINT_TYPE:
	*adi_oid = ADI_POINT_OP;
	break;

    case DB_MPOINT_TYPE:
	*adi_oid = ADI_MPOINT_OP;
	break;
	...
	*adi_oid = ADI_POLY_OP;
    ...
    case DB_MPOLY_TYPE:
	*adi_oid = ADI_MPOLY_OP;
	break;

    case DB_GEOMC_TYPE:
	*adi_oid = ADI_GEOMC_OP;
	break;
