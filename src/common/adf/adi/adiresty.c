...
**	    The SUBSTR functions can return NULL based on data input.
**	26-Aug-2011 (kiria01) SIR 125693
**	    Add NULL returning functions support for date_format(date,fmt),
**	    last_day(date), unix_timestamp(date) and from_unixtime(i, fmt)
*/
...
DB_DT_ID	*adi_res_dt)
{
    ...
    *adi_res_dt = adi_fidesc->adi_dtresult;
    ...
    else if (op == ADI_SUBSTR_OP && adi_numdts == 3 ||
	    op == ADI_DATEFMT_OP ||
	    op == ADI_LASTDAY_OP ||
	    op == ADI_UNXTMSTMP_OP && adi_numdts == 1 ||
	    op == ADI_FROMUNXTM_OP && adi_numdts == 2)
    {
	/* substr(s,p,l) needs to be nullable */
	/* date_format(date,fmt) needs to be nullable */
	/* last_day(date) needs to be nullable */
	/* unix_timestamp(date) needs to be nullable */
	/* from_unixtime(i,fmt) needs to be nullable */
