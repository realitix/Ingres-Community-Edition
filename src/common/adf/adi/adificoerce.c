...

/*{
** Name: adi_ficoerce_full() - Combine adi_ficoerce, adi_fidesc and dt_family processing
**
** Description:
**      This function is the external ADF call "adi_ficoerce_full()" which
**      returns a fixed up copy of a full description of a function
**      instance for a requested coercion and applies dt_family processing to it.
**
...
**
**	Unlike adi_ficoerce and adi_fidesc on their own, this routine knows about
**	dt_family types and hence will copy the block whose address adi_fidesc
**	would have returned and will perform the necessary checks and fixups for
**	dt_family processing.
**
...
**                                      given datatypes, this will be
**                                      returned as ADI_NOCOERCION.
**	adi_fdesc                       Address to recieve the full copy.
**
**      Returns:
**	      The following DB_STATUS codes may be returned:
**	    E_DB_OK, E_DB_WARN, E_DB_ERROR, E_DB_SEVERE, E_DB_FATAL
...
**          Initial creation. Combining adi_ficoerce(), adi_fidesc() and
**	    dt_family processing relevant to coercions.
*/
	...
	ADI_FI_DESC	*adi_fdesc)
{
    ...
    if (status = adi_ficoerce(adf_scb, adi_from_did, adi_into_did, &fid))
	return status;
    if (adi_fid)
	*adi_fid = fid;
    if ((u_i4)fid > Adf_globs->Adi_fi_biggest ||
		(fi = (ADI_FI_DESC *)Adf_globs->Adi_fi_lkup[
				ADI_LK_MAP_MACRO(fid)].adi_fi) == NULL)
	return (adu_error(adf_scb, E_AD2010_BAD_FIID, 0));

    return adi_coerce_dtfamily_resolve(adf_scb, fi, adi_from_did, adi_into_did,
			adi_fdesc);
}
