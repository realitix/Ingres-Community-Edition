...
**          Do not return a function if one of the FI returned has
**          ADI_F2097152_PARAM_BAD_TYPE_COMBO set.
**	18-Jul-2011 (kiria01) b125530
**	    Bias long coercions towards keeping their base type the same
**	    so that choosing between FN(LVCH) and FN(LBYTE) when passed
**	    VBYTE would choose the latter. This had been thought to be
**	    what happened but there had been no such linkage between
**	    long and scalar types to direct this behavior - instead the
**	    ranking scheme got used & varchar got the bias. The expected
**	    behavior is crucial to the spatials coercing predictably.
**/
...
**          Protype cleanup.
**	22-Sep-2011 (kiria01) b125776
**	    Support ADI_COMPAT_IDT and DB_ALL_TYPE with
**	    ADI_F2097152_PARAM_BAD_TYPE_COMBO to block other string types
**	    without having to represent all string forms.
**	17-Oct-2011 (kiria01) b125852
**	    Extend checks disambiguating between FIs that appear equally
**	    weighted when a string could coerce to a type or to its base
**	    type - we choose the less restrictive. We also correct the
**	    type class of dates and allow DB_DTE_TYPE to straddle DB_DATE_CLASS
**	    and DB_INTV_CLASS. DB_C_CLASS is removed as redundant.
**	03-Nov-2011 (kiria01) b125919, m2836
**	    Make the SUB operator bias apply to ANSI forms as well as DTE.
*/
...
bool		varchar_precedence)
{
    ...
    i4			itcl[ADI_MAX_OPERANDS];
    i4			istcl[ADI_MAX_OPERANDS];
	...
	istcl[i] = ad0_type_storage_class(idts[i]);
	itcl[i] = ad0_type_class(idts[i]);
	...
	if (unicode_op == -1 && itcl[i] == DB_NCHAR_CLASS)
	    unicode_op = i;
...
/* look thru fi table for operations with matching number of inputs */
	...
	*/
	...
	if (( (curr_fi->adi_fiflags & ADI_F128_VARARGS) == 0 && num_cdts != num_idts)
	  || ( (curr_fi->adi_fiflags & ADI_F128_VARARGS) && num_cdts > num_idts) )
        ...
        if (curr_fi->adi_fiflags & ADI_F2097152_PARAM_BAD_TYPE_COMBO)
        {
	    bool all_match = TRUE;
	    for (j = 0; j < num_cdts; j++)
	    {
		if (curr_fi->adi_dt[j] != idts[j] &&
			curr_fi->adi_dt[j] != DB_ALL_TYPE && (
			(curr_fi->adi_fiflags & ADI_COMPAT_IDT) == 0 ||
			!dt_iscompat(curr_fi->adi_dt[j], idts[j])))
		{
		    all_match = FALSE;
		    break;
		}
	...
	*/
	...
	{
	    i4 cstcl = ad0_type_storage_class(cdts[curr_dt]);
		...
		*/
		if (istcl[0] >= DB_LV_SCLASS ||
			istcl[1] >= DB_LV_SCLASS)
		    /* Don't support LONGs and LOCators */
		    break;
		if (cl_0 == DB_NUMB_CLASS && idts[0] != DB_MNY_TYPE &&
			(cl_1 == DB_CHAR_CLASS || cl_1 == DB_NCHAR_CLASS
			) ||
		    cl_1 == DB_NUMB_CLASS && idts[1] != DB_MNY_TYPE &&
			(cl_0 == DB_CHAR_CLASS || cl_0 == DB_NCHAR_CLASS)
			)
		{
	    ...
	    /* If coercing to a long datatype... */
	    if (cstcl == DB_LV_SCLASS)
	    {
		/* ... and depending on whether the is a type conversion...*/
		if (ad0_type_class(cdts[curr_dt]) == itcl[curr_dt])
		{
		    /* ... allow Locator types to be a cheap coerce to Long */
		    if (istcl[curr_dt] == DB_LOC_SCLASS)
			continue;
		}
		else if (istcl[curr_dt] != DB_LV_SCLASS)
		{
		    /* ... bias towards matching datatype. This is particularly
		    ** important for spatials which rely on distinquishing binary
		    ** from text */
		    curr_coerces++;
		}
	    }
		...
		*/
		    ...
		    itcl[curr_dt] == DB_NUMB_CLASS)
		{
		    ...
		    */
			...
			curr_fi->adi_fiopid == ADI_SUB_OP &&
			(ad0_type_class(cdts[0]) == DB_DATE_CLASS ||
			 ad0_type_class(cdts[1]) == DB_DATE_CLASS))
		{
	...
	*/
	if (curr_coerces == best_coerces  &&  curr_rank == best_rank)
	{
	    ...
	    */
		...
		{
		    i4 bi_tc = ad0_type_class(best_fi->adi_dt[par]);
		    i4 ci_tc = ad0_type_class(cdts[par]);
		    switch (state)
		    {
		    case SEEKING:
			state = FOUND1;
			if ((bi_tc != ci_tc &&  /* DTE should match INTV too */
			    (best_fi->adi_dt[par] != DB_DTE_TYPE || ci_tc != DB_INTV_CLASS) &&
			    (cdts[par] != DB_DTE_TYPE || bi_tc != DB_INTV_CLASS)))
			    state = REJECT;
			else if (ad0_type_storage_class(best_fi->adi_dt[par]) < ad0_type_storage_class(cdts[par]))
			    state = KEEP;
			/* If the input is a string type and the choice is between a type
			** and a family type, choose the latter, less restrictive form. */
			else if (itcl[par] == DB_CHAR_CLASS &&
				best_fi->adi_dt[par] == adi_dtfamily_retrieve(best_fi->adi_dt[par]) &&
				best_fi->adi_dt[par] == adi_dtfamily_retrieve(cdts[par]))
			    state = KEEP;
			break;
		    case REJECT:
			break;
			...
			state = REJECT;
                ...
                if (istcl[par] == ad0_type_storage_class(cdts[par]) &&
		    istcl[par] != ad0_type_storage_class(best_fi->adi_dt[par]))
		{
		    state = USE;
		    break;
		}
...
**	27-jan-2011 (dougi)
**	    Hack change to allow date() func on varchars.
*/
DB_STATUS
...
)
{
	...
	if (fdt != 0 && oldfidesc->adi_dt[0] == fdt)
	{
		...
		(idts[0] != DB_ADTE_TYPE || idts[1] != DB_VCH_TYPE))
		/* datatype of more than one family cannot be resolved.
		** varchar is exempted in hope that this is just a coercion
		** to ansidate. */
      		return(adu_error(adf_scb, E_AD9998_INTERNAL_ERROR, 0));
	}
	if (fdt == DB_DTE_TYPE)
...
**      09-nov-2010 (gupsh01) SIR 124685
**          Protype cleanup.
**	28-Mar-2011 (kiria01) b125223
**	    Move abs() call on output to before it is indexed off.
*/
...
)
{
    ...
    if (in_fdt && (input != fi_input) && (fi_input == in_fdt))
    {
      /* replace the FI input data type with input */
      newfidesc->adi_dt[0]= input;
    }
    ...
    if ((output != fi_output) && (fi_output == out_fdt))
    {
      /* replace the FI result data type with output */
      newfidesc->adi_dtresult = output;
