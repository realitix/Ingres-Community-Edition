/*
** Copyright (c) 1986 - 1999,2008,2010 Actian Corporation
*/
...
**	    Moved fix for b116215 out of here concentrating logic in resolver.
**	28-Mar-2011 (kiria01) 125223
**	    Correct glaring Unicode min_length error.
**	04-Jun-2011 (kiria01) SIR 125419
**	    Add REPEAT function length checks and correct handling of NULL length
**      07-Jun-2011 (coomi01) b125400
**          Add ADI_DPART_DECIMAL_OP calculation
**      04-Jul-2011 (coomi01) b125396
**          Add ADI_SCALE to allow us to scale casts.
**	28-Jul-2011 (kiria01) b125530 
**	    Cater for signalling coercions that have 3 params and need treating
**	    for constant lengths too.
**	09-Aug-2011 (kiria01) b125634
**	    Cleaned up Unicode coercion related lenspecs that
**	    overly inflated the results size assuming we would
**	    always write to NFD.
**	26-Aug-2011 (kiria01) SIR 125693
**	    Add ADI_DATEFMT for date_format and from_unixtime.
**      10-Oct-2011 (smeke01) b125834
**          Add ADI_INTMATH_FIXED for integer arithmetic. This allows us
**          to determine result length as either 'ADI_FIXED 8' or
**          'ADI_4ORLONGER' depending on adf_scb->adf_misc_flags
**          set via cbf parameter int_math_i8_result. Also remove the integer
**          results downgrade from ADI_4ORLONGER and the case for ADI_IMULI,
**          ADI_IADDI, ADI_ISUBI and ADI_IDIVI which should have been removed
**          when the fix for bug 118798 was replaced with the fix for bug
**          119368.
*/
DB_STATUS
adi_0calclen(
...
DB_DATA_VALUE      *adi_dvr)
{
    ...
    bool		nullable = FALSE;
    i4 			min_length;
    /* Lenspecs that inflate due to target being NFD use: */
    i4			nfd_factor = adf_scb->adf_uninorm_flag == AD_UNINORM_NFD
				? 4 : 1;
    /* Lenspecs that inflate due to target being utf8 use: */
    i4			utf8_factor = (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
				? DB_MAXUTF8_EXP_FACTOR : 1;
    if (abs(adi_dvr->db_datatype) == DB_NVCHR_TYPE || abs(adi_dvr->db_datatype) == DB_NCHR_TYPE)
    ...
    case ADI_CAST:
	    ...
	    if (DB_INT_TYPE != adi_dv[1]->db_datatype || !adi_dv[1]->db_data)
		return E_DB_ERROR;
	    if (adi_dv[1]->db_length == (i4)sizeof(i2))
		scale = *(u_i2 *)adi_dv[1]->db_data;
	    else if (adi_dv[1]->db_length == (i4)sizeof(i4))
		scale = *(u_i4 *)adi_dv[1]->db_data;
	    else
		return E_DB_ERROR;	

	    if (scale > 0xffff)
		return E_DB_ERROR;
	    local_lenspec.adi_lncompute = ADI_FIXED;
	    local_lenspec.adi_psfixed   = scale;
	    local_lenspec.adi_fixedsize = adi_lenspec->adi_fixedsize;
	    rprecSet = TRUE;
        }	
	break;
	...
	case ADI_RPT:
	    /* length of arg #1 * n + DB_CNTSIZE */
	    rprec = 0;
	    rlen = len[0];
	    if (rlen != ADE_LEN_UNKNOWN && !data[1])
		if (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
		    rlen = DB_UTF8_MAXSTRING;
		else
		    rlen = DB_CHAR_MAX;
	    break;
	...
	case ADI_RESLEN:
	    /* length and precision should already be known ... just return */
	    if (adi_dvr->db_length == ADE_LEN_UNKNOWN)
		return (adu_error(adf_scb, E_AD2022_UNKNOWN_LEN, 0));
	    else
		return (E_DB_OK);
	...
	case ADI_COUPON:
	{
	    ...
	    {
		/* Will either be var->var or non-var->no-var so if the former
		** fiddle with the count size (can only be ->DB_NVCHR_TYPE) */
		if (abs(adi_dvr->db_datatype) == DB_NVCHR_TYPE)
		    rlen = max(2, nfd_factor*(len[0]-DB_CNTSIZE)*sizeof(UCS2)+DB_CNTSIZE);
		else
		    rlen = max(2, nfd_factor*len[0]*sizeof(UCS2));

		if (adf_scb->adf_utf8_flag & AD_UTF8_ENABLED)
		    rlen = min(DB_UTF8_MAXSTRING, rlen);
		else
		    rlen = min(DB_CHAR_MAX, rlen);
	    }
            break;
        }
		...
		/* Will either be var->var or non-var->no-var so if the former
		** fiddle with the count size (can only be DB_NVCHR_TYPE->) */
		if (abs(adi_dv[0]->db_datatype) == DB_NVCHR_TYPE)
		    rlen = max(2, utf8_factor*((len[0]-DB_CNTSIZE)/sizeof(UCS2))+DB_CNTSIZE);
		else
		    rlen = max(2, utf8_factor*(len[0]/sizeof(UCS2)));
	    ...
	    {
		rlen = max(2, (nfd_factor*len[0]*sizeof(UCS2) + DB_CNTSIZE));
	    ...
	    {
		rlen = max(2, (nfd_factor*len[0]-DB_CNTSIZE)*sizeof(UCS2));
	    ...
	    {
		rlen = max(2, utf8_factor*(len[0]/sizeof(UCS2)) + DB_CNTSIZE);
	    ...
	    {
		rlen = max(2, (utf8_factor*(len[0]-DB_CNTSIZE)/sizeof(UCS2)));
	...
	case ADI_DATEFMT:
            rprec = 0;
	    if (len[1] == ADE_LEN_UNKNOWN)
		rlen = len[1];
	    else if (adi_numops == 2 &&
		    adi_dv[1]->db_data != NULL &&
		    abs(adi_dv[1]->db_datatype) == DB_VCH_TYPE)
	    {
		i4 l = local_lenspec.adi_fixedsize;
		adu_datefmt_calclen(adf_scb, adi_dv[1], &l);
		rlen = l + DB_CNTSIZE;
	    }
	    else
		rlen = local_lenspec.adi_fixedsize;
	...
	case ADI_INTMATH_FIXED:
	{
	    if ((adf_scb->adf_misc_flags & ADF_INTMATH_4ORLONGER) == 0)
	    {
		/* equivalent to ADI_FIXED, for integer maths ops only */
		rprec = local_lenspec.adi_psfixed;
		rlen  = local_lenspec.adi_fixedsize;
	    }
	    else
	    {
		/* equivalent to ADI_4ORLONGER, for integer maths ops only */
		rprec = local_lenspec.adi_psfixed;
		rlen = 4;	/* (to start) */
		if (len[0] > rlen)
		    rlen = len[0];
    ...
    */
	...
	if( (fi_desc->adi_fiopid == ADI_VARCH_OP || 
	     ...
	     fi_desc->adi_fiopid == ADI_NVCHAR_OP ||
	     fi_desc->adi_fiopid == ADI_REPEAT_OP) &&
	   abs(adi_dv[1]->db_datatype) == DB_INT_TYPE)
	{
	    i4 minlen = sizeof(char);
	    i4 nullen = 0;
	    if (ADF_ISNULL_MACRO(adi_dv[1]))
		rlen = 1;
	    else switch(adi_dv[1]->db_datatype < 0
			? adi_dv[1]->db_length-1
			: adi_dv[1]->db_length)
	    {
		case 1:
	    ...
	    if (fi_desc->adi_fiopid == ADI_NCHAR_OP ||
	        fi_desc->adi_fiopid == ADI_NVCHAR_OP ||
		fi_desc->adi_fiopid == ADI_REPEAT_OP &&
			abs(adi_dv[0]->db_datatype) == DB_NVCHR_TYPE)
	    {
	    	rlen *= sizeof(UCS2);
	    	minlen = sizeof(UCS2);
	    }

	    if (fi_desc->adi_fiopid == ADI_REPEAT_OP)
		rlen *= (adi_dv[0]->db_length - DB_CNTSIZE);
...
**      20-Apr-2011 (hanal04) Bug 124913
**          varchar(null) leads to a 0 length nullable DB_TXT_TYPE constant
**          being generated for input into the varchar routine. Use
**          ADI_ADD1 to ensure the result string meets the minimum length of
**          one. Do the same for text(null) and varbyte(null).
*/
static DB_STATUS
adi_1len_indirect( 
ADI_OP_ID          fiopid,
...
ADI_LENSPEC        *out_lenspec) 
{
	...
	switch(abs(dv1->db_datatype))
	{
	  ...
	  case DB_BYTE_TYPE:
	    ...
	    break;
	  case DB_LTXT_TYPE:
              if(dv1->db_datatype < 0 && dv1->db_length == 3)
              {
                  /* We have an internal TXT null, avoid a 0 length result */
                  out_lenspec->adi_lncompute = ADI_ADD1;
                  break;
              } /* else fall through */
	  case DB_VCH_TYPE:
	  case DB_TXT_TYPE:
	...
	switch(abs(dv1->db_datatype))
	{
          case DB_VCH_TYPE:
	  case DB_NVCHR_TYPE:
            out_lenspec->adi_lncompute = ADI_RPT;
            break;
	default:
	    return E_DB_ERROR;
	}
      ...
      case ADI_DPART_DECIMAL_OP:
	{
		...
		{
		  out_lenspec->adi_lncompute = ADI_FIXED;
		  out_lenspec->adi_fixedsize = DB_PREC_TO_LEN_MACRO(dv2->db_prec+2);
		  out_lenspec->adi_psfixed   = DB_PS_ENCODE_MACRO(dv2->db_prec+2,dv2->db_prec);
		}
		break;
