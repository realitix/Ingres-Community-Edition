/*
** Copyright (c) 1986, Actian Corporation
*/
...
**					table creation.)
**
**	collID				Collation ID to be used for spec. If
**					unset use DB_UNSET_COLL.
**
**	def_collID[2]			Collation ID defaults. [1] will apply
**					to NCHAR, NVARCHAR and LONG NVARCHAR,
**					[0] for the other string datatypes.
**					If no defaults are to be attempted, pass
**					as NULL.
**
** Outputs:
**	adf_scb				Pointer to an ADF session control block.
...
**	    Add missing collID initialisation.
**	05-Sep-2011 (kiria01) b125573
**	    Update adi_encode_colspec call for collation data.
**	    Callers needing to set collation information could not
**	    apply correct defaults due to not knowing actual datatype
**	    until this routine had returned. However, this routine
**	    needed to know chosen or derived collation before the length
**	    checks were applied so that logic has been moved to where it
**	    can be made.
*/
...
DB_DATA_VALUE	    *dv)
{
	...
	*/
    ...
    if ((idv.db_collID = collID) == DB_UNSET_COLL &&
	def_collID != NULL)
    {
	switch (abs(idv.db_datatype))
	{
	case DB_CHA_TYPE:
	case DB_CHR_TYPE:
	case DB_VCH_TYPE:
	case DB_TXT_TYPE:
	case DB_LVCH_TYPE:
	    idv.db_collID = def_collID[0];
	    break;
	case DB_NCHR_TYPE:
	case DB_NVCHR_TYPE:
	case DB_LNVCHR_TYPE:
	    idv.db_collID = def_collID[1];
	    break;
	}
    }
