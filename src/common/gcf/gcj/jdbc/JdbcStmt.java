/*
** Copyright (c) 1999, 2011 Actian Corporation All Rights Reserved.
*/
...
**	QueryIdentity	Identity query execution.
**
** History:
**	13-May-99 (gordy)
**	    Created.
...
**	15-Feb-11 (gordy)
**	    Add support for identity columns as generated keys.
*/
...
**	loadGeneratedKeys	Perform query to load generated keys.
...
**  Private Data:
**
**	queryIdentity		Execute query for identity value.
...
**	rsmdIdentity		Result-set meta-data for identity.
...
**	15-Feb-11 (gordy)
**	    Added QueryIdentity class and local instance queryIdentity 
**	    to provide access to identity values.  Added rsmdIdentity 
**	    for generated key result set meta-data for identity values.  
**	    Removed rsEmpty which provided an empty result set when no
**	    generated key was received, but failed subsequently since
**	    it got closed by first usage.
*/
...
**	15-Feb-11 (gordy)
**	    Added generated key parameter to exec().
*/
    ...
    throws SQLException
{
    ...
    exec( sql, UPDATE, (autoGeneratedKeys == RETURN_GENERATED_KEYS) );
    int count = ((rslt_items & RSLT_ROW_CNT) != 0) ? rslt_val_rowcnt : 0;
    if ( trace.enabled() )  trace.log( title + ".executeUpdate(): " + count );
    return( count );
} // executeUpdate
    ...
    throws SQLException
{
    ...
    exec( sql, UPDATE, (columns != null  &&  columns.length > 0) );
    int count = ((rslt_items & RSLT_ROW_CNT) != 0) ? rslt_val_rowcnt : 0;
    if ( trace.enabled() )  trace.log( title + ".executeUpdate(): " + count );
    return( count );
} // executeUpdate
    ...
    throws SQLException
{
    ...
    exec( sql, UNKNOWN, (autoGeneratedKeys == RETURN_GENERATED_KEYS) );
    boolean isRS = (resultSet != null);
    if ( trace.enabled() )  trace.log( title + ".execute(): " + isRS );
    return( isRS );
} // execute
    ...
    throws SQLException
{
    ...
    exec( sql, UNKNOWN, (columns != null  &&  columns.length > 0) );
    boolean isRS = (resultSet != null);
    if ( trace.enabled() )  trace.log( title + ".execute(): " + isRS );
    return( isRS );
} // execute
...
**	15-Feb-11 (gordy)
**	    If no table/object key is available, provide result set
**	    for identity value if available.  The queryIdentifier
**	    helper object reference will be null if no identity
**	    query has been issued or the object will be marked as
**	    null if no identity value was available.  Do not cache 
**	    an empty result set when no generated key is available 
**	    because it will be closed by the first usage and unusable 
**	    for subsequent requests.  SqlByte must be cleared prior 
**	    to setting new values to ensure it is marked not-null.
*/
    ...
    throws SQLException
{
    ...
    else  if ( queryIdentity != null  &&  ! queryIdentity.isNull() )
    {
	SqlData		data[][];
	SqlBigInt	identity;
	...
	{
	    rsmdIdentity = new JdbcRSMD( (short)1, trace );
	    rsmdIdentity.setColumnInfo( "identity", 1, (int)Types.BIGINT,
					(short)DBMS_TYPE_INT,
					(short)8, (byte)0, (byte)0, false );
	}

	data = new SqlData[1][];
	data[0] = new SqlData[1];
	    
	identity = new SqlBigInt();
	identity.set( queryIdentity.getLong() );
	data[0][0] = identity;

	rs = new RsltData( conn, rsmdIdentity, data );
    }
    else
    {
	if ( rsmdEmpty == null )
	...
	rs = new RsltData( conn, rsmdEmpty, null );
    }
...
**	15-Feb-11 (gordy)
**	    Clear table/object keys and identity values.
*/
...
{
    clearQueryResults( true );
    if ( queryIdentity != null )  queryIdentity.clear();
    rslt_items &= ~(RSLT_TBLKEY | RSLT_OBJKEY);
    super.clearResults();
    return;
} // clearResults
...
**	15-Feb-11 (gordy)
**	    Add loadGeneratedKeys parameter to enable access to
**	    generated keys.  If enabled, call loadGeneratedKeys()
**	    for non-query statements to access the generated keys.
*/

private void
exec( String text, int mode, boolean loadGeneratedKeys  )
    throws SQLException
{
	...
	else
	{
	    ...
	    */
	    ...
	    {
		/*
		** Connection was locked above, but may be unlocked
		** during generated key processing.
		*/
		boolean locked = true;

		/*
		** Load generated keys at this point if requested.
		*/
		if ( loadGeneratedKeys )  
		{
		    loadGeneratedKeys();

		    /*
		    ** Check to see if connection is 
		    ** still locked by this thread.
		    */
		    locked = msg.isLocked( false );
		}

		if ( locked )  msg.unlock();	// We be done.
	    }
	    else
	    {
		/*
...
/*
** Name: loadGeneratedKeys
**
** Description:
**	Checks driver configuration and current query results
**	to determine if query should be issued to retrieve
**	identity column value.  Local class is used to issue
**	query when determined to be necessary.
**
**	Connection is not locked/protected by this method.
**	Callers is responsibility to determine/ensure that
**	it is appropriate and safe to issue the query.
**
** Input:
**	None.
**
...
{
    /*
    ** Identity columns are supported at API connection
    ** level 6.  Retrieve identity value if enabled and
    ** no table or object key was received.
    */
    if ( conn.db_protocol_level >= DBMS_API_PROTO_6  &&
	 (conn.cnf_flags & conn.CNF_IDENTITY) != 0  &&
         (rslt_items & (RSLT_TBLKEY | RSLT_OBJKEY)) == 0 )
    {
	if ( queryIdentity == null )  
	    queryIdentity = new QueryIdentity( conn );

	queryIdentity.execute();
    }
...
**
** Description:
**	Class which implements identity query execution.
**
**  Public Methods:
**
**	execute			Execute identity query.
**	clear			Clears results (isNull() will return true).
**	isNull			Returns indication that no value is available.
**	getLong			Returns identity value.
**
**  Private Data:
**
**	isNull			Is value available.
**	value			Identity value.
**
**  Private Methods:
**
**	readDesc		Read descriptor message.
**	readResult		Read result message.
**
** History:
**	15-Feb-11 (gordy)
...
public
QueryIdentity( DrvConn conn )
{
    super( conn );

    title = trace.getTraceName() + "-QueryIdentity[" + msg.connID() + "]";
    tr_id = "Identity[" + msg.connID() + "]";
...
**
** Description:
**	Issue query to retrieve identity value.  If identity value
**	is retrieved, isNull() will subsequently return false and
**	the value can be obtained by getLong().  Otherwise, isNull()
**	will return true.
**
** Input:
**	None.
...
execute()
{
	...
	msg.write( MSG_QP_FLAGS );
	msg.write( (short)2 );
	msg.write( (short)(MSG_QF_AUTO_CLOSE | MSG_QF_FETCH_ALL ) );
	writeQueryText( "select last_identity()" );
	msg.done( true );

	if ( (rsmd = readResults()) != null )
	{
	    JdbcRslt resultSet = new RsltSlct( conn, null, rsmd, 
					       rslt_val_stmt, 2, 
					       msg.moreMessages() );
	...
	if ( trace.enabled( 1 ) )
	{
	    trace.write( tr_id + ": error retrieving identity value" );
...
**
** Description:
**	Clears identity result value.  Method isNull() will return
...
**
** Description:
**	Returns false if last call to execute() retrieved an
**	identity value.  Otherwise, returns true.
**
** Input:
**	None.
**
...
**
** Returns:
**	boolean		True if no identity value is available.
**
** History:
**	15-Feb-11 (gordy)
**	    Created.
...
**
** Description:
**	Returns the identity value retrieved by the last call
**	to execute().  Value is undefined if method isNull()
**	returns true.
**
** Input:
**	None.
...
** Description:
**	Read a query result DESC message.  Overrides the default method 
**	in DrvObj since descriptors are expected.
**
** Input:
**	None.
**
...
** Description:
**	Read a query result RESULT message.  Overrides the default method
**	in DrvObj to interrupt processing.
**
** Input:
**	None.
**
