...
**	15-Feb-11 (gordy)
**	    Add method isLocked() to determine connection lock status.
*/
...
**
** Description:
**	Returns indication that connection is currently locked.
**	The anyThread parameter permits caller to request that
**	lock indication pertains to any thread holding the lock,
**	otherwise the lock indication will only be true if the
**	current thread holds the lock.
**
** Input:
**	anyThread	Is connection locked by any thread or current thread.
**
** Output:
**	None.
**
** Returns:
**	boolean		True if connect locked by any/current thread.
**
** History:
**	15-Feb-11 (gordy)
**	    Created.
...
isLocked( boolean anyThread )
{
    ...
    {
	if ( lock_active )
	    locked = anyThread ? true : (lock_thread == Thread.currentThread());
    }
