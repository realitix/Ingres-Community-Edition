/*
** Copyright (c) 1987, 2011 Actian Corporation All Rights Reserved.
*/
...
**	26-Jan-2011 (kiria01) b124958
**	    Use MO_MAX_NUM_BUF definition to avoid buffer overrun.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**	15-Aug-11 (gordy)
**	    Make sure username and password are not zero length.
*/
...
**	15-Aug-11 (gordy)
**	    Make sure username is not zero length.
*/
	...
	if (ls_parms->gca_status == OK)
	{
	    ...
	    */
		...
		if ( ls_parms->gca_user_name  &&  *ls_parms->gca_user_name )
		{
		    ...
		    if ( ccb->ccb_hdr.username  &&  *ccb->ccb_hdr.username )
...
**	15-Aug-11 (gordy)
**	    Make sure password is not zero length.
*/
...
# endif /* GCXDEBUG */
	...
	*/
		   ...
		   ! (ccb->ccb_hdr.flags & CCB_USERNAME)  || 
		   ! ( ccb->ccb_hdr.authlen  ||
		       ( ccb->ccb_hdr.password  &&  *ccb->ccb_hdr.password ) )
		 )
	{
...
**          Implemented calls to GCnxxx functions to convert individual types
**          from network xfer syntax.
**	17-Dec-10 (rajus01 for gordy) SD issue 147532, Bug 124835
**	    When DBMS server abruptly releases a remotely connected
**	    replicator session it results in E_GC2205_RMT_ABORT
**	    /E_CLFE07_BS_READ_ERR errors.  Based on the GCA spec,
**	    for GCA protocol levels below GCA_PROTOCOL_LEVEL_3, the error
**	    code is placed gca_id_error and gca_local_error is 0. The
**	    GCC didn't support handling of error messages
**	    based on the GCA protocol specification. This change
**	    takes GCA protocol levels into consideration when building
**	    the error messages.
*/
static VOID
gcc_aaerr( GCC_MDE *mde, GCC_CCB *ccb, STATUS generic_status )
{
	...
	**  and send back to the client comm. server. Set up the generic
	    ...
	    ** are encoded into the gca_id_error element.  Generic error
	    ** codes were introduced at protocol level 3. GCC utilizes 
	    ** the generic error code E_GE9088_COMM_ERROR. This is
	    ** inserted in the gca_id_error element of each GCA_E_ELEMENT.
	    ** The actual GCC error code is inserted in the gca_local_error
	    ** element. Prior to protocol level 3, the actual error 
	    ** is placed in the gca_id_error element and the
	    ** gca_local_error element is unused. gca_severity carries
	    ** various flags, and is set to 0 to indicate that this is an 
	    ** "unformatted" message, i.e., it is in the raw format returned 
	    ** by ERlookup (we will use ule_format for consistency and 
	    ** simplicity; it returns the error text as it comes from ERlookup).
 		...
 		** error code in gca_local_error.
 		*/
 		er_elem.gca_id_error = ss_encode( SS50000_MISC_ERRORS );
 		er_elem.gca_local_error = 
 		...
 		** error code in gca_local_error.
 		*/
 		er_elem.gca_id_error = E_GE9088_COMM_ERROR;
 		er_elem.gca_local_error = 
 				GCC_LGLVL_STRIP_MACRO( tmp_gnrc_status[i] );
 	    }
 	    else
