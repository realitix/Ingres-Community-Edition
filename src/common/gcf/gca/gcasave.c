...
**	    replace nat and longnat with i4
**      11-feb-2011 (stial01) (Mantis 1313)
**          gca_save() if SIwrite fails, make sure we SIclose only once
**/
...
gca_save( GCA_SVC_PARMS *svc_parms )
{
    ...
    {
	GCA_DEBUG_MACRO(1)( "gca_send bad file write\n" );
	svc_parms->gc_parms.status = E_GC0014_SAVE_FAIL;
    }

    /* Close the save file */
    if (SIclose( save_file ) != OK )
    {
	if (svc_parms->gc_parms.status != E_GC0014_SAVE_FAIL)
	{
	    GCA_DEBUG_MACRO(1)( "gca_send bad file close\n" );
	}
