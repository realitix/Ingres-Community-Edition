...
**	    Disallow VWLOAD copies thru the API.  Only allow them thru ESQL.
*/
...
IIapi_readMsgCopyMap( IIAPI_STMTHNDL *stmtHndl )
{
    ...
    */
    ...
    if (map->cp_flags & CPY_VWLOAD)
    {
	IIAPI_TRACE( IIAPI_TR_FATAL )
	    ( "IIapi_readMsgCopyMap: VWLOAD copy not allowed via API\n" );
...
**	25-Mar-10 (gordy)
**	    Created.
**	15-Dec-10 (rajus01 for gordy) SD issue 147532, Bug 124835
...
**	    OpenAPI interfaces didn't support handling of error messages
**	    for GCA protocol level below GCA_PROTOCOL_LEVEL_3.    
**
**	    This change adds support for handling GCA_ERROR messages 
**	    below GCA_PROTOCOL_LEVEL_3. Also it checks for gca_local_error
**	    being zero. 
**
**	    During the investigation of this issue it was also determined that 
**	    GCC improperly builds GCA_ERROR messages without considering 
**	    the GCA protocol levels.
*/
...
IIapi_readMsgError( IIAPI_HNDL *handle )
{
	...
	*/
	...
	/*
	** Prior to GCA_PROTOCOL_LEVEL_3, the server error code
	** is placed in gca_id_error and gca_local_error is 0.
	** The Name Server also follows this convention at all
	** times, irregardless of protocol level.  There is also
	** at least one case in GCC where the GCA_ERROR message
	** is built, incorrectly, this same way. If gca_local_error
	** is 0, it is probably best to assume that the actual
	** error code is in gca_id_error.
	*/
	if (
	    connHndl->ch_partnerProtocol < GCA_PROTOCOL_LEVEL_3 ||
	    connHndl->ch_type == IIAPI_SMT_NS  ||
	    errInfo.svr_local_error == 0
	   )

	{
	    /*
	    ** Move server error code into correct place, and
	    ** provide a generic error code or encoded SQLSTATE.
	    */
