...
**          TRset_file() will now expand %p (pid) and %d (YYYYMMDD).
**          Pass TR_F_OPEN_UPD and the buffer length to request a copy
**          of the expanded file name.
**          Also correct the STprintf() that adds an init count to the 
**          api log file.
**      16-Jun-2011 (hanal04) SIR 125429
**          Remove the init_count from the log file name. If we need more
**          than date and pid a %t could be added to TRset_file() at a later
**          time.
*/
...
IIapi_initTrace( II_VOID )
{
    ...
    if ( retVal  &&  *retVal )
    {
        fnlen = STlength(retVal) + 32;    
        if (IIapi_static->api_trace_file = (char *)
		     MEreqmem( 0, fnlen, FALSE, &status ))
        {
	    STprintf( IIapi_static->api_trace_file, "%s", retVal);
	    TRset_file( TR_F_OPEN_UPD, IIapi_static->api_trace_file, 
		        fnlen, &err_code );
        }
    }
