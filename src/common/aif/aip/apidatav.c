...
**	25-Mar-10 (gordy)
**	    Replaced formatted GCA interface with byte stream.
**	04-Jan-2011 (rajus01) SD issue 148118, Bug 124831.
**	    Investigation of the issue revealed that splitting of GCA TUPLE 
**	    message between LOB locator and NULL indicator and OpenAPI fails 
**	    to read the trailing NULL indicator resulting in 
**	    E_AP0010_INVALID_COLUMN_COUNT error.
*/
...
)
{
	...
	** 
	** In cases when a tuple is split between a LOB
	** locator and the NULL indicator, dv_length will
	** be greater than 0 indicating that header has been 
	** read and therefore no need to skip the header again
	** as it was completed during the first time invoking
	** of this routine. It is only required to process the
	** NULL byte second time.
	*/
	if( ! dataValue->dv_length )
	{
