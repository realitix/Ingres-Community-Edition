...
**   03-March-2011 (Ralph Loen) Bug 125154
**      In SQLDataSources(), bulletproof the ServerName, NameLength1,
**      Description, or NameLength2 arguments if set to NULL.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**   25-Sep-2011 (Bruce Lunsford)
**      Default vendor attribute is now "Actian".
*/ 
...
**    14-jun-04 (loera01)
**      Created.
**    03-Mar-2011 (Ralph Loen) Bug 125154
**      Bulletproof the ServerName, NameLength1, Description, or NameLength2 
...
*/
    ...
    */
    ...
    if (getSys)
    {
        ...
        {
            if (ServerName)
                STcopy(penv->sysDsnList[penv->sysDsnCurCount].dsnName,
                    ServerName);
            if (NameLength1)
                *NameLength1 = 
    ...
    if (getUsr)
    {
        ...
        {
            if (ServerName)
                STcopy(penv->usrDsnList[penv->usrDsnCurCount].dsnName,
                    ServerName);
            if (NameLength1)
                *NameLength1 = 
        ...
        */
        for (j=0;j<attrTblCount;j++)
        {
            ...
            case DSN_ATTR_VENDOR:
                if (!dsnAttrList[j].value[0])
                    STcopy(DSN_STR_ACTIAN_CORP, dsnAttrList[j].value);
                break;
