...
**          Change company name from Ingres Corp to Actian Corp.
**  
*/
    ...
    for (q = ocfg_cb->sect_q.q_prev; q != &ocfg_cb->sect_q; q = q->q_prev)
    {
            ...
            aq = aq->q_prev)
        {
            ...
            if (attrID == DRV_ATTR_VENDOR)
            {
                ...
                if ((attrValue == DRV_VALUE_ACTIAN_CORP) ||
                    (attrValue == DRV_VALUE_INGRES_CORP) ||
                    (attrValue == DRV_VALUE_COMP_ASSOC))
                {
                    got_vendor = TRUE;
                }
            }
...
BOOL isDriverDef ( ATTR_ID *attr_id, i4 count)
{
    i4 i;
    BOOL got_vendor = FALSE, got_driver_type = FALSE;
    i4 attrValue = OCFG_UNKNOWN, driverValue = OCFG_UNKNOWN;

    for (i = 0; i < count; i++)
    { 
        if ( attr_id[i].id == DRV_ATTR_VENDOR )
        {
            attrValue = getAttributeID(driverValues, attr_id[i].value);
            if ((attrValue == DRV_VALUE_ACTIAN_CORP) ||
                (attrValue == DRV_VALUE_INGRES_CORP) ||
                (attrValue == DRV_VALUE_COMP_ASSOC))
                    got_vendor = TRUE;
