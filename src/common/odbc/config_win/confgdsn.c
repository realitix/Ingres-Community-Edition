/*
** Copyright 2010 Actian Corporation. All Rights Reserved.
*/
...
**     25-Jul-2011 (Ralph Loen) Bug 125588
**          In TestConnection(), use SQLGetDiagRec() to display up to 4
**          errors messages instead of SQLGetError().
**         
*/
...
{      
    ...
    DlgbPrompt = (IsDlgButtonChecked(hDlg, IDC_PROMPTUIDPWD) 
        == BST_CHECKED);
    /*
    ** Clear the ODBC.INI DSN of cached capabilities to force the driver to
    ** re-issue the queries to get them. 
    */
    ClearDSNCacheCapabilities(szDSN);  
    ...
    if (retcode == SQL_SUCCESS) 
    {
        ...
        if (retcode == SQL_SUCCESS) 
        {
            ...
            if (SQL_SUCCEEDED(retcode))
            {
                ...
                MessageBox(hDlg, (LPCTSTR) ERget(F_OD0132_IDS_MSG_CONNECTOK), 
                    szMsgTitle, MB_OK);
                SQLDisconnect(hdbc);
            }
            else
            {   
                ...
                {
                    i = 1;
                    while ( SQLGetDiagRec( SQL_HANDLE_DBC, hdbc, i, szSqlState,
                        &fNativeError, szErrorMsg, SQL_MAX_MESSAGE_LENGTH + 1,
                        &pcbErrorMsg ) == SQL_SUCCESS )
                    {
                        MessageBox(hDlg, szErrorMsg, (LPCTSTR) 
                            ERget(F_OD0133_IDS_MSG_CONNECTFAILED), 
                            MB_OK | MB_ICONERROR);
                        i++;
                        if (i > 4)
                            break;
...
#define MSG_DATABASEBROWSE " - Database Name Browse"
                ...
                if (SQL_SUCCEEDED(rc))
                {
                    if (strcmp(DlgszServerType, "STAR") == 0)
                    {
                        rc = SQLExecDirect(hstmt, 
                            "select ddb_name from iistar_cdbinfo", SQL_NTS);
                    }
                    else
                    {	
                        rc = SQLExecDirect(hstmt, 
                            "select name from iidatabase", SQL_NTS);
                    }
                }
                if (SQL_SUCCEEDED(rc))
                    rc = SQLFetch(hstmt);
    ...
    {
        if (CB_ERR == SendDlgItemMessage(hDlg, IDC_DBNAME, 
            CB_FINDSTRINGEXACT, (WPARAM) -1, (LPARAM) (LPCSTR) p))
