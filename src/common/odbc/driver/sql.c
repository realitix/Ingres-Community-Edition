...
**    02-Jan-2011 (frima01)
**          Correct compile error in odbc_scrollColumns.
**    27-Jan-2011 (Ralph Loen) Bug 124939
**          Return a RETCODE rather than a BOOL from odbc_retrieveByRefParms().
**          In odbc_query_sm(), retrieve the status from
**          odbc_retrieveByRefParms(), and propagate to the caller.
**    27-Jan-2011 (Ralph Loen) Bug 124939
**          In odbc_getByRefParams() set IIAPI_GETCOLPARM before any exit
**          paths are executed, to prevent possible ACCVIO's or uninitialized
**          variable errors.
**    28-Jan-2011 (Ralph Loen) Bug 124970
**          In odbc_query_sm(), for the QRY_QUERY state, for QST_PREPARE query 
**          types, invoke odbc_getQInfo() (QRY_GETQI) and odbc_close() 
**          (QRY_CLOSE) after fetching the descriptor (QRY_GETD).
**     15-Feb-2011 (Ralph Loen) Bug 125041
**         Fix compiler warnings on 64-bit Windows.
**    21-Mar-2011 (Bruce Lunsford) Bug 125212
**          If varchar compression is turned off in DBMS, connect requests
**          sometimes crash due to buffer overrun of szUnicode field in
**          GetServerInfo().
**    17-Jun-2011 (Ralph Loen) Bug 125457
**          In GetServerInfo(), retrieve the Vectorwise version independently
**          from fRelease in the connection block (but leave fRelease
**          unchanged).
**    18-Jun-2011 (Ralph Loen) Bug 125457
**          In GetServerInfo(), chop off the 'VW' part of the version
**          string, for compatibility with Ingres. 
**    18-Jun-2011 (Ralph Loen) Bug 125457
**          Trim off any leading spaces from the VW version in GetServerInfo().
**    27-Jul-2011 (Bruce Lunsford) Bug 125522
**          For 64-bit, change data type of sqlulength0 static field from
**          SQLUINTEGER to SQLULEN for 64-bit.  Similar for rowCount and
**          OctetLength.
*/
...
static  SQLULEN sqlulength0 = 0;  /* a convenient const ulen of zero */
...
static void GetCursorSelectStmtHandle(LPSTMT pstmt)
{
	...
	while (pNextStmt)
	{
		if ( (pNextStmt->fCommand == CMD_SELECT) &&
			 !STncasecmp(pNextStmt->szCursor, pstmt->szCursor, sizeof(pstmt->szCursor)))
		{
...
static BOOL odbc_setDescr( LPSTMT pstmt, IIAPI_QUERYTYPE apiQueryType )
{
    ...
    i2        fSqlType;
    SQLLEN    OctetLength;         /* Length in bytes of char or binary data*/
...
static BOOL odbc_scrollColumns(SESS *pSession, LPSTMT pstmt)
{
    ...
    rowCount = pstmt->pARD->ArraySize; 
    getColParm.gc_rowCount = (i2)rowCount;
    getColParm.gc_columnCount = pIRD->Count;
    getColParm.gc_columnData = NULL;
    getColParm.gc_columnData = ( IIAPI_DATAVALUE * )
        odbc_malloc( sizeof( IIAPI_DATAVALUE ) *
    ...
    */
    ...
    case SQL_FETCH_LAST:
        ...
        posParm.po_offset = -(i4)(pstmt->pARD->ArraySize);
        scroll_cursor = FALSE;
        break;
    ...
    case SQL_FETCH_PRIOR:
        ...
        {
            posParm.po_offset =  -(i4)(pstmt->fCursLastResSetSize + 
                pstmt->pARD->ArraySize);
        }
        else
            posParm.po_offset = -(i4)(2 * pstmt->pARD->ArraySize) + 1;
        scroll_cursor = FALSE;
        break;
    ...
    posParm.po_rowCount = (i2)rowCount;
...
** History:
**  27-Jul-2011 (Bruce Lunsford) Bug 125522
**      Remove unneeded code that sets local variable cbTotal,
**      which is not then later used in this function.
*/
...
**  27-Jul-2011 Bruce Lunsford Bug 125522
**      Change data type of OctetLength
**      from SQLINTEGER to SQLLEN for 64-bit.
*/
    ...
    for (i=0;  i<parmCount;  i++, ppd++, papd++, pipd++)
    {
        ...
        {    
			...
			*/
            ...
            ConvertUCS4ToUCS2((u_i4*)rgbData, (u_i2*)rgbData,
                              (SQLINTEGER)(OctetLength/2));
    ...
    apiQueryType, IIAPI_PUTPARMPARM *serviceParms, BOOL moreData)
{
	...
	SQLSMALLINT fCType;
	SQLSMALLINT fSqlType;
	SQLLEN      OctetLength;   /* byte length of char or binary data */
		...
		*/
		...
		if (fSqlType == SQL_WCHAR  &&  sizeof(SQLWCHAR) == 4)
		{
		    ...
		    ConvertUCS4ToUCS2((u_i4*)rgbData, 
                        (u_i2*)rgbData, (SQLINTEGER)(OctetLength/2));
		}
...
**
*/
static SQLRETURN odbc_retrieveByRefParms(LPSTMT  pstmt)
{
    RETCODE            rc = SQL_SUCCESS;
    BOOL               ret = TRUE;
    IIAPI_GETCOLPARM   getColParm;
    II_INT2            i, j;
    DESCFLD            apd = {0}; /* application param descriptor for target */
    ...
    */
    ...
    /*
    ** Return if no input/output or output parameters.
    */
    if (i >= APDCount)  /* return if no in/out parameters */
    {
        /*
        ** If a result set was returned, set the status mask
    ...
    /* 
    ** Loop through the returned columns, and allocate memory 
    ** for the result columns going into the parms.
    */
    ...
    IIapi_getColumns( &getColParm );  /* get return columns */
    ...
    */
    ...
    for( i=0, j=0; i < APDCount  &&  j < IRDCount; i++, papd++ )
    {
        ...
        if (getColParm.gc_columnData[j].dv_null)
        {
            SQLLEN  * IndicatorPtr;     /* Ptr to NULL indicator */
            /*  
            ** Get IndicatorPtr after adjustment for bind offset, 
            ** row number, and row or column binding. 
            */
        ...
        else
        {
            ...
            else if (pird->fIngApiType == IIAPI_NVCH_TYPE)  /* if NVARCHAR */
            {
                rgbDataui2 = (u_i2*)rgbData;
                if (sizeof(SQLWCHAR) == 4)  /* need UCS2->UCS4 conversion? */
                    ConvertUCS2ToUCS4(rgbDataui2+1, (u_i4*)(rgbDataui2+1),
...
routine_exit:
    ...
    for( i = 0; i < getColParm.gc_columnCount; i++ )
        ...
        odbc_free((PTR)getColParm.gc_columnData[i].dv_value);

    if (getColParm.gc_columnData)
        odbc_free((PTR) getColParm.gc_columnData);
...
**    21-Mar-2011 (Bruce Lunsford) Bug 125212
**          If varchar compression is turned off in DBMS, connect requests
**          sometimes crash due to buffer overrun of szUnicode field.
**          SQL function dbmsinfo() returns varchars of length 64 (66 bytes
**          total including 2-byte prefix) which, with vch compression OFF,
**          returns the full length into receiving field.  This caused
**          fields szUnicode (8 bytes in size) and szUsername (64 bytes)
**          to be overrun past the end.
**          For szUnicode field (a flag), add explicit VARCHAR scalar with
**          length parm set to 1 in "select dbmsinfo(...)" query.  Also
**          clear it first with memset() since it is not selected for
**          non-Ingres servers, so was uninitialized for data byte [2].
**          Remove unneeded inits to "" since memset now used.
**          For szUsername field, bump szUsername up by 2 bytes.
*/
BOOL GetServerInfo(SESS * pSession, LPSQLCA psqlca, LPDBC pdbc)
{
    ...
    char szRelease[100];
    char szUsername[66];  /* 2-byte vch pfx + 64 max if vch uncompressed */
    char szUnicode[3];    /* 2-byte vch pfx + 1 byte flag */
...
"select dbmsinfo('_version'), dbmsinfo('username'), VARCHAR(dbmsinfo('unicode_level'), 1)");
       }
	...
	if (*p == '1')
        ...
        */
        ...
        {
            /*
            ** The VW release number gets its own space, for SQLGetInfo only.  
            ** Internally, VW is treated as an Ingres server otherwise, with
            ** certain exceptions, so fRelease gets a value equating to
            ** Ingres 10. 
            */
            char vwRelease[100]; 
            char *p = NULL;
            STcopy(&szRelease[2], vwRelease); 
            p  = STindex(vwRelease, "(", 0);   
            if (p != NULL)
                *p = '\0';
            p = STstrindex(vwRelease, "VW", 0, TRUE);
            if (p != NULL)
            {
                p += 2;
            ...
            STcopy(vwRelease, pdbc->fVWRelease);
...
**    28-Jan-2011 (Ralph Loen) Bug 124970
**          For the QRY_QUERY state, for QST_PREPARE query types, invoke 
**          odbc_getQInfo() (QRY_GETQI) and odbc_close() (QRY_CLOSE) after 
**          fetching the descriptor (QRY_GETD).
*/
...
static RETCODE odbc_query_sm( QRY_CB *qry_cb )
{
	...
	if (rc == SQL_ERROR)
	{
        ...
        if (apiQueryType == IIAPI_QT_EXEC_PROCEDURE)
        {
             rc = odbc_retrieveByRefParms(pstmt);
             if (rc == SQL_ERROR)
             {
                 sequence = QRY_CANCEL;
                 break;
