/*
** Copyright 1992, 2011 Actian Corporation
*/
    ...
    case  SQL_DIAG_CLASS_ORIGIN:       /* document owner of SQLSTATE class*/
       {
        ...
        if (pStringLength && !*pStringLength)
            *pStringLength = (SQLSMALLINT)STlength(p) + 1;
    ...
    CHAR       * rgbWork)
{
    ...
    if (MEcmp(msgprefix, "***", 3)==0)  /* if msg not available */
       STcopy("[Actian][%s ODBC Driver]", msgprefix);
    ...
    CHAR       * rgbWork)
{
    ...
    */
    ...
    else
    {
        ...
        if (MEcmp(msgprefix, "***", 3)==0)  /* if msg not available */
           STcopy("[Actian][%s ODBC Driver][%s]", msgprefix);
    ...
    {
        MEcopy (pMessage, cb, &rgbWork[cbId]);  /* copy msg after [Actian][...] prefix*/
