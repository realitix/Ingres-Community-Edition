...
#define ADD_LENPTR_AND_OFFSET(ptr,offset) (SQLLEN*)((char*)(ptr) + (offset))
...
**         In PutParms() use new CvtCharTimeTimestamp(), CvtEditTimeTimestamp(),
**         and CvtTimeTimestampChar() for converting from SQL_C_TYPE_TIMESTAMP 
**         or into  SQL_TYPE_TIMESTAMP.  The existing funcitons CvtEditTime(),
**         CvtTimeChar() and CvtTimeChar() revert to their original usage of
**         converting time values without fractions.
**     27-Jul-2011 (Bruce Lunsford) Bug 125522
**         Change data types of BindOffset fields in function GetBoundDataPtrs
**         to be consistent (SQLULEN * and/or SQLULEN)--previously SQLINTEGER
**         which caused it to get truncated and appear negative with
**         parameterized queries using SQL Server 64-bit linked tables.
**         Also changed some other local variables from SQLINTEGER/u_i4/
**         SQLUINTEGER to appropriate SQLLEN/SQLULEN.
*/
...
static RETCODE PutParm     (LPSTMT, LPDESCFLD, LPDESCFLD, CHAR*, SQLINTEGER*, SQLLEN*, BOOL);
    ...
    LPDESCFLD pipd)
{
    ...
    SQLLEN         * IndicatorPtr;/* ptr to indicator             */
    CHAR *       rgbValue;    /* ptrto bound parameter        */
    SWORD        fSqlType;    /* target data type             */
    SWORD        fCType;      /* source data type             */
    SQLLEN       cbValueMax;  /* length in bytes of char or bin data */
    SQLULEN      cbColDef;    /* actual len of var data; max len of others */
    LPSQLTYPEATTR   ptype;
    SQLINTEGER       cb;
...
static RETCODE PutParm(
    ...
    SQLINTEGER * pcbValue,   /* -->actual column value length (StrLen_or_IndPtr)
                          or --> SQL_DEFAULT_PARAM
                                 SQL_DATA_AT_EXEC
                                 SQL_NTS          */
    SQLLEN  * IndicatorPtr,  /* -->null indicator  (SQL_NULL_DATA) */
    BOOL    offsetData ) /* Whether to offset char data */
{
    ...
    SWORD       cbData;              /* length of parm in buffer */
    SDWORD      cbdwData;            /* length of parm in buffer (UDWORD) */
    ...
    SQL_TIMESTAMP_STRUCT stampValue;
 ...
 /* if a procedure call is to just use the default value of the parm, just return */
    ...
    if (pcbValue)   /* SQLBindParameter's StrLen_or_IndPtr */
    {
        ...
        default:
            ...
            if (fCType == SQL_C_CHAR   ||
                ...
                fCType == SQL_C_WCHAR)
            {
				SQLINTEGER tmp = *pcbValue;
                cbdwValue = (UDWORD) tmp;  /* value length (bytes) */
    ...
    */
    ...
    case SQL_C_WCHAR:
        ...
        {
            if (cbValue)
                return ErrState (SQL_HY090, pstmt);
                /* invalid string or buffer length */
        }
    ...
    */
    ...
    if (IItrace.fTrace >= ODBC_IN_TRACE)
    {
        if (rgbValue)
        {
            SQLULEN blen = (pcbValue && *pcbValue > 0) ? *pcbValue : 0;
            SQLULEN llen = (fCType == SQL_C_WCHAR) ? pipd->Length * 
                sizeof(SQLWCHAR) : pipd->Length;
            SQLULEN wlen = (fCType == SQL_C_WCHAR) ? cbdwData * sizeof(SQLWCHAR) : 
                cbdwData;
            ...
            if (wlen)
            {
                ...
                odbc_hexdump(rgbValue, (SQLINTEGER)wlen);
            }
            else
...
numStrCvtError:
		    ...
		    ** the server class is Ingres or VSAM which require 
            ...
            if (fSqlType == SQL_TYPE_TIME || fVerboseType == SQL_INTERVAL)
            {
                ...
                if (rc == CVT_SUCCESS)
                {
		            ...
		            rc = CvtTimeTimestampChar (&timeValue, rgbData, cbData,
                        pstmt->fAPILevel);
                ...
                rc = CvtCharTimeTimestamp (&timeValue, rgbValue, cbData, 
                    pstmt->fAPILevel);
                if (rc == CVT_SUCCESS)
                {
                    rc = CvtTimeTimestampChar (&timeValue, rgbData, cbData, 
    ...
    case SQL_C_UTINYINT:
        ...
        case SQL_TINYINT:
            ...
            rc = PutTinyInt (rgbData, (SCHAR)uintValue, fSqlType, TRUE);
            break;
    ...
    case SQL_C_SLONG:
        ...
        case SQL_LONGVARCHAR:
            ...
            pipd->OctetLength = (UWORD)pipd->Length;
            cbData = (UWORD)pipd->OctetLength;
            rc = PutNumChar (papd, pipd, rgbData, szValue, cbData);
            break;
        ...
        case SQL_WLONGVARCHAR:
        {
            ...
            CVla((i4)intValue, szValue);
            /* byte length of buffer needed to hold WCHAR string*/
			cbdwValue = (UDWORD)STlength(szValue) + sizeof(CHAR);
...
/*          _gcvt (floatValue, 17, szValue);  */
    ...
    case SQL_C_TYPE_TIME:
        ...
        case SQL_LONGVARCHAR:
            ...
            rc =  CvtEditTime ((SQL_TIME_STRUCT *)rgbValue);
            if (rc != CVT_SUCCESS) break;
            rc = CvtTimeChar ((SQL_TIME_STRUCT *)rgbValue, rgbData, cbData);
            break;
        ...
        case SQL_TYPE_TIMESTAMP:
            ...
            stampValue.hour     = ((SQL_TIME_STRUCT     *)rgbValue)->hour;
            stampValue.minute   = ((SQL_TIME_STRUCT     *)rgbValue)->minute;
            stampValue.second   = ((SQL_TIME_STRUCT     *)rgbValue)->second;
            stampValue.fraction = 0;
            rc = CvtTimestampChar ( &stampValue, rgbData, cbData,
    ...
    case SQL_C_TYPE_TIMESTAMP:
        ...
        case SQL_TYPE_TIME:
            ...
            rc =  CvtEditTimeTimestamp (&timeValue, pstmt->fAPILevel);
            if (rc != CVT_SUCCESS) break;
    ...
    SQLLEN     ** pIndicatorPtr)
{
    ...
    SQLULEN     * BindOffsetPtr = pARD->BindOffsetPtr;
                                             /* Pointer to binding offset */

    TRACE_INTL(pstmt,"GetBoundDataPtrs");
    DataPtr        = pard->DataPtr;         /* or  = papd->DataPtr */
    OctetLengthPtr = (SQLINTEGER*)pard->OctetLengthPtr;  /* or  = papd->OctetLengthPtr */
    ...
    if ( BindOffsetPtr  &&          /* if SQL_ATTR_ROW_BIND_OFFSET_PTR */
        *BindOffsetPtr)             /* *SQL_ATTR_ROW_BIND_OFFSET_PTR!=0 */
       {
         ...
         if (IndicatorPtr)          /*  to SQL_DESC_INDICATOR_PTR */
             IndicatorPtr  = ADD_LENPTR_AND_OFFSET(
                                              IndicatorPtr,
                                              BindOffset);
       }
    ...
    if (RowIndex)    /* if 2nd thru nth element in rowset */
       {
        if (pARD->BindType != SQL_BIND_BY_COLUMN) /* ROW-wise bind */
        ...
        else                                   /* COLUMN-wise bind */
           {
            ...
            if (OctetLengthPtr)        /*  to SQL_DESC_OCTET_LENGTH_PTR */
            {
                SQLLEN *tmpLenPtr = (SQLLEN *)OctetLengthPtr;
                tmpLenPtr =            ADD_LENPTR_AND_OFFSET(
                                                 tmpLenPtr,
                                                 sizeof(SQLLEN) * RowIndex);
                OctetLengthPtr = (SQLINTEGER *)tmpLenPtr;
            }
            if (IndicatorPtr)          /*  to SQL_DESC_INDICATOR_PTR */
                IndicatorPtr  =       ADD_LENPTR_AND_OFFSET(
                                                 IndicatorPtr,
                                                 sizeof(SQLLEN) * RowIndex);
                //   IndicatorPtr += (sizeof(SQLINTEGER) * RowIndex);
           }
       }  /* end if RowIndex */
...
**  History:
**     27-Jul-2011 (Bruce Lunsford) Bug 125522
**         Change data types of 4th input parm (pcbParamDef) from
**         SQLUINTEGER to SQLULEN to agree with ODBC spec...needed
**         for 64-bit.
*/
