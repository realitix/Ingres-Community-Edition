...
**         Added command CMD_NATIVE_SQL. 
**     15-Feb-2011 (Ralph Loen) Bug 125041
**         Fix compiler warnings on 64-bit Windows.
**     17-Jun-2011 (Ralph Loen) Bug 125457
**         Added fVWRelease to tDBC to track VW versions independently.
**    21-Jul-2011 (Ralph Loen) Bug 125565
**         Added new functions CvtCharTimeTimestamp(), CvtEditTimeTimestamp(),
**         CvtTimeTimestampChar() for converting time values within the context
**         of a timestamp, i.e., with fractions of a second.  CvtEditTime(),
...
**         Change data type of BindOffsetPtr and RowsProcessedPtr fields
**         in DESC and DESCFLD from (SQLUINTEGER *) to (SQLULEN *) for 64-bit.
**         Similar change (SQLUINTEGER to SQLULEN) for ArraySize, Length,
**         OctetLength, prevOctetLength and Precision.
*/
...
#define CvtTimeTimestampChar       IIODcvtc_CvtTimeTimestampChar
#define CvtEditTimeTimestamp       IIODcvtc_CvtEditTimeTimestamp
#define CvtCharTimeTimestamp       IIODcvtc_CvtCharTimeTimestamp
...
#define isOld6xRelease(pdbc)                  ((pdbc)->fRelease < fReleaseRELEASE10)
    CHAR        fVWRelease[20];         /* VW Server release number. */
...
#define CMD_NATIVE_SQL              0x00020000 /* SQLNativeSql (dummy) cmd */
...
{                                   /* Note: an application can associate an 
    ...
    SQLULEN       ArraySize;        /* Num of rows in rowset or of param values.
    ...
    SQLULEN     * BindOffsetPtr;    /* Pointer to binding offset
    ...
    SQLULEN     * RowsProcessedPtr; /* Ptr to number of rows fetched (IRD) or
...
/*  SQLCHAR *     CatalogName;       * Result set's table's catalog name or ""
    ...
    CHAR *        Label;            /* Column name or ""
                                                       IRD-R         */
    SQLULEN       Length;           /* Max length of char string in chars,
...
/*  SQLINTEGER    NumPrecRadix;      * 2 if approximate (floating) numeric,
    ...
    SQLLEN       * OctetLengthPtr;     /* Ptr to a total length in bytes.
                                       If APD, 
    ...
    SQLSMALLINT   ParameterType;    /* Parameter type
                                       ...
                                       IPD set by SQLBindParameter(InputOutputType).
                                                              IPD-RW */
  /*SQLSMALLINT*/
    SQLLEN        Precision;        /* Number of digits for exact numeric.
...
#define COL_DATAPTR_ALLOCED 0x0100      /*     DataPtr is malloc'ed */
    ...
    SWORD        cbDataOffsetAdjustment;
                                    /* IRD, IPD Adjustment to cbDataOffset used
                                       ...
                                       Usually 0 or 2                 */
    SQLULEN     * BindOffsetPtr;    /* Pointer to binding offset to override
    ...
    SQLLEN        prevOctetLength;      /* SQLGetData prev octet length */
