/*
** Copyright (c) 1992, 2009 Actian Corporation
*/
    ...
    SQLSMALLINT        *pcbConnStrOut)
{
    LPDBC       pdbc  = (LPDBC)hdbc;
    CHAR     *  p;
    UWORD       cb = (cbConnStrIn == SQL_NTS) ? (UWORD)STlength ((char*)szConnStrIn) : 
                cbConnStrIn;
        ...
        */
        ...
        while (p)
        {
                ...
                sizeof ROLEPWD - 1, TRUE) == 0  && ! *pdbc->szRolePWD)
            {
                ...
                RPwdFound = TRUE; 
                pdbc->cbRoleName = (UWORD)STlength(pdbc->szRolePWD);
            }
            else
...
static VOID concatBrowseString(char *inputString, char *attr, BOOL optional)
{
    u_i2 len = (u_i2)STlength(inputString);
