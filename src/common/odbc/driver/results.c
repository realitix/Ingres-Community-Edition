...
**    21-Jul-2011 (Ralph Loen) Bug 125565
**         In GetColumn(), use revised CvtCharTime() to fetch columns into
**         SQL_C_TYPE_TIME.   Use the new CvtCharTimeStamp() to fetch
**         time columns into SQL_C_TYPE_TIMESTAMP with loss of seconds
**         precision.
**     27-Jul-2011 (Bruce Lunsford) Bug 125522
**         Change data type of pRowsFetched from SQLUINTEGER to SQLULEN
**         due to similar change to DESC->RowsProcessedPtr plus
**         several other variables...for 64-bit.
**     21-Sep-2011 Bruce Lunsford Bug 125720
**         In GetColumn(), convert dates and timestamps to character format
**         in a "work area" rather than "in place" in the API's returned
**         buffer area to eliminate overlay of subsequent column in
**         the API buffer area.  The symptom was columns retrieved
**         immediately after a date or timestamp were returned to the
**         application as corrupted and could cause spurious errors such
**         as "Data truncation".  This problem was introduced by fix for
**         bug 122174, which changed (reduced) value of OctetLength for
**         some data types like date and timestamp, which then reduced
**         the amount of API buffer space allocated for such fields.
*/
...
static UDWORD CvtIngresDateToChar(LPSTMT pstmt, CHAR *, CHAR *, LPDESCFLD);
static UDWORD CvtIsoTimestampToChar(LPDBC pdbc, CHAR *, CHAR *, LPDESCFLD);
...
/*      pstmt->pIRD->Count = icol;       do NOT overlay result set descriptor */
    ...
    pard->OctetLength   = (SQLINTEGER)cbValueMax;     /* ARD SQL_DESC_OCTET_LENGTH   */
    ...
    SQLLEN       FetchOffset)
{
    ...
    pstmt->fCursorOffset = (SDWORD)FetchOffset;
    ...
    */
    ...
    rc = FetchRowset(pstmt, FetchOrientation, (SQLINTEGER)FetchOffset);
...
**  27-Jul-2011 (Bruce Lunsford) Bug 125522
**      Change data type of pRowsFetched from SQLUINTEGER to SQLULEN
**      due to similar change to DESC->RowsProcessedPtr.  Ditto for
**      local variable RowArraySize.
*/
RETCODE   FetchRowset(
    LPSTMT         pstmt,
    SQLSMALLINT    FetchOrientation,
    SQLINTEGER   FetchOffset)
{
    ...
    SQLULEN        RowArraySize     = pARD->ArraySize;
    SQLUINTEGER        RowsFetched      = 0;
    SQLULEN       *pRowsFetched     = pIRD->RowsProcessedPtr;
    ...
    SQLUSMALLINT * RowStatusArray)
{
    ...
    */
    ...
    rc = FetchRowsetExtended(pstmt, FetchOrientation, (SQLINTEGER)FetchOffset, 
        RowStatusArray, (SQLUINTEGER *)RowCountPtr);
...
**      fetching anything.
**  27-Jul-2011 (Bruce Lunsford) Bug 125522
**      Change data type of RowArraySize local variable
**      from SQLUINTEGER to SQLULEN for 64-bit.
*/
    ...
    */
    ...
    for  (i = 1; i <= IRDCount; i++, pard++, pird++)
    {
        ...
        {                      /* if SQLBindCol called or pARD->SQL_DESC_DATA_PTR is set */
                ...
                SQLLEN      * SavedIndicatorPtr   = pard->IndicatorPtr;
                SQLLEN      * SavedOctetLengthPtr = pard->OctetLengthPtr;

                GetBoundDataPtrs(pstmt, Type_ARD, pARD, pard,
                                 &pard->DataPtr,
                                 (SQLINTEGER **)&pard->OctetLengthPtr,
                                 &pard->IndicatorPtr);
    ...
    SQLLEN       * pcbValue)
{
    ...
    */
    ...
    if (!(pard->fStatus & COL_BOUND))
        pard->OctetLength = (SQLINTEGER)cbValueMax;
    ...
    */
    ...
    ard.OctetLength   = (SQLINTEGER)cbValueMax;
    ...
    {
        *pcbValue = (SQLINTEGER)pird->blobLen;
        pird->blobLen = 0;
    }
...
**                           the string.
**  27-Jul-2011 Bruce Lunsford Bug 125522: Remove unreferenced variable
**                           timeValue.
**  21-Sep-2011 Bruce Lunsford Bug 125720: Convert dates and timestamps
**                           to character in work area szValue rather than
**                           "in place" in the API's returned buffer area
**                           to eliminate overlay of subsequent column.
**                           Added output work area to parm list for
**                           date and timestamp conversion functions.
*/
RETCODE GetColumn(
    LPSTMT       pstmt,
    LPDESCFLD    pard,
    ...
    CHAR     *   prgbData)
{
    ...
    SDWORD      intNum = 0;
    SQLLEN      cbValueSavedInCaseOfError = 0;
    SQLLEN      IndicatorSavedInCaseOfError = 0;
    ...
    */
    ...
    case SQL_TYPE_DATE:
        rc = CvtIngresDateToChar(pstmt, rgbData, szValue, pird);
        if (rc != CVT_SUCCESS)
            return(SQL_ERROR);
        rgbData = &szValue[0];
        cbData = (SQLINTEGER)STlength(rgbData);
    ...
    case SQL_TYPE_TIMESTAMP:
        ...
        */
        ...
        {
            rc = CvtIsoTimestampToChar(pstmt->pdbcOwner, rgbData, szValue, pird);
        }
        if (rc != CVT_SUCCESS)
            return(SQL_ERROR);
    ...
    if (IItrace.fTrace >= ODBC_IN_TRACE)
    {
        ...
        if (len)
        {
            TRdisplay("GetColumn entry C type %d SQL type %d len 0x%p " \
                "data hex dump\n", fCType, fSqlType, len);
            odbc_hexdump(rgbData, (i4)len);
        }
        else
    ...
    */
    ...
    case SQL_C_CHAR:
        ...
        case SQL_TINYINT:
            ...
            cbData = (SQLINTEGER)STlength (szValue);
        ...
        case SQL_NUMERIC:
        {
           char *p;
            ...
            rc = GetNumChar (pard, szNum, (SQLINTEGER)STlength (szNum), 
    ...
    case SQL_C_UTINYINT:
        ...
        case SQL_LONGVARCHAR:
            rc = CvtCharInt ((SDWORD *)&tmpLong, rgbData, 
                (UWORD)STlength(rgbData));
            tinyValue = (SCHAR)tmpLong;
            break;
    ...
    case SQL_C_TYPE_TIME:
        ...
        case SQL_TYPE_TIME:
            ...
            rc = CvtCharTime ((SQL_TIME_STRUCT*)pard->DataPtr, rgbData,
                (UWORD)cbData);
    ...
    case SQL_C_TYPE_TIMESTAMP:
        ...
        case SQL_TYPE_TIME:
            ...
            rc = CvtCharTimeTimestamp (&isoTimeValue, rgbData, (UWORD)cbData,
                pstmt->fAPILevel);
            if (rc == CVT_SUCCESS)
            {
                TIMESTAMP_STRUCT* tm = (TIMESTAMP_STRUCT*)pard->DataPtr;
                CvtCurrStampDate (tm, pstmt->fAPILevel);
                tm->hour     = isoTimeValue.hour;
                tm->minute   = isoTimeValue.minute;
                tm->second   = isoTimeValue.second;
                tm->fraction = isoTimeValue.fraction;
            }
            break;
            ...
            */
            ...
            else
            {
                ...
                if (fSqlType == SQL_CHAR && termSpace != NULL)
                    ns->scale = (SQLSCHAR)(termSpace - p);
                else
                    ns->scale = (SQLSCHAR)STlength(p);
            }
            break;
        }
        case SQL_INTEGER:
    ...
    if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
    {
        if (IItrace.fTrace >= ODBC_IN_TRACE)
        {
            SQLLEN len = pard->OctetLengthPtr ? *pard->OctetLengthPtr : 0;
            ...
            if (len)
            {
                TRdisplay("GetColumn exit C type %d SQL type %d len %d " \
                    "data hex dump\n",
                    fCType, fSqlType, len);
                 odbc_hexdump(pard->DataPtr, (i4)len);
            }
            else
                TRdisplay("GetColumn C type %d SQL type %d len 0\n",
                    fCType, fSqlType);
    ...
    */
    ...
    default:
        ...
        if (pard->IndicatorPtr)     /* SQLBindCol(pcbValue) */
            *pard->IndicatorPtr = (SQLINTEGER)IndicatorSavedInCaseOfError;
        if (pard->OctetLengthPtr)     /* SQLBindCol(pcbValue) */
            *pard->OctetLengthPtr = (SQLINTEGER)cbValueSavedInCaseOfError;
        return SQL_ERROR;
    }
}
...
{
    SQLLEN      OctetLengthMinusSizeofNullTerm;
    CHAR      * rgbCopy;
    SQLINTEGER      cbCopy, cbLeft;

    OctetLengthMinusSizeofNullTerm = pard->OctetLength - (SQLINTEGER)sizeofNullTerm;
 ...
 /* cbCopy  = length to move so it will fit in buffer with a null terminator  */
    ...
    ** truncation warning.
    */
    if (pard->OctetLength < (SQLINTEGER)sizeofNullTerm)
        return (CVT_TRUNCATION);
    ...
    CHAR        multibyteFillChar)
{
    ...
    */
    ...
    rc = ConvertWCharToChar(NULL,   /* wchar -> char */
              ...
              NULL,
              NULL,
              (SQLINTEGER *)pard->IndicatorPtr);  /* return length in bytes */
    ...
    SWORD       fSqlType)
{
    ...
    {
        STtrmwhite(rgbData);
        cb = (SQLINTEGER)STlength(rgbData);
    }
...
**  CvtIngresDateToChar
**
**  Convert an Ingres date value to a character string.
**
**  On entry: pstmt       -->STMT (Statement Control Block)
**            rgbData     -->Buffer pointing to the source data.
**            pTarget     -->Buffer pointing to the target data.
**                           Caller must ensure large enough for max length
**                           ODBC_DTE_OUTLENGTH + EOS.
**            pird        -->IRD (Implementation Row Descriptor)
**
**  On exit:  pTarget contains the converted data.
**
**  Returns:  CVT_SUCCESS  - Successful conversion.
**            CVT_ERROR    - Unsuccessful conversion.
**
**  History:
**     21-Sep-2011 Bruce Lunsford Bug 125720
**         Added output area (pTarget) to parmlist for converted
**         Ingres date output, rather than overlaying input area.
**         Changes were modeled after function GetTimeOrIntervalChar().
**         Use stack workarea for api_convertData() output instead
**         of allocating and freeing this small space (which also was
**         not always getting freed...hence memleaking).
*/
static UDWORD CvtIngresDateToChar(LPSTMT pstmt, CHAR *rgbData, CHAR *pTarget, LPDESCFLD pird)
{
    UDWORD      rc = CVT_SUCCESS;
    ...
    II_UINT2    dstLen = ODBC_DTE_OUTLENGTH;
    CHAR        dstCharDate[ODBC_DTE_OUTLENGTH + 1];
    II_INT2     dn_year;
    II_INT2     datelen;
    LPSQLTYPEATTR ptype;
    II_INT2 fIngApiType;
    ...
    if (fIngApiType == IIAPI_DTE_TYPE)
    {
        if (d->date_spec == ODBC_DATE_NULL) /* blank empty date */
        {
            if (pdbc->fOptions & OPT_BLANKDATEisNULL)
                STcopy("", pTarget);  /* return blank date as NULL */
            else
                STcopy("9999-12-31 23:59:59", pTarget); /* Use default*/
            return(rc);
        }
        ...
        if (pdbc->fOptions & OPT_DATE1582isNULL  &&
            ...
            d->lowday== 01)
        {
           STcopy("", pTarget);  /* return date 1582-01-01 as NULL */
           return(rc);
        }
        dn_year=d->year;
    }
    ...
    MEfill(sizeof(dstCharDate),0,dstCharDate);
    ...
    cv.cv_dstValue.dv_value     = dstCharDate;
    ...
    */
    ...
    {
        STcopy((char *)cv.cv_dstValue.dv_value, pTarget);
           return CVT_SUCCESS;
    }

    MEfill(cv.cv_dstValue.dv_length,' ',pTarget);
    ...
    */
    ...
    if (*t)                    /* if DATE field has time portion */
        MEcopy(t,8,pTarget+11);          /* fill in time portion */
    else if (fIngApiType == IIAPI_DTE_TYPE)
        MEcopy("00:00:00",8,pTarget+11); /* no time, use 00:00:00 */
    
    if (p[2]=='-')   /* convert US or DMY date format, dd-mmm-yyyy */
    { 
        MEcopy(p+7,4,pTarget+0);     /* yyyy */
        CvtMonthName(pTarget+5,p+3); /* mm   */
        MEcopy(p+0,2,pTarget+8);     /* dd   */
    }
    else if (p[2]=='/'  ||  p[2]=='.')  /* MULTINATIONALx or GERMAN */ 
    {   /* convert MULTINATIONAL date format, dd/mm/yy */
        if (CMspace(p+8))
        { 
            MEcopy(p+6,2,pTarget+2);         /* ??yy */
            /* 
    ...
    if (p[6]==' ')      /* convert ISO date format, yymmdd */
    {
        ...
        */
        ...
        MEcopy(p+2,2,pTarget+5);         /* mm   */
        MEcopy(p+4,2,pTarget+8);         /* dd   */
    }
    else if (datelen)
        MEcopy(p+0,datelen,pTarget+0); /* unknown format, just copy it */
    
    return(rc); 
}

/*
**  CvtIsoTimestampToChar
**
**  Convert an ISO timestamp value to a character string.
**
**  On entry: pdbc        -->DBC (Data Base Connection Control Block)
**            rgbData     -->Buffer pointing to the source data.
**            pTarget     -->Buffer pointing to the target data.
**                           Caller must ensure large enough for max length
**                           ODBC_xxxx_OUTLENGTH + EOS.
**            pird        -->IRD (Implementation Row Descriptor)
**
**  On exit:  pTarget contains the converted data.
**
...
**  History:
**     21-Sep-2011 Bruce Lunsford Bug 125720
**         Added output area (pTarget) to parmlist for converted date
**         or timestamp output, rather than overlaying input area.
**         Changes were modeled after function GetTimeOrIntervalChar().
*/
static UDWORD CvtIsoTimestampToChar(LPDBC pdbc, CHAR *rgbData, CHAR *pTarget, LPDESCFLD pird)
{
    UDWORD      rc = CVT_SUCCESS;
    II_UINT2 srcLen;
    II_UINT2 dstLen;
    ...
    if (pipd->ParameterType == SQL_PARAM_OUTPUT)
    {
        ...
        */
        ...
        case SQL_C_BINARY:
            ...
            *OctetLengthPtr = (SQLINTEGER)STlength(papd->DataPtr);
            break;
    ...
    VOID *envHndl, LPDESCFLD pird)
{
    ...
    fp.fd_srcDesc.ds_dataType   = pird->fIngApiType; 
    fp.fd_srcDesc.ds_nullable   = FALSE;  
    fp.fd_srcDesc.ds_length     = (II_UINT2)pird->OctetLength;
    fp.fd_srcDesc.ds_precision  = (II_UINT2) pird->IsoTimePrecision;
    fp.fd_srcDesc.ds_scale      = 0;
    fp.fd_srcDesc.ds_columnType = IIAPI_COL_TUPLE;
    fp.fd_srcDesc.ds_columnName = NULL;  
