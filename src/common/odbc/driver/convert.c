/*
** Copyright (c) 1993, 2011 Actian Corporation
*/
...
**    22-Jul-2011 (Ralph Loen) Bug 125565
**         Initialized a couple of timstamp char buffers to an empty string
**         to stop valgrind from complaining about usage of uninitialized  
**         values.
*/
...
/*
**  CvtTimeTimestampChar
**
**  Convert an ODBC time value to SQL time (character format) for a
**      timestamp.
**
    ...
    UDWORD            apiLevel)
{
    ...
    **  Extract time:
    */
    rc = CvtCharTimeTimestamp (&time, c, cbData, (UDWORD)apiLevel);
    if (rc != CVT_SUCCESS)
        return (rc);
    c += 8;
    i -= 8;
...
/*
**  CvtEditTimeTimestamp
**
**  Check for valid time in a timestamp.
**
**  On entry: ptime-->ODBC time structure.
**
**  Returns:  CVT_SUCCESS
   ...
   UDWORD apiLevel)
{
    ...
    rc = CvtEditTimeTimestamp (&timeValue, apiLevel);
    if (rc != CVT_SUCCESS)
        return (rc);
...
/*
**  CvtCharTimeTimestamp
**
**  Convert character data or an SQL time to an ODBC time value for a timestamp.
**
**  On entry: pValue -->ODBC time structure to receive value.
**            rbgData-->character data.
**            cbData  = length of character data.
    ...
    UDWORD           apiLevel)
{
    ...
    */
    ...
    rc = CvtEditTimeTimestamp(&time, apiLevel);
	...
	UDWORD apiLevel)
{
    ...
    */
    ...
    {	
        ...
        CvtTimeTimestampChar (&timeValue, rgbData + 11, 8, apiLevel);
    ...
    UWORD       cbPrecision)
{
    ...
    CvtTimeTimestampChar (&timeValue, rgbData + 11, 8, IIAPI_VERSION_3);
    ...
    CHAR     *szValue )
{
    ...
    */
    ...
    if (scratchStr)
    {
        current_scale = (i4)STlength(scratchStr) - 1;
        if (!ns->scale)
