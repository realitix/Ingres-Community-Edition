...
**   28-Jan-2011 (Ralph Loen) Bug 124969
**         In SqlNativeSql_InternalCall(), free the internally-allocated
**         statement handle using SQL_UNBIND and SQL_RESET_PARAMS.  Use of
**         SQL_DROP may force an unwanted commit to be issued.  In FreeStmt(),
**         make sure the statement handle is pointing at the right sqlca
**         block before calling SqlClose().
**   29-Jan-2011 (Ralph Loen) Bug 124939
**         In SQLBindParameter(), detect and return an error for 
**         out-of-range precision arguments.
**   02-Feb-2011 (Ralph Loen) Bug 124990
**         In SQLNativeSql_InternalCall(), set fCommand in the statement  
**         handle to CMD_NATIVE_SQL.  In FreeStmt(), in the SQL_UNBIND
**         case, continue on to deallocate pstmt and free from the 
**         connection handle's linked list.
**   15-Feb-2011 (Ralph Loen) Bug 125041
**         Fix compiler warnings on 64-bit Windows.
**
**   22-June-2011 (Ralph Loen) Bug 125471
**         Added token TOK_WITH, which is treated the same as TOK_SELECT. 
**	26-Oct-2011 (kiria01) b125830
**	    Correct loop termination where CMprev can nolonger return a
**	    pointer preceding the start pointer.
*/
    ...
    SQLLEN      *pcbValue)    /* StrLen_or_IndPtr */
{
    ...
    */
    ...
    case SQL_BINARY:
        ...
        fString = TRUE;
        SqlLength = (SQLUINTEGER)cbColDef;  /* IPD's SQL_DESC_LENGTH */
        break;
    ...
    case SQL_NUMERIC:
        ...
        /* 
        ** If the connection target is not Ingres or is old Ingres, then use 
        ** float instead ( bug 119274 to exclude the EA server).
        */
        if (( !isServerClassINGRES(pdbc)  ||  isOld6xRelease(pdbc) ) 
            && !isServerClassEA(pdbc) )
        {
        ...
        /*
        ** Precision arguments of zero or greater than the max precision of
        ** the target result in an invalid precision error.
        */
        if (!cbColDef)
             return ErrUnlockStmt (SQL_HY104, pstmt);  

        if (cbColDef > pdbc->max_decprec)
            return ErrUnlockStmt (SQL_HY104, pstmt);

        cbColDefMax = pdbc->max_decprec;
    ...
    */
    ...
    papd->OctetLength   = (SQLINTEGER)cbValueMax;     /* APD SQL_DESC_OCTET_LENGTH   */
    ...
    */
    if (cbValueMax == 0 &&
        ...
        papd->OctetLength = (SQLINTEGER)cbColDef + 1;
    ...
    UWORD   fOption)
{
    ...
    case SQL_UNBIND:
        ...
        */
        ...
        /*
        ** SQLNativeSql generates a dummy statement handle that is
        ** deallocated with SQL_UNBIND and SQL_RESET_PARMAMS.  One
        ** doesn't want to free with SQL_DROP because an unwanted 
        ** commit maybe issued due to simulated autocommit.
        */
        if (fOption == SQL_UNBIND && pstmt->fCommand != CMD_NATIVE_SQL)
            break;
    ...
    SQLINTEGER* pcbValue)
{
    ...
    if (cbSqlStr == SQL_NTS)
        cbSqlStr = (SQLINTEGER)STlength (szSqlStr);
    ...
    if (rc != SQL_ERROR)
        rc = FreeStmt(pstmt, SQL_RESET_PARAMS); 
   
    pstmt->fCommand = CMD_NATIVE_SQL;

    if (rc != SQL_ERROR)
        rc = FreeStmt(pstmt, SQL_UNBIND); 
    ...
    BOOL          rePrepared)
{
    ...
    */
    ...
    else
    {
        ...
        pstmt->cbPSqlStr = (SDWORD)STlength(szSqlStr);
        UnlockStmt(pstmt);
        return rc;
    }
 ...
 while (*p)
   { p = ScanPastSpaces(p);
	...
	continue;
     ...
     p++;                      /* skip past period   */
     cbOwnername=(SQLINTEGER)(p-pOwnername); /* length of "owner." */
    ...
    case TOK_ESC_FUNCTION:
    {
        ...
        */
        ...
        if (!STbcompare(p,7,"CONVERT",7,TRUE))
        {
            ...
            {
                len = (i4)STlength(sqlToScalarTbl[i].sqlType);
...
CHAR     * ScanSearchCondition(CHAR * p, CHAR ** KeywordList)
{
 ...
 for (;;)
 { 
   if (*p == EOS )    break;     /* if hit null terminator, return */
   ...
   if (*p == '\''  || *p == '"')
      { 
	char c = *p++;
        while ( (*p!=EOS)  &&  *p != c )  CMnext(p);        /* scan past 'quoted string' */
        if (*p == c) p++;                   /* if ending quote and not \0, incr*/
        continue;                            /* continue the main for loop */
      }
   ...
   if (*p == '-'  &&  STbcompare(p,10,(char *)ODBC_STD_ESC,10,TRUE)==0)   /* "--(*vendor" */
      { bracecount++;                        /* incr escape sequence brace nest */
	p+=10;
	continue;                            /* continue the main for loop */
      }
   ...
   if (!CMalpha(p) && p != '_')              /* if NOT the start of a keyword */
      { 
        CMnext(p);
        continue;                            /* continue the main for loop */
      }
   ...
   tokenlen = (SQLINTEGER)(q-p);                /* q->char past the identifier */
   ...
   if (KeywordList)   /* if also searching for specific keywords */
      ...
      {  keywordlen = (SQLINTEGER)STlength(kp);   /* kp->keyword;  keywordlen=keyword len */
