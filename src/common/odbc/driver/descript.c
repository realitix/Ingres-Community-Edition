/*
** Copyright (c) 2000, 2011 Actian Corporation
*/
...
**     22-Feb-2011 (Ralph Loen) Bug 125075
**          Define SQL_DESC_INDICATOR_PTR of the ARD, APD, IRD and IPD as
**          SQLLEN for 64-bit platforms.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**     27-Jul-2011 (Bruce Lunsford) Bug 125522
**          Change data type of ulenValue field from SQLUINTEGER to
**          SQLULEN for use with SQL_DESC_ARRAY_SIZE + SQL_DESC_LENGTH.
**     06-Sep-2011 (Bruce Lunsford) Bug 125729
**          Updated SQLColAttribute() to return 64-bit rather than
**          32-bit numeric values in output parm NumericAttributePtr
**          on 64-bit Windows.
*/
...
**          Updated SQLColAttribute() to return 64-bit rather than
**          32-bit numeric values in output parm NumericAttributePtr
**          on 64-bit Windows.  Also reworked slightly
**          SQL_DESC_CONCISE_TYPE and SQL_DESC_NULLABLE to protect
**          against ACCVIO in unlikely event that NumericAttributePtr
**          not set but option CONVERTINT8TOINT4 is set and data
**          type is a bigint.
*/
#ifdef _WIN64
...
#endif
{
    ...
    SQLLEN      *lenValuePtr = NumericAttributePtr;
    SQLULEN     *ulenValuePtr= NumericAttributePtr;
    ...
    case SQL_DESC_AUTO_UNIQUE_VALUE:
        ...
        if (lenValuePtr)
            *lenValuePtr = FALSE; /* = pdesc->AutoUniqueValue; */
        if (StringLengthPtr)
    ...
    case SQL_DESC_CASE_SENSITIVE:
        ...
        if (lenValuePtr)
            *lenValuePtr = GetDescCaseSensitive(pird);
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_DESC_CONCISE_TYPE:
        ...
        if (lenValuePtr)
        {
            if (option_bigint && pird->ConciseType == SQL_BIGINT)
                *lenValuePtr = SQL_INTEGER;
            else
                *lenValuePtr = pird->ConciseType;
        }
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_DESC_FIXED_PREC_SCALE:
        ...
        if (lenValuePtr)
            *lenValuePtr  = pird->FixedPrecScale;
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_DESC_NULLABLE:
        ...
        if (lenValuePtr)
        {
            *lenValuePtr  = isaIPD(pIRD)?TRUE:pird->Nullable;
            if ( option_bigint && pird->ConciseType == SQL_BIGINT)
                    *lenValuePtr  = 0;
        }
        if (StringLengthPtr)
    ...
    case SQL_DESC_PRECISION:
        ...
        if (lenValuePtr)
        {
            ...
            case IIAPI_INTDS_TYPE:
                *lenValuePtr  = (SQLINTEGER)pird->IsoTimePrecision;
                break;

            case IIAPI_DEC_TYPE:
                *lenValuePtr = pird->Precision;
                break;

            default:
                *lenValuePtr  = (SQLINTEGER)pird->Precision;
                break;
            }
        }
        if (StringLengthPtr)
    ...
    case SQL_DESC_SEARCHABLE:
        ...
        if (lenValuePtr)
            *lenValuePtr  = GetDescSearchable(pird);
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_DESC_UNSIGNED:
        ...
        if (lenValuePtr)
            *lenValuePtr  = GetDescUnsigned(pird);
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_DESC_UPDATABLE:
        ...
        if (lenValuePtr)
            *lenValuePtr  = SQL_ATTR_READWRITE_UNKNOWN;
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    case SQL_COLUMN_SCALE:       /* 2.x SCALE definition differs from 3.x */
        ...
        if (lenValuePtr)
            *lenValuePtr  = pird->Scale;       /* ???? different for 2.x ????*/
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLINTEGER);
        break;
    ...
    SQLINTEGER *StringLengthPtr)
{
    ...
    SQLULEN      *ulenValuePtr=(SQLULEN     *)ValuePtr;
    SQLLEN       *lenValuePtr =(SQLLEN      *)ValuePtr;
    ...
    SQLINTEGER  BufferLength)
{
    ...
    SQLULEN       ulenValue  = (SQLULEN) (SCALARP)ValuePtr;
    ...
    SQLLEN     *IndicatorPtr)
{
    ...
    pxxd->OctetLength   = (SQLINTEGER)Length;             /* SQL_DESC_OCTET_LENGTH */
...
void SetDescDefaultsFromType(LPDBC pdbc, LPDESCFLD pird)
{
    ...
    case IIAPI_INTDS_TYPE:
        ...
        pird->Scale = (SQLSMALLINT)pird->Precision;
        pird->IsoTimePrecision = (SQLUSMALLINT)pird->Scale;
