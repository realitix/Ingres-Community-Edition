...
**	    Added E_AD106C_ASSERTION_FAILURE for ASSERT function
**	17-Dec-2010 (kschendel) SIR 124685
**	    Get rid of ADI_OP_NAME, added no value and caused various
**	    compiler warnings.
**	17-Mar-2011 (kschendel) m1541
**	    adt-x100-print prototype.
**      08-Apr-2011 (horda03) b125177
**          Added ADI_F2097152_PARAM_BAD_TYPE_COMBO to identify FI which
**          have been defined to identify illegal combinations of data
**          types, for example ANSIDATE < TIMESTAMP.
**	22-Mar-2011 (smeke01) b125192
**	    Added ADF_ISNULL_BL_MACRO.
**	04-Jul-2011 (coomi01) b125396
**	    Add ADI_SCALE to allow manipulation of function output
**          scale at runtime.
**	15-Jul-2011 (kiria01) b125530
**	    Comments updated.
**	30-aug-2011 (coomi01) b125708
**          Add ADF_CVTA_ROUND bitmask to adf_misc_flags for use in format
**          routine f_cvta. This will round rather than truncate decimal
**          values.
**	05-Sep-2011 (kiria01) b125573
**	    Update adi_encode_colspec call for collation data.
**	10-Oct-2011 (smeke01) b125834
**          Add ADF_INTMATH_4ORLONGER and ADI_INTMATH_FIXED to support cbf
**          parameter int_math_i8_result. Also remove ADI_IMULI, ADI_IADDI,
**          ADI_ISUBI and ADI_IDIVI which should have been removed when the
**          fix for bug 118798 was replaced with the fix for bug 119368.
...
#define			ADF_UNI_TMPLEN_UCS2	4004/sizeof(UCS2) /* Length of temp buffer used for 
								  ** Unicode operations within adf. */	
#define			ADF_STRICT_ANSI_SUBSTRING
...
/* This macro tests whether the data value pointed to by base b of length l contains the
** null value.  If it does, the macro evaluates to TRUE, and if it does not, the
** macro evaluates to FALSE.  It assumes that b points to a nullable legal datatype.
*/
#define  ADF_ISNULL_BL_MACRO(b,l)  (*(((u_char *)b) + l - 1) & ADF_NVL_BIT ? TRUE : FALSE)
...
#define			E_AD106C_ASSERTION_FAILURE	(E_AD_MASK + 0x106CL)
...
#define			E_AD5514_BAD_DTID_GETSTATUS	(E_AD_MASK + 0x5514L)
#define			E_AD5515_BAD_DTLEN_GETSTATUS	(E_AD_MASK + 0x5515L)
...
**	16-Jun-2011 (kiria01) b122966
**	    Backed out support for ADX_IGNALL_MATHEX as now redundant.
*/
...
**	08-Feb-2010 (kiria01)
**	    Added AD_CNST_FOR_X100 flag to allow certain functions to
**	    be folded that normally would not be.
**	24-Jun-2010 (horda03) B123987
...
**          Added ADF_TYPE_TO_UNICODE. Used when calling adi_tyname()
**          for the unicode equivalent name of a given datatype.
...
# define		AD_CNST_USUPLMT	0x02	/* Will be set each time an otherwise
						...
						** See ADI_F16384_CHKVARFI */
# define		AD_CNST_FOR_X100 0x04	/* Will be set each time an otherwise
						** constant function is called from
						** a non-X100 context. */
    u_i4	    adf_misc_flags;
#define ADF_LONG_DATE_STRINGS 0x00000001    /* Date strings of AD_11_MAX_STRING_LEN required */
#define ADF_TYPE_TO_UNICODE   0x00000002    /* datatypes being converted to unicode */
#define ADF_CVTA_ROUND        0x00000004    /* Round Decimals when converting to ascii */
#define ADF_INTMATH_4ORLONGER 0x00000008    /* If set, use ADI_4ORLONGER for integer arithmetic */
}   ADF_CB;
...
**          Initial creation.
**	8-may-2011 (dougi)
**	    Add ADI_VECTORWISE for VW only functions (ranking funcs for now) and
**	    add flag field to define window funcs.
*/
...
typedef struct _ADI_OPINFO
{
    ADI_OP_ID	    adi_opid;		/* The OP id ... just for fun.
					*/
    char	    adi_opname[ADF_MAXNAME];	/* The OP name associated with the
					** OP id.
					*/
...
#define                 ADI_DB2             0x0004  /* Valid for all DB2    */
						    /* installations	    */
#define			ADI_VECTORWISE	    0x0008  /* Valid for VectorWise */
						    /* queries only.        */

    i4		    adi_opflag;		/* operator flags */
#define			ADI_WINDOW_FUNC	    0x0001  /* can be used in a     */
						    /* window function	    */
...
**	    Add ADI_RPT for REPEAT function
**	26-Aug-2011 (kiria01) SIR 125693
**	    Add ADI_DATEFMT for date_format and from_unixtime and made into
**	    an enum.
*/

typedef enum _ADI_LNCOMPUTE {
    ADI_O1 =		1,	/* Result length/prec is same as first operand. */

    ADI_O2 =		2,	/* Result length/prec is same as second operand. */

    ADI_LONGER =	3,	/* Result length is the longer of the two operands.
				** Precision is inherited from the longer. */

    ADI_SHORTER =	4,	/* Result length is the shorter of the two operands.
				** Precision is inherited from the shorter. */

    ADI_DIFFERENCE =	5,	/* Result length is length of first operand minus
				** length of second operand. Precision is set to 0. */

    ADI_CSUM =		6,	/* Result length is sum of the lengths of the two
				** operands, but <= maxstring and > 0.
				** Precision is set to 0. */

    ADI_FIXED =		7,	/* Result length and precision are constant as
				** specified in adi_fixedsize and adi_psfixed */

    ADI_PRINT =		8,	/* Result length is default print size of first
				** operand (numerics only).
				** Precision is set to 0. */

    ADI_O1CT =		9,	/* Result length is length of first operand plus
				** DB_CNTSIZE. Precision is set to 0. */

    ADI_O1TC =		10,	/* Result length is length of first operand minus
				** DB_CNTSIZE but <= maxstring and > 0.
				** Precision is set to 0. */

    ADI_TSUM =		12,	/* Result length is sum of the lengths of the two
				** operands minus DB_CNTSIZEbut <= maxstring plus
				** DB_CNTSIZE. Precision is set to 0. */

    ADI_ADD1 =		13,	/* Result length is length of the first operand
				** plus 1. Precision is set to 0. */

    ADI_SUB1 =		14,	/* Result length is length of the first operand 
				** minus 1. Precision is set to 0. */

    ADI_TPRINT =	15,	/* Result length is default print size of first
				** operand (numerics only) for text/varchar.
				** Precision is set to 0. */

    ADI_4ORLONGER =	16,	/* Result length is 4 or the longest operand length.
				** For integer ops possibly with a BIGINT operand */

    ADI_RESLEN =	18,	/* Result length and precision should already be
				** known. (i.e. It is whatever the length and
				** precision of the result DB_DATA_VALUE is.)
				** This is used by the coercions of a datatype into
				** itself in order to adjust its length and precision
				** before being output into a tuple.			    */

    ADI_SUMT =		19,	/* Result length is sum of the lengths of the two
				** operands plus DB_CNTSIZE. Precision is set to 0. */

    ADI_P2D2 =		20,	/* Result length is (length of 1st operand plus
				** DB_CNTSIZE) / 2
				** Precision is set to 0. */

    ADI_2XP2 =		21,	/* Result length is (length of 1st operand * 2) plus
				** DB_CNTSIZE Precision is set to 0. */

    ADI_2XM2 =		22,	/* Result length is (length of 1st operand * 2) minus
				** DB_CNTSIZE, but <= maxstring plus DB_CNTSIZE.
				** Precision is set to 0. */

    ADI_CTSUM =		23,	/* Result length is sum of the lengths of the two
				** operands minus DB_CNTSIZE, but <= maxstring, and > 0.
				** Precision is set to 0. */

    ADI_SUMV =		24,	/* Result length is sum of the lengths of the two
				** operands, but <= maxstring + DB_CNTSIZE.
				** Precision is set to 0. */

    ADI_RSLNO1CT =	25,	/* Result length should already be known. (i.e. It
				** is whatever the length of the result DB_DATA_VALUE
				** is.) However, a check will be made to assure that
				** the length is valid if it is not, the result length
				** will be set to the length of the of the input, plus
				** DB_CNTSIZE.  This is only used by the ii_notrm_txt
				** and ii_notrm_vch functions that are needed by OSL.
				** Precision is set to 0. */
/*
...
/* Handle unhex situation */
    ADI_X2P2 =		41,  /* Result length is (length of 1st operand / 2)
			     ** plus DB_CNTSIZE. Precision is set to 0.	*/
...
*/
    ...
    ADI_CTOU =		42,	/* Convert char to nchar or varchar to nvarchar.
				** Precision is set to 0. */
    ADI_UTOC =		43,	/* Convert nchar to char or nvarchar to varchar result
				** length is 1.5 times the length of first operand.
				** Precision is set to 0. */
    ADI_CTOVARU =	44,	/* convert char to nvarchar.
				** Precision is set to 0. */
    ADI_VARCTOU =	45,	/* convert varchar to nchar.
				** Precision is set to 0. */
    ADI_UTOVARC =	46,	/* convert nchar to varchar.
				** Precision is set to 0. */
    ADI_VARUTOC =	47,	/* convert nvarchar to char */
    ADI_NPRINT =	48,	/* Result length is default print size of first
				** operand (numerics only) multiplied by sizeof(UCS2)
				** for nchar output.
				** Precision is set to 0. */

    ADI_NTPRINT =	49,	/* Result length is default print size of first
				** operand (numerics only) multiplied by sizeof(UCS2)
				** and fixed for nvarchar.
				** Precision is set to 0. */

    ADI_CWTOVB =	50,	/* Computes result length of collation_weight()
				** function stored in varbyte. */

    ADI_DATE2DATE =	51,	/* Computes result length of date family coercions */
    ADI_DECROUND =	52,	/* result length of round(), trunc() */
    ADI_DECCEILFL =	53,	/* result length of ceil(), floor() */
    ADI_O1UNIDBL =	54,	/* same as ADI_01 but for unicode characters where the
				** result doubles in size. */

    ADI_MINADD1 =	59,	/* At least 'fixed' bytes +1 */
    ADI_O1UNORM =	60,	/* lenspec to unorm 1st op */
    ADI_PATCOMP =	61,	/* Est pat len */
    ADI_PATCOMPU =	62,	/* Est unicode pat len */
    ADI_O1AES =		63,	/* padded for block crypto */

/*
** Special for 3 operand functions where we need the length
** of the longest of parameters 2 and 3
*/
    ADI_LONGER23RES =	64,	/* longer of parms 2 and 3 taking the
				** result into account. */
    ADI_RPT = 		65,	/* For repeat() */
    ADI_CAST =		66,	/* For cast ops that include a scale definition */
    ADI_DATEFMT =	67,	/* For date_format. Exact if constant pattern
				** otherwise length as for fixed.
				** Precision set to 0. */
    ADI_INTMATH_FIXED =	68,	/* For integer maths, allowing backward compatibility */
/*
...
*/
    ADI_LEN_INDIRECT =	998,
    ADI_LEN_EXTERN =	999,
    ADI_LEN_EXTERN_COUNTVEC = 1000
} ADI_LNCOMPUTE;

typedef struct _ADI_LENSPEC
{
    ADI_LNCOMPUTE   adi_lncompute;      /* How to compute result length,
                                        ** as follows:
                                        */
...
**      08-Apr-2011 (horda03) b125177
**          Add ADI_F2097152_PARAM_BAD_TYPE_COMBO to identify FI that have illegal
*/
...
#define			ADI_F1048576_MUTABLE_RETN 1048576 /* Function may allow
					...
					** explicit coercion */
#define			ADI_F2097152_PARAM_BAD_TYPE_COMBO 2097152 /* Parameter 1 and 2 are
                                        ** not valid for this FI. Return error.
                                        */
    ...
    i2              adi_npre_instr;     /* "Nullable PRE-INSTRuction."  If one
					...
					**
					** This should have type ADE_OPS_ENUM
					** except is defined as an i2.
					**
					** Currently, the possible
					** pre-instructions are:
					**
					**   (These CX special instructions are
					**   defined and documented in <ade.h>
					**   with the other CX special instructions.)
					**
					...
					**
					** The above may be optimised into one
					** of the following list:
					**	ADE_3CXI_CMP_SET_SKIP
					**	ADE_4CXI_CMP_SET_SKIP
					**	ADE_5CXI_CLR_SKIP
					**	ADE_6CXI_CLR_OP0NULL
					**	ADE_7CXI_CKONLY
					*/
...
/*
** First typedef functions definitions that are used more than once
*/
...
FUNC_EXTERN DB_STATUS adt_x100_print(
	void (*outstr)(void *state, const char *str),
	void *state,
	ADF_CB *adfcb,
	DB_DATA_VALUE *dbv,
	DB_DT_ID nulltype);
...
FUNC_EXTERN DB_STATUS adi_ficoerce(ADF_CB     *adf_scb,
				   ...
				   DB_DT_ID   adi_into_did,
				   ADI_FI_ID  *adi_fid);
FUNC_EXTERN DB_STATUS adi_ficoerce_full(ADF_CB *adf_scb,
					DB_DT_ID adi_from_did,
					DB_DT_ID adi_into_did,
					ADI_FI_ID *adi_fid,
...
FUNC_EXTERN VOID adu_2prvalue(void (*fcn)(char *),
                              DB_DATA_VALUE  *db_dv);
...
FUNC_EXTERN DB_STATUS adu_datefmt_calclen(
	ADF_CB *adf_scb,
...
FUNC_EXTERN DB_STATUS adc_date_add(ADF_CB        *adf_scb,
				...
				DB_DATA_VALUE  *dv2,
				DB_DATA_VALUE  *rdv);
FUNC_EXTERN VOID adu_prvalarg(  void (*fcn)(PTR, ...), 
				PTR     arg,
				DB_DATA_VALUE      *db_dv);
...
/*
** Routine used by SC930 tracing to format and output data values
*/
FUNC_EXTERN STATUS adu_sc930prtdataval(i4 msg_type,
		char *parameter_name,
		i4 parameter_no,
		DB_DATA_VALUE *db_dv, 
		ADF_CB *tzcb,
