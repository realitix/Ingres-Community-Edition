...
**	    Minor followup prototype fixes.
...
**	    Corrected icode parameter type to aid debugging.
**/

/*
...
*/
#define ADE_INSTR_MACRO \
_DEFINE(ADE_FIRST_OP,	-1000)/* Must be first */\
_DEFINE(ADE_AND,	-1001)/* Logical AND */\
_DEFINE(ADE_OR,		-1002)/* Logical OR */\
_DEFINE(ADE_NOT,	-1003)/* Logical NOT */\
_DEFINE(ADE_MECOPY,	-1004)/* Move bytes of data */\
_DEFINE(ADE_AGBGN,	-1005)/* Ag-begin */\
_DEFINE(ADE_AGEND,	-1006)/* Ag-end w/def for empty set */\
_DEFINE(ADE_KEYBLD,	-1007)/* Build key & do rng-key chk */\
_DEFINE(ADE_CALCLEN,	-1008)/* Calculate length for VLT */\
_DEFINE(ADE_COMPARE,	-1009)/* Cmp 2 values (NULL != NULL)*/\
_DEFINE(ADE_NAGEND,	-1010)/* Ag-end w/NULL for empty set*/\
_DEFINE(ADE_PMQUEL,	-1011)/* Turns on QUEL PM semantics */\
_DEFINE(ADE_NO_PMQUEL,	-1012)/* Turns off QUEL PM semantics*/\
_DEFINE(ADE_PMFLIPFLOP,	-1013)/* Toggles QUEL PM semantics */\
_DEFINE(ADE_PMENCODE,	-1014)/* Encodes QUEL PM chars */\
_DEFINE(ADE_BYCOMPARE,	-1015)/* Cmp 2 values (NULL == NULL)*/\
_DEFINE(ADE_ESC_CHAR,	-1016)/* Set esc characteristics for LIKE */\
_DEFINE(ADE_PNAME,	-1017)/* Compute physical address */\
_DEFINE(ADE_DBDV,	-1018)/* Build DB_DATA_VALUE */\
_DEFINE(ADE_TMCVT,	-1019)/* CX version of adc_tmcvt */\
_DEFINE(ADE_REDEEM,	-1020)/* Materialize peripheral datatype. */\
_DEFINE(ADE_ANDL,	-1021)/* Logical(non-CNF) AND with label. */\
_DEFINE(ADE_ORL,	-1022)/* Logical(non-CNF) OR with label. */\
_DEFINE(ADE_BRANCH,	-1023)/* Simple branch (for case function) */\
_DEFINE(ADE_BRANCHF,	-1024)/* Conditional (on FALSE) branch */\
_DEFINE(ADE_MECOPYN,	-1025)/* MEcopy which does null check */\
_DEFINE(ADE_AVG,	-1026)/* FINIT avg computation for inline aggregates */\
_DEFINE(ADE_NULLCHECK,	-1027)/* Checks for null agg result to non-nullable field */\
_DEFINE(ADE_OLAP,	-1028)/* FINIT computation for OLAP aggregates */\
_DEFINE(ADE_MECOPYQA,	-1029)/* FINIT MECOPY of Quel agg results uses 2CXI_SKIP to check nulls */\
_DEFINE(ADE_AVGQ,	-1030)/* FINIT avg computation for Quel (doesn't set null) */\
_DEFINE(ADE_UNORM,	-1031)/* Normalization of Unicode source values */\
_DEFINE(ADE_AGNEXT,	-1032)/* Detail agg exec for FRS only */\
_DEFINE(ADE_OLAGBGN,	-1033)/* AGBGN for binary OLAP aggs */\
_DEFINE(ADE_OLAGEND,	-1034)/* AGEND for binary OLAP aggs */\
_DEFINE(ADE_NOOP,	-1035)/* NOOP to replace collapsed MECOPY's etc. */\
_DEFINE(WAS_settrue,	-1036)/* obsolete */\
_DEFINE(WAS_setfalse,	-1037)/* obsolete */\
_DEFINE(ADE_MECOPY_MANY,-1038)/* MECOPY, but not the last */\
_DEFINE(ADE_COMPAREN,	-1039)/* Bulk compare operator - true vararg */\
_DEFINE(ADE_NCOMPAREN,	-1040)/* Bulk not compare operator - true vararg */\
_DEFINE(ADE_SINGLETON,	-1041)/* FINIT computation for singleton inline aggregate */\
_DEFINE(ADE_GETSTATUS,	-1042)/* Collect CX status into bool */\
_DEFINE(ADE_LAST_OP,	-1043)/* Must be last */\
_ENDDEFINE
...
**
**  ADE_0CXI_IGNORE	This will never be compiled, but exists as a value
...
**
**  ADE_1CXI_SET_SKIP	Test for a null in any input operand.  If found, set
...
**
**  ADE_3CXI_CMP_SET_SKIP If not in a CX, (i.e. adf_func()) this instruction
...
**
**  ADE_2CXI_SKIP	Test for a null in any input operand.  If found,
**			skip the next instruction.  This will be used by
**			most aggregate function instances.
**
**  ADE_4CXI_CMP_SET_SKIP Combination of 1CXI and 3CXI which both sets the result
...
**			nullable, otherwise see 6CXI and 7CXI below.
**
**  ADE_5CXI_CLR_SKIP	A 1CXI which doesn't set the null indicator, only clears it.
...
**			nullable, otherwise see 6CXI and 7CXI below.
**
**  ADE_6CXI_CLR_OP0NULL When 1CXI, 4CXI, or 5CXI is generated for a nullable result
**			operand but no nullable input operands, all we need to do is
**			to clear the result's null indicator.  So we generate this
**			instruction instead.
**
**  ADE_7CXI_CKONLY	When a 1CXI, 4CXI, or 5CXI is generated for a non-nullable
...
*/
#define ADE_CXI_MACRO \
_DEFINE2(ADE_0CXI_IGNORE,	-2000)\
_DEFINE2(ADE_1CXI_SET_SKIP,	-2001)\
_DEFINE2(ADE_2CXI_SKIP,		-2002)\
_DEFINE2(ADE_3CXI_CMP_SET_SKIP,	-2003)\
_DEFINE2(ADE_4CXI_CMP_SET_SKIP,	-2004)\
_DEFINE2(ADE_5CXI_CLR_SKIP,	-2005)\
_DEFINE2(ADE_6CXI_CLR_OP0NULL,	-2006)\
_DEFINE2(ADE_7CXI_CKONLY,	-2007)\
_DEFINE2(ADE_CXI_LAST,		-2008)/* Must be last*/\
_ENDDEFINE

#define _DEFINE(name,val) name=val,
#define _DEFINE2(name,val) name=val,
#define _ENDDEFINE
typedef enum _ADE_OPS_ENUM {
    ADE_INSTR_MACRO ADE_CXI_MACRO ade_dummy
} ADE_OPS_ENUM;
#undef _DEFINE
#undef _DEFINE2
#undef _ENDDEFINE
...
#define		ADE_NOTIN	     6  /* 1st value not in 2nd..nth value */
