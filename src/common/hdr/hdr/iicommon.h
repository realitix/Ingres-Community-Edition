/*
** Copyright (c) 1993, 2001, 2010 Actian Corporation
*/
...
**	    encryption.
**	17-May-2010 (kiria01)
**	    Drop back IVW dbcompatlvl to 9.6.0 to create the potential for
**	    a later database upgrade.
**	01-apr-2010 (stial01)
...
**	30-Sep-2010 (kschendel)
**	    Restore IVW dbcmptlvl to 10.0 now that codev is merged to 10.0.
**     13-Oct-2010 (hanal04) Bug 124250
**        Include systypes.h with wchar.h to resolve build
**        problems on HP Itanium.
**      05-Jun-2010 (martinb) SIR: 125220
**          Add ISO4T and ISO4TC
**/
...
/* New style versions. */
#define		DU_DBV_VW10		DU_MAKE_VERSION(9,6,0)
...
**	    Treat DB_NOCOLLATION as UNSET too.
*/
...
#define DB_COLL_UNSET(dbv) ((dbv).db_collID==DB_NOCOLLATION ||\
				(u_i2)(dbv).db_collID >= DB_COLL_LIMIT)
...
**	    Corrected the VAR/PAD attributes for accuracy - not presently used
**	    but could be misleading.
**	17-Oct-2011 (kiria01) b125852
**	    Use DATE & INTV classes and remove C class.
**	03-Nov-2011 (kiria01) b125919, m2836
**	    Correct the ANSI date form priority to match that of DTE.
*/
...
_DEFINE(ADTE,    4, DATE, BASE,  6,  6, 10, 10  /* ANSI date type */)\
_DEFINE(MNY,     5, NUMB, BASE,  0,  0, 12, 12  /* "money" */)\
_DEFINE(TMWO,    6, DATE, BASE,  6,  6, 10, 10  /* time without time zone */)\
_DEFINE(TMW,     7, DATE, BASE,  6,  6, 10, 10  /* time with time zone */)\
_DEFINE(TME,     8, DATE, BASE,  6,  6, 10, 10  /* Ingres time (local time zone) */)\
_DEFINE(TSWO,    9, DATE, BASE,  6,  6, 10, 10  /* timestamp without time zone */)\
...
_DEFINE(TSW,    18, DATE, BASE,  6,  6, 10, 10  /* timestamp with time zone */)\
_DEFINE(TSTMP,  19, DATE, BASE,  6,  6, 10, 10  /* Ingres timestamp (local time zone) */)\
...
_DEFINE(CHR,    32, CHAR, PAD,   8,  8, 10, 10  /* "c" */)\
_DEFINE(INYM,   33, INTV, BASE,  6,  6, 10, 10  /* interval (year-month) */)\
_DEFINE(INDS,   34, INTV, BASE,  6,  6, 10, 10  /* interval (day-second) */)\
...
_DEFINE(QUE,    42, CHAR, VAR,   0,  0,  0,  0  /* query type      */)\
...
_DEFINE(UTF8,   47, NCHAR,VAR,   0,  0,  0,  0  /* UTF8 data type */)\
...
_DEFINE(NQTXT,  54, NCHAR,VAR,   0,  0,  0,  0  /* Type for qry text */)\
...
{ \
    ...
    ((DB_DATA_VALUE *)dv)->db_collID = DB_UNSET_COLL; \
}
...
**	5-june-2010 (dougi)
**	    Add #define for IVW_MAX_DECPREC (18 digits).
*/
...
#define                 DB_ISO4T_DFMT   (DB_DATE_FMT) 11 /* ISO date with     */                                                         /* yyyy              */
#define                 DB_ISO4TC_DFMT  (DB_DATE_FMT) 12 /* ISO date with     */
                                                         /* yyyy              */
														/* w 4-digit yr */
