...
**	24-sep-92 (daveb)
**	    documented
**	17-Jul-2011 (wanfr01) b125563
**	    Issue a one time warning if MO memory is full
*/
PTR
MO_alloc( i4  size, STATUS *stat )
{
    PTR block;
    static bool MOwarn = TRUE;
	...
	*stat = MO_MEM_LIMIT_EXCEEDED;
	...
	if (MOwarn)
	{
	    TRdisplay("%@ IMA MO memory exhausted.  IMA queries may show incomplete information.\n");
	    MOwarn = FALSE;
	}
    }
    else 
