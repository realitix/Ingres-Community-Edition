...
** Inputs:
**	size		number of bits in byte streams to 'xor'.
**			Rounded up to BITSPERBYTE.
**	vector1		input vector to 'xor' with vector2
**	vector2		'xor'ed with vector1
**
** Outputs:
...
/*{
** Name:	BTxor3 -- 'xor' two byte streams to distinct result
**
** Description:
**		Perform C 'xor' operation (^) on byte streams
**
...
**
** Outputs:
**	result		result of 'xor'ing vector1 and vector2
**
**	Returns:
**			address of result
** Exceptions:
    ...
    void	*result)
{
    ...
    /* Hand off to BTxor if result happens to one of vector1 or vector2 */
    if (vector1 == result)
	return BTxor(size, vector2, result);
    if (vector2 == result)
	return BTxor(size, vector1, result);
