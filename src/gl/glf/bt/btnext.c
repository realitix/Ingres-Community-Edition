...
BTnext(
	...
	i4		size)
{
	register const u_char *v = vector;	/* byte to examine */
	register i4	pos;
		 i4	start   = prev_bit + 1;
		 i4	ret_val = -1;
	...
	*/
	...
	else
	{
		...
		register const u_char *last = vector;	/* last byte to examine */

		last += BT_HIBYTE_MACRO(size);
