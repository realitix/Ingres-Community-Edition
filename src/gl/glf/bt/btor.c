...
** Inputs:
**	size		number of bits in byte streams to 'or'.
**			Rounded up to BITSPERBYTE.
**	vector1		input vector to 'or' with vector2
**	vector2		'or'ed with vector1
**
** Outputs:
...
/*{
** Name:	BTor3 -- 'or' two byte streams to distinct result
**
** Description:
**		Perform C 'or' operation (|) on byte streams
**
...
**
** Outputs:
**	result		result of 'or'ing vector1 and vector2
**
**	Returns:
**			address of result
** Exceptions:
    ...
    void	*result)
{
    ...
    /* Hand off to BTor if result happens to one of vector1 or vector2 */
    if (vector1 == result)
	return BTor(size, vector2, result);
    if (vector2 == result)
	return BTor(size, vector1, result);
