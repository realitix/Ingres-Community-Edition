...
/*{
** Name:	BTand3 -- 'and' two byte streams to distinct result
**
** Description:
**		Perform C 'and' operation (&) on byte streams
**
...
**
** Outputs:
**	result		result of 'and'ing vector1 and vector2
**
**	Returns:
**			address of result
** Exceptions:
    ...
    void	*result)
{
    ...
    /* Hand off to BTand if result happens to one of vector1 or vector2 */
    if (vector1 == result)
	return BTand(size, vector2, result);
    if (vector2 == result)
	return BTand(size, vector1, result);
