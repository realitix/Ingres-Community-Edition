...
**		Bug 99917. If the time is exactly the time that DST begins the
**		the incorrect GMT offset is returned.
**      22-Jun-2011 (coomi01) b125464
**          US-HAWAII fixed its gmt offset in 1947 having conducted a few
**          experiments before hand. Typically, our lookup sees the current
**          time as being somewhat later than 1947, and was searching the
**          experimental time sets to give the wrong response.
*/
i4
TMtz_search( PTR cb, TM_TIMETYPE tm_timetype, i4 tm_time)
{
    ...
    /* If lower than lowest bound.  Return standard offset */
    ...
    /* 
    ** If out of higher bound, return high end offset 
    ** ~ The higher bound is infact from some fixed point
    **   in the past to eternity, and therefore includes 
    **   the given momement in time.
    */
    if (tm_time > dt[high] + TM_tz_adj(high) )
    {
	return(tm_tz_cb->tzinfo[type[high]].gmtoff);
    }
