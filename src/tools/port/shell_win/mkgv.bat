...
REM	    Update script to also replace PRODUCTNAME.
REM
REM
call readvers
...
ccpp -s SYSTEM_PRODUCT_NAME| %AWK_CMD% "{print\"set PRODUCTNAME=\" $0}" >> varset.bat

cat varset.bat
call varset.bat
rm varset.bat
if "%inc%"=="" set inc=00
sed -e "s:RELVER1:%RELVER1%:g" -e "s:RELVER2:%RELVER2%:g" -e "s:RELVER3:%RELVER3%:g" -e "s:RELVER4:%RELVER4%:g" -e "s:BUILDNUM:%BUILD%%INC%:g" -e "s:YEAR:%date:~10, 4%:g" -e "s:PRODUCTNAME:%PRODUCTNAME%:g" "%ING_SRC%"\cl\clf\gv_win\gv.rc.template > "%ING_SRC%"\cl\clf\gv_win\gv.rc
endlocal
