...
REM	    Use VERS file options from readvers OPTDEF variable.
REM	12-Oct-2011 (drivi01)
REM	    Add functionality to retrieve SYSTEM_PRODUCT_NAME from gl.h.
REM	
...
call cl %FILENAME%.tmp /EP %OPTDEF% /DNT_GENERIC /DVERS=%CONFIG% /D%CONFIG% | sed -e "/^[	 ]*$/d" -e "s:/%%:/*:g" -e "s:%%/:*/:g" -e "s:^%%::" 
goto EXIT

:CYGWIN
call cl %FILENAME%.tmp /EP %OPTDEF% /DNT_GENERIC /DVERS=%CONFIG% /D%CONFIG% | sed -e "/^[	 ]*$/d" -e "s:/%%:/*:g" -e "s:%%/:*/:g" -e "s:^%%::" -e "s/$/\r/"
goto EXIT

:SYMB_GET
if "%2"=="SYSTEM_PRODUCT_NAME" goto PRODUCT_SYMB
echo %2| cat %ING_SRC%\tools\port\conf\CONFIG - > %2.tmp
call cl %2.tmp /EP /nologo %OPTDEF% /DNT_GENERIC /DVERS=%CONFIG% /D%CONFIG% > %2.tmp2 2>&1
...
:PRODUCT_SYMB
echo %2| cat %ING_SRC%\gl\hdr\hdr\gl.h - |grep -v GLOBALREF|grep -v GLOBALDLLREF> %2.tmp
call cl %2.tmp /EP /nologo %OPTDEF% /DNT_GENERIC /DVERS=%CONFIG% /D%CONFIG% >%2.tmp2 2>&1
sed -e "/^$/d" %2.tmp2|grep -v %2|cut -d"\"" -f2
rm %2.tmp %2.tmp2
