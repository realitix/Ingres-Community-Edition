...
REM	27-Jan-2011 (frima01)
REM	     Corrected setting CPU on a64.win.
REM     13-Jul-2011 (horda03)
REM          Corrected test for x%MT%, enclose it in "".
REM	17-Aug-2011 (drivi01)
REM	     Stop the GEOS loop from being infinite.  Instead ask
REM	     whether user wants to build GEOS, default is N (No).
REM	     If No is entered, GEOS loop is exited.
REM	     Fix %MT% setting, it's critical that this variable is
REM	     set correctly, it appears that depending on how it's set
REM	     before this script is run determines whether setup
REM	     script is successful or not.
REM	     This change will always reset MT whether it was set
REM	     by user or by this script, first the quotes will be
REM	     stripped out of the %MT%, and then the path will be expanded
REM	     to contain short names only, so there's no more dealing
REM	     with problems brought on by quotes.
REM	15-Sep-2011 (whiro01)
REM	     Find Xerces 3.1 version.
REM	23-Sep-2011 (drivi01) b125688
REM	     Add JAVA_HOME to the environment.  The intent is for
REM	     JAVA_HOME to be set automatically by detecting the correct
REM	     JDK version in registry and then setting JAVA_HOME 
REM	     appropriately.  If the routine fails then the script
REM	     will prompt for JDK location.  
REM	     Note this script will preserve JAVA_HOME setting 
REM	     in the environment if the version and build is correct.
REM	     Minimum JDK requirement is 1.6.0_14.
REM
...
if exist "%XERCESCROOT%\bin\xerces-c_3_1.dll" set XERCVERS=3_1
if "%XERCVERS"=="" echo XERCVERS could not be determined
echo XERCVERS=%XERCVERS%
:GEOS_LOOP
if "%BUILD_GEOS%"=="" SET /P BUILD_GEOS=Build Geospatial Y or N (default is N)?:
if "%BUILD_GEOS%"=="" SET BUILD_GEOS=N& goto KERBEROS_LOOP
if "%BUILD_GEOS%"=="N" goto KERBEROS_LOOP
...
{ ^
if (i==3) ^
printf "set JAVA_RESULT="; ^
printf $i; ^
if (i!=NF) ^
printf " "; ^
} ^
}'
if errorlevel 1 set OS_BIT=32
if errorlevel 0 set OS_BIT=64
if "%CPU%"=="AMD64" set SYS_WOW=false
if "%CPU%"=="x86" if "%OS_BIT%"=="64" set SYS_WOW=true
if "%CPU%"=="i386" if "%OS_BIT%"=="64" set SYS_WOW=true
if "%CPU%"=="x86" if "%OS_BIT%"=="32" set SYS_WOW=false
if "%CPU%"=="i386" if "%OS_BIT%"=="32" set SYS_WOW=false
if "x%JAVA_HOME%"=="x" goto NO_JAVA_HOME_SET

if not "x%JAVA_HOME%"=="x" for /F "usebackq tokens=1* delims=$" %%I in (`echo "%JAVA_HOME%"^|sed 's:jdk:$:g'`) do set JAVA_VERS=%%J
if NOT "x%JAVA_VERS:~0,-1%"=="x" if "%SYS_WOW%"=="false" for /F "usebackq tokens=1* delims=;" %%I in (`reg QUERY "HKLM\Software\JavaSoft\Java Development Kit\%JAVA_VERS:~0,-1%" ^|grep JavaHome`) do set JAVA_RESULT=%%I
set JAVA_VERS=%JAVA_VERS:~0,-1%
if not "x%JAVA_VERS%"=="x" if "%SYS_WOW%"=="true" for /F "usebackq delims=;" %%I in (`reg QUERY "HKLM\Software\Wow6432Node\JavaSoft\Java Development Kit\%JAVA_VERS:~0,-1%" ^|grep JavaHome`) do set JAVA_RESULT=%%I
if not "x%JAVA_HOME%"=="x" if exist %TEMP%\bldenv$$1.bat rm %TEMP%\bldenv$$1.bat
if not "x%JAVA_HOME%"=="x" if not "x%JAVA_RESULT%" == "x" echo %JAVA_RESULT%|%AWK_CMD% %AWK_SCRIPT% > %TEMP%\bldenv$$1.bat
if not "x%JAVA_HOME%"=="x" if exist "%TEMP%\bldenv$$1.bat" call %TEMP%\bldenv$$1.bat
if not "x%JAVA_HOME%"=="x" if exist "%TEMP%\bldenv$$1.bat" rm %TEMP%\bldenv$$1.bat

set TMP_JAVA_RESULT=%JAVA_RESULT:~1%
set TMP_JAVA_HOME=%JAVA_HOME:~1%
if not "x%JAVA_HOME%"=="x" if "%TMP_JAVA_RESULT%"=="%TMP_JAVA_HOME%" goto JAVA_PROMPT

:NO_JAVA_HOME_SET
if "%SYS_WOW%" == "false" (
for /F "usebackq tokens=1,2,3,4,5* delims=\" %%I in (`reg QUERY "HKLM\Software\JavaSoft\Java Development Kit"`) do (
if /I %%M GEQ 1.6.0_14 set JAVA_ACCEPTED=true& set JAVA_VERS=%%M
)
)
if "%SYS_WOW%" == "true" (
for /F "usebackq tokens=1,2,3,4,5* delims=\" %%I in (`reg QUERY "HKLM\SOFTWARE\Wow6432Node\JavaSoft\Java Development Kit"`) do (
if /I %%N GEQ 1.6.0_14 set JAVA_ACCEPTED=true& set JAVA_VERS=%%N
)
)
set AWK_SCRIPT='{ ^
for (i=3; i!=NF+1; i++) ^
...
if "%JAVA_ACCEPTED%" == "true" if "%SYS_WOW%" == "false" (
for /F "usebackq delims=;" %%I in (`reg QUERY "HKLM\Software\JavaSoft\Java Development Kit\%JAVA_VERS%" ^|grep JavaHome`) do set JAVA_RESULT=%%I
)
if "%JAVA_ACCEPTED%" == "true" if "%SYS_WOW%" == "true" (
for /F "usebackq delims=;" %%I in (`reg QUERY "HKLM\Software\Wow6432Node\JavaSoft\Java Development Kit\%JAVA_VERS%" ^|grep JavaHome`) do set JAVA_RESULT=%%I
)
if not "%JAVA_RESULT%" == "" echo %JAVA_RESULT%|%AWK_CMD% %AWK_SCRIPT% > %TEMP%\bldenv$$1.bat
if exist "%TEMP%\bldenv$$1.bat" call %TEMP%\bldenv$$1.bat
if exist "%TEMP%\bldenv$$1.bat" rm %TEMP%\bldenv$$1.bat

:JAVA_PROMPT
if "x%JAVA_HOME%"=="x" (
if "%CPU%"=="x86" set BUILD_BIT=32
if "%CPU%"=="i386" set BUILD_BIT=32
if "%CPU%"=="AMD64" set BUILD_BIT=64
)
if "x%JAVA_HOME%"=="x" if not "%BUILD_BIT%"=="" set /P JAVA_HOME=%BUILD_BIT%-bit JDK location (default C:\Program Files\Java\jdk1.7)?
if "x%JAVA_HOME%"=="x" if "%BUILD_BIT%"=="" set /P JAVA_HOME=JDK location (default C:\Program Files\Java\jdk1.7)?
if "x%JAVA_HOME%"=="x" set /P JAVA_HOME=C:\Program Files\Java\jdk1.7
if not "x%JAVA_HOME%"=="x" if exist "%JAVA_HOME%" set PATH=%JAVA_HOME%\bin;%PATH%
if "%JAVA_HOME%"=="" if not exist "%JAVA_HOME%" echo ERROR: Invalid Java location for this build.
set AWK_SCRIPT=
