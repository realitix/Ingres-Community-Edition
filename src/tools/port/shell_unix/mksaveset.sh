:
#
# Copyright (c) 1994, 2011 Actian Corporation
#
#
# DEST = utility
#
...
##	01-Feb-2011 (hanje04)
##	    Add local language message files for GUI to saveset
##	08-Apr-2011 (hanje04)
##	    Add support for building DEBs/APT saveset (non-GUI for now)
##	03-May-2011 (hanje04)
##	    Add check for reprepro utility and switch order of spec/control
##	    file generation and running buildrel so pack related errors are
##	    reported sooner.
##	17-May-2011 (hanje04)
##	    Pull extra python modules into saveset (for DEBs and RPM) as part
##	    of DEBs support for GUI install changes.
##	    Added missing GUI files to DEBs saveset
##	03-Jun-2011 (hanje04)
##	    Add support for signing packages.
##	    Triggered by "option SIGN_RELEASE" in the VERS file.
##	    Add appropriate checksig script to saveset for signed releases.
##	    NOTE: Requires gpg-agent running for signing.
##	15-Jun-2011 (hanje04)
##	    Mantis 2018
##	    Include uninstall_ingres_deb as uninstall_ingres for DEBs
##	19-Aug-2011 (frima01) SIR 125620
##	    Correct debrls setting to be valid in standard sh shell.
##	24-Aug-2011 (hanje04)
##	    Create link to new product specific logo graphics.
##	26-Sep-2011
##	    BUG 125688
##	    Add javautil.py to RPM/DEBs saveset
##                                         
. readvers
. (PROG1PRFX)sysdep
[ $SHELL_DEBUG ] && set -x
...
do
	...
	-deb)   case $vers in
		    *_lnx)
			...
			;;
		    *)
			echo "DEB not supported on ${unamem} ${uanmes}"
			exit 1
			;;
		esac
		shift
...
case $vers in
    ...
    i64_lnx|\
    ppc_lnx)
	if ( echo "$unamem" | grep -q "i.86" ) ; then
	    arch=i386
	else
	    arch=$unamem
	fi
	if $rpmrelease ; then
	    desc="linux-${arch}"
	elif $debrelease ; then
	    arch=${DEB_BUILD_ARCH}
	    desc="ubuntu-${DEB_BUILD_ARCH}"
	else
	    desc="linux-ingbuild-${arch}"
	fi
	;;
    int_w32)
	desc="win-x86"
...
if ( $rpmrelease || $debrelease )
then
    if $rpmrelease ; then
	cfgfile="${II_MANIFEST_DIR}/rpmconfig"
    else
	cfgfile="${II_MANIFEST_DIR}/debconfig"
    fi
...
$debrelease && 
{
    echo "Generating DEB CONTROL files"
    # check we have the tools we need
    reprepro --version >& /dev/null ||
    {
	cat << EOF
Cannot locate 'reprepro' utility, it can be installed by running:
...
EOF
    ...
    buildrel -d $unhidepkg ${spatflag} -l ${lictype} ||
    {
	echo "Failed to create CONTROL files..."
	exit 1
    }
}
...
if $rpmrelease || $debrelease ; then
    skipbld=false
    if $rpmrelease ; then
	basepkg=${productname}-${release}-${build}.${arch}.rpm
	bldroot=${II_RPM_BUILDROOT}/${arch}
	bldtool=iirpmbuild
    else
	debrls=`grep rpm_version $cfgfile | cut -d- -f2`
	debbldvers=${release}.${build}-${debrls}
	basepkg=${productname}_${debbldvers}_${arch}.deb
	bldroot=${II_DEB_BUILDROOT}/${arch}
	bldtool=iidebbuild
    fi
    ...
    do
	if $rpmrelease ; then
	    p=${productname}-${pkg}-${release}-${build}.${arch}.rpm
	else
	    p=${productname}-${pkg}_${release}-${build}_${arch}.deb
	fi
	[ -f ${bldroot}/${p} ] || [ -f ${bldroot}/${basepkg} ] &&
	{
	    echo "WARNING: One or more packages already exists for this build"
	    echo "Packages will NOT be recreated, existing files will be used"
	    skipbld=true
	    break
	}
    done

    # build pkgs if we need to
    [ "${unhidepkg}" ] && unhide=-unhide
    $skipbld || ${bldtool} ${unhide} ${spatflag} ||
    {
	echo "Failed to create one or more package..."
	exit 1
    }
...
then
    ...
    # create bin, pixmaps, locale and rpm directories
    mkdir bin locale pixmaps rpm || exit 1
    ...
    cp -p $ING_BUILD/lib/{instutil,javautil,debqry,rpmqry}.py bin || exit 1

    # signature validation script
    if $signrelease
    then
	cp $ING_BUILD/utility/checksig_rpm bin/checksig ||
	{
	    echo "Failed to copy checksig_rpm"
	    exit 1
 	}
    fi
...
EOF
	...
	rpm --addsign --define "_signature gpg" \
	    --define "_gpg_name (PROD1NAME) ING_VERSION" \
	    ./rpm/*.rpm ||
 	{
	    cat << EOF
An error occurred siging the release, does the correct key exist?
Try:
	gpg --list-keys "(PROD1NAME) ING_VERSION"
...
EOF
    ...
    # Link logo based on product
    ln -s (PROD1NAME)_logo.png pixmaps/logo.png ||
    {
	echo "Failed to create logo link"
	exit 1
    }

    # Copy images
    cp -pr $ING_BUILD/files/po/* locale ||
    {
	echo "Failed to find local language message files"
	exit 1
    }
...
then
    ## Layout for the DEB saveset should look like this:
    ##
    ##	<saveset root>
    ##		|
    ##		+--ingres_install
    ...
    {
	cat << EOF
Cannot locate /etc/lsb-release, which is required for building a DEBs saveset.
Is this a supported distribution?
...
EOF
	...
	{
	    cat << EOF
gpg-agent is required for signing DEBs release but does not appear to be
running. See the gpg-agen man page for info on how to start it

EOF
	    exit 1
	}
    fi
    # create bin and apt repository directories
    mkdir bin apt locale pixmaps || exit 1
    
    cp -p $ING_BUILD/bin/ingres_install . || exit 1
    cp -p $ING_BUILD/bin/uninstall_ingres_deb bin/uninstall_ingres || exit 1
    cp -p $ING_BUILD/utility/inginstgui bin || exit 1
    cp -p $ING_BUILD/lib/{instutil,javautil,debqry,rpmqry}.py bin || exit 1
    cp -p $ING_BUILD/utility/ingpkgqry bin || exit 1
    ...
    if $signrelease
    then
	cp $ING_BUILD/utility/checksig_deb bin/checksig ||
	{
	    echo "Failed to copy checksig_deb"
	    exit 1
 	}
    fi

    baserpm=${productname}_${release}-${build}_${arch}.deb

    # licensing
    cd bin &&
        mklicense -deb -l ${lictype} ||
	{
    	    echo "Failed to create ingres-LICENSE"
    	    exit 1
	}
...
EOF
	...
	for deb in ${II_DEB_BUILDROOT}/${arch}/*.deb
	do
	    dpkg-sig --verify-role origin $deb ||
	        dpkg-sig --sign-role origin -k "(PROD1NAME) ING_VERSION" $deb ||
		{
		    echo "Error signing deb packages"
		    exit 1
		}
	done
    ...
    # Generate repository
    echo "Generating repository with DEBs from $II_DEB_BUILDROOT"
    reprepro -V --basedir ${saveset}/apt \
	--confdir ${ING_BUILD}/files/apt/conf --component non-free \
	includedeb ${DISTRIB_CODENAME} ${II_DEB_BUILDROOT}/${arch}/*.deb &&
        reprepro -V --basedir ${saveset}/apt \
		--confdir ${ING_BUILD}/files/apt/conf export && 
        reprepro -V --basedir=${saveset}/apt \
		--confdir ${ING_BUILD}/files/apt/conf createsymlinks ||
    {
	echo "Error occurred creating DEBs repository"
	if $signrelease
	then
	    cat << EOF
The release is being signed, does the correct key exist?
Try:
	gpg --list-keys "(PROD1NAME) ING_VERSION"
...
then
    ...
    if $signrelease
    then
	echo "Signed release, exporting key..." &&
	    gpg --export --armour "(PROD1NAME) ING_VERSION" > \
		${tardir}-key.asc ||
 	{
	    cat << EOF
An error occurred exporting the public key, does the correct key exist?
Try:
	gpg --list-keys "(PROD1NAME) ING_VERSION"
