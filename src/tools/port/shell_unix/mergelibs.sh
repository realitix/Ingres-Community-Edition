...
##	16-Dec-2010 (bonro01)
##	    Save and restore the CC variable so that iisysdep does not
##	    override the value.  Ingres builds allow the CC variable to be
##	    changed but the iisysdep script is normally supposed to be used
##	    in Ingres installations to specify the defauls and be able to be
##	    modified by users to customize their installations.
##	    In this case gcapwd.c needs to use the same compiler used to build
##	    the rest of Ingres so the build CC needs to be restored after
##	    iisysdep is called.
...
# Save CC value so that iisysdep does not override the current environment
# setting so that it may be used to compile gcapwd.c with the same CC used
# to compile the rest of Ingres.
SAVE_CC=$CC

. (PROG1PRFX)sysdep

# Restore saved CC value
if [ -n "$SAVE_CC" ]; then
  CC=$SAVE_CC
fi
