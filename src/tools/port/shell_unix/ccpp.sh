...
#	4-Jun-2011 (kschendel)
#	    Do iquote for osx (see 17-jul-2009 above).
#	8-Jun-2011 (kschendel for dmonge)
#	    Discover that some osx's cpp doesn't like the space after -iquote.
#
...
case $conf in
	...
	*_osx)	CPP='/usr/bin/cpp'
		# OSX uses a gcc 4.x, so use iquote rather than -I-.
		incopt='-iquote'
		;;
	*)	CPP=/lib/cpp ;;
esac
