...
#	02-Nov-2009 (hweho01)
#	    Include x100client library in the iimerge archive.  
...
#	    Always include x100 now, x100 subdir is integrated from codev.
...
do_scf=false do_psf=false do_opf=false do_rdf=false do_qef=false do_qsf=false do_dbutil=false do_dmf=false do_gwf=false do_adf=false do_ulf=false do_tpf=false do_rqf=false do_gcf=false do_sxf=false do_cuf=false do_malloc=false do_becompat=false do_x100=false
[ $# = 0 ] && do_scf=true do_psf=true do_opf=true do_rdf=true do_qef=true do_qsf=true do_dbutil=true do_tpf=true do_dmf=true do_gwf=true do_adf=true do_ulf=true do_rqf=true do_gcf=true do_sxf=true do_cuf=true do_becompat=true do_x100=true
[ $# = 1 ] && [ $1 = "-lp${HB}" -o $1 = "-lp${RB}" ] &&  do_scf=true do_psf=true do_opf=true do_rdf=true do_qef=true do_qsf=true do_dbutil=true do_tpf=true do_dmf=true do_gwf=true do_adf=true do_ulf=true do_rqf=true do_gcf=true do_sxf=true do_cuf=true do_becompat=true do_x100=true

do_x100client=false
if [ -n "$conf_IVW" ] ; then
    [ $# = 0 ] && do_x100client=true
    [ $# = 1 ] && [ $1 = "-lp${HB}" -o $1 = "-lp${RB}" ] &&  do_x100=true
fi
...
do
 ...
 becompat)	do_becompat=true; shift ;;
   ...
   dbutil)	do_dbutil=true; shift ;;
     ...
     x100)	do_x100=true; shift ;;
x100client)	do_x100client=true; shift ;;
        *)	echo "Usage: $0 [-lp32|-lp64] [becompat] [scf] [psf] [opf] [rdf] [qef] \n\t[qsf] [dbutil] [tpf] [rqf] [gcf] [sxf] [cuf] [x100] [x100client]"; exit 1 ;;
    esac
done
...
if $do_x100 ; then check_libs="$check_libs iix100" ; fi
if $do_x100client ; then check_libs="$check_libs x100client" ; fi
...
#
# 	Adding x100 to iimerge.a
#

if $do_x100
then
    echo ""
    echo " Adding iix100 to iimerge.a "
    echo ""
    mkdir $workdir/iix100 2> /dev/null ; cd $workdir/iix100
    [ $? != 0 ] &&  { echo Aborting ... ; exit 3 ;}

    rm -f *
    echo Extracting objects ...
    ar x $INGLIB/libiix100.a
...
then
    echo ""
    echo " Adding x100client to iimerge.a "
    echo ""
    mkdir $workdir/x100client 2> /dev/null ; cd $workdir/x100client
    [ $? != 0 ] &&  { echo Aborting ... ; exit 3 ;}

    rm -f *
    echo Extracting objects ...
    ar x $INGLIB/libx100client.a
