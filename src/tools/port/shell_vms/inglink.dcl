...
$!!     25-Jul-2011 (horda03)
$!!         Enable CXX linking. CXX linking occurs when P1 = "-CPP"
$!!     19-Oct-2011 (horda03) b121386
$!!         Check for the existence of the symbol linkdebug_'imagename'
$!!         If it exists and is true then create a second image linked with
$!!         the debugger. This is quite separate from the case where 
$!!         linkflags_'imagename' has been assigned and contains the string "/deb"
$!!         Also allow .STB &.MAP files to be created, just set map_required to
$!!         some values. For POSIX thread builds, only build  /THREADS_ENABLED if
$!!         the .LOT file contains FRONTMAIN.
$!!
$skip:
$ if p1 .eqs. "-CPP"
$ then
...
$     if opttempl .nes. ""
$     then
$        search/noout/nowarn 'opttempl' frontmain.obj
$
$        if $status .eq. 1
$        then
...
$ if exepath .eqs. "ING_BUILD:[bin]"      .or. - 
     ...
     exepath .eqs. "ING_BUILD:[library]"  .or. -
     f$type( map_required ) .nes. ""
$ then 
...
$	'cxx'link /exec='exe' 'mapf''stb''dsff''te_flags' 'mainobj'
$
...
$
$ link_with_debug = 0
$ if (f$type(linkdebug_'exename') .nes. "")
$ then
$    if (linkdebug_'exename')
$    then
$      link_with_debug = 1
$      dbg_exe = exepath + exename + "_dbg"
$      if mapf .nes. "" 
$      then
$        dbg_stb = "/SYMB=ING_BUILD:[debug]''exename'_dbg.stb"
$        dbg_map_file="optpath:''exename'_dbg.map"
$        dbg_mapf = "/MAP=''dbg_map_file'"
$        dbg_mapf_copy = "ING_BUILD:[debug]''exename'_dbg.map"
$        if f$type( dsf_required ) .nes. "" then -
              dbg_dsff = "/DSF=ING_BUILD:[debug]''exename'_dbg.dsf"
$        endif
$    endif
$ endif
$
...
$     sysexe_flag = "/sysexe=selective"
$ endif
$ 'cxx'link 'LINKFLAGS' 'sysexe_flag' /exec='exe' 'stb' 'mapf' 'dsff' 'te_flags' -
	optpath:'exename'.opt/options
$ if (link_with_debug)
$ then
$   'cxx'link/deb 'LINKFLAGS' 'sysexe_flag' /exec='dbg_exe' 'dbg_stb' 'dbg_mapf' 'dbg_dsff' 'te_flags' -
        optpath:'exename'.opt/options
$ endif
$
$ if mapf_copy .nes. ""
...
$     if (link_with_debug)
$     then
$        backup /nolog /new_version /ignor=inter 'dbg_map_file'; 'dbg_mapf_copy'
$     endif
$ endif
