...
**	    Consolidated external references.
**	    Tightened type checking of the CM data.
**	    Collected common definitions.
*/
...
# define	CM_A_UPPER	0x0001	/* Upper case ALPHA */
# define	CM_A_LOWER	0x0002	/* Lower case ALPHA */
# define	CM_A_NCALPHA	0x0004	/* non-cased alpha */
# define	CM_A_DIGIT	0x0008	/* DIGIT */
# define	CM_A_SPACE	0x0010	/* SPACE */
# define	CM_A_PRINT	0x0020	/* PRINTABLE */
# define	CM_A_CONTROL	0x0040	/* CONTROL */
# define	CM_A_DBL1	0x0080	/* 1st byte of double byte character */
# define	CM_A_DBL2	0x0100	/* 2nd byte of double byte character */
# define	CM_A_NMSTART	0x0200	/* Leading character of a name */
# define	CM_A_NMCHAR	0x0400	/* Trailing character of a name */
# define	CM_A_HEX	0x0800	/* Hexadecimal Digit */
...
/*
** CM_DEREF - dereference pointer and (implied) apply 8 bit mask
**	10-Oct-2011 (kiria01) b125830
**	    Drop mask & use correct cast to avoid sign extension.
**	26-Oct-2011 (kiria01) b125830
**	    Make sensitive to bigendian use with legacy C use of
**	    int variables to cope with EOF.
*/
# define	CM_DEREF(s)	((u_char)(*(s)))
# define	CM_DEREF1(s)	(*((u_char*)(s)+1))
# define	CM_DEREF2(s)	(*((u_char*)(s)+2))
# define	CM_DEREF3(s)	(*((u_char*)(s)+3))

/*
** CM_UTF8MULTI - Checks if this is a multibyte UTF8 character.
**	10-Oct-2011 (kiria01) b125830
**		    Extra masking is not needed as we are just looking
**		    at the bit pattern. Similar applies below.
*/
# define        CM_UTF8MULTI(s)   (*(s) & 0x80)

/*
** CM_UTF8LEAD - Checks whether a lead byte of a UTF8 character
** Follows a pattern eg 0x11...
*/
# define        CM_UTF8LEAD(s)    ((*(s) & 0xC0) == 0xC0)

/*
** CM_UTF8FOLLOW - Checks whether a following byte of a UTF8 character.
** Follows a pattern eg 0x10...
*/
# define        CM_UTF8FOLLOW(s)  ((*(s) & 0xC0) == 0x80)

/*
** CM_UTF8_SZn - True if char has n bytes
**	10-Oct-2011 (kiria01) b125830
**	    This mirrors the CM_BytesForUTF8[] array except it does not
**	    need the extra memory lookup relying on just the character
*/
# define	CM_UTF8_SZ1(s)	((*(s) & 0x80) == 0x00)
# define	CM_UTF8_SZ2(s)	((*(s) & 0xE0) == 0xC0)
# define	CM_UTF8_SZ3(s)	((*(s) & 0xF0) == 0xE0)
# define	CM_UTF8_SZ4(s)	((*(s) & 0xF8) == 0xF0)
# define	CM_UTF8_SZEXC(s)((*(s) & 0xF8) == 0xF8)

/*
** CM_UTF8_SZnCHK - True if char has n bytes AND check that followers are valid
**	10-Oct-2011 (kiria01) b125830
**	    More paranoid form of CM_UTF8_SZn
*/
# define	CM_UTF8_SZ1CHK(s) (CM_UTF8_SZ1(s))
# define	CM_UTF8_SZ2CHK(s) (CM_UTF8_SZ2(s)&&CM_UTF8FOLLOW(s+1))
# define	CM_UTF8_SZ3CHK(s) (CM_UTF8_SZ3(s)&&CM_UTF8FOLLOW(s+1)&&CM_UTF8FOLLOW(s+2))
# define	CM_UTF8_SZ4CHK(s) (CM_UTF8_SZ4(s)&&CM_UTF8FOLLOW(s+1)&&CM_UTF8FOLLOW(s+2)&&CM_UTF8FOLLOW(s+3))

/*
** CMUTF8cnt - size in bytes of a UTF8 character. This used to reference
** CM_BytesForUTF8 (via CM_UTF8Bytes) but we now use compares inline to reduce
** memory accesses & hence upsetting the cache.
** This represents the size in number of bytes of a character beginning with the
** byte value index. Note that this is arranged to correctly decode 2, 3 and 4
** byte UTF-8 and that the 0x80-0xBF followers and the 5&6 bit leaders are
** configured to return 1.
*/
# define CMUTF8cnt(s)	(CM_DEREF(s) < 0xC0 ? 1\
			:CM_DEREF(s) < 0xE0 ? 2\
			:CM_DEREF(s) < 0xF0 ? 3\
			:CM_DEREF(s) < 0xF8 ? 4\
			:1)
...
/*
** CM_UTF8trailingbytes - bytes following for UTF8
** This represents the number of following bytes expected given a character
** beginning with the byte value index. Note that unlike CMUTF8cnt, this strictly
** decodes for 5&6 byte UTF-8 which do not encode legal Unicode values (yet).
** This too is inlined instead of being encoded as a table lookup.
*/
#define CM_UTF8trailingbytes(s) (CM_DEREF(s) < 0xC0 ? 0\
				:CM_DEREF(s) < 0xE0 ? 1\
				:CM_DEREF(s) < 0xF0 ? 2\
				:CM_DEREF(s) < 0xF8 ? 3\
				:CM_DEREF(s) < 0xFC ? 4\
...
/*
** Define macros for CM_isUTF8 & CM_isDBL so that CM macros that
** use them may may be externally controlled. (eg adupatda.c adupatexec.c) 
*/
...
/*
** These definitions apply for source to be linked against libcompat.
** These are exported so that accessors will be fixed up at link load time
*/
GLOBALDLLREF i4			CMdefcs;
GLOBALDLLREF const u_i2		*CM_AttrTab;
GLOBALDLLREF const u_char	*CM_CaseTab;
GLOBALDLLREF const UTF8		*CM_UTF8firstByteMask;
GLOBALDLLREF const UTF32	*CM_offsetsFromUTF8;
GLOBALDLLREF const u_i4		*CM_UTF8Bytes;
GLOBALDLLREF const CM_UTF8ATTR	*CM_UTF8AttrTab;
GLOBALDLLREF const CM_UTF8CASE	*CM_UTF8CaseTab;
GLOBALDLLREF bool		CM_isUTF8;
GLOBALDLLREF bool		CM_isDBL;

# else
/*
** These definitions are used for source code in libcompat or non-Windows.
** The __IN_CMTYPE_C__ and __IN_CMUTYPE_C__ conditionals avoid the windows
** compiler complaining about conflicts between GLOBALREF & GLOBALDEF
*/
#ifndef __IN_CMTYPE_C__
GLOBALREF const u_i2		CM_DefAttrTab[];
GLOBALREF const u_char		CM_DefCaseTab[];
GLOBALREF const i4		CMdefcs;
GLOBALREF const	u_i2		*CM_AttrTab;
GLOBALREF const	u_char		*CM_CaseTab;
#endif

#ifndef __IN_CMUTYPE_C__
GLOBALREF const CM_UTF8ATTR	CM_attrtab_UTF8[];
GLOBALREF const CM_UTF8CASE	CM_casetab_UTF8[];
GLOBALREF const u_i4		CM_BytesForUTF8[];
GLOBALREF const UTF8		*CM_UTF8firstByteMask;
GLOBALREF const UTF32		*CM_offsetsFromUTF8;
GLOBALREF const	u_i4		*CM_UTF8Bytes;
GLOBALREF const	CM_UTF8ATTR	*CM_UTF8AttrTab;
GLOBALREF const	CM_UTF8CASE	*CM_UTF8CaseTab;
#endif
...
# define CMGETATTR(s)	((CMGETUTF8 && CM_UTF8MULTI(s))\
			? cmu_getutf8property((u_char *)(s), (i4)CMUTF8cnt(s))\
			: CM_AttrTab[CM_DEREF(s)]\
			)
# define CMGETATTR_SB(s) CM_AttrTab[CM_DEREF(s)]

# define CMGETCASE(s)	CM_CaseTab[CM_DEREF(s)]
...
# define cmdbl2nd(s)	(CMGETUTF8\
			? CM_UTF8FOLLOW(s)\
			: (CMGETATTR(s) & CM_A_DBL2)\
			)
...
# ifdef	DOUBLEBYTE
# define CMdbl1st(str)	((CMGETUTF8) ? (CM_UTF8LEAD(str)) : (CMGETATTR(str) & CM_A_DBL1))
# else
/* Allow optimizer to cut out redundant DOUBLEBYTE code */
...
# define CMnmstart(nm)	(CMGETDBL\
			? ((CMGETATTR(nm) & CM_A_NMSTART)\
			    ? (CMdbl1st(nm)\
				? (cmdbl2nd((nm)+1) && !cmdblspace(nm))\
				: TRUE)\
			    : FALSE)\
...
# define CMnmchar(nm)	(CMGETDBL\
			? ((CMGETATTR(nm) & (CM_A_NMSTART|CM_A_NMCHAR))\
			    ? (CMdbl1st(nm)\
				? (cmdbl2nd((nm)+1) && !cmdblspace(nm))\
				: TRUE)\
			    : FALSE)\
...
# define CMwhite(str) (CMGETDBL\
		       ? ((CMGETUTF8 && CM_UTF8MULTI(str))\
			  ? ((CMGETATTR(str)&CM_A_SPACE)\
			    ? TRUE\
			    : FALSE)\
			  : ((CM_AttrTab[CM_DEREF(str)] & CM_A_SPACE) || cmdblspace(str)))\
		       : (CM_AttrTab[CM_DEREF(str)] & CM_A_SPACE))
# define CMwhite_SB(str) (CM_AttrTab[CM_DEREF(str)] & CM_A_SPACE)

/*{
...
# define CMspace(str)	(CM_DEREF(str) == ' ' || ((CMGETDBL) && cmdblspace(str)))
# define CMspace_SB(str) (CM_DEREF(str) == ' ')
...
**	    Written
*/
# define CM_ISUTF8_BOM(src) ((CMGETUTF8) && CM_DEREF(src) == 0xEF && CM_DEREF1(src) == 0xBB && CM_DEREF2(src) == 0xBF)
...
**	    Create single byte version
**	10-Oct-2011 (kiria01) b125830
**	    Flatten out reducing cache access
*/
# ifdef	DOUBLEBYTE
# define CMcpychar(src, dst)(CMGETUTF8\
			    ?	(CM_UTF8_SZ1(src)\
				? (*(dst) = *(src))\
				:CM_UTF8_SZ2(src)\
				? (*(dst) = *(src),*((dst)+1) = *((src)+1))\
				...
				:CM_UTF8_SZ4(src)\
				? (*(dst) = *(src),*((dst)+1) = *((src)+1),*((dst)+2) = *((src)+2),*((dst)+3) = *((src)+3))\
				: (*(dst) = *(src)))\
			    :	(CMGETATTR(src) & CM_A_DBL1)\
				? (*(dst) = *(src),*((dst)+1) = *((src)+1))\
				: (*(dst) = *(src))\
			    )
# else
...
*/
# ifdef	DOUBLEBYTE
# define CMcpyinc(src,dst) (CMGETUTF8\
			    ?	(CM_UTF8_SZ1(src)\
				? (*(dst)++ = *(src)++)\
				:CM_UTF8_SZ2(src)\
				? (*(dst)++ = *(src)++, *(dst)++ = *(src)++)\
...
*/
# ifdef	DOUBLEBYTE
# define CMtolower(src, dst) ((CMGETDBL)\
			     ? ((CMGETUTF8 && CM_UTF8MULTI(src))\
				? (cmu_getutf8_tolower((u_char *)src, CMUTF8cnt(src), (u_char *)dst, 0))\
				: (CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1))\
			     : (*(dst) = cmtolower(src)))) : (*(dst) = cmtolower_sb(src)))
# else
...
# ifdef	DOUBLEBYTE
# define CMtoupper(src, dst) ((CMGETDBL)\
			     ?((CMGETUTF8 && CM_UTF8MULTI(src))\
				? (cmu_getutf8_toupper((u_char *)src, CMUTF8cnt(src), (u_char *)dst, 0))\
				: (CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1))\
			     : (*(dst) = cmtoupper(src)))) : (*(dst) = cmtoupper_sb(src)))
# else
...
**	    Create single byte version
**	10-Oct-2011 (kiria01) b125830
**	    Use CMbytecnt directly allowing for common
**	    code fragments to be optimized.
*/

# ifdef	DOUBLEBYTE
# define CMbyteinc(count, str)	(count+=CMbytecnt(str))
# else
...
# ifdef	DOUBLEBYTE
# define CMbytedec(count, str)	(count-=CMbytecnt(str))
# else
...
# ifdef	DOUBLEBYTE
# define CMnext(str)	((str)+=CMbytecnt(str))
# else
...
**	the user to include the global header CM.h.  
**
**	WARNING: If the CMprev macro is called with the 'str'
**	already pointing at 'startpos' it will be simply
**	decremented. It is the responsibility of the caller
**	to check that the updated pointer is valid. Legacy code
**	relies on this behaviour and loop termination checking
**	must be done bearing this in mind.
**
**(Usage:
...
**
** Side Effects:
**	May leave 'str' pointing at invalid data.
**
** History:
**	18-sep-86 (yamamoto)
**		first written
...
**	    Create single byte version
**	26-Oct-2011 (kiria01) b125830
**	    Add explicit check for buffer start and do not allow
**	    needless dereference in that case. Added warning above.
*/
#define CMuprev(str, startpos) ((str) -= (cmupct((u_char *)(str), (u_char *)startpos))) 
# ifdef	DOUBLEBYTE
# define CMprev(str, startpos)	((!(CMGETDBL) || (u_char *)(str)==(u_char *)(startpos))\
				? --(str)\
				: CMGETUTF8\
				? CMuprev(str, startpos)\
				: (cmkcheck((u_char *)((str)-1), (u_char *)startpos) == CM_A_DBL2)\
				? (str) -= 2\
				: --(str)\
				)
# else
...
# ifdef	DOUBLEBYTE
# define CMcmpnocase(str1, str2)(CMGETDBL\
				? CMdbl1st(str1)\
				    ? CMcmpcase(str1, str2)\
...
# ifdef	DOUBLEBYTE
# define CMcmpcase(str1, str2)	(CMGETDBL\
				?  (CMdbl1st(str1)\
				    ?  ((i4)CM_DEREF(str1) == (i4)CM_DEREF(str2))\
					? ((i4)CM_DEREF1(str1) - (i4)CM_DEREF1(str2))\
					: ((i4)CM_DEREF(str1) - (i4)CM_DEREF(str2))\
				    : ((i4)CM_DEREF(str1) - (i4)CM_DEREF(str2)))\
				: ((i4)CM_DEREF(str1) - (i4)CM_DEREF(str2))\
				)
