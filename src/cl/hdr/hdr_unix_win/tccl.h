...
#ifdef SEP_SYNC
#define     TC_CONTINUE     -666
#define	    TC_BUSY	    (i4)-667 /* busy on TCgetc */
#define     TC_RESTART      -668
#endif /* SEP_SYNC */
...
struct _TCFILE
{
	...
	int	_size;
#ifdef SEP_SYNC
    /* Augment the TCFile descriptor with 
    ** details of SepSync connectivity
    */
        char    connected;
        char    direction;
        struct  _sepIPCBufferType *ipcBuffer;
#endif
};
# endif /* NT_GENERIC */
...
#ifdef SEP_SYNC
#define     _TCUSEPIPE      0x40 /* Pipes were used in an earlier version
				 ** ~ May be brought back to deal with shell script output
				 */
#define     _TCUSESOCK      0x80
#define     _TCUSEIPC       (_TCUSEPIPE | _TCUSESOCK)
#endif /* SEP_SYNC */
