/*
** Copyright (c) 1985, 2009, 2011 Actian Corporation
**
*/
...
**	09-dec-2009 (zhayu01) SIR 121210
**		Modified MINI4LL for Pocket PC/Windows CE port.
**		Define SETCLERR use wceerrno/(wce_errno()) on Windows CE/Pocket PC.
**		wce_errno() will use GetLastError() to get API error and map
**		OS error code to System V error code. If there is no match,
**		EINVAL (22) will be returned.
...
**	25-Feb-2011 (drivi01)
**	    if ssize_t is defined, SSIZE_TYPE should be defined as
**	    ssize_t not size_t to make it signed.
**	03-Mar-2011 (drivi01)
**	    Add #else for SIZE_TYPE definition if size_t isn't
**	    defined, otherwise SIZE_TYPE is redefined.
**	04-Mar-2011 (drivi01)
**	    Define SIZE_TYPE_MAX for size_t.  To avoid duplicate
**	    definitions, take it out of "ifdef (size_t)" block.
**	10-Oct-2011 (kiria01) b125830
**	    Collected Unicode related types.
*/
...
typedef 	u_i2    	UCS2;	/* This is the Unicode character 
					...
					** upto 2 bytes.
				        */
typedef 	u_i4    	UCS4;	/* This is the Unicode character 
					** data type, ie. unicode code point 
					** upto 4 bytes.
				        */
/*
...
typedef UCS2            g_char;
/*
** Unicode character types (deprecated)
*/
typedef UCS2	ucs2;
typedef UCS4	ucs4;
typedef UTF8	utf8;
...
# else
#ifdef LP64
# define SIZE_TYPE	unsigned __int64
#else
...
**
******************************************************************************/
#if defined(WCE_PPC) && !defined(errno)
int wce_errno(void);
#define wceerrno   (wce_errno())
