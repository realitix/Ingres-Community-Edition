...
bye(void)
{
	...
	{
	    SIfprintf(stderr, "*** Test Failed! ***  Expected %d, got %lld.\n",
	...
	else
	{
	    ...
	    SIfprintf(stderr, "Test Passed with %lld successful increments.",
		      Pshm->QuadSum);
	    SIfprintf(stderr, " %lld contentions detected.\n",
		      calls - Pshm->QuadSum);
	}
...
static VOID
logger( i4 errnum, DB_ERROR *dberror, CL_ERR_DESC *dum, i4 numargs, ... )
{
    char buf[ ER_MAX_LEN ];
    va_list args;
