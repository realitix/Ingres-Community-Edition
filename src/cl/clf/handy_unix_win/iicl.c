...
**      Use wceerrno for Pocket PC/Windows CE.
*/
...
**	9-Aug-2011 (kschendel)
**	    Fix above fix to pass the temp buffer len to snprintf, not the
**	    caller's buffer len -- which might be larger!
*/
...
# define have_fcvt_r
    ...
    */
	...
	char	*tmpbufp = tmpbuf;
        ...
        gen_Psem(&CL_misc_sem);
	*decpt = snprintf( tmpbuf, sizeof(tmpbuf), "%.0f", value );
	/*
