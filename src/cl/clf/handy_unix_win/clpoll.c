...
**	25-Jul-2011 (wanfr01) b125547
**	    Prevent timer.now from overflowing
*/
...
# endif /* USE_SELECT */
	...
	*/
	...
	if ( i == 0 )
	{
 	    if (MAXI4 - timein < CL_poll_ptr->timer.now)
 		CL_poll_ptr->timer.now = MAXI4;
 	    else
	...
	{
 	    if (MAXI4 - (i * 10) < CL_poll_ptr->timer.now)
 		CL_poll_ptr->timer.now = MAXI4;
 	    else
