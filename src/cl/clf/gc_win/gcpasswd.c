/*
** Copyright (c) 1987, 2011 Actian Corporation
*/
...
**	26-Jul-2011 (Bruce Lunsford) Bug 125598
**	    Remove default Ingres (iimksec) security attribute that
**	    allows public full control access to the shared memory
**	    used to cache IIPROMPT1/ONCE userids and passwords.
**	    Instead use a NULL security attribute, which defaults to
**	    Windows security descriptor, and prevents unauthorized
**	    users (that is, any user other than that under which Ingres
**	    is running) from accessing and modifying the data.
**	    Also, in case the shared memory data is nevertheless
**	    modified by some external program, use string copy/compare
**	    functions that limit the lengths of the strings to avoid
...
** History:
**	26-Jul-2011 (Bruce Lunsford) Bug 125598
**	    Use a "length-checking" string copy function (STlcopy)
**	    instead of STcopy() to prevent potential buffer overrun
**	    when copying data from the shared memory IIPROMPT cache
**	    to caller's buffers.  This reduces exposure to
**	    potential hacking of shared memory cache.  The size of
**	    the destination field of the caller is not known here
**	    (it's up to the caller to make large enough), so
**	    the size of the source VNODECACHE field is used as
**	    the limiting length. Similar for string compare function:
**	    STcompare() to STncmp().
**
*/
    ...
    char *password)
{
    ...
    for (i = 0; i < MAXVNODES; i++)
    {
        if (!STncmp(vnode, vinfo->vnode, sizeof(vinfo->vnode)))
        {
	    /*
	    ** Use STlcopy to limit length of copy and force EOS
	    ** to be safe since vinfo is in shared memory and its
	    ** string fields may not be properly EOS terminated.
	    */
            STlcopy (vinfo->username, username, sizeof(vinfo->username)-1);
            STlcopy (vinfo->password, password, sizeof(vinfo->password)-1);
            if (hCacheMutex != NULL)
                ReleaseMutex(hCacheMutex);
            return (TRUE);
        }
