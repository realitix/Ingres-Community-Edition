...
**          11-Feb-2000 (ashco01)
**          Extended size of envbuffer to 1024 via introduction of MAXENVLINE
**          defined value. This allows URL parameter lists of 1024 characters
**          to be passed via the CGI GET method to ICE. Amended      
**          GetEnvironmentVariable error handling to return NULL pointer if
**          environment variable greater than local buffer size.
**      24-Aug-2011 (ashco01) Bug:99835 (cont)
**          Missing line from inital cross-int above. 
**/
...
NMgtUserAt (char* name, char** value)
{
    ...
    envlen = GetEnvironmentVariable (name, envbuffer, MAXENVLINE);
     
    if ( envlen == 0 || envlen > MAXENVLINE )
    {
        /* Environment variable does not exist or larger than local buffer */
        *value = NULL;
    }
    else
    {
