...
**          CSMT_execute_asts expects the scb->cs_evcb->event_sem to be held,
**          on entry. If it has work to do then on exit it locked this semaphore
**          which lead to hangs in the (few) cases where the semaphore was not
**          expected to be held.
**/
...
#if defined(VMS)
static i4 CSMT_execute_asts(CS_SCB* scb, bool sem_locked);
#endif
...
CSMTterminate(i4 mode, i4  *active_count)
{
	...
	*/
	{
		     ...
		     scb = scb->cs_prev)
		{
			...
			(*Cs_srv_block.cs_disconnect)(scb);
			if (status)
			    (*Cs_srv_block.cs_elog)(status, (DB_ERROR *)NULL,
                                                    NULL, 0);
		    }
		}
		/*
	...
	*/
	    ...
	    Cs_srv_block.cs_num_sessions <= 0)
	{
		...
		(*Cs_srv_block.cs_elog)(E_CS0018_NORMAL_SHUTDOWN, 
...
# endif
	...
	*/
	...
	if ( status || ret_val )
	    (*Cs_srv_block.cs_elog)(ret_val, (DB_ERROR *)NULL, NULL, 0);
    }
...
#if defined(VMS)
static i4
CSMT_execute_asts(CS_SCB* scb, bool sem_locked)
{
    ...
    {
         if (!sem_locked) CS_synch_lock( &scb->cs_evcb->event_sem );
         ...
         if (!sem_locked) CS_synch_unlock( &scb->cs_evcb->event_sem );
    }
...
#if defined(VMS)
                /* ast_in_progress flag is set/unset with the scb->cs_evcb->event_sem locked, so
                ** if its set use then the session is executing IIMISC_executePollAst.
                ** Set the resume_pending flag (event_sem locked above).
                */
                if (scb->cs_evcb->ast_in_progress && !scb->cs_evcb->resume_pending)
                {
                   CS_LOCK_LONG(&scb->cs_evcb->resume_pending);
                }
#endif
	    }
	}
