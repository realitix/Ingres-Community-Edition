...
**          simply returns FAIL. Changed the third parameter of SIpinit and
**          SIpinitEx from i2 to HWND.
**	25-aug-2007 (crido01) Bug 119017
**	   In SIw30_write, break out of the Peek/Translate/Dispatch loop
**	   if 2 WM_PAINT or WM_NCPAINT messages are received in a row.
**	11-may-2009 (zhayu01) Bug 122037
**	    Convert and write out UTF16 string to trace window for UTF8 data.
**	29-sep-2009 (andreasgiessel, berbo01) Sir 122656
**	    In SIdoorfrmt, optionally add timestamp  to trace window messages.
**	21-jan-2011 (dupbr01) Sir 124951
**	   Do not wrap the message in the log file. For this do not add
**	   a '\n' in the buffer just add an flag used when writing to the
**	   window, this new flag is passed to SIw30_write.
**	   Introduce new environment variable II_W4GL_TRACE_TRUNCATED for
**	   backward compatibility.
**	   Also in SIw30_write move the code that scroll the window after the
**	   test of that window.
**	22-jan-2011 (dupbr01) 124954
**	   Integrate from w4si/sipinit.c the part of the code for the support
**	   of flag logonly on Windows. The part analyzing the command line
**	   had already been implemented on Windows.
**	16-mar-2011 (dupbr01) Bug 125197
**	   One character was eaten if a word had to be wrapped.
...
)
{
    ...
    */
	...
	if (TraceFlg == TRACE_LOGONLY)
	{
	    return(OK);                 /* Do not create trace window */
	}
...
**	29-sep-2009 (andreasgiessel, berbo01)
**	    Optionally add timestamp  to trace window messages (if II_W4GL_TRACE_TIMESTAMP=TRUE)
**
*/

VOID SIdoorfrmt(i4 calltype, FILE *outarg, char *fmt, va_list ap);

static i2     value_tracewrap = -1;
static i2     value_tracetimestamp = -1;
static u_i4   tracemsg_buffer_size = 500;
static bool   last_fmt_had_crlf = TRUE;
static char   timestamp_buffer[TM_SIZE_STAMP + 1];
static char   *tracemsg_buffer = NULL;
...
VOID SIprint_ortrace( char    *fmt, va_list ap )
{
	...
	i4 l_strlength;
	char *value_tracetimestamp_env;
	char *value_tracewrap_env;
	bool tracewithtime = FALSE;
	l_strlength = STlength(fmt);

	if (value_tracetimestamp == -1)
	{
		NMgtAt("II_W4GL_TRACE_TIMESTAMP", &value_tracetimestamp_env);
		if (value_tracetimestamp_env && *value_tracetimestamp_env
			&& (*value_tracetimestamp_env=='T' || *value_tracetimestamp_env=='t')
		)
		{
			value_tracetimestamp = TRUE;
			tracemsg_buffer = MEreqmem( NULL, tracemsg_buffer_size, FALSE, NULL );
		}
		else
		{
		value_tracetimestamp = FALSE;
		}
	}

	if (value_tracewrap == -1)
	{
		NMgtAt("II_W4GL_TRACE_WRAPPED", &value_tracewrap_env);
		if (value_tracewrap_env && *value_tracewrap_env
			&& (*value_tracewrap_env=='T' || *value_tracewrap_env=='t')
		)
		{
			value_tracewrap = TRUE;
		}
		else
		{
			value_tracewrap = FALSE;
		}
	}

	if (value_tracetimestamp == TRUE)
	{
		if (last_fmt_had_crlf)
		{
			SYSTIME systime;

			if ((i4)tracemsg_buffer_size < l_strlength + TM_SIZE_STAMP + 4)
			{
				MEfree(tracemsg_buffer);
				tracemsg_buffer_size = TM_SIZE_STAMP + l_strlength + 4;
				tracemsg_buffer = MEreqmem( NULL, tracemsg_buffer_size, FALSE, NULL );
			}
			TMnow( &systime );
			TMstr( &systime, timestamp_buffer );
			STprintf(tracemsg_buffer, "[%s] %s", timestamp_buffer, fmt);
			SIdoorfrmt((i4)SI_SPRINTF, (FILE *)printbuf, tracemsg_buffer, ap);
			tracewithtime = TRUE;
		}

		if (l_strlength > 0 && fmt[l_strlength-1]=='\n')
		{
			last_fmt_had_crlf = TRUE;
		}
		else
		{
			last_fmt_had_crlf = FALSE;
		}
	}
