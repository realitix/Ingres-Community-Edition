...
**  20-aug-2009 (zhayu01) SIR 121210
...
**
**	To reduce confusion with printf-class routines, an ll modifier
**	(as in %lld) is also allowed and means "i8".  Note that unlike
**	printf-class routines, %ld here asks for i8, not long.
**
** Inputs:
...
**	    printf-class routines need %lld/%llu for i8's, not just %ld/%lu.
**	    Allow the ll modifier here so that TR routines act more like
**	    printf routines.  (For TR, however, a single l still means i8
**	    and not just "long".)
*/
#if defined(__STDARG_H__)
STATUS
...
**	    for VMS clusters.
**	25-Aug-2009 (kiria01)
**	    Allow for a NULL address if %s specified with 0 precision.
...
# endif
	...
	case 's':
	    ...
	    if (precision == 0 && (noprecision || format_buffer)) /* Allow NULL ptr if prec=0 */
	        for (format_length = 0; format_buffer[format_length]; format_length++)
	            ;
	    break;
...
**	09-jun-2003 (devjo01)
**	    Add support for TR_F_SAFEOPEN. b110302.
**      10-Jun-2011 (hanal04) SIR 125429
**          Add support for %p (pid) and %d (YYYYMMDD) substitutions to all
**          TR_F_OPEN callers. Callers that want to record the expanded
**          file name can pass in the new TR_F_OPEN_UPD flag and should specify
**          the length of the buffer passed in (not the length of the string
**          inside that buffer.
**      31-Oct-2011 (hanal04) Bug 125906
**          Use PCpidOwn() to ensure %p is the current pid, even for spawned
**          processes.
*/
STATUS
...
CL_ERR_DESC        *err_code;
{
    ...
    if(flag == TR_F_OPEN_UPD)
    {
        flag = TR_F_OPEN;
        upd_buflen = name_length;
        name_length = STlength(filename);
    }
    ...
    if(flag == TR_F_OPEN)
    {
        un_env[0] = EOS;
        next_index=filename;
        while (NULLPTR != (pct_index = STchr(next_index, '%')))
        {
            if ((*(pct_index+1)) != 'p' && (*(pct_index+1)) != 'd' &&
                (*(pct_index+1)) != 'P' && (*(pct_index+1)) != 'D')
            {
                continue;
            }
            expanded = TRUE;
            (*pct_index) = EOS;
            STcat(un_env, next_index);
            (*pct_index)='%';
            if ((*(pct_index+1)) == 'p' || (*(pct_index+1)) == 'P')
            {
                PCpidOwn(&un_num);
#if defined (VMS)
                STprintf(un_str, "%x", un_num);
#else
                STprintf(un_str, "%d", un_num);
#endif
                STcat(un_env, un_str);
            }
            else if ((*(pct_index+1)) == 'd' || (*(pct_index+1)) == 'D')
            {
                if (OK == TMstr_format("%Y%m%d", sizeof(un_str), un_str))
                {
                    STcat(un_env, un_str);
                } else {
                    STcat(un_env, "DATEERR");
                }
            }
            next_index=pct_index+2;
        }
        STcat(un_env, next_index);      /* Append remainder of string   */
        name_length = STlength(un_env);
        if(upd_buflen && expanded)
        {
            if(upd_buflen <= name_length)
                name_length = upd_buflen - 1;
            STlcopy(un_env, filename, name_length);
        }
        filename = un_env;
    }
