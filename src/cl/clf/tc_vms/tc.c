...
**  FUNCTIONAL DESCRIPTION:
**
**      Function invoked when the timer for a TCgetc
**      operation has expired.
**      The TC_qio_timeout verifies that the TCgetc
**      operation is still active, then it cancels the
**      sys$qiow call.
**
**  FORMAL PARAMETERS:
**
**      timr     - Pointer to timer details
**
**  IMPLICIT INPUTS:
**
**      none
...
**  SIDE EFFECTS:
**
**      TCgetc's sys$qiow call is cancelled.
**
**  History:
**     31-Mar-2011 (horda03) b125233
**          Created.
*/
typedef struct
{
   i4              active;
   volatile i4     cancelled;
   TCFILE          *desc;
}
TC_TIMER;
...
**  (may-89) created by Eduardo
**     31-Mar-2011 (horda03) b125233
**          Replace PCsleep with a VMS timer.
*/
...
i4	seconds;
{
	...
	else
	{
	    ...
	    if (seconds > 0)
	    {
                /* Set up a timer to cancel the sys$qiow. This requires
                ** the conversion of the seconds to a BINTIM (ms).
                */
# define MILLI_TO_BINTIM     10*1000 /* ms -> 100ns */
                ...
                /* Note: The address of timr will be the parameter passed to the
                **       TC_qio_timeout function when the timer expires. This address
                **       uniquely identifies the timer and will be used to cancel
                **       the timer should the sys$qiow return with data within the
                **       timer period.
                */
                status = sys$setimr( EFN$C_ENF, bintime, TC_qio_timeout, &timr, 0);
                if ((status & 1) == 0)
                    lib$signal(status);

                unset_ast = (sys$setast( TRUE ) != SS$_WASSET);
	    }
		...
		status = sys$qiow(EFN$C_ENF, thechannel, readmode, &iosb, 0, 0,
                ...
                timr.active = 0; /* Stop the Timer (if set) cancelling the request */

                if (unset_ast) sys$setast( FALSE );

		if ((status & 1) == 0)
		    lib$signal(status);

                if (seconds && !timr.cancelled)
                {
                   sys$cantim( &timr, 0);
                }
