...
**      14-Oct-2011 (coomi01) b123945
**          Return error if unable to replace symbol.tab
**      17-Oct-2011 (coomi01) b123945
**          Replace E_CL1405_NM_STOPN by NM_STOPN, to resolve
...
NMwritesyms(void)
{
    ...
    */
    ...
    if(retval == OK)
    {	
	/* Replace real symbol table file by temp one */
	retval = LOrename(&t_loc, &NMSymloc);       
	if (retval != OK)
	{
	    /* Replacement failed, don't try to go further */
	    PEreset();
	    return  NM_STOPN;
	}
