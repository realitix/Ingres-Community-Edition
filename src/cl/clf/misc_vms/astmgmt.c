...
**          Ensure CTX memory is initialised before use.
*/
#include <compat.h>
#include <iicommon.h>
...
IIMISC_initialise_astmgmt (void* ccb)
{
    ...
    */
    ...
    if (multi_threaded)
    {
       /* Not using PTHREADs so don't initialise */
...
IIMISC_insertAstCtxLookaside(i4 cnt)
{
    ...
    for (i=0; i<cnt; i++)
    {
        astCtx = (AST_CTX *)MEreqmem(0, sizeof(AST_CTX), TRUE, &status);
...
**    10-Dec-2008 (stegr01)   Initial Version
**    19-Oct-2011 (horda03) b121386
**        Removed call_at_ast as never used
*/
void
...
*/
    ...
    if (ctx == NULL)
    {
        ...
        ctx = (AST_CTX *)MEreqmem(0, sizeof(AST_CTX), TRUE, &status);
    ...
    else
    {
        ...
        MEfill(sizeof(AST_CTX), 0, (char *)ctx);
    }
