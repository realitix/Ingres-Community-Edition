...
**          Add iicommon.h, and use ASTJACKET_DISPATCHER_TIMER to identify
**          sys$setimr calls that need to run in the AST Dispatcher thread.
*/
                  ...
                  u_i4  flags)
{
    ...
    if (astadr && !use_ast)
    {
        ...
        if (flags & ASTJACKET_DISPATCHER_TIMER)
        {
            flags &= ~ASTJACKET_DISPATCHER_TIMER;
...
**    10-Dec-2008 (stegr01)   Initial Version
**    19-Oct-2011 (horda03)
**       If the process is multi-threads (i.e. using
**       POSIX threads) then need to use the AST
**       jacket. If not, just call the AST routine as
**       an internal threaded system.
*/

void
IIMISC_initialise_jacket (int multi_threaded)
{
    use_ast  = !multi_threaded;
