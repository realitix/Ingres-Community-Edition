    ...
    LOINFORMATION *locinfo;
{
    ...
    */
    ...
    isDirectory = (attr & FILE_ATTRIBUTE_DIRECTORY) ? TRUE : FALSE;
#else
    ...
    {
#ifdef WCE_PPC
		locinfo->li_size = (OFFSET_TYPE) GetFileSize(hFile, NULL);
#else
...
#ifdef WCE_PPC
		/*
		** Since we opened it for read, set that flag.
		*/
        locinfo->li_perms = LO_P_READ|LO_P_EXECUTE;
        CloseHandle(hFile);
        if ( (hFile = CreateFile ( loc->string,
				GENERIC_WRITE,
				FILE_SHARE_WRITE,
    ...
    if ((*flags & LO_I_LAST) != 0)
    {
		...
		if (GetFileTime(hFile, NULL, &laTime, NULL))
		{
			if (FileTimeToSystemTime(&laTime, &systemTime))
		        locinfo->li_last.TM_secs = systemTime.wYear;
		}
#else
