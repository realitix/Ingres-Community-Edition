/*
** Copyright (c) 1988, 2009 Actian Corporation
*/
...
**	11-aug-2009 (zhayu01) SIR 121210
**		Do not include <signal.h> and <imagehlp.h> for Pocket PC and
**		Windows CE because they do nor exist there.
...
CS_setup(void *parm)
{
	...
	if (EXdeclare(cs_handler, &excontext)) {
		...
		if (pSCB->cs_mask & CS_FATAL_MASK) {
			...
			(*Cs_srv_block.cs_elog) (E_CS00FF_FATAL_ERROR, 
	...
	for (;;) {
		...
		if ((pSCB->cs_mask & CS_DEAD_MASK) == 0) {
			...
			default:
				(*Cs_srv_block.cs_elog) (E_CS000D_INVALID_RTN_MODE, (DB_ERROR *)NULL, NULL, 0);
				pSCB->cs_mode = CS_TERMINATE;
				break;
			}
		} else {
