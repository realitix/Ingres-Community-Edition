/*
**  Copyright (c) 1995, 2004 Actian Corporation
**
**  Name: opingsvc.c
**
**  Description:
...
**	29-Sep-2011 (drivi01)
**	    Fix the Windows service installer to install both VW and Ingres
**	    service.  The installer will determine which service to install 
**	    based on the product installation package, predefines, and constants
**	    defined in the environment.
**	    Add routines to allow removal of Ingres service before VectorWise
...
main(int argc, char *argv[])
{
	...
	if ((!stricmp(argv[2], "Ingres")) || 
		...
		(!stricmp(argv[2], SystemProductName)))
	{
		STprintf ( tchServiceName, "%s_Database", argv[2] );
		STprintf ( tchOldServiceName, "%s_Database", argv[2] );
#ifdef conf_IVW
		STprintf ( tchDisplayName, "%s", argv[2]);
#else
		STprintf ( tchDisplayName, "%s Intelligent Database", argv[2]);
#endif
		STprintf ( tchDescription, "Manages the startup and shutdown of %s Instance.", SystemProductName);
	}
    ...
    if (argc == 2 && !stricmp(argv[1], "remove"))
        RemoveIngresService();
    else if(argc == 3 && !stricmp(argv[1], "remove"))
	{
		if (!stricmp(argv[2], "DBATools"))
...
InitializeGlobals( char *exename )
{
	...
	sprintf ( tchDescription, "Manages the startup and shutdown of %s Instance.", SystemProductName);
}
