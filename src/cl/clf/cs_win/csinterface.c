...
**      13-Jul-2011 (hanal04) SIR 125458
**          Update missed cs_elog call inline with sc0e_putAsFcn changes.
**	14-Oct-2011 (kiria01) b125802
**	    Correct type of CsMultiTaskSem into CS_SYNCH.
*/
#ifdef WIN64
#pragma optimize ("",off)
#endif
...
**                      rv = (*cs_elog)(internal error, &dberror, &system_error,
**                                      p1_length, &p1_value, ...);
...
**      12-Jan-2011 (horda03) b124923
**          Moved Ex_diag_link setup to CSinitiate.
**
****************************************************************************/
STATUS
CSinitiate(i4   *argc,
            ...
            i4 *active_count)
{
	...
	if (mode != CS_CRASH) {
	    ...
	    if (status)
		(*Cs_srv_block.cs_elog) (status, (DB_ERROR *)NULL, 0, 0);
	    else
...
CSdispatch()
{
	...
	*/
	...
	if (EXdeclare(cs_handler, &excontext) != OK) {
		if (!got_ex++) {
			(*Cs_srv_block.cs_elog) (E_CS0014_ESCAPED_EXCPTN, 
                                                 (DB_ERROR *)NULL, NULL, 0);
		}
		EXdelete();
...
# endif /* MCT */
		...
		*/
		...
		if (CS_admin_task())
		{
		    (*Cs_srv_block.cs_elog) (E_CS00FE_RET_FROM_IDLE, 
                                             (DB_ERROR *)NULL, NULL, 0);
		    ret_val = E_CS00FF_FATAL_ERROR;
		}
		else
	...
	if (status || ret_val) {
		...
		(*Cs_srv_block.cs_elog) (ret_val, (DB_ERROR *)NULL, 
                                         &err_code, 0);
	}
...
# endif
	...
	{
		(*Cs_srv_block.cs_elog) (E_CS0019_INVALID_READY, 
                                         (DB_ERROR *)NULL, NULL, 0);
