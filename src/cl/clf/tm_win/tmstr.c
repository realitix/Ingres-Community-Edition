/*
** Copyright (c) 1987, 2003 Actian Corporation
*/
...
**      TMstr_format() -  Time to formatted string
**
** History:
...
**
** Description:
**      This routine returns a pointer to a formatted string of 'now'
**      with the passed format.
**
** Inputs:
**      fmt             FORMAT string for strftime();
**      blen            Size of result buffer;
...
**
** WARNING:
**      For cross-platform compatability, you should **ONLY** use
**      format specifiers which are listed in the ANSI standard,
**      as extensions may fail on some platforms.
**
** History:
**      5-Jun-2011 (chris.dawe@iua.org.uk)
**          Initial creation
*/
STATUS
TMstr_format(char *fmt, i4 blen, char *buf)
{
    STATUS      status=OK;
    time_t      secs;
    struct tm   *tm;
    ...
    {
        tm = localtime(&secs);
        if ((size_t)0 != strftime(buf, (size_t)blen, fmt, tm))
        {
            return(OK);
        }
    }
