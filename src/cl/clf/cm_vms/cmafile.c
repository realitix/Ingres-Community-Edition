/*
** Copyright (c) 1989, 2009, 2011 Actian Corporation
*/
...
**		Do not include <fcntl.h> and <errno.h> for Pocket PC
**		because they do not exist there.
**		Use CreateFile() instead of open() on Pocket PC.
...
CL_ERR_DESC	*err)
{
	...
	*/
	...
	if (init_frab(&fab,&rab,fn,FALSE) != OK)
	{
		SETCLERR(err, 0, ER_open);
		return FAIL;
	}
...
#ifdef WCE_PPC
	if ((fd = CreateFile(fn, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
				FILE_ATTRIBUTE_READONLY, NULL)) == NULL)
#else
...
CL_ERR_DESC *err;
{
	...
	if (init_frab(&fab,&rab,fn,TRUE) != OK)
	{
		SETCLERR(err, 0, ER_open);
		return FAIL;
	}
...
# ifdef DESKTOP    /* Special treatment for binary files. */
#ifdef WCE_PPC
	if ((fd = CreateFile(fn, GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
			OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL)) == NULL)
#else
...
const u_char *CMgetCaseTab(void)
{
    return CM_CaseTab;
}

const CM_UTF8ATTR *CMgetUTF8AttrTab(void)
{
    return CM_UTF8AttrTab;
}

const CM_UTF8CASE *CMgetUTF8CaseTab(void)
{
...
*/
const u_i4 *CMgetUTF8Bytes(void)
{
    return CM_UTF8Bytes;
}
# endif             /* NT_GENERIC  */
