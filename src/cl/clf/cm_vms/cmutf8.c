/*
** Copyright (c) 2007, 2009, 2011 Actian Corporation
*/
...
**	    SIfprintf.
**  10-may-2009 (zhayu01) SIR 121210
...
**	    Moved macros definitions and core typedef into cmcl.h
*/
...
cmupct (u_char *str, u_char *startpos)
{
  ...
  if (p == startpos && CM_UTF8FOLLOW(p))
    /* If a follower is at the start of the buffer it is badly
    ** formed and in this case we return 1 and treat the follower
...
*/
u_i2
cmu_getutf8property(const UTF8 *key, i4 keycnt)
{
    UTF32 index;
...
*/
static u_i2
cmu_getutf8casecnt(const UTF8 *key, i4 keycnt)
{
    UTF32 index;
    u_i2 caseindex;
    u_i2 casecnt = 0;
...
*/
u_i2
cmu_getutf8_tolower(const UTF8 *src, i4 srclen, UTF8 *dst, i4 dstlen)
{
  UTF32		index = 0;
  u_i2		caseindex = 0;
  u_char	temp[8] = {0};
...
*/
u_i2
cmu_getutf8_toupper(const UTF8 *src, i4 srclen, UTF8 *dst, i4 dstlen)
{
  UTF32		index = 0;
  u_i2		caseindex = 0;
  u_char	temp[8] = {0};
 ...
 *  length = CM_UTF8trailingbytes(source)+1;
...
static
bool CMisLegalUTF8Char(const UTF8 *source, int length) {
    register UTF8 a;
...
static bool isLegalSpcUTF8(const UTF8 *source, i4 stride, i4 len) {
    UTF8 a;
    switch (len) {
    default: return FALSE;
        /* Everything else falls through when "true"... */
    case 4: if ((a = (source[(--len)*stride])) < 0x80 || a > 0xBF) return FALSE;
    case 3: if ((a = (source[(--len)*stride])) < 0x80 || a > 0xBF) return FALSE;
    case 2: if ((a = (source[(--len)*stride])) > 0xBF) return FALSE;
	...
	UTF32** targetStart, UTF32* targetEnd, i4 flags) 
{
	...
	UTF32 ch = 0;
	unsigned short extraBytesToRead = CM_UTF8trailingbytes(source); 
	if (source + extraBytesToRead >= sourceEnd) {
	    result = sourceExhausted; break;
	}
	/* Do this check whether lenient or strict */
	if (!CMisLegalUTF8Char(source, extraBytesToRead+1)) {
	...
	ch -= CM_offsetsFromUTF8[extraBytesToRead];
	...
	UTF32* target) 
{
    ...
    i4 i;
    UTF32 ch = 0;
    if (len != CM_UTF8trailingbytes(src)+1)
	return sourceExhausted;
    
    /* Do this check whether lenient or strict */
    if (! isLegalSpcUTF8(src, stride, len))
    ...
    */
    i = 0;
    switch (--len) {
    case 5: ch += src[i*stride]; ch <<= 6; i++;
    case 4: ch += src[i*stride]; ch <<= 6; i++;
    case 3: ch += src[i*stride]; ch <<= 6; i++;
    case 2: ch += src[i*stride]; ch <<= 6; i++;
    case 1: ch += src[i*stride]; ch <<= 6; i++;
    case 0: ch += src[i*stride];
    }
    ch -= CM_offsetsFromUTF8[len];
...
*/
i4
CM_UTF8toUTF32 (const UTF8 *instr, i4 incnt, UTF32 *outval)
{
        i4 	n;
        ConversionResult result;
        UTF32 	utf32_buf[2];
        UTF8 	utf8_buf[5];
	const UTF8	*utf8SourceStart;
	UTF32	*utf32TargetStart;
	  ...
	  utf8_buf[n] = instr[n];
        ...
        result = CM_ConvertUTF8toUTF32(&utf8SourceStart,
        ...
        UTF8** targetStart, UTF8* targetEnd, i2 flags, i2 *rescnt) 
{
    ...
    while (source < sourceEnd) {
        ...
        switch (bytesToWrite) { /* note: everything falls through. */
            ...
            case 1: *--target = (UTF8) (ch | CM_UTF8firstByteMask[bytesToWrite]);
        }
        target += bytesToWrite;
	*rescnt = bytesToWrite;
    }
...
i4
CM_UTF32toUTF8 (UTF32 inval, UTF8 *resstr)
{
        i4 i, n;
        ConversionResult result;
        UTF32 utf32_buf[1];
        UTF8 utf8_buf[15];
        const UTF32 *utf32SourceStart;
        ...
        result = CM_ConvertUTF32toUTF8(&utf32SourceStart, utf32SourceStart+1,
				    &utf8TargetStart, &utf8_buf[15],
                                     0, &rescnt);
        if (result != conversionOK) {
        SIfprintf(stderr,
...
CM_UTF32toUTF16 (i4 inval, u_i2 *outbuf, u_i2 *outend, i4 *outlen)
{
	...
	utf16TargetStart = utf16_buf;
        ...
        if (result = CM_ConvertUTF32toUTF16(&utf32SourceStart,
...
{
    i4 length = CM_UTF8trailingbytes(source)+1;
    if (source+length > sourceEnd) {
	return FALSE;
    }
    return CMisLegalUTF8Char(source, length);
}
...
*/
i4
CM_UTF8_twidth (const UTF8 *strstart, const UTF8 *strend)
{
        i4      n;
        ConversionResult result;
        UTF32   utf32_buf[2];
	...
	if (strstart + incnt > strend)
        ...
        if ((result = CM_ConvertUTF8toUTF32(&utf8SourceStart,
                    		             &(utf8_buf[incnt]), 
				             &utf32TargetStart,
...
i4
CM_SpcUTF8_twidth (const UTF8 *src, i4 stride, i4 len)
{
    ConversionResult result;
    UTF32   utf32 = 0;

    if ((result = CM_Convert1SpcUTF8toUTF32(src,
			stride, len, &utf32)) !=  conversionOK)
	return -1;

    return CM_mk_wcwidth(utf32);
}
