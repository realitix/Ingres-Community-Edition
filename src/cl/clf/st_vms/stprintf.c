/*
**      Copyright (c) 1983, 1999 Actian Corporation
*/
# include	  <compat.h>  
# include	  <gl.h>
# include	  <si.h>  
...
**      07-Jun-2011 (horda03)
**         SIdofrmt() interface changed to match unix.
*/
...
STprintf(char *buffer, const char *fmt, ...)
{
	...
	va_start(ap2, fmt);
	SIdofrmt(CMCPYCHAR, buffer, fmt, ap2);
	va_end(ap2);
	return (buffer);
}
...
**          Created.
**      07-Jun-2011 (horda03)
**          New interface to SIdofrmt(). As this function limits the size
**          of the output buffer, use the SI_CMCPYCHAR_LEN structure
**          to pass the output buffer and the maximum length.
*/
char *
STnprintf(char *buffer, i4 len, const char *fmt, ...)
{
	va_list	ap2;
        SI_CMCPYCHAR_LEN buf_details;

        buf_details.buf = buffer;
        buf_details.len = len;

	va_start(ap2, fmt);
        SIdofrmt(CMCPYCHAR_LEN, (void *) &buf_details, fmt, ap2);
	va_end(ap2);
	return (buffer);
}
