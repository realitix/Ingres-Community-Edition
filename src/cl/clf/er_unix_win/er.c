...
**		Do not include <fcntl.h>, <errno.h>, <lmmsg.h> and <lmerr.h>  for
**		Pocket PC because they do not exist there.
...
ERlog(char *message, i4 msg_length, CL_ERR_DESC *err_code)
{
	...
	LOtos(&loc, &fname);
#ifdef WCE_PPC
	er_ifi = CreateFile(fname, GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
			OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (er_ifi == NULL)
		er_ifi = -1;
	else
		SetEndOfFile((HANDLE) er_ifi);
#else
