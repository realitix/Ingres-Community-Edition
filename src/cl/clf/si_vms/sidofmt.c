/*
**    Copyright (c) 1986, 2000 Actian Corporation
*/
...
**      23-May-2011 (horda03)
**          Bring SIdofrmt interface inline with Unix (as now this is called
**          directly from ADF routines. Use of SIdofrmt() on VMS in one instance
**          requires the length of the string to be limited.
*/
...
va_list		ap;
{
	...
	if (putfunc == CMCPYCHAR_LEN)
       	{
		SI_CMCPYCHAR_LEN *buf_details = (SI_CMCPYCHAR_LEN *)outarg;

		putfunc = CMCPYCHAR;

		len = buf_details->len - 1;
                outchar = (uchar *)buf_details->buf;
