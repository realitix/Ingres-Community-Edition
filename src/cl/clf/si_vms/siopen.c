...
**          Add iicommon.h and cs.h. Also no semaphore to
**          initialise.
**      19-Oct-2011 (horda03) b121386
**          Add clconfig.h and csev.h. Use CS_SPIN locks to
**          protect structures. These need to be initialised.
*/
...
static void log_error_message(i4 err, char *fname, char mode, char *usermsg);
   ...
   GLOBALREF CS_SPIN SI_lock_queue;
   CS_SPIN           SI_lock_free;
   i4                SI_init_spin_lock = 1;
...
FILE		**desc)
{
    ...
    if (threaded && SI_init_spin_lock)
    {
       SI_init_spin_lock = 0;

       CS_SPININIT( &SI_lock_queue);
       CS_SPININIT( &SI_lock_free);
    }
#endif
