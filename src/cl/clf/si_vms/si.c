/*
** Copyright (c) 1983, 2005 Actian Corporation
**
*/
...
**      18-Aug-2011 (horda03) Bug 123092
**          For an OS_THREADS_USED build can't us CSp_semaphore code to protect
**          the queue of waiting file streams to flush because utilities like
**          RCPSTAT don't initialise the Cs_fvp array. Now for OS threads use
**          TAS locks.
**      19-Oct-2011 (horda03) b121386
**          Use CS_SPIN spin locks to protect structure in an OS threaded server.
*/

...
volatile FILEX_LINK SI_pending_list = { &SI_pending_list, &SI_pending_list };
...
SIflush_pending()
{
   ...
   */
        ...
        filex != &SI_pending_list; filex = next)
   {
      ...
      if (threaded)
      {
         if (filex->busy_buf)
         {
            ...
            **
            ** busy_buf only set when CS_TAS lock set, rather than
            ** wait for the lock move on to the next one.
            */
   ...
   if (SI_pending_list.next != &SI_pending_list)
   {
      ...
      sys$setimr( 0,  &delay_time , SIflush_pending, &SI_pending_list, ASTJACKET_DISPATCHER_TIMER);
   }
   else
   {
