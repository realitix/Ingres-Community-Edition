...
**     KPS_THREADS only
**
**          CS_kp_stall - Stall the session such that it can be userped for stack dumps
**
**  History:
**      23-Oct-1986 (fred)
**          Created.
...
**      20-Jan-2011 (horda03) b124941
**          Close hole where IDLE threads hibernates the server even though there
**          could be COMPUTABLE sessions.
**     13-Jul-2011 (hanal04) SIR 125458
**         Update cs_elog calls inline with sc0e_putAsFcn changes.
**      09-Aug-2011 (horda03) b125459
**          In a KPS_THREAD system CS_ssprsm isn't used to switch thread context,
**          so need another way to generate stacks for other sessions. Now, the
**          sessions generate their own stack when CS_vms_dump_stack schedules the
**          session.
**      05-Oct-2011 (horda03) b125821
**          Add Symbol name/offset to a Stack dump.
**      14-Oct-2011 (horda03) b125821
**          ... but remember that AXP and IA64 have different structure definitions
**          for stack context.
**     19-Oct-2011 (horda03) b121386
**          Correct cast problem when building with OS_THREADS_USED
**/

/*
...
static i4 (*dump_stack_fcn)();
static i4 dump_stack_verbose;
void (*kp_do_action)( CS_SCB *scb) = NULL;
#endif
...
CS_quantum(i4 mode)
{
    ...
    (*Cs_srv_block.cs_elog)(E_CS0006_TIMER_ERROR, (DB_ERROR *)NULL, NULL, 0);
    return(status);
}

/*
...
CS_setup()
{
    ...
    */
	...
	for (;;)
	{
            if (scb != &Cs_repent_scb)
            {
                /* Close a window where an AST could make a session COMPUTABLE
                ** but because the server isn't CS_IDLING no sys$wake is called.
                ** Thus should the IDLE thread hibernate the server, the COMPUTABLE
                ** thread won't execute until a new (external) stimulus is received,
                ** such as a request from another session.
                */
                sys$setast(0);
		Cs_srv_block.cs_state = CS_IDLING;

                /* Now that the server is CS_IDLING and ASTs are disabled, check to 
                ** see if there are any sessions at a higher priority than the IDLE
                ** thread ready to run.
                */
                if (Cs_srv_block.cs_ready_mask != CS_PRIORITY_BIT)
                {
                   /* There are COMP sessions so don't hibernate. */

                   do_hiber = 0;
                }
		else if (Cs_srv_block.cs_user_sessions == 0)
		{
		    /*
...
i4                handler;
{
    ...
    if (handler)
	(*Cs_srv_block.cs_elog)(E_CS0015_FORCED_EXIT, (DB_ERROR *)NULL, 
                                NULL, 0);
...
CS_last_chance(EX_ARGS	*sig_vec)
{
    ...
    if (scb->cs_sem_count)
    {
	(*Cs_srv_block.cs_elog)(E_CS00FC_CRTCL_RESRC_HELD, (DB_ERROR *)NULL,
                                NULL, 0);
	fatal++;
    }
    else if (lib$ast_in_prog())
    {
	(*Cs_srv_block.cs_elog)(E_CS00FB_UNDELIV_AST, (DB_ERROR *)NULL, 
                                NULL, 0);
	fatal++;
    }
    else if (scb->cs_mask & CS_BAD_STACK_MASK)
    {
	(*Cs_srv_block.cs_elog)(E_CS00F9_2STK_OVFL, (DB_ERROR *)NULL, NULL, 0);
	fatal++;
    }
    scb->cs_mask |= CS_BAD_STACK_MASK;

    (*Cs_srv_block.cs_elog)(E_CS00FD_THREAD_STK_OVFL, (DB_ERROR *)NULL,
                            NULL, 2,
...
# endif /* xDEBUG */
	...
	*/
	...
	(*Cs_srv_block.cs_elog)(E_CS00FA_UNWIND_FAILURE, (DB_ERROR *)NULL,
                                NULL, 0);
    }
    if (fatal)
    {
...
**      09-Aug-2011 (horda03) b125459
**              Add cs_do_stack_dump for KPS_THREADS, such that a KPS thread
**              can be scheduled to generate it call stack. Note, the way
**              the "non-current" KPS thread is scheduled means that the cs_current
**              session isn't updated so it appears as though the thread that
**              called CS_dump_stack originally is still "current".
**      05-Oct-2011 (horda03)
**              Interface to CS_vms_dump_stack has been changed.
*/
#ifdef KPS_THREADS
static void
cs_do_stack_dump( CS_SCB *scb )
{
   /* Dump "current" thread stack, using original fcn and verbose settings */

   CS_vms_dump_stack( 0, 0, dump_stack_fcn, dump_stack_verbose);
}
#endif
		  ...
		  i4      verbose)
{
    ...
    {
#ifdef KPS_THREADS
        /* This is a KPS_THREADed system, so need to the scb session to
        ** generate its own stack, using the output_fcn and verbose settings.
        */
               dump_stack_fcn = output_fcn;
       dump_stack_verbose = verbose;

       kp_do_action = cs_do_stack_dump;
       exe$kp_restart(scb->kpb, SS$_NORMAL);
       kp_do_action = NULL;
...
s    pc: %x    pv: %x    fp: %x    sp: %x %s%s",
		 ...
		 stack_context.libicb$q_ireg[30],
                 EX_symname(stack_context.libicb$q_program_counter, symbol),
		 close_string );
#elif defined(i64_vms)
        TRformat( output_fcn, arg, buf, sizeof(buf) - 1,
                 "%\
s    pc: %x    sp: %x %s%s",
                 ...
                 EX_symname(stack_context.libicb$ih_pc, symbol),
                 close_string );
#endif
...
** Description:
**
**      Switch the current session out of context.
**      When session resumed, check if it has been resumed
**      to perform a specific action (kp_do_action set) or
**      it has been reschedulled.
**
** Inputs:
**      scb         Current session's SCB
**
** Outputs:
**      none.
**
...
CS_kp_stall(CS_SCB *scb)
{
   ...
   do
   {
      ...
      if (kp_do_action)
      {
         /* Session has been scedulled to perform the "kp_do_action"
         ** Once action has been performed, stall the session.
         */
         (*kp_do_action)(scb);
      ...
      /* Finally, the session can resume its normal duties */
