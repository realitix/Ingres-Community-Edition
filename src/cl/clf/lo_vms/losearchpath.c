...
LOCATION 	*loc)
{
	...
	/* Look for device */
	loc->path = loc->node + loc->nodelen;
	p = STindex(loc->path, COLON, 0);
	if (p != NULL)
		loc->devlen = p - loc->path + 1;
	else
		loc->devlen = 0;


	/* Look for directory */
	loc->dirptr = loc->path + loc->devlen;
	...
	/* Look for filename */
	loc->fname = loc->dirptr + loc->dirlen;
	dot = STindex(loc->fname, DOT, 0);
	semi = STindex(loc->fname, SEMI, 0);
	if (dot != NULL && semi == NULL)
		semi = STindex(dot+1, DOT, 0);

	if (dot != NULL)
		loc->namelen = dot - loc->fname;
	else if (semi != NULL)
		loc->namelen = semi - loc->fname;
	else
		loc->namelen = STlength(loc->fname);

	/* Look for type */
	loc->typeptr = loc->fname + loc->namelen;
	if (dot != NULL)	
	{
		if (semi != NULL)
