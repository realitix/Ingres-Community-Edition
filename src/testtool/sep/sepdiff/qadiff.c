...
**	    and linking of line tracking code.
*/

FUNC_EXTERN                yylex() ;
FUNC_EXTERN		   put_eoln() ;
FUNC_EXTERN  E_edit        G_do_miller( i4  m, i4  n, i4  max_d, i4  *merr ) ;
FUNC_EXTERN  i4		   expect_unordered_diff() ;
...
GLOBALDEF    FILE_LINES   *line_list_head = NULL ;
GLOBALDEF    FILE_LINES   *line_list_tail = NULL ;
GLOBALDEF    FILE_LINES   *line_list1 ;
GLOBALDEF    FILE_LINES   *line_list2 ;
	...
	char *unordered)
{
    ...
    line_list_head = line_list_tail = NULL;
    put_eoln();		/* Prime line store */
    ...
    if (unordered && expect_unordered_diff() == 0)
	totaldiffs = 0;
    else
