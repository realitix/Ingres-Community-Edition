...
**	    Add UNORDERED canon qualifier code - expect_unordered_diff().
*/
                          
GLOBALREF    FILE_TOKENS  *list1 ;
GLOBALREF    FILE_TOKENS  *list2 ;
GLOBALREF    FILE_LINES   *line_list1 ;
GLOBALREF    FILE_LINES   *line_list2 ;
...
i4		   compare ( i4  token1, i4  token2 ) ;
...
G_do_miller( i4  m, i4  n, i4  max_d, i4  *merr)
{
    ...
    *merr = 0;
    /* Decrement m & n for duplicate tails */
    for (; m > 0 && n > 0 && compare(m, n) == 0; m--, n--)
	/*SKIP*/;

    origin = m > n ? m : n;
    size_diag(2*origin + 1);

    for (row=0; row < m && row < n && compare(row+1,row+1) == 0; ++row)
	/*SKIP*/;
...
/*
** Function: compare_lines - compare whole lines of tokens
**
**	Ordering metric for a pair of lines of tokens.
**
** Returns:
**	0	a == b
**	+ve	a > b
...
*/
i4
compare_lines(FILE_LINES *a, FILE_LINES *b)
{
    i4	t1, t2;
    i4	ret_val ;
    i4	i;

    for (i = 0; i < a->token_count && i < b->token_count; i++)
    {
	if (ret_val = compare(a->token_no+i, b->token_no+i))
	    return ret_val;
    }
...
/*
** Function: expect_unordered_diff - optimised sorted check
**
**	Check for a potential sort match involving just sort order.
**
** Returns:
**	0   Match
**	-1  Mismatch
*/
i4
expect_unordered_diff()
{
    FILE_LINES *a = line_list1, *abeg, *aend;
    FILE_LINES *b = line_list2, *bbeg, *bend; 

    /* First skip common start rows */
    while (a && b)
    {
    ...
    /* If all common then identical match */
    if (!a && !b)
	return 0;
    /* If mismatched lengths remaining then can't be sorted */
    if (!a || !b)
	return -1;
    /* Find ends */
    while (a->next && b->next)
    ...
    /* If differing lengths - then can't be sorted */
    if (a->next || b->next)
	return -1;

    /* Same count of lines at least - look for common tails */
    while (a && b)
    {
	if (a->token_count != b->token_count ||
		compare_lines(a, b))
	{
	    /* aend & bend point beyond the end of the
	    ** list of potential unsorted rows. */
	    aend = a->next;
	    bend = b->next;
	    break;
	}
    ...
    for (a = abeg; a != aend; a = a->next)
	a->seen = FALSE;
    /* Look for each b row in a */
    for (b = bbeg; b < bend; b = b->next)
    {
	for (a = abeg; a != aend; a = a->next)
	{
	    /* look at each a thats not already been seen */
	    if (!a->seen &&
		a->token_count == b->token_count &&
		!compare_lines(a, b))
	    {
		a->seen = TRUE;
		/* Reduce list if we can */
		while (abeg->seen && abeg != aend)
		    abeg = abeg->next;
		while (aend && aend->prev && aend->prev->seen)
		    aend = aend->prev;
		goto wasseen;
	    }
	}
