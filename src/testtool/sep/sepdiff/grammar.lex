...
**          Sep Improvements. Stop confusion between lex and flex re line numbers
**          use our own symbol instead. Want to move to accurate diff reporting
**          rather than the approximation currently employed.
**	06-Sep-2011 (kiria01) b125725/b125726
**	    Add grammar support for UTF-8 and code to create structures to
**	    track line boundaries.
**	16-Sep-2011 (kiria01) b125725/b125726
**	    Remove the handling for continuation marks in the scanner as these
**	    will already have been stripped and lines recombined.
**      29-Sep-2011 (maspa05) b125773
**          Add Actian as possible COMPNAME
*/
...
static void put_token(i4 tokenid);
void put_eoln();
...
alnum		[-#$.%&@!_a-zA-Z0-9]
u8_follow	[\x80-\xBF]
u8_2		([\xC2-\xDF]{u8_follow})
u8_3		([\xE0-\xEF]{u8_follow}{u8_follow})
u8_4		([\xF0-\xF4]{u8_follow}{u8_follow}{u8_follow})
u8		({u8_2}|{u8_3}|{u8_4})
u8letter	({u8}|{letter})
u8alnum		({u8}|{alnum})
hexa		[0-9a-fA-F]
...
n_mon		(JAN|FEB|MA(R|Y)|APR|JU(N|L)|AUG|SEP|OCT|NOV|DEC|[Jj]an|[Ff]eb|[Mm]a(r|y)|[Aa]pr|[Jj]u(n|l)|[Aa]ug|[Ss]ep|[Oo]ct|[Nn]ov|[Dd]ec)
l_mon1		((JAN|FEBR)UARY|MARCH|APRIL|MAY|JU(NE|LY)|AUGUST|OCTOBER|(SEPT|NOV|DEC)EMBER)
...
(("Actian Corporation")|("Ingres Corporation")|("Computer Associates"(" Intl, Inc.")?))[\r]?	{put_token(A_COMPNAME);};
...
put_token(i4 tokenid)
{
    ...
    if (line_list_tail)
    {
	if (!line_list_tail->tok_next)
	{
	    line_list_tail->tok_next = token;
	    line_list_tail->token_no = token->token_no;
	}
	line_list_tail->token_count++;
    }
}
...
put_eoln()
{
    ...
    line = (FILE_LINES *)get_token( sizeof( *line ));

    if( !line )
	return;

    line->prev = line->next = NULL;
    line->tok_next = NULL;  /* Will be setup by put_token */
    line->tok_prev = list_tail;
    line->line_hash = 0;
    line->token_no = 0;
    line->token_count = 0;
    line->lineno = ourLineNo;

    if (!line_list_head)
    {
	line_list_head = line_list_tail = line;
    }
    else
    {
	line->prev = line_list_tail;
	line_list_tail->next = line;
	line_list_tail = line;
    }
}
