...
#ifdef SEP_SYNC
#include <tc_sync.h>
#define SEPfork fork   /* Do full fork(), do NOT share memory afterwards */
#else  /* SEP_SYNC */
...
#ifdef SEP_SYNC
	/* 
	** Adjust pipes before exec'ing child 
	*/
	if ( sepSyncGlobalVars.syncMode )
	{
	    TCcompleteSepChildEnviron(SEPtcoutptr, SEPtcinptr);
	}
    ...
    {
      /* 
      ** Complete the pipe handling
      */
      if ( 0 == TCcompleteSepParentEnviron(SEPtcoutptr, SEPtcinptr, *pid) )
	{
	  sepSyncGlobalVars.aborted = 2;
	  ret_val = FAIL;
	}
    }
#endif /* SEP_SYNC */

#else /* notdef NT_GENERIC */
