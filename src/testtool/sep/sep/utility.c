...
**	    Make sure line breaks don't split UTF-8 chars.
*/
...
/*
	...
	at the TEST_LINE+2 position(ish).
*/

    while (STlength(cp) > TEST_LINE+3)
    {
	i4 l = TEST_LINE+2;
	while ((cp[l] & 0xC0) == 0x80)
	    l--;
...
close_log()
{
    ...
    char                   timestr [TEST_LINE] ;
#ifdef SEP_SYNC
    if (sepSyncGlobalVars.aborted)
    {
      switch (sepSyncGlobalVars.aborted)
	{
	case(1):
	  {
	    append_line((char *)ERx("\n\nWARNING: A Mutual read deadlock was detected\n"),(i4)0);
	    append_line((char *)ERx(    "         The application had then to be aborted\n"),(i4)0);
	  }
	  break;

	case(2):
	  {
	    append_line((char *)ERx("\n\nWARNING: The child spawn did not syncronize with sep\n"),(i4)0);
	    append_line((char *)ERx(    "         The application had then to be aborted\n"),(i4)0);
	  }
	  break;
	}
	...
	writeModify("");
	fclose(sepSyncGlobalVars.test.modHndl);
