...
GLOBALREF    i4		   allow_unordered_flag;
bool                       wrong_canon ;


#ifdef SEP_SYNC
GLOBALDEF    i4      startTestLine = 0;
void
showFileLines(SEPFILE *sepResults)
{
    STATUS                 ioerr ;
    char                   buffer [SCR_LINE] ;

    FILE *fptr = sepResults->_fptr ;

    SEPrewind(sepResults,FALSE);
    while ((ioerr = SEPgetrec(buffer, sepResults)) == OK)
    {
	showLine(SHOW_LINE_RFILE,buffer);
    }
}
#endif /* SEP_SYNC */
...
diffing( FILE *testFile, i4  prtRes, i4  getprompt )
{
...
#ifdef SEP_SYNC
    if (!sepSyncGlobalVars.syncMode && sepSyncGlobalVars.showLine)
...
#ifdef SEP_SYNC
	startTestLine       = sepSyncGlobalVars.test.rawLine; 
#endif /* SEP_SYNC */
	     ...
	     == OK)
	{
	    return_val = (open_args == NULL) ? OK :
		         Do_open_args(open_args, open_args_array, &wrong_canon,
				      &if_expression, &unordered, prtRes);
	    ...
	    {
		append_results = ((matched = diff_canon(&diffound, unordered)) != OK)
			       ? FALSE : TRUE;
	    }
	}
	...
	{
	    if (matched == OK)
		showLine(SHOW_LINE_PASS,"\n");
	    else if (acanon == SKIP_CANON)
		showLine(SHOW_LINE_IGNRD,"\n");
	    else
...
#ifdef SEP_SYNC
    /* 
    ** When modifying (creating our own script), we write the 
    ** *actual* results out in-line right after the command
    ** and before the existing cannons start.
    ** This adds a tiny complication in that we have to scan
    ** the existing canons before we can decide there is no match.
    ** Hence, modState is a mini state machine that allows
    ** use to hang onto the material until we are ready
    ** to flush.
    */
    if (sepSyncGlobalVars.test.modify)
    {
	if ((matched != OK) && (acanon != SKIP_CANON))
	{
	    STATUS                 ioerr ;
	    char                   buffer [SCR_LINE] ;
	    ...
	    sepSyncGlobalVars.test.modState = 3;
	    writeModify("<< IF (SEP_SYNC)\n");
	    while ((ioerr = SEPgetrec(buffer, sepResults)) == OK)
	    {
		writeModify(buffer);
	    }
	    sepSyncGlobalVars.test.modState = 5;
	    writeModify(">> /* ENDS SEP_SYNC */\n");
	}
	else
	{
	    sepSyncGlobalVars.test.modState = 1;
	    writeModify("");
	}
	      ...
	      char **unordered, i4  prtRes )
{
	...
	*temp_line = EOS;
	...
	else if (STbcompare(opn_array[i], 0, ERx("UNORDERED"), 0, TRUE) == 0)
	{
	    if (allow_unordered_flag)
		*unordered = opn_array[i];
	}
	else if (prtRes)
	{
