...
#endif
#ifdef SEP_SYNC
/* Only return true when using sep in sync mode */
#define IS_SEP_SYNC_TorF (sepSyncGlobalVars.syncMode) ? ERx("TRUE") : ERx("FALSE")
#else
#define IS_SEP_SYNC_TorF ERx("FALSE")
#endif /* SEP_SYNC */
	...
	*optptr->o_retnat = 0;
	if (sub_arg)
	{
	    for (all_digits = TRUE, cptr = sub_arg;
		 (*cptr)&&(all_digits);
		 all_digits = CMdigit(cptr), CMnext(cptr))
		;
...
SEPEVAL_alnum(expression_node *exp)
{
	...
	*cptr1 = EOS;
    ...
    if (STbcompare(exp->exp_str, 0, ERx("SEP_SYNC"), 0, TRUE) == 0)
    {
#ifdef SEP_SYNC
	exp->exp_result = (sepSyncGlobalVars.syncMode) ? TRUE : FALSE;
#else
	exp->exp_result = FALSE;
#endif
    }
...
Trans_SEPvar(char **Token, bool Dont_free, i2 ME_tag)
{
    ...
    */    
    ...
    if (STbcompare(tmp_buffer2, 0, ERx("SEP_SYNC"), 0, TRUE) == 0)
    {
	STcopy(IS_SEP_SYNC_TorF,newstr);
    }
    else
