...
**	    UTF-8 char and unordered related changes. -NUNORDERED flag
**	    added to allow disabling the UNORDERED qualifier. -!
**	    flag added to aid debugging display.
*/
...
GLOBALDEF    i4            diffLevel = 40 ;	    /* max. tokens that might
						    ** diffed		      */
#ifdef SEP_SYNC
GLOBALDEF    i4            testLevel = 1000 ;	    /* max. commands that might
						    ** diffed		      */
#else
...
GLOBALDEF    i4		   allow_unordered_flag = TRUE;
...
main(i4 argc,char *argv[])
{
    ...
    */
    ...
    /*
    ** Prevent tool calling itself
    */
    sepSyncGlobalVars.inSepTool = 1;

    if (sepSyncGlobalVars.args.ipcXternal)
    {
	TCaddDebugService(sepSyncGlobalVars.args.xternalOut);
    }

    sepSyncGlobalVars.showLine = SHOW_LINE_OFF;
    if (sepSyncGlobalVars.args.ipcLogVerb)
    {
	sepSyncGlobalVars.showLine = SHOW_LINE_ON;
    }

    /* 
    ** Now is a good time to activate a socket service 
    */
    if ( !sepSyncGlobalVars.args.syncMasked && TClistSyncArgs() && TCcreateSepServerSocket() )
    {
	sepSyncGlobalVars.args.ipcMode |= _TCUSESOCK;
    }
...
#ifdef SEP_SYNC
    sepSyncGlobalVars.outDir  = outputDir;
    sepSyncGlobalVars.prefix  = testPrefix; 

    sepSyncGlobalVars.test.modify = sepSyncGlobalVars.args.ipcModify;
    if ( sepSyncGlobalVars.args.ipcModify )
    {
	char buffer[1024];
	sprintf(buffer, "%sSync", testName);
	sepSyncGlobalVars.test.modHndl = fopen(buffer, "w");
	sepSyncGlobalVars.test.modState = 2;
	if ( NULL == sepSyncGlobalVars.test.modHndl )
	{
	    sepSyncGlobalVars.test.modify   = FALSE;
	    sepSyncGlobalVars.test.modHndl  = 0;
	}
    }
#endif
...
#ifdef SEP_SYNC
    if (sepSyncGlobalVars.aborted && sepSyncGlobalVars.showLine)
    {
	showFileLines(sepResults);
	showLine(SHOW_LINE_FAIL, "\n");
	showLine(SHOW_LINE_FAIL, "\n");
	showLine(SHOW_LINE_FAIL, "**** DEAD-LOCK detected,\n");

	switch ( sepSyncGlobalVars.aborted )
	  {
	  case (1):
	    showLine(SHOW_LINE_FAIL, "**** Application is trying to read more input, sep is expecting more output!\n");
	    break;

	  case (2):
	    showLine(SHOW_LINE_FAIL, "**** Application aborted before connecting to SepSync!\n");
	    break;
	  }
	showLine(SHOW_LINE_FAIL, "\n");
	showLine(SHOW_LINE_FAIL, "**** ABORTED!!!\n");
    }
#endif
    close_log();
...
SEP_main_init( char *errmsg )
{
...
#ifdef SEP_SYNC
    /* 
    ** set defaults ahead of command line scan 
    */
    initSepSyncArgs();
#endif
    ...
    */
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("alog"),
			   NULL,               /* char * loaded here */
			   &sepSyncGlobalVars.args.ipcLogVerb,/* bool   loaded here */
			   NULL,               /* i4     loaded here */
...
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("Gdb"),
		           sepSyncGlobalVars.args.gdbString,  /* char * loaded here */
		           &sepSyncGlobalVars.args.gdbFlagged,/* bool   loaded here */
			   NULL,               /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
			   );
...
#endif
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("Modify"),
			   NULL,               /* char * loaded here */
			   &sepSyncGlobalVars.args.ipcModify, /* bool   loaded here */
			   NULL,               /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
			   );
...
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("sync"),
			   sepSyncGlobalVars.args.execArgs,    /* char * loaded here */
			   NULL,              /* bool   loaded here */
			   NULL,               /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
...
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("Timers"),
		           sepSyncGlobalVars.args.timeString, /* char * loaded here */
		           NULL,               /* bool   loaded here */
			   NULL,               /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
			   );
    SEP_alloc_Opt(&cmdopt, ERx("TimeOut"),
		           NULL,               /* char * loaded here */
		           NULL,               /* bool   loaded here */
			   &sepSyncGlobalVars.args.timeOut,   /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
			   );
#endif
...
#ifdef SEP_SYNC
    SEP_alloc_Opt(&cmdopt, ERx("Xternal"),
		           sepSyncGlobalVars.args.xternalOut, /* char * loaded here */
		           &sepSyncGlobalVars.args.ipcXternal,/* bool   loaded here */
			   NULL,               /* i4     loaded here */
			   NULL,               /* float  loaded here */
			   FALSE               /* is option required */
			   );
...
#ifdef SEP_SYNC
    sepSyncGlobalVars.test.cmd    = cmd;
    sepSyncGlobalVars.test.tokens = tokens;
#endif
...
get_args(i4 argc,char **argv,char *testname)
{
	...
	switch (*pa)
	{
	   ...
	   case 'N': case 'n':
		if (STbcompare(pa+1, 0, ERx("UNORDERED"), 0, TRUE) == 0)
		    allow_unordered_flag = FALSE;
		break;
	   case 'O': case 'o':
