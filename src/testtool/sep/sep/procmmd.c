...
*/
#ifdef SEP_SYNC
GLOBALREF   char           testName[MAX_LOC];
#endif /** SEP_SYNC **/
...
FUNC_EXTERN  int           QAdiff ( SEPFILE **canFile, SEPFILE **resFile,
				    SEPFILE *diffFile, i4  max_d, char *unordered) ;
...
#ifdef SEP_SYNC
                          /* 
			  ** For Sep sync, we add a little extra to the .log output 
                          */
char                      *cbannerSync = ERx("====  CANON FILE (line %d) ====") ;
#endif
...
#ifdef SEP_SYNC
static void
readySepIPC(char *command)
{
    char buffer[1024];
    bool inSensitiveCmd = FALSE;
    i4   loop;

    sepSyncGlobalVars.childTimeOut  = sepSyncGlobalVars.args.timeOut;
    sepSyncGlobalVars.childSyncMode = 0;
    sepSyncGlobalVars.aborted       = 0;
    
    sepSyncGlobalVars.syncMode      = 0;
    if ( sepSyncGlobalVars.args.ipcMode && !sepSyncGlobalVars.ipcDisable )
    {
	/*
	** For the time being, only trigger these script 'commands' into sync mode
	** **ALL** others will use standard sep method. 
	** The execs array content is populated through the -sync switch.
	** ~ The value of which is stored in the listexec config file.
	*/
	for (loop=0;loop<SEPSYNC_EXECS;loop++)
	{
	    /* No more left */
	    if ( 0 == sepSyncGlobalVars.args.execs[loop] )
		break;

            if ( 0 == STcompare(sepSyncGlobalVars.args.execs[loop],command) )
	    {
		/*
		** This turns the sync actions ON. 
		*/
		sepSyncGlobalVars.syncMode = sepSyncGlobalVars.args.ipcMode;
		break;
	    }
	}
    }

    sepSyncGlobalVars.test.name   = &testName[0];
    sepSyncGlobalVars.test.line   = sepSyncGlobalVars.test.rawLine;

    /* 
    ** Set GDB assistance
    */
    sepSyncGlobalVars.gdb    = 0;
    sepSyncGlobalVars.gdbStr = "not-set";
    if ( sepSyncGlobalVars.args.gdbFlagged )
    {
	sepSyncGlobalVars.gdb    = 1;
	sepSyncGlobalVars.gdbStr = &sepSyncGlobalVars.args.gdbString[0];
    }

    sprintf(buffer, ",%s,", &sepSyncGlobalVars.args.timeString[0]);
    sepSyncGlobalVars.timings = strdup(&buffer[0]);
}
#endif /* SEP_SYNC */
...
PTM_read_fm_TM( i4  *thechar )
{
	...
	*thechar = SEP_TCgetc_trace(SEPtcoutptr, waitLimit);
	...
	tmp[0] = *thechar;
	while(i < l)
	    tmp[i++] = SEP_TCgetc_trace(SEPtcoutptr, waitLimit);
...
#ifdef SEP_SYNC
	/* Mark the alog file */
	showLinePushMark(SHOW_LINE_CMD);
#endif
...
#ifdef SEP_SYNC
    readySepIPC(sepSyncGlobalVars.test.tokens[1]);
#endif
        ...
        else
        {
            ...
            */
            ...
            while (waited < wait_for)
            {
               ...
               {
#ifdef SEP_SYNC
		   /** PCis_alive() is fooled by <zombie> processes, 
		    ** and could cause sep to hang over a child that
		    ** is waiting to exit. 
		    **
		    ** So, ALWAYS first check for exiting process 
		    **/
		   waitpid(SEPsonpid, &wait_status, WNOHANG);

		   /* Then call PCis_alive() */
#endif
...
Read_fm_FE_not_recording(STATUS *ioerr, i4  *recording)
{
    ...
    *ioerr = OK;
...
#ifdef SEP_SYNC
		/* 
		** Supresss these hard-wired sleeps
		** ~ Put them in the scripts if they are needed?
		*/
	...
	l = CMUTF8cnt(tmp);
	while(i < l)
	    tmp[i++] = SEP_TCgetc_trace(SEPtcoutptr, 0);
	SEPputcn(tmp, l, sepResults);
    }
    EXinterrupt(EX_ON);
...
STATUS
diff_canon(i4 *diffound, char *unordered)
{
    STATUS                 ret_val = OK ;
    i4                     diffvalue ;
...
#ifdef SEP_SYNC
    char                   cannonLine[256];
    GLOBALREF  i4          startTestLine;
    i4                     startLine = startTestLine;
#endif

    if ((diffvalue = QAdiff(&sepCanons,&sepResults,sepDiffer,diffLevel, unordered)))
    {
	/*
...
#ifdef SEP_SYNC
	/* 
	** We may have to change this to be a real conditional
	** if there are scripts out there that are messed up by
	** the extra line detail in the format. 
	**
	** But the idea is that we can better associate a DIFF
	** in the log file with the orginating line in the cannon script.
	** Sometimes it is quite difficult to figure out from the log file
	** what the 'diff' is actually about. My longterm aim is to provide
	** and 'sdiff' viewer; put the cannon and result side by side with
	** some highlighting. Look *here*, *this* is wrong!
	*/
	if (1)
	{
	    sprintf(cannonLine, cbannerSync, startLine);
	    append_obj(cannonLine, diffout, sepCanons, NULL);
	}
	else
#else
...
# else
    ...
    {
	/*
	** Unlike the trad method, we potentially have
	** a whole chain of sub processes to attend to.
	** Walk the chain closing everything down, 
	** we're done, and that's final!
	*/
	TCDisconnect(SEPsonpid);
    }
    else
    {
#endif
...
#ifdef SEP_SYNC
    } /** ends if ( sepSyncGlobalVars.syncMode ) */ 
#endif
