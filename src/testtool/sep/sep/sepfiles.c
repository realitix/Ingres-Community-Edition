...
**      26-Sep-2011 (horda03) B125781
**          VMS returns OK and number of bytes read, this
**          can be less than the size of the buffer, so
**          ensure the output is terminated at the correct
**          point.
*/
...
STATUS
SEPputcn(char *achar, i4 n, SEPFILE *fileptr)
{
    u_char                 buffer [2] ;
    i4                     count ;
    ...
    return(SIwrite(n, achar, &count, fileptr->_fptr));
}
...
#ifdef VMS
        else if (count < fileptr->_buffSize)
        {
            /* VMS returns OK but count < buffSize if less than
            ** buffSize characters remain in file
            */
	    fileptr->_buf[count++] = SEP_ESCAPE;
	    fileptr->_buf[count] = SEPENDFILE;
        }
...
*/
        ...
        {
/*
**      VMS returns OK but a count < buffSize if there
**      is less than buffSize left in the file.
*/
	    fileptr->_buf[count++] = SEP_ESCAPE;
	    fileptr->_buf[count] = SEPENDFILE;
	    toreturn = *fileptr->_pos++;
