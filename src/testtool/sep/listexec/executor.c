...
char *argv[];
{
#ifdef SEP_SYNC_STATS
    /* To calculate elapsed time for each test */
    SYSTIME                start,stop;
    i4                     sdelay;
    i4                     mdelay;
    i4                     sAccumDelay = 0;
    i4                     mAccumDelay = 0;
    char                   timer[100];
    char                   repLine [512] ; /* Need more space */
#else
    char                   repLine [256] ;
#endif
    char                   keyvalue [10] ;
    ...
    */
    ...
    {
	cptr = buffer;
	if (CMcmpcase(cptr,LIST_COMMENT_CHAR) == 0)
	{
	    CMnext(cptr);
	    if (CMcmpcase(cptr,LIST_COMMENT_CHAR) != 0)
	    {
	...
	cptr2 = cptr = buffer;
	if (CMcmpcase(cptr,LISTCMD_CH) == 0)
	{
	...
	*cptr = EOS;                        /* get rid of nl */
	...
	else
	{
	    ...
	    {
		TMnow(&stop);
		mdelay = stop.TM_msecs - start.TM_msecs;
		if ( mdelay < 0 )
		{
		    mdelay       += 1000000;
		    stop.TM_secs -= 1;
		}
		sdelay = stop.TM_secs - start.TM_secs;

		sAccumDelay += sdelay;
		mAccumDelay += mdelay;
		if ( mAccumDelay > 1000000 )
		{
		    sAccumDelay += 1;
		    mAccumDelay -= 1000000;
		}
		STprintf(timer, "%d.%d secs", sdelay, mdelay);
		SIprintf(ERx("Time taken = %d.%d seconds\n"), sdelay, mdelay);
	    }
#endif
	    sonres = prores;
...
#ifdef SEP_SYNC_STATS
    STprintf(buffer, ERx("Total Elapsed Time           : %d.%d\n"), sAccumDelay, mAccumDelay);
    SIprintf(buffer);
    SIputrec(buffer, repPtr);
#endif
