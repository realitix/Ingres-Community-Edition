...
#
#This version works in VectorWise and in standard Ingres installations.
#The VectorWise part is executed in VectorWise installations only.
#
...
#
#VectorWise information is stored in a "vw" subdirectory of the 
#inglogs directory.
#
...
## 31-May-2010   Kristoff Picard       version 1.27V
##    (VectorWise only)
##    add support for VectorWise. In this very first version, only the
##    log and config files are collected (if exist):
##      $HOME/.x100.conf  --> vw.home.x100.conf
##      $HOME/.vectorwise.conf  --> vw.vectorwise.x100.conf
##      II_VWDATA/ingres/data/vectorwise/x100.conf --> vw.inst.x100.conf
##      II_VWDATA/ingres/data/vectorwise/vectorwise.conf --> vw.inst.vectorwise.conf
##      II_VWDATA/ingres/data/vectorwise/*/x100.conf --> vw.db.dbname.x100.conf
##      II_VWDATA/ingres/data/vectorwise/*/vectorwise.conf --> vw.db.dbname.vectorwise.conf
##      vectorwise.log --> vw.vectorwise.log
##    VectorWise only, but wouldn't harm in standard Ingres, so we
##    may go back to one single version in the near future.
## 08-Sep-2010 (hanje04)
##    BUG 124390
##    SD 146293
...
## 03-feb-2011   Kristoff Picard       version 1.29
##    added flags -static as synonym for -nosystemstate -noingresstate
##    and -dynamic as synonym for -nosystemconfig -nologfiles -noingresconfig
## 08-Feb-2011   Kristoff Picard       version 1.29V
##    (VectorWise only)
##    integrated changes for Vectorwise (from separate change for version 127V),
##    but changed collecting of the configuration files a bit(to fit 1.5):
##    Collect log and config files  (if exist):
##      $HOME/.x100.conf  --> vw/home.x100.conf
##      $HOME/.vectorwise.conf  --> vw/home.vectorwise.conf
##      II_VWDATA/ingres/data/vectorwise/x100.conf --> vw/iivwdata.x100.conf
##      II_VWDATA/ingres/data/vectorwise/vectorwise.conf --> vw/iivwdata.vectorwise.conf
##      II_VWDATA/ingres/data/vectorwise/x100.dbname.conf --> vw/iivwdata.x100.dbname.conf
##      II_VWDATA/ingres/data/vectorwise/vectorwise.dbname.conf --> vw/iivwdata.vectorwise.dbname.conf
##      II_VWDATA/ingres/data/vectorwise/*/x100.conf --> vw/db.dbname.x100.conf
##      II_VWDATA/ingres/data/vectorwise/*/vectorwise.conf --> vw/db.dbname.vectorwise.conf
##      vectorwise.log --> vw/vectorwise.log
##    Call iivwstats and iivwinfo for each database, but only if the
##    x100 server for the database is running.
##    get pstacks of running x100 processes
##    collect lock files to get pid and port
##    All the vectorwise stuff is now collected in vw subdirectory
## 15-Mar-2011   Kristoff Picard       version 1.30VI
##    merge the VectorWise inglogs version with current main.
##    No functional changes for the standard Ingres as the VectorWise stuff
##    is not executed there. (Checking II_VWDATA)
##    For VectorWise no difference to previous version 129V, except
##    some comments.
## 16-mar-2011   Kristoff Picard       version 1.31
##    Don't copy all the dbmslog files, but only those files that has been 
##    modified in the last thirty days.
## 02-may-2011   Kristoff Picard       version 1.32
##    Moved the infodb call from the ingstate to the ingconfig module.
##    This solves a problem where inglogs is called by a execption handler
##    where iidatabase is blocked. To avoid blocking you still should use the 
##    appropriate flags when calling inglogs within an exception handler.
##    (-dynamic for example)
## 16-jun-2011   Kristoff Picard       version 1.33VI
##    vectorwise.log was no longer copied, added it back to ingres_logfiles()
##    corrected a cosmetic problem when looking for database specific 
##    configuration files.
#####################################################################
#
show_usage()
...
# check whether this is a Vectorwise installation and if so create vw subdir
  unset iivwdata
  iivwdata=`ingprenv II_VWDATA`
  [ -n "$iivwdata" ] && mkdir vw
...
find_vw_conf()
{
#we look for all *.conf files to catch x100.conf and vectorwise.conf in
#II_VWDATA/ingres/data/vectorwise/ and II_VWDATA/ingres/data/vectorwise/*/
   iwdata=$1
   vwdataconfig=`ls $iivwdata/ingres/data/vectorwise/*.conf` 2>/dev/null
   dbconfig=`ls $iivwdata/ingres/data/vectorwise/*/*.conf 2>/dev/null`
}
...
{
  [ -z "$iivwdata" ] && return
  find_vw_conf $iivwdata
  for i in $vwdataconfig
  do
     confsave=`echo $i | sed -e 's#.*ingres/data/vectorwise/#iivwdata.#'`
     cp $i vw/$confsave
  done
  for i in $dbconfig
  do
     confsave=`echo $i | sed -e 's#.*ingres/data/vectorwise/#db.#' -e 's#/#.#'`
     cp $i vw/$confsave
  done
  [ -r $HOME/.x100.conf ] && cp $HOME/.x100.conf vw/home.x100.conf
  [ -r $HOME/.vectorwise.conf ] && cp $HOME/.vectorwise.conf vw/home.vectorwise.conf
}
...
copy_dbmslog()
{
  ...
  find $dbmslog -prune -type f -mtime -30 -exec cp -p {} . \;
}
...
ingres_logfiles()
{
  ...
  [ -r $F/vectorwise.log ]  && cp -p $F/vectorwise.log vw/
}
...
check_vectorwise_running()
{
  vwrunningdb=''
  vwpidlist=''
  for i in $iivwdata/ingres/data/vectorwise/*/CBM/lock
  do
     vwpidtmp=`sed -n -e '2 p' $i` 
     ptmp=`ps -fp $vwpidtmp | grep iix100 | grep -v '<defunct>'`
     [ -n "$ptmp" ] && {
         vwdbtmp=`echo $i | sed -e 's#.*ingres/data/vectorwise/##' -e 's#/CBM/lock##'`
         vwrunningdb="$vwrunningdb $vwdbtmp"
         vwpidlist="$vwpidlist $vwpidtmp"
         cp $i vw/$vwdbtmp.lock
     }
  done
}
...
vectorwise_state()
{
  [ -z "$iivwdata" ] && return
  check_vectorwise_running
  for i in $vwrunningdb
  do
     eval iivwinfo -db $i >vw/iivwinfo.$i.out 2>&1 $bg
     eval iivwinfo -c -db $i >vw/iivwinfo_config.$i.out 2>&1 $bg
  done
  [ -n "$STACKCMD" ] && {
    for i in 1 2
    do
       for pid in $vwpidlist
       do
          eval $STACKCMD $pid >vw/pstack_$pid.$i.out 2>&1 &
       done
       sleep 1
    done
  }
...
echo clean_up
   ...
   BG4=`cat .bg_ingresconfig`
   BGALL=`echo $BG1 $BG2 $BG3 $BG4`
   for i in $BGALL 
   do
      kill $i 2>/dev/null
   done
...
{
echo clean_up2
   BGCLEAN2=`cat .bg_iimon 2>/dev/null`
   for i in $BGCLEAN2 
   do
      kill $i 2>/dev/null
   done
