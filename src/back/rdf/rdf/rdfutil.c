...
**	05-apr-2011 (stial01) m1748 (and related m1595)
**	    rdu_private() if RDR_ATTRIBUTES is set, copy rdr_keys also
**/
...
**	9-nov-2010 (dougi)
**	    Add INTEGRITIES, COLCOMPARE, REFREL to rdu_private() (for IVW).
*/
DB_STATUS
		...
		RDR_INFO	   **infoblkpp)
{
	...
	*/
	...
	if (sys_infoblk->rdr_types & RDR_RELATION)
	{
	    ...
	    usr_infoblk->rdr_rel->tbl_attr_count = usr_infoblk->rdr_no_attr = 
			sys_infoblk->rdr_no_attr;
	...
	if (sys_infoblk->rdr_types & RDR_ATTRIBUTES)
	{
	    ...
	    /* rdr_keys are now init when RDR_ATTRIBUTES are done */
	...
	if (sys_infoblk->rdr_types & RDR_INTEGRITIES)
	{
	    usr_infoblk->rdr_ituples = sys_infoblk->rdr_ituples;
	    types |= RDR_INTEGRITIES;
	}
	if (sys_infoblk->rdr_2_types & RDR2_COLCOMPARE)
	{
	    usr_infoblk->rdr_colcompare = sys_infoblk->rdr_colcompare;
	    types2 |= RDR2_COLCOMPARE;
	}
	if (sys_infoblk->rdr_2_types & RDR2_REFREL)
	{
	...
	usr_infoblk->rdr_2_types = types2;
    }
...
rdu_attr_dump(RDR_INFO	*info)
{
	...
	for (i=1, attr = info->rdr_attr; i <= info->rdr_no_attr; i++)
	{
		...
		TRdisplay("attribute collation : %d\n", attr[i]->att_collID);
