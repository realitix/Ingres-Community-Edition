...
**	    lpb.pr_type is now lpb.pr_status.
...
**		    tree_type (PSQ_MODE - PSQ_PROT, PSQ_INTEG)
**		    database id (i4)
**		b) to specify a rule:
...
**		    unique table identifier (i4)
**		    tree_type (PSQ_MODE - rule, protection, integrity)
**		    database id (i4)
**		To use this option, the rdf_cb.rdf_info_blk must be set to NULL
...
**					protection tree: PSQ_PROT
**					integrity tree:  PSQ_INTEG
**					rule tree:	 PSQ_RULE
		...
		*/
		...
		if (rdfcb->rdf_rb.rdr_2types_mask & RDR2_KILLTREES)
		{
		    ...
		    */
		    ...
		    {
			status = rdr_killtree(global, PSQ_PROT, baseid);
			if (DB_FAILURE_MACRO(status))
			    break;
		    }
		    if (relptr->tbl_status_mask & DMT_INTEGRITIES)
		    {
			status = rdr_killtree(global, PSQ_INTEG, baseid);
			if (DB_FAILURE_MACRO(status))
			    break;
		    }
		    if (relptr->tbl_status_mask & DMT_RULE)
		    {
			status = rdr_killtree(global, PSQ_RULE, baseid);
			if (DB_FAILURE_MACRO(status))
			    break;
		    }
		} /* end if KILLTREES */
...
rdf_ismultiserver()
{
    ...
    for (;;)
    {
        ...
        if ( (lpb.pr_status & PR_FCT) || (lpb.pr_status & PR_SLAVE) )
	    count++;
	if ( count > 1 )
	    break;
    }
