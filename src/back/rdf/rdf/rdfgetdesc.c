...
/* rdu_get_tbl_stats - DMT_SHOW to get updated row/page counts */
static DB_STATUS rdu_get_tbl_stats(	RDF_GLOBAL         *global,
		    DMT_TBL_ENTRY *tbl, 
		    i8 *tbl_record_count, 
		    i8 *tbl_page_count);
...
**          rdu_bldattr() init primary key array and joinatt 
**          rdu_key_ary_bld() deprecated
**      14-mar-2011 (stial01) m1595
**          Do one memory allocation for attr and keys.
**      05-apr-2011 (stial01) m1748
**          Added rdu_get_tbl_stats called by rdu_cache.
**      05-apr-2011 (stial01) m1748
**          rdu_get_tbl_stats() don't refresh if distributed db.
**/

/*{
		...
		{ 
		    ...
		    *flagmask |= RDU_2CACHED;
			...
			if ((global->rdfcb->rdf_rb.rdr_2types_mask & RDR2_REFRESH) == 0)
			{
			    ...
			    ** we did a show:
			    */
			    memcpy(dmt_shw_cb->dmt_table.data_address,
				rdf_ulhobj->rdf_sysinfoblk->rdr_rel,
	    ...
	    bool	*refreshed)
{
    ...
    */
	...
	{	/* need to find table ID */
	    ...
	    {   
		tbl = rdf_ulhobject->rdf_sysinfoblk->rdr_rel;
		...
		** do not bother to validate the checksum.
		...
		checksum = ult_check_macro(&Rdi_svcb->rdf_trace,
					RDU_CHECKSUM, &v1, &v2);

		/*
		** If NOT refreshing table entry, get updated row/page counts
		** Get them now before we get mutex
		*/
                if ( (rdfcb->rdf_rb.rdr_2types_mask & RDR2_REFRESH) == 0)
		{
		    status = rdu_get_tbl_stats(global, tbl,
			&record_count, &page_count);

		    /* skip refresh for small changes */
		    if (status == E_DB_OK && 
			((tbl->tbl_record_count == 0 && record_count != 0)
			|| abs(tbl->tbl_record_count - record_count) > 100))
		    {
			refresh_stats = TRUE;
		    }
		}
		    ...
		    /*
		    ** If caller requested updated row/page counts
		    ** AND fixcount is 0
		    ** update rdr_rel now them now before rdu_rsemaphore
		    ** computes checksum, releases semaphore
		    */
		    if (refresh_stats && rdf_ulhobject->rdf_shared_sessions == 0)
		    {
			tbl->tbl_record_count = record_count;
			tbl->tbl_page_count = page_count;
		    }
	...
	** the ULH alias call */
	   ...
	   )
	{
	    ...
	    */
		...
		/*
		** NOT refreshing table entry, so get updated row/page counts
		** Get them now before we get mutex
		*/
		status = rdu_get_tbl_stats(global, tbl,
			    &record_count, &page_count);
	    ...
	    bool	*refreshed)
{
	...
	memcpy(rel_ptr, &global->rdf_trel, sizeof(*rel_ptr));
    }
    else
    {
...
**	5-july-2010 (dougi)
**	    Add an entry to attr array for IVW tables to possibly allow for
**	    join index column.
**	18-Mar-2011 (kschendel)
**	    attnametot from DMF includes null trailers, don't re-include.
**	    Allocate maxkeys key entries if VW, keys might be hidden away
**	    masquerading as an index.
*/
static DB_STATUS
		...
		bool		    *refreshed)
{
    ...
    char	    *attr_names;
    i4		    extracols = 0;
    DMT_ATT_ENTRY  *lastatt;
    DMT_ATT_ENTRY  *joinatt = NULL;
    ...
    ** whereas the sys_infoblk does not */
    ...
    /* If IVW table, make room for possible join index column, too.
    ** attnametot in the infoblk has already been tricked out to allow
    ** for the name, but we must allow for the pointer and att entry too.
    */
    if (usr_infoblk->rdr_rel->tbl_storage_type > DB_STDING_STORE_MAX)
    {
	extracols = 1;
    }

    /* Allocate att descriptors, pointers, and name space.
    ** Allocate extra name space for tricked-out join index column too.
    */
    rdr_attr_size = 
	(i4)(sizeof(PTR) * (usr_infoblk->rdr_no_attr + 1 + extracols) +
	     sizeof(DMT_ATT_ENTRY) * (usr_infoblk->rdr_no_attr+extracols) +
	     usr_infoblk->rdr_attnametot +
	     extracols * (DB_ATT_MAXNAME+1));
    rdr_attr_size = DB_ALIGN_MACRO(rdr_attr_size);

    /* add 1 for zero terminator key attno.
    ** Note, if we have a VW table with index, we may discover later
    ** (in rdu_bldindx) that the table keys are the index keys.  (An
    ** effect of the unfortunate decision to define a VW structure with
    ** create index instead of modify.)  So that we don't have to pull the
    ** rdr_keys rug out from a concurrent infoblk user, allow for DB_MAXKEYS
    ** keys for a VW table.
    */
    if (usr_infoblk->rdr_rel->tbl_storage_type > DB_STDING_STORE_MAX)
	rdr_key_size = (i4) (sizeof(RDD_KEY_ARRAY) + 
		sizeof(i4) * (DB_MAXKEYS+1) );
    else
	rdr_key_size = (i4) (sizeof(RDD_KEY_ARRAY) + 
		sizeof(i4) * (usr_infoblk->rdr_rel->tbl_key_count + 1)); 
    rdr_key_size = DB_ALIGN_MACRO(rdr_key_size);

    status = rdu_malloc(global, (rdr_attr_size + rdr_key_size), (PTR *)&attr);
    if (DB_FAILURE_MACRO(status))
	return(status);

    rdr_keys = (RDD_KEY_ARRAY *)((char *)attr + rdr_attr_size);

    attr[DB_IMTID] = (DMT_ATT_ENTRY *) NULL; /* initialize the TID attribute */

    {
	attr_entry = (DMT_ATT_ENTRY *)(attr + (usr_infoblk->rdr_no_attr + 1 + extracols));
	attr_names = (char *)(attr_entry + usr_infoblk->rdr_no_attr + extracols);
    ...
    {
       TRdisplay("warning nametot mismatch for tab %d %d\n",
	    global->rdfcb->rdf_rb.rdr_tabid.db_tab_base,
	    global->rdfcb->rdf_rb.rdr_tabid.db_tab_index);

	TRdisplay("rdu_bldattr atts %d nametot %d attr %p attr_entry %p attr_names %p (%p %p %p %p)\n",
	...
	*/
	...
	/* Don't need extracols here since dmt-show doesn't see the
	** extra fake join att
	*/
    ...
    if (extracols)
    {
	joinatt = &attr_entry[usr_infoblk->rdr_no_attr];
	attr[usr_infoblk->rdr_no_attr+1] = joinatt;
	lastatt = attr[usr_infoblk->rdr_no_attr];

	joinatt->att_nmstr = lastatt->att_nmstr + lastatt->att_nmlen + 1;
	memcpy(joinatt->att_nmstr, "_j", 2);
	cui_move(cui_trmwhite(DB_TAB_MAXNAME,usr_infoblk->rdr_rel->tbl_name.db_tab_name),
		usr_infoblk->rdr_rel->tbl_name.db_tab_name,
		'\0', DB_ATT_MAXNAME-2, &joinatt->att_nmstr[2]);

	joinatt->att_nmlen = STlength(joinatt->att_nmstr);
	joinatt->att_offset = lastatt->att_offset + lastatt->att_width;
	joinatt->att_type = DB_TID8_TYPE;
	joinatt->att_width = DB_TID8_LENGTH;
	joinatt->att_prec = 0;
	joinatt->att_flags = (DMU_F_KNOWN_NOT_NULLABLE | 
				DMU_F_HIDDEN | DMT_F_VWJOINIX);
	joinatt->att_key_seq_number = 1;
	joinatt->att_defaultID.db_tab_base = 3;
	joinatt->att_defaultID.db_tab_index = 0;
	joinatt->att_default = NULL;
	joinatt->att_intlid = lastatt->att_intlid + 1;
	joinatt->att_number = lastatt->att_number + 1;
	joinatt->att_collID = -1;
	joinatt->att_srid = -1;
	joinatt->att_geomtype = -1;
	joinatt->att_encflags = 0;
	joinatt->att_encwid = 0;
    }

    /* Init primary key array */
    rdr_keys->key_array = (i4 *)(((char *)rdr_keys) + sizeof(RDD_KEY_ARRAY));
    if (usr_infoblk->rdr_rel->tbl_key_count)
    {
	i4	key_seq;
	for (att_cnt = 1; att_cnt <= usr_infoblk->rdr_no_attr; att_cnt++)
	{
	    key_seq = attr[att_cnt]->att_key_seq_number;
	    if (key_seq > usr_infoblk->rdr_rel->tbl_key_count) 
	    {
	    ...
	    if (key_seq > 0)
		rdr_keys->key_array[key_seq-1] = attr[att_cnt]->att_number;
	}
    }
    rdr_keys->key_array[usr_infoblk->rdr_rel->tbl_key_count] = 0;
    rdr_keys->key_count = usr_infoblk->rdr_rel->tbl_key_count; 

    /* Store the info into the user info block (might be same as master) */
    if (updating_master)
    {
	status = rdu_gsemaphore(global);
	if (DB_FAILURE_MACRO(status))
...
**	18-Mar-2011 (kschendel)
**	    If a VW base table, the "index" is really equivalent to a
**	    structure on the base table.  Adjust the key array to match.
**	23-Mar-2011 (kschendel) m1805
**	    Fix to the above:  if caller has requested REL + INDEX info, but
**	    not ATT info, there's no key array.  Check first.
*/
static DB_STATUS
	...
	** check that this is the case */
	...
	{   /* rdr_indx needs to be consistent prior to making it visible in the
	    ** system descriptor */
	    ...
	    /* Fudge X100 table so that base keys match the index.
	    ** We allocated maxkeys space in the rdr-keys just to allow this.
	    ** Make sure we have att/key info before meddling, though.
	    */
	    if ((sys_infoblk->rdr_rel->tbl_storage_type == DB_X100_STORE
		 || sys_infoblk->rdr_rel->tbl_storage_type == DB_X100RW_STORE)
	      && sys_infoblk->rdr_keys != NULL
	      && sys_infoblk->rdr_no_index > 0
	      && sys_infoblk->rdr_keys->key_count <= 0)
	    {
		i2 i;
		for (i = 0; i < sys_infoblk->rdr_indx[0]->idx_array_count; i++)
		    sys_infoblk->rdr_keys->key_array[i] =
		    		sys_infoblk->rdr_indx[0]->idx_attr_id[i];

    		sys_infoblk->rdr_keys->key_count = i;
		sys_infoblk->rdr_rel->tbl_storage_type = DB_X100IX_STORE;
	    }
	    status = rdu_rsemaphore(global);
	    if (DB_FAILURE_MACRO(status))
		return (status);
	...
	/* Fudge X100 table so that base keys match the index.
	** We allocated maxkeys space in the rdr-keys just to allow this.
	** It's a bit scary doing this for a private infoblk because
	** for all we know, rdr-keys points to the master copy!
	** (which would imply that nobody has reached this point for
	** the master copy yet.)  If we're updating the master, do it
	** under mutex protection.
	** This is all just totally wrongheaded but it's the best we can do
	** for now.
	*/
	if ((usr_infoblk->rdr_rel->tbl_storage_type == DB_X100_STORE
	     || usr_infoblk->rdr_rel->tbl_storage_type == DB_X100RW_STORE)
	  && usr_infoblk->rdr_no_index > 0
	  && usr_infoblk->rdr_keys->key_count <= 0)
	{
	    i2 i;
	    if (usr_infoblk->rdr_keys == sys_infoblk->rdr_keys)
	    {
		status = rdu_gsemaphore(global);
		if (DB_FAILURE_MACRO(status))
		    return (status);
	    }
	    for (i = 0; i < usr_infoblk->rdr_indx[0]->idx_array_count; i++)
		usr_infoblk->rdr_keys->key_array[i] =
			    usr_infoblk->rdr_indx[0]->idx_attr_id[i];

	    usr_infoblk->rdr_keys->key_count = i;
	    usr_infoblk->rdr_rel->tbl_storage_type = DB_X100IX_STORE;
	    if (usr_infoblk->rdr_keys == sys_infoblk->rdr_keys)
	    {
		status = rdu_rsemaphore(global);
		if (DB_FAILURE_MACRO(status))
...
**	6-jan-06 (dougi)
**	    Add support for RDR2_COLCOMPARE for loading column comparison
**	    statistics.
**	10-Sep-2008 (jonj)
...
**	17-dec-2008 (dougi)
**	    Add support for RDR2_REFREL for loading referential relationship
**	    column mappings.
*/
DB_STATUS
    ...
    */
	...
	do
	{
		...
		** comparison statistics. */
	    ...
	    {
		/* Not much to do - just call func to load referential
		** relationship definition info (esp. column maps). */
		status = rdu_bld_refrel(global);
		if (DB_FAILURE_MACRO(status))
		    return(status);
		status = E_DB_OK;
	    }
...

/*{
** Name: rdu_get_tbl_stats - get updated row/page counts 
**
** Description:
**      This routine does dmf_call(DMT_SHOW) to get updates row/page counts
**
** Inputs:
**      global                          ptr to RDF state variable
**      tbl				DMT_TBL_ENTRY
**      tbl_record_count		ptr to record count variable
**      tbl_page_count			ptr to page count variable
**		    .
** Outputs:
**      tbl_record_count
**      tbl_page_count
...
**      05-apr-2011 (stial01) m1748
**          initial creation
**	12-Apr-2011 (kschendel) SIR 125253
**	    Don't get stats if by-name RDF request.  It's probably the
**	    parser and the table might not actually exist.  OPF, who is
**	    the guy who really needs the updates stats, will not do by-name
**	    RDF requests for tables.
*/
static DB_STATUS
rdu_get_tbl_stats(	RDF_GLOBAL         *global,
			DMT_TBL_ENTRY *tbl, 
			i8 *tbl_record_count, 
			i8 *tbl_page_count)
{
    ...
    /* Init to current record/page counts */
    *tbl_record_count = tbl->tbl_record_count;
    *tbl_page_count = tbl->tbl_page_count;

    /* Dont keep refreshing catalogs */
    if (tbl->tbl_status_mask & DMT_CATALOG)
	return (E_DB_WARN);

    /* Dont refresh if distributed db */
    rdfcb = global->rdfcb;
    if (rdfcb->rdf_rb.rdr_r1_distrib & DB_3_DDB_SESS)
	return (E_DB_WARN);

    /* Don't refresh if caller is asking for by-name.  It's possible that
    ** we have a stale table ID that isn't really there.  The by-name SHOW
    ** will fail (correctly) but this by-ID SHOW might sort-of-work, with
    ** all sorts of DMF errors.  (Example:  lar28, which creates a nonjnl
    ** table in a jnl database, inserts a row, then rollforwarddb +j the
    ** db.  The nonjnl table is now gone but cache remnants in the dbms
    ** server still show it as existing.
    */
    if (rdfcb->rdf_rb.rdr_types_mask & RDR_BY_NAME)
	return (E_DB_WARN);
    ...
    dmt_shw_cb.dmt_flags_mask = DMT_M_STATS;
    dmt_shw_cb.dmt_table.data_address = (PTR) NULL;
    dmt_shw_cb.dmt_table.data_in_size = 0;
    dmt_shw_cb.dmt_char_array.data_address = (PTR)NULL;
    dmt_shw_cb.dmt_char_array.data_in_size = 0;
    dmt_shw_cb.dmt_char_array.data_out_size  = 0;
    dmt_shw_cb.error.err_code = 0;
    dmt_shw_cb.dmt_db_id = global->rdfcb->rdf_rb.rdr_db_id;
    dmt_shw_cb.dmt_session_id = global->rdfcb->rdf_rb.rdr_session_id;
    STRUCT_ASSIGN_MACRO(tbl->tbl_id, dmt_shw_cb.dmt_tab_id);

    status = dmf_call(DMT_SHOW, &dmt_shw_cb);
    if ( status == E_DB_OK )
    {
	*tbl_record_count = dmt_shw_cb.dmt_reltups;
	*tbl_page_count = dmt_shw_cb.dmt_relpages;
    }
    return (status);
}  
