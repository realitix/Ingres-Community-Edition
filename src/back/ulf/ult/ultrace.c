...
**          close, FETCH and CLOSE are now for SQL only. 
*/
...
ult_print_tracefile(void *file, i2 type, char *string)
{
	...
	switch (type)
	{
		...
		case SC930_LTYPE_FETCH:
				type_str="FETCH";
				break;
		case SC930_LTYPE_QCLOSE:
				type_str="QCLOSE";
				break;
		case SC930_LTYPE_QFETCH:
				type_str="QFETCH";
				break;
		...
		case SC930_LTYPE_REQUERY:
 		...
 		case SC930_LTYPE_XA_ROLLBACK:
 				type_str="XA_RBCK";
 				break;
 		case SC930_LTYPE_VECTORWISE_QRY:
 				type_str="IVW";
 				break;
...
/*{
** Name: ult_set_trace_blobsegs	- set how many segments of a blob to output
**
** Description:
**      Sets the blobsegs static variable for this server. This is how many
**      segments of a blob to output in SC930 tracing.
**
** Inputs:
**      value to set the blobsegs value to. 0 is ALL
**
** Outputs:
**	Returns:
**	    none
...
static i4 blobsegs = SC930_DEFAULT_BLOBSEGS;
void
ult_set_trace_blobseg(i4 val)
{
      /* if we're changing the setting then log a message */
	  ...
	  if (val)
          {
	      TRdisplay("SC930 will output maximum of %d segments of BLOBs\n",
	                 val);
	  }
	  else
	  {
	      TRdisplay("SC930 will output ALL segments of BLOBs\n");
	  }
...
/*{
** Name: ult_trace_blobsegs	- Return how many blob segments to output
**
** Description:
**      Returns value of blobsegs which is how many segments of a blob to
**      output in SC930 tracing
**
** Inputs:
**
** Outputs:
**	Returns:
**	    blobsegs -      how many segments to output in SC930 tracing
**                          0 means all
**	Exceptions:
**	    none
**
