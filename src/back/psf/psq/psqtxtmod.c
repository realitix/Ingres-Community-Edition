	...
	DB_ERROR	   *err_blk)
{
	...
	if (ulm_rcb.ulm_error.err_code == E_UL0005_NOMEM)
	{
		...
		&ulm_rcb.ulm_error, PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
	...
	DB_ERROR	   *err_blk)
{
	...
	if (hp->psq_tmem.ulm_error.err_code == E_UL0005_NOMEM)
	{
		...
		&hp->psq_tmem.ulm_error, PSF_INTERR, &err_code,err_blk, 0);
	return (status);
    }
	...
	DB_ERROR	   *err_blk)
{
	...
	*/
	...
	{
	    (VOID) psf_error(E_PS0371_ALLOC_TEXT_CHAIN,
		&hp->psq_tmem.ulm_error, PSF_INTERR, &err_code, err_blk,
		0);
	}
	...
	DB_ERROR	   *err_blk)
{
	...
	(VOID) psf_error(E_PS0375_UNLOCK_QSF_TEXT,
	    &qsf_rb.qsf_error, PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
    mstream->psf_mlock = 0;
	...
	DB_ERROR	   *err_blk)
{
    ...
    status = adi_ficoerce_full(adf_cb, dbval->db_datatype, totype,
	...
	** data.
	*/
	status = adi_0calclen(adf_cb, &fi_desc.adi_lenspec, 1, &dbval, 
		&adffn.adf_r_dv);
	...
	if (hp->psq_tmem.ulm_error.err_code == E_UL0005_NOMEM)
	{
	    ...
	    (VOID) psf_error(E_PS0371_ALLOC_TEXT_CHAIN,
		&hp->psq_tmem.ulm_error, PSF_INTERR,
		&err_code, err_blk, 0);
	return (status);
    }
    *result = hp->psq_tmem.ulm_pptr;
