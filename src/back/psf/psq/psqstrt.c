...
**	    Make the PSF_SERVBLK truely private to PSF.
...
i4 psq_startup(
	PSQ_CB *psq_cb);

PSF_SERVBLK *Psf_srvblk = NULL;
static PSF_SERVBLK srvblk;
...
**          Pass table autostruct to server block
*/
DB_STATUS
psq_startup(
	PSQ_CB             *psq_cb)
{
    ...
    */
    if (Psf_srvblk)
    {
	(VOID) psf_error(E_PS0701_SRV_ALREADY_UP, (DB_ERROR *)NULL, PSF_INTERR,
	...
	*/
	...
	{
	    (VOID) psf_error(E_PS0704_INSF_MEM, (DB_ERROR *)NULL, PSF_INTERR, 
                &err_code, &psq_cb->psq_error, 0);
	    return (E_DB_FATAL);
	}
	else
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0702_NO_MEM, (DB_ERROR *)NULL, PSF_CALLERR, 
            &err_code, &psq_cb->psq_error, 0);
	return (status);
    }
    ...
    */
    ...
    if (psq_cb->psq_flag2 & PSQ_TABLE_AUTOSTRUCT)
    {
	Psf_srvblk->psf_flags |= PSF_TABLE_AUTOSTRUCT;
    }
