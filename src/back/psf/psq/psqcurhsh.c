	...
	DB_ERROR	   *err_blk)
{
    ...
    *cursor = rescsr;
    ...
    {
	(VOID) psf_error(2226L, (DB_ERROR *)NULL, PSF_USERERR, 
                 &err_code, err_blk, 1,
	...
	DB_ERROR	   *err_blk)
{
	...
	{
	    (VOID) psf_error(E_PS0A02_BADALLOC, &ulm_cb.ulm_error,
	...
	DB_ERROR	   *err_blk)
{
    ...
    */
    ...
    if (DB_FAILURE_MACRO(status))
    {
        (VOID) psf_error(E_PS0A05_BADMEMREQ, &ulm_cb.ulm_error,
	    PSF_INTERR, &err_code, err_blk, 0);
        if (ulm_cb.ulm_error.err_code == E_UL0004_CORRUPT)
	    status = E_DB_FATAL;
	return (status);
		...
		PSS_SESBLK  *sess_cb)
{
    ...
    */
	...
	i4	    lineno = 1;
        ...
        (VOID) psf_error(2210L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 2,
...

/*{
** Name: psq_any_vwtab_cursor Find any OPEN cursor on x100/vectorwise table
**
** Description:
**      This function looks for any OPEN cursor on x100/vectorwise table
**
** Inputs:
**	sess_cb			PSF session CB
**      err_blk
...
**
** History:
**	25-may-2011 (stial01) m1627
**	    created
*/
bool
psq_any_vwtab_cursor(
	PSS_SESBLK         *sess_cb,
	DB_ERROR	   *err_blk)
{
    register PSC_CURBLK	**bucket;
	...
	while (cursor != (PSC_CURBLK *) NULL)
	{
	    /* looking for open cursor on vw table */
	    if (cursor->psc_open && cursor->psc_vwtab_inq)
		return (TRUE);
