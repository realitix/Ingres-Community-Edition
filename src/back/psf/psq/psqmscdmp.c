...
**	    Replace some #s formats with blank-stripping ~t.
	...
	DB_DT_ID           dt_id)
{
    ...
    {
	TRdisplay("%~t", sizeof (adt_name.adi_dtname), adt_name.adi_dtname);
    }
    else
    {
...
**	19-sep-89 (andre)
**          written
**	9-Mar-2011 (kschendel)
**	    Dump varno's, not bits.
*/
DB_STATUS
psq_jrel_dmp(
	PST_J_MASK	*inner_rel,
	PST_J_MASK	*outer_rel)
{
    ...
    TRdisplay("\n\t\t\tInner relation mask:");
    i = -1;
    while ( (i = BTnext(i, (char *) inner_rel, PST_NUMVARS)) != -1)
	TRdisplay(" %d",i);

    TRdisplay("\n\t\t\tOuter relation mask:");
    i = -1;
    while ( (i = BTnext(i, (char *) outer_rel, PST_NUMVARS)) != -1)
	TRdisplay(" %d",i);
