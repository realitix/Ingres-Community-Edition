	...
	PSS_SESBLK	*sess_cb)
{
    ...
    {
	(VOID) psf_error(2205L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
	    &psq_cb->psq_error, 2,
    ...
    {
	(VOID) psf_error(2211L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
	    &psq_cb->psq_error, 2,
	    sizeof(sess_cb->pss_lineno), &sess_cb->pss_lineno,
	    psf_trmwhite(DB_CURSOR_MAXNAME, psq_cb->psq_cursid.db_cur_name),
	    psq_cb->psq_cursid.db_cur_name);
    ...
    */
	...
	if (MEcmp((PTR) &psq_cb->psq_tabname, (PTR) &csr->psc_tabnm, DB_TAB_MAXNAME)
	    ...
	    MEcmp((PTR) &psq_cb->psq_als_owner, (PTR) &csr->psc_ownnm,
		  DB_OWN_MAXNAME))
	{
	    (VOID) psf_error(2227L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
		&psq_cb->psq_error, 4,
