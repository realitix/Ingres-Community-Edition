...
#define	MAX_PARMS 5
    ...
    /* Take local copy of detail. */
    if(detail == (DB_ERROR *)NULL)
        CLRDBERR(&local);
    else
        STRUCT_ASSIGN_MACRO(*detail, local);
    detail = &local;
	...
	if (err_type == PSF_CALLERR)
	{
	    ...
	    err_blk->err_data = detail->err_code;
	    /* ...and whence it came */
	    err_blk->err_file = detail->err_file;
	    err_blk->err_line = detail->err_line;
	}
	else if (err_type == PSF_INTERR)
	{
	    if (detail->err_code)
	    {
		SETDBERR(err_blk, detail->err_code, E_PS0007_INT_OTHER_FAC_ERR);
	    }
	    else
	    {
		SETDBERR(err_blk, detail->err_code, E_PS0002_INTERNAL_ERROR);
	    }
	}
	else if (err_type == PSF_USERERR)
	{
	    SETDBERR(err_blk, detail->err_code, E_PS0001_USER_ERROR);
	}
	...
	*/
	if (err_type != PSF_CALLERR)
	{
	    ...
	    if (uletype == ULE_LOG && detail->err_code)
	    {
		uleret = uleFormat(detail, 0, NULL, ULE_LOG, (DB_SQLSTATE *) NULL,
		...
		*/
		if (uleret != E_DB_OK)
		{
			...
			not found in INGRES error file", detail->err_code, detail->err_code);
	...
	DB_ERROR    *err_blk)
{
	...
	case E_RD002B_LOCK_TIMER_EXPIRED:
           ...
           ** E_PS0010.
           */
           (VOID) psf_error(4702L, (DB_ERROR *)NULL,
	...
	case E_RD0050_TRANSACTION_ABORTED:
	    (VOID) psf_error(E_PS0F03_TRANSACTION_ABORTED, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;

	case E_RD0043_CACHE_FULL:
	    (VOID) psf_error(E_PS0F01_CACHE_FULL, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;

	case E_RD0145_ALIAS_MEM:
	    (VOID) psf_error(E_PS0458_ULH_ALIAS_ERROR, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;

	case E_RD001A_RESOURCE_ERROR:
	    (VOID) psf_error(E_PS0F04_RDF_RESOURCE_ERR, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;
	...
	case E_RD0200_NO_TUPLES:
	    (VOID) psf_error(E_PS0D31_NO_TUPLES, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;

	case E_RD0277_CANNOT_GET_ASSOCIATION:
	    (VOID) psf_error(E_PS091B_NO_CONNECTION, (DB_ERROR *)NULL,
		PSF_USERERR, &err_code, err_blk, 0);
	    break;
	...
	default:
	    ...
	    (VOID) psf_error(psferror, rdf_errblk, 
	...
	DB_ERROR    *err_blk)
{
	...
	default:
	{
	    (VOID) psf_error(E_PS0D20_QEF_ERROR, qef_errblk,
		PSF_INTERR, &err_code, err_blk, 0);
	    break;
	}
    }
