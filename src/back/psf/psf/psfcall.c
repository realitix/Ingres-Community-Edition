...
**	    Simplify batch insert-to-copy optimization flags, all parser needs
**	    is "last stmt was copy-optimized insert" flag, sequencer can
**	    figure out all the rest.  Move last-sname cleaning to pst.
	...
	PSS_SESBLK	*sess_cb)
{
    ...
    */
	...
	switch (qeu_cb.error.err_code)
	{
	    ...
	    {
		(VOID) psf_error(E_PS0D22_QEF_ERROR, &qeu_cb.error,
	    ...
	    {
		(VOID) psf_error(E_PS0003_INTERRUPTED, (DB_ERROR *)NULL, 
	    ...
	    {
		(VOID) psf_error(E_PS0D23_QEF_ERROR, &qeu_cb.error,
		    PSF_USERERR, &err_code, &db_error, 0);
		break;
	    }
	    /* resource quota */
	    ...
	    {
		(VOID) psf_error(4707L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	    case E_QE0086_ERROR_BEGINNING_TRAN:
	    {
		(VOID) psf_error(E_PS0D24_BGN_TX_ERR, &qeu_cb.error,
		    PSF_USERERR, &err_code, &db_error, 0);
		break;
	    }
	    /* log full */
	    ...
	    {
		(VOID) psf_error(4706L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	    /* deadlock */
	    ...
	    {
		(VOID) psf_error(4700L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	    /* lock quota */
	    ...
	    {
		(VOID) psf_error(4705L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	    ...
	    {
		(VOID) psf_error(38L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	...
	switch (qeu_cb.error.err_code)
	{
	    ...
	    {
		(VOID) psf_error(4702L, &qeu_cb.error, PSF_USERERR,
		    &err_code, &db_error, 0);
		break;
	    }
	    /* resource quota */
	    ...
	    {
		(VOID) psf_error(E_PS0D25_CMT_TX_ERR, &qeu_cb.error,
		    PSF_USERERR, &err_code, &db_error, 0);
		break;
	    }
	    case E_QE0088_ERROR_ABORTING_TRAN:
	    {
		(VOID) psf_error(E_PS0D26_ABT_TX_ERR, &qeu_cb.error,
		    PSF_USERERR, &err_code, &db_error, 0);
		break;
	    }
	    /* log full */
