	...
	DB_ERROR	   *err_blk)
{
    ...
    sess_cb->pss_qsf_rcb.qsf_feobj_id.qso_lname = 0;
	...
	if (qerr == E_QS0001_NOMEM)
	    (VOID) psf_error(qerr, &sess_cb->pss_qsf_rcb.qsf_error, 
                PSF_CALLERR, &err_code, err_blk, 0);
	else 
	    (VOID) psf_error(E_PS0A05_BADMEMREQ, 
                &sess_cb->pss_qsf_rcb.qsf_error, PSF_INTERR, &err_code, 
                err_blk, 0);
	...
	DB_ERROR	   *err_blk)
{
    ...
    */
    if (size <= 0)
    {
	psf_error(E_PS0A05_BADMEMREQ, (DB_ERROR *)NULL, PSF_INTERR, &err_code, 
                  err_blk, 0);
	*(void **) result = NULL;
	return (E_DB_SEVERE);
    }
	...
	DB_ERROR	    *err_blk)
{
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0A06_CANTFREE, &sess_cb->pss_qsf_rcb.qsf_error, 
	...
	DB_ERROR	   *err_blk)
{
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0A05_BADMEMREQ, &sess_cb->pss_qsf_rcb.qsf_error, 
	    PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
	...
	DB_ERROR	*err_blk)
{
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0A07_CANTCHANGE, &sess_cb->pss_qsf_rcb.qsf_error, 
	    PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
	...
	DB_ERROR	*err_blk)
{
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0A08_CANTLOCK, &sess_cb->pss_qsf_rcb.qsf_error, 
	    PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
	...
	DB_ERROR	*err_blk)
{
    ...
    if (status != E_DB_OK)
    {
	(VOID) psf_error(E_PS0B05_CANT_UNLOCK, &sess_cb->pss_qsf_rcb.qsf_error, 
	    PSF_INTERR, &err_code, err_blk, 0);
	return (status);
    }
	...
	i4	    size)
{
	...
	if (DB_FAILURE_MACRO(status))
	{
	    ...
	    {
		psf_error(2704L, (DB_ERROR *)NULL, PSF_USERERR,
		    &err_code, &psq_cb->psq_error, 0);
	    }
	    return (status);
	}
	...
	DB_ERROR	*err_blk)
{
	...
	if (ulm_rcb.ulm_error.err_code == E_UL0005_NOMEM)
	{
	    _VOID_ psf_error(E_PS0F02_MEMORY_FULL, (DB_ERROR *)NULL, 
                             PSF_CALLERR, &err_code, err_blk, 0);
	}
	else
	{
	    _VOID_ psf_error(E_PS0A02_BADALLOC, (DB_ERROR *)NULL, PSF_INTERR, 
                             &err_code, err_blk, 0);
	}
