	...
	PSS_SESBLK	   *sess_cb)
{
	...
	{ 
		(VOID) psf_error(E_SX002B_AUDIT_NOT_ACTIVE, (DB_ERROR *)NULL, 
			PSF_USERERR, 
	...
	*/
	...
	if (psy_cb->psy_autype == DU_SLVL)  
	{
		...
		else
		{
		    ...
		    (VOID) psf_error(E_PS0D48_INVALID_AUDIT_OP, 
    ...
    DB_ERROR    *err)
{
    ...
    */
    (VOID) psf_error(E_PS1005_SEC_WRITE_ERROR, &sxfrcb.sxf_error, PSF_INTERR, 
                     &local_error, err, 0);
    return (status);
}

...
)
{
	...
	*/
	if(status!=E_DB_OK)
	{
	    ...
	    */
	    (VOID) psf_error( sxfrcb.sxf_error.err_code, (DB_ERROR *)NULL, 
		PSF_USERERR, 
		&local_error, &psy_cb->psy_error, 0);
	    break;
	}
