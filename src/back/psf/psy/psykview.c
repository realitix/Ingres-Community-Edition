...
**	13-may-2009 (dougi)
**	    Issue QEU_X100DBU call for X100 tables.
**	14-Oct-2010 (kschendel) SIR 124544
**	    Make sure DMU_CB is zeroed, drop dmu-char-array.
**	03-Aug-2011 (kiria01) b125530
**	    Fix initialised status that can break createdb
**	16-Jun-2012 (kschendel) SIR 126676
**	    Implement explicit restrict/cascade on the drop.
*/
DB_STATUS
psy_kview(
	PSY_CB          *psy_cb,
	...
	QEU_CB		*qeu_cb)
{
    ...
    */
	...
	if (sess_cb->pss_distrib & DB_3_DDB_SESS)
	{
		...
		if (next_tbl->pss_delim_tbl)
		{
		    ...
		    {
		        (VOID) psf_error(psy_cb->psy_error.err_code, 
	...
	*/
	...
	/* Set restrict vs cascade in the DMU CB */
	BTset(DMU_CASCADE, dmu_cb.dmu_chars.dmu_indicators);
	if (BTtest(i, psy_cb->psy_restrict))
	    BTclear(DMU_CASCADE, dmu_cb.dmu_chars.dmu_indicators);
	
	/* Check for X100 table. */
	if (BTtest(i, psy_cb->psy_x100tab))
	{
	    dmu_cb.dmu_table_name = psy_cb->psy_tabname[i];

	    /* DROP INDEX on X100 is not allowed (yet). */
	    if (dmu_cb.dmu_tbl_id.db_tab_index != 0)
	    {
		err_code = E_PS04F1_X100_DROPIX;
	...
	status = qef_call(opcode, ( PTR ) qeu_cb);
	...
	if (DB_FAILURE_MACRO(status))
	{
		...
		case E_QE0400_GATEWAY_ERROR:
		    ...
		    (VOID) psf_error(err_code, (DB_ERROR *)NULL, PSF_USERERR, 
                        &err_code, &psy_cb->psy_error, 1,
		       psf_trmwhite(sizeof(psy_cb->psy_tabname[i]), 
			    (char *) &psy_cb->psy_tabname[i]),
			&psy_cb->psy_tabname[i]);
