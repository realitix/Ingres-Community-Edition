...
**	    Tidied pst_node call & fixed missing tid sizing.
...
**	03-Feb-2011 (kiria01) SIR 124690
**	    Corrected length mismatch on qdata result and
**	    initialised whole of qsf_rb.
*/
DB_STATUS
psy_dinteg(
	PSY_CB             *psy_cb,
	...
	QEU_CB		   *qeu_cb)
{
    ...
    */
    ...
    if (status = pst_node(sess_cb, &mstream, tidnode, NULL,
		PST_AOP, (PTR)&operator, sizeof(operator), 0, 0, 0, NULL,
		&aopnode, &psy_cb->psy_error, 0))
    {
	delinteg++;
	goto exit;
    }
    ...
    if (status = pst_node(sess_cb, &mstream, aopnode, qualnode,
		PST_AGHEAD, (PTR)&aghead, sizeof(aghead), 0, 0, 0, NULL,
		&agheadnode1, &psy_cb->psy_error, 0))
    {
	delinteg++;
	goto exit;
    }
    ...
    */
    ...
    if (status = pst_node(sess_cb, &mstream, aopnode, qlendnode,
		PST_AGHEAD, (PTR) &aghead, sizeof(aghead), 0, 0, 0, NULL,
		&agheadnode2, &psy_cb->psy_error, 0))
    {
	delinteg++;
	goto exit;
    }
    ...
    if (status = pst_node(sess_cb, &mstream, agheadnode2, agheadnode1,
		PST_BOP, (PTR)&operator, sizeof(operator), 0, 0, 0, NULL,
		&bopnode, &psy_cb->psy_error, 0))
    {
	delinteg++;
	goto exit;
    }
    ...
    if (status = pst_node(sess_cb, &mstream, qtree->pst_qtree->pst_left, bopnode,
		PST_RESDOM, (PTR) &resdom, sizeof(resdom),
		DB_INT_TYPE, 0, sizeof(qdata), NULL,
		&resdomnode, &psy_cb->psy_error, 0))
    {
	delinteg++;
	goto exit;
    }
    ...
    */
    ...
    if (DB_FAILURE_MACRO(status))
    {
	(VOID) psf_error(E_PS0D1C_QSF_UNLOCK, &qsf_rb.qsf_error,
	    PSF_INTERR, &err_code, &psy_cb->psy_error, 0);
	delinteg++;
	goto exit;
    }
    ...
    qresult.dt_size = sizeof(qdata);	    /* It contains an i8 */
	...
	for (;qeu_cb->error.err_code != E_QE0015_NO_MORE_ROWS;)
	{
		...
		switch (qeu_cb->error.err_code)
		{
		    ...
		    case E_QE0022_QUERY_ABORTED:
			    {
				(VOID) psf_error(4708L, (DB_ERROR *)NULL,
    ...
    */
    if (qdata != 0)
    {
	(VOID) psf_error(3492L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
	    &psy_cb->psy_error, 1,
	    psf_trmwhite(sizeof(psy_cb->psy_tabname[0]), 
		(char *) &psy_cb->psy_tabname[0]),
	    &psy_cb->psy_tabname[0]);
...
exit:
	...
	if (tree_lock == 0)
	{
	    ...
	    {
		(VOID) psf_error(E_PS0D18_QSF_LOCK, &qsf_rb.qsf_error,
		    PSF_INTERR, &err_code, &psy_cb->psy_error, 0);
		if (stat > status)
		    status = stat;
	    }
	...
	if (DB_FAILURE_MACRO(stat))
	{
	    (VOID) psf_error(E_PS0D1A_QSF_DESTROY, &qsf_rb.qsf_error,
		PSF_INTERR, &err_code, &psy_cb->psy_error, 0);
	    if (stat > status)
		status = stat;
	}
