/*
**Copyright (c) 2007, 2010 Actian Corporation
*/
	...
	case PST_VAR:
	    ...
	    */
		...
		if (v == (PST_QNODE *)NULL)
		{
		    ...
		    if (vmode == PSQ_VIEW)
		    {
			psf_error(E_PS0D03_ATT_NOT_FOUND, (DB_ERROR *)NULL, 
			...
			*/
			...
			{
			    psf_error(6319, (DB_ERROR *)NULL, PSF_USERERR, 
		    ...
		    */
			...
			if (status = pst_node(cb, dup_rb->pss_mstream, NULL, NULL,
				PST_CURVAL, (PTR)&curval, sizeof(curval),
				t->pst_sym.pst_dataval.db_datatype,
				t->pst_sym.pst_dataval.db_prec,
				t->pst_sym.pst_dataval.db_length,
				(DB_ANYTYPE *)t->pst_sym.pst_dataval.db_data,
				&v, dup_rb->pss_err_blk, 0))
			{
			    proot = NULL; /* Exiting to return error */
			    break;
			}
			v->pst_sym.pst_dataval.db_collID =
				t->pst_sym.pst_dataval.db_collID;
		    }
		}
		else
		{
	...
	case PST_RESDOM:
		...
		*/
		if (resvar && vn == resvar->pss_rgno)
		{
		    ...
		    {			
			(VOID) psf_error(E_PS0D02_BAD_TID_NODE, 
                            (DB_ERROR *)NULL, PSF_INTERR, &err_code,
