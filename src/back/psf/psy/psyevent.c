	...
	DB_ERROR	*err_blk)
{
	...
	*/
	...
	if (rdf_ev.rdf_error.err_code == E_RD0013_NO_TUPLE_FOUND)
	{
	    ...
	    {
		_VOID_ psf_error(E_US088F_2191_INACCESSIBLE_EVENT, 
	    ...
	    else
	    {
	        ...
	        _VOID_ psf_error(E_PS0D3B_DBEVENT_NOT_FOUND, (DB_ERROR *)NULL, 
    ...
    */
    ...
    if (status != E_DB_OK)
    {
	_VOID_ psf_error(E_PS0D13_SCU_INFO_ERR, &scf_cb.scf_error,
...
saw_the_perms:
    ...
    */
	...
	if (qmode == PSQ_GRANT)
	{
		...
		_VOID_ psf_error(E_PS0563_NOPRIV_ON_GRANT_EV, (DB_ERROR *)NULL,
                    PSF_USERERR, &err_code, err_blk, 2,
		    psf_trmwhite(sizeof(alert_name->dba_alert), 
			(char *) &alert_name->dba_alert),
		    &alert_name->dba_alert,
		 ...
		 && sess_cb->pss_dbp_flags & PSS_0DBPGRANT_CHECK)
	{
	    ...
	    */
	    ...
	    _VOID_ psf_error(E_PS0557_DBPGRANT_LACK_EVPRIV, (DB_ERROR *)NULL,
	...
	else
	{
	    ...
	    _VOID_ psf_error(E_PS0D3C_MISSING_DBEVENT_PRIV, (DB_ERROR *)NULL,
                PSF_USERERR, &err_code, err_blk, 4, 
		STlength(op), op, 
		STlength(buf), buf,
		psf_trmwhite(sizeof(alert_name->dba_alert), 
