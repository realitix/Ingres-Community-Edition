	...
	PSS_SESBLK	   *sess_cb)
{
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	{
		...
		{
		    (VOID) psf_error(E_PS0905_DBP_NOTFOUND, (DB_ERROR *)NULL, 
                        PSF_USERERR, &err_code, &psy_cb->psy_error, 1,
		        psf_trmwhite(sizeof(psy_cb->psy_tabname[0]),
			    (char *) &psy_cb->psy_tabname[0]),
		        &psy_cb->psy_tabname[0]);
		...
		case E_RD0210_ABANDONED_OBJECTS:
		{
		    ...
		    (VOID) psf_error(E_PS0564_ALLPROT_ABANDONED_OBJ, 
                        (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                        &psy_cb->psy_error, 2, type_len, obj_type,
			psf_trmwhite(sizeof(psy_cb->psy_tabname[0]),
	...
	*/
	...
	for (i = 0; i < psy_cb->psy_numnms; i++)
	{
		...
		switch (rdf_cb.rdf_error.err_code)
		{
		    case E_RD0201_PROC_NOT_FOUND:
		    {
			(VOID) psf_error(E_PS0905_DBP_NOTFOUND, 
			...
			(VOID) psf_error(E_PS0565_PROT_ABANDONED_OBJ, 
                            (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                            &psy_cb->psy_error, 3,
		    ...
		    {
			(VOID) psf_error((i4) 5204, (DB_ERROR *)NULL, 
	...
	*/
	...
	for (i = 0; i < psy_cb->psy_numnms; i++)
	{
	    if (cant_drop_perms)
	    {
		(VOID) psf_error(5204L, (DB_ERROR *)NULL, PSF_USERERR,
		    &err_code, &psy_cb->psy_error, 1,
		...
		switch (rdf_cb.rdf_error.err_code)
		{
			...
			if (psy_cb->psy_grant == PSY_SDROP)
			    ...
			    msgid = (i4)5204;
			(VOID) psf_error(msgid, (DB_ERROR *)NULL, PSF_USERERR,
					 &err_code, &psy_cb->psy_error, 1,
					 sizeof (rdf_rb->rdr_qrymod_id),
					 &rdf_rb->rdr_qrymod_id);
			continue;
