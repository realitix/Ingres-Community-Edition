...
exit:
    ...
    */
    ...
    {
	(VOID) psf_error(E_PS0A08_CANTLOCK, &qsf_rb.qsf_error,
	...
	i4		*ret_flags)
{
    ...
    */
    ...
    qsf_rb->qsf_feobj_id.qso_lname = 0;
    ...
    */
	...
	{
	    (VOID) psf_error(qerr, &qsf_rb->qsf_error,
		PSF_CALLERR, &err_code, &psq_cb->psq_error, 0);
	}
	else
	{
	    (VOID) psf_error(E_PS0A05_BADMEMREQ, &qsf_rb->qsf_error,
	...
	*p = '\0';			/* 2nd char past end */
    ...
    if (DB_FAILURE_MACRO(status))
    {
	(VOID) psf_error(E_PS0B05_CANT_UNLOCK, &qsf_rb->qsf_error,
		PSF_INTERR, &err_code, &psq_cb->psq_error, 0);
	goto cleanup1;
    }
