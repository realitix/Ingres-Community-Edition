	...
	PSS_SESBLK *sess_cb)
{
	...
	if (rdf_cb.rdf_error.err_code == E_RD0143_CREATE_SYNONYM)
	{
	    ...
	    (VOID) psf_error(E_PS0454_CREATE_SYN_ERROR, (DB_ERROR *)NULL, 
	...
	PSS_SESBLK *sess_cb)
{
	...
	if (DB_FAILURE_MACRO(status))
	{
	    ...
	    {
		(VOID) psf_error(E_PS0456_DROP_SYN_ERROR, (DB_ERROR *)NULL, 
                    PSF_USERERR, &err_code, &psy_cb->psy_error, 1,
		...
		*/
		...
		(VOID) psf_error(2753L, (DB_ERROR *)NULL, PSF_USERERR, 
