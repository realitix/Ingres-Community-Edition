	...
	PSS_SESBLK	   *sess_cb)
{
	...
	if (sensitive)
	{
		...
		(VOID) psf_error(E_US18D5_6357_FORM_NOT_AUTH, (DB_ERROR *)NULL,
		...
		(VOID) psf_error(E_US18D3_6355_NOT_AUTH, (DB_ERROR *)NULL,
			     PSF_USERERR, &err_code, &psy_cb->psy_error, 1,
	...
	if (status != E_DB_OK)
	{
	    ...
	    (VOID) psf_error(E_PS0D13_SCU_INFO_ERR, (DB_ERROR *)NULL,
	...
	if (MEcmp((PTR)dbname, (PTR)DB_DBDB_NAME, sizeof(dbname)))
	{
	    ...
	    (VOID) psf_error(E_US18D4_6356_NOT_DBDB, (DB_ERROR *)NULL,
			     PSF_USERERR, &err_code, &psy_cb->psy_error, 1,
	...
	*/
                           ...
                           (i4)sizeof(psy_cb->psy_apass)) != NULL)
	{
	    err_code=E_US18E7_6375;
            (VOID) psf_error(E_US18E7_6375, (DB_ERROR *)NULL,
                             PSF_USERERR, &err_code, &psy_cb->psy_error, 1,
	...
	default:
	    ...
	    (VOID) psf_error(E_PS0D46_INVALID_USER_OP, (DB_ERROR *)NULL,
		    PSF_INTERR, &err_code, &psy_cb->psy_error, 0);
	    break;
    }
	...
	PSS_SESBLK	   *sess_cb)
{
	  ...
	  */
	  {
	    ...
	    (VOID) psf_error( E_US1968_6504_UNOPRIV_WDEFAULT, (DB_ERROR *)NULL,
    ...
    */
	...
	/* Encrypt the password if nonblank and not already hex value */
		...
		{
		    err_code = scf_cb.scf_error.err_code;
		    (VOID) psf_error(E_PS0D43_XENCODE_ERROR, (DB_ERROR *)NULL,
			PSF_INTERR, &err_code, &psy_cb->psy_error, 0);
		    status = (status > E_DB_SEVERE) ? status : E_DB_SEVERE;
		    return(status);
		}
