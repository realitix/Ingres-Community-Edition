	...
	DB_ERROR	    *err_blk)
{
    ...
    {
	psf_error(E_PS0901_REDESC_RNG, (DB_ERROR *)NULL, PSF_INTERR, 
                  &err_code, err_blk, 0);
	return (E_DB_SEVERE);
    }
	...
	*/
	if (rdf_cb.rdf_error.err_code == E_RD0002_UNKNOWN_TBL)
	{
	    ...
	    (VOID) psf_error(E_PS0903_TAB_NOTFOUND, (DB_ERROR *)NULL, 
                PSF_CALLERR, &err_code, err_blk, 1,
	...
	else if (rdf_cb.rdf_error.err_code == E_RD0254_DD_NOTABLE)
	{
	    ...
	    (VOID) psf_error(E_PS0919_BAD_CATALOG_ENTRY, (DB_ERROR *)NULL,
	...
	else if (rdf_cb.rdf_error.err_code == E_RD0268_LOCALTABLE_NOTFOUND)
	{
	    if (showtype & PST_SHWID)
	    {
		(VOID) psf_error(E_PS0915_MISSING_LDB_TBL, (DB_ERROR *)NULL,
	    ...
	    {
		(VOID) psf_error(E_PS0912_MISSING_LDB_TBL, (DB_ERROR *)NULL, 
	...
	{
	     (VOID) psf_error(E_PS0913_SCHEMA_MISMATCH, (DB_ERROR *)NULL,
		 (showtype & PST_SHWID) ? PSF_CALLERR : PSF_USERERR,
		 &err_code, err_blk, 1,
		 psf_trmwhite(sizeof(*tabname), (char *) tabname), tabname);
	      return (E_DB_ERROR);
	...
	*/
    ...
    /* Check for X100 table type. */
    if (rngentry->pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX ||
	(rngentry->pss_tabdesc->tbl_2_status_mask & DMT_X100_INVIEW))
    {
	if (rngentry->pss_tabdesc->tbl_id.db_tab_index > 0 &&
	    (query_mode == PSQ_RETRIEVE || query_mode == PSQ_RETINTO ||
	    query_mode == PSQ_APPEND || query_mode == PSQ_REPLACE ||
	    query_mode == PSQ_DELETE))
	{
	    (VOID) psf_error(9485L, 0L, PSF_USERERR,
		&err_code, err_blk, 1,
		psf_trmwhite(sizeof(*tabname), (char *) tabname), tabname);
	    return(E_DB_ERROR);
	}

	/* Check for open cursor on x100 table, we check again in pst_header */
	if (sess_cb->pss_numcursors && psq_any_vwtab_cursor(sess_cb, err_blk) )
	{
	    if (query_mode != PSQ_REPCURS && query_mode != PSQ_DELCURS &&
		query_mode != PSQ_REPLACE && query_mode != PSQ_DELETE)
	    {
	...
	sess_cb->pss_stmt_flags2 |= PSS_2_X100TAB_INQ;
    }
...
**	    PSS_DBP_IF_EXISTS	If not found, don't raise error.
**		
	...
	i4		*ret_flags)
{
		...
		*/
		...
		else
		{
		    ...
		    *ret_flags |= PSS_MISSING_DBPROC;
		    ...
		    if (dbp_mask & PSS_DBP_BY_ID)
		    {
			(VOID) psf_error(E_PS055F_DBP_NOT_FOUND,  
			...
			*/
		    ...
		    /* Don't raise user error if IF EXISTS active */
		    else if (~dbp_mask & PSS_DBP_IF_EXISTS)
		    {
			_VOID_ psf_error(2405L, (DB_ERROR *)NULL, PSF_USERERR, 
                            &err_code, err_blk, 1,
			    psf_trmwhite(sizeof(DB_DBP_NAME), (char *) dbpname),
			    (char *) dbpname);
		    }
	...
	*ret_flags |= PSS_DUPL_DBPNAME;
	_VOID_ psf_error(2400L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
            err_blk, 1, psf_trmwhite(sizeof(DB_DBP_NAME), (char *) dbpname),
	    (char *) dbpname);
	...
	DB_ERROR		*err_blk)
{
    ...
    */
	...
	else
	{
		...
		*/
		psf_error(E_PS091D_PROC_MAPPED_COLS, (DB_ERROR *)NULL, 
	...
	*/
    ...
    {
	psf_error(E_PS0917_CANNOT_CONNECT, (DB_ERROR *)NULL, PSF_USERERR, 
                &err_code, err_blk, 3, 
                psf_trmwhite(sizeof(DD_NODE_NAME), ldb_desc->dd_l2_node_name),
    		ldb_desc->dd_l2_node_name,
    		psf_trmwhite(sizeof(DD_256C), ldb_desc->dd_l3_ldb_name),
    ...
    */
    ...
    {
	psf_error(E_PS091E_INCOMPAT_CASE_MAPPING, (DB_ERROR *)NULL, 
                PSF_USERERR, &err_code, err_blk, 1, 
		psf_trmwhite(sizeof(DD_TAB_NAME), ldb_tab_info->dd_t1_tab_name),
		ldb_tab_info->dd_t1_tab_name);
	return (E_DB_ERROR);
	...
	switch (rdf_cb.rdf_error.err_code)
	{
		...
		*/
		...
		{
		    /* no ldb procedure */
		    psf_error(E_PS1203_NO_LDB_PROC, (DB_ERROR *)NULL, 
		...
		{
		    psf_error(E_PS090D_NO_LDB_TABLE, (DB_ERROR *)NULL, 
                        PSF_USERERR, &err_code, err_blk, 4,
		    	psf_trmwhite(sizeof(DD_TAB_NAME), ldb_tab_info->dd_t1_tab_name),
		    	ldb_tab_info->dd_t1_tab_name,
		    	psf_trmwhite(sizeof(DD_NODE_NAME), ldb_desc->dd_l2_node_name),
		...
		*/
		psf_error(E_PS090E_COL_MISMATCH, (DB_ERROR *)NULL, PSF_USERERR,
                    &err_code, err_blk,0);
		break;
	    }
	    ...
	    {
		psf_error(E_PS0911_BAD_LDB_USER, (DB_ERROR *)NULL, PSF_USERERR,
                    &err_code, err_blk,2,
		    psf_trmwhite(sizeof(DD_NODE_NAME), ldb_desc->dd_l2_node_name),
		    ldb_desc->dd_l2_node_name,
		    psf_trmwhite(sizeof(DD_256C), ldb_desc->dd_l3_ldb_name),
    ...
    if (duplicate_table)
    {
	psf_error(E_PS0910_DUP_LDB_TABLE, (DB_ERROR *)NULL, PSF_USERERR, 
                &err_code, err_blk, 3,
	    	psf_trmwhite(sizeof(DD_TAB_NAME), ldb_tab_info->dd_t1_tab_name),
	    	ldb_tab_info->dd_t1_tab_name,
	    	psf_trmwhite(sizeof(DD_NODE_NAME), ldb_desc->dd_l2_node_name),
