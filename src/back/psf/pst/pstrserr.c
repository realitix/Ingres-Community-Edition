	...
	DB_ERROR	*err_blk)
{
    ...
    if (STcompare(opname, "sum") == 0 &&
	opnode->pst_sym.pst_value.pst_s_op.pst_flags & PST_AVG_AOP)
	STcopy("avg", opname);	/* it's really "avg" */
    
    if (STcompare(opname, "ifnull") == 0)
    	STcopy("ifnull/nvl", opname); /* could be ifnull or nvl */
	...
	{
	    (VOID) psf_error(E_PS0C04_BAD_TREE, &adf_scb->adf_errcb.ad_dberror,
    ...
    */
	...
	else if (STcompare(opname, "nvl2") == 0)
	{
	    /* NVL2 compares op2 and op3, report those as the types */
	    lefttype = opnode->pst_right->pst_left->pst_sym.pst_dataval.db_datatype;
	    righttype = 
	        opnode->pst_right->pst_right->pst_left->pst_sym.pst_dataval.db_datatype;
	}
	else
	{
	    lefttype = opnode->pst_left->pst_sym.pst_dataval.db_datatype;
	    righttype = opnode->pst_right->pst_left->pst_sym.pst_dataval.db_datatype;
	}
    }
    if (children >= 1)
    {
    ...
    (VOID) psf_error(err_blk->err_code, (DB_ERROR *)NULL, PSF_USERERR, 
        &err_code, err_blk, 4, sizeof(lineno), &lineno, 
	psf_trmwhite(ADF_MAXNAME, opname), opname, 
