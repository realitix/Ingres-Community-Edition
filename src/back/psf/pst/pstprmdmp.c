...
**	    Opened out the DOT tracing to allow OPF to trace its
**	    structures too.
**	17-Dec-2010 (kschendel) SIR 124685
**	    Replace ADI_OP_NAME with char array.
**	30-Jan-2011 (kiria01) SIR 124690
**	    Add missing collID display
**	14-Feb-2011 (kiria01) b124821
**	    Make diag code more defensive against segvs & avoid multiple
**	    traces of same structure.
**	14-Apr-2011 (kiria01)
**	    Display further data structures especially bitmaps and
**	    safe linkages between structures.
...
void pst_qtree_dot(
	...
	PSF_SESSCB_PTR cb,
	PST_QNODE *node);
static void pst_relations_dot1(
	PST_DOT_CTX *ctx,
	FILE *fd,
	PST_QNODE *node);
...
/* Range table types */
static const char *pst_rgtype_names[] = {
	...
	i4		    max_str_len)
{
    ...
    {
	MEmove(DB_TYPE_MAXLEN, opname, ' ', max_str_len - 2, buf + 1);
    }
    adf_scb->adf_errcb.ad_errmsgp = ad_errmsgp;
	...
	char               *buf)
{
    ...
    {
	MEcopy(opname, DB_TYPE_MAXLEN, buf);
    }
	...
	PST_QTREE          *header)
{
	...
	TRdisplay("\n\t\t[%d]\n", i);
	TRdisplay("\t\t\tpst_rgtype: %s (%d)\n",
		pst_rgtype_names[header->pst_rangetab[i]->pst_rgtype],
		header->pst_rangetab[i]->pst_rgtype);
	if (header->pst_rangetab[i]->pst_rgtype == PST_UNUSED)
	    continue;
...
**          Dump fe_dbpid as well as pst_dbpid.
*/
VOID
pst_dbpdump(
	PST_PROCEDURE	*pnode,
	i4		whichlink)
{
    DB_CURSOR_ID *cur = (DB_CURSOR_ID *)pnode->pst_fe_dbpid;
    ...
    */
    ...
    (VOID) pst_display("\tdbpid:%d:%d:%t\n",
        ...
        pnode->pst_dbpid.db_cursor_id[1],
        sizeof(pnode->pst_dbpid.db_cur_name),
        pnode->pst_dbpid.db_cur_name);
    (VOID) pst_display("\tfe_dbpid:\t%d:%d:%d:%t\n", pnode->pst_fe_lname,
        cur->db_cursor_id[0], cur->db_cursor_id[1],
        pnode->pst_fe_lname, cur->db_cur_name);
    if ((pnode->pst_parms) &&
	...
	{
	    (VOID) pst_display("\t%d:len:%d  prec:%x  collID:%d  type:%d  data:%x\n",
		i+first, (i4)dbvalue->db_length, (i4)dbvalue->db_prec, (i4)dbvalue->db_collID,
	    ...
	    )
	{
		   ...
		   )
		{
		    ...
		    {
			(VOID) pst_display(
			    "\t%d:len:%d   prec:%x  collID:%d  type:%d  data:%x\n",
			    i+first, (i4)dbvalue->db_length,
			    (i4)dbvalue->db_prec,
			    (i4)dbvalue->db_collID,
...
#define LINKBLOCK(p,f,r,x) if(p->f && pst_dot_chk(ctx, &p->f, p->f)){\
    SIfprintf(fd,"n%p->n%p["x"label="#f"]\n",p,p->f);\
...
i4
pst_ptr_ok(FILE *fd, void *p)
{
    char ulmsts[64];
    if (!ulm_safe_deref(p, NULL, ulmsts))
	return TRUE;
    SIfprintf(fd, "n%p[fillcolor=red,label=\"BAD ptr\\n%p\\n%s\"]\n",
		p, p, ulmsts);
    return FALSE;
}

/*{
** Name: pst_dot_qnode - DOT format dump
**
** Description:
	...
	PST_RSDM_NODE *r)
{
    ...
    STtrmwhite(t);
    SIfprintf(fd, "%s|{r%d|t%d|u%d}|{0x%x}",
	    t, r->pst_rsno, r->pst_ntargno,
    ...
    )
{
	...
	SIfprintf(fd, "n%p[%sfillcolor=beige,label=\"{%s",
		    node,
		    simple?"shape=Mrecord,":"",
	...
	case PST_MOP:
	    {
		char adi_oname[ADF_MAXNAME];
		char t1[20], t2[20];
		const char *pfx = "";
		i4 dist = v->pst_s_op.pst_distinct-PST_BDISTINCT;
		if ((u_i4)dist > PST_DISTINCT-PST_BDISTINCT)
		    dist = PST_DISTINCT-PST_BDISTINCT+1;
		adi_opname(&ctx->adfcb, v->pst_s_op.pst_opno, adi_oname);
		if (adi_oname[0] == '<' && adi_oname[1] == '>')
		    pfx = "\\<\\>";
		else if (adi_oname[0] == '<' || adi_oname[0] == '>')
	...
	case PST_CONST:
		...
		abs(node->pst_sym.pst_dataval.db_datatype) == DB_PAT_TYPE)
	    {
		char buf[40];
		u_i2 *pc = (u_i2 *)node->pst_sym.pst_dataval.db_data, *e;
		u_i2 i, n = pc[1];
		bool uni = ((pc[3] & 6) != 0);
		SIfprintf(fd, "|<p> %d %d %x}\"]\n"
				"n%p:p->p%p[style=dotted]\n"
				"p%p[shape=record,style=dotted,label=\"{",
			pc[0],pc[1],pc[2]+(pc[3]<<16),
			node, node, node);
		pc += 4;
		for (i = 0; i < n; i++)
		{
		    char lf = 'l';
		    SIfprintf(fd, "{alt %d|%d}|", i, *pc);
		    e = pc + *pc; pc++;
		    while (pc < e)
		    {
			i8 v = 0;
			char *p = buf, *bufp = buf, *jmp;
			bool tgt = FALSE;
			SIfprintf(fd, "%04x", (PTR)pc -
				node->pst_sym.pst_dataval.db_data);
			while (adu_pat_disass(buf, sizeof(buf), (u_i1**)&pc, &p, &v, uni)){
			    *p = '\0';
			    SIfprintf(fd, "%s\\%c", buf, lf);
			    p = buf;
			    lf = 'r';
			}
			*p = EOS;
			...
			if (*bufp == '#')
			{
			    ...
			    bufp = strchr(bufp, '\n');
			    *bufp++ = EOS; /* Leave bufp as new start */
			    tgt = TRUE;
			}
			/* Jump addr hint present in what's left? */
			if (jmp = strchr(bufp, '#'))
			{
			    /* Scan backwards to collect address */
			    jmp = p;
			    while (*--jmp != '#')
			    {
				if (!('0' <= *jmp && *jmp <= '9' ||
			...
			if (!STcmp(bufp, "PAT_"))
			    bufp += 4;
			SIfprintf(fd, "%c%s\\%c", tgt?'@':' ', bufp, lf);
			if (jmp)
			{
			    PTR pc2;
			    CVaxptr(jmp, &pc2);
			    SIfprintf(fd, "#%04x\\l", pc2 -
					node->pst_sym.pst_dataval.db_data);
			}
			lf = 'l';
		    }
		...
		if (adc_tmlen(&ctx->adfcb, datatype, 
			&default_width, &worst_width) == E_AD0000_OK)
		{
		    ...
		    if (adc_tmcvt(&ctx->adfcb, datatype, &dest_dt, &outlen)
			== E_AD0000_OK)
		    {
			if (worst_width > sizeof(tempbuf) &&
	...
	case PST_VAR:
		...
		if (ctx->tree)
		{
		    if ((u_i4)v->pst_s_var.pst_vno < (u_i4)ctx->tree->pst_rngvar_count)
		    {
			r = ctx->tree->pst_rangetab[v->pst_s_var.pst_vno];
		...
		{   
		    ...
		    {
			rngvar = &ctx->cb->pss_auxrng.pss_rngtab[i];
		...
		STtrmwhite(t);
		SIfprintf(fd, "|%d}|%s", v->pst_s_var.pst_atno.db_att_id, t);
		if (node->pst_sym.pst_type==PST_FWDVAR)
		    SIfprintf(fd, "|{dep=%d|%s}}\",fillcolor=whitesmoke]\n",
			    v->pst_s_fwdvar.pst_qualdepth_ref,
			    v->pst_s_fwdvar.pst_star_seen?"*":"");
		else
		    SIfprintf(fd, "}\",fillcolor=%s]\n",
			    itvrm&&BTtest(v->pst_s_var.pst_vno, itvrm)
			    ?"yellowgreen"
			    :otvrm&&BTtest(v->pst_s_var.pst_vno, otvrm)
			    ?"grey"
			    :itvrm&&otvrm
			    ?"orangered"
			    :"white");
	...
	case PST_AGHEAD:
	    ...
	    {	char bm[64];
		fmt_bm(&v->pst_s_root.pst_tvrm, sizeof(v->pst_s_root.pst_tvrm), bm);
		SIfprintf(fd, "T:%s\\r", bm);
		fmt_bm(&v->pst_s_root.pst_lvrm, sizeof(v->pst_s_root.pst_lvrm), bm);
		SIfprintf(fd, "L:%s\\r", bm);
		fmt_bm(&v->pst_s_root.pst_rvrm, sizeof(v->pst_s_root.pst_rvrm), bm);
		SIfprintf(fd, "R:%s\\r", bm);
	    }
	    if (node->pst_sym.pst_type != PST_AGHEAD &&
			v->pst_s_root.pst_union.pst_next &&
			pst_dot_chk(ctx, node, v->pst_s_root.pst_union.pst_next))
	    {
		...
		SIfprintf(fd, "n%p:e->n%p:w[weight=100,label=%s]\n",
		    node, v->pst_s_root.pst_union.pst_next, utype);
		if (pst_ptr_ok(fd, v->pst_s_root.pst_union.pst_next))
		{
		    pst_dot_qnode1(ctx, fd, v->pst_s_root.pst_union.pst_next,
				itvrm, &v->pst_s_root.pst_tvrm);
		}
	    }
	    else
	    {
		...
		if (node->pst_sym.pst_type == PST_AGHEAD &&
		    v->pst_s_root.pst_union.pst_next &&
			pst_ptr_ok(fd, v->pst_s_root.pst_union.pst_next))
		    SIfprintf(fd, "n%p->n%p[weight=1,label=\"common\\nwhere\"]\n",
	...
	case PST_CURVAL:
	    {
		char t[sizeof(v->pst_s_curval.pst_cursor)+1];
		MEcopy((PTR)&v->pst_s_curval.pst_cursor,
			sizeof(v->pst_s_curval.pst_cursor),t);
		t[sizeof(v->pst_s_curval.pst_cursor)] = 0;
		STtrmwhite(t);
 		SIfprintf(fd, "{%s|%d}}\"]\n", t,
			v->pst_s_curval.pst_curcol);
	    }
	    break;
	case PST_RULEVAR:
	    SIfprintf(fd, "{%d|%d|%d}}\"]\n",
	...
	case PST_TAB:
	    {
		char t[sizeof(v->pst_s_tab.pst_tabname)+1];
/*		v->pst_s_tab.pst_owner
*/
		MEcopy((PTR)&v->pst_s_tab.pst_tabname,
			sizeof(v->pst_s_tab.pst_tabname), t);
		t[sizeof(v->pst_s_tab.pst_tabname)] = 0;
		STtrmwhite(t);
		SIfprintf(fd, "%s}\"]\n", t);
	    }
	    break;
	case PST_SEQOP:
	    {
		char t[sizeof(v->pst_s_seqop.pst_seqname)+1];
/*		v->pst_s_seqop.pst_owner
*/
		MEcopy((PTR)&v->pst_s_seqop.pst_seqname,
			sizeof(v->pst_s_seqop.pst_seqname), t);
		t[sizeof(v->pst_s_seqop.pst_seqname)] = 0;
		STtrmwhite(t);
		SIfprintf(fd, "{%s}|{%d|%s}}\"]\n", t,
			v->pst_s_seqop.pst_seqid,
			v->pst_s_seqop.pst_seqflag==PST_SEQOP_NEXT
			?"NEXT":
			v->pst_s_seqop.pst_seqflag==PST_SEQOP_CURR
			?"CURR":"?");
	    }
	    break;
	case PST_GBCR:
	    SIfprintf(fd, "%s}\"]\n",
		v->pst_s_group.pst_groupmask==PST_GROUP_ROLLUP
		?"ROLLUP":
		v->pst_s_group.pst_groupmask==PST_GROUP_CUBE
		?"CUBE":"?");
	    break;
	...
	if (node->pst_left && pst_dot_chk(ctx, node, node->pst_left))
	{
	    SIfprintf(fd, "n%p:sw->n%p:n[weight=100,label=left]\n",
		node, node->pst_left);
	    if (node->pst_right && pst_dot_chk(ctx, node, node->pst_right))
	    {
		SIfprintf(fd, "n%p:se->n%p:n[weight=100,label=right]\n",
		    node, node->pst_right);
		if (pst_ptr_ok(fd, node->pst_right))
		    pst_dot_qnode1(ctx, fd, node->pst_right, otvrm, itvrm);
	    }
	    if (!pst_ptr_ok(fd, node->pst_left))
		break;
	    node = node->pst_left;
	}
	else if (node->pst_right && pst_dot_chk(ctx, node, node->pst_right))
	{
	    SIfprintf(fd, "n%p:se->n%p:n[weight=100,label=right]\n",
		node, node->pst_right);
	    if (!pst_ptr_ok(fd, node->pst_right))
...
pst_dot_qnode(
    ...
    PST_QNODE *node)
{
    pst_dot_qnode1(ctx, fd, node, NULL, NULL);
}
...
{
    i4 i;
    PST_QTREE *save_tree = ctx->tree;
    ctx->tree = tree;
    SIfprintf(fd, "n%p[style=solid,fillcolor=yellow,label=\"{QTREE|{%s|vsn:%d}|RNGENTRY",
	...
	if (!r)
	{
	    SIfprintf(fd, "|{%d|elided}", i);
	    continue;
	}
	MEcopy((PTR)&r->pst_corr_name, sizeof(cv),cv);
	cv[sizeof(cv)-1] = 0;
	STtrmwhite(cv);
	SIfprintf(fd, "|{<f%d>%d|{%.32s|{%d,%d|%d|%s}|",
		i, i, cv,
		r->pst_rngvar.db_tab_base,
		r->pst_rngvar.db_tab_index,
		r->pst_rgparent,
		pst_rgtype_names[r->pst_rgtype<PST_RGTYPE_MAX
			    ?r->pst_rgtype:PST_RGTYPE_MAX]);
	fmt_bm(&r->pst_inner_rel, sizeof(r->pst_inner_rel), cv);
	SIfprintf(fd, "I:%s\\r", cv);
	fmt_bm(&r->pst_outer_rel, sizeof(r->pst_outer_rel), cv);
	SIfprintf(fd, "O:%s\\r}}", cv);
    }
    ...
    {
	PST_RNGENTRY *r = tree->pst_rangetab[i];
	if (r && r->pst_rgroot && pst_dot_chk(ctx, tree, r->pst_rgroot))
	{
	    SIfprintf(fd, "n%p:(f%d,w)->n%p:n[label=\"rgroot(%d)\"]\n",
			tree, i, r->pst_rgroot,i);
	    switch(r->pst_rgtype)
	    {
	    case PST_RTREE:
	    case PST_DRTREE:
	    case PST_WETREE:
	    case PST_TPROC:
		pst_dot_qnode(ctx, fd, r->pst_rgroot);
		break;
	    default:
		break;
	    }
    ...
    LINKBLOCK(tree, pst_qtree,		pst_dot_qnode, "weight=10,")
    LINKBLOCK(tree, pst_offsetn,	pst_dot_qnode, "weight=10,")
    LINKBLOCK(tree, pst_stlist, 	pst_dot_qnode, "weight=10,")
    LINKBLOCK(tree, pst_firstn,		pst_dot_qnode, "weight=10,")
    ctx->tree = save_tree;
}
...
/*{
** Name: pst_dot_stmt - DOT format statment dump
**
** Description:
...
pst_dot_stmt(
    ...
    PST_STATEMENT *stmt)
{
    const char BASE_LINK3[] = "n%p:e->n%p:nw[weight=10,label=%s]\n";
    while (stmt)
    {
	if (stmt->pst_next && pst_dot_chk(ctx, stmt, stmt->pst_next))
	    SIfprintf(fd, "n%p:s->n%p:n[weight=1000,label=next]\n",
			stmt, stmt->pst_next);
	SIfprintf(fd, "n%p[fillcolor=lightskyblue,"
				"label=\"{{%s|%d}",
		    stmt,
	...
	case PST_RP_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_condition && pst_dot_chk(ctx, stmt, v->pst_condition))
		{
		    SIfprintf(fd, "n%p:ne->n%p:w[weight=10,label=pst_condition]\n",
			stmt, v->pst_condition);
		    pst_dot_qnode(ctx, fd, v->pst_condition);
		}
		if (v->pst_true && pst_dot_chk(ctx, stmt, v->pst_true))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_true, "pst_true");
		    if (v->pst_true != stmt->pst_next)
			pst_dot_stmt(ctx, fd, v->pst_true);
		}
		if (v->pst_false && pst_dot_chk(ctx, stmt, v->pst_false))
		{
		    SIfprintf(fd, "n%p:se->n%p:nw[weight=10,label=pst_false]\n",
			stmt, v->pst_false);
		    if (v->pst_false != stmt->pst_next)
			pst_dot_stmt(ctx, fd, v->pst_false);
		}
	    }
	    break;
	case PST_MSG_TYPE:
	case PST_EMSG_TYPE:
		...
		if (v->pst_msgnumber && pst_dot_chk(ctx, stmt, v->pst_msgnumber))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_msgnumber, "pst_msgnumber");
		    pst_dot_qnode(ctx, fd, v->pst_msgnumber);
		}
		if (v->pst_msgtext && pst_dot_chk(ctx, stmt, v->pst_msgtext))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_msgtext, "pst_msgtext");
		    pst_dot_qnode(ctx, fd, v->pst_msgtext);
		}
	    }
	    break;
	case PST_RTN_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_rtn_value && pst_dot_chk(ctx, stmt, v->pst_rtn_value))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_rtn_value, "pst_rtn_value");
		    pst_dot_qnode(ctx, fd, v->pst_rtn_value);
		}
	    }
	    break;
	case PST_RSP_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_savept && pst_dot_chk(ctx, stmt, v->pst_savept))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_savept, "pst_savept");
		    pst_dot_qnode(ctx, fd, v->pst_savept);
		}
	    }
	    break;
	case PST_CP_TYPE:
		...
		SIfprintf(fd, "|%.32s|%.32s|{0x%x|nargs:%d}",
			...
			v->pst_pmask,
			v->pst_argcnt);
		if (v->pst_return_status && pst_dot_chk(ctx, stmt, v->pst_return_status))
		{
		    SIfprintf(fd, "|return_status|");
		    pst_qtree_rsdm_node(fd, v->pst_return_status);
		}
		SIfprintf(fd, "}\"]\n");
		if (v->pst_arglist && pst_dot_chk(ctx, stmt, v->pst_arglist))
		{
		    SIfprintf(fd, "n%p->n%p[weight=10,label=pst_arglist]\n",
			stmt, v->pst_arglist);
		    pst_dot_qnode(ctx, fd, v->pst_arglist);
		}
	    }
	    break;
	case PST_IP_TYPE:
	...
	case PST_EVRAISE_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_evvalue && pst_dot_chk(ctx, stmt, v->pst_evvalue))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_evvalue, "pst_evvalue");
		    pst_dot_qnode(ctx, fd, v->pst_evvalue);
		}
	    }
	    break;
	case PST_EXEC_IMM_TYPE:
	...
	case PST_FOR_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_forhead && pst_dot_chk(ctx, stmt, v->pst_forhead))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_forhead, "pst_forhead");
		    if (v->pst_forhead != stmt->pst_next)
			pst_dot_stmt(ctx, fd, v->pst_forhead);
		}
	    }
	    break;
	...
	case PST_CREATE_RULE_TYPE:
		...
		if (v->pst_ruleTree && pst_dot_chk(ctx, stmt, v->pst_ruleTree))
		{
		    SIfprintf(fd, BASE_LINK3, stmt, v->pst_ruleTree, "pst_ruleTree");
		    pst_qtree_dot1(ctx, fd, v->pst_ruleTree);
		}
		if (v->pst_ruleTuple && pst_dot_chk(ctx, stmt, v->pst_ruleTuple))
		{
		    DB_IIRULE *d = v->pst_ruleTuple;
		    SIfprintf(fd, "n%p:e->n%p:w[weight=10,label=pst_ruleTuple]\n",
				stmt, d);
		    SIfprintf(fd, "n%p[fillcolor=lightskyblue,label=\"{DB_IIRULE|%.32s|{%s|0x%x}|...}\"]\n",
				d,d->dbr_name,
	...
	case PST_CREATE_PROCEDURE_TYPE:
		...
		SIfprintf(fd, "}\"]\n");
		if (v->pst_procedureTuple && pst_dot_chk(ctx, stmt, v->pst_procedureTuple))
		{
		    DB_PROCEDURE *d = v->pst_procedureTuple;
		    SIfprintf(fd, "n%p:e->n%p:w[weight=10,label=pst_procedureTuple]\n",
				stmt, d);
		    SIfprintf(fd, "n%p[fillcolor=lightskyblue,label=\"{DB_PROCEDURE|%.32s|...}\"]\n",
				d,d->db_dbpname);
		}
	    }
	    break;
	...
	case PST_CREATE_VIEW_TYPE:
		...
		char bm[64];
		fmt_bm(v->pst_updt_cols.db_domset, sizeof(v->pst_updt_cols.db_domset), bm);
		SIfprintf(fd, "|{%.32s|0x%x|%d}|C:%s}\"]\n",
			v->pst_view_name, v->pst_flags, v->pst_status, bm);
		if (v->pst_view_tree && pst_dot_chk(ctx, stmt, v->pst_view_tree))
		{
		    SIfprintf(fd, "n%p:e->n%p:w[weight=10,label=pst_view_tree]\n",
				stmt, v->pst_view_tree);
		    pst_qtree_dot1(ctx, fd, v->pst_view_tree);
		}
	    }
	    break;
...
common_end:
	...
	LINKBLOCK(stmt, pst_after_stmt,
		pst_dot_stmt, "tailport=s,headport=nw,weight=100,")
	LINKBLOCK(stmt, pst_before_stmt,
		pst_dot_stmt, "tailport=n,headport=sw,weight=100,")
	LINKBLOCK(stmt, pst_statementEndRules,
		pst_dot_stmt, "tailport=s,headport=nw,weight=10,")
	LINKBLOCK(stmt, pst_before_statementEndRules,
		pst_dot_stmt, "tailport=ne,headport=sw,weight=10,")
	stmt = stmt->pst_next;
    }
}
	...
	PSF_SESSCB_PTR cb)
{
    ...
    SIfprintf(fd, "n%p[style=solid,fillcolor=yellow,label=\"{RNGTAB",
	    ...
	    rngvar->pss_rgno >= 0)
	{
	    ...
	    STtrmwhite(name);
	    SIfprintf(fd, "|{{%s|%s|{<f%d>%d|%s|%d}|",
		cv, name, rngvar->pss_rgno, i,
		pst_rgtype_names[rngvar->pss_rgtype<PST_RGTYPE_MAX
			?rngvar->pss_rgtype:PST_RGTYPE_MAX],
		rngvar->pss_rgparent);
	    fmt_bm(&rngvar->pss_inner_rel, sizeof(rngvar->pss_inner_rel), cv);
	    SIfprintf(fd, "I:%s\\r", cv);
	    fmt_bm(&rngvar->pss_outer_rel, sizeof(rngvar->pss_outer_rel), cv);
	    SIfprintf(fd, "O:%s\\r}|%d}", cv, rngvar->pss_rgno);
	}
    }
    SIfprintf(fd, "}\"]\n");
	...
	if (rngvar->pss_used &&
	    rngvar->pss_rgno >= 0)
	{
	    if (rngvar->pss_qtree && pst_dot_chk(ctx, &cb->pss_auxrng.pss_rngtab[0],
						rngvar->pss_qtree))
	    {
		SIfprintf(fd, "n%p:(f%d,w)->n%p:n[label=rgroot]\n",
		...
		case PST_WETREE:
		case PST_TPROC:
		    pst_dot_qnode(ctx, fd, rngvar->pss_qtree);
		    break;
		default:
		    break;
		}
    ...
    void *args[2])
{
	...
	SIfprintf(fd, "digraph qtree{fontname=\"Courier\";fontsize=10;"
		...
		"edge[fontname=\"Courier\",fontsize=10,style=solid];\n", name);
    }

    ctx.adfcb.adf_outarg.ad_f4width = 11;/* min field width for f4 */
    ctx.adfcb.adf_outarg.ad_f4prec = 5;	/* prec for f4 */
    ctx.adfcb.adf_outarg.ad_f8width = 22;/* min field width for f8 */
    ctx.adfcb.adf_outarg.ad_f8prec = 10;	/* prec for f8 */
    ctx.adfcb.adf_outarg.ad_f4style = 'n'; /* e f g or n */
    ctx.adfcb.adf_outarg.ad_f8style = 'n'; /* e f g or n */

    ctx.cb = args[1];
    if (ctx.cb &&
	(rtn == (PST_DOT_RTN)pst_proc_dot1 ||
	 rtn == (PST_DOT_RTN)pst_dot_qnode))
    {
	pst_cb_dot1(&ctx, fd, ctx.cb);
    }
    (*rtn)(&ctx, fd, args[0]);
    ...
    while (ctx.links)
    {
	struct _PST_DOT_LINKS *cur = ctx.links;
	ctx.links = ctx.links->next;
	MEfree(cur);
    }
}
    ...
    PST_QNODE *node)
{
    ...
    args[0] = node;
    args[1] = cb;
    pst_dot(fname, (PST_DOT_RTN)pst_dot_qnode, args);
}
	...
	PST_QNODE *node)
{
    ...
    if (ctx->cb && node)
    {
	i4 n = ctx->cb->pss_auxrng.pss_maxrng+1;
...
#define BIT_RO	0x1000
	...
	for (i = 0; i < PST_NUMVARS; i++)
	{
	    PSS_RNGTAB *rngvar = &ctx->cb->pss_auxrng.pss_rngtab[i];
	    if (rngvar->pss_used &&
		rngvar->pss_rgno >= 0)
	    {
		char name[sizeof(rngvar->pss_tabname)];
	...
	while (p)
	{
	    ...
	    case PST_BOP:
		l = r = NULL;
		if (!(status = adi_op_info(ctx->cb->pss_adfcb,
				...
				BITS_IN(PST_J_MASK))) >= 0;)
			{
				...
				if (bits & BIT_ID)
				{
					...
					BTtest(bits & BIT_ID, (char*)&rngvar->pss_outer_rel))
					    bits |= BIT_LO;
				    rngvar = &ctx->cb->pss_auxrng.pss_rngtab[j];
				    if (rngvar->pss_used && rngvar->pss_rgno >= 0 &&
					BTtest(bits & BIT_ID, (char*)&rngvar->pss_outer_rel))
					    bits |= BIT_RO;
				}
				...
				if ((bits&(BIT_LO|BIT_RO)) == BIT_LO)
				    arr = "forward";
				else if ((bits&(BIT_LO|BIT_RO)) == BIT_RO)
				    arr = "back";
				else if (bits&(BIT_LO|BIT_RO))
				    arr = "both";
				if (bits & BIT_EQ)
				{
				    SIfprintf(fd, "v%d->v%d[color=red,dir=%s", i, j, arr);
				    if (bits & BIT_ID)
	...
	for (j = 0; j < n; j++)
	{
		...
		if ((mapij&(BIT_LO|BIT_RO)) == BIT_LO)
		    arr = "forward";
		else if ((mapij&(BIT_LO|BIT_RO)) == BIT_RO)
		    arr = "back";
		else if (mapij&(BIT_LO|BIT_RO))
		    arr = "both";
		if (mapij & BIT_EQ)
		{
		    SIfprintf(fd, "v%d->v%d[dir=%s", i, j, arr);
		    if (mapij & BIT_ID)
	...
	PST_QNODE *node)
{
    ...
    args[0] = node;
    args[1] = cb;
    pst_dot(fname, (PST_DOT_RTN)pst_relations_dot1, args);
}
...
pst_proc_dot1(
	...
	PST_PROCEDURE *proc)
{
    SIfprintf(fd, "n%p[fillcolor=lightskyblue,label=\"{{PROCEDURE|vsn:%d}",
			proc, proc->pst_vsn);
    if (proc->pst_isdbp)
	SIfprintf(fd, "|%.32s", proc->pst_dbpid.db_cur_name);
    if (proc->pst_parms)
	pst_decvar_dot1(ctx, fd, proc->pst_parms);
    SIfprintf(fd, "}\"]\n");

    LINKBLOCK(proc, pst_stmts, pst_dot_stmt, "weight=10,")
}
	...
	PST_PROCEDURE *proc)
{
    ...
    args[0] = proc;
    args[1] = NULL;
    pst_dot(fname, (PST_DOT_RTN)pst_proc_dot1, args);
}

/*{
** Name: pst_dot_chk - Check if link seen before
**
** Description:
**	Check if link has been addressed before.
**
** Inputs:
**	ctx		DOT context
**	f,t		Addresses of src & dest to check.
**
** History:
**	21-Oct-2010 (kiria01) b124629
**	    Created
	...
	void *t)
{
	...
	for (i = 0; i < l; i++)
	{
	    if (cur->list[i].f == f && cur->list[i].t == t)
		return 0;
	}
	l = PST_DOT_N_LINKS;
	cur = cur->next;
    }
    if (!(cur = ctx->links) || (i = ctx->idx) >= PST_DOT_N_LINKS)
    {
	ctx->links = (struct _PST_DOT_LINKS*)MEreqmem(0, sizeof(*cur), FALSE, NULL);
	ctx->links->next = cur;
	cur = ctx->links;
	i = ctx->idx = 0;
    }
