...
** Rather than choose based on buffer size, use number of rows, since
** tests indicated that the DMF loader does best with a moderate number
** of row at a time.  The sequencer will dump the buffer when it reaches
** this number of rows, so making the buffer more than a row larger is a
** waste.
** (Note, I consider this #-of-rows thing a bug, not a feature, so I have
** not centralized the #-of-rows define for now!)
** If the best number of rows generates a too-large buffer, we can trim it.
*/
#define BEST_COPYBUF_ROWS	100
#define MAX_COPYBUF_SIZE	(1024 * 1024)	/* stop at 1 meg;  this number
					** has to be large enough to fit at
					** least a few 256K-byte rows */
	...
	PST_QNODE	   *updcollst)
{
	...
	** really is the same. */
	...
	if ((status = ulm_openstream(&ulm_rcb)) != E_DB_OK)
	{
	    ...
	    {
		psf_error(E_PS0F02_MEMORY_FULL, (DB_ERROR *)NULL, PSF_CALLERR, 
		    &err_code, &psq_cb->psq_error, 0);
	    }
	    else
		(VOID) psf_error(E_PS0A05_BADMEMREQ, &ulm_rcb.ulm_error,
    ...
    */
	...
	if ((status = ulm_palloc(&ulm_rcb)) != E_DB_OK)
	{
		...
		(VOID) psf_error(E_PS0A02_BADALLOC, &ulm_rcb.ulm_error,
			PSF_INTERR, &err_code, &psq_cb->psq_error, 0);
	...
	*/
	if (sess_cb->pss_highparm >= proto->pst_num_parmptrs)
	{
		...
		if (ulm_rcb.ulm_error.err_code == E_UL0005_NOMEM)
		{
		    psf_error(E_PS0F02_MEMORY_FULL, (DB_ERROR *)NULL, 
    ...
    */
    ...
    proto->pst_cant_copy_optim = FALSE;	/* Don't know yet */
...
**	13-dec-2010 (stephenb)
**	    fill out the rest of the sqlda when using the batch copy optimization,
**	    x100 uses it to determine the format of the input record.
...
**	    Replace copied code with call to common utilities.
**       1-Feb-2011 (hanal04) Bug 124876
**          Initialise the new qeu_msg_ptr field.
**	22-Mar-2011 (kschendel) m1542
**	    Relax the test for batch copy optimization so that we don't
**	    exclude VW tables with constraints.  It's OK to bulk-load those.
**	    Also, unfix the temp range-tab used for the test.
**	24-Mar-2011 (kschendel) b125204 m1782
**	    The copy control blocks for batch copy optimization need to be
**	    per-prototype since they are allocated from the prototype stream.
**	    Only one insert-to-copy can be active per batch (due to sequencer
**	    limitations), though.  Rearrange checks so that if we're doing
**	    an insert for a statement that OK'ed insert-to-copy last time,
**	    avoid all the pst-show and checking;  not needed.  Add "no optim"
**	    flag so we don't check a known-can't-optim stmt over and over.
**	29-Mar-2011 (kschendel) b125204
**	    Init sequence counters in qeu-copy for new copy.
**	8-Aug-2011 (kschendel) SIR 125587
**	    Set up table name in qeu-copy.
**	16-Aug-2011 (kschendel) b125661
**	    My 26-Jan change passed the wrong memory stream to psl-copy-missing
**	    which caused batched inserts of (?, ?, 100) or similar to fail.
**	    Pass the correct (ie prototype) stream.
*/
#define	RT_NODE_SIZE \
	...
	bool		    rdonly)
{
    ...
    *updcollst = (PST_QNODE *) NULL;
    ...
    if (proto == (PST_PROTO *) NULL)
    {
	(VOID) psf_error(2304L, (DB_ERROR *)NULL, PSF_USERERR,
	...
	*/
	(VOID) psf_error(E_PS0355_DIFF_EFF_USER, (DB_ERROR *)NULL, PSF_USERERR,
    ...
    */
	...
	{
	    (VOID) psf_error(E_PS0D19_QSF_INFO, &qsf_rb.qsf_error,
		PSF_INTERR, &err_code, &psq_cb->psq_error, 0);
	    return (E_DB_ERROR);
	}
    }
    ...
    ** then it is not used.
    ** Also, no copy optim if we're already doing it for some statement and
    ** this one isn't it.  Sequencer currently (Mar 2011) runs copy-finish
    ** for only one in-progress copy, the "most recent" one addressed by
    ** psq_last_sname and psq_cp_qefrcb.
    */
    ...
    if ((psq_cb->psq_flag2 & PSQ_BATCH) && proto->pst_mode == PSQ_APPEND &&
	  ...
	  || STcompare(psq_cb->psq_last_sname,proto->pst_sname) == 0)
	    )
    {
	if (psq_cb->psq_last_sname[0] != EOS)
	{
	    /* same as last statement (name checked above) so it's a
	    ** copy optim and not first time.
	    */
	    copy_optim = TRUE;
	}
	else if (proto->pst_cp_qefrcb != NULL)
	{
	    QEF_RCB *qef_rcb;
	    QEU_COPY *qe_copy;

	    /* First time for this batch, but since copy stuff exists it
	    ** must be OK, set things to start a copy batch.
	    */
	    copy_optim = TRUE;
	    STcopy(proto->pst_sname, psq_cb->psq_last_sname);
	    qef_rcb = (QEF_RCB *) proto->pst_cp_qefrcb;
	    qe_copy = qef_rcb->qeu_copy;
	    qe_copy->qeu_cp_scf_seq = 0;
	    qe_copy->qeu_cp_qef_seq = 0;
	    qe_copy->qeu_load_buf = FALSE;
	    qe_copy->qeu_count = 0;
	    qe_copy->qeu_stat &= ~(CPY_FAIL | CPY_ERROR | CPY_WARN);
	    qe_copy->qeu_insert_data = NULL;
	    qe_copy->qeu_cur_tups = 0;
	}
	else if (! proto->pst_cant_copy_optim)
	{
	    ...
	    MEfill(sizeof(PSS_RNGTAB), 0, (PTR)&rngvar);
		...
		{
		    (VOID) psf_error(4709L, (DB_ERROR *)NULL, PSF_USERERR,
	    ...
	    ** Not a view, index, gateway table;  no rules on table (ideally
	    ** we'd check specifically for insert rules but that's not easy
	    ** here);  no constraints, unless it's a VW table.
	    */

	    if ( (rngvar.pss_tabdesc->tbl_status_mask & (DMT_VIEW | DMT_IDX | DMT_GATEWAY | DMT_RULE)) == 0
	      && (rngvar.pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX
		  || (rngvar.pss_tabdesc->tbl_status_mask & DMT_INTEGRITIES) == 0) )
	    {
		i4	rowbuf_size;
		...
		/*
		** we only allocate the RCB once for each proto (prepared
		** query), but we may run several batches of executes against
		** the same proto.  So we will use the proto stream. The memory
		** for the stream will be eventually freed when we commit.
		** Memory for the copy input row buffer is allocated here;
		** ultimately we fill it out in pst_cpdata when we get the
		** input from each query.
		*/

		/* Determine size of row buffer based on BEST_COPYBUF_ROWS,
		** plus one more to make sure it all fits, clipped at
		** MAX_COPYBUF_SIZE.
		*/
		rowbuf_size = (BEST_COPYBUF_ROWS+1) * rngvar.pss_tabdesc->tbl_width;
		if (rowbuf_size > MAX_COPYBUF_SIZE)
		    rowbuf_size = MAX_COPYBUF_SIZE;
		/*
		...
		*/
		...
		proto->pst_cp_qefrcb = qef_rcb;
		qef_rcb->qeu_copy = (QEU_COPY *) ((char *)qef_rcb + sizeof(QEF_RCB));
		qe_copy  = qef_rcb->qeu_copy;
		qe_copy->qeu_dmrcb = 
			(DMR_CB *) ((char *)qe_copy + sizeof(QEU_COPY));
		...
		if (rngvar.pss_tabid.db_tab_base < 0
		  && rngvar.pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX)
		{
		...
		memcpy(p, rngvar.pss_tabname.db_tab_name, len);
		qe_copy->qeu_tablename[len] = '\0';
		qe_copy->qeu_cbuf = qe_copy->qeu_cptr = (char *)dmr_cb + sizeof(DMR_CB);
		...
		*/
				...
				*/
				...
				if (status)
				{
				    (VOID) psf_error(E_PS0C05_BAD_ADF_STATUS,
					    &sess_cb->pss_adfcb->adf_errcb.ad_dberror, 
					    PSF_INTERR, &err_code,
					    &psq_cb->psq_error, 0);
				    return (E_DB_ERROR);
				}
				...
				status = psl_copy_missing(sess_cb, &proto->pst_ostream,
					    qe_copy, right_child,
					    att_entry, dt_bits,
					    &psq_cb->psq_error);
				if (status != E_DB_OK)
				...
				if (right_child->pst_sym.pst_type == PST_SEQOP)
				{
				    ++ seq_defaults;
		...
		if (copy_optim)
		{
		    ...
		    ** the values are added */
		    ...
		    /*
		    ** we don't use the rest of sqlda for the copy optimization
		    ** since the description is taken from QEF_INS_DATA, which
		    ** is fabricated in pst_cpdata, but X100 wants to know the
		    ** format of the input record, and it uses sqlda to find
		    ** it. What we do here is fill out sqlda for X100's benefit
		    ** using the target table's column format. Ultimately, this
		    ** *is* the format in which x100 will see the data because
		    ** it gets correctly converted in qeu_r_copy.
		    ** rngvar atts start at 1, sqlda starts at 0.
		    */
		    for (i = 0; i < num_atts; i++)
		    {
			sqlda->gca_col_desc[i].gca_l_attname = 0;
			sqlda->gca_col_desc[i].gca_attname[1] = EOS;
			sqlda->gca_col_desc[i].gca_attdbv.db_datatype =
				rngvar.pss_attdesc[i+1]->att_type;
			sqlda->gca_col_desc[i].gca_attdbv.db_length = 
				rngvar.pss_attdesc[i+1]->att_width;
			sqlda->gca_col_desc[i].gca_attdbv.db_prec = 
				rngvar.pss_attdesc[i+1]->att_prec;
		    }
		    ...
		    status = psl_copy_finish_seq_defaults(sess_cb,
			    &proto->pst_ostream, seq_defaults,
			    seq_datalen, qe_copy,
			    &psq_cb->psq_error);
		    if (status != E_DB_OK)
			return (status);

		    /* Indicate copy batch is starting */
		    STcopy(proto->pst_sname, psq_cb->psq_last_sname);
		}
	    }
	    /* If we decided against copy optim, remember so that we
	    ** don't go thru the motions again
	    */
	    if (! copy_optim)
	    {
		proto->pst_cant_copy_optim = TRUE;  /* Don't try again */
		proto->pst_cp_qefrcb = NULL;
	    }
	    (void) pst_rng_unfix(sess_cb, &rngvar, &psq_cb->psq_error);
	}
    }

    /* Set "this stmt gets insert-to-copy optim" flag as appropriate */
    if (copy_optim)
    {
	psq_cb->psq_flag |= PSQ_INSERT_TO_COPY;
	psq_cb->psq_cp_qefrcb = proto->pst_cp_qefrcb;
    }
    else
    {
	psq_cb->psq_flag &= ~PSQ_INSERT_TO_COPY;
	/* but don't clear psq-cb-qefrcb since we still want to be able
	** to finish the batch if the last batch stmt doesn't happen to
	** be an insert-to-copy insert.
	*/
    }
    ...
    */
    ...
    {
	(VOID) psf_error(2302L, (DB_ERROR *)NULL, PSF_USERERR,
	    &err_code, &psq_cb->psq_error, 2, sizeof(sess_cb->pss_lineno),
	    &sess_cb->pss_lineno, STtrmwhite(sname), sname);
	...
	*/
		...
		if (sess_cb->pss_retry & PSS_REFRESH_CACHE)
		{
			...
			if (DB_FAILURE_MACRO(status))
			{
                            if((psq_cb->psq_error.err_code == 
                               ...
                               && (psf_in_retry(sess_cb, psq_cb)))
                            {
                                (VOID) psf_error(4709L, (DB_ERROR *)NULL, 
		...
		*/
		...
		{
		    (VOID) psf_error(2310L, (DB_ERROR *)NULL, PSF_USERERR,
...
**       8-Dec-2010 (hanal04) Bug 124773
**          A PST_CASEOP may have an unknown type when query parameters (?)
**          are used. With the underlying nodes resolved/substituted we
**          need to update the PST_CASEOP node with a call to pst_resolve().
**	19-May-2011 (kiria01) SIR 124826/m1949
**	    Make the conversion of constant bools into IITRUE/IIFALSE depend
**	    on being in a qualifier context.
*/
DB_STATUS
pst_prmsub(
	PSQ_CB		    *psq_cb,
...
#define STATE_IN_WHERE 0x1
	...
	*/
	  ...
	  (node->pst_left || node->pst_right))
	{
	    ...
	    {
	    case PST_AGHEAD:
		/* No resdoms - so in WHERE */
		state[++depth] = STATE_IN_WHERE;
		break;
	    case PST_ROOT:
	    case PST_SUBSEL:
		state[++depth] = 0;
	...
	case PST_SUBSEL:
	{
		...
		node->pst_sym.pst_dataval.db_collID = src_dbval->db_collID;
	    }
	...
	case PST_UOP:
	{
		...
		node->pst_sym.pst_dataval.db_collID = DB_UNSET_COLL;
	...
	case PST_MOP:
	{
	    ...
	    */
	    ...
	    if (status = pst_resolve(sess_cb, (ADF_CB *) sess_cb->pss_adfcb, nodep,
			sess_cb->pss_lang, &psq_cb->psq_error))
	    if (DB_FAILURE_MACRO(status))
		return (pst_rserror(sess_cb->pss_lineno, node, &psq_cb->psq_error));
	    node = *nodep;

	    /* If we turned the node into a boolean TRUE/FALSE, turn it
	    ** back into a boolean operator, so that the joinid isn't lost
	    ** but only if we are in a qualifier. */
	    if (state[depth] & STATE_IN_WHERE &&
		pst_is_const_bool(node, &bval) &&
	...
	*/
	...
	case PST_CASEOP:
            if (node->pst_sym.pst_dataval.db_datatype == 0)
            {
                if (status = pst_resolve(sess_cb, (ADF_CB *) sess_cb->pss_adfcb, 
                            nodep, sess_cb->pss_lang, &psq_cb->psq_error))
                    return (pst_rserror(sess_cb->pss_lineno, node,
                        &psq_cb->psq_error));
                node = *nodep;
            }
	...
	case PST_RESDOM:
		...
		*/
		if (node->pst_sym.pst_dataval.db_datatype == 0)
		{
			...
			node->pst_right->pst_sym.pst_dataval.db_collID;
		}
		else
		{
...
**	    Make sure copy-optim pointer is cleared here.
*/
DB_STATUS
pst_commit_dsql(
	PSQ_CB		    *psq_cb,
	PSS_SESBLK	    *sess_cb)
{
    ...
    */
    ...
    /* Make sure insert-to-copy control block pointer is gone since we
    ** closed all the prototypes.  Should be null already but make sure.
    */
    psq_cb->psq_cp_qefrcb = NULL;
    psq_cb->psq_last_sname[0] = EOS;
    psq_cb->psq_flag &= ~PSQ_INSERT_TO_COPY;	/* Just making sure */
	...
	if (resdom->pst_sym.pst_dataval.db_datatype == DB_NODT)
	{
		...
		else
		{
			...
			/* it's an op node, lets see if we can resolve it now */
			    status = pst_resolve(cb, (ADF_CB*) cb->pss_adfcb, &resdom->pst_right, 
				cb->pss_lang, &err_blk);
			    right = resdom->pst_right;
			    if (status == E_DB_OK && 
...
**	24-Mar-2011 (kschendel) b125204 m1782
**	    Make sure psq-cp-qefrcb is cleared if we delete the proto that
**	    it happens to be pointing to.
*/
static DB_STATUS
pst_dstobj(
	PSQ_CB		    *psq_cb,
	...
	PST_PROTO	    *proto)
{
    ...
    /* If we're deleting the prototype that psq-cb is pointing into for
    ** insert-to-copy optimization, reflect that.
    ** FIXME! WHAT IF COPY BATCH ALREADY STARTED AND USER RE-PREPARES STMT?
    ** would seem to leave the copy hanging.  sequencer can't finish the
    ** copy because it won't have valid control blocks any more.
    ** EITHER a) finish it now (and tell sequencer somehow) or b) error,
    ** re-preparing copy-optimized stmt mid batch!
    */
    if (psq_cb->psq_cp_qefrcb == proto->pst_cp_qefrcb)
	psq_cb->psq_cp_qefrcb = NULL;
    proto->pst_cp_qefrcb = NULL;
...
**	    Use explicit coercion information now that resolver can determine the
**	    datatype even when there are parameters present. Also, set coercion
**	    parameters to nullable as they can cope with nullability.
*/
		...
		else if (desc->gca_attdbv.db_datatype == DB_NODT)
		{
		    ...
		    */
			    ...
			    adi_fitab(sess_cb->pss_adfcb, opno, &fi_tab) == E_DB_OK)
			{
			    ...
			    {
				/* case 3 */
				ADI_FI_DESC *fdesc = parent->pst_sym.pst_value.pst_s_op.pst_fdesc;
				/* Resolver understands that a coercion's datatype is
				** usually known even when the param DTs might not be. */
				if (fdesc &&
					(fdesc->adi_fiflags&ADI_F524288_EXPL_COERCION))
				{
				    /* Allow param to be nullable - coercion will cope. */
				    if (dv->db_datatype > 0)
				    {
					desc->gca_attdbv.db_datatype = -dv->db_datatype;
					desc->gca_attdbv.db_length = dv->db_length+1;
				    }
				    else
				    {
					desc->gca_attdbv.db_datatype = dv->db_datatype;
...
pst_cpdata(PSS_SESBLK *sess_cb, PSQ_CB *psq_cb, PST_QNODE *tree, bool use_qsf)
{
    ...
    {
	(VOID) psf_error(E_PS03B1_NULL_RCB, (DB_ERROR *)NULL,
		PSF_CALLERR, &err_code, &psq_cb->psq_error, 0);
	return (E_DB_ERROR);
    }
    qef_rcb  = psq_cb->psq_cp_qefrcb;
    qe_copy  = qef_rcb->qeu_copy;
    sqlda = (GCA_TD_DATA *) qe_copy->qeu_tdesc;
	...
	*/
	(VOID) psf_error(E_PS03B4_NO_COPY_CACHE, (DB_ERROR *)NULL,
		PSF_CALLERR, &err_code, &psq_cb->psq_error, 0);
	return (E_DB_ERROR);
    }    
	...
	*/
	...
	for (i=0 ;i < qdesc->psq_dnum;i++)
	{
	    ...
	    {
		(VOID) psf_error(E_PS03B2_INVALID_CPLENGTH, (DB_ERROR *)NULL,
			PSF_CALLERR, &err_code, &psq_cb->psq_error, 0);
		return (E_DB_ERROR);
	    }
	    ...
	    col_val[attno].dtcol_value.db_collID = qdesc->psq_qrydata[i]->db_collID;
	...
	if (tree == NULL)
	{
	    (VOID) psf_error(E_PS03B3_NULL_QUERY_TREE, (DB_ERROR *)NULL,
		    PSF_CALLERR, &err_code, &psq_cb->psq_error, 0);
	    return (E_DB_ERROR);
	}
	...
	for (node = tree->pst_left; node->pst_left != NULL; node = node->pst_left)
	{
		...
		col_val[attno].dtcol_value.db_collID = 
			const_node->pst_sym.pst_dataval.db_collID;
