...
**	    Tidied pst_node call & fixed missing collID passing.
	...
	DB_ERROR	    *err_blk)
{
    ...
    status = pst_node(sess_cb, stream, NULL, NULL,
		...
		sort_attr->pst_sym.pst_dataval.db_length, NULL,
		sort_node, err_blk, 0);

    /* if node has been created successfully, append it to the list */
    if (DB_SUCCESS_MACRO(status))
    {
	(*sort_node)->pst_sym.pst_dataval.db_collID =
		sort_attr->pst_sym.pst_dataval.db_collID;
	if (*list != (PST_QNODE *) NULL)
	{
	...
	DB_ERROR	   *err_blk)
{
    ...
    {
	psf_error(2161L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, err_blk, 1,
    ...
    {
	(VOID) psf_error(2163L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	PSQ_CB		    *psq_cb)
{
    ...
    */
    if (!save_attr)
    {
	(VOID) psf_error(2161L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	{
	    (VOID) psf_error(2181L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 1,(i4) STtrmwhite(expname), expname);
	    return (E_DB_ERROR);
	}
    }
	...
	PSQ_CB		    *psq_cb)
{
    ...
    */
    if (!found)
    {
	(VOID) psf_error(2162L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 0);
	return (E_DB_ERROR);
    }
	...
	{
	    (VOID) psf_error(2182L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
...
**	18-june-2009 (dougi)
**	    Assign name to RESDOM for order by expression.
**	09-Dec-2010 (kiria01) SIR 124690
**	    Detect case where pst_union_resolve has found it necessary to
**	    cast a primary resdom and this resdom was referenced in the
**	    sort order. In this case, the cast needs stepping over to spot
**	    the match. This would not be necessary if any unions were
**	    resolved after any order by processing had been applied.
**	10-Dec-2010 (kiria01) SIR 124690
**	    Restrict this last change to just unions.
**	26-Feb-2011 (kiria01)
**	    Remove need for psq_cb in pst_qtree_norm.
**	20-May-2011 (kiria01) m1605
**	    Drop (query) constant expressions from sort list as they have
**	    no effect on the requested sort order. E.g. ORDER BY CURRENT_DATE
**	    or ORDER BY col1 when 'col1' equated to a query constant expression.
*/
DB_STATUS
pst_sqlsort(
	PSS_SESBLK	    *sess_cb,
	...
	PSS_RNGTAB	    **rng_vars)
{
    ...
    DB_ERROR		err_blk;
    bool                aggQry = FALSE; 
    bool		in_union = !sess_cb->pss_yyvars->isdbp &&
				(attr = sess_cb->pss_yyvars->union_head) &&
				attr->pst_sym.pst_type == PST_ROOT &&
				attr->pst_sym.pst_value.pst_s_root.pst_union.pst_next;
    *newnode = NULL;
    ...
    ** expression and replace them by the pst_right of the resdom. */
	...
	for (attr = rlist;
	 ...
	 attr = attr->pst_left)
	{
	    PST_QNODE *expr = attr->pst_right;
	    /* It is possible that pst_union_resolve has coerced a
	    ** primary resdom so we need to look beneath explicit
	    ** coercion operators. */
	    if (in_union &&
		(expr->pst_sym.pst_type == PST_BOP ||
		 expr->pst_sym.pst_type == PST_UOP ||
		 expr->pst_sym.pst_type == PST_MOP) &&
		expr->pst_sym.pst_value.pst_s_op.pst_fdesc &&
		(expr->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags &
				ADI_F524288_EXPL_COERCION) != 0)
	    {
		expr = expr->pst_left;
	    }

            if ((expr->pst_sym.pst_type == PST_AGHEAD) ||
                (expr->pst_sym.pst_type == PST_BOP &&
                 expr->pst_left->pst_sym.pst_type == PST_AGHEAD))
            {
               aggQry = TRUE; 
            }

	    if (expr->pst_sym.pst_type == PST_VAR &&
		...
		expnode->pst_sym.pst_dataval.db_data)
	{
	    ...
	    psf_error(2162L, (DB_ERROR *)NULL, PSF_USERERR, 
                      &err_code, &psq_cb->psq_error, 0);
	    return (E_DB_ERROR);
	}

	/* If we get here, could be a literal constant that we 'could' add
	** to the resdom list as a constant express but we won't - we won't
	** even add a sort block! */
	return E_DB_OK;
	break;
	...
	** NOTE: aggregate expressions under resdoms don't need this stuff. */
	...
	save_attr = NULL;
	{
	    ...
	    ** treated similarly. */
	    if (status = pst_qtree_norm(&cmp_ctx.stk, &expnode, &psq_cb->psq_error))
		return status; 
	    /* Find next available pst_rsno too. */
	    for (nodep = rlist, maxrsno = -1; nodep && 
		nodep->pst_sym.pst_type == PST_RESDOM; nodep = nodep->pst_left)
	    {
		PST_QNODE *expr = nodep->pst_right;
		pst_qtree_compare(&cmp_ctx, &expr, &expnode, FALSE);
		if (cmp_ctx.same)
		{
			...
			pst_qtree_compare(&cmp_ctx, &expr, &expnode, TRUE);
			cmp_ctx.n_covno = 0;
		    }
		    if (cmp_ctx.same)
		    {
		...
		/* Here too it is possible that pst_union_resolve has
		** coerced a primary resdom so we need to look beneath
    ...
    /* If sort col resolves to a constant expression - don't add it or sort by it. */
    if (save_attr &&
	save_attr->pst_sym.pst_type == PST_RESDOM)
    {
	/* We had an ordinal so check for invariance under resdom.
	** This logic does not apply if in a union as there could be
	** several distinct constants. */
	if (!in_union && pst_expr_is_qry_invar(save_attr->pst_right))
	    return E_DB_OK;
    }
    /* If invarience left - drop it. */
    else if (pst_expr_is_qry_invar(expnode))
	return E_DB_OK;
    ...
    ** expression. */
	...
	{
	    STprintf(rsname, "SORTEXPR_%-u", resdom.pst_rsno);
	    MEmove(STlen(rsname), rsname, ' ', DB_MAXNAME, (char *)&resdom.pst_rsname);
	}
...
**	11-may-2011 (dougi)
**	    If we hit a WINDOW node, leave RESDOMs alone.
*/
