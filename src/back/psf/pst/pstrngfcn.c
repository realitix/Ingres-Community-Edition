	...
	DB_ERROR	   *err_blk)
{
    ...
    */
	...
	{
	    (VOID) psf_error(3000L, (DB_ERROR *)NULL, PSF_USERERR,
		&err_code, err_blk, 0);
	...
	DB_ERROR	   *err_blk)
{
	...
	if (rptr == (PSS_RNGTAB *) NULL)
	{
	    ...
	    */
	    ...
	    {
		QUinsert((QUEUE *) rptr, rngtable->pss_qhead.q_prev);
		(VOID) psf_error(3100L, (DB_ERROR *)NULL, PSF_USERERR,
		    &err_code, err_blk, 0);
		*rngvar = (PSS_RNGTAB *) NULL;
		return (E_DB_ERROR);
	    }
	...
	bool		   scope_flag)
{
		...
		*/
		if (first)
		{
			   ...
			   )
			{
			    ...
			    (VOID) psf_error(2921L, (DB_ERROR *)NULL, 
	...
	*/
	...
	{
	    (VOID) psf_error(2921L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                err_blk, 2, (i4) sizeof(cb->pss_lineno), (PTR) &cb->pss_lineno,
		(i4) STlength(name), (PTR) name);
	    return (E_DB_ERROR);
	}
    }
	...
	*/
	for (i = 0; i <= scope; i++)
	{
		...
		{
		    /* ambiguous reference - report an error */
		    (VOID) psf_error(2921L, (DB_ERROR *)NULL, PSF_USERERR,
			&err_code, err_blk, 2,
			(i4) sizeof (cb->pss_lineno),
			(PTR) &cb->pss_lineno,
			(i4) STlength(name), (PTR) name);
...
**	    Raise a user error if derived table or CTE column has a datatype
**	    of DB_NODT.
*/
DB_STATUS
pst_sdent(
	...
	DB_ERROR	   *err_blk)
{
	...
	*rngvar = (PSS_RNGTAB *) NULL;
	...
	if (nodep->pst_sym.pst_dataval.db_datatype == DB_NODT)
	{
	    (VOID) psf_error(2195L, (DB_ERROR *)NULL, PSF_USERERR, 
			     &err_code, err_blk, 0);
	    return (E_DB_ERROR);
	}
    }
	...
	*/
	...
	attrp->att_collID = nodep->pst_sym.pst_dataval.db_collID;
    }
	...
	DB_ERROR	   *err_blk)
{
    ...
    */
    ...
    if (dbptupp->db_mask[0] & DBP_DATA_CHANGE)
    {
        ...
        (VOID)psf_error(2449L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                        err_blk, 1, sizeof(dbptupp->db_dbpname.db_dbp_name),
