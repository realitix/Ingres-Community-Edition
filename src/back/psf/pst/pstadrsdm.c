	...
	PST_QNODE	   **newnode)
{
	...
	if (!STcasecmp(((*cb->pss_dbxlate & CUI_ID_REG_U) ? "TID" : "tid"),
			colname ))
	{
	    psf_error(2100L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
	...
	{
	    psf_error(2100L, (DB_ERROR *)NULL, PSF_USERERR, 
	...
	{
	    psf_error(E_US1900_6400_UPD_LOGKEY, (DB_ERROR *)NULL, PSF_USERERR, 
		&err_code, &psq_cb->psq_error, 4,
		(i4) sizeof(cb->pss_lineno), &cb->pss_lineno,
		psf_trmwhite(sizeof(DB_TAB_NAME), 
		    (char *) &resrange->pss_tabname),
	...
	*/
	...
	{
	    psf_error(2207L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                &psq_cb->psq_error, 3,
	...
	{
	    psf_error(2207L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, &psq_cb->psq_error, 3,
		sizeof(cb->pss_lineno), &cb->pss_lineno,
		psf_trmwhite(DB_CURSOR_MAXNAME, 
			cb->pss_crsr->psc_blkid.db_cur_name),
		cb->pss_crsr->psc_blkid.db_cur_name,
	...
	*/
	...
	{
	    psf_error(2130L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	coldesc->att_collID	= DB_UNSET_COLL;
    ...
    */
    ...
    if (status = pst_node(cb, &cb->pss_ostream, left, right,
		PST_RESDOM, (PTR)&resdom, sizeof(resdom),
		coldesc->att_type, coldesc->att_prec,
		coldesc->att_width, NULL,
