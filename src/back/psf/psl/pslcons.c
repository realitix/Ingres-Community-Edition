...
**	    Replace DB_xxx stuff with equivalent PSQ_MODE values.
*/
...
**	15-dec-2008 (dougi)
**	    Add check for X100 table(s) and set integrity flag.
**	19-aug-2010 (dougi)
**	    Add error for check constraints on IVW tables.
**	4-oct-2010 (dougi)
**	    Extend check for X100 tables.
**	29-oct-2010 (dougi)
**	    Disallow referential actions on VW tables.
**      29-Jun-2011 (maspa05) b122380
**          If table autostruct in use then choose the constraint to be used
**          for the base table here, so the correct querytext can be 
**          generated. 
*/
DB_STATUS
psl_verify_cons(
		...
		PST_STATEMENT	**stmt_listp)
{
    ...
    i4		    val1, val2;
    i2	            bts_cons_lvl=0;      /* track most appropriate constraint 
                                            for base table structure */
    PSS_CONS        *bts_cons=NULL;      /* constraint to use for base table    
					    structure */
    bool	    x100;
    bool            with_autostruct = FALSE;
    bool            with_noautostruct = FALSE;
    ...
    */
    ...
    /* if this is a WITH AUTOSTRUCT create table, OR
     * autostruct is on (via CBF globally of via trace point for session)
     * and no NOAUTOSTRUCT on this table -
     *   then find the constraint to be used for the table struct and mark it 
     * as needing the "with b.t.s" qry text 
     */

    /* first determine if WITH [NO]AUTOSTRUCT was used */
    if (dmu_cb && BTtest(DMU_AUTOSTRUCT, dmu_cb->dmu_chars.dmu_indicators))
    {
	  if ((dmu_cb->dmu_chars.dmu_flags & DMU_FLAG_AUTOSTRUCT) != 0)
	  {
		  with_autostruct=TRUE;
	  }
          else
	  {
		  with_noautostruct=TRUE;
	  }
    }

    if (qmode == PSQ_CREATE &&        /* create table */
        (with_autostruct ||           /* with autostruct */
        ((ult_check_macro(&sess_cb->pss_trace, PSS_SESSION_TABLE_AUTOSTRUCT,
				&val1, &val2)  ||  /* trace point PS212 */
	(Psf_srvblk->psf_flags & PSF_TABLE_AUTOSTRUCT) ) &&   /* CBF */
		     !with_noautostruct)))
    {
	/* check all constraints or until we find a PRIMARY key (since
	 * there'll only be one of those 
	 */
        for (curcons = cons_list; 
    	 (curcons != (PSS_CONS *) NULL && bts_cons_lvl < 3);
    	 curcons = curcons->pss_next_cons)
        {
    	        i2 clvl=0;
    
		/* determine constraint precedence level where
		 *  PRIMARY > UNIQUE > REFERENTIAL
		 */

		switch (curcons->pss_cons_type 
			& (PSS_CONS_PRIMARY|PSS_CONS_UNIQUE|PSS_CONS_REF))
		{
		/* Primary keys have both the primary and unique flags */
                case (PSS_CONS_PRIMARY|PSS_CONS_UNIQUE):
                case PSS_CONS_PRIMARY:
			clvl=3;
			break;
		...
		/* if we've found a higher precedence constraint then 
		 * mark it as the one to use
		 *
		 * Note: the >= here seems to preserve the choice under
		 * the old code where we have two equivalent possibilities
		 */
    
    	        if (clvl >= bts_cons_lvl)
    	        {
    	            bts_cons=curcons;
    	            bts_cons_lvl=clvl;
    	        }
    ...
    if (bts_cons)
    {
       bts_cons->pss_cons_flags |= PSS_CONS_BTS_AUTOSTR;
    }

    /* See if this constraint involves X100 table and set flag accordingly. */
    x100 = FALSE;
    if (dmu_cb != (DMU_CB *) NULL)
    {
	x100 = (dmu_cb->dmu_chars.dmu_struct > DB_STDING_STORE_MAX);
    }
    else if (rngvar != (PSS_RNGTAB *) NULL)
    {
	if (rngvar->pss_tabdesc && 
	...
	*/
	...
	if (x100)
	    newstmt->pst_type = PST_X100_CREATE_INTEGRITY_TYPE;
	else newstmt->pst_type = PST_CREATE_INTEGRITY_TYPE;
	...
	*/
	...
	if (x100)			/* flag x100 constraints */
	    cr_integ->pst_createIntegrityFlags |= PST_CONS_X100;
	...
	*/
	...
	case PSS_CONS_REF:
	    if (x100 && (curcons->pss_cons_flags & PSS_CONS_DEL_CASCADE ||
		    curcons->pss_cons_flags & PSS_CONS_DEL_SETNULL ||
		    curcons->pss_cons_flags & PSS_CONS_UPD_CASCADE ||
		    curcons->pss_cons_flags & PSS_CONS_UPD_SETNULL))
	    {
		/* Only RESTRICT is permitted for IVW refrels. */
		(void) psf_error(9484L, 0L, PSF_USERERR, 
			     &err_code, err_blk, 0);
		return(E_DB_ERROR);
	    }
	...
	case PSS_CONS_CHECK:
	    if (x100)
	    {
		(void) psf_error(9477L, 0L, PSF_USERERR, 
			     &err_code, err_blk, 0);
		return(E_DB_ERROR);
	    }
	...
	default:
	    /* should never happen */
	    (VOID) psf_error(E_PS0486_INVALID_CONS_TYPE, (DB_ERROR *)NULL, 
	...
	*/
		...
		newstmt->pst_type == PST_X100_CREATE_INTEGRITY_TYPE)
	{
	    ...
	    */
		...
		if (x100)
		{
		    /* No "with" with VW tables. */
		    /* It would be nice to exclude this in pslwithopts, but
		    ** for column-level constraints, we might not know the
		    ** ultimate storage structure until runtime.
		    */
		    (void) psf_error(9482L, 0L, PSF_USERERR, 
			     &err_code, err_blk, 0);
		    return(E_DB_ERROR);
		}
	    ...
	    if (curcons->pss_cons_flags & PSS_CONS_BTS_AUTOSTR)
	    {
		cr_integ->pst_createIntegrityFlags |= PST_CONS_TABLE_STRUCT;
		cr_integ->pst_autocompress = dmu_cb->dmu_chars.dmu_dcompress;
	    }
...
**				for added column if alter table add column, or
**				NULL if not create or add column
**	rngvar	    	    info block for existing table if ALTER TABLE or
...
/*returned*/	     DB_ERROR  	      *err_blk)
{
    ...
    *num_cols = 0;
    ...
    if (*num_cols > DB_MAXKEYS) {
        psf_error(E_PS03A9_CONSTRAINT_COL_EXCEEDED, (DB_ERROR *)NULL,
    ...
    */
	     ...
	     col2 = (PSY_COL *) col2->queue.q_next)
	{
		...
		_VOID_ psf_error(E_PS0481_CONS_DUP_COL, (DB_ERROR *)NULL, 
	...
	if (colno == -1)
	{
	    ...
	    _VOID_ psf_error(E_PS0483_CONS_NO_COL, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 3,
			     length, command,
			     psf_trmwhite(sizeof(DB_ATT_NAME),
					  curcol->psy_colnm.db_att_name),
	...
	if (colno == DB_IMTID)
	{
	    (void) psf_error(3496L, (DB_ERROR *)NULL, PSF_USERERR, 
			     &err_code, err_blk, 0);
	    return (E_DB_ERROR);
	}
	      ...
	      DB_ERROR	  *err_blk)
{
	...
	if (cons->pss_cons_flags & 
		(PSS_CONS_BTS_AUTOSTR|PSS_CONS_BASETAB | PSS_CONS_IXNAME | PSS_CONS_INDEX_OPTS))
	{
	    ...
	    ** index options. */
	    ...
	    if (cons->pss_cons_flags & (PSS_CONS_BTS_AUTOSTR|PSS_CONS_BASETAB | PSS_CONS_IXNAME))
	    {
		...
		** "base table structure". */
		...
		if (cons->pss_cons_flags & (PSS_CONS_BTS_AUTOSTR|PSS_CONS_BASETAB))
		{
	...
	*/
	...
	if (cons->pss_cons_flags & 
		(PSS_CONS_BTS_AUTOSTR | PSS_CONS_BASETAB | 
		 PSS_CONS_IXNAME | PSS_CONS_INDEX_OPTS))

	{
	    ...
	    ** index options. */
	    ...
	    if (cons->pss_cons_flags & 
	         (PSS_CONS_BTS_AUTOSTR| PSS_CONS_BASETAB | PSS_CONS_IXNAME))
	    {
		/* Add "index = ", followed by "<index name>" or
		** "base table structure". */
		status = psq_tadd(text_chain, (u_char *) ERx("INDEX = "),
		   ...
		   DB_ERROR	   *err_blk)
{
    ...
    if (cons->pss_cons_type & PSS_CONS_NAME_SPEC)
    {
        ...
        if (DB_FAILURE_MACRO(status))
        {
    	psf_error(err_blk->err_code, (DB_ERROR *)NULL, PSF_INTERR, 
    		  &err_code, err_blk, 2, 
		    ...
		    DB_ERROR      *err_blk)
{
	...
	(void) psf_error(E_PS048E_CONS_NOINDEX, (DB_ERROR *)NULL, PSF_USERERR,
			&err_code, err_blk, 0);
	return(E_DB_ERROR);
    }
    ...
    */
    ...
    if (cons->pss_cons_flags & (PSS_CONS_BASETAB|PSS_CONS_BTS_AUTOSTR))
    {
    ...
    */
	...
	if (~rngvar->pss_tabdesc->tbl_status_mask & DMT_JNL)
	{
	    ...
	    {
                (void) psf_error(E_PS04AC_CONS_NON_JOR_TBL_JOR_DB, 
		...
		*/
                (VOID) psf_error(W_PS04B0_CONS_NON_JOR_TBL_JOR_DB, 
                    (DB_ERROR *)NULL, PSF_INTERR, &err_code, err_blk, 3,
		    STlength(cons_str), (PTR) cons_str,
                    psf_trmwhite(sizeof(rngvar->pss_tabname),
                        (char *) &rngvar->pss_tabname),
                    (PTR) &rngvar->pss_tabname,
...
**	18-may-2011 (dougi) m1954
**	    Add check for self-referencing constraints on VW tables. They're
**	    not allowed.
*/
static DB_STATUS
psl_ver_ref_cons(
...
{	
	...
	*/
	  ...
	  && dmu_cb->dmu_chars.dmu_journaled != DMU_JOURNAL_OFF)
	{
	    ...
	    ** message to the error log
	    ...
	    if (~cons->pss_cons_flags & PSS_NO_JOURNAL_CHECK)
	    {
	        (void) psf_error(E_PS04A9_NON_JOUR_REFING_JOUR_DB, 
                                 (DB_ERROR *)NULL, PSF_USERERR, 
                                 &err_code, err_blk, 2,
		...
		** must remember to reset err_blk->err_code
		*/
	        (VOID) psf_error(W_PS04AD_NON_JOUR_REFING_JOUR_DB, 
	...
	if (self_ref && vw_involved)
	{
	    /* Self-referencing relationships aren't supported (yet) in VW. */
	    (void) psf_error(E_PS04F5_X100_SELFREF, 0L,
	...
	/* Check for VW self ref - those aren't allowed. */
	if (ref_obj_name->pss_objspec_flags & PSS_OBJSPEC_EXPL_SCHEMA)
	{
	    if ((MEcmp(ref_obj_name->pss_obj_name.db_tab_name,
		   refing_rngvar->pss_tabname.db_tab_name, DB_TAB_MAXNAME) == 0)
		&& (MEcmp(ref_obj_name->pss_owner.db_own_name,
		   refing_rngvar->pss_ownname.db_own_name, DB_OWN_MAXNAME) == 0))
		self_ref = TRUE;
	}
	else if (MEcmp(ref_obj_name->pss_obj_name.db_tab_name,
		   refing_rngvar->pss_tabname.db_tab_name, DB_TAB_MAXNAME) == 0)
	{
	    self_ref = TRUE;
	    MEcopy((PTR) &refing_rngvar->pss_ownname,
		   sizeof(DB_OWN_NAME), 
		   (PTR) &ref_obj_name->pss_owner);
	}
	if (self_ref && vw_involved)
	{
	    ...
	    (void) psf_error(E_PS04F5_X100_SELFREF, 0L,
			 PSF_USERERR, &err_code, err_blk, 2,
			 psf_trmwhite(sizeof(ref_obj_name->pss_obj_name),
				      (char *) &ref_obj_name->pss_obj_name),
			 (PTR) &ref_obj_name->pss_obj_name,
			 psf_trmwhite(sizeof(ref_obj_name->pss_owner),
				      (char *) &ref_obj_name->pss_owner),
			 (PTR) &ref_obj_name->pss_owner);
	...
	if (! vw_involved && (~refing_jour & DMT_JNL)
		...
		*/
	        (VOID) psf_error(W_PS04AD_NON_JOUR_REFING_JOUR_DB, 
                    (DB_ERROR *)NULL, PSF_INTERR, &err_code, err_blk, 2,
	            psf_trmwhite(sizeof(refing_rngvar->pss_tabname), 
		        (char *) &refing_rngvar->pss_tabname),
	            (PTR) &refing_rngvar->pss_tabname,
	            psf_trmwhite(sizeof(refing_rngvar->pss_ownname),
	...
	*/
	...
	(VOID) psf_error(E_PS04A2_NO_REF_VIEW, (DB_ERROR *)NULL, PSF_USERERR,
			 &err_code, err_blk, 1,
    ...
    */
	...
	if (~rngvar->pss_tabdesc->tbl_status_mask & DMT_JNL)
	    ...
	    ** message to the error log
	    ...
	    if (~cons->pss_cons_flags & PSS_NO_JOURNAL_CHECK)
	    {
	        (void) psf_error(E_PS04AA_NON_JOUR_REFD_JOUR_DB, 
                                 (DB_ERROR *)NULL, PSF_USERERR, 
                                 &err_code, err_blk, 4,
		...
		** must remember to reset err_blk->err_code
		*/
	        (VOID) psf_error(W_PS04AE_NON_JOUR_REFD_JOUR_DB, 
                    (DB_ERROR *)NULL, PSF_INTERR, &err_code, err_blk, 4,
	...
	*/
	    ...
	    ^ (rngvar->pss_tabdesc->tbl_status_mask & DMT_JON))
	{
	    ...
	    if (~cons->pss_cons_flags & PSS_NO_JOURNAL_CHECK)
	    {
	        (VOID) psf_error(E_PS04AB_REF_CONS_JOUR_MISMATCH, 
                                 (DB_ERROR *)NULL, PSF_USERERR, 
                                 &err_code, err_blk, 4,
				 psf_trmwhite(sizeof(*refing_name), 
					      (char *) refing_name),
		...
		** must remember to reset err_blk->err_code
		*/
	        (VOID) psf_error(W_PS04AF_REF_CONS_JOUR_MISMATCH, 
                    (DB_ERROR *)NULL, PSF_INTERR, &err_code, err_blk, 4,
		    psf_trmwhite(sizeof(*refing_name), (char *) refing_name),
		    (PTR) refing_name,
		    psf_trmwhite(sizeof(*refing_schema), 
	...
	*/
	...
	(void) psf_error(E_PS0484_REF_NUM_COL, (DB_ERROR *)NULL, PSF_USERERR, 
			 &err_code, err_blk, 2,
			 length, command,
	...
	{
	    (void) psf_error(E_PS0494_CONS_NAME, (DB_ERROR *)NULL, PSF_USERERR, 
			     &err_code, err_blk, 1,
		  ...
		  DB_ERROR       *err_blk)
{
    ...
    */
	...
	if (DB_FAILURE_MACRO(status))
	    return (status);
	node2->pst_sym.pst_dataval.db_collID = cur_ref_col->att_collID;
	...
	if (DB_FAILURE_MACRO(status))
	    return (status);
	node1->pst_sym.pst_dataval.db_collID = cur_col->att_collID;
		 ...
		 DB_ERROR	     *err_blk)
{
    ...
    */
    if (privs_to_find)
    {
	(void) psf_error(E_PS04A1_NO_REF_PRIV, (DB_ERROR *)NULL, PSF_USERERR,
			 &err_code, err_blk, 2,
	      ...
	      DB_ERROR          	*err_blk)
{
	...
	*/
	if (status != E_DB_OK)
	{
		...
		{   
		    (VOID) psf_error(2117L, (DB_ERROR *)NULL, PSF_USERERR,
		        &err_code, err_blk, 1,
			psf_trmwhite(sizeof(DB_TAB_NAME), 
			    rngvar->pss_tabname.db_tab_name),
			 rngvar->pss_tabname.db_tab_name);
		      ...
		      DB_ERROR		*err_blk)
{
    ...
    */
    ...
    if (!found)
    {
	(void) psf_error(E_PS0491_NO_PRIMARY_KEY, (DB_ERROR *)NULL, PSF_USERERR,
			 &err_code, err_blk, 1,
			 psf_trmwhite(sizeof(DB_TAB_NAME), 
				      rngvar->pss_tabname.db_tab_name),
			 rngvar->pss_tabname.db_tab_name);
		     ...
		     DB_ERROR		*err_blk)   /* returned */
{
	...
	*integ_no = integ.dbi_number;
	...
	(void) psf_error(E_PS0490_NO_UNIQUE_CONSTRAINT, (DB_ERROR *)NULL,
                         PSF_USERERR, &err_code, err_blk, 1,
			 psf_trmwhite(sizeof(DB_TAB_NAME), 
				      rngvar->pss_tabname.db_tab_name),
			 rngvar->pss_tabname.db_tab_name);
	    ...
	    */
		    ...
		    && (col_id != cons_col_id))
		{
		    ...
		    */
		    ...
		    _VOID_ psf_error(E_PS0472_COL_CHECK_CONSTRAINT, 
		     ...
		     DB_ERROR     *err_blk)
{
    ...
    */
	...
	{
	    _VOID_ psf_error(E_PS047D_2UNIQUE_CONS_CONFLICT, (DB_ERROR *)NULL,
			     PSF_USERERR, &err_code, err_blk, 3,
			     length, command,
	...
	{
	    _VOID_ psf_error(E_PS047E_1UNIQUE_CONS_CONFLICT, (DB_ERROR *)NULL, 
			     PSF_USERERR, &err_code, err_blk, 2,
			     length, command,
			     psf_trmwhite(sizeof(DB_CONSTRAINT_NAME),
				   integ_tup1->dbi_consname.db_constraint_name),
	...
	{
	    _VOID_ psf_error(E_PS047F_0UNIQUE_CONS_CONFLICT, (DB_ERROR *)NULL,
                             PSF_USERERR, &err_code, err_blk, 1,
			     length, command);
	}
	return (E_DB_ERROR);
		      ...
		      DB_ERROR		*err_blk)
{
	...
	*/
	...
	if (cr_integ1->pst_createIntegrityFlags & PST_CONS_NEWIX)
	{
		...
		{
		    (void) psf_error(E_PS048D_CONS_BADINDEX, (DB_ERROR *)NULL,
                                PSF_USERERR, &err_code, err_blk, 1,
				psf_trmwhite(sizeof(DB_TAB_NAME),
				  (PTR)&cr_integ1->pst_indexres.pst_resname),
	...
	*/
	     ...
	     stmt2 = stmt2->pst_next)
	{
			   ...
			   sizeof(DB_CONSTRAINT_NAME)))
		{
		    ...
		    _VOID_ psf_error(E_PS047C_DUP_CONS_NAME, (DB_ERROR *)NULL,
                                 PSF_USERERR, &err_code, err_blk, 2,
				 length, command,
				 psf_trmwhite(sizeof(DB_CONSTRAINT_NAME),
				   integ_tup2->dbi_consname.db_constraint_name),
	    ...
	    */
		...
		{
		    (void) psf_error(E_PS0493_DUPL_PRIMARY_KEY,
                                     (DB_ERROR *)NULL, PSF_USERERR,
				     &err_code, err_blk, 1,
				     psf_trmwhite(sizeof(DB_TAB_NAME),
	...
	*/
	...
	if (MEcmp((PTR) &tst_cols, (PTR) &empty_cols, sizeof(tst_cols)))
	{
	    ...
	    (void) psf_error(E_PS0480_UNIQUE_NOT_NULL, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 2,
			     length, command,
			     psf_trmwhite(DB_TAB_MAXNAME, tabname), tabname);
	    ...
	    if (name1_spec)
	    {
		(void) psf_error(E_PS0494_CONS_NAME, (DB_ERROR *)NULL,
                                 PSF_USERERR, &err_code, err_blk, 1,
				 psf_trmwhite(DB_CONS_MAXNAME,
				   integ_tup1->dbi_consname.db_constraint_name),
				 integ_tup1->dbi_consname.db_constraint_name);
		       ...
		       DB_ERROR		*err_blk)
{
    ...
    ** verify key structure to be suitable for the constraint. */
					...
					PST_CONS_BASEIX)
	{
		...
		(void) psf_error(E_PS048F_CONS_NOBASE, (DB_ERROR *)NULL, 
			PSF_USERERR, &err_code, err_blk, 0);
		return(E_DB_ERROR);
	    }
	 ...
	 if (badixopts)
	{
	    ...
	    ** unique_scope = statement" combination. */
	    (void) psf_error(E_PS0497_CONS_IXOPTS, (DB_ERROR *)NULL, 
			PSF_USERERR, &err_code, err_blk, 0);
	    return(E_DB_ERROR);
	}
		  ...
		  PSS_SESBLK	*sess_cb)
{
    ...
    */
	...
	{
	    (void) psf_error(E_PS0493_DUPL_PRIMARY_KEY, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 1,
			     psf_trmwhite(sizeof(DB_TAB_NAME), 
				      rngvar->pss_tabname.db_tab_name),
			     rngvar->pss_tabname.db_tab_name);
			...
			DB_ERROR	*err_blk)
{
	...
	*/
	if (status == E_DB_OK)
	{
	    ...
	    ** so return an error
	    */
	    (void) psf_error(E_PS0495_DUPL_CONS_NAME, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 2,
			     psf_trmwhite(DB_CONS_MAXNAME,
...
DB_ERROR                  *err_blk)
{
	...
	*/
	     ...
	     tupcount++)
	{
	    ...
	    if (cur_dbdep->dbv_dtype == PSQ_CONS)
	    {
		*found = TRUE;
		break;
	    }
