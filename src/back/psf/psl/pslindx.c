...
**      12-Nov-2009 (gefei01)
**          psl_ci1_create_index() validate storage type for X100.
**	07-Dec-2009 (troal01)
	...
	i4		unique)
{
    ...
    {
	(VOID) psf_error(5311L, (DB_ERROR *)NULL, PSF_USERERR, 
                         &err_code, err_blk, 0);
	return (E_DB_ERROR);
    }
...
**	    Change qmode here if base table is VW table.
*/
DB_STATUS
psl_ci3_indexrel(
	PSS_SESBLK	*sess_cb,
	...
	PSS_OBJ_NAME	*tbl_spec)
{
    ...
    */
	...
	if (DB_SUCCESS_MACRO(status) && !rngtab)
	{
	        ...
	        (VOID) psf_error(E_PS1106_IDX_NOPTBL, (DB_ERROR *)NULL, 
                            PSF_USERERR, &err_code, err_blk, 1,
                            psf_trmwhite(sizeof(DB_TAB_NAME),
                                (char *) &tbl_spec->pss_obj_name),
			    &tbl_spec->pss_obj_name);
            else
		(VOID) psf_error(5300L, (DB_ERROR *)NULL, PSF_USERERR, 
                            &err_code, err_blk, 1,
		    	    psf_trmwhite(sizeof(DB_TAB_NAME),
				(char *) &tbl_spec->pss_obj_name),
			    &tbl_spec->pss_obj_name);
	...
	*/
	...
	{
	    _VOID_ psf_error(E_PS0424_ILLEGAL_TBL_REF, (DB_ERROR *)NULL, 
	...
	{
	    _VOID_ psf_error(err_num, (DB_ERROR *)NULL, PSF_USERERR, 
    ...
    */
    ...
    if (rngtab->pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX)
    {
	psq_cb->psq_mode = PSQ_X100_CRINDEX;
	dmu_cb->dmu_flags_mask |= DMU_BASE_NOFILE;
    }
    return(E_DB_OK);
}

	...
	bool		cond_add)
{
	...
	{
	    (VOID) psf_error(2111L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 1, sizeof(sess_cb->pss_lineno), &sess_cb->pss_lineno);
	}
	else
	{
	    (VOID) psf_error(5328L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                             err_blk, 0);
	}
	return (E_DB_ERROR);
    }
    ...
    {
	_VOID_ psf_error(5302L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
    ...
    */
    ...
    {
	_VOID_ psf_error(2105L, (DB_ERROR *)NULL, PSF_USERERR, 
            &err_code, err_blk, 2,
		...
		DB_ATT_MAXNAME, (*key)->key_attr_name.db_att_name) == 0)
	{
	    ...
	    */
	    ...
	    {
	        _VOID_ psf_error(5307L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &err_code, err_blk, 1, STlength(colname), colname);
	        return (E_DB_ERROR);
	    }
	}
    }
    ...
    if (DB_FAILURE_MACRO(status))
    {
        ...
        {
	    _VOID_ psf_error(2180L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
	...
	PSQ_CB		*psq_cb)
{
    ...
    */
	...
	!(sess_cb->pss_ses_flag & PSS_CATUPD))
    {
	_VOID_ psf_error(5116L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
            err_blk, 3, qry_len, qry,
	...
	MEcmp((PTR) &index_spec->pss_owner, (PTR) &sess_cb->pss_user,
	      sizeof(DB_OWN_NAME)))
    {
	_VOID_ psf_error(E_PS0421_CRTOBJ_NOT_OBJ_OWNER, (DB_ERROR *)NULL, 
