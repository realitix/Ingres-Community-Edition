...
**	    pslscanprep.awk.
**	29-apr-2010 (dougi)
**	    Added "set [no]vectorwise_default" & "set [no]x100_default".
...
# ifdef BYTE_ALIGN
	...
	case '5': case '6': case '7': case '8': case '9':
		...
		*/
                ...
                if (CVaf((char *) number, decimal, &double_number) != OK)
		{
		    ...
		    _VOID_ psf_error(2707L, (DB_ERROR *)NULL, PSF_USERERR, 
                        &err_code,
		...
		left = &pss_cb->pss_symblk->pss_symdata[PSS_SBSIZE] - start;
			...
			*/
			...
			{
			    _VOID_ psf_error(E_PS0376_INSUF_MEM, 
		...
		*(pss_cb->pss_symnext + dst_len) = EOS;
		...
		{
		    pss_cb->pss_lineno += lineno;
		    _VOID_ psf_error(2702L, (DB_ERROR *)NULL, PSF_USERERR, 
	...
	case PSQ_HV:
		...
		*/
		...
		else
		{
		    ...
		    {
			_VOID_ psf_error(2716L, (DB_ERROR *)NULL, PSF_USERERR, 
                            &err_code, &psq_cb->psq_error, 1,
			    (i4) sizeof(pss_cb->pss_lineno),
			    &(pss_cb->pss_lineno));
			return (-1);
	...
	if (CMnmstart(next_char) || *next_char == '_')
	{
		...
		*/
		...
		if (length > DB_MAXNAME)
		{
		    ...
		    *word = '\0';
		    _VOID_ psf_error(2703L, (DB_ERROR *)NULL, PSF_USERERR,
			&err_code, &psq_cb->psq_error, 2,
	...
	** it's a name, to be recycled if necessary in parser. */
	...
	pss_cb->pss_lineno += lineno;
	_VOID_ psf_error(2708L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 2,
