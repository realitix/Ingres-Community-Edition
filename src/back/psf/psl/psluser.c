...
)
{
	...
	if (STcasecmp(word, "NOGROUP") == 0)
	{
	    ...
	    {
		(VOID) psf_error(6351L, (DB_ERROR *)NULL, PSF_USERERR, 
	   ...
	   {
		(VOID) psf_error(6353L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &err_code, &psq_cb->psq_error, 2,
	...
	{
	    (VOID) psf_error(6353L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 2,
		sizeof(CRTALTUSRPRO)-1,
		CRTALTUSRPRO,
...
)
{
	...
	else if (STcasecmp(word1, "EXPIRE_DATE")==0)
	{
	   ...
	   */
	   ...
	   {
	    	(VOID) psf_error(4302, (DB_ERROR *)NULL, PSF_USERERR,
	   ...
	   */
	   if(!(psy_cb->psy_usflag & (PSY_CPROFILE|PSY_APROFILE)))
	   {
		    ...
		    */
	           ...
	           date_now.db_prec	    = 0;
		   date_now.db_collID	    = DB_UNSET_COLL;
...
)
{
	...
	if (STcasecmp(word1, "PASSWORD") == 0)
	{
	   ...
	   */
	   ...
	   {
		(VOID) psf_error(6374L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &err_code, &psq_cb->psq_error, 1,
...
)
{
	...
	*priv_var=0;
	...
	{ /*
	  ** Not found 
	  */
	  (VOID) psf_error(6288, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 1,
		(i4) STtrmwhite(word), word);
	  return (E_DB_ERROR);    /* non-zero return means error */
	}
	...
	for( p=0; privlist[p].privname; p++)
	{
		if(privs & privlist[p].priv )
		{
			if(privlist[p].flags & PSL_PRV_AUDIT)
			{
				...
				** Can't specify audit priv with privs
				*/
				(VOID) psf_error(6353L, (DB_ERROR *)NULL, 
...
)
{
	...
	else if (STcasecmp(word1, "on_logfull")==0)
	{
		...
		(VOID) psf_error(6350L, (DB_ERROR *)NULL, PSF_USERERR,
		    &err_code, &psq_cb->psq_error, 1,
	    ...
	    */
	    ...
	    {
		(VOID) psf_error(E_PS0F87_SET_ON_USER_ERROR, (DB_ERROR *)NULL,
...
)
{
	...
	*/
	...
	{
            _VOID_ psf_error(E_PS0F83_SET_ONERROR_STMT, (DB_ERROR *)NULL, 
...
)
{
	...
	else
	{
	    ...
	    */
	    ...
	    {
		_VOID_ psf_error(E_PS0F83_SET_ONERROR_STMT, (DB_ERROR *)NULL, 
                    PSF_USERERR, &err_code, &psq_cb->psq_error, 1, (i4) 
			    STlength(word2), word2);
		return (E_DB_ERROR);    /* non-zero return means error */
	    }
	}
