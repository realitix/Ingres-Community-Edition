	...
	PSQ_CB		*psq_cb)
{
    ...
    {
	(VOID) psf_error(6247L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 0);
	/*
	...
	DB_ERROR    *err_blk)
{
	...
	*char_type = MAXLOCKS;
    ...
    {
	(VOID) psf_error(5928L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	DB_ERROR    *err_blk)
{
    ...
    if (not_distr)
    {
	    ...
	    MAX_LOCKMODE_CHARS)
    	{
	    (VOID) psf_error(5931L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                             err_blk, 0);
	    return (E_DB_ERROR);	/* non-zero return means error */
    	}   
	...
	case LOCKLEVEL:
		...
		*val = DMC_C_TABLE;
		    ...
		    if (tbl->pss_tabdesc->tbl_pg_type == DB_PG_V1)
                    {
                        (VOID) psf_error(E_PS03B0_ILLEGAL_ROW_LOCKING, 
	...
	default:
	    ...
	    (VOID) psf_error(E_PS0351_UNKNOWN_LOCKPARM, (DB_ERROR *)NULL, 
	...
	(VOID) psf_error(err_no, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
            err_blk, 1, (i4) STlength(char_val), char_val);
	return (E_DB_ERROR);	/* non-zero return means error */
    }
    else
    {
	...
	DB_ERROR    *err_blk)
{
	...
	case TIMEOUT:
	{
	    ...
	    */
		...
		{
		    (VOID) psf_error(E_PS0352_ILLEGAL_0_TIMEOUT, (DB_ERROR *)NULL,
	...
	(VOID) psf_error(err_no, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
            err_blk, 1, (i4) STlength(num_buf), num_buf);
	return (E_DB_ERROR);    /* non-zero return means error */
    }
    else if (not_distr)
    {
	...
	DB_ERROR    	*err_blk)
{
    ...
    else
    {
    	...
    	*/
    	...
    	{
	    (VOID) psf_error(5986L, (DB_ERROR *)NULL, PSF_USERERR, 
                             &err_code, err_blk, 0);
	    return (E_DB_ERROR);
    	}
