    ...
    */
	...
	default:
	{
	    ...
	    */
	    ...
	    db_data.db_collID	= attr->attr_collID;
	    ...
	    */
		...
		(void) psf_error(E_PS04A7_UDT_NO_GETEMPTY,
				 ...
				 PSF_INTERR, &err_code,
                                 err_blk, 3, sizeof(ERx("CREATE TABLE")) -1,
				 ERx("CREATE TABLE"),
	    ...
	    */
	    ...
	    conv_func_blk.adf_r_dv.db_collID   = DB_UNSET_COLL;
		...
		*/
		...
		if (err_blk->err_code == E_US1A25_ID_TOO_LONG)
		{
		    ...
		    (void) psf_error(E_PS04A0_DEFAULT_TOO_LONG, 
		...
		else 
		{
		    ...
		    (void) psf_error(err_blk->err_code, (DB_ERROR *)NULL, 
		...
		DB_ERROR	*err_blk)
{
    ...
    */
	...
	default:
	    (void) psf_error(E_PS049B_BAD_DEFAULT_FUNC, (DB_ERROR *)NULL, 
	...
	*/
		...
		&& (dtype != DB_LVCH_TYPE))))
	{
	    ...
	    (void) psf_error(E_PS0498_DEFAULT_USER_SIZE, (DB_ERROR *)NULL, 
	...
	*/
	...
	if (abstype != DB_INT_TYPE && abstype != DB_FLT_TYPE
	  && abstype != DB_DEC_TYPE)
	{
	    (void) psf_error(E_US0805_2053_SEQDEF_TYPE, (DB_ERROR *)NULL, 
	...
	*/
	psf_error(2076L, (DB_ERROR *)NULL, PSF_USERERR,
		  &err_code, err_blk, 1,
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	{
	    (void) psf_error(E_PS04A4_INT_ERROR, (DB_ERROR *)NULL, PSF_INTERR,
			     &err_code, err_blk, 2,
			     sizeof(ERx("CREATE TABLE"))-1,
			     ERx("CREATE TABLE"),
    ...
    */
	...
	(void) psf_error(E_PS04A0_DEFAULT_TOO_LONG, (DB_ERROR *)NULL, 
                         PSF_USERERR, &err_code, err_blk, 2,
			 psf_trmwhite(sizeof(DB_ATT_NAME), 
				      attr->attr_name.db_att_name),
			 attr->attr_name.db_att_name,
    ...
    resultval.db_collID   = attr->attr_collID;
    ...
    */
    ...
    if (dataval->db_datatype == DB_FLT_TYPE)
    {
      ...
      */
      ...
      dataval->db_prec = 0;
      dataval->db_collID = DB_UNSET_COLL;
    }
	...
	*/
	...
	case E_AD2009_NOCOERCION:
	    (void) psf_error(E_PS0499_BAD_DEFAULT_VALUE, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 1, 
			     psf_trmwhite(sizeof(DB_ATT_NAME), 
					  attr->attr_name.db_att_name),
			     attr->attr_name.db_att_name);
	...
	case E_AD1090_BAD_DTDEF:
	    (void) psf_error(E_PS049A_BAD_DEFAULT_SIZE, (DB_ERROR *)NULL, 
                             PSF_USERERR, &err_code, err_blk, 1, 
			     psf_trmwhite(sizeof(DB_ATT_NAME), 
					  attr->attr_name.db_att_name),
			     attr->attr_name.db_att_name);
	...
	case E_AD1020_BAD_ERROR_LOOKUP:
	    (void) psf_error(E_PS0103_ERROR_NOT_FOUND, (DB_ERROR *)NULL, 
		...
		DB_ERROR	*err_blk)
{
	...
	*/
	...
	if (dtinfo & AD_USER_DEFINED)
	{
	    ...
	    (void) psf_error(E_PS049F_NO_PERIPH_DEFAULT, 
                             (DB_ERROR *)NULL, PSF_USERERR,
			     &err_code, err_blk, 2,
			     psf_trmwhite(sizeof(DB_ATT_NAME), 
					  attr->attr_name.db_att_name),
		   ...
		   DB_ERROR	*err_blk)
{
	...
	*/
    ...
    if (status != E_DB_OK)
    {
	psf_error(E_PS04B3_BAD_SEQOP_DEFAULT, (DB_ERROR *)NULL, PSF_USERERR,
		&err_code, err_blk, 1,
		length, defvalue);
	return (E_DB_ERROR);
    }
		      ...
		      DB_ERROR 	    *err_blk)
{
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	{
		...
		(void) psf_error(E_PS049C_NO_DEFAULT_FROM_RDF, (DB_ERROR *)NULL,
                                 PSF_INTERR, &err_code, err_blk, 1, 
		...
		(void) psf_error(E_PS049D_RDF_CANT_READ_DEF, (DB_ERROR *)NULL,
                                 PSF_INTERR, &err_code, err_blk, 1, 
				 psf_trmwhite(sizeof(DB_TAB_NAME), 
					      rngvar->pss_tabname.db_tab_name),
				 rngvar->pss_tabname.db_tab_name);
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	{
	    (void) psf_error(err_blk->err_code, (DB_ERROR *)NULL, PSF_INTERR,
			     &err_code, err_blk, 2,
...
**	22-jan-2010 (dougi)
**	    Add code to prevent addition of X100 join index columns.
		   ...
		   DB_ERROR	*err_blk)
{
    ...
    i4		err_code;
    i4			mandatory;
    bool		x100tab = (resrng->pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX);
	...
	*/
	if ((att->att_flags & DMU_F_SYS_MAINTAINED) && !x100tab)
	    continue;
	...
	*/
	...
	/* if a column is HIDDEN and in an X100 table, it must be a
	** join index column abd isn't insertable
	*/
	if ((att->att_flags & DMU_F_HIDDEN) && x100tab)
	    continue;

	/* if a column is identity in an X100 table and this is insert ...
	** select, it is in error. */
	if ((att->att_flags & (DMU_F_IDENTITY_ALWAYS |
		DMU_F_IDENTITY_BY_DEFAULT)) && x100tab &&
	    root_node->pst_sym.pst_value.pst_s_root.pst_tvrc > 0)
	{
	    (void) psf_error(9473L, 0L, PSF_USERERR, &err_code,
				 err_blk, 0);
	    return(E_DB_ERROR);
	}
	...
	*/
	if (mandatory)
	{
		...
		(void) psf_error(2781L, (DB_ERROR *)NULL, PSF_USERERR, 
                                 &err_code, err_blk, 3,
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	    return (status);
	resdom_node->pst_sym.pst_dataval.db_collID = att->att_collID;
	...
	*pnode = resdom_node;
	...
	{
	    /* should never happen */
	    psf_error(2130L, (DB_ERROR *)NULL, PSF_INTERR, &err_code,
