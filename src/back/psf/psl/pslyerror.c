...
**      12-Nov-2009 (gefei01)
**          Added error handling for PSQ_X100_CRINDEX.
...
**          Make sure the correct error code is set under PSQ_SNUMOVFL.
**/
	...
	PSQ_CB		   *psq_cb)
{
    ...
    case 1:
      psf_error(E_PS030F_BACKUP, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	&psq_cb->psq_error, 0);
      break;

    case 2:
      psf_error(2800L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	else if (qmode == PSQ_SETROLE)
	{
        ...
        else if (qmode == PSQ_SNUMOVFL)
        {
                /* SET NUMERIC_OVERFLOW */
                err_no=9415L;
        }
	...
	else if (qmode == PSQ_RREVOKE)
	{
        ...
        else if (qmode == PSQ_X100_CRINDEX)
        {
		/* CREATE index (on VW table). */
		err_no=9472;
        }
	else
	{
	    ...
	    err_no = ((qmode < 100) ? (2500 + qmode)
				    : (3700 + qmode));
	    if (err_no == 2549) err_no = 2500;	/* set vectorwise-default hack */
	}
      }
      else
      {
	...
	else 
	{
      ...
      psf_error(err_no, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	&psq_cb->psq_error, 2, (i4) sizeof(lineno), &lineno,
	0, buf);
      break;

    default:
      psf_error(E_PS0311_YACC_UNKNOWN, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	&psq_cb->psq_error, 0);
      break;
    }
	...
	PSQ_CB	    *psq_cb)
{
    ...
    psf_error(error, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	&psq_cb->psq_error, 2, (i4) sizeof(lineno), &lineno,
	0, buf);
