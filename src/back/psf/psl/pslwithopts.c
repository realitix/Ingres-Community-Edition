	...
	*/
	{
	    ...
	    ** Vectorwise_ix is also disallowed.
	    */
	    if ((qmode == PSQ_CREATE || qmode == PSQ_DGTT)
	      && storestruct != DB_HEAP_STORE
	      && (storestruct <= DB_STDING_STORE_MAX || storestruct == DB_X100IX_STORE))
	    {
	    ...
	    /* VW base table only allows VW index. */
	    ...
	    /* Ingres base table only allows Ingres index */
	    if (qmode == PSQ_INDEX && storestruct > DB_STDING_STORE_MAX)
	    {
		(void) psf_error(E_US1947_6465_X_NOTWITH_Y,
			    0, PSF_USERERR, &err_code, err_blk, 3,
			    qry_len, qry,
			    0, (char *) valueptr,
			    0, "an Ingres-structured base table");
		return (E_DB_ERROR);
	    }
	...
	*/
	...
	if (sstruct > DB_STDING_STORE_MAX)
	{
	    ...
	    */
		...
		BTclear(DMU_TEMP_TABLE, temp_ind);
		BTclear(DMU_RECOVERY, temp_ind);
	    ...
	    */
	    ...
	    /* Vectorwise index doesn't support UNIQUE */
	    if (sstruct == DB_X100IX_STORE && BTtest(DMU_UNIQUE, dmu_indicators))
	    {
		(void) psf_error(9483, 0, PSF_USERERR, &err_code, err_blk, 0);
		return (E_DB_ERROR);
	    }

	    /* Make all of the user WITH-options go away except for structure=.
	    ** This is to ignore options on an implicit vectorwise struct.
	    ** System defined WITH-options (such as temp-table) should remain,
	    ** so we do this with individual bit clearing.  (Ugh.)
	    */
	    BTclear(DMU_AUTOSTRUCT, dmu_indicators);
	    BTclear(DMU_PERSISTS_OVER_MODIFIES, dmu_indicators);
	    BTclear(DMU_STATEMENT_LEVEL_UNIQUE, dmu_indicators);
	    BTclear(DMU_DATAFILL, dmu_indicators);
	    BTclear(DMU_IFILL, dmu_indicators);
	    BTclear(DMU_LEAFFILL, dmu_indicators);
	    BTclear(DMU_MINPAGES, dmu_indicators);
	    BTclear(DMU_MAXPAGES, dmu_indicators);
	    BTclear(DMU_ALLOCATION, dmu_indicators);
	    BTclear(DMU_EXTEND, dmu_indicators);
	    BTclear(DMU_PAGE_SIZE, dmu_indicators);
	    BTclear(DMU_TABLE_PRIORITY, dmu_indicators);
	    BTclear(DMU_DIMENSION, dmu_indicators);
	    BTclear(DMU_DCOMPRESSION, dmu_indicators);
	    BTclear(DMU_KCOMPRESSION, dmu_indicators);
	    BTclear(PSS_WC_COMPRESSION, dmu_indicators);
	    BTclear(PSS_WC_RANGE, dmu_indicators);
	    BTclear(PSS_WC_ROW_SEC_AUDIT, dmu_indicators);
	    BTclear(PSS_WC_ROW_SEC_KEY, dmu_indicators);
	    BTclear(PSS_WC_SEC_AUDIT, dmu_indicators);
	    yyvars->withkey_count = 0;
	    /* Force "with nojournaling" */
	    BTset(DMU_JOURNALED, dmu_indicators);
	    dmuchar->dmu_journaled = DMU_JOURNAL_OFF;
	    /* Force "with duplicates" */
	    BTset(DMU_DUPLICATES, dmu_indicators);
	    dmuchar->dmu_flags |= DMU_FLAG_DUPS;
	    /* Ensure no compression regardless of what might have been set */
	    dmuchar->dmu_dcompress = dmuchar->dmu_kcompress = DMU_COMP_OFF;
	} /* vectorwise checks */
