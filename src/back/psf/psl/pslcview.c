/*
** Copyright (c) 1985, 2004, 2010 Actian Corporation
*/
...
**          psl_cv1_create_view: Dont RCB_INVALIDATE X100 view base tables
...
**	17-nov-2009 (dougi)
**	    Add support for DMU_X100_INVIEW flag.
	...
	PSS_YYVARS	    *yyvarsp)
{
    ...
    bool		vbase_invalidate;

    if (sess_cb->pss_stmt_flags2 & PSS_2_X100TAB_INQ)
	BTset(DMU_X100_INVIEW, dmu_cb->dmu_chars.dmu_indicators);
	...
	*/
	...
	(VOID) psf_error(err_no, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 1,
	...
	*/
	...
	if (!updatable_attr)
	{
	    ...
	    */
	    (VOID) psf_error(W_PS058D_WCO_AND_NO_UPDATABLE_COLS, 
                (DB_ERROR *)NULL, PSF_USERERR, &err_code, &psq_cb->psq_error, 
                1, psf_trmwhite(sizeof(crt_view->pst_view_name),
		    (char *) &crt_view->pst_view_name),
		(PTR) &crt_view->pst_view_name);
    ...
    */
	    ...
	    )
	{
	    ...
	    {	/* not equal # of nodes */
		(VOID) psf_error(2051L, (DB_ERROR *)NULL, PSF_USERERR, 
	...
	*/
	...
	{
	    (VOID) psf_error(2052L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 1,
	     ...
	     i++, attr_pp++)
	{
		...
		if (sqlview)
		{
		    (VOID) psf_error(2013L, (DB_ERROR *)NULL, PSF_USERERR, 
	...
	*/
	...
	else	/* column of view is based on a column of underlying table */
	{
	    ...
	    if (status != E_DB_OK)
	    {
		(VOID) psf_error(E_PS0909_NO_RANGE_ENTRY, (DB_ERROR *)NULL, 
	    ...
	    {
		(VOID) psf_error(E_PS090A_NO_ATTR_ENTRY, (DB_ERROR *)NULL, 
                    PSF_INTERR, &err_code, &psq_cb->psq_error, 1,
		    psf_trmwhite(DB_ATT_MAXNAME, name), name);
		goto cleanup;
	    }
	...
	*/
	...
	for (i = 0; i < qeuq_cb->qeuq_ano; i++)
	{
		...
		*/
		...
		col_width.db_collID = attr->attr_collID;
		...
		*/
		...
		dv.db_prec = attr->attr_precision;
		dv.db_collID = attr->attr_collID;
...
cleanup:
	...
	*/
    ...
    /*
    ** DON'T RDF_INVALIDATE X100 view base tables
    **
    ** RDF_INVALIDATE should not be done without X table lock
    ** If view base is an ingres table we have X table lock
    ** If view base is an X100 table we don't have X table lock
    **
    ** Note: even for Ingres vbase tables, RDF_INVALIDATE vbase tables
    ** to set relstat & TCB_VBASE is not strictly necessary
    ** The TCB_VBASE setting is only looked at by help table
    ** which does not get the value from the rdf cache anyway
    */
    if (sess_cb->pss_stmt_flags2 & PSS_2_X100TAB_INQ)
	vbase_invalidate = FALSE;
    else
	vbase_invalidate = TRUE;

    if (vbase_invalidate && DB_SUCCESS_MACRO(status))
    {
	/* 
...
**	17-nov-2009 (dougi)
**	    Add room for DMU_X100_INVIEW flag.
