...
**	9-march-2009 (dougi)
**	    Added "set [no]reuse" to control use of OPF "reuse" logic (X100
**	    only, for now).
**      10-mar-2009 (stial01)
**          Added unorm error handling (b121769)
**	7-may-2009 (dougi)
**	    Added "set [no]x100_default" to control defaulting table
**	    structures in CREATE TABLE.
**	24-sep-2009 (joea)
...
**	    Added SRID keyword.
**	9-mar-2010 (dougi)
**	    Added double word - "call vectorwise".
...
**	15-sep-2010 (dougi)
**	    Add "set [no]vectorwise_row_default".
**	08-Nov-2010 (kiria01) SIR 124685
**	    Rationalise function prototypes
**	19-Nov-2010 (kiria01) SIR 124690
**	    Add support for SET SESSION *COLLATION*
**	09-Dec-2010 (kiria01) SIR 124826
**	    Added ASSERT. Actual tag will be ASSERTKWD to avoid confusion
**	    will ASSERT macros.
...
**	    Implement 'set [no]hashjoin' and 'set [no]hashagg'.
**	6-may-2011 (dougi)
**	    Add "over", "partition by" for window functions.
**	4-june-2011 (dougi) Code sprint
**	    Add "decode" for Oracle DECODE function.
...
/*
** This table contains the words and the corresponding tokens and values that
** can follow the introductory word "If" to form a keyword.
*/
static const SECONDARY      Ifwords[] = {
		       { "exists",	IFEXISTS,   PSL_GOVAL }
					     };

#define             IFSIZE		(sizeof(Ifwords) / sizeof(SECONDARY))

/*
...
static const SECONDARY      Setwords[] = {
			...
			{ "hashagg",      SETHASHAGG,  PSL_ONSET  },
			{ "hashjoin",     SETHASHJOIN, PSL_ONSET  },
			...
			{ "nohashagg",    SETHASHAGG,  PSL_OFFSET },
			{ "nohashjoin",   SETHASHJOIN, PSL_OFFSET },
			...
			{ "nounicode_substitution",	  SETUNICODESUB, PSL_OFFSET },
                        { "numeric_overflow", SETNUMOVFL, PSL_ONSET },
...
/*
** This table contains the words and the corresponding tokens and values that
** can follow the introductory word "CALL" to form a keyword.
*/
static const SECONDARY      Callwords[] = {
			{ "vectorwise", IVWCALL, 0}
					     };

#define             CALLSIZE		(sizeof(Callwords) / sizeof(SECONDARY))

/*
** This table contains the words and the corresponding tokens and values that
** can follow the introductory word "PARTITION" to form a keyword.
*/
static const SECONDARY      Partitionwords[] = {
			{ "by", PARTBY, 0}
					     };

#define             PARTITIONSIZE	(sizeof(Partitionwords) / sizeof(SECONDARY))

/*
...
/* 27 */	       {    IF,	    0,		IFSIZE,	(SECONDARY *) Ifwords   },
...
/* 220 */	       { /* CALL */ 0,	    0, CALLSIZE,(SECONDARY *) Callwords},
/* 221 */              { SRID,		    0,  0,      (SECONDARY *)NULL },
/* 222 */              { ASSERTKWD,	    0,  0,      (SECONDARY *)NULL },
/* 223 */	       { OVER,		    0,	0,	(SECONDARY *)NULL },
/* 224 */	       { /* PARTITION */ 0, 0,	PARTITIONSIZE, (SECONDARY *) Partitionwords},
/* 225 */	       { DECODE,	    0,	0,	(SECONDARY *)NULL },
};

...
common_datetime_lit:
	   {
		...
		while(!close_quote)
		{
		    ...
		    {
			_VOID_ psf_error(2700L, (DB_ERROR *)NULL, PSF_USERERR,
			...
			*/
			...
			CMnext(next_char);
  		    ...
  		    {
  		  	pss_cb->pss_lineno += lineno;
  			_VOID_ psf_error(2701L, (DB_ERROR *)NULL, PSF_USERERR,
  			    &err_code, &psq_cb->psq_error, 1,
  			    (i4) sizeof(pss_cb->pss_lineno),
  			    &pss_cb->pss_lineno);
  			return (-1);
  			...
  			*/
  			...
  			{
  			    psf_error(E_PS0376_INSUF_MEM, (DB_ERROR *)NULL, 
		...
		** for name/symbol validation */
		...
		for (;;)
		{
			...
			default:
			    ...
			    pss_cb->pss_lineno += lineno;
			    psf_error(2709L, (DB_ERROR *)NULL, PSF_USERERR,
...
# endif /* BYTE_ALIGN */
	    ...
	    */
	    ...
	    {
		pss_cb->pss_lineno += lineno;
		psf_error(2707L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		    &psq_cb->psq_error, 2,
		    (i4) sizeof(pss_cb->pss_lineno),
		    &(pss_cb->pss_lineno),
        ...
        case 'N': case 'n':
	{
		...
		/* utoken == uchunk here */
			...
			** escape characters. */
			...
			CMnext(next_char);	/* next char to process */
                	...
                	if (next_char > qry_end)
			{
			    _VOID_ psf_error(2700L, (DB_ERROR *)NULL, 
			...
			*/
			...
			{
			    psf_error(E_PS0376_INSUF_MEM, (DB_ERROR *)NULL, 
                                    PSF_INTERR, &err_code, &psq_cb->psq_error, 
                                    0);
			    return (-1);
    			}
			...
			** append to token. */
			...
			if (DB_FAILURE_MACRO(status))
			{
			    ...
			    {
				_VOID_ psf_error(2732L, (DB_ERROR *)NULL, 
                                    PSF_USERERR, &err_code, &psq_cb->psq_error,
                                    1, (i4) sizeof(pss_cb->pss_lineno),
				    &pss_cb->pss_lineno);
			    }
			    return(-1);
			}
			...
			** point of token. */
			...
			for (i = 0; i < hcount; i++)
			{
				...
				break;
			      default:
				/* Syntax error. */
				_VOID_ psf_error(2729L, (DB_ERROR *)NULL, 
                                    PSF_USERERR,
				    &err_code, &psq_cb->psq_error, 1,
				    (i4) sizeof(pss_cb->pss_lineno),
				    &pss_cb->pss_lineno);
		...
		*/
		...
		if (DB_FAILURE_MACRO(status))
		{
		    psf_error(psq_cb->psq_error.err_code, (DB_ERROR *)NULL, 
        ...
        case '$':
	{
		...
		*/
		...
		{
		    pss_cb->pss_lineno += lineno;
		    psf_error(2702L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	case '|':
	    ...
	    nonwhite++;
		    ...
		    CMnext(next_char);
                ...
                if (endcomment == 0)
		{
		    _VOID_ psf_error(2705L, (DB_ERROR *)NULL, PSF_USERERR,
			&err_code, &psq_cb->psq_error, 1,
			(i4) sizeof(pss_cb->pss_lineno),
			&pss_cb->pss_lineno);
		    return (-1);
	...
	case PSQ_HV:
		...
		*/
		...
		else
		{
		    ...
		    {
			psf_error(2716L, (DB_ERROR *)NULL, PSF_USERERR, 
                            &err_code, &psq_cb->psq_error, 1,
			    (i4) sizeof(pss_cb->pss_lineno),
			    &(pss_cb->pss_lineno));
			return (-1);
	...
	if (CMnmstart(next_char) || *next_char == '_')
	{
		...
		*/
		...
		if (length > DB_MAXNAME)
		{
		    /* Name too long */
		    *word = '\0';
		    psf_error(2703L, (DB_ERROR *)NULL, PSF_USERERR,
			&err_code, &psq_cb->psq_error, 2,
			(i4) sizeof(pss_cb->pss_lineno),
			&(pss_cb->pss_lineno),
	    ...
	    *mark = -1;
		...
		for (keylen = 0; ; keylen++)
			...
			if (keylen == length)
			{
			    ...
			    ** DGTT-AS -> X100-DGTT-AS probably hasn't happened
			    ** yet, but no harm in checking for it too.
			    */
			    if ((tret->token != SET ||
	...
	*/
	...
	nonwhite++;
	pss_cb->pss_lineno += lineno;
	psf_error(2708L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 2, sizeof (pss_cb->pss_lineno), 
...
i4	    token)
{
	...
	if (dv1.db_length > (maxlen + DB_CNTSIZE))
	{
		...
		_VOID_ psf_error(9412L, (DB_ERROR *)NULL, PSF_USERERR, &error,
...
DB_STATUS err_status)
{
    ...
    */
	...
	_VOID_ psf_error(2710L, (DB_ERROR *)NULL, PSF_USERERR, &error,
