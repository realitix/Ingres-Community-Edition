/*
** Copyright 2004, 2010 Actian Corporation
*/
...
psl_partdef_end(PSS_SESBLK *sess_cb, PSQ_CB *psq_cb, PSS_YYVARS *yyvarsp)
{
    ...
    */
	...
	if (dim_ptr->nparts == 1)
	{
	    (void) psf_error(E_US1951_6481_PART_ONEP, (DB_ERROR *)NULL, 
                        PSF_USERERR, &toss_err, &psq_cb->psq_error, 2,
    ...
    */
    ...
    {
	i = MAXI2;
	(void) psf_error(E_US1949_6467_PART_TOOMANYP, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 3,
		yyvarsp->qry_len, yyvarsp->qry_name,
    ...
    */
    ...
    {
	(void) psf_error(E_US1940_6458_PART_INVALID, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 2,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, errbuf);
	return (E_DB_ERROR);
	...
	PSS_YYVARS *yyvarsp, i4 distrule)
{
    ...
    {
	i = DBDS_MAX_LEVELS;
	(void) psf_error(E_US1933_6451_PART_TOOMANY, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 2,
		yyvarsp->qry_len, yyvarsp->qry_name,
		sizeof(i), &i);
	return (E_DB_ERROR);
	...
	PSS_YYVARS *yyvarsp, i4 nparts)
{
	...
	*/
	...
	{
	    (void) psf_error(E_US1936_6454_PART_NNAMES, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 3,
		yyvarsp->qry_len, yyvarsp->qry_name,
	...
	PSS_YYVARS *yyvarsp, char *colname)
{
	...
	(void) psf_error(E_US1934_6452_PART_BADCOL, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 5,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, "Column",
	...
	*/
    ...
    if (DB_FAILURE_MACRO(status))
    {
	(void) psf_error(E_US1935_6453_PART_BADCOLTYPE, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 2,
		yyvarsp->qry_len, yyvarsp->qry_name,
		STlength(colname), colname);
	return (E_DB_ERROR);
	...
	PSS_YYVARS *yyvarsp, char *pname, bool allow_ii)
{
	...
	(void) psf_error(E_PS0459_ILLEGAL_II_NAME, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 4,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, "partition",
...
**	    Fix up a couple prototype issues.
*/
	...
	PSS_YYVARS *yyvarsp, i4 sign_flag, DB_DATA_VALUE *value)
{
    ...
    */
	...
	if (yyvarsp->textptrs_cur != 0)
	{
	    (void) psf_error(E_US1930_6448_PARTITION_EXPECTS, (DB_ERROR *)NULL,
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 3,
		yyvarsp->qry_len, yyvarsp->qry_name,
	...
	*/
	...
	if (part_entry_ptr->type > 0 && psq_cb->psq_mode != PSQ_RETINTO)
	{
	    ...
	    (void) psf_error(E_US1938_6456_PART_VALTYPE, (DB_ERROR *)NULL, 
                        PSF_USERERR, &toss_err, &psq_cb->psq_error, 4,
			yyvarsp->qry_len, yyvarsp->qry_name,
    ...
    */
	...
	(void) psf_error(E_US1937_6455_PART_VALTOOLONG, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, &psq_cb->psq_error, 5,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, what,
		0, buf,
	...
	*/
	{
		...
		if (DB_FAILURE_MACRO(status))
		{
			...
			adfcb->adf_errcb.ad_errmsgp[adfcb->adf_errcb.ad_emsglen] = '\0';
		    (void) psf_error(E_US1938_6456_PART_VALTYPE, 
...
type_error:
    adi_tyname(adfcb, col_type, &col_dtname);
    adi_tyname(adfcb, val_type, &val_dtname);
    ...
    (void) psf_error(E_US1938_6456_PART_VALTYPE, (DB_ERROR *)NULL, PSF_USERERR,
		&toss_err, &psq_cb->psq_error, 4,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, what, 0, col_name,
		0, buf);
	...
	PSS_YYVARS *yyvarsp)
{
    ...
    (void) psf_error(E_US1939_6457_PART_VALCOUNT, (DB_ERROR *)NULL, PSF_USERERR,
	&toss_err, &psq_cb->psq_error, 4,
	yyvarsp->qry_len, yyvarsp->qry_name,
...
ppd_ulm_errmsg(PSS_SESBLK *sess_cb, PSQ_CB *psq_cb)
{
	...
	(void) psf_error(E_PS0F02_MEMORY_FULL, (DB_ERROR *)NULL, PSF_CALLERR,
	...
	(void) psf_error(E_PS0A0D_ULM_ERROR, (DB_ERROR *)NULL, PSF_INTERR,
