/*
** Copyright (c) 1991, 2010 Actian Corporation
*/
...
**      12-Nov-2009 (gefei01)
**          Validate the WITH clause for X100 CREATE INDEX.
**      16-Nov-2009 (gefei01)
**          psl_ct3_crwith(): Allow global temp table
**          when vectorwise_default is set.
**	17-Nov-2009 (kschendel) SIR 122890
**	    resjour values expanded so that we can do suitable things
**	    with CTAS in a non-journaled DB.
**      04-Dec-2009 (gefei01)
**          CTAS doesn't allow the mix of X100 table and regular Ingres table.
**	07-Dec-2009 (troal01)
...
**	07-jun-2011 (stial01) m1984
**	    Move check for unsupported VectorWise datatypes to psl_ct3_crwith
...
**	    Flag attemts to create a table with unsupported collation
**	    on columns.
*/
DB_STATUS
	...
	PSS_CONS	*cons_list)
{
    ...
    */
    ...
    if (num_cols == 0)
    {
	(void) psf_error(E_PS04A5_NO_COLUMNS, (DB_ERROR *)NULL, PSF_USERERR,
			 &err_code, err_blk, 1,
    ...
    */
    ...
    {
	(void) psf_error(E_PS0C86_ENCRYPT_NOTABLVL, (DB_ERROR *)NULL, 
	...
	BTtest(PSS_WC_ENCRYPT, dmu_cb->dmu_chars.dmu_indicators))
    {
	(void) psf_error(E_PS0C87_ENCRYPT_NOCOLS, (DB_ERROR *)NULL, 
                PSF_USERERR, &err_code, err_blk, 1,
		psf_trmwhite(DB_TAB_MAXNAME,
			dmu_cb->dmu_table_name.db_tab_name),
		dmu_cb->dmu_table_name.db_tab_name);
...
**	14-Dec-2010 (troal01)
**	    Set srid to SRID_UNDEFINED as default.
*/
	...
	PSQ_CB		*psq_cb)
{
    ...
    */
	...
	for (;;)
	{
	    ...
	    {   /* not equal # of nodes */
		(VOID) psf_error(2001L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &toss_err, err_blk, 0);
		return (E_DB_ERROR);
	    }
	...
	*/
	...
	if ((right->pst_sym.pst_type == PST_CONST) &&
	    (abs(ssnode->pst_sym.pst_dataval.db_datatype)
						    == DB_LTXT_TYPE))
	{
	    (VOID) psf_error(2016L, (DB_ERROR *)NULL, PSF_USERERR,
		&toss_err, err_blk, 0);
	    return (E_DB_ERROR);
	}
	...
	*/
	if (right->pst_sym.pst_type != PST_VAR)
	{
		...
		*/
		...
		attr.attr_collID    = dbdata->db_collID;
...
**	    Corrected the column count for star.
*/
DB_STATUS
	...
	PST_QNODE	*offset_n)
{
    ...
    */
    ...
    /* pst-header can't set X100-target for CTAS, so we have to do it */
    if (dmu_cb->dmu_chars.dmu_struct > DB_STDING_STORE_MAX)
	tree->pst_mask1 |= PST_X100_TARGET;
...
**	semantic routines.
**
**	For a VectorWise structure, the with-option parser has already
**	turned off all (or nearly all) of the option indicators, since
**	none of the usual WITH-options are relevant for VW tables.
**	This is where we will change the query mode to indicate a
**	vectorwise table create.
**
...
**				    AS SELECT.
**	    PSQ_REG_IMPORT	    processing REGISTER .. AS IMPORT
**	    Other qmodes can get here, but we immediately return.
**
** Outputs:
**	May change psq_mode for VW creates.
...
**	26-aug-2008 (dougi)
**	    Add X100 support (no options).
**      16-NOv-2009 (gefei01)
**          Allow global temp table when vectorwise_default is set.
**      04-Dec-2009 (gefei01)
**          CTAS doesn't allow the mix of X100 table and regular Ingres table.
...
**	12-apr-2010 (dougi)
**	    Disallow all "with" options in conjunction with X100 default.
**	5-june-2010 (dougi)
**	    Remnove limitation on mixing native & IVW Ingres in CTAS.
**	8-july-2010 (dougi)
**	    Allow DGTT of an X100 table.
**	20-sep-2010 (dougi)
**	    No crwith call for X100 indexes, either.
**	24-Sep-2010 (kschendel)
**	    Only look at sess cb now for IVW defaults.
**	27-sep-2010 (dougi)
**	    Remove prohibition of CTAS from VW to Ingres native.
	...
	PSQ_CB		*psq_cb)
{
    ...
    ** can't be done.
    ** IVW tables force "with nojournaling" in the with-option handler.
    */
	...
	*/
	  ...
	  && yyvars->secaudkey == NULL)
	{
	    ...
	    */
	    (VOID) psf_error(6662, (DB_ERROR *)NULL, PSF_USERERR, 
                             &err_code, err_blk, 0);
	...
	*/
	   ...
	   ))
	{
		...
		*/
		(VOID) psf_error(2024L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &err_code, err_blk, 1,
		...
		*/
		...
		if( ++attr_count > DB_MAX_COLS)
		{
			...
			(VOID) psf_error(err_num, (DB_ERROR *)NULL, 
		...
		cur_attr->attr_collID = DB_UNSET_COLL;
    ...
    */
	...
	if (status != E_DB_OK)
	{
	    (VOID) psf_error(2179L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 2,
	...
	*keyptrs = keyentry;
    ...
    /* Revise the query mode if appropriate */
    if (dmu_cb->dmu_chars.dmu_struct > DB_STDING_STORE_MAX)
    {
	if (qmode == PSQ_DGTT)
	    psq_cb->psq_mode = PSQ_X100_DGTT;
	else if (qmode == PSQ_DGTT_AS_SELECT)
	    psq_cb->psq_mode = PSQ_X100_DGTT_AS;
	/* CREATE and RETINTO don't change the query mode, but they do
	** reset the statement type in the qtree header.  (DGTT variants
	** don't make a qtree header, who knows why.)
	*/
	if (sess_cb->pss_crt_tbl_stmt != NULL)
	    sess_cb->pss_crt_tbl_stmt->pst_type = PST_X100_CREATE_TABLE_TYPE;
	...
	char		*table_name)
{
	...
	if (cui_chk3_locname(loc_name) != OK)
	{
	    ...
	    (VOID) psf_error(2733, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	PSS_OBJ_NAME	*tbl_spec)
{
    ...
    */
	...
	{
	    (VOID) psf_error(E_PS0BD3_ONLY_ON_TEMP, (DB_ERROR *)NULL, 
	...
	*/
	...
	{
	    (VOID) psf_error(2129L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	*/
	if (psq_cb->psq_mode == PSQ_DGTT || psq_cb->psq_mode == PSQ_DGTT_AS_SELECT)
	{
	    ...
	    {
		(VOID) psf_error(2129L, (DB_ERROR *)NULL, PSF_USERERR, 
	...
	*/
	if (psq_cb->psq_mode == PSQ_CSYNONYM)
	{
	    ...
	    {
		(void) psf_error(E_PS0459_ILLEGAL_II_NAME, (DB_ERROR *)NULL, 
                                 PSF_USERERR, &err_code, &psq_cb->psq_error, 4,
	...
	else
	{
	    ...
	    */
		...
		(VOID) psf_error(5116L, (DB_ERROR *)NULL, PSF_USERERR, 
                                 &err_code, &psq_cb->psq_error, 3, qry_len, qry,
				 STlength(tbl_spec->pss_orig_obj_name),
				 tbl_spec->pss_orig_obj_name,
				 STlength(SystemCatPrefix), SystemCatPrefix);
		return (E_DB_ERROR);
	...
	*/
	if (*(sess_cb->pss_dbxlate) & CUI_ID_DLM_M)
	{
	    ...
	    {
		(void) psf_error(E_PS045F_WRONG_CASE, (DB_ERROR *)NULL, 
	...
	*/
	...
	if (err_num)
	{
	    ...
	    */
	    ...
	    {
		(VOID) psf_error(err_num, (DB_ERROR *)NULL, PSF_USERERR,
                    &err_code, &psq_cb->psq_error, 2, qry_len, qry,
		    STlength(tbl_spec->pss_orig_obj_name),
		    tbl_spec->pss_orig_obj_name);
	    }
	...
	PTR		*scanbuf_ptr)
{
	...
	if (sess_cb->pss_lang == DB_QUEL)
	{
	    (VOID) psf_error(5104L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 2, sizeof(dmu_cb->dmu_table_name),
		(char *) &dmu_cb->dmu_table_name,
		...
		sizeof((attrs[colno])->attr_name)))
	{
	    if (sess_cb->pss_lang == DB_QUEL)
	    {
		(VOID) psf_error(5105L, (DB_ERROR *)NULL, PSF_USERERR, 
                            &err_code, err_blk, 2, 
...
**	20-Jul-2011 (kiria01) SIR 124690
**	    Move collation checks & defaulting before adi_encode_colspec
**	    is called as collations can affect lengths required.
**	05-Sep-2011 (kiria01) b125573
**	    Update adi_encode_colspec call for collation data.
**	    Moved application of collation info and especially defaulting
**	    into adi_encode_colspec.
*/
DB_STATUS
psl_ct14_typedesc(
	PSS_SESBLK	*sess_cb,
	...
	i4		encrypt_spec)
{
    ...
    ** checked later. */
	...
	_VOID_ psf_error(3857L, (DB_ERROR *)NULL, PSF_USERERR,
    ...
    */
    ...
    def_collID[0] = sess_cb->pss_def_coll;
    def_collID[1] = sess_cb->pss_def_unicode_coll;
 
    status = adi_encode_colspec(adf_scb, type_name, num_len_prec_vals,
	len_prec, adi_flags, collationID, def_collID, &dt_dv);
    ...
    ** collation is supported for defined collation. */
    if (status == E_DB_OK && collationID > DB_NOCOLLATION)
    {
	...
	if (adf_scb->adf_errcb.ad_errcode == E_AD5065_DATE_ALIAS_NOTSET)
	 ...
	 /* send the text message to users */
         (VOID) psf_error(4320,
                         &adf_scb->adf_errcb.ad_dberror, PSF_USERERR, &err_code,
                         &psq_cb->psq_error, 0);
          return (E_DB_ERROR);
    	}
	else if ((len_prec) && 
		 ...
		 (adf_scb->adf_errcb.ad_errcode == E_AD2006_BAD_DTUSRLEN))
	{
	 ...
	 /* send the text message to users */
         (VOID) psf_error(2092, (DB_ERROR *)NULL,
	  ...
	  (VOID) psf_error((psq_cb->psq_mode == PSQ_REG_IMPORT)
	    ...
	    (DB_ERROR *)NULL, PSF_USERERR, &err_code, &psq_cb->psq_error, 2,
	    STtrmwhite(type_name), type_name,
	...
	(VOID) psf_error(2048L, (DB_ERROR *)NULL, PSF_USERERR, 
		&err_code, &psq_cb->psq_error, 0);
	return(E_DB_ERROR);
    }
			...
			DB_S_DECODE_MACRO(dt_dv.db_prec) != 0))
	{
	    ...
	    _VOID_ psf_error(E_PS04B7_BAD_IDENTITY_TYPE, (DB_ERROR *)NULL, 
	...
	if (null_def & (PSS_TYPE_DEFAULT | PSS_TYPE_USER_DEFAULT))
	{
	    ...
	    _VOID_ psf_error(E_PS04B8_IDENTITY_DEFAULT, (DB_ERROR *)NULL, 
                PSF_USERERR, &err_code, &psq_cb->psq_error, 1,
		psf_trmwhite(DB_ATT_MAXNAME, (char *) &cur_attr->attr_name),
		&cur_attr->attr_name);
	    return (E_DB_ERROR);
    ...
    cur_attr->attr_collID = dt_dv.db_collID;
    ...
    */
	...
	{
	    _VOID_ psf_error(E_PS0C85_ENCRYPT_INVALID, (DB_ERROR *)NULL, 
                PSF_USERERR, &err_code, &psq_cb->psq_error, 1,
		psf_trmwhite(DB_ATT_MAXNAME, (char *) &cur_attr->attr_name),
		&cur_attr->attr_name);
	    return (E_DB_ERROR);
    ...
    */
    ...
    {
	_VOID_ psf_error(5559L, (DB_ERROR *)NULL, PSF_USERERR,
		&err_code, &psq_cb->psq_error, 1,
		psf_trmwhite(DB_ATT_MAXNAME, (char *) &cur_attr->attr_name),
		&cur_attr->attr_name);
	return (E_DB_ERROR);
	...
	if (alt_attr == NULL)
	{
	    _VOID_ psf_error(5560L, (DB_ERROR *)NULL, PSF_USERERR,
		&err_code, &psq_cb->psq_error, 1,
		psf_trmwhite(DB_ATT_MAXNAME, (char *) &cur_attr->attr_name),
		&cur_attr->attr_name);
	    return (E_DB_ERROR);
	...
	*/
	...
	if ((cur_attr->attr_type > 0) && (alt_attr->att_type < 0))
        {
	    _VOID_ psf_error(3859L, (DB_ERROR *)NULL, PSF_USERERR,
	        &err_code, &psq_cb->psq_error, 0);
	    return (E_DB_ERROR);
	}
	...
	*/
	...
	{
	    (VOID) psf_error(E_US1901_6401_BAD_SYSMNT, (DB_ERROR *)NULL, 
                PSF_USERERR, &err_code, &psq_cb->psq_error, 2,
		STtrmwhite(type_name), type_name,
		psf_trmwhite(DB_ATT_MAXNAME, (char *) &cur_attr->attr_name),
		&cur_attr->attr_name);
	...
	PSQ_CB		*psq_cb)
{
    ...
    */
    ...
    {
	(VOID) psf_error(2083L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
			&psq_cb->psq_error, 0);
	return (E_DB_ERROR);
    }
    ...
    {
	(VOID) psf_error(2079L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
    ...
    if (qmode == PSQ_REG_LINK)
    {
	(VOID) psf_error(2080L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 1, STlength(name), name);
	return (E_DB_ERROR);
    }
...
((QED_DDL_INFO *)((QEU_CB *)sess_cb->pss_object)->qeu_ddl_info)->qed_d6_tab_info_p;
    ...
    {
	(VOID) psf_error(2087L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		&psq_cb->psq_error, 1, STlength(name), name);
	return (E_DB_ERROR);
    }
    else
	...
	*/
	...
	if (quoted_val)
	{
	    (VOID) psf_error(E_PS03A7_QUOTED_WITH_CLAUSE, (DB_ERROR *)NULL, 
...
**      10-Jan-2011 (hanal04) Bug 124908
**          Always reject duplicate default values. For a column default
		    ...
		    DB_ERROR	*err_blk)
{
	...
	*/
	...
	{
	    (VOID) psf_error(8003L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
			     err_blk, 1,
    ...
    ** may happen as result of UNIQUE/PRIMARY KEY constraint. */
    if (((qual1 & qual2) && !(qual1 & PSS_TYPE_NOT_NULL)) ||
        ((qual1 & qual2) & PSS_TYPE_DEFAULT))
    {
	psl_find_18colname(qmode, sess_cb, dbpinfo, &colname);
	psl_command_string(qmode, sess_cb, command, &length);
    
	_VOID_ psf_error(E_PS0479_DUP_COL_QUAL, (DB_ERROR *)NULL, PSF_USERERR,
			 &err_code, err_blk, 2,
			 length, command,
    ...
    */
	...
	_VOID_ psf_error(E_PS047A_CONFLICT_COL_QUAL, (DB_ERROR *)NULL, 
		     ...
		     DB_ERROR	  *err_blk)
{
	...
	*/
	if (ref_tabname->pss_schema_id_type == PSS_ID_SCONST)
	{
	    ...
	    (void) psf_error(2715L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	*/
	if (~cons->pss_cons_type & PSS_CONS_NOT_NULL)
	{
	    ...
	    */
	    ...
	    {
		_VOID_ psf_error(E_PS0471_CHECK_CONSTRAINT, (DB_ERROR *)NULL, 
                                 PSF_USERERR, &err_code, err_blk, 1,
		       ...
		       PSS_YYVARS *yyvarsp)
{
    ...
    */
	...
	_VOID_ psf_error(err, (DB_ERROR *)NULL, PSF_USERERR, 
			 &err_code, &psq_cb->psq_error, 1,
			 length, command);
	return (E_DB_ERROR);
    }
...
** Returns:
**
** Side effect: This may 'fix' attribute collation
**
** History:
**	06-may-2011 (stial01)
**	    Created
	...
	DMF_ATTR_ENTRY	*attr)
{
    ...
    i4			err_code;
    ADI_DT_NAME		dt_name;
    ADF_CB		*adfcb = (ADF_CB*) sess_cb->pss_adfcb;
    i4			attr_type;
    DB_STATUS		status;
    i4			x100_type;

    /* For X100 tables ensure that collation is set to UCS_BASIC  */
    attr_type = abs(attr->attr_type);
    switch (attr_type)
    {
	case DB_CHA_TYPE:
	case DB_CHR_TYPE:
	...
	case DB_NVCHR_TYPE:
	case DB_LNVCHR_TYPE:
	    if (adfcb->adf_collation == NULL &&
		    attr->attr_collID <= DB_NOCOLLATION)
		attr->attr_collID = DB_UCS_BASIC_COLL;
	    else if (attr->attr_collID != DB_UCS_BASIC_COLL ||
		    adfcb->adf_collation)
	    {
		(void) psf_error(5566, NULL, PSF_USERERR,
		    &err_code, err_blk, 1,
		    psf_trmwhite(DB_TAB_MAXNAME, table_name->db_tab_name),
		    table_name->db_tab_name);
		return(E_DB_ERROR);
	    }
    }

    if (x100_supported_dt(attr_type) != E_DB_OK)
    {
	status = adi_tyname(adfcb, attr_type, &dt_name);
	(VOID) psf_error(5567L, 0L, PSF_USERERR, &err_code, 
	    &psq_cb->psq_error, 2,
	    psf_trmwhite(sizeof(ADI_DT_NAME), (char *) &dt_name), &dt_name,
	    psf_trmwhite(DB_ATT_MAXNAME, (char *) &attr->attr_name),
	    (char *) &attr->attr_name);
	return (E_DB_ERROR);
    }
