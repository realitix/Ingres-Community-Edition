	...
	char		*authid)
{
	...
	*/
	...
	(VOID) psf_error(6661L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
			 err_blk, 2,
	...
	PST_PROCEDURE	*prnode)
{
    ...
    */
	...
	(VOID) psf_error(E_PS04C0_BAD_SCHEMA_TREE, (DB_ERROR *)NULL, PSF_INTERR,
			 &err_code, err_blk, 0);
	return(E_DB_ERROR);
    }
    ...
    */
	...
	if (current_node->pst_type != PST_EXEC_IMM_TYPE)
	{
	    ...
	    (VOID) psf_error(E_PS04C1_BAD_SCHEMA_NODE, (DB_ERROR *)NULL, 
		 ...
		 == (PST_OBJDEP *) NULL)
	{
	    ...
	    (VOID) psf_error(E_PS04C2_BAD_SCHEMA_OBJECT, (DB_ERROR *)NULL, 
                             PSF_INTERR, &err_code, err_blk, 0);
	    return(E_DB_ERROR);
	}
		...
		!= 0)
	{
	    ...
	    (VOID) psf_error(6657L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
				err_blk, 2,
				sizeof("CREATE SCHEMA")-1, "CREATE SCHEMA",
				STlength((char *) exec_node->
					 	       pst_qrytext->db_t_text),
				exec_node->pst_qrytext->db_t_text);
	    (VOID) psf_error(6658L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
			     	err_blk, 3,
			     	sizeof("CREATE SCHEMA")-1, "CREATE SCHEMA",
    ...
    */
	     ...
	     dependency = dependency->pst_nextdep)
	{
		...
		/* ERROR - unresolved dependency */
	    	(VOID) psf_error(6657L, (DB_ERROR *)NULL, PSF_USERERR, 
                                &err_code, err_blk, 2,
				sizeof("CREATE SCHEMA")-1, "CREATE SCHEMA",
				STlength((char *) exec_node->
						       pst_qrytext->db_t_text),
				exec_node->pst_qrytext->db_t_text);
	    	(VOID) psf_error(6659L, (DB_ERROR *)NULL, PSF_USERERR,
			    	&err_code, err_blk, 2,
				sizeof("CREATE SCHEMA")-1, "CREATE SCHEMA",
			    	psf_trmwhite(sizeof(dependency->pst_depname),
					 (char *)&dependency->pst_depname),
	...
	*/
	     ...
	     current_node = current_node->pst_next)
	{
		...
		(VOID) psf_error(6660L, (DB_ERROR *)NULL, PSF_USERERR, 
                                &err_code, err_blk, 2,
				sizeof("CREATE SCHEMA")-1, "CREATE SCHEMA",
				psf_trmwhite(sizeof(dependency->pst_depname),
					  (char *)&dependency->pst_depname),
