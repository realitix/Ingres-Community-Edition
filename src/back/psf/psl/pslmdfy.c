...
/* Some actions don't actually modify the table (such as modify to encrypt,
** which actually just supplies an encryption key).  For such actions,
** tell QEF that it should skip any constraint dependency checking that
...
*/
	...
	/* You'll note that there are no vectorwise structures listed...
	** MODIFY is not currently allowed to (or from) a VW structure.
	*/
	{"reconstruct",
	 DMU_ACT_STORAGE,
	...
	DB_ERROR	*err_blk)
{
	...
	*/
	...
	{
	    _VOID_ psf_error(5542L, (DB_ERROR *)NULL, PSF_USERERR, 
                             &err_code, err_blk, 0);
	    return(E_DB_ERROR);
	}
    }
	...
	*/
	...
	if (!dmu_cb->dmu_location.data_in_size)
	{
	    ...
	    _VOID_ psf_error(5548L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 0);
	    return (E_DB_ERROR);
	}
    }
	...
	PSS_YYVARS	*yyvarsp)
{
    ...
    {
	(VOID) psf_error(5530L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    &psq_cb->psq_error, 0);
	...
	DB_ERROR	*err_blk)
{
    ...
    */
	...
	/* "unique" not allowed */
	(VOID) psf_error(5512L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    err_blk, 2,
	...
	char		*storname)
{
	...
	*/
	...
	{
	    (VOID) psf_error(5510L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	...
	if (stornames_ptr->special_flags & PSL_MDF_ERROR)
	{
	    ...
	    (VOID) psf_error(5505L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 1,
		psf_trmwhite(sizeof (DB_TAB_NAME), 
	  ...
	  && showptr->tbl_status_mask & DMT_IDX)
	{
	    ...
	    (void) psf_error(5540, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 2,
		psf_trmwhite(sizeof (DB_TAB_NAME), 
		    (char *) &showptr->tbl_name),
		&showptr->tbl_name,
	...
	if (stornames_ptr->special_flags & PSL_MDF_MERGE)
	{
	    ...
	    {
		(VOID) psf_error(5525L, (DB_ERROR *)NULL, PSF_USERERR, 
                    &err_code, err_blk, 1,
		    psf_trmwhite(sizeof(DB_TAB_NAME),
			(char *) &showptr->tbl_name),
		    &showptr->tbl_name);
	...
	if (showptr->tbl_status_mask & DMT_IS_PARTITIONED)
	{
	    ...
	    */
		...
		{
		    (void) psf_error(E_US1946_6464_PPART_ONLY, (DB_ERROR *)NULL,
		...
		(void) psf_error(E_US1945_6463_LPART_NOTALLOWED, 
    ...
    */
	...
	if (DB_FAILURE_MACRO(status))
	{
	    ...
	    (VOID) psf_error(2179L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                err_blk, 2, sizeof(sess_cb->pss_lineno), &sess_cb->pss_lineno,
	...
	DB_ERROR	*err_blk)
{
	...
	(VOID) psf_error(5520L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, 
                err_blk, 3, psf_trmwhite(sizeof(DB_TAB_NAME),
		(char *) &sess_cb->pss_resrng->pss_tabdesc->tbl_name),
	    &sess_cb->pss_resrng->pss_tabdesc->tbl_name,
	    sortw_len, sortword,
	...
	DB_ERROR	*err_blk)
{
	...
	(VOID) psf_error(5515L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    err_blk, 0);
	return (E_DB_ERROR);
    }
    ...
    if (err_string != NULL)
    {
	(VOID) psf_error(5502L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 2,
		psf_trmwhite(sizeof(DB_TAB_NAME),
		(char *) &sess_cb->pss_resrng->pss_tabdesc->tbl_name),
		&sess_cb->pss_resrng->pss_tabdesc->tbl_name,
    ...
    {
	(VOID) psf_error(5503L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    err_blk, 0);
	return (E_DB_ERROR);
    }
    ...
    {
	(VOID) psf_error(5511L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
	    err_blk, 2,
	    psf_trmwhite(sizeof(DB_TAB_NAME),
		(char *) &sess_cb->pss_resrng->pss_tabdesc->tbl_name),
	    &sess_cb->pss_resrng->pss_tabdesc->tbl_name,
    ...
    */
	...
	!MEcmp(keyname,
		...
		sizeof("tid") - 1))
    {
	(VOID) psf_error(2105L, (DB_ERROR *)NULL, PSF_USERERR,
	    &err_code, err_blk, 2,
    ...
    if (DB_FAILURE_MACRO(status))
    {
	(VOID) psf_error(2179L, (DB_ERROR *)NULL, PSF_USERERR, &err_code, err_blk, 
            2, sizeof(sess_cb->pss_lineno), &sess_cb->pss_lineno,
	...
	if (!MEcmp((char *) &attname, (char *) &(*key)->key_attr_name,
	    sizeof(DB_ATT_NAME)))
	{
	    (VOID) psf_error(5507L, (DB_ERROR *)NULL, PSF_USERERR, &err_code,
		err_blk, 2,
		psf_trmwhite(sizeof(DB_TAB_NAME),
		    (char *) &sess_cb->pss_resrng->pss_tabdesc->tbl_name),
		&sess_cb->pss_resrng->pss_tabdesc->tbl_name,
...
**	3-sep-2009 (dougi)
**	    Check for and flag MODIFY on X100 tables.
	...
	DB_ERROR	*err_blk)
{
    ...
    /* MODIFY on a vectorwise table not currently supported,
    ** no matter what the action is!
    */
    if (rngvar->pss_tabdesc->tbl_storage_type > DB_STDING_STORE_MAX)
    {
	(void) psf_error(5561, 0, PSF_USERERR, &err_code, err_blk, 1,
		psf_trmwhite(sizeof(DB_TAB_NAME), rngvar->pss_tabname.db_tab_name),
	...
	PSS_YYVARS *yyvarsp, char *logname)
{
    ...
    if ((sess_cb->pss_resrng->pss_tabdesc->tbl_status_mask & DMT_IS_PARTITIONED) == 0
      || part_def_ptr == NULL)
    {
	(void) psf_error(E_US1941_6459_NOT_PART, (DB_ERROR *)NULL, PSF_USERERR,
    ...
    */
    ...
    if (! found)
    {
	(void) psf_error(E_US1942_6460_EXPECTED_BUT, (DB_ERROR *)NULL, PSF_USERERR,
    ...
    {
	(void) psf_error(E_US1943_6461_NOT_SUBPART, (DB_ERROR *)NULL, PSF_USERERR,
		&toss_err, &psq_cb->psq_error, 3,
		0, "MODIFY",
		0, logname,
...
psl_md_reconstruct(PSS_SESBLK *sess_cb, DMU_CB *dmucb, DB_ERROR *err_blk)
{
    ...
    */
	...
	if (BTcount(local_indicators, PSS_WC_LAST) > 0)
	{
	    ...
	    (void) psf_error(E_US1945_6463_LPART_NOTALLOWED, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, err_blk, 2,
		0, "RECONSTRUCT",
    ...
    */
    ...
    {
	(void) psf_error(E_US1947_6465_X_NOTWITH_Y, (DB_ERROR *)NULL, PSF_USERERR,
		&toss_err, err_blk, 3,
	...
	PSS_YYVARS *yyvarsp, DB_ERROR *err_blk)
{
    ...
    */
	...
	{
	    (void) psf_error(E_PS0F02_MEMORY_FULL, (DB_ERROR *)NULL, PSF_INTERR,
			&toss_err, err_blk, 0);
	    return (E_DB_ERROR);
	}
    }
    ...
    */
    ...
    {
	(void) psf_error(E_US1948_6466_UNIQUE_PART_INCOMPAT, (DB_ERROR *)NULL, 
                PSF_USERERR, &toss_err, err_blk, 2,
		yyvarsp->qry_len, yyvarsp->qry_name,
		0, noise);
	return (E_DB_ERROR);
