/*
**Copyright (c) 1986, 2005, 2009, 2010 Actian Corporation
**
*/
...
**	    Added ref to Psf_valid_coll[]
**	09-Dec-2010 (kiria01) SIR 124690
**	    Corrected DLL linkages for last change.
**	2-Feb-2011 (kschendel) b124962
**	    Add new copy-processing externs.
**	03-Feb-2011 (kiria01) b124995
**	    Need to track not just whether we are parsing in a target list
**	    generally but also specifically. The in_target_clause variable
**	    is now a depth qualified bit map, summed by active_target_clauses.
...
**          Added prototype for psl_check_x100_att
**      24-May-2011 (horda03) b125118
**          Added definitions for ALTER TABLE ALTER COLUMN options
**          PSL_ALT_COL_NONE, PSL_ALT_COL_GENALWAYS, PSL_ALT_COL_GENDEFAULT and
**          PSL_ALT_COL_DROPIDENTITY
**	24-may-2011 (stial01) m1627
**	    Added psc_vwtab_inq, true if cursor defined on x100 table
**      29-Jun-2011 (maspa05) b122380
**          Added trace point ps212 (replaces op212) 
**      04-Jul-2011 (coomi01) 
**          Introduce has_cast_length to allow cast_length to take on a
**          zero value for timestamps. 
...
**	    Updated call interface to pst_resolve to support returning whole
**	    new nodes if need be.
**	20-Oct-2011 (kiria01) b125878
**	    Added prototype for pst_resolve_err
*/

/*
**  TRACE FLAGS:   
...
**                          PSF memory in ULM.
**    N     84 (ps212)  -   enable table_autostruct (replaces op212)
**
...
#define	    PSS_SESSION_TABLE_AUTOSTRUCT    84	/* PS212 */
...
**	8-Apr-2011 (kschendel) SIR 125235
**	    Add storage structure for IVW checking.  Bool-ize some bools.
*/
...
#define                 PSCCUR_ID       CV_C_CONST_MACRO('P', 'S', 'C', 'B')
    ...
    bool	    psc_delall;		/* TRUE iff delete permit given */
    bool	    psc_forupd;		/* TRUE iff "for update" clause given */
    bool	    psc_repeat;		/* TRUE iff this is a repeat cursor */
    bool	    psc_open;		/* TRUE iff this cursor is open */
    bool	    psc_used;		/* TRUE iff this cb is being used */
    bool	    psc_vwtab_inq;	/* TRUE iff IVW/x100 table in query */
    PSC_RESTAB      psc_restab;         /* Result column descriptions */
    PST_QNODE	    *psc_integ;		/* Integrity qualification */
    i4		    psc_struct;		/* Storage structure for updates,
					** deletes -- for IVW checking */
...
**	    The insert-to-copy copy control block info needs to be associated
**	    with the prepared statement prototype, since there might be more
**	    than one, and said info has the same lifetime as the prototype
**	    (the copy control info is allocated from the proto ostream).
*/
typedef struct _PST_PROTO
{
    ...
    bool		pst_cant_copy_optim; /* TRUE if insert-to-copy optim is
					    ** known to NOT be allowed for this
					    ** stmt.  Irrelevant unless insert.
					    */
    ...
    struct _QEF_RCB	*pst_cp_qefrcb;	    /* Pointer to QEF_RCB and assoc
					    ** copy control blocks, if stmt
					    ** is an INSERT that is being done
					    ** via insert-to-copy optimization
					    */
...
**	    Remove vectorwise default flags, now done with result structure.
**	24-Mar-2011 (kschendel) b125204
**	    Move pss-last-sname to PSQ-CB so that sequencer can clear it
**	    after copy is ended.  This is less fragile than trying to
**	    clear it the next time into the parser.
**	10-may-2011 (dougi)
**	    Add PSS_WINDOW_INQ statement flag.
**	15-Jul-2011 (kiria01) b125530
**	    Corrected pss_defqry type.
*/
...
ULT_VECTOR_MACRO(PSS_TBITS, PSS_TVAO) pss_trace; 
    ...
    PSQ_MODE	    pss_defqry;		/* 0		    - regular query
...
#define	    PSS_SESSION_STARTED		0x10000000L
    ...
    u_i4	pss_stmt_flags;		/* this field will be used for flags
...
#define     PSS_TXTEMIT2		0x8000L
					...
					** views
					*/
#define	    PSS_VALIDATING_CHECK_OPTION	0x00010000L

					/*
					** cursor has been declared FOR UPDATE
					** without the column list
					*/
#define	    PSS_IMPL_COL_LIST_IN_DECL_CURS  0x00020000L
					...
					*/
#define	    PSS_RESOLVING_CHECK_CONS	0x00040000L
					...
					** should be flattened
					*/
#define     PSS_CREATE_STMT		0x00080000L
					...
					**
					*/
#define     PSS_HAS_DGTT    0x00100000L
					/*
...
#define	 PSS_XTABLE_UPDATE		0x00400000L
					/*
					** flag up xtable update
					*/
#define  PSS_PARSING_INSERT_SUBSEL	0x00800000L
					/*
...
#define		PSS_KEYSET		0x80000000L
					...
					*/
    u_i4	pss_stmt_flags2;	/* yet more statement-level flags */
#define	PSS_2_X100TAB_INQ		0x00000001L
					/* At least 1 X100 table is in query */
#define	PSS_2_NEXTVAL_INQ		0x00000002L
					/* A "next value ..." operation appears
					** in the query */
#define PSS_STMT_NO_CACHEDYN		0x00000004L
                                        /* indicates this is query must not
                                        ** be cached */
#define     PSS_DERTAB_INQ              0x0008L
					/*
					** query has at least one derived table
					*/
#define	    PSS_WINDOW_INQ		0x00000010L
					/* At least one window function in q */

    u_i4	pss_flattening_flags;
...
**	7-may-2009 (dougi)
**	    Add PSF_X100DEFAULT server wide flag.
**	15-Feb-2010 (maspa05) SIR 123293
**	    Add psf_server_class so SC930 can output server_class
**	20-Jul-2010 (kschendel) SIR 124104
**	    Add default create-table compression.
**	15-sep-2010 (dougi)
**	    Add PSF_VWRDEFAULT server wide flag.
**	29-Oct-2010 (kschendel)
**	    Remove VW default flags, now done with result structure.
**	29-Jun-2011 (maspa05) bug 122380
**	    Add PSF_TABLE_AUTOSTRUCT so we know when CBF option for autostruct
**          is on.
**	01-Jul-2011 (kiria01) b125504
**	    The following structure is now properly private to PSF.
*/
...
#define     PSF_NOCHK_SINGLETON_CARD	   0x0080L
                                        ...
                                        /*
					** CBF table_autostruct is ON
					 */
#define     PSF_TABLE_AUTOSTRUCT	   0x0100L
...
#define		PSS_DBP_BY_ID		((i4) 0x40)
					/*
					** IF EXISTS active - missing is not an error
					*/
#define		PSS_DBP_IF_EXISTS	((i4) 0x80)
...
**      29-Jun-2011 (maspa05) b122380
**          Added PSS_CONS_BTS_AUTOSTR so we know when "index=base table 
**          structure" is due to table autostruct 
*/
...
#define PSS_CONS_BTS_AUTOSTR	0x10000	/* "index = base table structure" due
					   specifically to table_autostruct 
					   being used */
...
**    26-aug-2008 (dougi)
**        Add PSS_WC_X100 for X100 support.
**	10-oct-2008 (dougi) Bug 120998
...
**	    DBP contexts.
**	11-may-2011 (dougi)
**	    Add in_window flag for parsing window functions.
**	04-Jun-2011 (kiria01) SIR 125416
**	    Added syntax for multi-row insert.
**/
...
#define	    MAX_NESTING	    100
    ...
    i4	    in_from_clause;
    i4	    in_where_clause;
    u_char  in_target_clause[MAX_NESTING / BITS_IN(u_char) + 1];
    i4	    active_target_clauses;
...
*/
    ...
    bool	    in_window;	/* TRUE if parsing window function syntax */
    ...
    */
    ...
    bool            has_cast_length;    /* Does cast_length contain anything, even a 0 ? */
    ...
    DB_COLL_ID	    collID;
    u_i2	    union_prong;	/* Ordinal value of current select in union list */
}   PSS_YYVARS;

...

/* Values that define permitted value for psl_alt_tbl_col_add's
** altopts parameter.
*/
#define PSL_ALT_COL_NONE		0
#define PSL_ALT_COL_GENALWAYS		1
#define PSL_ALT_COL_GENDEFAULT		2
#define PSL_ALT_COL_DROPIDENTITY	3

/*
...
typedef struct _PST_STK_CMP
{
    ...
    bool same:1;		/* Output - tree match*/
    bool reversed:1;		/* Output - tree matched but reversed */
    bool inconsistant:1;	/* Output - inconsistent tree compare */
    bool all_rslvd:1;		/* Input - assume all fields resolved (def=T)*/
} PST_STK_CMP;
#define PST_STK_CMP_INIT(_c,_cb,_rt) \
    {PST_STK_INIT((_c).stk, _cb) \
    (_c).rngtabs=_rt;(_c).n_covno=0;(_c).all_rslvd=TRUE;}

/*
** Psf_valid_coll[] - collation ID category table.
*/
typedef enum _COLL_FLAGS {
    F_NON_UNICODE = 1,	/* CHAR,VARCHAR,C,TEXT & LONG VARCHAR */
    F_UNICODE = 2,	/* NCHAR,NVARCHAR & LONG NVARCHAR */

    /* Composite flags for the DB_COLL_MACRO table macro to
    ** define the _ and Y settings for the Ch & Un fields */
    F_Ch_ = 0,
    F_Un_ = F_Ch_,
    F_ChY = F_NON_UNICODE,
    F_UnY = F_UNICODE
} PSF_COLL_FLAGS;
GLOBALCONSTREF PSF_COLL_FLAGS Psf_valid_coll[];

/*
** PSF_DT_CLASS datatype class lookup
*/
GLOBALCONSTREF i4 Psf_dt_class[];
#define PSF_DT_CLASS(v) (abs(v)>=DB_MAX_TYPE?DB_NONE_CLASS:Psf_dt_class[abs(v)])
...
FUNC_EXTERN i4 psl_cp13_nm_single(
	...
	char *name,
	DB_ERROR *err_blk);
FUNC_EXTERN DB_STATUS psl_copy_missing(
	PSS_SESBLK *sess_cb,
	PSF_MSTREAM *ostreamp,
	QEU_COPY *qe_copy,
	PST_QNODE *def_node,
	DMT_ATT_ENTRY *att_entry,
	i4 dt_bits,
	DB_ERROR *errblk);
FUNC_EXTERN DB_STATUS psl_copy_finish_seq_defaults(
	PSS_SESBLK *sess_cb,
	PSF_MSTREAM *ostreamp,
	i4 seq_defaults,
	i4 seq_datalen,
...
FUNC_EXTERN void psl_command_string(
	...
	char *command,
	i4 *length);
FUNC_EXTERN DB_STATUS psl_check_x100_att(
	PSS_SESBLK	*sess_cb,
	PSQ_CB		*psq_cb,
...
FUNC_EXTERN i4 psq_open_rep_cursor(
	PSQ_CB *psq_cb,
	PSS_SESBLK *sess_cb);
FUNC_EXTERN bool psq_any_vwtab_cursor(
	PSS_SESBLK *sess_cb,
	DB_ERROR *err_blk);
/* psqcvtdw.c */
#include "psqcvtdw.h"
...
FUNC_EXTERN PST_QNODE *pst_non_const_core(
	PST_QNODE *node);
FUNC_EXTERN bool pst_expr_is_qry_invar(
	PST_QNODE *node);
...
FUNC_EXTERN bool pst_convlit(
	...
	PST_QNODE *srcenode);
FUNC_EXTERN DB_STATUS pst_mk_similar(
	PSS_SESBLK *sess_cb,
	ADF_CB *adf_scb,
	PSF_MSTREAM *mstream,
	DB_DATA_VALUE *templ,
	PST_QNODE **nodep,
	DB_ERROR *err_blk);
FUNC_EXTERN bool pst_is_literal_int (
	PST_QNODE *node,
	i8 val);
FUNC_EXTERN DB_STATUS pst_augment_decimal_cast (
	PSS_SESBLK *sess_cb,
	PSF_MSTREAM *mstream,
	PST_QNODE *node,
	DB_ERROR *err_blk);
FUNC_EXTERN DB_STATUS pst_resolve_err(
	PSS_SESBLK *cb,
	PST_QNODE *node,
	DB_ERROR *err_blk,
	i4 *flags);
...
FUNC_EXTERN i4 psy_ubtid(
	...
	DB_TAB_ID *ubt_id);
/**/
FUNC_EXTERN DB_STATUS psy_syscall(
	PSY_CB *psy_cb,
	PSS_SESBLK *sess_cb);
