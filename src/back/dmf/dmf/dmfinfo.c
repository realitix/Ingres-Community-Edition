...
**          When JSX2_INFO_DBS print only dbname
*/
DB_STATUS
dmfinfo(
DMF_JSX		    *jsx,
DMP_DCB		    *dcb)
{
    ...
    */
    ...
    if (jsx->jsx_status2 & JSX2_INFO_DBS)
    {
    ...
    */
    ...
    /* Show X100 database if there is one. */
    if (jsx->jsx_x100_db)
    {
        TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
                 "\n%15* The database has the following VectorWise storage location:\n");
        TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
                 "%15* %~t\n", 
		 jsx->jsx_x100_data.phys_length,
		 jsx->jsx_x100_data.physical.name);
    }
