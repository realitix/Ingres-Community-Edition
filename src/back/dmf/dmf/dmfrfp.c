...
**          Added x100 rollforwarddb processing.
**      01-jun-2011 (stial01) m1916
**          Moved x100 checkpoint processing to restore_x100()
**          Dont assume x100 location existed when database was checkpointed
**      23-jun-2011 (stial01)
**          x100_terminate after getting db lock, move x100_terminate to dmfx100
**       7-jul-2011 (stial01)
**          Increase sizeof message buf for path strings
		...
		*/
		...
		if (status == E_DB_OK)
		{
			...
			*/
		    ...
		    TRformat(dmf_diag_put_line, 0, line_buffer, sizeof(line_buffer),
		    "WARNING: It is strongly recommended that you contact Actian Corporation Technical Support for further information.");
		}
		else
		{
		    dmfWriteMsg(&jsx->jsx_dberr, 0, 0);
...
**	15-Dec-2011 (drivi01) m1871
**	    Add the code to delete VectorWise data locations before starting
**	    a restore of the database.  Also add the code to restore the
**	    VW data from a checkpoint.
[@history_template@]...
*/
static	DB_STATUS
db_restore_ckp(
...
DMP_DCB		    *dcb)
{
    ...
    */
    ...
    /* Delete all files/directories at the root of vectorwise data directory */
    if (jsx->jsx_x100_db)
    {
	DELETE_INFO delete_info;
	delete_info.path_name = jsx->jsx_x100_data.physical.name;
	delete_info.path_length = jsx->jsx_x100_data.phys_length;
	status = DIlistfile(&di_file, delete_info.path_name,
		delete_info.path_length, del_ck_files, (PTR)&delete_info,
		&sys_err);
	if (status != DI_ENDFILE)
	{
	    SETDBERR(&jsx->jsx_dberr, 0, E_DM130C_RFP_DELETE_ALL);
    ...
    */
    ...
    /* Restore Vectorwise tables from a checkpoint  */
    if (jsx->jsx_x100_db)
    {
	status = restore_x100(rfp, jsx, dcb, ckp_sequence);
	if (status)
	    return (status);
    }
...
**	04-Jan-2011 (thaju02) B124896
**	    For -b and -sync_lsn, do not add_tx for JEOF records.
[@history_template@]...
*/
static DB_STATUS
process_record(
...
{
    DMCKP_CB            *d = &rfp->rfp_dmckp;
    DMP_LOC_ENTRY 	*dataloc = &jsx->jsx_x100_data;
    DMP_LOC_ENTRY 	*ckploc = &jsx->jsx_x100_ckp;
    DM_FILE		filename;
    i4			loc_sequence;
    i4			*err_code = &jsx->jsx_dberr.err_code;
    DB_STATUS		status;
    CL_ERR_DESC		sys_err;
    char		line_buffer[DB_AREA_MAX + 132];
    LOCATION		x100_ckp_loc;	
    char		ckppath[DB_AREA_MAX + 1];
    char		name[DB_AREA_MAX];

    /* This code assumes restore from disk */
    if (jsx->jsx_status & JSX_DEVICE)
    {
	SETDBERR(&jsx->jsx_dberr, 0, E_DM1307_RFP_RESTORE);
	return (E_DB_ERROR);
    ...
    /*
    ** II_VWDATA is not defined in the location array, 
    ** so it doesn't have an associated location sequence number.
    ** For now use location sequence 1, same as default data 
    ** Another alternative would be the unused location sequence 0 
    */
    loc_sequence = 1;
    ...
    /*
    ** Since we don't checkpoint x100 locations until after 
    ** a Vectorwise table was created, it is possible that the 
    ** II_VWDATA location  did not exist at the time of this checkpoint
    ** If we stored II_VWDATA properly in the config file,
    ** we would know if the II_VWDATA was part of this checkpoint
    */ 
    MEcopy( jsx->jsx_x100_ckp.physical.name, 
	jsx->jsx_x100_ckp.phys_length, ckppath);
    ckppath[jsx->jsx_x100_ckp.phys_length] = '\0';
    LOfroms(PATH, ckppath, &x100_ckp_loc);
    MEcopy( filename.name, 12, name);
    name[12] = '\0';
    status = LOfaddpath(&x100_ckp_loc, name, &x100_ckp_loc);
    if (LOexist(&x100_ckp_loc) != OK)
    {
	/* ckp file doesn't exist, assume it was never checkpointed */
	TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
		"%@ RFP: Checkpoint file %~t does not exist\n",
		STlength(ckppath), ckppath);
	return (E_DB_OK);
    }

    TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	"%@ RFP: Start restore of location: %~t from disk:\n",
	sizeof(dataloc->logical), &dataloc->logical);

    TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	"%4* path = '%t'\n%4* file = '%~t'\n",
	ckploc->phys_length, &ckploc->physical, sizeof(filename), &filename);

    /* Set up the directory path to the x100 ckp location */
    d->dmckp_ckp_path     = ckploc->physical.name;
    d->dmckp_ckp_l_path   = ckploc->phys_length;

    /* Set up the directory path to the x100 data location */
    *(dataloc->physical.name + dataloc->phys_length) = '\0';
    d->dmckp_di_path   = dataloc->physical.name;
    d->dmckp_di_l_path = dataloc->phys_length;
    d->dmckp_raw_start = dataloc->raw_start;
    d->dmckp_raw_blocks = dataloc->raw_blocks;
    d->dmckp_raw_total_blocks = dataloc->raw_total_blocks;

    /* Call the restore routine */
