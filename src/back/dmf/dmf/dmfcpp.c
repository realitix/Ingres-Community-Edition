...
**      13-apr-2011 (stial01) m1667
**          Don't unset TCB2_TBL_RECOVERY_DISALLOWED if vectorwise table
**      29-apr-2011 (stial01) m1700
**          Added x100 checkpoint processing.
**      25-May-2011 (drivi01) m1871
**          Added backup and restore capabilities for VectorWise databases
**			for Windows/Linux.
**      01-jun-2011 (stial01) m1871
**          create_ckpjnldmp() x100 ckp location is II_CHECKPOINT 
**          Moved x100 checkpoint processing to check_point_x100()
**      23-jun-2011 (stial01)
**          x100_stall after getting db lock, move x100 routines to dmfx100.c
**      07-jul-2011 (stial01) m2230
**          create_ckpjnldmp() move up code to create vectorwise ckp directory
...
static DB_STATUS check_point_x100(
    DMF_CPP	*cpp,
    DMF_JSX    *jsx,
    DMP_DCB    *dcb,
    i4		ckp_sequence);
...
DMP_DCB	    *dcb)
{
    ...
    */
    ...
    if (jsx->jsx_x100_db)
    {
	status = x100_stall(jsx, dcb);
	if (status)
	{
	    SETDBERR(&jsx->jsx_dberr, 0, E_DM110D_CPP_DB_BUSY);
	    return (E_DB_INFO);
	}
	x100_is_stalled = TRUE;
    }
...
i4	    ckp_sequence)
{
	...
	*/
    ...
    if (jsx->jsx_x100_db)
    {
	status = check_point_x100(cpp, jsx, dcb, ckp_sequence);
	if (status)
	    return (E_DB_ERROR);
    }
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    /*
	    ** VectorWise tables are not journaled, table recovery disallowed
	    ** No update to iirelation tuple needed
	    */
...
bool		    dflag)
{
    ...
    /*
    ** Create a vectorwise checkpoint directory if not done already
    ** This should be checked even if +j is not specified, 
    ** even if this is not first checkpoint... 
    ** because currently vectorwise data directories dont get created
    ** until the first vectorwise table is created
    */
    if (jsx->jsx_x100_db)
    {
	...
	/* 
	** Currently x100 files are checkpointed to II_CHECKPOINT which is
	** not necessarily the same as the database ckp location (ckp_location)
	** if the db was created with -cckp_location_name
	*/
	...
	if (LOexist(&x100_ckp_loc) != OK)
	{
	    ...
	    /* jsx_x100_ckp_pathlen is pathlen without dbname */
	    status = CKdircreate(ckppath, jsx->jsx_x100_ckp_pathlen,
			dcb->dcb_name.db_db_name, i, &sys_err);
...
i4	    ckp_sequence)
{
    ...
    /* This code assumes checkpoint to disk */
    if (jsx->jsx_status & JSX_DEVICE)
    {
	SETDBERR(&jsx->jsx_dberr, 0, E_DM1101_CPP_WRITE_ERROR);
	return (E_DB_ERROR);
