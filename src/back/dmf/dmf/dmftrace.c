...
**          Added "set tracepoint dm37 n" to test etab full error handling
**          when inserting segment having per_segment1 == n.
**          Added "set trace point dm38" to turn off buffered put optimization
**          in dmpe_buffered_put.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**      07-Oct-2011 (horda03)
**          Add "set trace point  dm724 [<low page>] [<high page>]" to set the
**          range of Page numbers to be dumped by TP dm725. 
**          Added "set trace point dm725 <Table ID> [<Index ID>]" to dump out
**          the specifed range of tables.
**          print_table() created to do the work of TP dm725.
*/

/*
...
DB_DEBUG_CB	    *debug_cb)
{
	...
	**      DM724   - Store page range
	**      DM725   - Dump table/index pages
	...
	*/
	if ((DMZ_MEM_MACRO(1))  || (DMZ_BUF_MACRO(20)) || (DMZ_BUF_MACRO(21)) ||
	    ...
	    (DMZ_BUF_MACRO(5))  || (DMZ_BUF_MACRO(6)) ||
            (DMZ_TBL_MACRO(24)) || (DMZ_TBL_MACRO(25))
	    )
	{
...
i4		trace_point)
{
    ...
    */
	...
	scb->scb_dop = d->db_vals[0];
     ...
     /* DM37 - Simulate etab full condition when inserting segment */
    if ( trace_point == 37 && d->db_value_count == 1 )
	scb->scb_etfull_seg = d->db_vals[0];
    ...
    */
    ...
    /*
    ** DM724	- Set the range of pages to be output
    **
    **	db_vals[0]  - Lower page number (optional)
    **  db_vals[1]  - Upper page number (optional).
    */
    if (trace_point == 724)
    {
        switch(d->db_value_count)
        {
           case 0:
              /* Display all pages. */
              scb->scb_pg_range [0] = (DM_PAGENO) 0;
              scb->scb_pg_range [1] = (DM_PAGENO) MAXI4;
              break;

           case 1:
              scb->scb_pg_range [0] = (DM_PAGENO) d->db_vals[0];
              scb->scb_pg_range [1] = (DM_PAGENO) d->db_vals[0];
              break;

           case 2:
              scb->scb_pg_range [0] = (DM_PAGENO) d->db_vals[0];
              scb->scb_pg_range [1] = (DM_PAGENO) d->db_vals[1];
              break;

           default:
              /* invalid val count, so don't display anything.
              ** Lower set greater than upper.
              */
              scb->scb_pg_range [0] = (DM_PAGENO) 1;
              scb->scb_pg_range [1] = (DM_PAGENO) 0;
              break;
        }
    }
    ...
    **	db_vals[1]  - index table_id (optional).
    */
    if ((trace_point == 725) && (d->db_value_count > 0) &&
	(scb->scb_oq_next) && (scb->scb_oq_next->odcb_dcb_ptr) &&
        scb->scb_pg_range [0] <= scb->scb_pg_range [1])
    {
	i4 lock_id, log_id;

	table_id.db_tab_base = d->db_vals[0];
	table_id.db_tab_index = (d->db_value_count == 2) ? d->db_vals[1] : 0;

	print_table(scb, &table_id);
    }
...
/*
** Name: print_table	- Print the indicated table
**
** Description:
**	This routine is called by trace point DM725 to print the contents of
**	a table to the DBMS server trace log.
**
**	It opens the table and calls dmd_prtbl to do the work.
**
**	Note that we may not be in a transaction. For example, if this trace
**	point is issued from QUEL, we are typically NOT in a transaction at
**	this point. In this case, we must begin a transaction, and remember
...
DB_TAB_ID   *tableid)
{
	...
	*/
	...
	if ((scb->scb_oq_next) && (scb->scb_oq_next->odcb_dcb_ptr))
	{
	    ...
	    dmd_prtbl(rcb, scb->scb_pg_range [0], scb->scb_pg_range [1]);
