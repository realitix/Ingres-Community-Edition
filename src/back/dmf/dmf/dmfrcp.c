...
**	08-Dec-2010 (smeke01)
**	    If CSterminate returns a non-zero result, print a message similar
**	    to that printed by iimonitor in the same situation.
**	25-May-2011 (kschendel) m1260
**	    Add call to shutdown x100 here.
*/
static
VOID
rcp_shutdown(DB_ERROR *dberr)
{
	...
	*/
	...
	/* Before we go away, shut down all running X100 servers.  This might
	** seem like an odd place to do it, but it does make sense;  if there
	** aren't any RCP's running, nobody can run any queries, so there's no
	** point in keeping X100 servers around.
	** Do this before altering the logging system offline!  because
	** that's what tells ingstop (on unix) that it should proceed, and
	** it might do its belt-and-suspenders kill before we could get to
	** it here.
	*/
	x100_exit();
	x100_exited = TRUE;
	...
	*/
    ...
    /* If we exited the logging shutdown with error, go ahead and try to
    ** kill any X100 servers now.
    */
    if (!x100_exited)
	x100_exit();
    ...
    */
    ...
    if(cl_status)
    {
        TRdisplay("%@ RCP Shutdown, CSterminate() issued. Server will stop. %d sessions remaining.\n", scount);
    }
    else
    {
