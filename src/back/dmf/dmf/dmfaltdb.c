...
static DB_STATUS convert_readmode(
    DMF_JSX     *jsx,
    DMP_DCB     *dcb,
    DM0C_CNF    *cnf);
...
**       6-Jun-2011 (hanal04) SIR 121980
**          If -readonly or -readwrite is used set DSC_READ|DSC_WRITE in the 
**          cnf file by calling set_accmode(). Also call new convert_readmode()
**          to set or unset DU_RDONLY in the iidatabase column 'access' in 
**          IIDBDB for the specified database.
[@history_template@]...
*/
DB_STATUS
dmfalter(
...
DMP_DCB	    *dcb)
{
	...
	*/
	...
	{
            /* Change read mode */
            status = convert_readmode(jsx, dcb, cnf);
            if (DB_FAILURE_MACRO(status))
                break;
...
/*
** Name: convert_readmode -- Sets or unsets DU_RDONLY based on jsx_status2
**                           flags.
**
** Description:
**     The routine is used to make a readwrite database readonly
**     and allow that operation to be reversed. Note, it does NOT
**     change directory permissions so a database that is made
**     readonly may not act entirely like a READONLY database.
**
** Inputs:
**      jsx                        Journal context.
**      dcb                        Database context.
