...
**          Delete VectorWise checkpoints. Moved ckp processing to jsp_ckp_maint
**	16-May-2011 (drivi01)
...
DB_STATUS jsp_ckp_maint(
DMF_JSX		*jsx,
DMP_DCB		*dcb,
char		*ckpath, /* a string */
i4		ckp_seq,
i4		ckp_cnt);
...
bool		fully_delete)
{
    ...
    i4		local_error;
    CL_ERR_DESC		sys_err;
    char		line_buffer[132 + (2 * DB_AREA_MAX)];
    DMP_EXT             *ext = dcb->dcb_ext;
    i4             ckp_cnt; 
    CL_ERR_DESC         *error_code;
    char		ckpath[DB_AREA_MAX + 1]; /* directory path */
    bool		alter_db = FALSE;
    i4			jnl_index;
    i4			dmp_index;
    i4			first_jnl;
	...
	*/
        STncpy (ckpath, (char *) (dcb->dcb_ckp->physical.name),
			 dcb->dcb_ckp->phys_length);
        ckpath[dcb->dcb_ckp->phys_length ] = '\0';
	status = jsp_ckp_maint( jsx, dcb, ckpath, ckp_seq, ckp_cnt);
	if (status)
	    return(status);

        if (jsx->jsx_x100_db)
	{
	    /* One vectorwise data location for now */
	    MEmove(jsx->jsx_x100_ckp.phys_length,
		jsx->jsx_x100_ckp.physical.name, '\0', sizeof(ckpath), ckpath);
		
	    status = jsp_ckp_maint( jsx, dcb, ckpath, ckp_seq, 1);
	    if (status)
		return (status);
	}
...
/*{
** Name: jsp_ckp_maint	    - Checkpoint file maintenence.
**
** Description:
**      This routine manages the destruction of old checkpoints.
**	The routine is called by ckpdb and alterdb -delete_oldest_ckp.
**	
** 	Routine assumes that config file is locked at entry, and is updated
**	after return.
...
**      jsx                             Journal support context.
**	dcb				Pointer to DCB.
**      ckppath				checkpoint path
**      ckp_seq				checkpoint sequence
**      ckp_cnt				number of data locations
**
** Outputs:
**      err_code                        Reason for error return status.
**	Returns:
...
** History:
**      01-mar-2011 (stial01) m1505
**          Created from code in jsp_file_maint().
**	16-May-2011 (drivi01)
**	    Add routines for removing nested directories
**	    for VectorWise checkpoint files/directories.
*/
DB_STATUS
jsp_ckp_maint(
DMF_JSX		*jsx,
...
i4		ckp_cnt)
{
    ...
    CL_ERR_DESC         *error_code;
    DI_IO	        f;
    char		ck_loc_buf[DB_AREA_MAX + 1]; /* location string */
    char		ck_dir_buf[DB_AREA_MAX + 1]; /* directory */
    char		save_dir_buf[DB_AREA_MAX + 1]; /* directory */
    char		ck_fn_buf[DB_AREA_MAX +1]; /* filename string */
    char		*ck_fname;
    char		*ck_dir;
    LOCATION		ck_loc;
    ...
    /* Get length of input path, Do not alter path string */
    ckpathlen = STlength(ckpath);
	    ...
	    (loinf.li_type == LO_IS_DIR))   
	{
	    ...
	    {	
			...
			** Use del_ck_files from dmmdes.c to remove
			** directories.
			*/
			if ((LOinfo(&ck_loc, &flagword, &loinf) == OK) &&
				loinf.li_type == LO_IS_DIR)
			{
				delete_info.path_name = ck_loc.path;
				delete_info.path_length = STlength (ck_loc.path);
			    delete_info.error = 0;
				if (jsx->jsx_status & JSX_VERBOSE)
					TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
					"%@ CPP/ALTDB: Deleting folder %s.\n", ck_loc.path);
				status = CKlistfile(ck_loc.path, STlength(ck_loc.path), 
							del_ck_files, (PTR)&delete_info, &sys_err);
				if (status == CK_ENDFILE)
				{
					status = delete_info.error;
					if (status == OK)
						if (CKdirdelete(ck_loc.path, STlength(ck_loc.path), 0, 0, &sys_err) != OK)
							break;
				}
				continue;
			}
	...
	else
	{
	    if (jsx->jsx_status & JSX_VERBOSE)
	       TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	       "%@ CPP/ALTDB: Deleting file %~t in directory %~s\n", 
	       sizeof(filename), filename.name, ckpath);
	    status = CKdelete(ckpath, ckpathlen,
