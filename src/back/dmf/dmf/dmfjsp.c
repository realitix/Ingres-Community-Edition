...
**      05-Nov-2009 (gefei01)
**          Enable infodb for X100 database.
**	01-apr-2010 (stial01)
...
**	    Journaling not supported for Vectorwise unless db is iidbdb    
**      01-mar-2011 (stial01) m1505
**          build_location() Get path for Vectorwise checkpoint location.
**      3-mar-2011 (stial01) m1520
**          Remove 22-feb-2011 change to support journaling for ingres tables
**      16-May-2011 (drivi01) m1871
**          Remove DELETE_INFO structure from this file, it is being moved
**          to dm.h.
**      01-jun-2011 (stial01) m1916
**          Move x100 locations into DMF_JSX
**       4-Jun-2011 (hanal04) SIR 121980
**          Add support for ALTERDB -readonly and ALTERDB -readwrite
**       4-Jun-2011 (Chris Dawe - chris.dawe@iua.org.uk) SIR 125406
**          Allow '-keep=<nnn>' for ckpdb as well as alterdb.
**      07-jul-2011 (stial01) m2230
**          build_location() Init jsx_x100_db
...
# endif
	...
	*/
	...
	while (jsx.jsx_next != (DMP_DCB *)&jsx.jsx_next)
	{
		...
		/* SIR 125406: if ckpdb worked and -keep=<nnn> specified. */
                if (status == E_DB_OK && 
                    (jsx.jsx_status2 & JSX2_ALTDB_KEEP_N))
                {   /* Use same code as alterdb to delete old checkpoints. */
		...
		*/
		        ...
		        (STcompare("disable_mvcc",  &argv[i][1]) == 0))
		{
			...
			SETDBERR(&jsx->jsx_dberr, 0, E_DM105C_JSP_INV_ARG_VAL);
                         ...
                         (STcompare("databases", &argv[i][1] ) == 0))
                    {
                        jsx->jsx_status2 |= JSX2_INFO_DBS;
                    }
		else
		    SETDBERR(&jsx->jsx_dberr, 0, E_DM1004_JSP_BAD_ARGUMENT);
...
# endif
	    ...
	    case 'k':
		/* SIR 125406: Allow -keep for ckpdb as well as alterdb. */
                if (((operator == JSX_O_ALTER) || (operator == JSX_O_CPP)) &&
		...
		if (operator == JSX_O_RFP)
		{
			...
			*/
			...
			SETDBERR(&jsx->jsx_dberr, 0, E_DM1004_JSP_BAD_ARGUMENT);
                ...
                {
                    /* alterdb -readwrite of -readonly */
                    if (STcmp(&argv[i][1], "readwrite" ) == 0)
                    {
                        jsx->jsx_status2 |= JSX2_READ_WRITE;
                    }
                    else if (STcmp(&argv[i][1], "readonly" ) == 0)
                    {
                        jsx->jsx_status2 |= JSX2_READ_ONLY;
                    }
                }
		else
		    SETDBERR(&jsx->jsx_dberr, 0, E_DM1004_JSP_BAD_ARGUMENT);
...
**       4-Jun-2011 (hanal04) SIR 121980
**          JSX2_READ_ONLY and JSX2_READ_WRITE will update iidatabase
**          make sure we do not deadlock ourseleves.
[@history_template@]...
*/
static DB_STATUS
search_dcb(
DMF_JSX             *journal_context)
{
    ...
    if (build_location(jsx, dcb, sizeof(ING_DBDIR), ING_DBDIR, LO_DB,
	...
	*/
	...
	if (build_location(jsx, ndcb, sizeof(ING_DBDIR), ING_DBDIR, LO_DB,
	...
	if (build_location(jsx, ndcb, sizeof(ING_DMPDIR), ING_DMPDIR, LO_DMP,
	...
	*/
        if (   (jsx->jsx_status1 & (JSX_CKPT_RELOC | JSX_DMP_RELOC |
            ...
            || (jsx->jsx_status2 & (JSX2_PMODE | JSX2_OCKP | 
                        JSX2_READ_ONLY | JSX2_READ_WRITE |
...
*/
		...
		if (status == E_DB_OK)
		{
		    if (build_location(jsx, ndcb, location.du_l_area,
...
**      05-Nov-2009 (gefei01)
**          Check the existence of X100 database.     
**	20-apr-2010 (dougi)
**	    X100 dbs now live under "vectorwise" directory.
**	25-May-2011 (drivi01) m1871
**	    Add DMP_LOC_ENTRY structures for VectorWise checkpoint
**	    and ii_vwdata directories in dcb and populate it.
**      14-Jun-2011 (horda03)
**          NMgtIngAt on VMS adds the installation ID to the symbol name
**          look up, so use NMgtAt instead on VMS only..
**          Also, don't bother searching for the directory if neither
**          II_VWDATA nor II_DATABASE defined.
*/
static DB_STATUS
build_location(
DMF_JSX		   *jsx,
...
DMP_LOC_ENTRY	    *location)
{
    i4		    i;
    char	    *cp;
    LOCATION	    loc, x100_loc;
    char	    db_name[DB_DB_MAXNAME+1];
    char	    area[256];
    char           *value;
    char	   x100_path[DB_AREA_MAX + 1];
    DMP_LOC_ENTRY   *xloc;
    LOINFORMATION   loinf;
    i4              flagword;
    DB_STATUS	    status;
    ...
    /*
    ** JSP operations call build_location first for the iidbdb, then for
    ** the target database. 
    ** Reset jsx->jsx_x100_db every time build_location is called...
    ** just in case iidbdb has x100 tables but db specified to jsp does not.
    */ 
    jsx->jsx_x100_db = FALSE;
    ...
    /* Check for X100 database location */
#ifdef VMS
    NMgtAt("II_VWDATA", &value );
    if (!value || !*value)
	NMgtAt("II_DATABASE", &value );
#else
    NMgtIngAt("II_VWDATA", &value );
    if (!value || !*value)
	NMgtIngAt("II_DATABASE", &value );
#endif

    if (value)
    {
	STcopy(value, x100_path);
	LOfroms(PATH, x100_path, &x100_loc);
	status = LOfaddpath(&x100_loc, "ingres", &x100_loc);
	if (status == E_DB_OK)
	    status = LOfaddpath(&x100_loc, "data", &x100_loc);
	if (status == E_DB_OK)
	    status = LOfaddpath(&x100_loc, "vectorwise", &x100_loc);
	if (status == E_DB_OK)
	    status = LOfaddpath(&x100_loc, db_name, &x100_loc);

        /* Check if there is an X100 database at the specified path */
        flagword = LO_I_TYPE;
    
        if ((LOinfo(&x100_loc, &flagword, &loinf) == OK) &&
            (loinf.li_type == LO_IS_DIR))
        {
	    /*
	    ** Init DMP_LOC_ENTRY for II_VWDATA
	    ** II_VWDATA is not defined as a proper Ingres location
	    ** (there is no entry for it in iilocations or the config file)
	    ** We haven't decided how we will implement x100 locations:
	    ** - use one default data location for ingres and x100 tables
	    ** or
	    ** - allow different default data locations for ingres and x100 tables
	    */
	    jsx->jsx_x100_db = TRUE;
	    xloc = &jsx->jsx_x100_data;

	    xloc->phys_length = STlength(x100_path);
	    STmove(x100_path, ' ', sizeof(xloc->physical), (PTR)&xloc->physical);
	    STmove("ii_vwdata", ' ', 
		sizeof(xloc->logical.db_loc_name), (PTR)&xloc->logical.db_loc_name);
	    xloc->flags = DCB_DATA;
	    xloc->raw_start = 0;
	    xloc->raw_blocks = 0;
	    xloc->raw_total_blocks = 0;

	    /*
	    ** Right now, vw checkpoint files are in II_CHECKPOINT
	    ** (vs. Ingres checkpoint files are in ckp location for this db)
	    ** We havent decided how we will implement x100 locations:
	    ** - use one default ckp location for ingres and x100 tables
	    ** or
	    ** - allow different default ckp locations for ingres and x100 tables
	    */
#ifdef VMS
	    NMgtAt("II_CHECKPOINT", &value );
#else
	    NMgtIngAt("II_CHECKPOINT", &value );
#endif
	    if (value)
	    {
	        STcopy(value, x100_path);
	        ...
	        status = LOfaddpath(&x100_loc, "ingres", &x100_loc);
	        if (status == E_DB_OK)
		    status = LOfaddpath(&x100_loc, "ckp", &x100_loc);
	        if (status == E_DB_OK)
		    status = LOfaddpath(&x100_loc, "vectorwise", &x100_loc);

		/* save length of path without dbname for CKdircreate*/
		jsx->jsx_x100_ckp_pathlen = STlength(x100_path);

	        if (status == E_DB_OK)
		    status = LOfaddpath(&x100_loc, db_name, &x100_loc);
	        if (status == E_DB_OK)
	        {
		    xloc = &jsx->jsx_x100_ckp;
		    xloc->phys_length = STlength(x100_path);
		    STmove(x100_path, ' ', 
			sizeof(xloc->physical), (PTR)&xloc->physical);
		    STmove("ii_checkpoint", ' ', 
			sizeof(xloc->logical.db_loc_name), 
			(PTR)&xloc->logical.db_loc_name);
		    xloc->flags = DCB_CHECKPOINT;
		    xloc->raw_start = 0;
		    xloc->raw_blocks = 0;
		    xloc->raw_total_blocks = 0;
                }
...
**          for rollforwarddb. 
**      4-Jun-2011 (chris.dawe@iua.org.uk) SIR 125406
**          Allow -keep=<nnn> for ckpdb.
**      05-Jun-2011 (verde02) SIR 125421
**          For infodb allow "-databases" argument to print all database names
**
[@history_template@]...
*/
static VOID
...
/* ---> Removed until implemented
 ...
 <--- */
	...
	SIprintf(
	    "usage: infodb [#c[n]] [-uusername] [-databases]  {dbname}\n");
...
**      05-Jul-2011 (ashco01) Bug:124708.
**          Max Database Name length remains unchanged at 32, although
**          GL_MAXNAME now 256. Only write DB_DB_MAXNAME bytes to audit log.
*/
static DB_STATUS
audit_jsp_action(
	i4		action,
