...
**          Removed dm1b_bulk_put(), updated bulkpage() to decide where
**          to insert etab segment, (ASSOC or DISASSOC) data page.
**      24-Jun-2011 (hanal04) Bug 125477
**          Add db_id to REPOSITION TRdisplay() messages. A reltid may
**          exist in multiple DBs.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**      27-oct-2011 (stial01) b125899 SD 152855
**          prev(), next() unfix etab datapage before moving to another leaf
*/
    ...
    DB_ERROR    *dberr)
{
    ...
    */
	...
	if ( t->tcb_extended && r->rcb_et_seg_cnt)
	{
	    /* Try to use DISASSOC data page for etab segments */
...
**	09-Dec-2010 (thaju02) B124819
**	    Removed uninitialized bid ptr and reference to it, to
**	    avoid exception.
*/
DB_STATUS
dm1b_get(
	...
	if ( t->tcb_rel.relstat & (TCB_INDEX | TCB_CLUSTERED) )
	{
		...
		*/
		if ((r->rcb_update_mode == RCB_U_DEFERRED || crow_locking(r))&& 
		    dm1cx_isnew(r, r->rcb_other.page, (i4)r->rcb_lowtid.tid_tid.tid_line))
		{
		    SETDBERR(dberr, 0, E_DM003C_BAD_TID);
		    break;
		}
    ...
    DB_ERROR           *dberr)
{
	...
	*/
	...
	if (t->tcb_extended && r->rcb_data.page)
	{
	    /* Unfix data page to avoid etab deadlock (b125899) */
	    dm0pUnlockBuf(r, &r->rcb_data);
	    s = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_data, dberr);
	    if (s != E_DB_OK)
		break;
    ...
    DB_ERROR        *dberr)
{
    ...
    /* first check data page from previous insert */
    if (*data)
    {
	s = dm1r_allocate(r, dataPinfo, record, need, tid, dberr);
	if (s == E_DB_OK)
    ...
    /*
    ** Use ASSOC data page ... if we haven't buffered tcb_tperpage segments
    ** or if we're close to filling up current leaf
    ** (we dont want a leaf split to result in unfilled DISASSOC data page)
    */
    if ( r->rcb_et_seg_cnt < t->tcb_tperpage
	|| DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, *leaf) 
			> (t->tcb_kperleaf - t->tcb_tperpage))
    {
	s = findpage(r, leafPinfo, dataPinfo, record,
		    need, key, tid, bid, dberr);
	return (s);
    }
	...
	*/
	...
	if (DMZ_AM_MACRO(7))
	{
	    TRdisplay("bulkpage: New DISASSOC data page is %d, leaf = %d\n",
		DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, *newdata), 
		DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, *leaf));
	    TRdisplay("For table %t in database %t.\n",
		sizeof(DB_TAB_NAME), &t->tcb_rel.relid,
    ...
    DB_ERROR    *dberr)
{
    ...
    */
    ...
    {
	TRdisplay("REPOSITION to TIDEOF (%d %d %d) leaf %d, startleaf %d\n",
            t->tcb_dcb_ptr->dcb_id,
	    t->tcb_rel.reltid.db_tab_base, t->tcb_rel.reltid.db_tab_index,
    ...
    DB_ERROR       *dberr)
{
	...
	*/
	...
	{
	    TRdisplay("REPOSITION after LKWAIT, %p, xid %x db %d table %d\n  get saved-bid %d,%d tid %d,%d acc %d\n",
		r, r->rcb_tran_id.db_low_tran, t->tcb_dcb_ptr->dcb_id,
                t->tcb_rel.reltid.db_tab_base,
		r->rcb_pos_info[RCB_P_GET].bid.tid_tid.tid_page,
		r->rcb_pos_info[RCB_P_GET].bid.tid_tid.tid_line,
		tid_to_lock->tid_tid.tid_page, tid_to_lock->tid_tid.tid_line, access);
