...
**      01-Apr-2011 (horda03) b125231
**          Check length of segments when underlying datatype is
**          DB_NVCHR_TYPE. Fixed a few compiler warning too.
...
**      01-Apr-2011 (horda03) b125231
**          Check length of segments when underlying datatype is
**          DB_NVCHR_TYPE. Rather than checking against all the
**          datatypes for every segment read, workout before
**          reading the segments whether len check is required.
*/
static DB_STATUS
periph_check(
DM1U_CB		*dm1u,
	...
	*/
	...
	for (i = 1; i <= t->tcb_rel.relatts; i++)
	{
		...
		if (under_dv.db_datatype == DB_VCH_TYPE ||
		    ...
		    under_dv.db_datatype == DB_NVCHR_TYPE ||
