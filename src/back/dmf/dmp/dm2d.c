/*
** Copyright (c) 1985, 2010, 2011 Actian Corporation
**
**
...
**      19-aug-2009 (stial01) 
**          dm2d_open_db() check for charset conflict (b122505)
**      24-aug-2009 (stial01)
**          Partial undo of 19-aug-2009 change for createdb -S iidbdb (b122505)
...
**	22-mar-2011 (stial01)
**	    During upgrade dont set the TCB_VBASE since
    ...
    DB_ERROR	*dberr )
{
    ...
    */
	...
	if (dcb->dcb_ref_count <= 0)
	{
	    ...
	    aduucolterm((ADUUCETAB **)&dcb->dcb_ucollation);
    ...
    DB_ERROR		*dberr )
{
	...
	*/
	...
	if (dcb->dcb_id != 1 && CMischarset_utf8())
	{
	    if (( cnf->cnf_dsc->dsc_dbservice & DU_UTYPES_ALLOWED ) == 0)
	    {
		/* II_CHARSET is utf8 but utypes not allowed */
		SETDBERR(dberr, 0, E_DM907A_CHARSET_CONFLICT);
		status = E_DB_ERROR;
		break;
	    }
	}
...
DM0C_CNF *cnf)
{
    ...
    */
    ...
    core->relstat &= (TCB_PROALL | TCB_PROTECT | TCB_JON | TCB_JOURNAL
