...
**	    Changed all the page header access as macro for New Page Format
**	    project.
**      30-mar-2011 (coomi01) b125082
**          Allow for longer compressed keys in work space.
**      31-mar-2011 (coomi01) b125082
**          Add cautionary comment for future reference.
*/
static DB_STATUS
    ...
    DB_ERROR		*dberr)
{
    ...
    */
    ...
    {
	/*
	** When allocating work space, ensure extra 
	** space added for compression. Each compressed
	** key component might be two bytes longer than
	** the original!
        **
	** FIXME :: The hardwired 2 bytes comes from
	** the compression technique. If the technique
        ** ever changes, then the value may need changing
        ** or better still computed on a per algorithm
        ** basis. 
	*/
	KeyBufSize = DB_ALIGN_MACRO(t->tcb_rngklen + (2 * t->tcb_keys));
