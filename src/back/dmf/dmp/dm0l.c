...
**          Identify log records that require Archiver processing (flag
**          LGwrite calls with LG_ARCHIVER_TO_PROCESS). Log records that
**          need this flag are DM0LSBACKUP, DM0LEBACKUP, DM0LOPENDB and
**          DM0LECP.  
...
**      30-Mar-2011 (horda03) B124968
**          Add LG_ARCHIVER_TO_PROCESS flag. This log record needs to be processed
**          by the archiver even if the record isn't journalled or dumped.
*/
DB_STATUS
dm0l_opendb(
    i4		    lg_id,
    ...
    DB_ERROR	    *dberr)
{
    ...
    */
    ...
    status = LGwrite((LG_FIRST | LG_FORCE | LG_NOT_RESERVED | LG_LAST | LG_ARCHIVER_TO_PROCESS),
                      lx_id, (i4)1, &lg_object, &lri, &error);
    if (status != OK)
    {
    ...
    DB_ERROR	    *dberr)
{
    ...
    status = LGwrite((LG_FORCE | LG_FIRST | LG_NOT_RESERVED | LG_ARCHIVER_TO_PROCESS),
                     lg_id, (i4)1, &lg_object, &lri, &sys_err);
    if (status)
	return (log_error(E_DM9015_BAD_LOG_WRITE, &sys_err, lg_id,
		    E_DM9343_DM0L_ECP, dberr, status));
    ...
    DB_ERROR		*dberr)
{
    ...
    */
    ...
    status = LGwrite(LG_FIRST|LG_LAST|LG_FORCE|LG_NOT_RESERVED|LG_ARCHIVER_TO_PROCESS,
                     lx_id, (i4)1, &lg_object, &lri, &error);
    *lsn = lri.lri_lsn;
