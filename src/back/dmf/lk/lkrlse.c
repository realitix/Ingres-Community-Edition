...
**      03-May-2011 (ashco01) Bug: 125266
**          LK_Request/Release() concurrency can result in Mutex deadlock
**          in VMS Clustered environment. In LK_do_cancel() also release/reacquire
**          RSB Mutex during DLM lock release.
...
**       03-May-2011 (ashco01) Bug: 125266
**           Also release/reacquire RSB Mutex to prevent LLB/RSB Mutex deadlock
**           in VMS clustered environment.
*/
STATUS
	...
	*/
	if (lkb->lkb_attribute & LKB_DISTRIBUTED_LOCK)
	{
		...
		status = CXdlm_cancel( CX_F_PROXY_OK, &lkb->lkb_cx_req );
                ...
                /* Reacquire LLB mutex, but first release RSB mutex to 
                ** prevent blocking LK_requests leading to mutex deadlock */ 
                (VOID)LK_unmutex(&rsb->rsb_mutex);
                rsbIsMutexed = FALSE;
		     ...
		     (MutexStatus = LK_mutex(SEM_EXCL, &llb->llb_mutex)) )
		{
		    ...
		    return(MutexStatus);
                ...
                if ( MutexStatus = LK_mutex(SEM_EXCL, &rsb->rsb_mutex) )
                {
                    /* In this case also release LLB Mutex before returning */
                    if ( llb->llb_status & LLB_MULTITHREAD )
                        (VOID)LK_unmutex(&llb->llb_mutex);
                    return(MutexStatus);
                }
