...

/*{
** Name: dmd_prtbl - Prints table in page order.
**
** Description:
**      This routine prints the pages of a table in page order.
**
** Inputs:
**      rcb                           Pointer to RCB.
**      low_page                        First page to be dumped
**      high_page                       Last page to dump.
**
** Outputs:
**      none
**    Returns:
...
DM_PAGENO high_page)
{
    ...
    if (last < high_page) high_page = last;

    for (pageno = low_page; pageno <= high_page; pageno++)
    {
      s = dm0p_fix_page(rcb, pageno, DM0P_RPHYS, &pinfo, &local_dberr); 
        if (s)
      {
        ...
        dmve_trace_page_header( t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, pinfo.page, t->tcb_acc_plv, "DM725" );
        dmve_trace_page_contents(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, pinfo.page, "DM725" );
