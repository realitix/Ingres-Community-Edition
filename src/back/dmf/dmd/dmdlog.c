...
**	    pr_type now pr_status, LBB_STATE now LG_BUF_STATE,
**	    add LFB_ACTIVE_LOG define.
...
i4		    l_line_buf)
{
    ...
    case DM0LP1:
	{
		...
		for (i = 0; i < r->p1_count; i++)
		{
		    ...
		    LOCK_MODE,  r->p1_type_lk[i].lkb_grant_mode,
...
%80*-\n");
		...
		if (length)
		{
		    ...
		    lpb.pr_id, lpb.pr_pid, 
                       PR_STATUS, lpb.pr_status, &lpb.pr_stat, 0);
		}
	    } while ( length );
