...
**	12-july-2010 (dougi)
**	    Support X1000 temp tables.
**	20-Jul-2010 (kschendel) SIR 124104
**	    Pass in compression too.
**	29-Dec-2010 (kschendel) b124886
**	    A session GTT should start with the current savepoint ID, *not* -1,
**	    else a rollback of the txn won't delete the GTT!  Goodness knows
**	    how long THIS one has been broken. :-(
**	19-Apr-2011 (kschendel) b125281
**	    Always set XCCB back-pointer in temp TCB, otherwise a modify
**	    (of e.g. an etab tt) will lose the connection between XCCB
**	    and TCB -- because modify deletes the old TCB and creates a
**	    new one.
*/
DB_STATUS
dmt_create_temp(
DMT_CB  *dmt_cb)
{
	...
	/* Force structure to heap unless it is VW.  (IVW temp has to be
	** explicitly specified by caller.)
	*/
	if (structure <= DB_STDING_STORE_MAX)
	    structure = DB_HEAP_STORE;
...
# endif
	    ...
	    */
	    ...
	    /* Start the TT out with a savepoint ID of "current", so that
	    ** it's dropped if we rollback to or beyond that savepoint.
	    ** This will be updated if we commit.
	    */
	    ...
	    */
	    ...
	    /* Point TCB at XCCB, so that XCCB can be fixed up as needed
	    ** if something interesting happens at the TCB level.
	    */
...

/*
** Name: dmt_gtt_update -- Mark temp table as updated
**
** Description:
**
**	We want to have the same semantics for IVW temp tables as Ingres,
**	namely that if the TT is updated and then that transaction (or
**	savepoint) is rolled back, the temp table is dropped.  (I.e.,
**	all temp tables are "with norecovery".)  For ordinary Ingres
**	temp tables, we do this by updating the temp table's savepoint
**	ID when DMF realizes that it has inserted, deleted, or updated
**	a temp table row.
**
**	For VectorWise temp tables, this doesn't work.  Instead, the
**	X100 query driver calls this DMF entry point to indicate that
**	the X100 query just completed has updated an X100 temp table.
**	Even though the TT has structure vectorwise, it still has a
**	TCB and XCCB here in DMF, because that's how we track the
**	temp table definition.  So, it's not hard to follow the links
**	and update the XCCB savepoint ID with the current one.
**
**	DMT_GTT_UPD is called with a DMT_CB giving an open table
**	reference in its dmt_record_access_id.  Nothing else in the
**	DMT_CB is relevant, or changed, other than the error info.
**
** Inputs:
**	dmt		An open DMT_CB with valid dmt_record_access_id
**
** Outputs:
**	None, DMT_CB is unchanged other than error info
**	Returns E_DB_OK or error status
**
** Edit History:
**	4-Jan-2011 (kschendel)
**	    Invent to give IVW temp tables the same updating semantics
**	    as traditional Ingres temp tables.
*/

DB_STATUS
dmt_gtt_update(DMT_CB *dmt)
{
	...
	if (dm0m_check((DM_OBJECT *)rcb, RCB_CB) != E_DB_OK)
	    break;
	t = rcb->rcb_tcb_ptr;
	if (t->tcb_temp_xccb == NULL)
	    break;
	/* The whole point is to set the XCCB savepoint id: */
	t->tcb_temp_xccb->xccb_sp_id = rcb->rcb_xcb_ptr->xcb_sp_id;
	return (E_DB_OK);
    }
