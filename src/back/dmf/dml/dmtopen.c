...
**      06-Jan-2011 (maspa05) B124902
**          Bump readlock=nolock up to readlock=shared for constraint
**          checking (DMT_CONSTRAINT)
**      14-Sep-2011 (horda03) B125755
**          If no timeout and DMT_NO_LOCK_WAIT specified, don't set timeout
**          to DMC_C_NOWAIT.
*/
...
DMT_CB   *dmt_cb)
{
	...
	*/
	...
	/*
	** if called as part of constraint checking then override
	** READLOCK NOLOCK and set READLOCK SHARED instead
	*/
	if (dmt->dmt_flags_mask & DMT_CONSTRAINT &&
	    readlock==DMC_C_READ_NOLOCK)
	{
	    readlock=DMC_C_READ_SHARE;
	}
	...
	orig_timeout = timeout;
        ...
        if (dmt->dmt_flags_mask & DMT_NO_LOCK_WAIT)
        {
           ...
           if (timeout) timeout = DMC_C_NOWAIT;
        }
