...
**	    .xccb_t_tcb		TCB, if null nothing to do
...
**	    Drop IVW temp tables here.  The drop might be from a rollback,
**	    so we can't count on QEF doing it.  QEF X100-drop-table will no
**	    longer attempt a separate X100 drop if it's a temp table.
**	19-Apr-2011 (kschendel) b125281
**	    Check for a dangling XCCB (no TCB) and just return.
*/
...
dmt_destroy_temp(DML_SCB *scb, DML_XCCB *xccb, DB_ERROR *error)
{
    ...
    DMP_TCB   *t;		/* Temp table TCB */
    i4 	      lockid;		/* Lock list ID */
    RDF_CB    rdfcb;		/* RDF call request block */

    /* If no TCB, a lower level (i.e. dm2t) destroyed the TT and the
    ** xccb is a dangler.  This is not normal but can happen if some sort
    ** of error occurred.
    */
	...
	*/
    ...
    /* If it's an X100 / IVW table, drop the Vectorwise side.
    ** This is a somewhat unfortunate up-call to QEF, but the
    ** X100 interface needs session info that QEF has and we don't.
    ** If we have to call QEF for the info, we might as well have
    ** QEF call X100 as well.
    */
	...
	MEfill(sizeof(rcb), 0, &rcb);
	rcb.qef_length = sizeof(rcb);
	rcb.qef_type = QEFRCB_CB;
	rcb.qef_ascii_id = QEFRCB_ASCII_ID;
	rcb.qef_sess_id = scb->scb_sid;
	rcb.qef_tabname_p = &t->tcb_rel.relid;
	rcb.qef_eflag = QEF_INTERNAL;
	(void) qef_call(QEU_X100_DROPTT, &rcb);
    }
