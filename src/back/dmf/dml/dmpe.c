...
**          Fix dmpe_buffered_put() Save buffered segments in DMPE_PCB,
**          not DMP_RCB so that the buffered segments still available after
**          etab full error switches to new etab.
...
**	23-Aug-2011 (troal01) b125687
**	    Ensure the Geometry SRID check is only done on the FIRST segment
**	    of a LOB.
*/
static DB_STATUS
               ...
               DMPE_PCB  **pcb_ptr)
{
	...
	*/
	...
	pcb->pcb_loaded = 0;
	pcb->pcb_per_etab = 0;
	pcb->pcb_seg_alloc = 0; /* # buffers allocated */
	pcb->pcb_seg_cnt = 0; /* # buffered segments */
	pcb->pcb_seg_max = 0; /* max buffered segments */
	MEfill(sizeof(pcb->pcb_seg_ptrs), 0, &pcb->pcb_seg_ptrs);
	...
	*/
	...
	pcb->pcb_seg0_next = 0;
	pcb->pcb_seg1_next = 0;
	pcb->pcb_prev_table.db_tab_base = 0;
	pcb->pcb_prev_table.db_tab_index = 0;
	pcb->pcb_prev_seg0 = 0;
	pcb->pcb_prev_seg1 = 0;
...

{
	...
	if (pcb->pcb_seg_alloc)
	{
	    i4		i;
	    DMP_MISC	*misc_cb;

	    for (i = pcb->pcb_seg_alloc; i >= 1; i--)
	    {
		DMP_MISC *misc_cb;
		misc_cb = (DMP_MISC *)((char *)pcb->pcb_seg_ptrs[i] - sizeof(DMP_MISC));
#ifdef xDEBUG
		TRdisplay("dealloc seg %d misc_cb %x dmpe_rec %x\n", 
		    i, (char *)misc_cb, misc_cb->misc_data);
#endif
...
**	    Prototype fixing uncovered incorrect adi-per-under calls, fix.
*/
		...
		*/
		...
		do	/* break-out dummy */
		{
		    ...
		    */
			...
			DB_DATA_VALUE source_under_dv, att_under_dv;
			i4 dt_bits;
			...
			if (abs_source != abs_att)
			{
			    ...
			    */
			    ...
			    {
				status = adi_per_under(adf_scb, abs_source, &source_under_dv);
				if (status != E_DB_OK)
				    break;
				source_under = source_under_dv.db_datatype;
			    }
			    status = adi_per_under(adf_scb, abs_att, &att_under_dv);
			    if (status != E_DB_OK)
				break;
			    att_under = att_under_dv.db_datatype;
...
#define             DMPE_ATT_COUNT      5
	...
	*/
	...
	att_ents[0].attr_collID = DB_UNSET_COLL;
	...
	att_ents[1].attr_collID = DB_UNSET_COLL;
	...
	att_ents[2].attr_collID = DB_UNSET_COLL;
	...
	att_ents[3].attr_collID = DB_UNSET_COLL;
	...
	att_ents[4].attr_collID = DB_UNSET_COLL;
...
**	previous record is found is opened (using the pcb_prev_table table
...
**		pcb->pcb_prev_table		- table in which record
...

{
    ...
    /* We ended up placing a not-first segment in an etab which was not 
    ** the same as the previous segment's etab.
    ** So that we can find this segment, we must go back and fix up 
    ** the previous ** row to point to this new table.
    **
    ** To accomplish this, we will open the previous table, fetch the record, 
    ** alter its next pointer, rewrite it, and close the table.
    */
    ...
    do /* dummy loop */
    {
	segment0 = pcb->pcb_prev_seg0;
	segment1 = pcb->pcb_prev_seg1;
	...
	dmt_cb->type = DMT_TABLE_CB;
	dmt_cb->ascii_id = DMT_ASCII_ID;
	STRUCT_ASSIGN_MACRO(pcb->pcb_prev_table, dmt_cb->dmt_id);
	...
	if (status == E_DB_OK)
	{
	    ...
	    */
	    TRdisplay("nextchain fixup %~t %d %d ((%d,%d) (%d,%d) %d -> %d\n",
		16, r->rcb_tcb_ptr->tcb_rel.relid.db_tab_name,
		r->rcb_tcb_ptr->tcb_rel.relpgsize,
		r->rcb_tcb_ptr->tcb_rel.relspec,
		record->prd_log_key.tlk_low_id,
		record->prd_log_key.tlk_high_id,
		record->prd_segment0, record->prd_segment1,
		record->prd_r_next,
		(pop_cb->pop_segment ? pcb->pcb_dmtcb->dmt_id.db_tab_base:0));
...
DMR_CB		*dmr_cb)
{
    ...
    input_rec = (DMPE_RECORD *)dmr_cb->dmr_data.data_address;

    /*
    ** buffer segments only if etab is V2, BTREE and compression NONE 
    ** Never buffer first segment, so that this optimization doesn't
    ** waste space on the associated data page
    ** After allocating 1st segment bufer, this code assumes all subsequent
    ** segments will be buffered
    */
	...
	pcb->pcb_seg_max = 1;
	pcb->pcb_seg_cnt = 1;
	pcb->pcb_seg_ptrs[pcb->pcb_seg_cnt] = input_rec;
    }
    else
    {
	/* Buffer tcb_tperpage segments */
	pcb->pcb_seg_max = t->tcb_tperpage;
	
	/* Reset segcnt if input is first segment */
	if (input_rec->prd_segment0 == 0 && input_rec->prd_segment1 == 1)
	    pcb->pcb_seg_cnt = 0;

	/* If we got etab full error, caller may send same segment twice */
	if (!pcb->pcb_seg_cnt
	    || pcb->pcb_seg_ptrs[pcb->pcb_seg_cnt]->prd_segment0 !=
		input_rec->prd_segment0
	    || pcb->pcb_seg_ptrs[pcb->pcb_seg_cnt]->prd_segment1 !=
		input_rec->prd_segment1)
	{
	    pcb->pcb_seg_cnt++;

	    if (pcb->pcb_seg_alloc < pcb->pcb_seg_cnt)
	    {
		/*
		** Allocate memory to hold DMPE_RECORD
		** Ptr to this memory saved in the DMPE_PCB
		** dm0m_flag should be same as DMPE_PCB
		**
		*/
		if (DM_PCB_CB & DM0M_SHORTTERM)
		    dm0m_flag = DM0M_SHORTTERM;
		else
		    dm0m_flag = DM0M_LONGTERM;

		status = dm0m_allocate( sizeof(DMP_MISC) + sizeof(DMPE_RECORD),
			    dm0m_flag, (i4) MISC_CB, (i4) MISC_ASCII_ID, 
			    (char *) 0, (DM_OBJECT **)&misc_cb, &dmr_cb->error);
		if ( status == E_DB_OK )
		{
		    pcb->pcb_seg_alloc++;
		    misc_cb->misc_data = ((char*)misc_cb + sizeof(DMP_MISC));
#ifdef xDEBUG
		    TRdisplay("alloc seg %d misc_cb %x dmpe_rec %x\n", 
			pcb->pcb_seg_alloc,(char *)misc_cb,misc_cb->misc_data);
#endif
		    pcb->pcb_seg_ptrs[pcb->pcb_seg_alloc] = 
					(DMPE_RECORD *)(misc_cb->misc_data);
		}
		else
		{
		    pcb->pcb_seg_ptrs[pcb->pcb_seg_cnt] = NULL;
	    ...
	    if (status == E_DB_OK)
	    {
		STRUCT_ASSIGN_MACRO(*input_rec, 
		    *pcb->pcb_seg_ptrs[pcb->pcb_seg_cnt]);
#ifdef xDEBUG
		TRdisplay("buffer seg0 %d seg1 %d next %d pcb_seg_cnt %d\n",
		    input_rec->prd_segment0, input_rec->prd_segment1,
		    input_rec->prd_r_next, pcb->pcb_seg_cnt);
#endif
	    }
	}
    }

    /* put segments(s) if input is last segment, or segcnt = maxcnt */
    if (status == E_DB_OK &&
	(input_rec->prd_r_next == 0 || pcb->pcb_seg_cnt == pcb->pcb_seg_max))
	put_seg_cnt = pcb->pcb_seg_cnt;
    else
	put_seg_cnt = 0;

    for ( ; put_seg_cnt;  put_seg_cnt = pcb->pcb_seg_cnt)
    {
	put_rec = pcb->pcb_seg_ptrs[1];

	/* Fix prd_r_next = current etab base table id before dmr_put */
	if (put_rec->prd_r_next)
	    put_rec->prd_r_next = t->tcb_rel.reltid.db_tab_base;

	STRUCT_ASSIGN_MACRO(*dmr_cb, dmr_bulk_cb);
	dmr_bulk_cb.dmr_data.data_address = (PTR)put_rec;

	/* let dm1b know how many segments we have buffered */
	r->rcb_et_seg_cnt = pcb->pcb_seg_cnt;

	if (pcb->pcb_xcb->xcb_scb_ptr->scb_etfull_seg == put_rec->prd_segment1
	    && t->tcb_rel.reltid.db_tab_base > 0)
	{
	    /* possible if you did "set trace point dm37 segment-number" */
	    pcb->pcb_xcb->xcb_scb_ptr->scb_etfull_seg = 0;
	    status = E_DB_ERROR;
	    SETDBERR(&dmr_bulk_cb.error, 0, E_DM0138_TOO_MANY_PAGES);
	}
	else
	{
	...
	put_tid.tid_i4 = dmr_bulk_cb.dmr_tid;

#ifdef xDEBUG
	TRdisplay("put seg0 %d seg1 %d next %d status %d err %d tid (%d,%d)\n",
	    put_rec->prd_segment0, put_rec->prd_segment1,
	    put_rec->prd_r_next, status, dmr_bulk_cb.error.err_code,
	...
	if (put_rec->prd_segment0 == 0 && put_rec->prd_segment1 == 1)
	{
	    /* First segment0,segment1 (0,1), and already saved coupon */
	    if (put_rec->prd_log_key.tlk_low_id 
			== output->cpn_log_key.tlk_low_id
	    && put_rec->prd_log_key.tlk_high_id 
			== output->cpn_log_key.tlk_high_id)
	    {
#ifdef xDEBUG
		TRdisplay("coupon fixup ((%d,%d) (%d,%d) %d -> %d\n",
		    put_rec->prd_log_key.tlk_low_id,
		    put_rec->prd_log_key.tlk_high_id,
		    put_rec->prd_segment0, put_rec->prd_segment1,
		    output->cpn_etab_id, pcb->pcb_dmtcb->dmt_id.db_tab_base);
#endif
	...
	else if (pcb->pcb_prev_table.db_tab_base != 
		pcb->pcb_dmtcb->dmt_id.db_tab_base)
	{
	    /* previous segment put into different etab 
	    ** need dmpe_nextchain_fixup
	    */
	...
	/* Save last successful put-to etab for any future dmpe_nextchain_fixup
	*/
	STRUCT_ASSIGN_MACRO(pcb->pcb_dmtcb->dmt_id, pcb->pcb_prev_table);
	pcb->pcb_prev_seg0 = put_rec->prd_segment0;
	pcb->pcb_prev_seg1 = put_rec->prd_segment1;

	/* after successful put, decrement pcb_seg_cnt */
	pcb->pcb_seg_cnt--;

	/* shift DMPE_RECORD pointers */
	if (pcb->pcb_seg_cnt)
	{
	    MEcopy(&pcb->pcb_seg_ptrs[2], 
		    (pcb->pcb_seg_alloc-1) * sizeof(DMPE_RECORD *),
		    &pcb->pcb_seg_ptrs[1]); 
	    /* put the segment we just inserted at the end */
	    pcb->pcb_seg_ptrs[pcb->pcb_seg_alloc] = put_rec; 
	    
	}

	/*
	** if put_rec is not last seg, and we input_rec is not last seg
	** break (so we can buffer up some more segs)
	** we only want to insert to disassoc data pages when we have
	** tcb_tperpage segments buffered
	** (and we try to fill assoc data page first)
	*/
	if (put_rec->prd_r_next != 0 && input_rec->prd_r_next != 0)
	    break;
    }

    if (status != E_DB_OK)
    {
	/* caller will handle E_DM0138_TOO_MANY_PAGES */
	if (dmr_cb->error.err_code != E_DM0138_TOO_MANY_PAGES) 
	    uleFormat(&dmr_cb->error, 0, NULL, ULE_LOG , NULL, (char * )NULL,
		0L, (i4 *)NULL, &error, 0);
    }
    return (status);
}
