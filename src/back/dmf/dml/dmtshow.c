...
**          Added DMT_M_STATS (doesn't require DMT_TBL_ENTRY)
...
**	17-nov-2009 (dougi)
**	    Add TCB2_X100_INVIEW flag to relstat2.
*/
...
DMT_SHW_CB  *dmt_show_cb)
{
	...
	if ((dmt->dmt_flags_mask & 
              (DMT_M_TABLE|DMT_M_ATTR|DMT_M_NO_ATTR_NAMES|DMT_M_INDEX|DMT_M_LOC|DMT_M_COUNT_UPD|DMT_M_PHYPART|DMT_M_STATS)) == 0)
	{
	...
	** are looking at the information in it.
	...
	/*
	** Note DMT_M_STATS does not require caller to pass DMT_TBL_ENTRY,
	** stats are returned in DMT_SHW_CB
	*/
	if (dmt->dmt_flags_mask & (DMT_M_TABLE | DMT_M_COUNT_UPD | DMT_M_STATS))
	{
	...
	if (dmt->dmt_flags_mask & DMT_M_TABLE)
	{
	    ...
	    tbl->tbl_key_count = tcb->tcb_rel.relkeys;
	    ...
	    */
	    ...
	    if ( tcb->tcb_rel.relstat2 & TCB2_X100_INVIEW)
	    {   tbl->tbl_2_status_mask |= DMT_X100_INVIEW;}
	    ...
	    /* These are filled in by table type - but init 'em here anyway. */
	    tbl->tbl_dpage_count = 0;
