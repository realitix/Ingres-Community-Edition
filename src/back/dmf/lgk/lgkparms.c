/*
** Copyright (c) 1993, 2005 Actian Corporation
**
*/
...
**	22-Feb-2011 (wanfr01) Bug 125077
**	    Fail with error if log file size is missing.
*/
STATUS
lgk_get_config(RCONF *rconf, CL_ERR_DESC *sys_err, i4  wordy)
{
    ...
    */
	...
	{
	    /* Print an error onscreen, but not to errlog - this will flood
	    the errlog.log if it's a client installation!  */
	    TRdisplay("%@ Error retrieving log file size information.\n");
	    return (E_DMA805_LGK_CONFIG_ERROR);
	}
    }
    else
...
**	    Include X100 server stuff in the calculations.
*/
STATUS
lgk_calculate_size(
i4		wordy,
...
SIZE_TYPE	*totsize)
{
    ...
    */
    ...
    /* Include X100 stuff as well, one server per database.  The
    ** X100Context has one X100Server in it.
    */
#if defined(conf_IVW)
    size = sizeof(X100Context) + (plgkcounts->lgk_max_dbs-1) * sizeof(X100Server);
    if (MAX_SIZE_TYPE - total < size)
	overflowed = TRUE;
    total += size;
    subtotal += size;
