...
**          If retrying to load memory, then need to free the LGK memory
**          first.
*/

...
**	    Allocate and initialize X100 server context stuff.
**	14-Jun-2011 (kschendel)
**	    The X100 area was made one too large, fix.
**      04-Nov-2011 (horda03)
**          After LGK_destroy() don't attempt to free memory on next retry.
*/
STATUS
LGK_initialize(
i4	  	flag,
...
char		*lgk_info)
{
    ...
    */
	...
	cxmemreq = DB_ALIGN_MACRO(cxmemreq);
...
#if defined(conf_IVW)
    /* X100Context has one X100Server included */
    x100memreq = sizeof(X100Context) + (lgkcount.lgk_max_dbs-1) * sizeof(X100Server);
    x100memreq = DB_ALIGN_MACRO(x100memreq);
    /* Already included in memory_needed... */
#else
    x100memreq = 0;
#endif
    ...
    */
...
#endif
	{
            if (mem_exists == ME_ALREADY_EXISTS)
            {
               /* Loaded the memory once, so release it before trying again. */
	...
	if ( mem_exists = ret_val )
	{
		...
		ret_val = MEget_pages((me_locked_flag | 
                ...
                if (ret_val && !retries)
                {
                    /* Memory hasn't been loaded. */
                    mem_exists = OK; /* Mustn't be ME_ALREADY_EXISTS */
	...
	if ( mem_exists )
	{
	    ...
	    */
		...
		if ( LGK_destroy(allocated_pages, sys_err) )
		{
		    ...
		    break;
                ...
                /* Memory destroyed here, so don't attempt to free the
                ** pages on the next retry.
                */
                mem_exists = OK;
		continue;
	    }
		...
		*/
		...
		/* X100 memory will be past CX memory */
		x100mem = pcxmem + cxmemreq;
#if defined(conf_IVW)
		/* Tell x100 about the already-set-up context area */
		(void) x100_init((X100Context *)x100mem, 0, TRUE);
#endif
	...
	else
	{
	    ...
	    **	|							   |
	    **	------------------------------------------------------------
	    **  | If a VectorWise installation, X100 server context	   |
	    **	------------------------------------------------------------
	    ...
	    */
	    ...
	    LGK_base.lgk_lkd_ptr = x100mem + x100memreq;
	    ...
	    status = E_DB_OK;
#if defined(conf_IVW)
	    status = x100_init((X100Context *)x100mem, lgkcount.lgk_max_dbs, FALSE);
#endif
	    if (status != E_DB_OK)
	    {
		ret_val = E_DMA802_LGKINIT_ERROR;
	    ...
	    else if ( (ret_val = lgkm_initialize_mem(memleft, ptr)) == OK &&
