...
**	    Do VW index creation serially;  it's just catalog hacking on
**	    the Ingres side.
*/
STATUS
dm2u_pindex(
DM2U_INDEX_CB   *index_cbs)
{
    ...
    ** Ditto for vectorwise table, the Ingres side operation is just
    ** catalog updating.
    */
    if ( r->rcb_tcb_ptr->tcb_rel.relstat & TCB_IS_PARTITIONED
      || r->rcb_tcb_ptr->tcb_rel.relspec > DB_STDING_STORE_MAX)
    {
