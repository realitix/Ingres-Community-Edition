...
**	20-sep-2010 (dougi)
**	    No tidp needed for VectorWise indexes.
...
DM2U_INDEX_CB   *index_cb)
{
    ...
    *index_cb->indxcb_tup_info = 0;
    ...
    nofile = ((index_cb->indxcb_index_flags & DM2U_NOFILE) != 0);
...

	...
	**  Data fields are supported for all index types.
	    ...
	    */
        ...
        {
	    /* Gateway or Vectorwise table (nofile) */
	    ...
	    */
	    ...
	    if (bbase_tcb->tcb_rel.relgwid == GW_RMS)
	    {
    ...
    */
    if (!nofile && tp && tp->tpcb_locations && (tp->tpcb_locations[0].loc_fcb))
    {
        status = dm2f_release_fcb(lk_id, log_id,
		    tp->tpcb_locations,
	...
	i4 sources, i4 targets)
{
    ...
    i8 reltups;				/* Base table row count */
...
DB_ERROR	*dberr)
{
	...
	*/
	...
	if (m->mx_structure == TCB_RTREE)
        ...
        ** For gateway and VectorWise indexes, no TIDP column.
	**
...
DB_ERROR	*dberr)
{
    ...
    */
    status = dm2uGetTempTableLocs(dcb,
				       xcb->xcb_scb_ptr,
				       &idx->indxcb_location,
				       &idx->indxcb_l_count,
				       &locs_mem,
