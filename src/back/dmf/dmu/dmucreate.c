...
**      11-Feb-2009 (gefei01)
**          Remove X100 CREATE TABLE from DMU.
**	24-aug-2009 (stial01)
...
**	17-nov-2009 (dougi)
**	    Add support for X100 tables in views (TCB2_X100_INVIEW).
**	23-june-2010 (dougi)
**	    Track IVW tables and avoid DM0159 error.
...
dmu_create(DMU_CB        *dmu_cb)
{
	...
	*/
	...
	while ((indicator = BTnext(indicator, dmu->dmu_chars.dmu_indicators, DMU_CHARIND_LAST)) != -1)
	{
		...
		case DMU_X100_INVIEW:
		    relstat2 |= TCB2_X100_INVIEW;
		    break;
...
**          flag DM2U_SYS_MAINTAINED for temp tables.
**	12-july-2010 (dougi)
**	    Pass structure to dmt_create_temporary() in case it is a VW
**	    temp table.
**	20-Jul-2010 (kschendel) SIR 124104
**	    Pass optional compression.
*/

#define NUM_TEMP_CHARACTERISTICS 9
...
DB_ERROR	    *errcb)
{
    ...
    */
    ...
    characteristics[8].char_id = DMT_C_STRUCTURE;
    characteristics[8].char_value = structure;
