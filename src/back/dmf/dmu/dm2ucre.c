...
**	15-Mar-2010 (hweho01)
**	    Close the file descriptor after table is created in IVW mode.  
**	01-apr-2010 (stial01)
...
**      22-mar-2011 (stial01)
**          Don't set TCB_VBASE for view base tables since
...
**	    Don't create table files for Vectorwise tables.
*/
...
DB_ERROR	    *errcb)
{
    ...
    */
    if (view || gateway || (relstat & TCB_IS_PARTITIONED)
      || structure > DB_STDING_STORE_MAX)
    {
	nofile_create = TRUE;
	dm0l_create_flags |= DM0L_CREATE_NOFILE;
	if (view || gateway || structure > DB_STDING_STORE_MAX)
	    l_count = 1;
    }
	...
	/* After validating at least one location, if there's no file,
	** set loc-count and l-count to zero so that we make no further
	** attempt to do "stuff" with locations.
	** FIXME should prevent locations in the first place, but the
	** question is how many sep tests would break if we prevented
	** location= on vectorwise tables ... need to look into later!
	*/
	if (nofile_create)
	    loc_count = l_count = 0;
	...
	*/
	...
	/*
	** If an ordinary index then update the base table
	** accordingly. A concurrent index is only exposed once the
	** base table is modified back to NOREADONLY.
        */

	if (index && (!concurrent_index))
	{
	    /*
	    ** If the table creating is a an index, update the base
