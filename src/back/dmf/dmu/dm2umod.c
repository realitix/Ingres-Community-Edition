...
**      27-jan-2011 (stial01)
**          dm2u_update_reltups() DM2U_2_REP_RELTUP just update tcb if possible.
**          If replace iirelation.reltups, toss tcb mcb_tbl_id tcb afterwards.
...
static DB_STATUS dm2u_update_reltups(
			DM2U_MOD_CB *mcb,
			DB_ERROR *dberr);
...
**	14-feb-2010 (dougi)
**	    New code to update reltups for X100 tables.
...
DB_ERROR        *dberr)
{
    ...
    */
	...
	reorg = 1;
    if (mcb->mcb_mod_options2 & (DM2U_2_UPD_RELTUPS | DM2U_2_REP_RELTUPS))
	return dm2u_update_reltups(mcb, dberr);
	...
	if (! gateway)
	{
		...
		*/
		    ...
		    t->tcb_rel.reltid.db_tab_index == 0)
		{
			...
			r = rcb;
			m->mx_rcb = r;
			m->mx_newtup_cnt = (i4) *mcb->mcb_tup_info;
...
**	    Tweak reconnect of XCCB and TCB to make sure that we're dealing
**	    with a base tt XCCB (not an index).
*/
static DB_STATUS
update_temp_table_tcb(
DM2U_MXCB           *mxcb,
DB_ERROR      	    *dberr)
{
	...
	*/
	...
	/* Save pointer to XCCB so we can reconnect XCCB to the new
	** TCB that we'll create.
	*/
	xccb = t->tcb_temp_xccb;
	...
	*/
	...
	/* Reconnect XCCB and (new) TCB if originally connected */
	if ( xccb != NULL )
	{
	    t->tcb_temp_xccb = xccb;
	    if (xccb->xccb_t_tcb == NULL)
		xccb->xccb_t_tcb = t;
	}
...

/*
** Name: dm2u_update_reltups	- update iirelation.reltups (that's all)
**
** Description:
**
**	This routine updates the reltups value of a given table - initially
**	(and maybe only ever) for X100 tables, to give them a reasonable
**	reltups value after COPY FROM statements.
**
** Inputs:
**	mcb			Modify info block with actions in modopts2
**	dberr			Ptr to result code
**
** Outputs:
**	err_code		E_xxx error code returned here, if any
**	Returns E_DB_xxx status
...
** History:
**	15-feb-2010 (dougi)
**	    Written to assure reasonable row counts for X100 tables.
**	13-dec-2010 (dougi)
**	    Increment reltups - don't replace it.
**	7-Jan-2011 (kschendel)
**	    Make sure we close iirelation even if table not found.
*/

static DB_STATUS
dm2u_update_reltups(DM2U_MOD_CB *mcb,
	DB_ERROR *dberr)
{
    DB_STATUS	status = E_DB_OK;
    DMP_RCB	*r;
    DM_TID	tid;
    DB_TAB_ID	table_id;
    DM2R_KEY_DESC qual_list;
    DMP_RELATION rel;
    DB_TAB_TIMESTAMP timestamp;
    DMP_TCB	*tcb;

    if (mcb->mcb_mod_options2 & DM2U_2_UPD_RELTUPS)
    {
	r = (DMP_RCB *)mcb->mcb_dmu->dmu_record_access_id;
	if (r && r->rcb_tcb_ptr->tcb_rel.reltid.db_tab_base 
			== mcb->mcb_tbl_id->db_tab_base)
	{
	    r->rcb_tup_adds += mcb->mcb_reltups;
	    r->rcb_page_adds += 0;
	    return (E_DB_OK);
	}

	/* try to update the in-memory reltups using existing protocols */
	status = dm2t_fix_tcb(mcb->mcb_dcb->dcb_id, mcb->mcb_tbl_id, 
		&mcb->mcb_xcb->xcb_tran_id, 
		mcb->mcb_xcb->xcb_lk_id, mcb->mcb_xcb->xcb_log_id,
		(i4)(DM2T_NOBUILD | DM2T_NOWAIT), mcb->mcb_dcb, &tcb, dberr);
	if (status == E_DB_OK)
	{
	    dm0s_mlock(&tcb->tcb_mutex);

	    /* Roll x100 tup_adds and page_adds deltas into TCB */
	    tcb->tcb_tup_adds  += mcb->mcb_reltups;
	    tcb->tcb_page_adds += 0;  /* not implemented */
	    
	    dm0s_munlock(&tcb->tcb_mutex);

	    status = dm2t_unfix_tcb(&tcb, 0, 
		    mcb->mcb_xcb->xcb_lk_id, mcb->mcb_xcb->xcb_log_id, dberr);

	    return (status);
	}
    }

    /*  Update the iirelation record.  reltups, only. */
    
    table_id.db_tab_base = DM_B_RELATION_TAB_ID;
    table_id.db_tab_index = DM_I_RELATION_TAB_ID;
    qual_list.attr_number = DM_1_RELATION_KEY;
    qual_list.attr_operator = DM2R_EQ;
    qual_list.attr_value = (char *)mcb->mcb_tbl_id;

    status = dm2t_open(mcb->mcb_dcb, &table_id, DM2T_IX,
		DM2T_UDIRECT, DM2T_A_WRITE,
		(i4)0, (i4)20, (i4)0,
		mcb->mcb_xcb->xcb_log_id,mcb->mcb_xcb->xcb_lk_id,
		(i4)0, (i4)0, DM2T_S, &mcb->mcb_xcb->xcb_tran_id,
                &timestamp, &r, (DML_SCB *)0, dberr);
    if (status != E_DB_OK)
	return(status);

    r->rcb_xcb_ptr = mcb->mcb_xcb;
    r->rcb_logging = 0;
    r->rcb_usertab_jnl = 0;
	...
	if (status != E_DB_OK)
	{
	    ...
	    (void) dm2t_close(r, DM2T_NOPURGE, dberr);
	    return(status);
	}
    }
    while (rel.reltid.db_tab_base != mcb->mcb_tbl_id->db_tab_base
	   || rel.reltid.db_tab_index != mcb->mcb_tbl_id->db_tab_index);

    rel.relmoddate = TMsecs();
    if (mcb->mcb_mod_options2 & DM2U_2_REP_RELTUPS)
	rel.reltups = mcb->mcb_reltups;	/* replace reltups or ... */
    else rel.reltups += mcb->mcb_reltups;	/* just mod reltups */
    ...
    /*
    ** Try to fix statistics in the TCB
    ** This will only work if the tcb isn't fixed by some other session
    */
    status = dm2t_fix_tcb(mcb->mcb_dcb->dcb_id, mcb->mcb_tbl_id, 
		&mcb->mcb_xcb->xcb_tran_id, 
		mcb->mcb_xcb->xcb_lk_id, mcb->mcb_xcb->xcb_log_id,
    ...
    if (status == E_DB_OK)
    {
	status = dm2t_unfix_tcb(&tcb, DM2T_TOSS, 
		mcb->mcb_xcb->xcb_lk_id, mcb->mcb_xcb->xcb_log_id, dberr);
    }
    else
	status = E_DB_OK;
