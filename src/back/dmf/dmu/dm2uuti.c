...
**	15-sep-2009 (dougi)
**	    Take quick exit when modifying X100 table structures.
...
DB_ERROR	    *dberr)
{
	...
	/* If we get this far and this is a X100 table, it is probably
	** safe to return without doing the actual work. Leaving earlier 
	** seems to leave some fields used to update iirelation/iiattribute
	** uninitialized. This is an inexact science!
	*/
	if (m->mx_structure > DB_STDING_STORE_MAX)
	    return(status);
    }
...
**      07-Sep-2011 (coomi01) b125699
**          Check iintegrity and then iikey for dropped column, 
**          to make sure that and new column positions correspond.
*/
DB_STATUS
...
DB_ERROR      	    *dberr)
{
    ...
    DB_TAB_ID         ii_integ;
    DB_INTEGRITY      integRec;
    DB_TAB_ID           ii_key;
    DB_IIKEY          iikeyRec;
    i4               kEstimate;
    i4           kRecorded = 0;
    i4          kAllocated = 0;
    DM_OBJECT    *allocPtr = 0;
    DB_CONSTRAINT_ID *kSet = 0;
    i4                newIdent;
    DM2R_KEY_DESC   subqual[2];
	...
	if (m->mx_reorg == 0)
	{
		...
		*/
	    ...
	    /* 
	    ** Bug 125699
	    ** iikeys was out of step with iiintegrity after a column drop.
	    ** Having made adjustments for the column drop above, we now 
	    ** tidy up iikeys in keeping.
	    */
	    if (0 < drop_cnt)
	    {
		/* Open iiintegrity */
		ii_integ.db_tab_base  = DM_B_INTEGRITY_TAB_ID;
		ii_integ.db_tab_index = DM_I_INTEGRITY_TAB_ID;
		status = dm2t_open(m->mx_dcb, &ii_integ, DM2T_IX, DM2T_UDIRECT,
				    DM2T_A_WRITE, (i4)0, (i4)20, 
				    (i4)0, m->mx_log_id, m->mx_lk_id,
				    (i4)0, (i4)0, DM2T_S, 
				    &m->mx_xcb->xcb_tran_id, &timestamp, &rcb, 
		...
		/* Assign pointer, and do setup */
		r = rcb;
		r->rcb_xcb_ptr = m->mx_xcb;

		/*
		 *  Estimate integrity constraints on our input table 
		 */
		kEstimate = 0;
		status = dm2r_position(r, DM2R_QUAL, qual_list,
				       (i4)2,
		...
		{
		    /*
		    **  Allocate more than enough room for the data, 
		    **   (There might be more constraints than max columns)
		    */
		    if (kEstimate > kAllocated)
		    {		    
			if ((DM_OBJECT *)0 != allocPtr)
			{
			    /* Been here before but not enough this time
			    ** around, release it before grabbing some more
			    */
			    dm0m_deallocate(&allocPtr);
			    kAllocated = 0;
			    allocPtr = (DM_OBJECT *)0;
			}

			/* Add some headroom first */
			kEstimate += DB_MAX_COLS;

			status = dm0m_allocate( sizeof(DMP_MISC) + kEstimate * sizeof(DB_CONSTRAINT_ID), 
						0, (i4)MISC_CB, 
			...
			/* Grab our memory just beyound end of alloc header */
			kSet = (DB_CONSTRAINT_ID *)((char *)allocPtr + sizeof(DMP_MISC));

			/* Record no of slots allocated */
			kAllocated = kEstimate;
		    }

		    /*
		     * Read integrity constraints on our input table 
		     */ 
		    ...
		    /* Loop, but do not overflow allocation */
		    while (kRecorded < kAllocated)
		    {	
			status = dm2r_get(r, &tid, DM2R_GETNEXT,
					  (char *)&integRec, dberr);
			...
			/* Record the details */
			kSet[kRecorded].db_tab_base  = integRec.dbi_cons_id.db_tab_base; 
			kSet[kRecorded].db_tab_index = integRec.dbi_cons_id.db_tab_index;

			/* Increase number of recorded items */
			kRecorded++;
		    }
		...
		if (0 < kRecorded)
		{
		    ii_key.db_tab_base    = DM_B_IIKEY_TAB_ID;
		    ii_key.db_tab_index   = DM_I_IIKEY_TAB_ID;

		    /* Use our own qual array */
		    subqual[0].attr_number = DM_1_RELATION_KEY;
		    subqual[0].attr_operator = DM2R_EQ;
		    subqual[0].attr_value = (char *)&iikeyRec.dbk_constraint_id.db_tab_base;
		    subqual[1].attr_number = DM_2_ATTRIBUTE_KEY;
		    subqual[1].attr_operator = DM2R_EQ;
		    subqual[1].attr_value = (char *)&iikeyRec.dbk_constraint_id.db_tab_index;

		    /*
		    ** Step through each constraint found 
		    */
		    while (kRecorded--)
		    {
			/* Re-open iikey */
			status = dm2t_open(m->mx_dcb, &ii_key, DM2T_IX, DM2T_UDIRECT,
					   DM2T_A_WRITE, (i4)0, (i4)20, 
					   (i4)0, m->mx_log_id, m->mx_lk_id,
					   (i4)0, (i4)0, DM2T_S, 
					   &m->mx_xcb->xcb_tran_id, &timestamp, &rcb, 
			...
			/*
			 * Search for integrity constraint on our input table 
			 */ 
			iikeyRec.dbk_constraint_id.db_tab_base  = kSet[kRecorded].db_tab_base;
			iikeyRec.dbk_constraint_id.db_tab_index = kSet[kRecorded].db_tab_index;

			status = dm2r_position(r, DM2R_QUAL, subqual,
					       (i4)2,
					       (DM_TID *)0, dberr);
			...
			if (status == E_DB_OK)
			{
				...
				/*
				** Look to see if we need change the value 
				** ~ new_map is an identity map for unshifted content.
				**   Where a shift occurs the old value indexes the new.
				*/
				newIdent = new_map[iikeyRec.dbk_column_id.db_att_id];
				if ( newIdent == iikeyRec.dbk_column_id.db_att_id )
				{
				    /* No change */
				    continue;
				}

				/* We do need to rewrite the entry */
				iikeyRec.dbk_column_id.db_att_id = newIdent;
				status = dm2r_replace(r, &tid, DM2R_BYPOSITION,
					     (char *)&iikeyRec, 
					     (char *)NULL, dberr);
	    ...
	    } /* End of non zero drop_cnt updates to iikey */
	...
	for (;;)
	{
		...
		*/
		nofiles = (rel.relstat & TCB_GATEWAY)
				|| (rel.relspec > DB_STDING_STORE_MAX);
	    }
	    else if ((rel.reltid.db_tab_base ==
					m->mx_table_id.db_tab_base) &&
		      (rel.relstat & TCB_INDEX) == 0)
	...
	** Check for case of table that has no underlying files.
	...
	/* Ensure no malloc'd memory remains lurking */
	if ((DM_OBJECT *)0 != allocPtr)
	{
	    dm0m_deallocate(&allocPtr);
	    allocPtr = (DM_OBJECT *)0;
...
**	9-Feb-2010 (dougi)
**	    Allow mct to be init'ed, then leave for X100.
*/
...
dm2uInitTpcbMct(DM2U_TPCB *tp)
{
    ...
    */
    ...
    /* X100 requires the above mct init - then we leave. */
    if (m->mx_structure > DB_STDING_STORE_MAX)
	return(E_DB_OK);
