...
**	    Deal properly with no-file (VW) indexes.
*/
DB_STATUS
dmuIndexSetup(
DMU_CB		*dmu,
PTR		ix_cb)
{
	...
	if ( dmu->dmu_flags_mask & ~(DMU_ONLINE_END | DMU_ONLINE_START | DMU_BASE_NOFILE) )
	{
	...
	*/
	...
	indx_cb->indxcb_unique = FALSE;
	indx_cb->indxcb_min_pages = 0;
	indx_cb->indxcb_max_pages = 0;
	if (dmu->dmu_flags_mask & DMU_BASE_NOFILE)
	    indx_cb->indxcb_index_flags |= DM2U_NOFILE;
	...
	while ((indicator = BTnext(indicator, dmu->dmu_chars.dmu_indicators, DMU_CHARIND_LAST)) != -1)
	{
		...
		*/
	    ...
	    case DMU_GATEWAY:
		indx_cb->indxcb_index_flags |= DM2U_GATEWAY | DM2U_NOFILE;
	...
	*/
	...
	{
	    /* Note that even nofile indexes have to have a location.
	    ** This should be fixed, but there are too many scary assumptions
	    ** about relloccount >= 1 to fix for now.
	    */
	    if (dmu->dmu_location.data_address &&
