...
**          dmv_reput() skip redo put into iirtemp for iirelation and
**          iiridxtemp if corresponding row not found in iirelation
**          dmv_unput() ignore undo put errors for iirtemp,iiridxtemp
*/
...
DMP_PINFO	    *pinfo)
{
	...
	*/
    ...
    /*
    ** iirtemp is created by sysmod for SCONCUR catalogs 
    ** The code below applies only to inserts into iirtemp for iirelation 
    */
    if ((dmve->dmve_action == DMVE_DO) &&
	...
	*/
	...
	DMP_RELATION   cur_reltup;
	DMP_RELATION   log_reltup;
	DB_STATUS      rel_status;
        DB_TAB_ID      rel_id;
	DMP_TCB	       *iirel_tcb = dcb->dcb_rel_tcb_ptr;

	/*
	** iirelation tups are compressed, but all the compressed fields 
	** have been placed at the end of the record.. so we can look
	** at reltid without uncompressing 
	** First copy to an aligned structure
	*/
	...
	/* We'll get a warning if we had a non-journalled CREATE */
        ...
        rel_status = dmve_get_iirel_row(dmve, &rel_id, &cur_reltup); 
	...
	if(rel_status == E_DB_OK)
	{
	    ...
	    */
		...
		(char *)&cur_reltup, sizeof(DMP_RELATION), creltup, &csize);
		...
		TRdisplay("DM0LPUT fix iirtemp for %~t rec_size %d csize %d\n",
		    cur_reltup.relid.db_tab_name, log_rec->put_rec_size, csize);
		SETDBERR(&dmve->dmve_error, 0, E_DM963B_REDO_PUT);
		return(E_DB_ERROR);
	    }
	}

	if (rel_status == E_DB_WARN)
	{
	    /*
	    ** Dont redo insert into iirtemp since the row is not in iirelation
	    ** Add an action so that we skip the DM0LPUT(iirelidx) 
	    ** for this table 
	    */
	    status = (*iirel_tcb->tcb_data_rac.dmpp_uncompress)
		(&iirel_tcb->tcb_data_rac, 
		(char *)log_rel, (char *)&log_reltup, sizeof(DMP_RELATION), 
		&size, NULL, 0, (ADF_CB *)NULL);
	    if (status == E_DB_OK)
	    {
		status = rfp_add_action(dmve, &dmve->dmve_tran_id, 
			&log_rec->put_header.lsn, RFP_IIRTEMP_SKIP, &action);
		if (status == E_DB_OK)
		{
		    action->rfp_act_dbid = log_rec->put_header.database_id;
		    STRUCT_ASSIGN_MACRO(log_rec->put_tbl_id, 
				action->rfp_act_tabid);
		    STRUCT_ASSIGN_MACRO(log_reltup.relowner, 
				action->rfp_act_tabown);
		    STRUCT_ASSIGN_MACRO(log_reltup.relid, 
				action->rfp_act_tabname);
		    TRdisplay("SKIP insert into iirtemp for %~t %~t \n",
			DB_OWN_MAXNAME, log_reltup.relowner.db_own_name,
			DB_TAB_MAXNAME, log_reltup.relid.db_tab_name);
		    return (E_DB_OK);
		}
	    }

	    /*
	    ** Issue an warning but let the rollforward continue
	    ** The dba can fix up iirelidx inconsistencies after
	    ** rollforwarddb by doing "sysmod dbname iirelation"
	    */
	    uleFormat(NULL, E_DM9692_DMVE_NONJNL_PUT, (CL_ERR_DESC *)NULL, 
		ULE_LOG, NULL, (char *)NULL, (i4)0, (i4 *)NULL, err_code, 3,
		DB_TAB_MAXNAME, log_reltup.relid.db_tab_name,
		DB_OWN_MAXNAME, log_reltup.relowner.db_own_name,
		DB_DB_MAXNAME, tabio->tbio_dbname->db_db_name);
	    return (E_DB_OK);
	}
    }

    /*
    ** iiridxtemp is created by sysmod for iirelidx
    ** The code below applies only to inserts into iiridxtemp
    */
    if ((dmve->dmve_action == DMVE_DO) && 
	(MEcmp(tabio->tbio_relid->db_tab_name, "iiridxtemp", 10) == 0 ||
	MEcmp(tabio->tbio_relid->db_tab_name, "IIRIDXTEMP", 10) == 0))
    {
	DMP_RINDEX	iiridxtemp_tup;
	DB_STATUS	ridx_status;
	DMP_TCB *iirelidx_tcb = dcb->dcb_relidx_tcb_ptr;
	i4		size;

	ridx_status = (*iirelidx_tcb->tcb_data_rac.dmpp_uncompress)
		    (&iirelidx_tcb->tcb_data_rac, 
		    log_row, (char *)&iiridxtemp_tup, sizeof(DMP_RINDEX), &size,
		    NULL, 0, (ADF_CB *)NULL);

	for (action = NULL; ridx_status == E_DB_OK; )
	{
	    ...
	    if (action->rfp_act_action == RFP_IIRTEMP_SKIP &&
		    MEcmp(action->rfp_act_tabname.db_tab_name, 
		    iiridxtemp_tup.relname.db_tab_name, DB_TAB_MAXNAME) == 0
		&& MEcmp(action->rfp_act_tabown.db_own_name, 
		    iiridxtemp_tup.relowner.db_own_name, DB_OWN_MAXNAME) == 0)
	    {
		/* Skip insert into iiridxtemp also */
		TRdisplay("SKIP insert into iiridxtemp for %~t %~t \n",
		    DB_OWN_MAXNAME, iiridxtemp_tup.relowner.db_own_name,
		    DB_TAB_MAXNAME, iiridxtemp_tup.relname.db_tab_name);
...
DMP_PINFO	    *pinfo)
{
    ...
    */
    ...
    {
	/* Ignore undo put errors for iirtemp,iiridxtemp */
	if (MEcmp(tabio->tbio_relid->db_tab_name, "iirtemp", 7) == 0 ||
	    MEcmp(tabio->tbio_relid->db_tab_name, "IIRTEMP", 7) == 0 ||
	    MEcmp(tabio->tbio_relid->db_tab_name, "iiridxtemp", 10) == 0 ||
	    MEcmp(tabio->tbio_relid->db_tab_name, "IIRIDXTEMP", 10) == 0)
	{
	    /*
	    ** we might have 'skipped' put for non-jnl tables
	    ** Ignore undo put errors, if undo, this sysmod temp is deleted 
	    */
	    return (E_DB_OK);
	}
