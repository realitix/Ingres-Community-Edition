...
**          Delete VectorWise checkpoints. Added dmm_x100_ckp_del()
**	16-May-2011 (drivi01) m1871
**	    Remove DELETE_INFO from this file, it's been moved to dm.h.
**	    Remove static from del_ck_files, it will be now externally
**	    exposed to dmf files.
...
static DB_STATUS destroy_loc_list(
			DMM_CB    *dmm_cb);
static DB_STATUS dmm_x100_ckp_delete(char *dbname);
	...
	*/
	...
	for (i = 0; i < cnf->cnf_c_ext; i++)
	{
	    ...
	    */
		...
		/* Also for now, dmf is handling x100 checkpoints */
		local_status = dmm_x100_ckp_delete(db_buffer);
		if (local_status)
		    status = local_status;
...
/*{
** Name: dmm_x100_ckp_del - Delete VectorWise checkpoint files/directories
**
** Description: Delete Vectorwise checkpoint files and directories
**
** Inputs:
**      dbname
**
...
*/
static DB_STATUS
dmm_x100_ckp_delete(char *dbname)
{
    DELETE_INFO		delete_info;
    LOCATION		x100_loc;
    char		x100_path[MAX_LOC + 1];
    i4			x100_pathlen;
    STATUS		status;
    CL_ERR_DESC		sys_err;
    char		*value;
	...
	if (status == E_DB_OK)
	    status = LOfaddpath(&x100_loc, dbname, &x100_loc);
    }
    x100_pathlen = STlength(x100_path);

    delete_info.error = OK;
    delete_info.path_name = x100_path;
    delete_info.path_length = x100_pathlen;

    status = CKlistfile( x100_path, x100_pathlen,
	del_ck_files, (PTR)&delete_info, &sys_err);
    if (status == CK_ENDFILE)
    {
	status = delete_info.error;
	if (status == OK)
	    status = CKdirdelete(x100_path, x100_pathlen, 0, 0, &sys_err);
    }
    else if (status == CK_DIRNOTFOUND)
	status = OK;
