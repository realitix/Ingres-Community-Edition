/* Copyright (c) 1995, 2005 Actian Corporation
**
**
*/
...
DMF_ATTR_ENTRY  **dmf_attr)
{
   ...
   QEF_DATA        vqef_data;
   i4		   obj_type = PSQ_DB_TABLE; /* To set the key value for iidbdepends request */
   i4		   tree_mode = PSQ_VIEW;  /* To set the key value for iitree request */ 
	...
	*/
        ...
        if (dtuple->dbv_dtype == PSQ_VIEW)
	{
		...
		*/
        ...
        else if (dtuple->dbv_dtype == PSQ_CREDBP)
        {
	...
	break;
     ...
     for (;;)
     {
	    ...
	    if (prtuple->db_dep_obj_type == PSQ_VIEW)
            {
		...
		** used to look up information on views.
	    ...
	    else if ( prtuple->db_dep_obj_type == PSQ_CREDBP )
    	    {
              xdbpqeu.qeu_getnext = QEU_REPO;
              xdbpqeu.qeu_flag = 0;
              xdbpqeu.qeu_klen = 2;
