...
**	    Fix for bug b119490.
**      11-Feb-2009 (gefei01)
**          Support X100 CREATE TABLE.
**	7-may-2009 (dougi)
**	    Move X100 CREATE TABLE to qeax100.c.
**	07-Dec-2009 (troal01)
...
static bool qeu_valid_srid(
	DMU_CB *dmu,
	QEF_CB *qef,
	i4 *errcode);

static DB_STATUS qeu_drop_restrict_check(
    QEF_CB	*qef_cb,
    QEUQ_CB	*qeuq_cb,
    ULM_RCB	*ulm,
    bool	modify_check,
...
**	3-may-2009 (dougi)
**	    No modify's (yet) with X100 tp set.
...
**	    Set restrict/cascade from the incoming DMU CB.
*/
DB_STATUS
qeu_dbu(
QEF_CB		*qef_cb,
...
i4		caller )
{
    ...
    /* If X100 tp is on, only perform create/drop. Otherwise, return 
    ** success. */
    if ((ult_check_macro(&qef_cb->qef_trace, QEF_TRACE_NO_X100_STRUCT,
				  &dummy1, &dummy2) ||
	ult_check_macro(&Qef_s_cb->qef_trace, QEF_T_X100_STRUCT - QEF_SSNB,
				  &dummy1, &dummy2)) &&
	opcode != DMU_CREATE_TABLE && opcode != DMU_DESTROY_TABLE)
	return(E_DB_OK);
...

	...
	*/
	...
	if ( opcode == DMU_DESTROY_TABLE || dmu_ptr->dmu_action == DMU_ALT_DROPCOL )
	{
	    DMT_SHW_CB		dmt_show;
	    ...
	    */
	    ...
	    /* If DROP RESTRICT, check for dependent objects.  The innards
	    ** of qeu-d-cascade are sufficiently obscure that it seems best
	    ** to simply pre-check, even if it might not be the most
	    ** efficient way.
	    ** Temp tables don't need checking, they can't have dependent
	    ** objects (they have no catalog presence).
	    **
	    ** DROP INDEX CASCADE is a bit quirky.  We still check for
	    ** constraints that depend on the index.  The reason is that the
	    ** constraint belongs to the base table, not the index;  it's
	    ** inappropriate to cascade a constraint deletion that affects
	    ** the base table, simply because we're dropping an index used
	    ** to implement the constraint.  The user would need to drop
	    ** the constraint first.  As an escape hatch, allow the
	    ** DROP INDEX CASCADE to proceed without checking if the
	    ** no-dependency-check tracepoint or config setting is set.
	    ** qeu-d-cascade will drop the index but not the constraint(s).
	    */
	    if (opcode == DMU_DESTROY_TABLE
	      && ( !BTtest(DMU_CASCADE, dmu_ptr->dmu_chars.dmu_indicators)
		...
		cascade = BTtest(DMU_CASCADE, dmu_ptr->dmu_chars.dmu_indicators);
                if (!memoryStreamOpened)
                {
                    /* get a memory stream for the checking */
                    STRUCT_ASSIGN_MACRO(Qef_s_cb->qef_d_ulmcb, ulm);
                    if ((status = qec_mopen(&ulm)) != E_DB_OK)
                    {
                        err = ulm.ulm_error.err_code;
		...
		status = qeu_drop_restrict_check(qef_cb, &qeuq_cb,
				&ulm, cascade, &dep_string, &err);
		if (status != E_DB_OK)
		{
		    goto exit;
		}
		else if (dep_string != NULL)
		{
		    char *opstring = "DROP RESTRICT";
		    if (cascade)
			opstring = "DROP";
		    /* N.B. qef-error substitutes E_US2490 */
		    qef_error(I_QE2037_DROP_RESTRICT_ERR, 0L,
			       status, &err, error_block, 4,
			       strlen(opstring), opstring,
			       (sizeof("drop") - 1), "drop",
			       qec_trimwhite(DB_TAB_MAXNAME,
			       dmt_tbl_entry.tbl_name.db_tab_name),
			       dmt_tbl_entry.tbl_name.db_tab_name,
			       0, dep_string);
		    status = E_DB_ERROR;
		...
		*/
		...
		{
		    obj_type = PSQ_DB_TABLE;

		    if (dmu_ptr->dmu_action == DMU_ALT_DROPCOL)
		    {
	...
	else if (opcode != DMU_DESTROY_TABLE)	
	{
	    ...
	    */
		  ...
		  && ! dmt_tbl_entry.tbl_temporary)
		{
		    ...
		    status = qeu_drop_restrict_check(qef_cb, &qeuq_cb,
				&ulm, TRUE, &dep_string, &err);
		    if (status != E_DB_OK || dep_string != NULL)
		    {
			if(status == E_DB_OK)
			{
	   ...
	   */
            if (dmu_ptr->dmu_action == DMU_ALT_COL_RENAME || dmu_ptr->dmu_action == DMU_ALT_TBL_RENAME)
	    {
                i4               error;
	        QEUQ_CB          qeuq_cb;
                DMT_TBL_ENTRY    dmt_tbl_entry;
		...
		if (dmu_ptr->dmu_action == DMU_ALT_COL_RENAME)
                {
                    dmu_attr = (DMF_ATTR_ENTRY **)
                                  dmu_ptr->dmu_attr_array.ptr_address;
	    ...
	    */
	    ...
	    if(opcode == DMU_ALTER_TABLE &&
	    		(dmu_ptr->dmu_action == DMU_ALT_ADDCOL || dmu_ptr->dmu_action == DMU_ALT_ALTERCOL) &&
		...
		if (qeu_cb->qeu_eflag == QEF_EXTERNAL)
		{
			...
			case E_DM007D_BTREE_BAD_KEY_LENGTH:
			    ...
			    */
			    maxkeysize = (i4) errdmu_ptr->dmu_tup_cnt;
		    ...
		    */
		        ...
		        if ( status != E_DB_OK )
			{
		        ...
		        */
			    ...
			    numberOfPersistentIndices, PSQ_INDEX );
		        if ( status != E_DB_OK )
			{
			    err = qef_rcb.error.err_code;
			    goto exit;
	...
	*/
	if (! suppress_rowcount(opcode, dmu_ptr)
	  && (opcode != DMU_MODIFY_TABLE
	      || dmu_ptr->dmu_action == DMU_ACT_STORAGE
	      || dmu_ptr->dmu_action == DMU_ACT_TRUNC) )
	    *row_count = (i4) dmu_ptr->dmu_tup_cnt;
    }
...
QEF_CB  *qef_cb)
{
    ...
    */
    ...
    status = qeu_qrytext_to_tuple(qeu_cb, PSQ_REG_IMPORT, &qef_iiqrydata,
...
**	    Completed init of dmt_shw_cb.dmt_char_array
*/
DB_STATUS
qeu_dqrymod_objects(
QEF_CB		*qef_cb,
...
**	dependentObjectType	currently a PSQ_INDEX, but this routine was
...

/*
** Name: qeu_drop_restrict_check -- Check DROP RESTRICT dependencies
**
** Description:
**
**	This routine is called to verify whether a DROP RESTRICT operation
**	on a table, index, or view is permitted to continue.  The
**	operation of qeu_d_cascade is sufficiently convoluted that it
**	seems easier to do a pre-check, even though it means some
**	additional opening and closing of catalogs.
**
**	Tables and views are restricted by views, DB procedures, synonyms,
**	and some constraints.  (Specifically, referential constraints where
**	the table being dropped is the referenced table.)  The catalog
**	entries are non-obvious (at least to me);  we check:
**	    in iidbdepends, rows with inid == the table to drop,
**		- itype == PSQ_DB_TABLE and dtype == PSQ_VIEW/PSQ_CREDBP
**		- itype and dtype both == PSQ_CONS (ref constraint)
**		- (indexes or modify-check only) inid == the base table,
**		  itype == PSQ_CONS,
**		  deid == the table to drop, dtype == PSQ_INDEX
**	    in iixsynonym, any rows with syntabbase == the table;
**	    in iixpriv, any rows with i_obj_id == the table;
**
**	If the object is an index, we only look at rows for that specific
**	index.  If however it's a base table, we'll consider all rows for
**	that table (i.e. including its indexes);  this will catch views
**	or synonyms created on an index of the table (if any).
**
**	Note that the iixpriv check is to catch cross-owner references.
**	If user U1 owns table T, and user U2 does "create view V
**	as select * from U1.T", the view V dependency is not in iidbdepends!
**	I've no idea if this is a bug or a feature, but for the present
**	purposes it seems best to leave the catalog entries alone and
**	simply deal with them.
**
**	For MODIFY, QEF wants to verify that there's no constraint that
**	depends on the base table or index being modified.  Since the
**	check is basically similar to the drop restrict check, except
**	somewhat shorter, we'll do that code here with a caller flag.
**
**	There are a bunch of things that could be considered as dependent
**	objects that do NOT restrict a drop.  The general rule is that if
**	the dependent object is purely local to the table, it's not
**	considered a restrictive object.  Thus, permits and secondary
**	indexes do not block a DROP RESTRICT.  Perhaps less obviously,
**	neither do check constraints or QUEL integrities.  An object
**	has to have some standing on its own (view, DBP, synonym) or
**	has to belong to some other object (ref constraint) for it
**	to be considered restrictive.
**
** Inputs:
**	qef_cb			Session QEF control block
**	qeuq_cb			QEUQ_CB that contains:
**	    .qeuq_rtbl		DB_TAB_ID pointer for t/v/i to check
**	    .qeuq_status_mask	tbl_status_mask from DMT_SHOW for object
**	ulm			A memory stream to use for checking
**	modify_check		TRUE if only doing special constraint check
**				on indexes for MODIFY, FALSE otherwise
**	dep_string		An output
**	error			An output
**
** Outputs:
**	dep_string		(char **) returned NULL if no error, or
**				set to some sort of moderately descriptive
**				string if error.  (E_DB_OK and non-null
**				dep_string is the usual error situation.)
**	error			E_whatever
**	Returns E_DB_OK or error status
**
**	The clumsiness with the returned string comes about because it
**	turns out to be a lot easier for the caller to construct the
**	proper error message.
**
** Edit History:
**	17-Jun-2012 (kschendel) SIR 126676
**	    Written for DROP RESTRICT (based on old qeu-d-check-conix).
*/

static DB_STATUS
qeu_drop_restrict_check(
    QEF_CB	*qef_cb,
    QEUQ_CB	*qeuq_cb,
    ULM_RCB	*ulm,
    bool	modify_check,
    ...
    i4		*error)
{
    ...
    bool		found;
    bool		isIndex = ((qeuq_cb->qeuq_status_mask & DMT_IDX) != 0);
    QEU_CB		*qeu;
    QEF_DATA		vqef_data;
    DMR_ATTR_ENTRY	vkey_array[3];
    DMR_ATTR_ENTRY	*vkey_ptr_array[3];
    DB_TAB_ID		droptab;
    DB_STATUS		status;
    DB_IIDBDEPENDS	*dtuple;
    DB_IIXPRIV		*xprivtuple;
    DB_IIXSYNONYM	*xsyntuple;
    i4			rsize;
    PSQ_MODE		objtype = PSQ_CONS;
    ...
    *dep_string = NULL;
    ...
    /* Since we're going to need a bunch of stuff, go ahead and allocate
    ** them from the heap.  Zero the whole mess.
    */
    rsize = sizeof(DB_IIDBDEPENDS);
    if (rsize < sizeof(DB_IIXPRIV))
	rsize = sizeof(DB_IIXPRIV);
    if (rsize < sizeof(DB_IIXSYNONYM))
	rsize = sizeof(DB_IIXSYNONYM);
    rsize = DB_ALIGN_MACRO(rsize);
    ulm->ulm_psize = sizeof(QEU_CB) + sizeof(DMT_SHW_CB) + sizeof(DMT_TBL_ENTRY)
		+ sizeof(DMT_CB) + rsize;
	...
	*error = ulm->ulm_error.err_code;
    ...
    memset(ulm->ulm_pptr, 0, ulm->ulm_psize);
    qeu = (QEU_CB *) ulm->ulm_pptr;
    dmt_show = (DMT_SHW_CB *) ((char *)qeu + sizeof(QEU_CB));
    dmt_tbl_entry = (DMT_TBL_ENTRY *) ((char *)dmt_show + sizeof(DMT_SHW_CB));
    dmt_cb = (DMT_CB *) ((char *)dmt_tbl_entry + sizeof(DMT_TBL_ENTRY));
    dtuple = (DB_IIDBDEPENDS *) ((char *)dmt_cb + sizeof(DMT_CB));
    xsyntuple = (DB_IIXSYNONYM *) dtuple;
    xprivtuple = (DB_IIXPRIV *) dtuple;
    ...
    */
    ...
    {
	/* Must be createdb!  or else catalogs are hopeless */
        return(E_DB_OK);
    }
    ...
    **
    ** (kschendel) actually no locks are released;  maybe that happened
    ** at one point in the original qeu_d_conix_check's life, but it
	...
	*error = dmt_cb->error.err_code;
    ...
    droptab = *qeuq_cb->qeuq_rtbl;	/* For convenience */
    qeu->qeu_type = QEUCB_CB;
    qeu->qeu_length = sizeof(QEU_CB);
    qeu->qeu_db_id = qeuq_cb->qeuq_db_id;

    qeu->qeu_lk_mode = DMT_N;
    qeu->qeu_access_mode = DMT_A_READ;

    qeu->qeu_tab_id.db_tab_base = DM_B_DEPENDS_TAB_ID;
    qeu->qeu_tab_id.db_tab_index = DM_I_DEPENDS_TAB_ID;

    status = qeu_open(qef_cb, qeu);   /* open iidbdepends */
    if (status != E_DB_OK)
    {
	*error = qeu->error.err_code;
    ...
    {
	/* For the constraint check for modify, look specifically for a
	** constraint row for the base table, since we don't care about
	** other dependencies.
	*/
	...
	qeu->qeu_getnext = QEU_NOREPO;  /* just get next from here */

	/* Check for constraint/view/dbp.  Only look at the rows for the
	** specific index if doing an index.
	*/
	if (!modify_check &&
	  (!isIndex || dtuple->dbv_indep.db_tab_index == droptab.db_tab_index))
	{
	    if ((dtuple->dbv_itype == PSQ_CONS && dtuple->dbv_dtype == PSQ_CONS)
	      || (dtuple->dbv_itype == PSQ_DB_TABLE
		  && (dtuple->dbv_dtype == PSQ_VIEW || dtuple->dbv_dtype == PSQ_CREDBP)))
	    {
		*dep_string = "view";
		if (dtuple->dbv_dtype == PSQ_CREDBP)
		    *dep_string = "DB procedure";
		else if (dtuple->dbv_dtype == PSQ_CONS)
		    *dep_string = "constraint";
		break;
	    }
	}
	/* For index, check for constraint on the index */
	if ((isIndex || modify_check)
	  && dtuple->dbv_indep.db_tab_index == 0
	  && dtuple->dbv_itype == PSQ_CONS
	  && dtuple->dbv_dtype == PSQ_INDEX
	  && dtuple->dbv_dep.db_tab_base == droptab.db_tab_base
	  && dtuple->dbv_dep.db_tab_index == droptab.db_tab_index)
	{
	    *dep_string = "constraint";
	    break;
	}
	...
	*error = qeu->error.err_code;
    ...
    /* Keep looking if no problem yet.  Next up is iixpriv.
    ** (we're trying to go in the same order as qeu-d-cascade, more or less.)
    */
    if (*dep_string == NULL && !modify_check)
    {
	qeu->qeu_tab_id.db_tab_base = DM_B_XPRIV_TAB_ID;
	qeu->qeu_tab_id.db_tab_index = DM_I_XPRIV_TAB_ID;

	status = qeu_open(qef_cb, qeu);   /* open iixpriv */
	if (status != E_DB_OK)
	{
	    *error = qeu->error.err_code;
	    return(status);
	...
	qeu->qeu_count = 1;
	qeu->qeu_tup_length = sizeof(DB_IIXPRIV);
	qeu->qeu_output = &vqef_data;
	vqef_data.dt_next = 0;
	vqef_data.dt_size = sizeof(DB_IIXPRIV);
	vqef_data.dt_data = (PTR) xprivtuple;

	qeu->qeu_key = vkey_ptr_array;

	/* For iixpriv, only look for rows with the i_obj_id == the object
	** base ID.  As it happens, even if one creates a (cross-user) view
	** on a secondary index, the iipriv entry looks like it's on the
	** base table, so we could explicitly check for zero i_obj_idx.
	** Doing it this way seems more generic though (and should end up
	** doing no more work.)
	*/

	vkey_ptr_array[0] = &vkey_array[0];
	vkey_ptr_array[0]->attr_number = DM_1_XPRIV_KEY;
	vkey_ptr_array[0]->attr_operator = DMR_OP_EQ;
	vkey_ptr_array[0]->attr_value = (char *) &droptab.db_tab_base;
	qeu->qeu_klen = 1;
	qeu->qeu_getnext = QEU_REPO;
	...
	if (status == E_DB_OK)
	{
	    /* Would need to read the iipriv row to find out which,
	    ** can't be bothered.
	    */
	    *dep_string = "view or DB procedure";
	}
    ...
    /* Last thing to check is for a synonym.  Here, unlike iixpriv, we'll
    ** look for the specific base+index if dropping an index.
    */
    if (*dep_string == NULL && !modify_check)
    {
	qeu->qeu_tab_id.db_tab_base = DM_B_XSYN_TAB_ID;
	qeu->qeu_tab_id.db_tab_index = DM_I_XSYN_TAB_ID;
	...
	qeu->qeu_count = 1;
	qeu->qeu_tup_length = sizeof(DB_IIXSYNONYM);
	qeu->qeu_output = &vqef_data;
	vqef_data.dt_next = 0;
	vqef_data.dt_size = sizeof(DB_IIXSYNONYM);
	vqef_data.dt_data = (PTR) xsyntuple;

	qeu->qeu_key = vkey_ptr_array;

	/* DMF key is base ID, or base+index ID if dropping an index. */

	vkey_ptr_array[0] = &vkey_array[0];
	vkey_ptr_array[0]->attr_number = DM_1_XSYN_KEY;
	vkey_ptr_array[0]->attr_operator = DMR_OP_EQ;
	vkey_ptr_array[0]->attr_value = (char *) &droptab.db_tab_base;
	if (!isIndex)
	{
	...
	{
	    vkey_ptr_array[1] = &vkey_array[1];
	    vkey_ptr_array[1]->attr_number = DM_2_XSYN_KEY;
	    vkey_ptr_array[1]->attr_operator = DMR_OP_EQ;
	    vkey_ptr_array[1]->attr_value = (char *) &droptab.db_tab_index;
	    qeu->qeu_klen = 2;
	}
	qeu->qeu_getnext = QEU_REPO;
	status = qeu_get(qef_cb, qeu);
