...
**	29-sep-2010 (dougi)
**	    First implementation of X100 query under QP node (for CTAS).
**      01-apr-2011 (huazh01)
**          turn off QEN8_NODE_CLOSED bit when we open the node. (b125094)
*/
DB_STATUS
qen_qp(
QEN_NODE	*node,
...
i4		function)
{
    ...
    */
	...
	for (;;)
	{
		...
		(act->ahd_atype == QEA_X100_QRY && 
		 --(qen_status->node_u.node_qp.node_rowcount) <= 0 &&
		 qen_status->node_u.node_qp.node_qe0015 == 1))
	    {
		...
		*/
	    ...
	    else if (act->ahd_atype == QEA_X100_QRY)
	    {
		if (qen_status->node_u.node_qp.node_rowcount <= 0)
		{
		    status = qea_x100(act, rcb, function);
		    if (status)
		    {
		    ...
		    else qen_status->node_u.node_qp.node_qe0015 = 0;

		    x100ptr = (QEA_X100 *)dsh->dsh_cbs[act->
                                        qhd_obj.qhd_x100Query.ahd_x100_cb];
		    qen_status->node_u.node_qp.node_rowcount = 
					*(x100ptr->x100_resrow_countp);
		    if (qen_status->node_u.node_qp.node_rowcount <= 0)
		    {
			dsh->dsh_error.err_code = E_QE0015_NO_MORE_ROWS;
			status = E_DB_WARN;
		    ...
		    status = E_DB_OK;			/* reset */
		    dsh->dsh_row[rowno] = qen_status->node_u.node_qp.
				node_rowptr = x100ptr->x100_rowbuf_addr;
		}
		else
		{
		    qen_status->node_u.node_qp.node_rowptr += rowlen;
		    dsh->dsh_row[rowno] = qen_status->node_u.node_qp.node_rowptr;
		}
	    }
	    else
	...
	*/
	...
	switch (act->ahd_atype)
	{
		...
		*/
		...
		{
		    status = E_DB_WARN;
		    goto exit;			/* really is "no more rows" */
		}
		else
		{
		    status = E_DB_OK;
		...
		dsh->dsh_row[rowno] = qen_status->node_u.node_qp.node_rowptr =
					x100ptr->x100_rowbuf_addr;
		goto exit;			/* X100 returns a bunch */
	    case QEA_GET:
	    case QEA_HAGGF:
	    case QEA_RAGGF:
		for (;;)
