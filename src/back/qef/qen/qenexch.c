...
#define	MAX_CUT_BUFFER	(SIZE_TYPE)(1024*512)
	...
	*/
	...
	if ( --exch_cb->cchild_running == 0 )
	{
	    ...
	    if (exch_cb->trace)
		TRdisplay("%@ %p %2d %s %s LAST child: status = %d err_code = %x rows %ld\n",
		    exch_cb->parent_sid, NodeNum, OutNode, DbgParent,
		...
		for ( i = 0; i < qp->qp_stat_cnt; i++ )
		{
			...
			if (QenStatus != ParentStatus)
			{
			    ...
			    CSadjust_counter(&ParentStatus->node_dio,
					QenStatus->node_dio);
			    CSadjust_i8counter(&ParentStatus->node_rcount,
...
**      13-Apr-2011 (smeke01) b125272
**          Add fixup for inner tuple materialisation in sejoins.
*/
static
void
qen_dsh_fixup(QEE_DSH *dsh, QEN_NODE *node, QEN_EXCH *exch)
{
	...
	/*
	** The address for inner tuple materialisation at this
	** point is still the address held on the parent dsh.
	** So fix up to the address held in the child dsh. Do the
	** attribute length here too, though it should be the same
	** as in the parent dsh anyway.
	*/
