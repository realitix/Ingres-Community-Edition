...
**	27-aug-2008 (dougi)
**	    Allocate/format QEA_X100 for X100 support.
**	10-Mar-2010 (kiria01)
...
**	27-sep-2010 (dougi)
**	    Get row descriptor from X100 action instead of qp_sqlda.
**	5-Jan-2011 (kschendel)
**	    Fill in DMT_CB table name, x100 queries can use it.
**	16-Mar-2011 (kschendel) m1541
**	    Defer copying x100 text until action executes, actual-parameters
**	    might need to be inserted.
**	28-Apr-2011 (kschendel) m1920
**	    Tweak X100 output buffer allocation for VW->Ing CTAS etc, to keep
**	    size under control for large rows.  Ideally this should be
**	    further tweaked based on the operation (e.g. larger for ctas,
**	    insert;  smaller for scrollable cursor;  ?? for mixed query.)
**	24-May-2011 (kschendel) m1260
**	    X100 core now a substructure, fix here.
*/
static DB_STATUS
qee_cract(
	QEF_RCB            *qef_rcb,
	...
	ULM_RCB		   *temp_ulm)
{
    ...
    ** system catalog as well as temporary table */
	...
	for (vl = act->ahd_valid; vl; vl = vl->vl_next)
	{
		...
		** table) partition ID. */
		...
		dmt_cb->dmt_table = vl->vl_tab_name;
    ...
    */
	...
	switch (act->ahd_atype)
	{
	    ...
	    case QEA_X100_QRY:
		/* X100 query - allocate QEA_X100 structure to house
		** X100 parameter list. */
		ulm->ulm_psize = sizeof(QEA_X100);
		status = qec_malloc(ulm);
		if (status != E_DB_OK)
		{
		    qef_rcb->error.err_code = ulm->ulm_error.err_code;
		...
		dsh->dsh_cbs[act->qhd_obj.qhd_x100Query.ahd_x100_cb] = 
			(PTR) ulm->ulm_pptr;
		x100_ptr = (QEA_X100 *) ulm->ulm_pptr;
		x100_ptr->core.adfcb = dsh->dsh_adf_cb;
		x100_ptr->core.dbname = NULL;
		x100_ptr->core.session = NULL;
		x100_ptr->x100_errorp = &dsh->dsh_error.err_code;
		/* Text is fetched when the action is executed */
		x100_ptr->x100_resrow_format = (GCA_TUP_DESCR *) act->
		    	qhd_obj.qhd_x100Query.ahd_resrow_format;
		x100_ptr->x100_resrow_size = act->qhd_obj.qhd_x100Query.
			ahd_resrow_size;
		x100_ptr->x100_resrow_countp = &dsh->dsh_qef_count;
		x100_ptr->x100_query_handle = NULL;
		x100_ptr->x100_flags = 0;
		if (act->qhd_obj.qhd_x100Query.ahd_x100_flag & AHD_X100_DMLOPS)
		{
		    x100_ptr->x100_rowbuf_count = 1;	/* single result row */
		    x100_ptr->x100_rowbuf_addr = (PTR)&x100_ptr->
				x100_resrow_count;	/* i8 result count */
		}
		else if (act->qhd_obj.qhd_x100Query.ahd_x100_flag &
							AHD_X100_SCFBUF)
		{
		    if (qef_cb->qef_operation != QEQ_OPEN)
		    {
			x100_ptr->x100_rowbuf_count = qef_rcb->qef_rowcount;
			x100_ptr->x100_rowbuf_addr = qef_rcb->qef_output->
				dt_data;
		    }
		}
		else
		{
		    /* Locally allocated result buffers, e.g. for CTAS
		    ** from IVW to native Ingres.
		    ** FIXME we need a better way to decide on allocation.
		    ** CTAS / insert-select want large values, scroll
		    ** cursor probably wants smaller.
		    ** For now take min(500K,100rows)  where
		    ** these numbers are pretty much pulled out of the air.
		    ** (500K is enough for the largest possible row.)
		    */
		    ulm->ulm_psize = 100 * x100_ptr->x100_resrow_size;
		    if (ulm->ulm_psize > 512*1024)
			ulm->ulm_psize = 512 * 1024;
		    x100_ptr->x100_rowbuf_count = ulm->ulm_psize / x100_ptr->x100_resrow_size;
		    status = qec_malloc(ulm);
		    if (status != E_DB_OK)
		    {
			qef_rcb->error.err_code = ulm->ulm_error.err_code;
		    ...
		    x100_ptr->x100_rowbuf_addr = ulm->ulm_pptr;
		}
		break;
		...
		*/
		    ...
		    qep_node; qep_node = qep_node->qen_postfix)
		{
			...
			case QE_FSMJOIN:
			    ...
			    */
			    if (qp->qp_hash_cnt == 1 && qp->qp_hashagg_cnt == 0)
			    {
		...
		*/
		     ...
		     dsh->dsh_cbs[i] == 0)
		{
		    ...
		    dmr_cb->dmr_data.data_in_size = qp->qp_row_len[i];
		}
		...
		** control block. */
		if (act->ahd_atype == QEA_HAGGF)
		{
		    ...
		    ** poop for that simple and common case.
		    */
		    if (qp->qp_hashagg_cnt == 1 && qp->qp_hash_cnt == 0)
		    {
		...
		** QEE_RSET_SPOOL structure. */
			...
			(act->qhd_obj.qhd_qep.ahd_qepflag & AHD_SCROLL))
		{
		    ...
		    Mem = (PTR) sptr;
		    dsh->dsh_cbs[qp->qp_rssplix] = (PTR)sptr;
...
**	24-May-2011 (smeke01) b125384
**	    Handle the case where the procedure has no parameters defined.
*/
DB_STATUS
qee_dbparam(
	QEF_RCB		*qef_rcb,
...
#		endif /* BYTE_ALIGN */
	...
	** in the QP. */
    ...
    if (pcount == 0)
    {
	/* It's an error if a procedure with no parameters is called with a parameter */
	if (ucount)
	{
	    /* Just get the first parameter to use in the error message */
	    if (nested)
	    {
		STRUCT_ASSIGN_MACRO(cp_params[0].ahd_parm,
				    uparm_val);
		uparm_val.parm_dbv.db_data = (PTR)
		    ((char *) callers_dsh->
		     dsh_row[cp_params[0].ahd_parm_row] +
		     cp_params[0].ahd_parm_offset);
		uparm = &uparm_val;
	    }
	    else
	    {
		uparm = *(qef_rcb->qef_usr_param);
	    }

	    (VOID) qef_error(E_QE0123_PARAM_INVALID, 0L, E_DB_ERROR,
		    &err, &dsh->dsh_error, 1,
		    qec_trimwhite(uparm->parm_nlen, uparm->parm_name),
		    uparm->parm_name);
	    return (E_DB_ERROR);
	}
	else
