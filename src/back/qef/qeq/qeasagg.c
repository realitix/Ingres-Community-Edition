...
**	    Reposition NOROWS check;  opf may not bother doing an
**	    ahd-constant any more if norows is set.
*/
...
i4		    function )
{
    ...
    if (action->qhd_obj.qhd_qep.ahd_qepflag & AHD_NOROWS
      || qen_adfconst != NULL)
    {
	if ((action->qhd_obj.qhd_qep.ahd_qepflag & AHD_NOROWS) == 0)
	{
	...
	if (action->qhd_obj.qhd_qep.ahd_qepflag & AHD_NOROWS
	  || ade_excbconst->excb_value != ADE_TRUE)
	    goto finitit;
    }
