...
**      12-Sep-2011 (hanal04) Bug 125744
**          node_rcount is now an i8. Update format specifiers accordingly.
*/

...
QEF_RCB		*qef_rcb )
{
	...
	*/
	...
	dsh->dsh_act_ptr->ahd_atype != QEA_X100_QRY)
    {
	/* no legal row here */
    ...
    ** SCF-QEF and application programs to be in sync. */
    ...
    {
	QEA_X100	*x100ptr = (QEA_X100 *)dsh->dsh_cbs[dsh->dsh_act_ptr->
				qhd_obj.qhd_x100Query.ahd_x100_cb];

	/* Setup fetch buffers & call X100. */
	x100ptr->x100_rowbuf_count = dsh->dsh_qef_rowcount;
	x100ptr->x100_rowbuf_addr = dsh->dsh_qef_output->dt_data;
	status = qea_x100(dsh->dsh_act_ptr, qef_rcb, (i4) NO_FUNC);
    }
    ...
    */
	...
	switch (act->ahd_atype)
	{
	    ...
	    case QEA_X100_QRY:
		status = qea_x100(act, qef_rcb, (i4) NO_FUNC);
		break;
...
**      01-apr-2011 (huazh01)
**          when handling a QEA_GET inside a db procedure, we need to close
**          all dsh nodes after qea_fetch() returns successfully. Otherwise, 
**          node with OPEN status will not be abled to launch parallel 
**          threads, causing session hangs and server uses 100% CPU.
**          (125094)
**	26-May-2011 (smeke01) b125384
**	    Remove the if-condition around the call to qee_dbparam.  We now
**	    call qee_dbparam even if there are no parameters defined. This
**	    enables qee_dbparam to return an error if appropriate.
**	03-Jun-2011 (smeke01) b125384
**	    STAR can't check the parameters because it doesn't have the
**	    stored query plan for the procedure. The checking is instead done
**	    by the DBMS server since this is where the query plan is stored.
*/

DB_STATUS
qeq_query(
...
i4		mode )
{
	...
	*/
	...
	/* Don't do this if we are STAR - the dbms server will do this for us */
	if (qp->qp_qmode == QEQP_EXEDBP && !ddb_b)
	{
	...
	*/
	...
	/* Likewise, if it is the continuation of an X100 query, do 
	** something similar. */
	else if (act && act->ahd_atype == QEA_X100_QRY)
	{ 
	    /* LDB processing */
	    qeq_rcbtodsh(qef_rcb, dsh);
	    status = qea_x100(act, qef_rcb, (i4) NO_FUNC);
	...
	*/
	...
	if (status == E_DB_OK)
	{
		    ...
		    */
		    ...
		    return (status);
                ...
                if (qp->qp_qmode == QEQP_EXEDBP && 
                   (status == E_DB_OK || status == E_DB_WARN) && 
                   dsh)
                {
		...
		*/
		...
		status = qea_x100_crtable(act, qef_rcb);
		dshcopy = FALSE;
                qef_rcb->qef_rowcount = -1; /* this eliminates the spurious
                                            ** "(0 rows)" diagnostic. */
		if (qef_rcb->error.err_code == E_QE0025_USER_ERROR)
		...
		*/
		...
		status = qea_x100_crconstraint(act, qef_rcb, dsh);
		qeq_dshtorcb(qef_rcb, dsh);
		dshcopy = FALSE;
                qef_rcb->qef_rowcount = -1; /* this eliminates the spurious
                                            ** "(0 rows)" diagnostic. */
		...
		status = qea_x100_drconstraint(act, qef_rcb);
...
qeq_close_dsh_nodes(QEE_DSH *dsh)
{
	...
	if (action->ahd_atype == QEA_X100_QRY)
	{
	    QEA_X100	*x100cb = (QEA_X100 *)dsh->dsh_cbs[action->
					qhd_obj.qhd_x100Query.ahd_x100_cb];

	    if (!(x100cb->x100_flags & QEA_X100_QUERYDONE))
		x100_query_stop(x100cb);
	}
    }
} /* qeq_close_dsh_nodes */

...
**      13-Sep-2011 (maspa05) b125735
**          Check whether we have a handle before doing the extra unlock
**          as it's only when we don't have a handle that the extra lock is 
**          taken
*/
DB_STATUS
qeq_endret(
QEF_RCB		*qef_rcb )
{
    ...
    /* if we just retrieved the same dsh that we already have and
     * we did so without a hanlde then unlock it 
     * (we'll have locked it twice) */

    if (!qef_rcb->qef_qso_handle &&
	dsh == (QEE_DSH *)qef_cb->qef_dsh)
    {
        QSF_RCB	qsf_rcb;
