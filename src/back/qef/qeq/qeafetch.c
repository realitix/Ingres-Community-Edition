...
**	11-Jan-2011 (smeke01) b124834
**	    'offset n' handling with blobs. If we have blob data, make sure
**	    that we read to the end of it. Also restore the output->dt_size
**	    value before each call to qea_fetch_readqep() to ensure we have
**	    the maximum size buffer available for reading in the data.
**	19-Jul-2011 (kiria01) b125564
**	    Ensure bools initialised for all paths.
*/
DB_STATUS
qea_fetch(
QEF_AHD		*action,
...
i4		function )
{
    ...
    bool	rowinspool = FALSE;
    bool	fetchlast = FALSE;
    bool	pre_read = FALSE;
    bool	skipread = FALSE;
    bool	gotarow;
    bool	first = FALSE;
    ...
    ** If there's an output buffer, we must assume there's at least 1. */
	...
	if (rowoffset)
	{
	    /* Save dt_size so we can restore it before each call to qea_fetch_readqep */
	    i4 dt_size = output->dt_size;
		...
		if (status != E_DB_OK)
		{
		    ...
		    {
			/*
			** This row contains a blob, and we haven't finished
			** reading to the end of it.  Set things to go round
			** the loop again and read the next chunk of the blob.
			*/
			status = E_DB_OK;
			rowoffset++;
		    }
		...
		/*
		** qea_fetch_readqep() reduces output->dt_size to the size of
		** the data just read. For blobs this could cause dt_size to be
		** reduced to less than the size required for a subsequent
		** chunk of the blob, or the next row. So we make sure we have
		** the maximum space available for the next chunk of a blob, or
		** the next row, by restoring the size.
		*/
		output->dt_size = dt_size;
	    }
	}
    }
