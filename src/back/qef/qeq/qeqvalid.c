...
**	28-aug-2008 (dougi)
**	    Add preiminary X100 support - just return from 
**	    QEA_X100_QUERY action.
...
**	    Validate X100 actions just like anything else.  We need the
**	    DMT opens for temp tables, and the timestamp checks for invalid
**	    QP's.
**	4-Mar-2011 (kschendel) b125146
**	    Obey NOROWS flag before running ahd-constant.  OPB now knows
**	    more about constant true/false clauses, and may not bother
**	    generating the ahd-constant if the where is FALSE.
**      11-Oct-2011 (horda03) b125755
**          Don't wait for lock conversion from N to required level, 
**          session has been waiting for locks long enough.
*/
...
bool		size_check)
{
	...
	*/
	...
	/* We don't need anything else for X100 actions */
	...
	*/
	if (dmt_cb->dmt_lock_mode == DMT_N)
	{
	    ...
	    status = dmf_call(DMT_OPEN, dmt_cb);
            ...
            /* Unset DMT_NO_LOCK_WAIT for future calls. */
            dmt_cb->dmt_flags_mask &= ~DMT_NO_LOCK_WAIT;
...
#define QEQ_OVERHEAD_NEEDED 200
	...
	*/
	...
	if (act->ahd_atype != QEA_AGGF && act->ahd_atype != QEA_SAGG)
	{
	    ...
	    if (act->qhd_obj.qhd_qep.ahd_qepflag & AHD_NOROWS
	      || qen_adf != NULL)
	    {
		if ((act->qhd_obj.qhd_qep.ahd_qepflag & AHD_NOROWS) == 0)
		{
		    ade_excb = (ADE_EXCB*) cbs[qen_adf->qen_pos];
		    ade_excb->excb_seg = ADE_SMAIN;
...
**	6-Jan-2011 (kschendel)
**	    X100 valid entries might not ask for a DMR_CB, so take that
**	    into account.  (qee doesn't allocate one anyway.)
**      11-Oct-2011 (horda03) B125755
**          Try to take lock at desired lock mode, only if that times out
**          do a quick DMT_N lock request.
**	    
*/
static DB_STATUS
qeq_vopen(
	...
	QEF_VALID   *vl)
{
	...
	** Note that (at present) a partitioned table can't be an IVW
	** table, and thus there will be a valid vl_dmr_cb.
	*/
	...
	*/
	...
	if (status && dmt_cb->error.err_code == E_DM004D_LOCK_TIMER_EXPIRED)
	{
	    ...
	    ** Don't wait if the lock cannot be granted.
	    */
	    dmt_cb->dmt_flags_mask |= (DMT_FORCE_NOLOCK|DMT_NO_LOCK_WAIT);
	    dmt_cb->dmt_lock_mode = DMT_N;
	    status = qen_openAndLink(dmt_cb, dsh);
	    /* Reset DMT_FORCE_NOLOCK & DMT_NO_LOCK_WAIT for future calls. */
	    dmt_cb->dmt_flags_mask &= ~(DMT_FORCE_NOLOCK|DMT_NO_LOCK_WAIT);
	}
	...
	/* Verify that qee allocated the DMR_CB for this entry.
	** (IVW tables will have a -1 index, no DMR_CB.)
	*/
	if (vl->vl_dmr_cb >= 0)
	{
	    ...
	    ** partitioned master open, helps qeq-part-open find the associated
	    ** valid list entry.
	    */
