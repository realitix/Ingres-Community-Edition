...
**          qea_67_check_patch() check for X100 tables
...
**	16-oct-2009 (dougi)
**	    Added code to call X100 when db's are created.
...
*/
	...
	/* And finally, call X100, too. For createdb they don't do anything
	** yet, so this is a stub call. However, if they do eventually
	** require this call, it should presumably only be done when a
	** corresponding X100 database is explicitly requested on the
	** createdb. */
	x100cb.core.adfcb = qef_rcb->qef_cb->qef_adf_cb;
	x100cb.core.dbname = (char *)&dmm.dmm_db_name;
	x100cb.core.session = NULL;
	/* status = x100_create_database(&x100cb);
	** if (status)
	**	status = vw_error(&x100cb, status, 0);
	*/
    }
...
**	    otherwise succeeds.
**	16-oct-2009(dougi)
**	    Added code to call X100 when db's are destroyed.
*/
static DB_STATUS
qea_3destroy_db(
QEF_RCB            	*qef_rcb,
...
/* Bug #67531: Put info in correct array element */
	...
	/* And finally, call X100, too, in case this was a X100-connected db. */
	x100cb.core.adfcb = qef_rcb->qef_cb->qef_adf_cb;
	x100cb.core.dbname = (char *)&dmm_cb.dmm_db_name;
	x100cb.core.session = NULL;
	status = x100_destroy_database(&x100cb);
    }
...
i4			patch_flag )
{
    ...
    */
    ...
    dmt_show.dmt_tab_id = dmu_cb.dmu_tbl_id;
    ...
    /*
    ** X100 tables verifydb check/patch table currently unsupported
    ** This is where we would call a vw entry point to check/patch vw table
    */
    if (tbl_info.tbl_storage_type > DB_STDING_STORE_MAX)
	return (E_DB_OK);
