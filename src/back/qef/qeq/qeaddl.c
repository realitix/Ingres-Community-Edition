...
populateIIREFREL(
    QEE_DSH		*dsh,
    QEF_CREATE_INTEGRITY_STATEMENT *details,
    DB_TAB_ID		*refedtabid,
    DB_TAB_ID		*refingtabid,
    DB_CONSTRAINT_ID	*consid,
    DB_SCHEMA_ID	*schmid,
    DB_CONSTRAINT_NAME	*consname);
...
**	16-dec-2008 (dougi)
**	    Add support for X100 and for IIREFREL catalog.
...
( DMT_ATT_ENTRY ***) &(dsh->dsh_row[ details->qci_referredTableAttributeArray]);
    ...
    {
	case CONS_CHECK:
	    if (details->qci_flags & QCI_X100_CONS)
	    {
		/* Check constraints not supported for X100. */
	    }
    ...
    for ( ; ; )		/* code block to break out of */
    {
	    ...
	    */
		...
		/* While we're here (processing a referential constraint),
		** prepare and write the new IIREFREL tuple. */
		status = populateIIREFREL(dsh, details, 
			referredTableID, constrainedTableID,
			constraintID,
			&integrityTuple.dbi_consschema,
			&constraintName);
		if ( DB_FAILURE_MACRO( status ) )	break;
	    ...
	    if (details->qci_flags & QCI_X100_CONS)
		*state = CLEANUP;
            else switch ( details->qci_integrityTuple->dbi_consflags &
	             ( CONS_CHECK | CONS_UNIQUE | CONS_REF ) )
            {
	        case CONS_CHECK:
	...
	/* Re-positioned "populateIIKEY" calls. */
	if (*state == CLEANUP && (details->qci_flags & QCI_X100_CONS) &&
		(details->qci_flags & (CONS_UNIQUE | CONS_REF)))
	{
	...
	/* Call constraint funcs if not X100. */
	if (!(details->qci_flags & QCI_X100_CONS))
            switch ( details->qci_integrityTuple->dbi_consflags &
	         ( CONS_CHECK | CONS_UNIQUE | CONS_REF ) )
        {
	...
	qef_rcb->qef_intstate &= ~QEF_EXEIMM_PROC;
        ...
        /* tear down the memory stream unless X100 */
	if (~details->qci_flags & QCI_X100_CONS)
	    ulm_closestream( ulm );
	...
	QEF_RCB             *qef_rcb)
{
	...
	*/
	...
	status = qeu_d_cascade(qef_cb, qeuq_cb, PSQ_CONS, 
	    (DMT_TBL_ENTRY *) NULL);
	if (DB_FAILURE_MACRO(status))
	{
	...
	case LINK_DEPENDENT_OBJECTS:
	    ...
	    ** rule */
	    ...
	    status = makeIIDBDEPENDStuple( dsh, ulm, constrainedTableID,
			( i4 ) *integrityNumber, PSQ_CONS, ruleIDarray,
			( i4 ) 0, PSQ_RULE, &tuplePTR );
	    if ( status != E_DB_OK )	return( status );
...
unique_noindex_splice:	/* for now, this is how index creation is bypassed */
	    ...
	    status = makeIIDBDEPENDStuple( dsh, ulm, constrainedTableID,
		( i4 ) *integrityNumber, PSQ_CONS, &indexID,
		( i4 ) 0, PSQ_INDEX, &tuples[ 0 ] );
	    if ( status != E_DB_OK )	return( status );
    ...
    QEE_DSH		*dsh )
{
    ...
    */
	...
	status = makeIIDBDEPENDStuple( dsh, ulm, constrainedTableID,
		( i4 ) *integrityNumber, PSQ_CONS, &indexID,
		( i4 ) 0, PSQ_INDEX, &tuples[ i++ ] );
    if ( status != E_DB_OK )	return( status );

    for ( j = 0; j < NUMBER_OF_PROCEDURES; j++ )
    {
	status = makeIIDBDEPENDStuple( dsh, ulm, constrainedTableID,
			( i4 ) *integrityNumber, PSQ_CONS,
			&procedureIDarray[ j ], ( i4 ) 0, PSQ_CREDBP,
			&tuples[ i++ ] );
	if ( status != E_DB_OK )	return( status );
    }
    ...
    QEE_DSH		*dsh)
{
    ...
    */
    ...
    status = makeIIDBDEPENDStuple(dsh, ulm, view_id, (i4) 0, PSQ_DB_TABLE,
	&dbp_id, ( i4 ) 0, PSQ_CREDBP, &dep_tuple);
    if (status != E_DB_OK)
    	return(status);
...
**	    Replace ADI_OP_NAME with string.
*/
    ...
    ADI_FI_DESC		**fi2ptr )
{
    ...
    if ( fi2 == ( ADI_FI_DESC * ) NULL )
    {
    	...
    	status = adi_opid( adf_cb, "ii_dv_desc", &op_dv_desc );
    ...
    i4			viewStatus )
{
    ...
    pack_rcb.rdf_qt_mode	    = PSQ_INTEG;
...

/*{
** Name: populateIIREFREL - prepare and write row to IIREFREL
**
** Description:
**	This routine prepares a row describing a referential relationship
**	and writes it to the IIREFREL catalog.
**
** Inputs:
**
** Outputs:
    ...
    DB_CONSTRAINT_NAME	*consname)
{
    ...
    /* Fill up the tuple. */
    STRUCT_ASSIGN_MACRO(*refedtabid, refreltup.dbrr_refedtabid);
    STRUCT_ASSIGN_MACRO(*refingtabid, refreltup.dbrr_refingtabid);
    STRUCT_ASSIGN_MACRO(*consid, refreltup.dbrr_consid);
    STRUCT_ASSIGN_MACRO(*schmid, refreltup.dbrr_schemaid);
    MEcopy((char *)consname, sizeof(DB_CONSTRAINT_NAME), 
				(char *)&refreltup.dbrr_relname);
    refreltup.dbrr_relflags = 0;

    /* Load colno's of refed, refing columns. */
    for (i = 0, refedcol = (PST_COL_ID *)details->qci_ref_collist, 
	refingcol = (PST_COL_ID *)details->qci_cons_collist;
	refedcol && refingcol; 
	refedcol = refedcol->next, refingcol = refingcol->next, i++)
    {
	refreltup.dbrr_refedcols[i] = refedcol->col_id.db_att_id;
	refreltup.dbrr_refingcols[i] = refingcol->col_id.db_att_id;
    }

    refreltup.dbrr_colcount = i;
    refreltup.dbrr_refingixcol = -1;		/* until we create join IX
						** column here */

    /* Fill rest of arrays. */
    for (; i < 16; i++)
	refreltup.dbrr_refedcols[i] = refreltup.dbrr_refingcols[i] = 0;

    /* Write the tuple to IIREFREL. */
    ...
    qeu->qeu_tab_id.db_tab_base = DM_B_REFREL_TAB_ID;
    qeu->qeu_tab_id.db_tab_index = DM_I_REFREL_TAB_ID;	 
    status = qeu_open(qef_cb, qeu);
    if (status != E_DB_OK)
    {
	error = qeu->error.err_code;
    ...
    qef_data.dt_data = (PTR)&refreltup;
    qef_data.dt_next = (QEF_DATA *) NULL;

    qeu->qeu_count = 1;
    qeu->qeu_tup_length = sizeof( DB_REFREL );
	...
	/* don't return. we have to close IIREFREL first */
    }
	...
	QEU_QUAL_PARAMS	*qparams)
{
    ...
    if (dep_tuple->dbv_dtype == PSQ_CONS)
