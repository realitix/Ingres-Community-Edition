...
**
**	The IVW side of things has to be aborted too.  If a savepoint
**	has been issued to IVW, it will not abort anything without
**	direction from the Ingres side.  If no savepoint has been issued,
**	IVW may have already aborted the entire transaction.  That is
**	OK, since we're going to tell it to do just that anyway.
**
**	The reason for the different handling on the IVW side is to cater
**	to non-Ingres clients, such as x100client, which obviously can't
**	depend on Ingres to control transaction handling.  Those non-
**	Ingres clients aren't going to be issuing savepoints, so full
**	transaction rollback is the right thing to do for them.
**
** Inputs:
**      qef_rcb
...
**	    Tell X100 side to abort too, when doing full txn abort.
**	4-Nov-2010 (kschendel)
**	    IVW side knows about savepoints now.
**      08-feb-2011 (stial01)
**          qet_commit(), qet_abort(): even though VW errors may have already 
**          been reported, call qef_error so that qef code gets set to
**          E_QE0122_ALREADY_REPORTED and then E_QE0025_ERROR.
**	19-May-2011 (kschendel) m1428
**	    Ignore errors from X100 rollback.  The most likely cause is
**	    a dead or broken X100 server and in that case we really want to
**	    close the DMF transaction as well anyway.
**	    Note that we now never use the return status from x100_rollback,
**	    I'm leaving it returning DB_STATUS for now (rather than changing
**	    it to a void routine) in case a use for the return status arises
**	    later.  If no such event happens, x100-rollback should be changed
**	    to a void function.
**	12-Aug-2011 (kschendel) b125656
**	    Abort to savepoint in a Vectorwise transaction should error out
**	    the statement, not abort the transaction (which is what it had
**	    been doing).  Move the check to the start so that we don't
**	    clear uncommitted QSF objects before checking.
**	4-Oct-2011 (kschendel) b125811
**	    DMF can return E_DB_WARN with no error code if we're force
**	    aborting.  Deal with that properly in the x100 code (i.e. don't
**	    assume it's an x100 error).
*/
DB_STATUS
qet_abort(
QEF_CB         *qef_cb)
{
    ...
    */
	...
	if (save_point)
	{
		...
		if (qef_cb->qef_stm_error == QEF_XACT_ROLLBACK)
		{
		    save_point = FALSE;
		    sp_name = NULL;
		}
	    ...
	    if (save_point
	      ...
	      && (!internal_savepoint || qef_cb->qef_stm_error == QEF_SVPT_ROLLBACK) )
	    {
		/* User rollback to savepoint not allowed if IVW query
		** has happened.
		*/
		qef_error(E_QE0313_IVW_NO_SP_ABORT, 0L, status, &err,
			&qef_cb->qef_rcb->error, 0);
		return (E_DB_ERROR);
	    }
	}
	...
	*/
	...
	/* Abort the IVW side first.
	** Note that if it's abort to savepoint, and IVW doesn't know
	** about the savepoint, it just returns without error.
	** That's actually a good thing, since the DMF call will
	** detect the problem and return DM0102 which is handled
	** appropriately.
	** Ignore errors because there's nothing constructive to be
	** done about them, and leaving the Ingres side transaction
	** open is a bad idea.  (The most plausible x100 error is a
	** dead or broken x100 session.)
	*/
	(void) x100_rollback(qef_cb->qef_ivwsesscb, sp_name);
	...
	if ( status == E_DB_WARN )
	{
	    ...
	    ** Issue X100 side full rollback as well.
	    */
	    save_point = FALSE;
	    (void) x100_rollback(qef_cb->qef_ivwsesscb, NULL);
	}
	if (status != E_DB_OK)
	{
	    /* If no DMF error indication, it's probably an x100 error,
	    ** UNLESS it's a warning.  DMF sends back warning with no
	    ** error code for an attempted stmt abort during force-abort.
	    */
	    if (dmx_cb.error.err_code != 0)
	    {
		qef_error(dmx_cb.error.err_code, 0L, status, &err,
			&qef_cb->qef_rcb->error, 0);
	    }
	    else if (status != E_DB_WARN && qef_cb->qef_ivwsesscb != NULL)
	    {
		/* x100 interface routine issued error, but put error info
		** in the QEF RCB as well so callers aren't confused.
		*/
		qef_error( (x100_session_get_adfcb(qef_cb->qef_ivwsesscb))->adf_errcb.ad_dberror.err_code,
				0L, status, &err, &qef_cb->qef_rcb->error, 0);
	    }
...
**	4-Nov-2010 (kschendel)
**	    Begin IVW side transaction too.
*/
DB_STATUS
qet_begin(
QEF_CB     *qef_cb)
{
    ...
    */
	...
	{
	    /* Start IVW transaction first */
	    status = x100_start_txn(qef_cb->qef_ivwsesscb);
	...
	if (status != E_DB_OK)
	{
		...
		*/
		qef_cb->qef_rcb->error =
			(x100_session_get_adfcb(qef_cb->qef_ivwsesscb))->adf_errcb.ad_dberror;
	    }
	    return (status);
	}
    ...
    */
	...
	status = x100_savepoint(qef_cb->qef_ivwsesscb, &dmx_cb.dmx_savepoint_name);
	if (status == E_DB_OK)
    ...
    qef_cb->qef_sess_flags &= ~(QEF_SP_IN_TXN | QEF_IVW_SINCE_SP);
    qef_cb->qef_rcb->error.err_code = E_QE0000_OK;
    return (E_DB_OK);
}

...
**	22-Sep-2010 (kschendel)
**	    Issue commit to X100 side as well.
**	4-Nov-2010 (kschendel)
**	    IVW side knows about savepoints now.
**	    Commit IVW first, as it's the one more likely to fail thanks
**	    to deferred constraint checking.  The Ingres-side commit is
**	    just a lock/log operation and is unlikely to fail unless something
**	    disastrous happens.
*/
DB_STATUS
qet_commit(
QEF_CB     *qef_cb)
{
    ...
    ** Set state back to quiescent state */
	...
	else
	{
	    ...
	    status = x100_commit(qef_cb->qef_ivwsesscb);
	    if (status == E_DB_OK)
		...
		/* Rollback both sides even if IVW abort fails */
		(void) x100_rollback(qef_cb->qef_ivwsesscb, NULL);
		dmx_cb.dmx_option = 0;
		local_status = dmf_call(DMX_ABORT, &dmx_cb);
		if (local_status != E_DB_OK)
...
QEF_CB         *qef_cb)
{
    ...
    */
    ...
    /* Code used to look at qef_operation, but sequencer can issue statement
    ** savepoints.  Look at the name instead.
    */
    user_sp = (MEcmp(QEF_SP_SAVEPOINT, qef_cb->qef_rcb->qef_spoint->db_sp_name,
		QEF_PS_SZ) != 0);

    /* Must be in a transaction */

    if (qef_cb->qef_stat == QEF_NOTRAN ||
	(user_sp && qef_cb->qef_stat != QEF_MSTRAN))
    {
    ...
    ** If it's a user savepoint, remember, since IVW can't do these yet.
    */
    if (user_sp)
    {
	qef_cb->qef_sess_flags |= QEF_SP_IN_TXN;
    }
    else if (qef_cb->qef_stm_error != QEF_STMT_ROLLBACK)
    {
	qef_cb->qef_rcb->error.err_code = E_QE0000_OK;
	return (E_DB_OK);
    }
    ...
    */
    ...
    if (user_sp && (qef_cb->qef_stm_error == QEF_STMT_ROLLBACK))
    {
	dmx_cb.dmx_savepoint_name = Qef_s_cb->qef_sp_savepoint;
	status = x100_savepoint(qef_cb->qef_ivwsesscb, &dmx_cb.dmx_savepoint_name);
	if (status == E_DB_OK)
	    status = dmf_call(DMX_SAVEPOINT, &dmx_cb);
	if (status != E_DB_OK)
