...
QEF_SRCB       *qef_srcb )
{
    ...
    Qef_s_cb->qef_s_reserved = 0;
    Qef_s_cb->qef_l_reserved = NULL;
    Qef_s_cb->qef_owner = (PTR)DB_QEF_ID;
    Qef_s_cb->qef_ascii_id = QEFSCB_ASCII_ID;
    Qef_s_cb->qef_qefcb = NULL;
    ...
    */
    ...
    Qef_s_cb->qef_drop_procedure_value_lock = 0;
    Qef_s_cb->qef_procedure_dropped = 0;
...
**	25-aug-2010 (dougi)
**	    Add support for IVW session management.
**	27-aug-2010 (dougi)
**	    Copy qef_dba from RCB to CB.
**	14-Dec-2010 (kschendel)
**	    Change rollback default to transaction.  (temporary?)
**	10-Feb-2011 (kschendel)
**	    Restore statement rollback default.
*/
DB_STATUS
qec_begin_session(
QEF_CB         *qef_cb )
{
	...
	*/
	...
	qef_cb->qef_dba = qef_rcb->qef_dba;
	...
	** variable, we must lock the semaphore */
    ...
    if (status == E_DB_OK)
	status = qea_x100_begin_session(qef_cb);
...
QEF_CB       *qef_cb )
{
    ...
    /* Terminate IVW session (if there). */
    status = qea_x100_end_session(qef_cb);
...

/*{
** Name: qec_register_interrupt_handler	- initialize the attn handler
**
** Description:
**      This routine will inform SCF of the QEF attn handler.
...
QEF_CB		*qef_cb )
{
    ...
    scf_cb.scf_nbr_union.scf_amask = 0;
