...
**	13-may-2009 (dougi)
**	    Add QEU_X100DBU to handle X100-specfic DBU operations.
**      28-Sep-2009 (gefei01)
**          Support CREATE INDEX on X100 table.
**	8-mar-2010 (dougi)
**	    Add QEU_IVWCALL to process X100 "syscall".
**	2-Dec-2010 (kschendel) SIR 124685
**	    Warning, prototype fixes.
**	6-Jan-2011 (kschendel)
**	    Add X100_DROPTT for DMF to call when dropping session temp
**	    due to rollback.
...
**	    Init the X100 error/interrupt status for the operation.
*/
DB_STATUS
qef_call(
i4	    qef_id,
...
#if defined(conf_IVW)
	x100_session_clear_error(qef_cb->qef_ivwsesscb);
#endif
...

    ...
    */
	...
	case QEU_X100_DROPTT:
	    /* Note that this one might be recursive!  might be in the
	    ** middle of an X100DBU to drop a temp table.
	    */
	    status = qea_x100_droptt(qef_cb, qef_rcb, err_cb.err_block);
	    break;
	...
	case QEU_X100DBU:
	    /* perform the dbu with transaction support */
            status = qea_x100_dbu(qef_cb, qeu_cb, err_cb.err_block);
	    break;

	case QEU_IVWCALL:
	    /* perform X100 "syscall" */
            status = qea_x100_syscall(qef_cb, err_cb.err_block);
	    break;
