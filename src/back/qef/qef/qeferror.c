...
**          are not fixed.   
**      11-Feb-2009 (gefei01)
**          Force E_QE0311_X100_ERROR to be QEF_EXTERNAL.
**       1-Feb-2011 (hanal04) Bug 124876
**          Move QEF_ERR_MAP to qefkon.h rather than holding duplicates
**          in qeferror.c and qefdata.c. This move also makes QEF_ERR_MAP
**          available to qeucopy.c.
**/
...
**	    Rename 2037 error for drop restrict.
*/

VOID
qefErrorFcn(
...
#define MAX_PARMS	5
	...
	** with procname and line number, if so. */
	...
	else if ((DBerror->err_code >> 16) == DB_VWF_ID)
	{
	    /* Only define VW errors that have different handling in QEF
	    ** If the qefdata entry was the default then no entry needed. */
	    while (err_map->qef_code != 0L && err_map->err_no != DBerror->err_code)
		++err_map; /* search for exception VF code */
	    ...
	    {
		/* Most VW errors will be 'ALREADY_REPORTED' */
		qefError = *DBerror;
		qefError.err_code = E_QE0122_ALREADY_REPORTED;
	    }
	    uletype = 0L;
	}
...
/* added error message from INGRES 6.4 -- bug #62708 (harpa06) */
	...
	/* More special cases for Vectorwise */
	if ((DBerror->err_code >> 16) == DB_VWF_ID)
	{
	    /* Look for conditions indicating the X100 server is gone */
	    if (DBerror->err_code == E_VW1035_CONNECTION
	      || DBerror->err_code == E_VW1099_SERVER_START
	      || DBerror->err_code == E_VW1036_NETWORK)
	    {
		qef_cb->qef_abort = TRUE;	/* Abort entire transaction */
