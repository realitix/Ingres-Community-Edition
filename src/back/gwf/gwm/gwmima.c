...
**	8-Aug-2011 (kschendel for dmonge)
**	    Get proper PMhost declaration.
*/
...
**      24-Jul-1997 (stial01)
**          GM_init() Init GM_globals.gwm_gchdr_size
**      23-Jun-2011 (ashco01) Bug:125093     
**          If the local nodename (or II_HOSTNAME) is specified as a fully
**          qualified (dotted) nodename (ie: xxxxxx.ingres.com ) then 
**          connecting to the IMA GWF results in E_GW8045 and E_GC0132. 
**          Now process nodename via PM to ensure special characters within
**          the nodename are converted to underscore ('_').
**      13-Sep-2011 (ashco01) Bug: 125752
**          Backout above change (b125093) as it conflicts with fix for b122277
**          which now writes fully qualified hostnames including '.' (dots) to 
**          !.local_vnode in config.dat.
*/
static DB_STATUS
