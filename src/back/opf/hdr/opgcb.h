...
**	    Replace boolean ops_hash with flags on ops_amask to enable
**	    independent enabling/disabling of hash aggregation and hash join.
[@history_line@]...
**/
...
**	13-may-2008 (dougi)
**	    Added ops_x100 for X100 algebra generation.
**	30-oct-08 (hayke02)
...
**	30-sep-2010 (dougi)
**	    Remove ops_x100 flag - never used.
...
*/
    ...
    bool	    ops_parallel;	/* TRUE if parallel plans are to
					** be generated */
    bool	    ops_reuse;		/* TRUE if subquery reuse logic is
					** to be used (X100 only, for now) */
...
#define                 OPS_AHASHAGG  16
/* TRUE if hash aggregation should be enabled */
#define                 OPS_AHASHJOIN  32
/* TRUE if hash join should be enabled */
...
**	8-oct-2009 (dougi)
**	    Added opg_add/sub/mul/div.
**	5-feb-2010 (dougi)
**	    Added opg_pow.
**	3-Mar-2011 (kschendel) b125146
**	    Drop opg-ifnull, it's not valid anyway.  (Nowadays there are
**	    more than one function that does an equivalent of ifnull, so
**	    one needs to check an adi function bit.)
*/
...
/* maximum number of tables which can be contained in the RDF cache */
    ...
    ADI_OP_ID       opg_gt;             /* the ADF operator id for the
                                        ** ">" operator
                                        */
    ADI_OP_ID       opg_add;		/* the ADF operator id for the
                                        ** "+" operator */
    ADI_OP_ID       opg_sub;		/* the ADF operator id for the
                                        ** binary "-" operator */
    ADI_OP_ID       opg_mul;		/* the ADF operator id for the
                                        ** "*" operator */
    ADI_OP_ID       opg_div;		/* the ADF operator id for the
                                        ** "/" operator */
    ADI_OP_ID       opg_abs;		/* the ADF operator id for the
                                        ** "abs" operator */
    ADI_OP_ID       opg_sqrt;		/* the ADF operator id for the
                                        ** "sqrt" function */
    ADI_OP_ID	    opg_pow;		/* the ADF operator id for the
					** "power" (or "**") function */
    ...
    ADI_OP_ID       opg_scount;		/* the ADF operator id for the
                                        ** COUNT(*) */
    ADI_OP_ID       opg_f8;             /* the ADF operator id for the
                                        ** "float8" function */
