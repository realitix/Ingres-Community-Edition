...
**      Changed TEXT2 and TEXT2B to %lld for pages and row count which are now
**      i8 values
*/

...
# define TEXT1I "\n\n*** statistics for database %s version: %s"
# define TEXT2 	"\n*** table %s rows:%lld pages:%lld overflow pages:%d"
# define TEXT2A	"*** table "
# define TEXT2B	"rows:%lld pages:%lld overflow pages:%d"
# define TEXT2C "rows"
...
# define TEXT3E "(length: %d, scale: %d, %s)"
...
# define TEXT7OD "\ncell:%%5d    count:%%-%d.%df    cumcnt:%%-%d.%df    repf:%%-%d.%df    value:"
...
# define TEXT7ID "\ncell:%d    count:%f    cumcount:%f    repf:%f    value:"
...
typedef struct _OPQ_RLIST
{
	...
	DB_DELIM_STR	argname;	/* Relation name exactly as given on
					** the command line */
	i8		ntups;		/* number of tuples in relation */
	...
	i8		pages;		/* estimated no. of physical pages*/
	...
	bool		x100tab;	/* TRUE if X100 table, else FALSE */
        i4              relwid;         /* row width for rms gateway use */
	char		storage_str[DB_IISTRUCT_LEN];/* storage_structure type */
} OPQ_RLIST;
...
# define    OPQ_INGRESVW    0x10    /* bit 4 */
} OPQ_DBMS;

/*
...
#define		    OPQ_INIPREC	    7
    ...
    bool	opq_hexout;	/* Flag indicating whether "-zhex" pam
				** was specified */
    bool	opq_dougout;	/* Flag indicating whether "-zdug" pam
				** was specified to display cumulative counts */
