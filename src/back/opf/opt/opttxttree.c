...
**      justone		TRUE - caller wants only the first action header printed
**			FALSE - caller wants the whole list of action headers printed
**
** Outputs:
**	Returns:
**	    {@return_description@}
...
**	    Added parameter to signal whether to print just the
**	    single action header, or the whole list.
[@history_template@]...
*/
VOID
opt_qtdump  (
...
**          created.
**	21-Jan-2011 (smeke01) Bug 124949
**	    Use new parameter to opt_qtdump to signal that we want the
**	    whole list printed, not just the first action header.
[@history_template@]...
*/
VOID
opt_qpdump  (
	...
	QEF_QP_CB   *qplan)
{
	...
	opt_qtdump( global, qplan->qp_ahd, FALSE);
	...
	if (qplan->qp_ddq_cb.qeq_d1_end_p != NULL)
	{
	    ...
	    opt_qtdump( global, qplan->qp_ddq_cb.qeq_d1_end_p, FALSE);
	}
	else
	{
	    TRformat( opt_scc, (i4 *) global,
