/*
**Copyright (c) 1995, 2005, 2010 Actian Corporation
*/
...
**	    Clean up 153/156 a bit to add refinements from codev,
**	    and to reduce redundant noise output.
...
static void opt_pt_gbcr_node(
	...
	char *indentstr,
	char *temp);
static void opt_pt_win_node(
	PST_WINDOW_NODE *wnode,
	char *indentstr,
	char *temp);
...
/* Range table types */
static const char *rgtype_names[] = {
	...
	DB_DATA_VALUE      *datatype)
{
	...
	tempbuf = (char *)opu_memory(global, DB_MAXSTRING + 1);
...
opt_fall()
{
    ...
    if (subquery->ops_funcs.opz_fv <= 0)
    {
	TRformat(opt_scc, (i4 *)global,
		(char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
	...
	i4                opid)
{
    ...
    status = adi_opname( global->ops_adfcb, (ADI_OP_ID)opid, name);
    if (status != E_DB_OK)
	TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
...
**          initial creation
**	3-Mar-2011 (kschendel) m1475
**	    Clean up op156: don't print the same global over and over.
*/
static VOID
opt_big(
	OPB_BFKEYINFO      *bfinfop)
{
    ...
    else if (bfglobalp->opb_bfmask & OPB_GLOBAL_PRINTED)
    {
	TRformat(opt_scc, (i4 *)global,
		(char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"OPB_BFGLOBAL: addr=%p already dumped\n\n" , bfglobalp);
	return;
    }
    bfglobalp->opb_bfmask |= OPB_GLOBAL_PRINTED;
    TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	...
	OPB_IBF            bfi)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	 "    bfkeyinfo=%p, querytree=%p, ojid=%d, mask=%x\n\n",
	bfp->opb_keys,
    ...
    opt_newline(global);
    /* now that op153 is a little more useful, I think maybe this can be
    ** left in for the full op156 ... if you don't like it you can comment
    ** it out, or maybe add a global bool for enabling via debugger.
    ** This is of course a GROSSLY illegal facility cross-call!  :-)
    */
...
**	6-Mar-2011 (kschendel) b125146
**	    Init global-printed flag to reduce redundant output.
*/
static VOID
opt_ball()
{
    ...
    opt_newline(global);
    /* Brute force traverse thru BF's to init the global flag.  Doesn't
    ** seem to be an easier way to reach all the bfglobal's.
    */
	...
	for (bfkp = bfp->opb_keys; bfkp; bfkp = bfkp->opb_next)
	{
	    bfgp = bfkp->opb_global;
	    if (bfgp != NULL)
		bfgp->opb_bfmask &= ~OPB_GLOBAL_PRINTED;
	}
    }
    /* Print the BF's */
    for (bfi = 0; bfi < subquery->ops_bfs.opb_bv; bfi++)
...
**	7-jan-2010 (dougi)
**	    Changed to use opv_varname as table name.
**	8-Apr-2011 (kschendel) SIR 125253
**	    Above change lost the ability to print both table and corr
**	    names, repair.
*/
bool
opt_ptname(
	...
	bool		    corelation)
{
    ...
    grv_ptr = subquery->ops_vars.opv_base->opv_rt[varno]->opv_grv; /*
				...
				*/
    if (grv_ptr->opv_gmask & OPV_TPROC)
    {
	DB_TAB_NAME		*tabname;
	i4                  tabnamesize;
	...
	/* Use RDF name if we see one and if caller isn't asking for
	** correlation name.  opv-relation is the real table name.
	** opv_varname is the temp table or correlation name.  If caller
	** wants corr name, compare to RDF name (if any) and return
	** indication of the difference.
	*/
	if (!corelation && grv_ptr->opv_relation && grv_ptr->opv_relation->rdr_rel)
	    tabname = &grv_ptr->opv_relation->rdr_rel->tbl_name;
	else
	{
	    tabname = &subquery->ops_vars.opv_base->opv_rt[varno]->opv_varname;
	    if (corelation && grv_ptr->opv_relation && grv_ptr->opv_relation->rdr_rel)
		retval = memcmp(tabname, &grv_ptr->opv_relation->rdr_rel->tbl_name, sizeof(DB_TAB_NAME));
	}
	tabnamesize = opt_noblanks( sizeof(tabname->db_tab_name),
	    (char *)tabname->db_tab_name );		/* number of significant chars*/
	...
	*/
	...
	else
	{
		...
		if (cop->opo_variant.opo_local->opo_mask & OPO_ANTIJOIN)
		    outer_join = "left anti join";
		else if (cop->opo_variant.opo_local->opo_mask & OPO_SUBQJOIN)
		    outer_join = "left subq join";
		else if (cop->opo_variant.opo_local->opo_mask & OPO_SEMIJOIN)
		    outer_join = "left semi join";
            	break;
            case OPL_RIGHTJOIN:
            	outer_join = "right join";
		if (cop->opo_variant.opo_local->opo_mask & OPO_ANTIJOIN)
		    outer_join = "right anti join";
		else if (cop->opo_variant.opo_local->opo_mask & OPO_SUBQJOIN)
		    outer_join = "right subq join";
		else if (cop->opo_variant.opo_local->opo_mask & OPO_SEMIJOIN)
		    outer_join = "right semi join";
            	break;
            case OPL_FULLJOIN:
        ...
        else
	{
	    ...
	    if ((global->ops_gmask & OPS_X100) && cop->opo_variant.opo_local)
		uld_prnode(uld_control, cop->opo_variant.opo_local->opo_x100jt);
	}
	break;
    }
	...
	i4                 dmfattr)
{
    ...
    */
	    ...
	    attrp->att_key_seq_number);
	{
	    ...
	    datatype.db_collID = attrp->att_collID;
	    opt_dt(&datatype);
	}
    opt_newline(global);
	...
	OPV_IVARS          rv, OPS_SUBQUERY   *sq, bool	histdump)
{
    ...
    OPS_STATE           *global;    /* ptr to global state variable */
    OPV_VARS            *rvp;       /* ptr to range var element to display*/
    OPV_GRV		*grvp;	    /* ptr to global range var element */
    RDR_INFO		*relp;	    /* ptr to RDF info for base table */
    DMT_REFREL		*refrelp;
    ...
    opt_map(" uniqmp=", (PTR)&rvp->opv_uniquemap, maxeqcls);
    opt_newline(global);
	...
	TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    "    X100 cluster ID=%d", rvp->opv_x100clid);
    if (relp != (RDR_INFO *) NULL && relp->rdr_rel != (DMT_TBL_ENTRY *) NULL)
	TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    "    timestamp = (%d, %d)", relp->rdr_rel->tbl_date_modified.
	    db_tab_high_time, relp->rdr_rel->tbl_date_modified.db_tab_low_time);
    opt_newline(global);
    /* Display refrel info (if any). */
    if (relp != (RDR_INFO *) NULL && (refrelp = relp->rdr_refrel) !=
		(DMT_REFREL *) NULL && refrelp->refrel_cnt > 0)
    {
	TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    "Referential relationship info: refrel_cnt: %d",
	    refrelp->refrel_cnt);
	opt_newline(global);
	for (i = 0; i < refrelp->refrel_cnt; i++)
	{
	    DB_REFREL	*rrarrayp;
	    rrarrayp = &refrelp->refrel_array[i];
	    TRformat(opt_scc, (i4 *)global,
		(char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"   [%d]: refed: (%d, %d), refing: (%d, %d), ixcol: %d\n\n",
		i, rrarrayp->dbrr_refedtabid.db_tab_base,
		rrarrayp->dbrr_refedtabid.db_tab_index,
		rrarrayp->dbrr_refingtabid.db_tab_base,
		rrarrayp->dbrr_refingtabid.db_tab_index,
		rrarrayp->dbrr_refingixcol);
	}
    }
...
opt_ojstuff(OPS_SUBQUERY  *sq1)
{
	...
	TRformat(opt_scc, (i4 *)global,
       	   (char *)&global->ops_trace.opt_trformat[0],
   	   (i4)sizeof(global->ops_trace.opt_trformat),
   "OPL_OUTER: entry=%d, id=%d, node=%p, type=%d, translate=%d, mask=%d\n",
	   i, outerp->opl_id, outerp->opl_nodep, outerp->opl_type,
...
**	2-Mar-2011 (kschendel) m1475
**	    Caller can ask for eqc dump (for op153)
*/
VOID
opt_rall(bool	histdump, bool bfdump)
{
    OPS_SUBQUERY        *subquery;  /* ptr to subquery being traced */
    OPS_CB              *opscb;	    /* ptr to session control block */
    OPS_STATE           *global;    /* ptr to global state variable */
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"SUBQUERY (%d) gentry=%d, sqtype=%d\n\n", subquery->ops_tsubquery, subquery->ops_gentry, subquery->ops_sqtype);
    TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
    ...
    if (subquery->ops_msort.opo_sv > 0)
	opt_mall();			    /* multi-attr order info */
    /* Dump outer join descriptors, too. */
    if (subquery->ops_oj.opl_base)
	opt_ojstuff(subquery);
    opt_fall();			    /* func attr stuff */
    if (bfdump)
    {
	opt_ball();		    /* Boolean factor stuff */
    }
	...
	OPE_IEQCLS         eqcls)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	" EQCLS= %d, eqctype= %s, bfindex= %d, nbf= %d, mask= %d, nulljoin= %d, ",
	eqcls,
        eqctype,
	eqclsp->ope_bfindex,
        eqclsp->ope_nbf,
    ...
    if (eqclsp->ope_joinrep)
	opt_map("joinrep", (PTR)eqclsp->ope_joinrep,
				(i4)subquery->ops_eclass.ope_ev);
    if (eqclsp->ope_repjoin)
	opt_map("repjoin", (PTR)eqclsp->ope_repjoin,
				(i4)subquery->ops_eclass.ope_ev);
    if (eqclsp->ope_mask & OPE_REFNOJOINIX)
	opt_map("nojoinix", (PTR)eqclsp->ope_nojoinix,
				(i4)subquery->ops_eclass.ope_ev);
    if (eqclsp->ope_rjrefed)
	opt_map("refed", (PTR)eqclsp->ope_rjrefed,
				(i4)subquery->ops_vars.opv_rv);
    if (eqclsp->ope_rjrefing)
	opt_map("refing", (PTR)eqclsp->ope_rjrefing,
				(i4)subquery->ops_vars.opv_rv);
    opt_newline(global);
}

/*{
** Name: opt_morder	- print info on one multi-attr ordering
**
** Description:
**
** Inputs:
**      eqcls                           ordering to print
**
** Outputs:
**	Returns:
**	    VOID
	...
	OPE_IEQCLS         eqcls)
{
    ...
    OPS_CB              *opscb;	    /* ptr to session control block */
    OPS_STATE           *global;    /* ptr to global state variable */
    OPO_SORT		*sortp;	    /* ptr to ordering */
    i4			i;
    OPO_ISORT		j;
    ...
    subquery = global->ops_trace.opt_subquery; /* get ptr to current subquery
                                    ** which is being traced */
    sortp = subquery->ops_msort.opo_base->opo_stable[eqcls]; /* ptr to
                                    ** ordering element */
	...
	(i4)sizeof(global->ops_trace.opt_trformat),
	" MORDER= %d, stype= %d, ",
	eqcls+subquery->ops_eclass.ope_ev,
        sortp->opo_stype);

    opt_map ("eqcmap", (PTR)sortp->opo_bmeqcls, subquery->ops_eclass.ope_ev);
    if (sortp->opo_stype == OPO_SEXACT)
    {
	TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    ", order= ");
	for (i = 0; (j = sortp->opo_eqlist->opo_eqorder[i]) > OPE_NOEQCLS; i++)
	 TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    " %d", j);
...

/*{
** Name: opt_mall	- print all the multi-EQC orderings
**
** Description:
**      This routine will print out info on all the multi-attribute orders
**      currently defined.
**
** Inputs:
**	Input parameters are implicitly obtained from the session control
...
opt_mall()
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"OPO_SORT: base=%p, number of orders=%d, mask=%x",
	subquery->ops_msort.opo_base, subquery->ops_msort.opo_sv,
	subquery->ops_msort.opo_mask);
    opt_newline(global);

    for (eqcls = 0; eqcls < subquery->ops_msort.opo_sv; eqcls++)
	if (subquery->ops_msort.opo_base->opo_stable[eqcls] )
	    opt_morder(eqcls);	    /* print the ordering info */
}

/*{
...
**          initial creation prEqlist
**	2-Mar-2011 (kschendel) m1475
**	    Only print global stuff for main, not every sq.
*/
VOID
opt_jall(
	OPS_SUBQUERY	*subquery)
{
    ...
    opt_rall(TRUE,TRUE);	    /* op153 plus bf info */
    opt_newline(global);
}

/*{
	...
	bool		withStats)
{
	...
	case OPS_WINDOW:
	    opt_printPlanType( global, withStats,
	     (subquery->ops_sqtype == OPS_REUSE) ? " of reuseable fragment " :
	     (subquery->ops_sqtype == OPS_HFAGG) ?  " of hash func aggregate " :
	     (subquery->ops_sqtype == OPS_WINDOW) ? " of window fragment " :
...
**	18-jan-2010 (dougi)
**	    Expand subqueries contained in var arrays (used for X100 "after
**	    enum" calls).
*/
static VOID
opt_pt_sq(global, header, subqry, nest, already_done,
					nodetail, noconc, vtexpand)
OPS_STATE          *global;
PST_QTREE          *header;
OPS_SUBQUERY	   *subqry;
i4		   *nest;
...
bool		    vtexpand;
{
	...
	if (i < MAX_ALREADY_DONE)
	{
		...
		subqry->ops_byexpr);
        TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
          OPT_PBLEN, "  ops_tsubquery: %d   ops_tplan: %d\n\n",
               subqry->ops_tsubquery, subqry->ops_tplan);
        TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
	...
	if (subqry->ops_gentry > 0)
	{
	    OPV_GRV	*grvp = global->ops_rangetab.opv_base->
				opv_grv[subqry->ops_gentry];

	    TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
		OPT_PBLEN, "  corresponding OPV_GRV: opv_gsubselect: 0x%p, opv_created: %3d, opv_gquery: 0x%p\n",
		grvp->opv_gsubselect, grvp->opv_created, grvp->opv_gquery);
	    TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
		OPT_PBLEN, "  opv_mask: %d", grvp->opv_gmask);
	    if (grvp->opv_gsubselect)
		TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
		    OPT_PBLEN, ", opv_subquery: 0x%p\n",
		    grvp->opv_gsubselect->opv_subquery);
	    else
		TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
		    OPT_PBLEN, "\n");
	}
	...
	if (subqry->ops_agg.opa_concurrent && *nest < 10 && !noconc)
	{
            TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
        	OPT_PBLEN, "\nConcurrent Subquery: \n\n");
	    (*nest)++;
	    opt_pt_sq(global, header, subqry->ops_agg.opa_concurrent, nest,
			already_done, nodetail, noconc, vtexpand);
	...
	if (vtexpand)
	{
	    ...
	    for (i = 0; i < subqry->ops_vars.opv_rv; i++)
	    {
		if ((gvarp = subqry->ops_vars.opv_base->opv_rt[i]->opv_grv)->
			opv_gquery)
		{
		    TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
			OPT_PBLEN, "Virtual Table: %s: \n",
			&subqry->ops_vars.opv_base->opv_rt[i]->opv_varname);
		    opt_pt_sq(global, header, gvarp->opv_gquery, nest,
			already_done, nodetail, noconc, vtexpand);
		}
	    }
	    (*nest)--;
	     ...
	     i2             indent)
{
   ...
   do {
	...
	qnode->pst_sym.pst_dataval.db_prec-
      ...
      if (qnode->pst_sym.pst_type != PST_ROOT &&
	...
	"\n%spst_len: %d\n", indentstr, qnode->pst_sym.pst_len);
	 ...
	 break;
       ...
       case PST_WINDOW:
	 opt_pt_win_node(&qnode->pst_sym.pst_value.pst_s_win, indentstr, temp);
	 break;
...
static VOID
opt_pt_win_node(PST_WINDOW_NODE    *wnode,
	     char            *indentstr,
             char            *temp)
{
   /* Just print the WINDOW node field */
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_pcount: %d\n", indentstr, wnode->pst_pcount);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_ocount: %d\n", indentstr, wnode->pst_ocount);
}    /* end of opt_pt_win_node */
...
**	    Move type display, skip rest if unused.
*/
static VOID
opt_pt_hddump(
	PST_QTREE	   *header,
	bool		   printviews)
{
	...
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	   "\n  pst_rgtype: %s (%d)\n",
	   rgtype_names[header->pst_rangetab[i]->pst_rgtype],
...

{
    ...
    TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
           OPT_PBLEN, "\n\nDump of QEP for subquery plan (%d, %d), type: %s, duplicates: %s\n\n",
	   subquery->ops_tsubquery, subquery->ops_tplan, sqtype,
	   (subquery->ops_duplicates == OPS_DKEEP) ? "keep" :
		(subquery->ops_duplicates == OPS_DREMOVE) ? "remove" : "undefined");
    /* Then the multi-attr orderings */
    if (subquery->ops_msort.opo_sv)
    {
	opt_newline(global);
	opt_mall();
    }
...

{
	...
	break;
     case DB_X100CL_STORE:
	storage_struct = "X100CL";
	break;
     case DB_X100IX_STORE:
	storage_struct = "X100IX";
	break;
     case DB_X100RW_STORE:
	storage_struct = "X100RW";
	break;
     default:
    ...
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%s===========================================\n", indentstr);
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sCO Node: 0x%p, type: %s%s, storage: %s, ID: %d\n",
	indentstr, conode, nodetype, jointype, storage_struct,
	conode->opo_variant.opo_local->opo_id);
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_outer: 0x%p, opo_inner: 0x%p, opo_x100clid: %d\n",
	indentstr, conode->opo_outer, conode->opo_inner, conode->opo_x100clid);
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
    ...
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%s           tups: %f, adfactor: %f, sortfact: %f, twidth:%d)\n",
	indentstr, conode->opo_cost.opo_tups, conode->opo_cost.opo_adfactor,
	conode->opo_cost.opo_sortfact, conode->opo_cost.opo_twidth);
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sFlags - delete: %s, existence: %s, odups: %s, idups: %s, x100merge: %s\n",
	indentstr, (conode->opo_deleteflag) ? "T" : "F",
	(conode->opo_existence) ? "T" : "F", (conode->opo_odups) ? "T" : "F",
	(conode->opo_idups) ? "T" : "F", (conode->opo_x100merge) ? "T" : "F");
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%s        osort: %s, isort: %s, psort: %s, pdups: %s, joinix: %s, opo_bfs: %s\n",
	indentstr, (conode->opo_osort) ? "T" : "F", (conode->opo_isort) ? "T" : "F",
	(conode->opo_psort) ? "T" : "F", (conode->opo_pdups) ? "T" : "F",
	(conode->opo_joinix) ? "T" : "F", (conode->opo_bfs) ? "T" : "F");
    if (global->ops_gmask & OPS_X100)
    {
      TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%s       1-N: %s, N-1: %s, N-M: %s, refrel: %s, nulljoin: %s\n",
	indentstr, (conode->opo_1n) ? "T" : "F", (conode->opo_n1) ? "T" : "F",
	(conode->opo_nm) ? "T" : "F", (conode->opo_refrel) ? "T" : "F",
	(conode->opo_x100nullj) ? "T" : "F");
    }

    if (conode->opo_variant.opo_local)
    {
	opt_map("  opo_bmbf", (PTR)conode->opo_variant.opo_local->opo_bmbf,
				subquery->ops_bfs.opb_bv);
	opt_map("  opo_bmvars", (PTR)conode->opo_variant.opo_local->opo_bmvars,
				subquery->ops_vars.opv_prv);
	if (conode->opo_variant.opo_local->opo_ojid >= 0)
	{
	    opt_newline(global);
	    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
		"	opo_ojid: %d,  opo_ojtype: %d,",
		conode->opo_variant.opo_local->opo_ojid,
		conode->opo_variant.opo_local->opo_type);
	    opt_map("  opo_ojoin", (PTR)conode->opo_variant.opo_local->opo_ojoin,
				subquery->ops_vars.opv_prv);
	}
    }
	...
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_ordeqc: %d %s\n", indentstr, conode->opo_ordeqc, temp1);
    }
	...
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_sjeqc: %d %s\n", indentstr, conode->opo_sjeqc, temp1);
    }

    for (i = 0; i < conode->opo_clcount; i++)
    {
	char	temp1[OPT_PBLEN+1];

	opt_qepd_alist(global, subquery, conode,
				conode->opo_clusteqcs[i], temp1);
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_clusteqcs[%d]: %d %s\n", indentstr, i,
				conode->opo_clusteqcs[i], temp1);
    }

    for (i = 0; i < conode->opo_FDcount; i++)
    {
	char	temp1[OPT_PBLEN+1];

	opt_qepd_alist(global, subquery, conode,
				conode->opo_FDieqcs[i], temp1);
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_FDieqcs[%d]: %d %s\n", indentstr, i,
				conode->opo_FDieqcs[i], temp1);
	opt_qepd_alist(global, subquery, conode,
				conode->opo_FDdeqcs[i], temp1);
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_FDdeqcs[%d]: %d %s\n", indentstr, i,
				conode->opo_FDdeqcs[i], temp1);
    }

    for (i = 0; i < conode->opo_ucount; i++)
    {
	char	temp1[OPT_PBLEN+1];

	opt_qepd_alist(global, subquery, conode,
				conode->opo_ueqcs[i], temp1);
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_ueqcs[%d]: %d %s\n", indentstr, i,
				conode->opo_ueqcs[i], temp1);
    }

    if (conode->opo_jixeqcs && BTcount((char *)conode->opo_jixeqcs, subquery->
			ops_eclass.ope_ev) > 0)
    {
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	    "\n%s", indentstr);
	opt_map ("opo_jixeqcs", (char *)conode->opo_jixeqcs,
			subquery->ops_eclass.ope_ev);
    }
    if (conode->opo_njixeqcs && BTcount((char *)conode->opo_njixeqcs, subquery->
			ops_eclass.ope_ev) > 0)
    {
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	    "\n%s", indentstr);
	opt_map ("opo_njixeqcs", (char *)conode->opo_njixeqcs,
			subquery->ops_eclass.ope_ev);
    }

    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%s", indentstr);
    opt_map ("opo_eqcmap", (char *)&conode->opo_maps->opo_eqcmap,
			subquery->ops_eclass.ope_ev);

    if (BTnext((i4)-1, (char *)&conode->opo_maps->opo_eqcmap,
		subquery->ops_eclass.ope_ev) >= 0)
    {
	char	temp1[OPT_PBLEN+1];

	opt_qepd_alisteqc(global, subquery, conode,
				&conode->opo_maps->opo_eqcmap, temp1);
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	 "\n%sopo_eqcmap: %s\n", indentstr, temp1);
    }
	...
	char		   *prbuf)
{
	...
	/* Hacky check for buffer overflow. */
	if (off + 64 > OPT_PBLEN)
	{
	    eof = TRUE;
	    TRformat(NULL, (i4 *) NULL, &prbuf[off], OPT_PBLEN-off,
...

/*{
** Name: OPT_QEPD_ALISTEQC	- opt_qepd_alist from an eqcmap
**
** Description:
**      This routine will dump an attribute list for a node
**      in the QEP dumping routines when passed an eqc map
**
** Inputs:
**
** Outputs:
	...
	char		   *prbuf)
{
    OPE_IEQCLS      eqcls;	    /* next attribute of a multi-attribute
				    ** ordering */
    bool	    eof = FALSE;    /* TRUE if the last attribute is
				    ** processed */
    i4		    level;	    /* used to process multi-attribute
				    ** ordering */
    i4		    off = 0;	    /* Offset to next print buffer entry */
    ...
    eqcls = BTnext((i4)-1, (char *)eqcmap, subquery->ops_eclass.ope_ev);
    MEfill(OPT_PBLEN+1, (u_char)0, (PTR)prbuf);
    prefix = "(";
    do
    {
	opt_attrnm(subquery, cop, (i4) eqcls, &stoname); /* get the name of
				    ** the attribute associated with this
				    ** subtree cop */
	if ((eqcls = BTnext((i4)eqcls, (char *)eqcmap,
				subquery->ops_eclass.ope_ev)) < 0)
	{
	    eof = TRUE;
	    delimiter = ")";	    /* use closing bracket for the last
