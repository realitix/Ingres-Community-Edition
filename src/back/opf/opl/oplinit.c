...
**	27-oct-2008 (dougi)
**	    Flag anti-joins (from NOT IN/EXISTS subselect flattening).
**	29-sep-2009 (dougi)
**	    Flag semi-joins (from ... ANY ... subselect flattening).
**	15-Feb-2011 (kschendel)
**	    Delete opl_gid, not used.
*/
OPL_IOUTER
opl_ioutjoin(
	OPS_SUBQUERY       *subquery,
	PST_J_ID	   joinid)
{
	...
	if (global->ops_goj.opl_antijmap &&
		BTtest((i4)joinid, (char *)global->ops_goj.opl_antijmap))
	{
	    outjoinp->opl_mask |= OPL_ANTIJOIN;
	    outjoinp->opl_type = OPL_LEFTJOIN;
	}

	if (global->ops_goj.opl_semijmap &&
		BTtest((i4)joinid, (char *)global->ops_goj.opl_semijmap))
	{
	    outjoinp->opl_mask |= OPL_SEMIJOIN;
	    outjoinp->opl_type = OPL_LEFTJOIN;
	}

	if (global->ops_goj.opl_subqjmap &&
		BTtest((i4)joinid, (char *)global->ops_goj.opl_subqjmap))
	{
	    outjoinp->opl_mask |= OPL_SUBQJOIN;
	    outjoinp->opl_type = OPL_LEFTJOIN;
	}
    }
...
**	    the original bug.
**	15-Feb-2011 (kschendel)
**	    Delete opl-subvar, not used anywhere.
*/
VOID
opl_ojinit(subquery)
OPS_SUBQUERY       *subquery;
