...
**	    Delete lots of ancient #if-0'ed outer join code, causes confusion.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**	29-Jul-2011 (kiria01) b125613
**	    Updated call interface of opl_resolve reflecting
**	    pst_resolve change.
**/
...
**	17-april-2009 (dougi)
**	    No iiiftrue() in X100 queries.
**	28-june-2010 (dougi)
**	    Fine tune previous change so CTAS with OJ query works ok for IVW.
*/
static VOID
opl_target(
	OPS_SUBQUERY       *subquery,
	PST_QNODE          **qnodepp)
{
	    ...
	    (subquery->ops_oj.opl_smask & OPL_IFTRUE))
	{
		...
		(subquery->ops_mode == PSQ_X100_DGTT_AS)  ||
		...
		/* cannot require equal datatypes because DB_TABKEY_TYPE gets
		** converted to char */
	    }
	...
	OPS_SUBQUERY       *subquery)
{
	...
	** which were referenced in the on clauses */
	...
	{
	    ojp1 =  lbase->opl_ojt[ojid1];
	    if (ojp1->opl_mask & (OPL_ANTIJOIN | OPL_SEMIJOIN | OPL_SUBQJOIN))
		continue;
