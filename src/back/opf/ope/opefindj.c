...
**	    If we have a PST_IMPLICIT equals node on nullables, tell
**	    mergeatts that it's a nulljoin.  That kind of node is an
**	    agg subquery link-back (ie join of agg temp with outer),
**	    and opa couldn't be arsed to generate the full nulljoin
**	    expression, it just generated an equals.  (Without this,
**	    mergeatts may decide that it's an explicit user join and
**	    drop the nulljoin, giving the wrong answer.)
*/
static bool
ope_jnclaus(
	OPS_SUBQUERY        *subquery,
	...
	bool		    *funcattr)
{
    ...
    PST_QNODE	*bop;		/* The qualification op node */
    ...
    */
	...
	vbase = subquery->ops_vars.opv_base;
        ojid = bop->pst_sym.pst_value.pst_s_op.pst_joinid;
        if ((ojid >= 0)
            &&
...
# endif
	/* See if it's a nulljoin that opalink was too lazy to do right.
	** FIXME this fixes m1880 for ingres queries but not IVW, for
	** vw we probably have to teach opalink about the whole schmeer.
	** [later] tried that, still not good enough, plus then opaoptim
	** doesn't know how to unlink the nulljoin style fragment...
	** there might still be hope with fixing opaoptim as well, but
	** this change will do for now.
	*/
	nulljoin = (oldnodep != NULL
		|| (bop->pst_sym.pst_value.pst_s_op.pst_flags & PST_IMPLICIT
		    && bop->pst_sym.pst_value.pst_s_op.pst_opno == ADI_EQ_OP
		    && l->pst_sym.pst_dataval.db_datatype < 0
		    && r->pst_sym.pst_dataval.db_datatype < 0) );
	    ...
	    (OPZ_IATTS)r->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id,
            nulljoin, ojid)); /* return if  a "normal"
	...
	*/
	    ...
	    )
	{
		...
		*njoinpp = and_node;	/* add to null join list */
		...
		&& 
		!(subquery->ops_global->ops_gmask & OPS_X100)
		&&
