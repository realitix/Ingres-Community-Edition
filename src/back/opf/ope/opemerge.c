...
**	21-dec-2010 (dougi)
**	    Refine to avoid nulljoin when nullable aggs are being joined but 
**	    between tables and not just within a table caused by agg dist.
*/
bool
ope_mergeatts(
	OPS_SUBQUERY       *subquery,
	...
	OPL_IOUTER	   ojid)
{
	...
	*/
	...
	if (nulljoin && agh1)
	{
		...
		(sqtype == OPS_HFAGG) ||
		(sqtype  == OPS_FAGG))
		agh2 = grvp->opv_gquery->ops_root;
	    else if (sqtype == OPS_MAIN && subquery->ops_sqtype == OPS_MAIN)
		agh2 = subquery->ops_root;

	    if (agh2 != (PST_QNODE *) NULL)
	    {
		OPV_BMVARS	vmap;
		i4		vcount;

		MEcopy((char *)&agh1->pst_sym.pst_value.pst_s_root.pst_tvrm,
		    sizeof(vmap), (char *)&vmap);
		BTand(OPV_MAXVAR, (char *)&agh2->pst_sym.pst_value.pst_s_root.
			pst_tvrm, (char *)&vmap);
		if ((vcount = BTcount((char *)&vmap, OPV_MAXVAR)) == 0 ||
		    BTcount((char *)&agh1->pst_sym.pst_value.pst_s_root.
			pst_tvrm, OPV_MAXVAR) > vcount ||
		    BTcount((char *)&agh2->pst_sym.pst_value.pst_s_root.
			pst_tvrm, OPV_MAXVAR) > vcount)
		    nulljoin = FALSE;
	    }
	}
