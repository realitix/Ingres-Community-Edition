...
**	7-jan-2009 (dougi)
**	    Disable the "1 replacement IX only" constraint for X100 queries.
*/
static VOID
opv_uindex(
	OPS_SUBQUERY       *subquery,
	...
	bool		   explicit_index)
{
    ...
    ** choice, when the cost of TID joining is added in. */
    if (index_sub  && !(subquery->ops_global->ops_gmask & OPS_X100)
	&& *gotsub && mbfkl.opb_count == 0) return;
...
**	22-sep-2010 (dougi)
**	    Skip VectorWise tables - indexes are handled differently.
*/
VOID
opv_index(
	OPS_SUBQUERY       *subquery)
...
** to avoid the halloween problem */
    ...
    ** incrementing if there isn't enough room in the range table. */
	...
	if (gvar_ptr->opv_relation->rdr_rel->tbl_storage_type > DB_STDING_STORE_MAX)
	    continue;		    /* skip VectorWise tables */
