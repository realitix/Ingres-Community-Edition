...
**	29-april-2009 (dougi)
**	    Init. X100-only field opv_x100clid.
**	7-jan-2009 (dougi)
**	    Load opv_varname with [virtual]table name. Used by X100 cross 
**	    compiler.
*/
OPV_IVARS
opv_relatts(
	...
	OPV_IVARS          primary)
{
    ...
    */
    ...
    var_ptr->opv_primary.opv_tid = OPE_NOEQCLS; /* equivalence class not
                                    ** assigned yet */
    var_ptr->opv_x100clid = -1;		/* X100 only */
	...
	if ((subquery->ops_global->ops_cb->ops_server->opg_qeftype == OPF_GQEF)
	    ...
	    subquery->ops_gateway.opg_smask |= OPG_INDEX; /* set the index
				    ...
				    ** is involved */
	if (var_ptr->opv_grv->opv_qrt > OPV_NOGVAR)
	    MEcopy((char *) &global->ops_qheader->pst_rangetab[var_ptr->
				opv_grv->opv_qrt]->pst_corr_name, 
		sizeof(DB_TAB_NAME), (char *)&var_ptr->opv_varname);
	else MEcopy((char *) &var_ptr->opv_grv->opv_relation->rdr_rel->
				tbl_name.db_tab_name,
		sizeof(DB_TAB_NAME), (char *)&var_ptr->opv_varname);
					/* copy table/correlation name */
    }
/*  else
...
*/
    ...
    /* If this is a virtual table, generate a "name" - "_VTm_n_o_p" where M is
    ** the ops_gentry value, n is localrv and o is the "subquery number" that
    ** defines it and p is the subquery number from which it is referenced. */
    if (var_ptr->opv_grv->opv_gquery)
	STprintf((char *)&var_ptr->opv_varname, "_VT_%-u_%-u_%-u_%-u", 
			var_ptr->opv_grv->opv_gquery->ops_gentry, 
			localrv, var_ptr->opv_grv->opv_gquery->ops_tsubquery,
