...
PST_QNODE *opv_i1const(
	OPS_STATE *global,
	i4 intvalue);
PST_QNODE *opv_dblconst(
	OPS_STATE *global,
	f8 dblvalue);
PST_QNODE *opv_nullconst(
	OPS_STATE *global);
void opv_uparameter(
	OPS_STATE *global,
	PST_QNODE *qnode);
...

/*{
** Name: opv_dblconst	- create float8 constant node with value
**
** Description:
**      This routine will create a f8 integer constant with the given value 
**
** Inputs:
**      global                          ptr to global state variable
**      dblvalue                        value constant should take
**
** Outputs:
**	Returns:
**	    ptr to constant node
	...
	f8                dblvalue)
{
    ...
    datavalue.db_datatype = DB_FLT_TYPE;
    datavalue.db_prec = 0;
    datavalue.db_collID = DB_NOCOLLATION;
    datavalue.db_length = sizeof(dblvalue);
    datavalue.db_data = (PTR)opu_memory(global, (i4)sizeof(dblvalue));
    dblptr = (f8 *)datavalue.db_data;
    *dblptr = dblvalue;
    return(opv_constnode(global, &datavalue, 0));
}

/*{
** Name: opv_nullconst	- create NULL constant node 
**
** Description:
**      This routine will create a DB_LTXT_TYPE constant node with the empty
**	value (a.k.a. NULL)
**
** Inputs:
**      global                          ptr to global state variable
	...
	OPS_STATE          *global)
{
    ...
    datavalue.db_datatype = -DB_LTXT_TYPE;
    datavalue.db_prec = 0;
    datavalue.db_collID = DB_NOCOLLATION;
    datavalue.db_length = DB_CNTSIZE + 1;
    datavalue.db_data = (PTR)opu_memory(global, (i4)DB_CNTSIZE+1);
   
    status = adc_getempty(global->ops_adfcb, &datavalue);
