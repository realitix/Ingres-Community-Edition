...
**	12-Apr-2011 (kiria01)
**	    Handle common AGHEAD linkage handling. If tree being copied, has
**	    common AGHEADs linked together then link the copies into the peer
**	    lists.
*/
VOID
opv_copytree(
	...
	PST_QNODE          **nodepp)
{
	...
	{
	    opv_push_nodep(&stk, &(*nodepp)->pst_sym.pst_value.pst_s_root.pst_union.pst_next);
	}
	
	if (nodep->pst_sym.pst_type == PST_AGHEAD &&
		nodep->pst_sym.pst_value.pst_s_root.pst_union.pst_next)
	{
	    /* Common AGHEAD linkage present */
	    PST_QNODE *p = nodep->pst_sym.pst_value.pst_s_root.pst_union.pst_next;
	    nodep->pst_sym.pst_value.pst_s_root.pst_union.pst_next =
		p->pst_sym.pst_value.pst_s_root.pst_union.pst_next;
	    p->pst_sym.pst_value.pst_s_root.pst_union.pst_next = nodep;
	    /* If linkage present, don't replicate WHERE clause */
	    if (nodep->pst_left && nodep->pst_left != (PST_QNODE *) -1)
		nodepp = &(*nodepp)->pst_left;
	    else if (!(nodepp = opv_pop_nodep(&stk)))
		break;
	}

	/* VAR nodes can be tricked out, don't fall for it */
	else if (nodep->pst_sym.pst_type == PST_VAR)
	{
	    if (!(nodepp = opv_pop_nodep(&stk)))
		break;
	}
	    ...
	    nodep->pst_left != (PST_QNODE *) -1)
	{
		...
		opv_push_nodep(&stk, &(*nodepp)->pst_right);
	    nodepp = &(*nodepp)->pst_left;
	}
