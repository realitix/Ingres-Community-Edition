...
**	    Added minimal DOT tracing code to track parse tree elements.
**	31-Dec-2010 (kiria01) SIR 124690
**	    Propagate collID where previously missed.
**	14-Feb-2011 (kiria01) b124821
**	    Allow CO & SQ nodes be dumped and avoid multiple common pointers
**	    from being ignored in the trace.
**	14-Apr-2011 (kiria01)
**	    Display further data structures from global state.
**	28-Jun-2011 (kiria01)
**	    Build dot trace code unconditionally to allow use on windows.
...
**	14-Aug-2009 (kiria01)
**	    Protect against segv with X100 query text.
**	18-Oct-2010 (kschendel) SIR 124544
**	    Update create-integrity details, has DMU characteristics.
**	2-Dec-2010 (kschendel)
**	    Don't need x100 CTAS recovery info any more, delete here.
**	21-Jan-2011 (smeke01) Bug 124949
**	    Use new parameter to opt_qtdump to signal that we want just
**	    the single action header printed here, not the whole list.
**
*/
static VOID
opt_ahd(
...
SCROLL,KEYSET,PARM_FIRSTN,PARM_OFFSETN,LOAD_CTAS,PART_SEPARATE,HAS-HASHOP,MAIN",
		...
		(ahd->qhd_obj.qhd_qep.ahd_qepflag & AHD_SCROLL))
	{
		    ...
		    i++, attp++, off1++, off2++)
		{
		    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
		     OPT_PBLEN,
		     "[%2d] type: %d, size: %d, prec: %x, collID: %d, flags: %x, off1:%d, off2:%d\n\n",
			i, attp->attr_type, attp->attr_size, 
			attp->attr_precision, attp->attr_collID,
			attp->attr_flags_mask, *off1, *off2);
		}
	...
	{
	    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
		OPT_PBLEN, "X100 query length: %d, X100 cb index: %d, X100 flag: %d\n\n",
		ahd->qhd_obj.qhd_x100Query.ahd_x100_textlen,
		ahd->qhd_obj.qhd_x100Query.ahd_x100_cb,
		ahd->qhd_obj.qhd_x100Query.ahd_x100_flag);
	    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
		OPT_PBLEN, "X100 query text: %.#s\n\n",
		ahd->qhd_obj.qhd_x100Query.ahd_x100_textlen,
		ahd->qhd_obj.qhd_x100Query.ahd_x100_text);

	    if (ahd->qhd_obj.qhd_x100Query.ahd_resrow_format != NULL)
	    {
		GCA_COL_ATT	*coldesc;
		i4		i, offset = 0;

		TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
		    OPT_PBLEN, "\nahd_resrow_size: %d\tahd_resrow_format: \n\n",
		    ahd->qhd_obj.qhd_x100Query.ahd_resrow_size);
		for (i = 0, coldesc = &(((GCA_TD_DATA *)ahd->qhd_obj.
			qhd_x100Query.ahd_resrow_format)->gca_col_desc[0]);
		    i < ((GCA_TD_DATA *)ahd->qhd_obj.qhd_x100Query.
			ahd_resrow_format)->gca_l_col_desc; i++)
		{	/* loop over column descriptors */
		    DB_DATA_VALUE	dataval;
		    i4			l_attname;
		    DB_COPY_ATT_TO_DV( coldesc, &dataval );
    ...
    case QEA_X100_CRTABLE:
	{
	    ...
	    /* X100 CREATE TABLE AS SELECT needs this. */
	    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
		    OPT_PBLEN, "ahd_alt: %p, ahd_ID: %d\n\n",
		    ahd->qhd_obj.qhd_DDLstatement.ahd_alt, ahd->ahd_ID);
...
opt_dmucb(OPS_STATE *global, DMU_CB *dmucb)
{
    ...
    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
	OPT_PBLEN, "table structure: %4d\n\n", dmucb->dmu_chars.dmu_struct);
    TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
	...
	TRformat(opt_scc, (i4*)NULL, global->ops_cstate.opc_prbuf,
	    OPT_PBLEN,
	    "att#%3d name: %.24s type: %3d  size: %5d  prec: %2d collID: %2d flags: %5d\n\n",
	    i+1, att->attr_name.db_att_name, att->attr_type, 
	    att->attr_size, att->attr_precision, att->attr_collID,
	    att->attr_flags_mask);
    }
...
static void
fmt_bm(const void *bmv, i4 sz, char *buf)
{
    const unsigned char *bm = (const unsigned char*)bmv;
    if (bmv)
    {
	while (--sz && !bm[sz])
	    ;/*SKIP*/
...
static void fmt_i2_ar(FILE *fd, const char *title, const i2 *a, i4 sz, void *p)
{
    i4 i, lm = sz;
    if (p)
	SIfprintf(fd, "n%p->n%p\n", p, a);
    SIfprintf(fd, "n%p[fillcolor=white,label=\"{{%s", a, title);
    while (lm > 0 && a[lm-1] == 0)
	lm--;
    lm = ((lm + 9)/10)*10;
    for (i = 0; i < lm; i++)
    ...
    if (lm < sz)
	SIfprintf(fd, "0 to end");
    SIfprintf(fd, "}}\"]\n");
}

#define LINKBLOCK(p,f,r) if(p->f && pst_dot_chk(ctx, &p->f, p->f)){\
    SIfprintf(fd,"n%p->n%p[label="#f"]\n",p,p->f);\
    if ((void*)p!=(void*)p->f&&pst_ptr_ok(fd,p->f))r(ctx,fd,p->f);}
#define LINKBLOCKF(p,f,r,pn,n) if(p->f && pst_dot_chk(ctx,&p->f,p->f)){\
    SIfprintf(fd,"n%p:f%d->n%p[label="#f"]\n",pn,n,p->f);\
    if ((void*)p!=(void*)p->f&&pst_ptr_ok(fd,p->f))r(ctx,fd,p->f);}
...
opt_co_dot1(
	...
	OPO_CO *co)
{
    static const char *sjpr[] = {"","DB_ORIG","DB_PR","DB_SJ","DB_EXCH",
			"DB_RSORT","DB_RISAM","DB_RHASH","DB_RBTREE"};
    static const char *sjt[] = {"", "SJFSM", "SJPSM","SEJOIN","SJTID",
			"SJKEY", "SJHASH", "SJCARTPROD", "SJNOJOIN"};
    static const char *sojt[] = {"", "RIGHT","LEFT","FULL","INNER","FOLDED"};
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"{%s", co, sjpr[co->opo_sjpr]);
    if (co->opo_sjpr == DB_ORIG)
	SIfprintf(fd, "|opo_orig:%d", co->opo_union.opo_orig);
    else
	SIfprintf(fd, "|opo_ojid:%d", co->opo_union.opo_ojid);
    if (co->opo_maps)
    {
	OPO_COMAPS *p = co->opo_maps;
	char bm[OPV_BITMAPVARS*2+1];
	/* Strictly these should just be to sq->ops_eclass.ope_ev */
	fmt_bm(&p->opo_eqcmap, sizeof(p->opo_eqcmap), bm);
#ifndef OPM_FLATAGGON
	SIfprintf(fd, "|opo_eqcmap:\\n%s\\r", bm);
#else
	SIfprintf(fd, "|eqcmap/aggmap:\\n%s\\r", bm);
	fmt_bm(&p->opo_aggmap, sizeof(p->opo_aggmap), bm);
	SIfprintf(fd, "%s\\r", bm);
#endif
    }
    if (pst_ptr_ok(fd, co->opo_variant.opo_local))
    {
	OPD_LOCAL *l = co->opo_variant.opo_local;
	char bm[OPV_BITMAPVARS*2+1], *p;
	SIfprintf(fd, "|opo_id:%d\\nojid:%d\\n", l->opo_id, l->opo_ojid);
	if (*sjt[l->opo_jtype] || *sojt[l->opo_type])
	    SIfprintf(fd, "%s %s\\n", sjt[l->opo_jtype], sojt[l->opo_type]);
	if (l->opo_x100jt)
	    SIfprintf(fd, "%s\\n",l->opo_x100jt);
	fmt_bm(l->opo_bmbf, sizeof(*l->opo_bmbf), bm);
	SIfprintf(fd, "bf:%s\\r", bm);
	fmt_bm(l->opo_bmvars, sizeof(*l->opo_bmvars), bm);
	SIfprintf(fd, "vars:%s\\r", bm);
	fmt_bm(l->opo_x100bmvars, sizeof(*l->opo_x100bmvars), bm);
	SIfprintf(fd, "x100:%s\\r", bm);
	fmt_bm(l->opo_ojoin, sizeof(*l->opo_ojoin), bm);
	SIfprintf(fd, "ojoin:%s\\r", bm);
	fmt_bm(l->opo_special, sizeof(*l->opo_special), bm);
	SIfprintf(fd, "sp:%s\\r", bm);
	fmt_bm(l->opo_innereqc, sizeof(*l->opo_innereqc), bm);
	SIfprintf(fd, "inr:%s\\r", bm);
	p = bm;

	if (l->opo_mask&OPO_PCJ_NESTED) *p++ = 'P';
	if (l->opo_mask&OPO_NOORDER) *p++ = 'O';
	if (l->opo_mask&OPO_READBACK) *p++ = 'R';
	if (l->opo_mask&OPO_HASH_TRANSFORM) *p++ = 'H';
	if (l->opo_mask&OPO_ANTIJOIN) *p++ = 'A';
	if (l->opo_mask&OPO_SEMIJOIN) *p++ = 'S';
	if (l->opo_mask&OPO_REVJOIN) *p++ = 'R';
	if (l->opo_mask&OPO_SUBQJOIN) *p++ = 'Q';
	*p = EOS;
	if (co->opo_is_swapped)
	    strcat(p, " swapped");
	SIfprintf(fd, "%s\\l", bm);
    }
    SIfprintf(fd, "}|");
    if (co->opo_deleteflag)	SIfprintf(fd, "del\\r");
    if (co->opo_existence)	SIfprintf(fd, "exi\\r");
    if (co->opo_odups)		SIfprintf(fd, "ode\r");
    if (co->opo_idups)		SIfprintf(fd, "ide\\r");
    if (co->opo_psort)		SIfprintf(fd, "pst\\r");
    if (co->opo_osort)		SIfprintf(fd, "ost\\r");
    if (co->opo_isort)		SIfprintf(fd, "ist\\r");
    if (co->opo_pdups)		SIfprintf(fd, "pde\\r");
    if (co->opo_held)		SIfprintf(fd, "hld\\r");
    if (co->opo_punion)		SIfprintf(fd, "pUN\\r");
    if (co->opo_exchagg)	SIfprintf(fd, "xag\\r");
    if (co->opo_tprocjoin)	SIfprintf(fd, "TPj\\r");
    if (co->opo_refrel)		SIfprintf(fd, "rfr\\r");
    if (co->opo_1n)		SIfprintf(fd, "1n\\r");
    if (co->opo_n1)		SIfprintf(fd, "n1\\r");
    if (co->opo_nm)		SIfprintf(fd, "nm\\r");
    if (co->opo_joinix)		SIfprintf(fd, "jnX\\r");
    if (co->opo_bfs)		SIfprintf(fd, "bfs\\r");
    if (co->opo_refnojix)	SIfprintf(fd, "rjn\\r");
    if (co->opo_dupselim)	SIfprintf(fd, "Xde\\r");
    if (co->opo_x100merge)	SIfprintf(fd, "Xmg\\r");
    if (co->opo_x100nullj)	SIfprintf(fd, "Xnj\\r");
    if (co->opo_x100proj)	SIfprintf(fd, "XPr\\r");
    if (co->opo_card_outer)	SIfprintf(fd, "o%c\\r"," 1n*"[co->opo_card_outer]);
    if (co->opo_card_inner)	SIfprintf(fd, "i%c\\r"," 1n*"[co->opo_card_inner]);
    if (co->opo_card_own)	SIfprintf(fd, "p%c\\r"," 1n*"[co->opo_card_own]);
    if (co->opo_is_swapped)	SIfprintf(fd, "swp\r");
    if (co->opo_no_exch)	SIfprintf(fd, "nox\\r");
    SIfprintf(fd, "\"]\n");

    LINKBLOCK(co,opo_coforw,opt_co_dot1)
    LINKBLOCK(co,opo_coback,opt_co_dot1)
    LINKBLOCK(co,opo_outer,opt_co_dot1)
    LINKBLOCK(co,opo_inner,opt_co_dot1)
}
...
opt_bf_dot1(
	...
	OPB_BOOLFACT *bf)
{
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"{BOOLFACT"
		"|j%d"
		"|%x}\"]\n",
		bf,
		bf->opb_ojid,
		bf->opb_mask);
    LINKBLOCK(bf,opb_qnode,	pst_dot_qnode)
    LINKBLOCK(bf,opb_bfp,	opt_bf_dot1)
}
...
opt_sq_dot1(
	...
	OPS_SUBQUERY *sq)
{
    const char *sqtype[] = {"MAIN","SAGG","FAGG","PROJ","SELECT","UNION","VIEW,"
				"RFAGG","RSAGG","HFAGG","REUSE","X100FRAG"};
    i4 i;
    SIfprintf(fd, "n%p[style=solid,fillcolor=lightgrey,label=\"{subquery"
		"|%s|ops_bfs",
		sq,
		(u_i4)sq->ops_sqtype > OPS_X100FRAG
			?"bad":sqtype[sq->ops_sqtype]);
    for (i = 0; i < sq->ops_bfs.opb_bv; i++)
	SIfprintf(fd, "|<f%d> %d", i, i);
    if (sq->ops_bfs.opb_qfalse)
	SIfprintf(fd, "|Q FALSE");
    if (sq->ops_duplicates == OPS_DKEEP)
	SIfprintf(fd, "|keepdup");
    else if (sq->ops_duplicates == OPS_DREMOVE)
	SIfprintf(fd, "|elimdup");
    if (sq->ops_keep_dups || sq->ops_remove_dups)
    {
	char bm[4096];
	SIfprintf(fd, "|");
	if (sq->ops_keep_dups)
	{
	    fmt_bm(sq->ops_keep_dups, sizeof(*sq->ops_keep_dups), bm);
	    SIfprintf(fd, "kp:%s\\r",  bm);
	}
	if (sq->ops_remove_dups)
	{
	    fmt_bm(sq->ops_remove_dups, sizeof(*sq->ops_remove_dups), bm);
	    SIfprintf(fd, "rm:%s\\r",  bm);
	}
    }
 
    if (sq->ops_oj.opl_base)
    {
	SIfprintf(fd, "|opl_base");
	for (i = 0; i < sq->ops_oj.opl_lv; i++)
	    if (sq->ops_oj.opl_base->opl_ojt[i])
		SIfprintf(fd, "|<g%d> %d", i, i);
    }
    SIfprintf(fd, "}\"]\n");
    if (sq->ops_bfs.opb_base)
    {
	for (i = 0; i < sq->ops_bfs.opb_bv; i++)
	    if (sq->ops_bfs.opb_base->opb_boolfact[i] &&
		    pst_dot_chk(ctx, &sq->ops_bfs.opb_base->opb_boolfact[i],
					sq->ops_bfs.opb_base->opb_boolfact[i]))
		SIfprintf(fd,"n%p:f%d->n%p[weight=10]\n",
				sq,i,sq->ops_bfs.opb_base->opb_boolfact[i]);
	for (i = 0; i < sq->ops_bfs.opb_bv; i++)
	    if (sq->ops_bfs.opb_base->opb_boolfact[i])
		opt_bf_dot1(ctx,fd,sq->ops_bfs.opb_base->opb_boolfact[i]);
    }
    if (sq->ops_oj.opl_base)
    {
	for (i = 0; i < sq->ops_oj.opl_lv; i++)
	{
	    OPL_OUTER *o = sq->ops_oj.opl_base->opl_ojt[i];
	    if (o && pst_dot_chk(ctx,sq,o))
	    {
		char bm[4096];
		SIfprintf(fd, "n%p:(g%d,e)->n%p:n[weight=100]\n", sq, i, o);
		SIfprintf(fd, "n%p[style=solid,fillcolor=lightgrey,label=\"{OPL_OUTER|{oj%d|%s|0x%x}|",
				o, o->opl_id,
				o->opl_type==OPL_UNKNOWN?"unknown":
				o->opl_type==OPL_RIGHTJOIN?"right":
				o->opl_type==OPL_LEFTJOIN?"left":
				o->opl_type==OPL_FULLJOIN?"full":
				o->opl_type==OPL_INNERJOIN?"inner":
				o->opl_type==OPL_FOLDEDJOIN?"folded":"bad",
				o->opl_mask);
		fmt_bm(o->opl_innereqc, sizeof(*o->opl_innereqc), bm);
		SIfprintf(fd, "innereqc:%s\\r",  bm);
		fmt_bm(o->opl_ojtotal, sizeof(*o->opl_ojtotal), bm);
		SIfprintf(fd, "ojtotal:%s\\r", bm);
		fmt_bm(o->opl_onclause, sizeof(*o->opl_onclause), bm);
		SIfprintf(fd, "onclause:%s\\r", bm);
		fmt_bm(o->opl_ivmap, sizeof(*o->opl_ivmap), bm);
		SIfprintf(fd, "ivmap:%s\\r", bm);
		fmt_bm(o->opl_ovmap, sizeof(*o->opl_ovmap), bm);
		SIfprintf(fd, "ovmap:%s\\r", bm);
		fmt_bm(o->opl_bvmap, sizeof(*o->opl_bvmap), bm);
		SIfprintf(fd, "bvmap:%s\\r", bm);
		fmt_bm(o->opl_ojbvmap, sizeof(*o->opl_ojbvmap), bm);
		SIfprintf(fd, "ojbvmap:%s\\r", bm);
		fmt_bm(o->opl_p1, sizeof(*o->opl_p1), bm);
		SIfprintf(fd, "p1:%s\\r", bm);
		fmt_bm(o->opl_p2, sizeof(*o->opl_p2), bm);
		SIfprintf(fd, "p2:%s\\r", bm);
		fmt_bm(o->opl_agscope, sizeof(*o->opl_agscope), bm);
		SIfprintf(fd, "agscope:%s\\r", bm);
		fmt_bm(o->opl_bfmap, sizeof(*o->opl_bfmap), bm);
		SIfprintf(fd, "bfmap:%s\\r", bm);
		fmt_bm(o->opl_minojmap, sizeof(*o->opl_minojmap), bm);
		SIfprintf(fd, "minojmap:%s\\r", bm);
		fmt_bm(o->opl_maxojmap, sizeof(*o->opl_maxojmap), bm);
		SIfprintf(fd, "maxojmap:%s\\r", bm);
		fmt_bm(o->opl_ojattr, sizeof(*o->opl_ojattr), bm);
		SIfprintf(fd, "ojattr:%s\\r", bm);
		fmt_bm(o->opl_reqinner, sizeof(*o->opl_reqinner), bm);
		SIfprintf(fd, "reqinner:%s\\r", bm);
		fmt_bm(o->opl_rmaxojmap, sizeof(*o->opl_rmaxojmap), bm);
		SIfprintf(fd, "rmaxojmap:%s\\r", bm);
		SIfprintf(fd, "}\"]\n");
	    }
	}
    }
    if (sq->ops_attrs.opz_base)
    {
	for (i = 0; i < sq->ops_attrs.opz_av; i++)
	{
	    OPZ_ATTS *a = sq->ops_attrs.opz_base->opz_attnums[i];
	    if (a && pst_dot_chk(ctx, sq, a) && pst_ptr_ok(fd, a))
	    {
		char bm[4096];
		SIfprintf(fd, "n%p->n%p[color=blue,label=\"opz_attnums[%d]\"]\n",
			sq, a, i);
		
		SIfprintf(fd, "n%p[fillcolor=lightpink,label=\"{ATTS|{i%d|a%d|eqc%d|g%d}"
				"|{%d,%d,%d,%d,%d}|{0x%x}|", a,
				a->opz_varnm,
				a->opz_attnm,
				a->opz_equcls,
				a->opz_gvar,
				a->opz_dataval.db_datatype,
				a->opz_dataval.db_length,
				DB_P_DECODE_MACRO(a->opz_dataval.db_prec),
				DB_S_DECODE_MACRO(a->opz_dataval.db_prec),
				a->opz_dataval.db_collID,
				a->opz_mask);
		fmt_bm(&a->opz_outervars, sizeof(a->opz_outervars), bm);
		SIfprintf(fd, "ovars:%s\\r", bm);		
		fmt_bm(&a->opz_eqcmap, sizeof(a->opz_eqcmap), bm);
		SIfprintf(fd, "eqc:%s\\r", bm);		
		SIfprintf(fd, "}\"]\n");
	    }
	}
    }
    LINKBLOCK(sq,ops_root,	pst_dot_qnode)
    LINKBLOCK(sq,ops_next,	opt_sq_dot1)
    LINKBLOCK(sq,ops_union,	opt_sq_dot1)
    LINKBLOCK(sq,ops_byexpr,	pst_dot_qnode)
    LINKBLOCK(sq,ops_bygsets,	pst_dot_qnode)
    LINKBLOCK(sq,ops_bfs.opb_bfconstants,	pst_dot_qnode)
    LINKBLOCK(sq,ops_bestfragco,opt_co_dot1)
    LINKBLOCK(sq,ops_bestco,	opt_co_dot1)
    LINKBLOCK(sq,ops_onext,	opt_sq_dot1)
    LINKBLOCK(sq,ops_seqctest,	opt_bf_dot1)
}
...
opt_ahd_dot1(
	...
	QEF_AHD *ahd)
{
    LINKBLOCK(ahd,ahd_next,	opt_ahd_dot1)
    LINKBLOCK(ahd,ahd_prev,	opt_ahd_dot1)
    LINKBLOCK(ahd,ahd_list,	opt_ahd_dot1)

    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"{AHD|{%d}", ahd, ahd->ahd_atype);
    switch (ahd->ahd_atype)
    {
    case QEA_DMU:
    case QEA_APP:
...
opt_qp_dot1(
	...
	QEF_QP_CB *qp)
{
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"{QP|%.32s|0x%x}\"]\n", qp,
			qp->qp_id.db_cur_name, qp->qp_status);
    LINKBLOCK(qp,qp_ahd,	opt_ahd_dot1)
    LINKBLOCK(qp,qp_fetch_ahd,	opt_ahd_dot1)
}
static void
opt_separm_dot1(
	PST_DOT_CTX *ctx,
	...
	OPV_SEPARM *parm)
{
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"separm\"]\n", parm);
    LINKBLOCK(parm,opv_senext,	opt_separm_dot1)
    LINKBLOCK(parm,opv_sevar,	pst_dot_qnode)
    LINKBLOCK(parm,opv_seconst,	pst_dot_qnode)
}
static void
opt_gss_dot1(
	PST_DOT_CTX *ctx,
	...
	OPV_GSUBSELECT *gss)
{
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"gsubselect\"]\n", gss);
    LINKBLOCK(gss,opv_subquery,	opt_sq_dot1)
    LINKBLOCK(gss,opv_parm,	opt_separm_dot1)
    LINKBLOCK(gss,opv_ahd,	opt_ahd_dot1)
}
static void
opt_dot1(
	PST_DOT_CTX *ctx,
	...
	OPS_STATE *global)
{
    ...
    OPC_STATE *st = &global->ops_cstate;
    i4 i;
    char b1[sizeof(global->ops_rangetab.opv_mrdf)*2+1];
    char b2[sizeof(global->ops_rangetab.opv_msubselect)*2+1];
    fmt_bm(&global->ops_rangetab.opv_mrdf,
			sizeof(global->ops_rangetab.opv_mrdf),b1);
    fmt_bm(&global->ops_rangetab.opv_msubselect,
			sizeof(global->ops_rangetab.opv_msubselect),b2);
    SIfprintf(fd, "n%p[style=solid,fillcolor=lightgrey,label=\"{GLOBAL|{ops_rangetab|%d}"
			"|mrdf/msub:\\n%s\\r%s\\r",
			global,
			global->ops_rangetab.opv_gv,
			b1, b2);
    if (!global->ops_rangetab.opv_base)
	SIfprintf(fd, "NULL");
    else
    for (i = 0; i < global->ops_rangetab.opv_gv; i++)
    {
	OPV_GRV *g = global->ops_rangetab.opv_base->opv_grv[i];
	if (g)
	{
	    SIfprintf(fd, "|{<f%d> %d|{%s|qrt:%d|sam:%d|cmp:%d|0x%x}}", i, i,
			(u_i4)g->opv_created > OPS_X100FRAG
				?"bad":sqtype[g->opv_created],
			g->opv_qrt,
			g->opv_same,
			g->opv_compare,
			g->opv_gmask);
    ...
    {	char bm[sizeof(OPL_BMOJ)*2+1];
	SIfprintf(fd, "|{glv:%d|0x%x}|", global->ops_goj.opl_glv,
					global->ops_goj.opl_mask);
	fmt_bm(global->ops_goj.opl_antijmap, sizeof(OPL_BMOJ), bm);
	SIfprintf(fd, "anti:%s\\r", bm);
	fmt_bm(global->ops_goj.opl_semijmap, sizeof(OPL_BMOJ), bm);
	SIfprintf(fd, "semi:%s\\r", bm);
	fmt_bm(global->ops_goj.opl_subqjmap, sizeof(OPL_BMOJ), bm);
	SIfprintf(fd, "subq:%s\\r", bm);
    }
    SIfprintf(fd, "}\"]\n");
    if (global->ops_goj.opl_gbase)
    {
	fmt_i2_ar(fd, "opl_gbase", global->ops_goj.opl_gbase->opl_gojt,
				global->ops_goj.opl_glv, global);
    }
    if (global->ops_goj.opl_view)
    {
	fmt_i2_ar(fd, "opl_view", global->ops_goj.opl_view->opl_gojt,
				OPL_MAXOUTER, global);
    }
    if (global->ops_goj.opl_fjview)
    {
	fmt_i2_ar(fd, "opl_fjview", global->ops_goj.opl_fjview->opl_gojt,
				OPL_MAXOUTER, global);
    }
    LINKBLOCK(global,ops_qheader,	pst_qtree_dot1)
    LINKBLOCK(global,ops_copiedco,	opt_co_dot1)
    LINKBLOCK(global,ops_statement,	pst_dot_stmt)
    LINKBLOCK(global,ops_trigger,	pst_dot_stmt)
    LINKBLOCK(global,ops_firstAfterRule,pst_dot_stmt)
    LINKBLOCK(global,ops_firstBeforeRule,pst_dot_stmt)
    LINKBLOCK(global,ops_procedure,	pst_proc_dot1)
    LINKBLOCK(global,ops_subquery,	opt_sq_dot1)
    LINKBLOCK(global,ops_osubquery,	opt_sq_dot1)
	...
	if (g)
	{
	    LINKBLOCKF(g, opv_gquery, opt_sq_dot1, global, i)
	    LINKBLOCKF(g, opv_gsubselect, opt_gss_dot1, global, i)
	}
    }
    
    SIfprintf(fd, "n%p->n%p[label=ops_cstate]\n", global, st);
    SIfprintf(fd, "n%p[fillcolor=lightgrey,label=\"{OPC_STATE}\"]\n", st);
    LINKBLOCK(st,opc_qp,		opt_qp_dot1)
    LINKBLOCK(st,opc_firstahd,		opt_ahd_dot1)
    LINKBLOCK(st,opc_topahd,		opt_ahd_dot1)
    LINKBLOCK(st,opc_retrow_rsd,	pst_dot_qnode)
    LINKBLOCK(st,opc_topdecvar,		pst_dot_stmt)
    LINKBLOCK(st,opc_subqry,		opt_sq_dot1)
}
	...
	OPS_STATE *global)
{
    ...
    args[0] = global;
    args[1] = NULL;
    pst_dot(fname, (PST_DOT_RTN)opt_dot1, args);
}
	...
	OPS_SUBQUERY *sq)
{
    ...
    args[0] = sq;
    args[1] = NULL;
    pst_dot(fname, (PST_DOT_RTN)opt_sq_dot1, args);
}
	...
	OPO_CO *co)
{
    ...
    args[0] = co;
    args[1] = NULL;
    pst_dot(fname, (PST_DOT_RTN)opt_co_dot1, args);
}
