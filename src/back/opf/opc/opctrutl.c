...
**	    Pick up real att name length if doing a real att.
*/
static VOID
opcu_prconst(
	ULD_TTCB        *ttcbptr,
...
** Inputs:
**      ttcbptr                         text generation state variable
**      qnode                           PST_CASEOP parse tree node
**
** Outputs:
**	Returns:
**	    VOID
	...
	bool		   base_colname )
{
    ...
    ** NULL to the datatype indicated in the corresponding RESDOM. */
	...
	&& dvp->db_data && (dvp->db_data[dvp->db_length-1] & ADF_NVL_BIT))
