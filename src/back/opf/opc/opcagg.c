	...
	PST_QNODE   *subexlistp)
{
	...
	** aopparm is NULL and no code is generated. */
	if (subqry->ops_sqtype == OPS_HFAGG && aopparm)
	{
		...
		ops[0].opr_collID = aopparm2->pst_sym.pst_dataval.db_collID;

		ops[1].opr_dt = aopparm2->pst_sym.pst_dataval.db_datatype;
		ops[1].opr_len = OPC_UNKNOWN_LEN;
		ops[1].opr_prec = OPC_UNKNOWN_PREC;
		ops[1].opr_collID = aopparm2->pst_sym.pst_dataval.db_collID;
	...
	** aggregate operation. */
	if (aopno == ADI_MAX_OP || aopno == ADI_MIN_OP || aopno == ADI_SUM_OP)
	{
	    ...
	    ** underlying data value intact for max/min/sum semantics. */
	    ...
	    ops[0].opr_collID = tginfo->opc_tsoffsets[rsno].opc_tcollID;
	    ...
	    null_minmaxdv.db_collID = ops[0].opr_collID;
		...
		opc_adempty(global, &null_minmaxdv,
	...
	else
	{
		...
		ops[1].opr_collID = aop_resdom->pst_sym.pst_dataval.db_collID;
		...
		opc_adempty(global, &otherdv,
			ops[1].opr_dt, ops[1].opr_prec,
			ops[1].opr_len, ops[1].opr_collID);
	...
	** (if OPS_HFAGG) run hash function. */
	    ...
	    )
	{
	    ...
	    */
	    ...
	    ops[0].opr_collID = (i4) tginfo->opc_tsoffsets[byvalno].opc_tcollID;
	    ...
	    ** and explicitly MECOPY it to buffer. */
		...
		ops[1].opr_collID = sortinfo->opc_tsoffsets[byvalno].opc_tcollID;
		...
		** fi opid to apply "trim" during projection to buffer. */
		...
		ops[1].opr_len = OPC_UNKNOWN_LEN;
                ops[1].opr_prec = OPC_UNKNOWN_PREC;
		ops[1].opr_collID = byval->pst_sym.pst_dataval.db_collID;
	...
	PST_QNODE   *subexlistp)
{
	...
	if (nadfops > 1)
	{
		...
		ops[1].opr_len = OPC_UNKNOWN_LEN;
                ops[1].opr_prec = OPC_UNKNOWN_PREC;
		ops[1].opr_collID = aop_resdom->pst_right->pst_left->
		...
		if (nadfops > 2 && binagg)
		{
		    ...
		    ops[2].opr_collID = aop_resdom->pst_right->pst_right->
					pst_sym.pst_dataval.db_collID;
		...
		opov.opr_collID = aopparm->pst_sym.pst_dataval.db_collID;
	    }
	    ...
	    ** in overflow buffer. */
		...
		opov2.opr_collID = aopparm2->pst_sym.pst_dataval.db_collID;
	    }
	}
	    ...
	    !(aop->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags & ADI_F4_WORKSPACE)) 
	{
	    ...
	    ops[0].opr_collID = aop_resdom->pst_sym.pst_dataval.db_collID;
	    agwslen = 0;
	...
	i4	    outbase)
{
	    ...
	    )
	{
	    ...
	    */
	    ...
	    ops[1].opr_collID = (i4) tginfo->opc_tsoffsets[byvalno].opc_tcollID;
	...
	else
	{
	    ...
	    ops[0].opr_collID = (i4) tginfo->opc_tsoffsets[rsno].opc_tcollID;
	}
	...
	** ADE_OLAGEND opcode with aopno as extra parm. */
	...
	else
	{
		...
		ops[3].opr_len = sizeof(i4);
		ops[3].opr_prec = 0;
		ops[3].opr_collID = DB_UNSET_COLL;
		nops = 4;
	    }
	}
		...
		|| opno == ADE_AVG))
	{
		...
		if (f8tof4)
		{
		    ...
		    */
		    ...
		    saveop.opr_prec = 0;
		    saveop.opr_collID = DB_UNSET_COLL;
	...
	i4		proj_src_base )
{
    ...
    */
	...
	ops[0].opr_collID = ops[1].opr_collID = DB_UNSET_COLL;
    }
    else
    {
