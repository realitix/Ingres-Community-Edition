		...
		i4		noutops)
{
	...
	*/
	if (nops < exp_ops)
	{
	    ...
	    ops[nops].opr_dt = 0;
	    ops[nops].opr_prec = 0;
	    ops[nops].opr_collID = DB_UNSET_COLL;
...
**	    Drop adi_fidesc() call and dt_family processing & use extended
**	    adi_ficoerce_full() that addresses this in one place.
*/
VOID
opc_adtransform(
		OPS_STATE	*global,
		...
		bool		pqmat_retry_char)
{
	...
	else resop->opr_collID = DB_UNSET_COLL;
    }
    ...
    */
    if ((ret = adi_ficoerce_full(global->ops_adfcb, srcop->opr_dt,
	    resop->opr_dt, &fid, &fdesc)) != E_DB_OK
	)
    {
	/*
	...
	*/
	...
	{
		ret=adi_ficoerce_full(global->ops_adfcb, DB_LTXT_TYPE,
			resop->opr_dt, &fid, &fdesc);
		if (ret == E_DB_OK)
		{
		...
		DB_DATA_VALUE	*rdv)
{
    ...
    */
	...
	status = adi_ficoerce_full(global->ops_adfcb, odv->db_datatype,
	    best_fidesc->adi_dt[0], &adi_fid, &fi_desc);
	if (status != E_DB_OK)
	{
	    /* FIXME */
    ...
    {
	status = adi_ficoerce_full(global->ops_adfcb, idv->db_datatype,
	    best_fidesc->adi_dt[1], &adi_fid, &fi_desc);
	if (status != E_DB_OK)
	{
	    /* FIXME */
	    opx_error( E_OP0783_ADI_FICOERCE );
