/*
** Copyright (c) 1986-2003, 2010 Actian Corporation
*/
#include    <compat.h>
#include    <gl.h>
#include    <iicommon.h>
...
**	19-feb-2010 (dougi)
**	    Replaced nasty MEreqmem/free's with opu_memory().
...
**	    Provide the means of supporting boolean paramaters in functions.
...
**	    Add collID to opc_adempty to correct defaults of related constants
**	    to ensure correct length checks applied where UCS_BASIC is used.
**/
...
static i4 opc_cqual1(
	...
	ADE_OPERAND *resop,
	ADI_FI_ID rescnvtid,
#define QUAL_FL_INCASE 1
#define QUAL_FL_NEED_BOOL_DV 2
	u_i4 flags);
		...
		i4	    *szconst)
{
	...
	case PST_MOP:
	{
		...
		*nops += 1;
		...
		resqnode = resqnode->pst_right;
		if (resqnode)
		    lqnode = resqnode->pst_left;
		else
		    break;
		i += 1;
	    }
...
**	    the result to be. The opr_offset field is init'ed to OPR_NO_OFFSET
**	    here to indicate to opc_cqual1 that a result offset is NOT being
**	    sent in. The remaining fields are uninitialized and are for output.
**	tidatt -
		...
		i4	    *tidatt)
{
    ...
    ** on to opc_cqual1. */
    ...
    if (resop != (ADE_OPERAND *)NULL) resop->opr_offset = OPR_NO_OFFSET;
						/* init result offset */
    result = opc_cqual1(global, cnode, root, cadf, segment, resop, (ADI_FI_ID)0, 0);
...
**	rescnvtid -
**	    The opid that the parser expects will be required to convert result
**	    or ADI_NILCOERCE if directly compatible.
**	flags -
**		QUAL_FL_INCASE if set indicates that the invocation is in a CASEOP
**			build and constant testing then is disabled forcing ops into
**			main segment (NOTE: Seems odd to do this as it should be
**			invariant. However, probably not important as constant
**			folding in the parser will likely have elided such by now.) 
**		QUAL_FL_NEED_BOOL_DV if set indicates that the calling context
**			needs a data value as opposed to a 'logical' state setting.
**			This arises as conditionals and logicals set CX status
**			flags and don't pass on a nice boolean result. If set, the
**			comparison operators for example, will ensure that a value
**			is returned as expected. With this in place, parse trees
**			can now handle such as (1=2)=(3=4) and fn(x AND y) with
**			bools passed on.
**
** Outputs:
**	cadf -
...
**	09-Dec-2010 (kiria01) SIR 124826
**	    Added ADE_GETSTATUS to lift CX status value back into bool DV for
**	    param input. Essentially if we have a resop to load and the calling
**	    level of opc_cqual1() was not in an logical operation like PST_AND, PST_OR
**	    etc, then we will be passed the QUAL_FL_NEED_BOOL_DV flag indicating
**	    that comparison operators and logical operators should generate
**	    an ADE_GETSTATUS afterwards to materialise the status as a boolean.
**	18-May-2011 (kiria01) SIR 124826/m1949
**	    Correct the length calc where QUAL_FL_NEED_BOOL_DV is set and
**	    we are presented with the correct length already and nullable.
*/
		...
		u_i4	    flags)
{
		...
		whlistp = whlistp->pst_left)
	{
		...
		** is made between SEARCHED and SIMPLE cases. */
				...
				PST_SEARCHED_CASE)
		{
			...
			cadf, segment, (ADE_OPERAND *)NULL, (ADI_FI_ID)0, QUAL_FL_INCASE);
	    ...
	    ** OR true labels encountered in the when expression. */
	    ...
	    (VOID) opc_cqual1(global, cnode, thenp, cadf, segment, resop,
			(ADI_FI_ID)0, QUAL_FL_INCASE|QUAL_FL_NEED_BOOL_DV);
	...
	** However, if it is a comparison, it may have been requested
	** to produce a result if flags has QUAL_FL_NEED_BOOL_DV set.
	** In this case we will add the ADE_GETSTATUS operator to
	** materialise the status flags.
	*/
	...
	if (root->pst_sym.pst_type == PST_MOP)
	{
            resqnode = root;
            lqnode = resqnode->pst_left;
	    cnvrtid = resqnode->pst_sym.pst_value.pst_s_op.pst_oplcnvrtid;
	}
	else
	{
		...
		root->pst_right->pst_sym.pst_type == PST_CONST)
	{
		...
		inlist = root->pst_right;
		Pops = (ADE_OPERAND *) opu_memory(global, 
						sizeof(ADE_OPERAND)*cnt);
		...
		** LHS as the parser will not generate PST_INLIST in that case. */
		...
		(VOID) opc_cqual1(global, cnode, lqnode,
                        cadf, segment, &Pops[0], cnvrtid, QUAL_FL_NEED_BOOL_DV);
		for (i = 1; i < cnt; i++)
		{
		...
		if (resop && (flags & QUAL_FL_NEED_BOOL_DV))
		{
		    ...
		    resop->opr_collID = root->pst_sym.pst_dataval.db_collID;
		    if (align = ops[0].opr_len % sizeof (ALIGN_RESTRICT))
			align = sizeof (ALIGN_RESTRICT) - align;
		    global->ops_cstate.opc_temprow->opc_olen += ops[0].opr_len + align;
		    opc_adinstr(global, cadf, ADE_GETSTATUS, segment, 1, resop, 1);
		}
	...
	while (lqnode)
	{
	    ...
	    */
		...
		else ops[opnum].opr_collID = DB_UNSET_COLL;
            }
            else
            {
	    ...
	    ** for a binary operation (major kludge). */
	    ...
	    lret = opc_cqual1(global, cnode, lqnode,
			cadf, segment, &ops[opnum], cnvrtid, QUAL_FL_NEED_BOOL_DV);
	    global->ops_gmask &= (~OPS_DECCOERCION);
	    ...
	    *opsptr = &ops[opnum];
		...
		resqnode = resqnode->pst_right;
        	if (!resqnode)
                    break;
                lqnode = resqnode->pst_left;   /* point to the operand */
	...
	if (comparison)
	{
	    ...
	    ** Unless flags & QUAL_FL_NEED_BOOL_DV! In which case we leave
	    ** resop intact & fix up shortly
	    */
	    if (flags & QUAL_FL_NEED_BOOL_DV)
	    {
		resop->opr_dt = root->pst_sym.pst_dataval.db_datatype;
		resop->opr_len = root->pst_sym.pst_dataval.db_length;
		if (resop->opr_len <= 0)
	...
	if (inlist != (PST_QNODE *) NULL)
	{
		...
		** for each remaining PST_CONST in the constant list. */
		opc_adinstr(global, cadf, opcode, segment, 1, 
        	...
        	ops[1].opr_collID = inlist->pst_sym.pst_dataval.db_collID;
		...
		** for a binary operation (major kludge). */
		...
		lret = opc_cqual1(global, cnode, inlist,
			cadf, segment, &ops[1], (ADI_FI_ID)0, QUAL_FL_NEED_BOOL_DV);
	...
	*/
	...
	if (attno != OPC_NOATT)
	{
		...
		(VOID) opc_cqual1(global, cnode, func_tree, cadf, segment,
			    resop, (ADI_FI_ID)0, QUAL_FL_NEED_BOOL_DV);
	    }
	    else
	    {
...

{
    ...
    escape_dv.db_prec = 0;
    escape_dv.db_collID = DB_UNSET_COLL;
	...
	ADE_OPERAND *resop)
{
	...
	*/
	...
	rdi->opc_srcop.opr_len = OPC_UNKNOWN_LEN;
        ...
        rdi->opc_srcop.opr_collID = DB_UNSET_COLL;
	...
	(VOID) opc_cqual1(global, cnode, resdom->pst_right, 
		cadf, segment, &rdi->opc_srcop, (ADI_FI_ID)0, QUAL_FL_NEED_BOOL_DV);
		...
		QEN_ADF	    **qadf)
{
	...
	** and replace in original expressions by PST_VARs. */
	...
	if (subexlistp != (PST_QNODE *) NULL)
	{
	    ...
	    resop.opr_collID = DB_UNSET_COLL;
...

/*
** Name: opc_x100_target -- Compile resdom list move for X100 mixed query
**
** Description:
**	When we have an X100 action for a result-returning query such as
**	a scrollable cursor select, the GET action needs an ahd_current
**	CX (since QEF expects one, and it's a bit too complicated to
**	spray if-tests around in qeafetch.c to do otherwise.)  However,
**	just calling opc-target fails, because a) the entire result
**	expression list is already compiled into the X100 query, and
**	b) none of the eqclass availability stuff is set up.  What
**	we really want to do is simply move the staged X100 result row
**	into the query output.  So, this routine generates a CX to do
**	just that.
**
**	We don't need to worry about sequences, defaults, blobs, or any
**	of that stuff, because they aren't supported for Vectorwise,
**	and we know this is not an APPEND.
**
** Inputs:
**	global		- Opf session state control block
**	cnode		- Current top CO/QEN node info
**	resdom		- Pointer to first (ie rightmost) resdom
**			  (actually, not used, drop this later?)
**	qadf		- Where to put pointer to new QEN_ADF struct
**
** Outputs:
**	qadf		filled in with new CX info block
**
** No return value, aborts with opx-error if problems.
**
** History:
**	2-May-2011 (kschendel) m1920
**	    Written for scrollable Vectorwise cursors.
*/
	...
	QEN_ADF	    **qadf)
{
    ...
    opc_adalloc_begin(global, &cadf, qadf, 1, 2, 1, 0,
	cnode->opc_below_rows + 2, OPC_STACK_MEM);

    /* Assume that we're always sending to an output row, for now. */
    opc_adbase(global, &cadf, QEN_OUT, 0);

    /* Take the row size from the top node */

    rowsz = cnode->opc_qennode->qen_rsz;

    /* Generate move to output row */
    ops[0].opr_base = cadf.opc_qeadf->qen_uoutput + ADE_ZBASE;
    ops[0].opr_offset = 0;
    ops[0].opr_dt = DB_BYTE_TYPE;	/* Irrelevant, use byte */
    ops[0].opr_prec = 0;
    ops[0].opr_len = rowsz;
    ops[0].opr_collID = 0;
    opc_adbase(global, &cadf, QEN_ROW, cnode->opc_qennode->qen_row);
    ops[1].opr_base = cadf.opc_row_base[cnode->opc_qennode->qen_row];
    ops[1].opr_offset = 0;
    ops[1].opr_dt = DB_BYTE_TYPE;	/* Irrelevant, use byte */
    ops[1].opr_prec = 0;
    ops[1].opr_len = rowsz;
    ops[1].opr_collID = 0;
		...
		OPC_ADF	    *cadf)
{
	...
	*/
	...
	opc_adempty(global, &dv,
			ops[0].opr_dt, ops[0].opr_prec,
			ops[0].opr_len, ops[0].opr_collID);
...
**      22-Jun-2011 (coomi01) b125433
**          Though date types may have same length, they might not
**          have the same precision. This matters when going high
**          to low, but not vice-versa.
**	15-Jul-2011 (kiria01) b125530
**	    Don't coerce just to change from non-nullable to nullable.
**	    Instead, use ADE_MECOPY & pre-instruction
**	25-Jul-2011 (kiria01) b125530
**	    Include test to equality of length as these coercions should not
**	    be skipped.
*/
VOID
opc_rdmove(
		...
		bool		leavebase)
{
    ...
    */
	...
	(ops[0].opr_dt != -ops[1].opr_dt || ops[0].opr_dt > 0)  ||
	/* If still checking, have identical dt, or Tnullable = Tnot-nullable.
	** Check lengths, but don't coerce a long type to change length
	** from unknown to fixed. */
	(((bits & AD_PERIPHERAL) == 0)
		&& ( ((ops[0].opr_dt > 0 || ops[0].opr_dt == ops[1].opr_dt)
			 ? ops[0].opr_len : ops[0].opr_len-1) != ops[1].opr_len))  || 
	/*
	...
	*/
	...
	/* For timestamp types must be careful going high to 
	** low precision. Coercing will expunge the trailing 
	** digits
	*/
	( ops[0].opr_prec < ops[1].opr_prec)
	  && ( (abs(ops[0].opr_dt) == DB_TSWO_TYPE)  ||
	       (abs(ops[0].opr_dt) == DB_TMWO_TYPE)  ||
	       (abs(ops[0].opr_dt) == DB_TMW_TYPE)   ||
	       (abs(ops[0].opr_dt) == DB_TME_TYPE)   ||
	       (abs(ops[0].opr_dt) == DB_TSW_TYPE)   ||
	       (abs(ops[0].opr_dt) == DB_TSTMP_TYPE) ||
	       (abs(ops[0].opr_dt) == DB_INDS_TYPE)
	      )	  
	)
    {
    ...
    ** else, the corresponding default value). */
	...
	if (passed_params_left == 0)
	{
	    ...
	    ** Otherwise, compile in the default/null value. */
	     ...
	     *((u_char *)dv.db_data + dv.db_length-1) = ADF_NVL_BIT;
	...
	if (inlist != (PST_QNODE *) NULL)
	{
		...
		** for each remaining PST_CONST in the constant list. */
		opc_adinstr(global, cadf, opcode, ADE_SMAIN, 1, 
        	...
        	ops[1].opr_len = OPC_UNKNOWN_LEN;
        	ops[1].opr_prec = OPC_UNKNOWN_PREC;
        	ops[1].opr_collID = DB_UNSET_COLL;
		ops[1].opr_offset = OPR_NO_OFFSET;	/* show no result loc. */
		/* Trick it into using reasonable scale if coercing to DECIMAL
		** for a binary operation (major kludge). */
		if ((abs(root->pst_left->pst_sym.pst_dataval.db_datatype) ==
...
**	9-sept-2008 (dougi)
**	    Slight change for X100 (cnode may be NULL).
...

{
    ...
    ** expression optimization. */
    ...
    if (subex_count && (subquery->ops_global->ops_gmask & OPS_X100))
    {
	/* Need to set new RESDOM names into corresponding VAR nodes. */
	opc_exprname(*subexlistp, *rootp);
	opc_exprname(*subexlistp, *subexlistp);	/* search common exprs, too */
    }
...
# endif
	opt_pt_entry(subquery->ops_global, NULL, *subexlistp,
		"extracted subexpressions", FALSE, TRUE, FALSE);
# ifdef OPT_F086_DUMP_QTREE2
	if (subquery->ops_global->ops_cb->ops_check &&
	   opt_strace(subquery->ops_global->ops_cb, OPT_F086_DUMP_QTREE2))
	{	/* op214 - dump parse tree in op146 style */
...
# endif
	opt_pt_entry(subquery->ops_global, NULL, *rootp,
		"transformed tree fragment", FALSE, TRUE, FALSE);
...
**	13-apr-2010 (dougi)
**	    Fine tune to recognize subexpressions under the same RESDOM.
...
**	14-june-2011 (dougi) m2054
**	    Don't optimize window functions - it confuses the cross compiler.
*/
...

{
    ...
    */
    ...
    if (nodep == NULL)
	return(FALSE);		/* can happen with transformed window funcs */
	...
	if (nodep->pst_sym.pst_value.pst_s_op.pst_flags & PST_WINDOW_OP)
	    return(FALSE);	/* don't optimize window funcs */

	/* Recurse down the fragment and analyze upon return. */
	result = opc_exprsrch(subquery, cnode, subexpp, &nodep->pst_left, 
		subexlistp, subex_countp, gotone);

	if (nodep->pst_right != (PST_QNODE *) NULL && 
		!opc_exprsrch(subquery, cnode, subexpp, &nodep->pst_right, 
			subexlistp, subex_countp, gotone))
	    return(FALSE);
	else if (!result)
	    return(FALSE);
	...
	** (random(), uuid()). */
	if (nodep->pst_sym.pst_value.pst_s_op.pst_fdesc &&
	    (nodep->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags & 
							ADI_F32_NOTVIRGIN))
	    return(FALSE);

	/* Don't optimize for Unorm operation */
	if (opno == ADI_UNORM_OP || opno == ADI_VWDIFF_OP ||
		opno == ADI_VWREDIFF_OP || opno == ADI_VWROWNUM_OP)
	    return(FALSE);
...

/*{
** Name: OPC_EXPRNAME	- searches for "introduced" PST_VAR nodes and
**	copies name from corresponding RESDOM nodes.
**
** Description:
**
** Inputs:
**	subexlistp	- ptr to RESDOM list of subexpressions
**	nodep		- ptr to subtree to be searched
**
** Outputs:
**
**	Returns:
...

{
    ...
    /* Simply recurse down subtree looking for PST_VAR nodes with
    ** pst_vno == -1. These are the replacements for the subexpressions. */

    if (nodep == (PST_QNODE *) NULL)
	return;

    if (nodep->pst_sym.pst_type == PST_VAR && 
		nodep->pst_sym.pst_value.pst_s_var.pst_vno == -1)
    {
	/* Got one - search subex list for matching RESDOM. */
	for (workp = subexlistp; workp; workp = workp->pst_left)
	 if (workp->pst_sym.pst_type == PST_RESDOM &&
		workp->pst_sym.pst_value.pst_s_rsdm.pst_ntargno ==
		    nodep->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id)
	 {
	    /* Copy fabricated name and return. */
	    MEcopy((char *)&workp->pst_sym.pst_value.pst_s_rsdm.
    ...
    /* Otherwise, recurse. */
    if (nodep->pst_right != (PST_QNODE *) NULL)
	opc_exprname(subexlistp, nodep->pst_right);
    if (nodep->pst_left != (PST_QNODE *) NULL)
	opc_exprname(subexlistp, nodep->pst_left);
