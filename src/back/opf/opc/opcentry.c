...
**          Removed opc_checkcons - this is now done in the parser
...
**          When creating shared objects we need to establish the 
**          master's alias using the feobj_id. Otherwise we hit E_QS000A
**          when other sessions try to access the shared object.
*/
		...
		OPS_STATE          *global)
{
	...
	if (global->ops_procedure->pst_flags & PST_REPEAT_DYNAMIC)
	{
	    ...
	    I4ASSIGN_MACRO(global->ops_caller_cb->opf_udbid, *(i4 *) p); 
            global->ops_qsfcb.qsf_feobj_id.qso_lname = 0;
	}
	   ...
	   )
	{
	    ...
	    MEcopy((PTR)&global->ops_procedure->pst_dbpid, 
		   ...
		   (PTR)global->ops_qsfcb.qsf_obj_id.qso_name);
            ...
            /* Set qsf_feobj_id.qso_lname and if non-zero setup 
            ** qsf_feobj_id.qso_name
            */
            if(global->ops_qsfcb.qsf_feobj_id.qso_lname = global->ops_procedure->pst_fe_lname)
            {
                MEcopy((PTR)&global->ops_procedure->pst_dbpid,
                   sizeof (global->ops_procedure->pst_fe_dbpid),
                   (PTR)global->ops_qsfcb.qsf_feobj_id.qso_name);

                global->ops_qsfcb.qsf_feobj_id.qso_type = QSO_ALIAS_OBJ;
            }    
	}
	else
	{
