...
**	    Add double-check for real range var when doing deferred check.
*/
VOID
opc_iqp_init(
		OPS_STATE          *global)
{
    ...
    ** statement level rules being triggered. */
	    ...
	    ** loop. */
		    ...
		    tree->pst_rngvar_count > 1)
		{
			...
			if (tree->pst_rangetab[i] != NULL &&
			  tree->pst_rangetab[i]->pst_rgtype == PST_TABLE &&
	    ...
	    )
	{
	    ...
	    if (!(qn->pst_sym.pst_value.pst_s_rsdm.pst_rsflags & PST_RS_PRINT))
		continue;		/* skip non-print's */
...
**	13-may-2008 (dougi)
**	    First crack at X100 support.
**	6-may-2009 (dougi)
**	    X100 DDL support.
**	7-jan-2010 (dougi)
**	    Move SQLDA build here for X100.
**	18-mar-2010 (gupsh01) SIR 123444
**	    Added support for alter table ...rename table/column.
**	5-june-2010 (dougi)
**	    More logic and a bit of reorg. to handle CTAS and INSERT ... SELECT
**	    from native Ingres to IVW.
**	21-Jun-2010 (kschendel) b123775
**	    Delete unused statement no.
**	30-sep-2010 (dougi)
**	    Move X100 logic for PST_QT_TYPE to opc_ahd_build().
*/
VOID
opc_cqp_continue(
		OPS_STATE          *global)
{
    QEF_AHD	    *new_ahd, *ct_ahd = (QEF_AHD *) NULL;
    OPS_SUBQUERY    *sq;
    ...
    */
	...
	break;  	
     ...
     case PST_X100_CREATE_TABLE_TYPE:
	...
	break;
     ...
     case PST_X100_CREATE_INTEGRITY_TYPE:
	opc_allocateIDrow( global, &dummyRow );
	...
	break;
     ...
     case PST_X100_DROP_INTEGRITY_TYPE:
...
**	30-sep-2010 (dougi)
**	    Generalized some X100 logic.
**	21-Mar-2011 (kschendel) m1772
**	    Ensure that names get generated into an insert-into-VW plan sqlda,
**	    even if normally they wouldn't (repeated).
**	28-Apr-2011 (kschendel) m1920
**	    Generate sqlda normally for scrollable cursor query against VW.
**      21-jun-2011 (huazh01)
**          ensure all QEF_AHD actions for rule list's are linked together.
**          Some statements (for example PST_CP_TYPE without parameters) have 
**          NULL QEF_AHD, causing actions list returned by opc_resolveif()
**          not linked properly. (b125282)
*/
VOID
opc_fqp_finish(
		OPS_STATE          *global)
{
    ...
    i4		    row_size;
    DB_STATUS	    status;
    PST_QTREE	    *qheader = global->ops_qheader;
    QEF_AHD         *last_ahd = NULL;
    PST_STATEMENT   *cur_stmt; 
    QEF_AHD         *cur_ahd, *rule_ahd = NULL;
    ...
    */
	...
	if (global->ops_inAfterRules)
		...
		(PST_STATEMENT *)NULL, &last_ahd);
            cur_stmt = global->ops_firstAfterRule; 
        }
	else 
        {
            opc_resolveif(global, 0, global->ops_firstBeforeRule, 
		(PST_STATEMENT *)NULL, &last_ahd);
            cur_stmt = global->ops_firstBeforeRule; 
        }

	while (cur_stmt)
	{
           cur_ahd = ((OPC_PST_STATEMENT *) cur_stmt->pst_opf)->opc_stmtahd; 

	   if (cur_ahd)
	   {
              if (rule_ahd)
              {
		 rule_ahd->ahd_prev->ahd_next = cur_ahd; 
		 cur_ahd->ahd_prev = rule_ahd->ahd_prev; 
		 rule_ahd->ahd_prev = cur_ahd; 
	      }
	      else
	      {
                rule_ahd = cur_ahd; 
	...
	*/
	if (global->ops_inAfterRules)
	    global->ops_cstate.opc_rule_ahd->qhd_obj.qhd_qep.ahd_after_act = rule_ahd; 
	else global->ops_cstate.opc_rule_ahd->qhd_obj.qhd_qep.ahd_before_act = rule_ahd; 
    }
    else
    {
	if (   global->ops_cb->ops_smask & OPS_MDISTRIBUTED &&
	...
	** OPENs. */
	if ((qp->qp_status & (QEQP_REPDYN | QEQP_RPT)) && 
		qheader != (PST_QTREE *) NULL &&
		qheader->pst_numparm > 0)
	{
	    OPC_BASE	*parms = global->ops_cstate.opc_rparms;
	    DB_DATA_VALUE *dvp;
	    i4		i;

	    qp->qp_nparams = qheader->pst_numparm;
	    qp->qp_params = (DB_DATA_VALUE **) opu_qsfmem(global,
			qheader->pst_numparm * sizeof(PTR));
	    for (i = 1; i <= qheader->pst_numparm; i++)
	    {
	...
	/* Build SQLDA for SCF (and users) for:
	** - retrieve, define cursor for non-X100 queries (X100 is
	**   done earlier because result column names can be altered
	**   during action header build),
	** - define cursor for scrollable query, X100 or not (these are
	**   compiled as if they are regular Ingres, with the IVW-ness
	**   buried down in the qp),
	** - X100 INSERTs (X100 interface needs row descriptor),
	** - table & row producing procedures.
	*/
	if (qheader != NULL &&
	    global->ops_procedure->pst_isdbp == FALSE
	    && ((qheader->pst_mode == PSQ_DEFCURS && (qheader->pst_mask1 & PST_SCROLL))
		|| (!(global->ops_gmask & OPS_X100_ONLY)
	         && (qheader->pst_mode == PSQ_RETRIEVE || qheader->pst_mode == PSQ_DEFCURS))
		|| (qheader->pst_mode == PSQ_APPEND &&
		   ((global->ops_gmask & (OPS_X100_ONLY | OPS_X100_INS)) ||
		    qp->qp_ahd->ahd_atype == QEA_X100_CRTABLE))))
	{
	    i4 names_mask = global->ops_caller_cb->opf_names;

	    if ((global->ops_gmask & (OPS_X100_ONLY | OPS_X100_INS)) &&
		qheader->pst_mode == PSQ_APPEND)
	    {
		/* These may be out of sequence - sort into pst_rsno 
		** order (reverse, actually) using an array (to avoid need
		** for sort logic in a linked list). */
		PST_QNODE	**rsdmarray, *rsdmp, *treep;
		i4		rscount;

		/* Count RESDOMs and allocate array to hold 'em. */
		for (rscount = 0, rsdmp = qheader->pst_qtree->pst_left;
			rsdmp && rsdmp->pst_sym.pst_type == PST_RESDOM;
			rsdmp = rsdmp->pst_left)
		    rscount++;
		treep = rsdmp;			/* save end-of-list ptr */
		if (rscount)
        	    rsdmarray = (PST_QNODE **)opu_memory(global,
                                rscount * sizeof(PST_QNODE *));

		/* Place in correct position in array. */
		for (rsdmp = qheader->pst_qtree->pst_left;
			rsdmp && rsdmp->pst_sym.pst_type == PST_RESDOM;
			rsdmp = rsdmp->pst_left)
		 rsdmarray[rsdmp->pst_sym.pst_value.pst_s_rsdm.pst_rsno-1] =
					rsdmp;
		
		/* One more pass to re-link them in reverse. */
		qheader->pst_qtree->pst_left = rsdmarray[rscount-1];
		for (; rscount > 0; rscount--)
		 rsdmarray[rscount-1]->pst_left = rsdmarray[rscount-2];
		rsdmarray[0]->pst_left = treep;

		/* And, we need names for X100 descriptors */
		names_mask = GCA_NAMES_MASK;
	    }
	    status = ulc_bld_descriptor(qheader->pst_qtree,
				names_mask,
				&global->ops_qsfcb,
	...
	else if (!(global->ops_gmask & OPS_X100_ONLY))	/* X100 already done */
	{
	    qp->qp_sqlda = NULL;
	}
