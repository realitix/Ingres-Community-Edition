...

{
    ...
    ** constant buffer */
    ...
    range.pst_sym.pst_dataval.db_collID = DB_UNSET_COLL;
    ...
    ops[2].opr_collID = DB_UNSET_COLL;
	...
	bool	    another_coming )
{
	...
	if (cqual->opc_qsinfo.opc_tidkey)
	{
	    ...
	    tidatt.att_prec = 0;
	    tidatt.att_collID = DB_UNSET_COLL;
	    tidatt.att_flags = 0;
	...
	bool		another_coming )
{
    ...
    *end_qekand = qekand;
    ...
    qekatt->attr_collID = att->att_collID;
	...
	*/
	...
	if (qconst->opc_tconst_type == PST_USER)
	{
	    ...
	    adeops[0].opr_collID = att->att_collID;
	...
	else
	{
	    ...
	    */
	    ...
	    adeops[3].opr_collID = root->pst_sym.pst_dataval.db_collID;
	    ...
	    adeops[0].opr_len = 2;
	    adeops[0].opr_prec = 0;
	    adeops[0].opr_collID = DB_UNSET_COLL;
	    adeops[0].opr_base = cadf->opc_key_base;
	    ...
	    adeops[1].opr_collID = adeops[2].opr_collID = att->att_collID;
	    ...
	    adeops[4].opr_prec   = 0;
	    adeops[4].opr_collID = DB_UNSET_COLL;
	    adeops[4].opr_base   = 0;
	...
	*/
	...
	{	
	    ...
	    */
	    ...
	    adeops[3].opr_len = OPC_UNKNOWN_LEN;
            adeops[3].opr_prec = OPC_UNKNOWN_PREC;
            adeops[3].opr_collID = DB_UNSET_COLL;
...
**	    be attached there.
**	14-june-2010 (dougi)
**	    Fill in table name to help VectorWise inserts.
**	17-Jun-2010 (kschendel) b123775
...
**	    Bypass some unneeded lockmode-related stuff if it's an IVW table.
*/
QEF_VALID *
opc_valid(
		OPS_STATE	*global,
		...
		bool		size_sensitive)
{
    ...
    gateway = x100tab = FALSE;
    if (rel->rdr_rel->tbl_relgwid == GW_RMS)
	gateway = TRUE;
    else if (rel->rdr_rel->tbl_storage_type > DB_STDING_STORE_MAX)
	x100tab = TRUE;
    ...
    ** of lock desired.
    ** A lot of this is unnecessary if it's an IVW table.  (IVW tables
    ** are dmf-opened only for catalog related reasons, timestamp checks
    ** and control locks and the like.)
    */
    new_rw = DMT_A_READ;
    if (! x100tab)
    {
	...
	if (grv->opv_topvalid != NULL)
	{
		...
		*/
		...
		{
		    /*
		    ** For RMSGW, we only want the last instance to get write
		    ** mode.  If we have a write mode coming in, update all
		    ** other instances of this range variable to be read mode.
		    ** This is exactly opposite to what Ingres does for
		    ** its native tables:  they all get the maximum lockmode if
		    ** any instance is open for write.  The trick for us is to
		    ** determine which instance should be left open for write;
		    ** the assumption here is that only the last instance to
		    ** come in with write mode is the one to leave in that mode.
		    */
    ...
    /* QEF gets upset if either is zero */
    if (ret_vl->vl_est_pages <= 0)
	ret_vl->vl_est_pages = 1;
    if (ret_vl->vl_total_pages <= 0)
	ret_vl->vl_total_pages = 1;
    ret_vl->vl_alt = NULL;
	...
	STRUCT_ASSIGN_MACRO(rel->rdr_rel->tbl_id,
			    ret_vl->vl_tab_id);
	STRUCT_ASSIGN_MACRO(rel->rdr_rel->tbl_name,
			    ret_vl->vl_tab_name);
    ...
    */
    ...
    /* Note, x100 table doesn't necessarily mean x100query action,
    ** generate the DMR_CB index always and let the caller fix it
    ** if necessary.
    */
		...
		OPC_ADF		*cadf)
{
    ...
    */
	    ...
	    pst_sym.pst_value.pst_s_root.pst_qlang == DB_QUEL)
	{
		...
		adeops[0].opr_collID = parms[parmno].opc_dv.db_collID;
    ...
    */
	    ...
	    )
	{
	    ...
	    adeops[0].opr_collID = decvar->pst_vardef[lvarno].db_collID;
	    adeops[0].opr_offset = 0;
...

{
	...
	nulldv.db_collID = DB_UNSET_COLL;
...

{
	...
	*/
	...
	if (nodep->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags & ADI_F32_NOTVIRGIN)
	    return (FALSE);
	if (!opc_pq_novar(nodep->pst_left))
	    return (FALSE);
	while (nodep->pst_right != NULL
	  && nodep->pst_right->pst_sym.pst_type == PST_OPERAND)
	{
	    nodep = nodep->pst_right;	/* run thru the operands */
	    if (!opc_pq_novar(nodep->pst_left))
		return (FALSE);
	}
