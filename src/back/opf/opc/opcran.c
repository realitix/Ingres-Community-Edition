...
**	    Add VW support.
**	28-oct-2010 (dougi)
**	    Fix VW support slightly for CTAS/INSERT ... SELECT.
*/
VOID
opc_ran_build(
		OPS_STATE   *global,
		OPC_NODE    *cnode)
{
    ...
    */
    ...
    {
	/* This must be CTAS or INS ... SEL from VW to Ingres. */
	for (rsubqry = global->ops_subquery; rsubqry && 
	    rsubqry->ops_sqtype != OPS_MAIN; rsubqry = rsubqry->ops_next);
	subqry = rsubqry;
	oldmode = subqry->ops_mode;
	subqry->ops_mode = PSQ_RETRIEVE;
	opc_x100ahd_build(global, &ran->qp_act);
	ran->qp_qual = (QEN_ADF *)NULL;
	subqry->ops_mode = oldmode;
	opvvar = (OPV_VARS *) NULL;
	opvsub = (OPV_SUBSELECT *) NULL;
	opvgrv = (OPV_GRV *) NULL;
	ivwtoing = TRUE;
    }
    else
    ...
    */
    ...
    /* Add row to hold the result row. */
    opc_ptrow(global, &qn->qen_row, qn->qen_rsz);

    /* If this is a CTAS/INSERT ... SELECT from VectorWise to Ingres, 
    ** the rest of this function is irrelevant and can be skipped. 
    **
    ** If it is a hybrid query with bits of VW query joined to bits of
    ** Ingres query, the normal flow of the function is fine. */
    if (ivwtoing)
	goto ivw_skip;			/* rest isn't relevant */
	    ...
	    */
	    for (resdom = rsubqry->ops_root->pst_left;
	     ...
	     if (resdom->pst_sym.pst_value.pst_s_rsdm.pst_rsno == iattno &&
			(resdom->pst_sym.pst_value.pst_s_rsdm.pst_rsflags &
					PST_RS_PRINT))
		  break;
		...
		i4		is_top_ahd)
{
	    ...
	    )
	{
		...
		*/
	    ...
	    /* If this is a pure VectorWise query, exit loop. */
	    if (global->ops_gmask & OPS_X100_ONLY)
		break;
	}
