...
**	03-dec-2010 (smeke01) b124774
**	    Correct the handling of coercion followed by normalisation.
*/
VOID
		...
		bool		oj)
{
    ...
    DB_STATUS		status;
    bool		keysdone = FALSE;
    bool		coerce, normalise, iskey, isnormalisable;
    DB_DT_ID		keytype;
    ...
    */
	...
	for (eqcno = 0, i = 0; eqcno < subqry->ops_eclass.ope_ev; eqcno += 1)
	{
	    ...
	    ** them on second pass (keysdone). */
	    ...
	    iskey = BTtest((i4)eqcno, (char *)keqcmp);

	    /* Only process key attributes during the 'key' iteration. */
	    if (!keysdone && !iskey)
		continue;
	    ...
	    /*
	    ** We DO process normalisable key attributes during the 'non-key'
	    ** iteration. We pass over all other key attributes, but we make
	    ** sure we increment the index into the cmplist, as we will need
	    ** this if we later get a normalisable key attribute.
	    */
	    if (keysdone && iskey && !isnormalisable)
	    {
		i++;
		continue;
	    };
		...
		** that cleans it up. */
	    ...
	    /* Only normalise during the 'key' iteration */
	    if (!keysdone && isnormalisable)
	    {
	    ...
	    if (coerce)
	    {
		/* Coerce ops[1] to ops[0] */
		opc_adtransform(global, &cadf, &ops[1], &ops[0], ADE_SMAIN, 
						(bool)FALSE, (bool)FALSE);
		/*
		** If we ALSO need to do a hash normalisation then we have to
		** make sure that we take the coercion output as the input to
		** the hash normalisation. 
		*/
		if (normalise)
		{
		    /* Initialise ops[1] from ops[0] */
		    STRUCT_ASSIGN_MACRO(ops[0], ops[1]);
		    /* Put results of normalise in the hash row */
		    opc_adinstr(global, &cadf, opcode, ADE_SMAIN, 2, ops, 1);
		} 
	    }
	    else
	    {
		/* Copy or normalise ops[1] to ops[0] */
		if (!normalise)
		{
	    ...
	    ** Update the dshrow and dshoffset for the current
	    ** att to reflect where the att now currently sits.
	    ** Only do this for attributes that haven't been normalised.
	    ** Attributes are normalised only in the 'key' iteration.
	    ** Attributes that are normalised are processed again in
	    ** the 'non-key' iteration, and these are copied or coerced
	    ** into the non-key part of the row without normalisation.
	    ** These key attributes will then have their dshrow and dshoffset
	    ** updated below in the 'non-key' iteration.
	    */
	    if (!normalise)
	    {
		ceq[eqcno].opc_dshrow = rowno;
	    ...
	    /* 
	    ** Only update the opc_eqcdv in the 'key' iteration if we haven't
	    ** normalised the attribute. For key normalisable attributes the
	    ** original opc_eqcdv values may be needed during the 'non-key'
	    ** iteration to trigger a coerce instruction. The opc_eqcdv is then
	    ** updated during the 'non-key' iteration.
	    */
	    if ((!keysdone && !normalise) || (keysdone && iskey && isnormalisable))
	    {
