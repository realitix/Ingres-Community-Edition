...
**	8-sep-2010 (dougi)
**	    Add support for DGTT AS to IVW temp tables.
...
**	    Correct init of collation data.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**/

/* Define a convenience enum to coordinate mixed X100/Ingres queries. */

enum mixed_plan_enum {
    NOT_MIXED,			/* Pure VW or pure Ingres */
    MIXED_APP_LOAD,		/* X100 QP under insert (or ctas) */
    MIXED_SCROLL_GET		/* X100 QP under GET for scroll cursor */
};
...
static QEF_AHD *opc_qepahd(
	OPS_STATE *global,
	QEF_AHD **proj_ahd,
	enum mixed_plan_enum mixed_plan);
...
**	30-sep-2010 (dougi)
**	    Moved code from opc_cqp_continue() for X100 processing. Part of
**	    generalizing for CTAS from VW to Ingres and later, hybrid queries.
...
**	27-oct-2010 (dougi)
**	    Add "ivwtoing" flag to opc_qepahd() call to identiify inserts from 
**	    VW query to Ingres table.
**	03-Mar-2011 (kiria01) m1552
**	    Block X100 update/delete from cursor.
**	28-Apr-2011 (kschendel) m1920
**	    Add scrollable IVW cursor support.  Minor simplifications.
*/
VOID
opc_ahd_build(
		OPS_STATE   *global,
		...
		QEF_AHD	    **proj_ahd)
{
    ...
    /* Handle delete / update where current of cursor and get out */

    if (subqry->ops_sqtype == OPS_MAIN && 
	    global->ops_qheader->pst_mode == PSQ_REPCURS
	)
    {
	if (global->ops_qheader->pst_mask1 & PST_X100_TARGET)
	     opx_verror(E_DB_ERROR, 9481, 0);
	/* make the action to replace the cursor; */
	*pahd = opc_rupahd(global);
	return;
    }
	...
	if (global->ops_qheader->pst_mask1 & PST_X100_TARGET)
	     opx_verror(E_DB_ERROR, 9481, 0);
	*pahd = opc_rdelahd(global);
	return;
    }

    modify_post_load = FALSE;
    mixed_plan = NOT_MIXED;

    /* Agg, project, and CTAS variants need some sort of leading
    ** file-create action header.
    ** Insert (append) and define cursor don't, but need X100 flag checking.
    */
    ...
    else if (global->ops_qheader->pst_mode == PSQ_APPEND)
    {
	/* Check for and flag any insert into VW table;
	** insert-select into Ingres from VW table.
	** (Might also be a ctas if we're looking at a subquery
	** after the first one.)
	*/
	if (global->ops_qheader->pst_mask1 & PST_X100_TARGET)
	    ivwINS = TRUE;
	else if (global->ops_gmask & OPS_X100_ONLY)
	    mixed_plan = MIXED_APP_LOAD;
    }
    else if (global->ops_qheader->pst_mode == PSQ_DEFCURS)
    {
	/* Scrollable cursor on IVW table buries X100 algebra under a QP node,
	** so that QEF doesn't have to know what's going on.  Ordinary
	** (read-only, nonscrolling) cursor on IVW table is simple enough
	** for QEF to do directly, so that one generates X100 at the top.
	*/
	if (global->ops_gmask & OPS_X100_ONLY
	  && global->ops_qheader->pst_mask1 & PST_SCROLL)
	    mixed_plan = MIXED_SCROLL_GET;
    }
    else if (global->ops_qheader->pst_mode == PSQ_RETINTO ||
	     global->ops_qheader->pst_mode == PSQ_DGTT_AS_SELECT ||
	     global->ops_qheader->pst_mode == PSQ_X100_DGTT_AS)
    {
	...
	*/
	...
	/* make an action to create the relation.  Do it first for
	** IVW create, only do it when we reach main for Ingres create.
	*/
	if (storage_type > DB_STDING_STORE_MAX)
	{
	    ivwINS = TRUE;
	    opc_x100_dmuahd(global, global->ops_subquery, &ahd_list);
	    /* If ingres -> IVW, change the query mode to APPEND..
	    ** This ensures that we don't do the create again, and
	    ** ensures that when we finally reach the MAIN sq (either
	    ** this one, or later), ivw-ins will be set, and the
	    ** proper flag will be lit in the APPEND/LOAD action.
	    ** For the same reason, make double sure that X100-target is lit.
	    */
	    if (!(global->ops_gmask & OPS_X100_ONLY))
	    {
		global->ops_qheader->pst_mode = PSQ_APPEND;
		global->ops_qheader->pst_mask1 |= PST_X100_TARGET;
	    }
	}
	else if (global->ops_gmask & OPS_X100_ONLY)
	    mixed_plan = MIXED_APP_LOAD;

	/* Do create/modify stuff now if main and NOT into VW target */
	if (!ivwINS && subqry->ops_sqtype == OPS_MAIN)
	{
	    ahd_list = 
	    ...
	    ** the result structure is HEAP or VW, we don't modify at all.
	    **
	...
	****** if the modify is post-load?  */
	...
	} /* If ingres-target and main */
    } /* if ctas */

    /* So far we have either no action;  or an X100 create table;
    ** or an Ingres create table possibly followed by a modify.
    ** ahd_list points to the last action (if any).
    */

    ahd = NULL;

    /*
    ** Generate an X100 header with sqlda, if it's an x100-only plan, and
    ** it's not an IVW -> Ingres step, e.g. ctas or insert-select to an
    ** Ingres table, or a VW scrollable cursor.
    ** Note that for INSERT specifically, the x100-only flag excludes
    ** the result table.  Thus an INSERT INTO vw VALUES (...) does not
    ** compile an X100 header, it compiles an Ingres QP (with the X100
    ** flag set in the PUT header.)
    ** If we don't do an X100 action, generate a good 'ol qp action.
    */
    if (global->ops_gmask & OPS_X100_ONLY && mixed_plan == NOT_MIXED)
    {
	/* if we're compiling X100 algebra ... */
	DB_STATUS	status;

	/* Build SQLDA here because opc_x100ahd_build() may change 
	** result column names. */
	status = ulc_bld_descriptor(global->ops_qheader->pst_qtree,
		    (global->ops_caller_cb->opf_names | GCA_NAMES_MASK),
		    &global->ops_qsfcb,
		    (GCA_TD_DATA **)&global->ops_cstate.opc_qp->qp_sqlda,
	...
	/* Generate X100 query into action header. */
	opc_x100ahd_build(global, &ahd);
	ahd->qhd_obj.qhd_x100Query.ahd_x100_flag |= AHD_X100_SCFBUF;
    }
    else
    {
	/* Generate Ingres style action header */
	ahd = opc_qepahd(global, proj_ahd, mixed_plan);
    }

    /* Append the query action to the list */
    if (ahd != NULL)
    {
	if (ahd_list == NULL)
	{
	...
	if (ivwINS && (ahd->ahd_atype == QEA_LOAD ||
			ahd->ahd_atype == QEA_APP))
	{
	    /* Flag inserts into IVW table. */
	    ahd->qhd_obj.qhd_qep.ahd_qepflag |= AHD_X100_QUERY;
	    global->ops_gmask |= OPS_X100_INS;
	}
    }
	...
	*retinto_alt = grv->opv_topvalid;
    ...
    if (modify_post_load)
    {
	/* CTAS wants a modify after the load, generate the modify */
	ahd = opc_dmuahd(global, DMU_MODIFY_TABLE,
			global->ops_qheader, subqry->ops_result);
	ahd->ahd_prev = ahd_list->ahd_prev;
	ahd->ahd_next = ahd_list;
...

/*{
** Name: OPC_X100_DMUAHD	Constructs DMU & action header for CREATE TABLE
**	AS ... SELECT ... for X100 table
**
** Description:
**
** Inputs:
...
**	5-may-2010 (dougi)
**	    Make sure we send OPS_MAIN ops_result to opc_dmuahd().
**	5-june-2010 (dougi)
**	    Add check for too-big DECIMAL columns (from CTAS).
**	5-june-2010 (dougi)
**	    Hook up opv_ptrvalid for IVW INSERTs.
**	26-sep-2010 (dougi)
**	    Oops - forgot to reallocate QEU_CB. It's in PSF memory and gets
**	    freed at end of OPF.
**	29-Oct-2010 (kschendel)
**	    No need to worry about structure here.
*/
static VOID
opc_x100_dmuahd(
		OPS_STATE   *global,
		...
		QEF_AHD	    **pahd)
{
    ...
    QEU_CB	*qeu_cb;
    DMU_CB	*dmucb;
    DMF_ATTR_ENTRY *attrp, **attrpp;
    OPV_GRV	*grvp;
    i4		i, size, prec, scale;

    /* Build the QP copy of a DMU_CB, plus all the right RDF-ish
    ** stuff.  But first find the OPS_MAIN subquery (could be SAGG or
    ** something else).  This makes sure we have the proper ops_result!
    */

    for (msq = subquery; msq->ops_sqtype != OPS_MAIN; msq = msq->ops_next)
	/* just loop */ ;
    dmucb = opc_dmu_dmucb(global, DMU_CREATE_TABLE,
		global->ops_qheader, msq->ops_result);

    /* Allocate  an action header, fill in all the standard and
    ** boiler-plate stuff.  Unlike an Ingres CTAS, the VW CTAS uses a
    ** QEF_DDL_STATEMENT action header for the create, so we'll also
    ** need a stub copy of the parser's QEU_CB.
    */

    (*pahd) = ahd = (QEF_AHD *)opu_qsfmem(global, sizeof(QEF_AHD) - 
    ...
    ahd->ahd_atype = QEA_X100_CRTABLE;
    ahd->ahd_valid = NULL;
    ahd->ahd_mkey = NULL;

    ahd->qhd_obj.qhd_DDLstatement.ahd_qeuCB = opu_qsfmem(global, 
							sizeof(QEU_CB));
    qeu_cb = (QEU_CB *)(*pahd)->qhd_obj.qhd_DDLstatement.ahd_qeuCB;
    MEcopy((char *)global->ops_qheader->pst_qeucb, sizeof(QEU_CB),
		(char *)qeu_cb);
    /* The following aren't interesting and are cleared (to be safe). */
    ...
    qeu_cb->qeu_d_cb = (PTR) dmucb;
    (*pahd)->qhd_obj.qhd_DDLstatement.ahd_alt = (QEF_VALID *) NULL;
    grvp = global->ops_rangetab.opv_base->opv_grv[msq->ops_result];
    /* Since this is always called before any possible use of the
    ** result table, don't bother with opc_get_resource;  it would
    ** only return NULL (no such resource yet).
    */
    ahd->qhd_obj.qhd_dmu.ahd_alt = NULL;
    grvp->opv_ptrvalid = &ahd->qhd_obj.qhd_DDLstatement.ahd_alt;	

    /* Do quick check for DECIMAL types that overflow and modify them. */
    attrpp = (DMF_ATTR_ENTRY **)dmucb->dmu_attr_array.ptr_address;
    for (i = 0; i < dmucb->dmu_attr_array.ptr_in_count; i++)
    {
	attrp = attrpp[i];
	if (abs(attrp->attr_type) != DB_DEC_TYPE)
	    continue;				/* only interested in DEC */

	prec = DB_P_DECODE_MACRO(attrp->attr_precision);
	if (prec <= IVW_MAX_DECPREC)
	    continue;				/* it's ok */

	/* Clip to VW's max, keeping scale the same if possible */
	scale = DB_S_DECODE_MACRO(attrp->attr_precision);
	prec = IVW_MAX_DECPREC;
	if (scale > prec)
	    scale = prec;
	size = DB_PREC_TO_LEN_MACRO(prec);
	if (attrp->attr_type < 0)
	    ++ size;

	attrp->attr_size = size;
	attrp->attr_precision = DB_PS_ENCODE_MACRO(prec, scale);
    }
...
**	13-feb-2009 (dougi)
**	    Minor changes to support X100 inserts.
...
**	5-june-2010 (dougi)
**	    Broaden setting of x100app for CTAS/INSERT ... SELECT.
...
**	    VW query to Ingres table.
**	28-Apr-2011 (kschendel) m1920
**	    X100 flag for load/append is set in ahd build, don't need here.
**	    Do proper target stuff for X100 scroll cursor.
*/
static QEF_AHD *
opc_qepahd(
	OPS_STATE   *global,
	...
	enum mixed_plan_enum mixed_plan)
{
    ...
    */
	...
	switch (sq->ops_sqtype)
	{
	 ...
	 default:
	    /* If there's X100 underneath, we can't do the usual opc-target
	    ** business.  For append/load, do nothing, as QEF sees the
	    ** X100 flag and hares off to special code right away.
	    ** For scroll-cursor gets, the result has been fully calculated
	    ** and qualified by X100, just build an ahd-current to move it
	    ** where it belongs.
	    */
	    if (mixed_plan == NOT_MIXED)
	    {
		opc_target(global, &cnode, sq->ops_root->pst_left, 
				    &ahd->qhd_obj.qhd_qep.ahd_current );
	    }
	    else if (mixed_plan == MIXED_APP_LOAD)
	    {
		ahd->qhd_obj.qhd_qep.ahd_current = (QEN_ADF *) NULL;
		ahd->qhd_obj.qhd_qep.ahd_qepflag |= AHD_X100_INGRES;
	    }
	    else if (mixed_plan == MIXED_SCROLL_GET)
	    {
		opc_x100_target(global, &cnode, sq->ops_root->pst_left, 
				    &ahd->qhd_obj.qhd_qep.ahd_current );
	    }
	    ahd->qhd_obj.qhd_qep.ahd_by_comp = (QEN_ADF *)NULL;
	    ahd->qhd_obj.qhd_qep.ahd_proj = -1;
	...
	*/
	...
	for (loop=0;loop<natts;loop++)
	{
	    ...
	    att[loop].attr_collID = DB_UNSET_COLL;
...

/*{
** Name: opc_dmu_dmucb - Build DMU_CB and RDF info for CREATE/MODIFY
**
** Description:
**	A CTAS query will need to build one or more DMU action headers
**	which tell the query plan to create and/or modify a table at
**	that point.  In either case, a DMU_CB needs to be created and
**	filled in as part of the query plan (and pointed to by the
**	action header).  In the CREATE case, we also need to build the
**	equivalent of an RDF descriptor for the table being created.
**
**	This routine builds the DMU_CB, including a copy of any partition
**	info needed, and sets up the RDF descriptor from a combination
**	of the parser generated DMU_CB attribute list and the select
**	resdom list (possibly altered by the optimizer to allow for
**	outer join nullability, etc).
**
** Inputs:
**	global			Optimizer session state
**	dmu_func		DMU_CREATE_TABLE or DMU_MODIFY_TABLE
**	qtree			Query tree header which points to parser
**				generated QEU_CB, DMU_CB.
**	gresvno			Global var # of (main) result table
**
** Outputs:
**	Returns pointer to QP copy of DMU_CB with applicable stuff
**	filled in.
**
** History:
**	1-Nov-2010 (kschendel)
**	    Split original opc_dmuahd into two pieces so that X100
**	    create-as-select can use just the DMU/RDF bits.
**	03-Feb-2011 (kiria01) b124996
**	    Fix SRID retrieval SEGV and loop logic.
*/
	...
	OPV_IGVARS  gresvno)
{
    ...
    **
    ** An IVW create-as-select only does a CREATE and doesn't have any
    ** concept of keys as such;  QEF may dig some relevant info out of
    ** X100 constraint actions, but that's none of our problem here.
    */
	...
	*/
	...
	global->ops_rangetab.opv_base->
				opv_grv[gresvno]->opv_relation = rel;
	...
	*/
	...
	/* Make sure that CREATE sees a heap, or a vectorwise table!
	** (Ingres, non-heap creates start with a heap and modify to the
	** structure later.)
	*/
	if (dmucb->dmu_chars.dmu_struct <= DB_STDING_STORE_MAX)
	...
	*/
	    ...
	    )
	{
		...
		if (bits & AD_PERIPHERAL)
		{
		    ...
		    pdv.db_length = 0;
		    pdv.db_prec = 0;
		    pdv.db_collID = attr->attr_collID;
		    ...
		    att->att_collID = attr->attr_collID = rdv.db_collID;
	    ...
	    */
		...
		OPV_GRV *grv = global->ops_rangetab.opv_base->opv_grv[vno];
	    	if (grv && grv->opv_relation)
		{
	    	    i4 attr_count = grv->opv_relation->rdr_no_attr;
	    	    /* Find attribute number */
	    	    for(i = 1; i <= attr_count; i++)
	    	    {
	    		if(!memcmp(resdom->pst_right->pst_sym.pst_value.pst_s_var.pst_atname.db_att_name,
	    			    grv->opv_relation->rdr_attr[i]->att_nmstr, DB_MAXNAME))
	    		{
	    		    /* If attribute is found, let's fix the new DMF_ATTR_ENTRY */
			    attr->attr_srid = grv->opv_relation->rdr_attr[i]->att_srid;
    			    break;
	    		}
		    }
	    	}
...
**
** Inputs:
**	global			Optimizer session global state
**	dmu_func		DMU_CREATE_TABLE or DMU_MODIFY_TABLE
**	qtree			Query tree header pointing at parser DMU stuff
**	gresvno			Global variable # of result relation
**
** Outputs:
**	Builds and returns a QEA_DMU action header
**	No return (opx-error) if error
**
** History:
**      31-jul-86 (eric)
**          written
...
**	1-Nov-2010 (kschendel)
**	    In order to integrate 124544 and X100 stuff, split action header
**	    building off from DMU-CB building.  X100 wants the latter but
**	    has its own ideas about the action header.
*/
	...
	OPV_IGVARS  gresvno)
{
    ...
    /* Generate the QP copy of the DMU_CB and (if CREATE) any necessary
    ** pseudo-RDF description of the table we're about to create.
    */

    dmucb = opc_dmu_dmucb(global, dmu_func, qtree, gresvno);
	...
	OPV_IGVARS  gresvno )
{
    ...
    */
	...
	attr->attr_collID = tsoffsets->opc_tcollID;
		...
		QEF_AHD		**pahd)
{
	...
	ops[1].opr_collID = qretnum->pst_sym.pst_dataval.db_collID;
	...
	ops[0].opr_len = 4;
	ops[0].opr_prec = 0;
	ops[0].opr_collID = DB_UNSET_COLL;
	...
	*/
	...
	)
	{
	    ...
	    {		/* check matching column/parm */
			/* We allow att_collID-dbpp_collID to differ here */
    ...
    i4		*p_offset_list[])
{
	...
	*/
	...
	)
	{
	    ...
	    attr->attr_collID = param_tup->dbpp_collID;
	    attr->attr_flags_mask = 0;
...
**	24-nov-2010 (dougi)
**	    Accomodate ins./sel. from VW to Ingres with rules. Row number
**	    comes from QP node under append action.
*/
	...
	RDR_INFO    *rel )
{
	...
	switch (ahd->ahd_atype)
	{
		...
		*/
		...
		/* by the semantics for rules. Also note, if this is a "insert 
		** select" from VW to Ingres, buffer comes from QP node. */
		if (ahd->qhd_obj.qhd_qep.ahd_qepflag & AHD_X100_INGRES)
		    ...
		    ahd->qhd_obj.qhd_qep.ahd_qep->qen_row;
		else ahd->qhd_obj.qhd_qep.u1.s1.ahd_b_row = global->ops_cstate.opc_before_row =
		    ahd->qhd_obj.qhd_qep.u1.s1.ahd_a_row = global->ops_cstate.opc_after_row =
		    (ahd->qhd_obj.qhd_qep.ahd_current) ?
		    	ahd->qhd_obj.qhd_qep.ahd_current->qen_output : -1;
		...
		QEF_AHD		**new_ahd)
{
    ...
    ahd->ahd_ascii_id = QEAHD_ASCII_ID;
    ahd->ahd_valid = NULL;
    if (global->ops_statement->pst_type == PST_X100_CREATE_TABLE_TYPE)
	ahd->ahd_atype = QEA_X100_CRTABLE;
    else ahd->ahd_atype = QEA_CREATE_TABLE;
		...
		QEF_AHD		**new_ahd )
{
    ...
    *new_ahd = ( QEF_AHD * ) NULL;
    ...
    ahd->ahd_ascii_id = QEAHD_ASCII_ID;
    ahd->ahd_valid = NULL;
    if (global->ops_statement->pst_type == PST_X100_CREATE_INTEGRITY_TYPE)
	ahd->ahd_atype = QEA_X100_CRCONSTRAINT;
    else ahd->ahd_atype = QEA_CREATE_INTEGRITY;
...
**	15-dec-2008 (dougi)
**	    Added QCI_X100_REFREL flag for referential relationships on X100
**	    tables.
		...
		QEF_CREATE_INTEGRITY_STATEMENT	*qefDetails )
{
    ...
    if ( psfDetails->pst_createIntegrityFlags & PST_CONS_X100 )
    {	qefDetails->qci_flags |= QCI_X100_CONS; }
	    ...
	    QEF_AHD         **new_ahd)
{
    ...
    ahd->ahd_ascii_id = QEAHD_ASCII_ID;
    ahd->ahd_valid = NULL;
    if (global->ops_statement->pst_type == PST_X100_DROP_INTEGRITY_TYPE)
	ahd->ahd_atype = QEA_X100_DRCONSTRAINT;
    else ahd->ahd_atype = QEA_DROP_INTEGRITY;
...
**	11-Mar-2011 (kschendel) m1717
**	    Adjust tests to allow ivw->ingres loads.
*/
...
opc_loadopt(OPS_STATE *global, QEF_AHD *ahd, QEF_VALID *valid)
{
    ...
    if ((global->ops_qheader->pst_mask1 & PST_X100_TARGET) ||
	 (qmode != PSQ_APPEND && qmode != PSQ_RETINTO
	...
	*/
    ...
    /* *** FIXME still need to integrate datallegro insert-select code! */
    return;
