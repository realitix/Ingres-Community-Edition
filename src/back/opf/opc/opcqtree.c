		...
		i4	    tid_key)
{
	...
	*/
	if (tid_key == TRUE)
	{
	    ...
	    kconst->opc_keyblk.adc_lokey.db_collID = DB_UNSET_COLL;
	}
	else
	{
	    ...
	    kconst->opc_keyblk.adc_lokey.db_data = NULL;
	    kconst->opc_keyblk.adc_lokey.db_collID = att->att_collID;
	}
     ...
     case PST_BOP:
		...
		*/
		...
		if (qconst == NULL)
		{
		    ...
		    kconst->opc_keyblk.adc_kdv.db_collID = DB_UNSET_COLL;
		...
		PST_QNODE	*qconst)
{
    ...
    if ((ret = adi_ficoerce_full(global->ops_adfcb, 
			    qconst->pst_sym.pst_dataval.db_datatype, 
			    dval->db_datatype, &adffn.adf_fi_id, &fi_desc)) != E_DB_OK
	)
    {
