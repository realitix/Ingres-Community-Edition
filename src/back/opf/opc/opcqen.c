...
**	4-oct-2010 (dougi)
**	    Add support for mixed IVW/native Ingres.
**	28-oct-2010 (dougi)
**	    Skip opc_qnatts() call for QE_QP nodes reading an X100 query result.
*/
VOID
opc_qentree_build(
		OPS_STATE   *global,
		OPC_NODE    *cnode)
{
	...
	*/
    ...
    {
	/* Must be CTAS or INSERT ... SELECT from X100 query. */
	qntype = QE_QP;
    ...
    */
    ...
    if (qn->qen_atts == NULL && (qn->qen_type != QE_QP ||
	    qn->node_qen.qen_qp.qp_act->ahd_atype != QEA_X100_QRY))
    {
		...
		QEN_NODE	*qn)
{
	...
	*/
	...
	if (opt_strace(global->ops_cb, OPT_F045_QENPROW) == TRUE)
	{
	    ...
	    dv_vertbar.db_collID = DB_UNSET_COLL;
	    dv_vertbar.db_data = "|";
	    ...
	    )
	{
	    ...
	    */
	    ...
	    iatt->att_collID = cnode->opc_ceq[eqcno].opc_eqcdv.db_collID;
	    iatt->att_flags = 0;
		...
		*/
		...
		ops[1].opr_collID = cnode->opc_ceq[eqcno].opc_eqcdv.db_collID;
	...
	*/
	    ...
	    )
	{
	    ...
	    */
	    ...
	    equivClassDataValue->db_collID = ops[1].opr_collID;
