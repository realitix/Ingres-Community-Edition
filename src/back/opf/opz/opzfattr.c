	...
	DB_DATA_VALUE      *resultdatatype)
{
    ...
    status = adi_ficoerce_full(adf_scb, indatatype->db_datatype, outdtid,
			&convid, &adi_fdesc);
...
**	    Corrected bad indirection of a BTclear referencing stack params
**	    instead of data. This was obscured by casts to the BT functions.
**	    Remaining casts have been removed.
*/
static OPV_IVARS
opz_fatest(
	OPS_SUBQUERY       *subquery,
	...
	*/
	...
	attrmap = &subquery->ops_eclass.ope_base->ope_eqclist[equcls]
	    ->ope_attrmap;		/* get ptr to attribute map of
                                        ...
                                        */
	for( attr = -1; 
	     (attr = BTnext(attr, attrmap, BITS_IN(*attrmap)))
		>= 0;)
	    /* test if equivalence class variable is not in the varmap, 
	    ** otherwise the creation of a new function attribute will fail
	    */
	    BTclear(abase->opz_attnums[attr]->opz_varnm, varmap);
    }
    if (BTcount(varmap, OPV_MAXVAR))
	return( BThigh(varmap, BITS_IN(OPV_GBMVARS) )); /* at 
	...
	OPV_IVARS	    varno)
{
    ...
    for (ojid1 = -1; (ojid1=BTnext(ojid1, ojp->opl_idmap,
	maxoj))>=0; )
    {
	OPL_OUTER	*ojp1;
	...
	{
	    if (BTtest(varno, ojp1->opl_ivmap))
...
**	1-dec-2009 (dougi)
**	    Unremove setting of PST_OJJOIN flag (reckless bastard). Removing
**	    the flag resulted in equijoin predicates being evaluated in the 
**	    DB_PR node instead of DB_SJ.
*/
static VOID
opz_copyqual(
	OPS_SUBQUERY	*subquery, 
...
not needed since strength reduction solves the problem
	...
	** list for outer join semantics */
	...
	OPZ_FATTS           *tempfatt;	/* ptr to function attribute element
					...
					** qualification
					*/
        rvarno = BThigh(&fatt->opz_right.opz_varmap, BITS_IN(OPV_GBMVARS));
...
** to give more flexibility since they may not be needed */
	...
	OPV_IVARS	    lvarno;	/* range variable to use for creation
					** of the function attribute
					*/
        lvarno = BThigh(&fatt->opz_left.opz_varmap, BITS_IN(OPV_GBMVARS));
	...
	for (fattr = 0; fattr < maxfattr; fattr++)
	{
		...
		if (!(fattrp->opz_mask & OPZ_OJFA)
		    ...
		    ** outer variable exists */
		    &&
		    BTsubset (&fattrp->opz_eqcm, targetp,
				maxeqcls)	/* check that equivalence classes
			...
			(   varmap
			    &&
			    BTsubset(&fattrp->opz_fagg->opm_ivmap, varmap,
				subquery->ops_vars.opv_rv)    /* check 
		...
		{   /* set the equivalence class if the attribute can be evaluated
		    ** at this point */
		    BTset(abase->opz_attnums[fattrp->opz_attno]->opz_equcls,
			targetp);
		}
		break;
	    }
