/*
** Copyright (c) 1986, 2005, 2010, 2011 Actian Corporation
**
**
*/
# include    <compat.h>
...
**	08-apr-2010 (dougi)
**	    Add INGRES_VECTORWISE dbms_type.
**	08-Nov-2010 (kiria01) SIR 124685
**	    Rationalise function prototypes
**	31-Dec-2010 (kiria01) SIR 124690
**	    Setup the db_collID field where previously ignored.
**      24-feb-2011 (stial01) m1604
**          Use i8 for row and page counts.
**      28-jun-2011 (stial01) m2051
**          Use DEFINE_DB_TEXT_STRING to define text string
...
FILE		   *outf)
{
    ...
    */
	...
	hexout.db_collID = DB_NOCOLLATION;
	hexout.db_prec = 0;
	...
	dest_dt.db_collID = DB_NOCOLLATION;
	dest_dt.db_prec = 0;
...
{
    SYSTIME		stime;
    /* buffer to store "now" as a string constant */
    DEFINE_DB_TEXT_STRING(time,"now")
...
bool		quiet)
{
	...
	** don't print histogram. */
	...
	char		text7o_fmt[sizeof(TEXT7OD) + 10];
	
	i4             extra_len = 0;

	OPN_PERCENT	cum_count;


	if (sizeof(OPN_PERCENT) == sizeof(f8))
	{
	    if (g->opq_dougout)
		STprintf(text7o_fmt, TEXT7OD,
		     g->opq_adfcb->adf_outarg.ad_f8width,
		     g->opq_adfcb->adf_outarg.ad_f8prec, 
		     g->opq_adfcb->adf_outarg.ad_f8width,
		     g->opq_adfcb->adf_outarg.ad_f8prec, 
		     g->opq_adfcb->adf_outarg.ad_f8width,
		     g->opq_adfcb->adf_outarg.ad_f8prec);
	    else STprintf(text7o_fmt, hexOut ? TEXT7O1_hex : TEXT7O1,
		     g->opq_adfcb->adf_outarg.ad_f8width,
		     g->opq_adfcb->adf_outarg.ad_f8prec, 
		     g->opq_adfcb->adf_outarg.ad_f8width,
		     g->opq_adfcb->adf_outarg.ad_f8prec);
	...
	for (cell = 0; cell < cellnm; cell++)
	{
	    ...
	    /* Print cell number, count, cumulative count and value for cell */
	    if (outf)
	    {
		if (g->opq_dougout)
		    SIfprintf(outf, text7o_fmt, cell, cell_count[cell],
			g->opq_adfcb->adf_decimal.db_decimal,
			cum_count, g->opq_adfcb->adf_decimal.db_decimal,
			cell_repf[cell], g->opq_adfcb->adf_decimal.db_decimal);
		else SIfprintf(outf, text7o_fmt, cell, cell_count[cell],
			g->opq_adfcb->adf_decimal.db_decimal,
			cell_repf[cell], g->opq_adfcb->adf_decimal.db_decimal);
	    }
	    else
		...
		if (g->opq_dougout)
			...
			cum_count, g->opq_adfcb->adf_decimal.db_decimal,
			cell_repf[cell], g->opq_adfcb->adf_decimal.db_decimal);
		else SIprintf(text7o_fmt, cell, cell_count[cell],
			g->opq_adfcb->adf_decimal.db_decimal,
			cell_repf[cell], g->opq_adfcb->adf_decimal.db_decimal);
	    }
...
**          on axp.osf
**	22-feb-2010 (dougi)
**	    Guard against tables with more than MAXI4 rows.
*/
VOID
opq_cntrows(
OPQ_RLIST   *rp)
...
# define OPQ_CQRY1	"select count(*) from %s"
    ...
    */
    ...
    sqlda->sqlvar[0].sqllen = sizeof(i8);
...
bool		statdump)
{
	     ...
	     first = FALSE, es_owner = (char *)&g->opq_dba)
	{
			...
			:es_storage_str
 		    ...
 		    {	/* No need for this if we're excluding the table. */
 		        ...
 		        rp->ntups = es_nrows;
                        rp->relwid = es_relwid;
			(VOID) STtrmwhite(es_storage_str);
			STcopy(es_storage_str, rp->storage_str);
			...
			if (!STncmp(rp->storage_str, "VECTORWISE", 10))
			{
			    rp->x100tab = TRUE;
			    if (TRUE || rp->ntups == 0)
				x100tab_to_count = TRUE;
			}
			else rp->x100tab = FALSE;
 		    }
	...
	else
	{
		...
		** count if necessary
		*/
		if (!deleteflag && !exrel && (rp->ntups <= 0 || 
					(x100tab_to_count && !pgupdate)))
		{
		    opq_cntrows(rp);
		}
...
OPQ_GLOBAL *g)
{
	...
	** and initialize the ADF control block to point to it. */
	if (status = aduucolinit(es_collation, &utbl, &sys_err) != OK)
	{
	    opq_error((DB_STATUS)E_DB_ERROR,
		      (i4)E_DM00A0_UNKNOWN_COLLATION, (i4)0);
	}
...
OPQ_GLOBAL *g)
{
    ...
    */
    ...
    {
	if (g->opq_dbms.dbms_type & (OPQ_INGRES | OPQ_INGRESVW | OPQ_STAR | OPQ_RMSGW))
	{
	    /* 
...
# define OPQ_CQRY2 "select cap_capability, cap_value from iidbcapabilities"
	...
	if (!STcompare(es_cap, "DBMS_TYPE"))
	{
	    ...
	    else if (STcompare(es_val, "INGRES_VECTORWISE") == 0)
	    {
		g->opq_dbms.dbms_type = OPQ_INGRESVW;
	    }
    ...
    */
    ...
    {
	/*
	** Only INGRES/IVW DBMS supports table_statistics currently
	*/
...
**	6-dec-2010 (dougi)
**	    Exclude VW indexes from the rel_list.
**	7-mar-2011 (dougi)
**	    Reapply above fix after the "big merge".
*/
bool
    ...
    */
    if(g->opq_dbms.dbms_type == OPQ_STAR)
    {
        ...
        STpolycat(4, stmtbuf, "t.table_reltid, t.table_reltidx, ",
                              ...
                              "t.row_width, t.table_type, t.table_stats, t.storage_structure ",
                              stmtbuf);
	...
	STpolycat(6, stmtbuf, "and (t.table_type = 'T' or t.table_type = 'I' ",
			      "and t.storage_structure not like 'VECTORWISE%')",
...
# endif /* xDEBUG */
    ...
    if (g->opq_dbcaps.phys_tbl_info_src != (i4)OPQ_T_SRC || x100tab_to_count)
    {
	register OPQ_RLIST   **rpp;
	...
	{
	    if (!statdump ||
		((*rpp)->withstats == (bool)TRUE && !x100tab_to_count))
		opq_phys(*rpp);
