...
**	08-apr-2010 (dougi)
**	    Add support for INGRES_VECTORWISE dbms_type.
**	08-Nov-2010 (kiria01) SIR 124685
**	    Rationalise function prototypes
**	08-Dec-2010 (kiria01) SIR 124690
**	    Propagate collation ID
**	31-Dec-2010 (kiria01) SIR 124690
**	    Setup the db_collID field where previously ignored.
**      11-feb-2011 (stial01) Mantis 1492
**          Fix for multi word datatype names e.g timestamp without time zone
**      24-feb-2011 (stial01) m1604
**          Use i8 hostvar for count() (x100 returns i8 for count())
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**      16-Jun-2012 (Geraint Jones) SIR 126682
**          Add the ability to supress the I_OP091C_NOROWS message to
**          optimizedb through the new -zqq flag.
*/
...
static bool opq_scaleunique(
	...
	OPO_F8TUPLES totalsize,
	OPH_DOMAIN *totalunique);
static OPO_F8TUPLES opq_jackknife_est(
	OPO_TUPLES samptups,
	OPO_TUPLES sampunique,
	OPO_F8TUPLES actualtups);
...
PTR		histdata)
{
    ...
    es2_hist.db_prec = 		0;
    es2_hist.db_collID = 	DB_NOCOLLATION;
...
char		   *version)
{
    ...
    */
    if (g->opq_dbms.dbms_type & (OPQ_INGRES | OPQ_INGRESVW | OPQ_RMSGW))
	/* Ingres case */
	sequence = 0;
...
**	x100_table	    flag indicating whether hist is on X100 table
**
** Outputs:
**	Returns:
**	    VOID
...
bool		   x100_table)
{
    ...
    */
    ...
    if (x100_table && numtups <= 0)
	reptfact = 1.0;
    else reptfact = ((OPO_TUPLES) numtups) / nunique;
	...
	else if (opq_global.opq_newrepfest)
	{
	    ...
	    */
	    nunique = opq_jackknife_est((OPO_F8TUPLES)numtups, 
		nunique,
	...
	else
	{
	    ...
	    */
		...
		opq_error( (DB_STATUS)E_DB_ERROR,
		    ...
		    (i8)sizeof(relp->ntups), (PTR)&relp->ntups);
		(*opq_exit)();
	    }
...
# ifdef FUTURE_CODE
    ...
    */
    if (g->opq_dbms.dbms_type & (OPQ_INGRES | OPQ_INGRESVW | OPQ_STAR))
    {
	maxlen = DB_MAXTUP;
    }
    else
	...
	/* retrieve min/max statistics from relation */
	{
	    ...
	    */
		...
		if (g->opq_dbms.dbms_type & 
			(OPQ_INGRES | OPQ_INGRESVW | OPQ_STAR | OPQ_RMSGW) &&
		    (
...
bool		*use_hex)
{
    ...
    ** composite "column". */
    ...
    compdt.db_collID = DB_NOCOLLATION;
...
**			    vector is passed in case we're building composite
**	sample		    boolean indicating if this histogram is sampled
**	samplepct	    proportion of sample (for X100)
**
** Outputs:
**	Returns:
...
**	17-Jul-2009 (kiria01) mantis 000168
**	    Allow reltups update for X100 tables.
*/
static DB_STATUS
process_oldq(
OPQ_GLOBAL	*g,
...
f8		samplepct)
{
    ...
    OPQ_MAXELEMENT      prev_val, prev1_val;  /* histogram element of
					      ** cell boundary */
    i8             row_count;	      /* number of rows in the
...
# endif	/* x3B5 */
    ...
    */
    ...
    {
	if (relp->x100tab)
	    relp->nsample = relp->ntups * random_comp;
    ...
    */
	...
	else if (sample && (relp->x100tab))
	{
	    STprintf(stmt,
		"select %s from %s%s where randomf() <= %-15.7f order by 1",
		    selexpr,
		    sestbl,
		    relp->delimname,
		    random_comp,
...
# endif /* (xDEBUG) */
    ...
    ** We let X100 structure tables through as their reltups
    ** are not maintained during normal runtime.
    */
    if (row_count == 1 && !(relp->x100tab))
    {
        if (!supress_nostats)
        {
	    opq_error( (DB_STATUS)E_DB_WARN,
...
**     14-Mar-2011 (maspa05) b125183
**          Need to check tempt (use temp table) rather than sample (we're 
**          sampling) when deciding the table name since sampling is not
**          the only case where a temp table is used.
**	10-feb-2010 (dougi)
**	    Add (temporary) fix to compute row count in i8, then store
**	    a max of MAXI4, if more than 2B rows.
*/
static DB_STATUS
process_newq(
OPQ_GLOBAL	*g,
...
f8		samplepct)
{
    ...
    OPO_TUPLES		currval_count;	      /* count of rows with current
					      ** value, so far */
    i8			value_count;	      /* count (from FETCH) of current
					      ** column value */
    IISQLDA		*sqlda = &_sqlda;     /* ptr to SQLDA */
    bool		nulls_only = (bool)TRUE; /* assume all values are NULL,
						 ** until proven otherwise. */
	...
	** optimizedb overhead in processing the individual rows. */
	{
	    ...
	    {
		STprintf(stmt,
		"select %s, count(*) from %s%s where randomf() <= %15.7f group by 1 order by 1",
		    selexpr,
		    sestbl,
		    name,
		    random_comp,
...
# endif /* (xDEBUG) */
	...
	/* allocate buffer, describe location for the column. */
	{
	    ...
	    */
	    ...
	    sqlda->sqlvar[1].sqllen  = sizeof(i8);
...
**      01-Aug-2011 (maspa05) b125614
**          irows and ipages need to be i8s
...
char	    **argv)
{
	...
	/* Read column name */
	    ...
	    /* Copy column name */
	    colnamelen = pos-stlen2-namestart;
	    STncpy( cname, namestart, colnamelen);
	    cname[ colnamelen ] = '\0';
	    STtrmwhite(cname);
	}

	text3e = STchr(pos-stlen2, '(');/* Look for '(' */
	typenamelen = text3e - (pos-stlen2+8);
	STncpy(typename, pos-stlen2+8, typenamelen);
	typename[typenamelen] = '\0';
	STtrmwhite_DB(typename);

	/* check for valid datatype */
	s = adi_encode_colspec(g->opq_adfcb, typename, 0, NULL, 0,
			DB_UNSET_COLL, NULL, &typelen);
	if (s)
	{
	    opq_error((DB_STATUS)E_DB_ERROR,
		(i4)E_OP0941_TEXT_NOTFOUND, (i4)2,
	...
	/* Finish processing column atts */
	acnt = opq_scanf(text3e, TEXT3E,
			 g->opq_adfcb->adf_decimal.db_decimal, 
			 (PTR)&userlen, (PTR)&scale,
			 (PTR)nullable, (PTR)0, (PTR)0);
	if (acnt != 3)
	{
	    opq_error((DB_STATUS)E_DB_ERROR,
		(i4)E_OP0941_TEXT_NOTFOUND, (i4)2,
...
# endif /* xDEBUG */
		...
		*/
		...
		if (hexFlag)
		{
		    ...
		    */
		    ...
		    unpackedHex.db_collID   = DB_NOCOLLATION;
	...
	** add to end of histogram buffer */
	...
	insert_histo(g, *relp, *attrp, (OPO_TUPLES)inunique, exacthist, 
		     (OPN_PERCENT *)exactcount, (i4)icellno, (i8)irows,
		     (i8)ipages, (i4)ioverflow, (OPO_TUPLES)incount,
		     (bool)FALSE, (bool)TRUE, (char)iuflag, complete_flag,
		     (f8)irptfactor, (char *)version, (bool)TRUE, FALSE);

        /* 07-jun-1999 (chash01) convert code to equivalent routine */
	if (pgupdate)
	{
	    (VOID) update_row_and_page_stats(g, (OPQ_RLIST *)*relp, 
		(i8)irows, (i8)ipages, (i4)ioverflow);
	}
    }
...
**	    transaction.
**	15-feb-2010 (dougi)
**	    Always use "newq" for X100 tables - it's always faster, even for 
**	    unique columns.
*/
i4
main(
...
#define OPQ_DDTDROP "drop %s"
    ...
    */
	...
	if (badarglist(&opq_global, argc, argv, &parmno, ifs,
	    ...
	    &pgupdate, &samplepct, &supress_nostats, &opq_cflag, &nosetstats,
	...
	*/
		...
		(OPQ_INGRES | OPQ_INGRESVW | OPQ_STAR | OPQ_RMSGW))
	{
	    ...
	    */
	    if (opq_global.opq_dbms.dbms_type & (OPQ_INGRES | OPQ_INGRESVW))
	    {
			...
			*/
			...
			else
			{
				...
				*was_nothing_done = TRUE;
				...
				if (opq_global.opq_newquery || (*relp)->x100tab)
				    stat = process_newq(&opq_global, *relp, 
						attrp, sample, tempt, samplepct);
				else stat = process_oldq(&opq_global, *relp, 
						attrp, sample, samplepct);
...
**	13-jan-2010 (dougi)
**	    For X100 tables and sampling, use row count from relp.
*/
static 
DB_STATUS
    ...
    */
	...
	exec sql end declare section;
		...
		*/
        ...
        es1_rows = (i8)rows;
	if ((g->opq_sample) && (relp->x100tab) && relp->ntups > 0)
	    es1_rows = relp->ntups;
