...
**      01-Aug-2011 (maspa05) bug 125614
**          Added support for %lld for i8 values. Rows and Pages values are now
**          i8s 
**      21-Jul-2011 (hanal04) Bug 125543
**          If we consume a string and we are not in sync with the format
**          specified consume strings until we match the next character
**          in the format string. We may have a multistring, for example
**          a datatype of 'time with time zone'. Code in opqoptdb.sc
**          Makes this change redundant in main for datatypes, but
**          this may be useful for other multiword instances and the
**          other change can not be easily crossed back to earlier
**          codelines as it is part of a mass integration from Code V.
*/
...
# define	OPQFLTLNGLNG	024
# define	OPQINTSRT	00
# define	OPQINTREG	01
# define	OPQINTLNG	02
# define	OPQINTLNGLNG	04
	...
	case '%':
	    ...
	    */
		...
		nmatch++;
            ...
            if (!lenspec && (f[0] == 'c' || f[0] == 's') && !endfile && ptr)
            {
                /* If the next character in the frmat specifier is not
                ** a format specifier and it does not match the next character
                ** to be consumed we may have a multistring. Type names
                ** such as timestamp with timmezone for example. Consume
                ** the rest of the multistring.
                */
                char	*next_fmt = buf;
                char    next_word[DB_GW1_MAXNAME+1];
                char    *next_wordp = next_word;
                ...
                while(!endfile)
                {
                    ...
                    if(*next_fmt == '%')
                        break;
                    buf++; /* Skip over the space  that end opq_innum() */
                    if((*fmt == EOS) || (*next_fmt == *(fmt + 1)))
                        break;

	            if (opq_innum(&next_wordp, f[0], len, size, &buf,
		        &endfile, decimal) && ptr)

                    STcat(*ptr, " ");
                    STcat(*ptr, next_wordp);
                    next_fmt = buf;
                }
            }
