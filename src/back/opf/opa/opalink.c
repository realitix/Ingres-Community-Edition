...
**	17-mar-2011 (dougi)
**	    Don't link when grouping column is SAGG result in X100 query.
**	14-apr-2011 (dougi)
**	    In fact, don't link on constants at all in X100.
**	4-july-2011 (dougi) m2196
**	    Replace pst_resolve() by opa_gresolve().
...
opa_modqual(
	...
	OPS_SUBQUERY       *outer)
{
    PST_QNODE           *bylist, *nodep;  /* used to traverse the bylist of the
				    ** inner aggregate
				    */
	...
	/* Check for annoying SAGG result and skip. */
	if ((global->ops_gmask & OPS_X100) &&
	    (nodep = bylist->pst_right) != (PST_QNODE *) NULL &&
	    ...
	    ** by setting maps in opz_ctree correctly */
	    ...
	    equals_node->pst_right = bylist->pst_right; /* use the same
            ...
            opv_copytree(global, &equals_node->pst_right); /* copy tree since
                                        ...
                                        */
	    opa_gresolve(global, &and_node->pst_left); /*equals_node*/
	}
    }
}

