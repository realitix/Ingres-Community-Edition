...
**	    Updated call interface opa_resolve and opa_gresolve reflecting
**	    pst_resolve change.
**/
...
**	    bug 120633.
**	2-june-2011 (dougi) m1970
**	    Don't copy preds to SAGG.
*/
static VOID
opa_qview(
        OPA_RECURSION      *gstate)
{
        ...
        )
    {
            ...
            )
        {
            ...
            if (subquery->ops_sqtype != OPS_SAGG && qual
                &&
...
**	25-oct-2010 (dougi)
**	    Lingering bug from fiddling with PST_QNODE type def.
*/
static VOID
opa_sview(
        OPS_STATE          *global,
...
if (opt_strace(global->ops_cb, OPT_F001_DUMP_QTREE))
    ...
    {   /* copy the var node if possible */
        MEcopy((PTR)expr->pst_right, 
            (sizeof(PST_VAR_NODE)+sizeof(PST_QNODE)-sizeof(PST_SYMVALUE)),
            (PTR)(*viewpp));
    }
    else
    {   /* copy the expression */
        ...
        PST_QNODE          *root)
{
    ...
    PST_QNODE		*rsdmp;	    /* RESDOM ptr to detect window funcs */
    ...
    /* Check if this view has window functions in the RESDOM list. */
    for (winfuncs = FALSE, rsdmp = viewp->pst_left;
	!winfuncs && rsdmp != NULL && rsdmp->pst_sym.pst_type == PST_RESDOM;
    ...
    {   /* if unions exist in view then it cannot be substituted
        ** so create a global range variable for it */
        ...
        (VOID)opv_parser(global, gvar, (winfuncs) ? OPS_WINDOW : OPS_VIEW, 
            FALSE,                  /* no RDF info */
		...
		if (new_subquery->ops_sqtype != OPS_WINDOW)
		{
                    new_subquery->ops_sqtype = OPS_VIEW;
		    if (!mixture && oj_view &&
			viewp->pst_sym.pst_value.pst_s_root.pst_union.
							pst_dups == PST_NODUPS)
...
**      17-May-2011 (smeke01) b125365
**          Back in the dim & distant past someone forgot to set pst_rgparent.
[@history_line@]...
[@history_template@]...
*/
OPV_IGVARS
        ...
        bool               copytarget)
{
    ...
    MEfill(sizeof(PST_QNODE), 0, mainp);
    ...
    uvp->pst_rgroot = rootp;                /* use the
                                            ...
                                            ** for this view */
    uvp->pst_rgparent = -1;     /* this entry has no parent */
...
**	16-april-2009 (dougi)
**	    Save possible aggregate name for X100 scoping issue.
*/
static VOID
opa_rnode(
...
*/
    ...
    /* Check if this view has window functions in the RESDOM list. */
    for (winfuncs = FALSE, rsdmp = q->pst_left;
	!winfuncs && rsdmp != NULL && rsdmp->pst_sym.pst_type == PST_RESDOM;
	rsdmp = rsdmp->pst_left)
     if (IS_A_PST_OP_NODE(rsdmp->pst_right->pst_sym.pst_type) &&
	(rsdmp->pst_right->pst_sym.pst_value.pst_s_op.
    ...
    if (winfuncs && global->ops_subquery != NULL)
    {
	new_subquery->ops_sqtype = OPS_WINDOW;
	global->ops_qheader->pst_mask1 |= PST_WINDOW_IN_QUERY;
    }
...

/*{
** Name: opa_gresolve    - resolve data types for a newly created node
**
** Description:
**      Resolve data types for a newly created OP node. Uses OPS_STATE
**	instead of OPS_SUBQUERY, 'cuz there are places in opa where the
**	function is needed before any subqueries have been identified
**
** Inputs:
        ...
        PST_QNODE          **nodep)
{
    ...
    */
    CLRDBERR(&psqCb.psq_error);
    psqCb.psq_adfcb = (PTR)global->ops_adfcb;
    psqCb.psq_qlang = global->ops_qheader->pst_qtree->
    ...
    */
    psqCb.psq_qnodep  = nodep; 
    psqCb.psq_qsfcb  = &global->ops_qsfcb;
...
**          Recurse over subselects. Nested subselects that contain
**          a correlated aggregate inside an ifnull should not be flattened.
[@history_line@]...
[@history_template@]...
*/
static bool
        ...
        PST_QNODE       **ifnull_aop)
{
    ...
    {   /* iterate down right side and recurse down left */
        ...
        case PST_SUBSEL:
            break;                      /* Recurse over subselects. Whilst
                                        ** a subselect has its' own scope
                                        ** and should therefore be able to
                                        ** have correlated values, flattening
                                        ** of nested subselects containing
                                        ** a correlated aggregate inside an 
                                        ** "ifnull" still causes problems.
                                        ** Bug 124863.
                                        */
...
**	21-oct-2009 (dougi)
**	    Allow flattening when subq is a single value aggregate.
**	23-dec-2009 (dougi)
**	    Detect in/exists/= any nesting.
*/
static bool
opa_flatten(
        OPA_RECURSION      *gstate,
        ...
        PST_QNODE          *conjunct)
{
    ...
    /* No flattening requested - but check first that subq isn't a single 
    ** value aggregate. They're safe. */
    if (global->ops_gmask & OPS_QUERYFLATTEN) 
/* Following chunk was added to handle small flattening case, but broke
   something else. So it is being removed for the time being (Nov. 25, 2009).
	&&
	!(subselect->pst_sym.pst_type == PST_SUBSEL &&
	subselect->pst_left && subselect->pst_left->pst_right &&
	subselect->pst_left->pst_right->pst_sym.pst_type == PST_AGHEAD &&
	subselect->pst_left->pst_right->pst_left &&
	subselect->pst_left->pst_right->pst_left->pst_sym.pst_type == PST_AOP))
*/
...
\p\g
    ...
    /* Once we're here, we are going to flatten - check for nesting. */
    if (father->ops_mask2 & OPS_UNDER_INEXANY)
	father->ops_mask2 |= OPS_NESTED_INEXANY;

    father->ops_mask2 |= OPS_UNDER_INEXANY;
	...
	** pst_dups setting on the root. */
	...
	if (conjunct->pst_sym.pst_value.pst_s_op.pst_opmeta != PST_CARD_01R &&
        ...
        opa_resolve (father, &conjunct); /* re-resolve the datatypes
                            ** for this new node */
	subselect->pst_left = conjunct; /* Reset if updated */
        if( subselect->pst_right 
            && 
    ...
    if (insertpp
        ...
        ** point */
        opa_ojflatten(gstate, insertpp, 
            (PST_J_ID)((joinid > PST_NOJOIN)?joinid : PST_NOJOIN));
    father->ops_mask2 &= ~OPS_UNDER_INEXANY;	/* turn off temp flag */
    return (TRUE);
}

/*{
...
**	9-sep-2009 (dougi)
**	    Treat "exists" of noncorrelated the same for X100 (no ANY) as for
**	    distributed.
**	23-Mar-2010 (horda03) B123416
                                        ...
                                        */ )
{
    ...
    */
    ...
    {   /* create PST_BOP for exact subselect */
        ...
        if ((distributed || x100) && (!corelated) && exists)
    ...
    opa_resolve( gstate->opa_gfather, agg_qnode); /*exists_ptr*/
    return(FALSE);
}

/*{
...
**          Changes for range table expansion.
**	7-july-2009 (dougi)
**	    Assign "name" to RESDOMs of "group by" expressions (only useful
**	    for X100 queries).
**	14-Jun-2011 (kiria01) m2106
**	    Call common routine to fixup name as prior check missed degenerate
**	    grouping expressions that started off without name but ended up
**	    replaced by equivalent VAR.
*/
static VOID
opa_byhead(
        OPA_RECURSION      *gstate,
...
{   /* update the bit map for the by list of the aggregate */
    ...
    /* Before anything else, run through the RESDOMs looking for "group by"
    ** expressions. Generate names for them. */
    for (rsdmp = (*agg_qnode)->pst_left; 
		rsdmp && rsdmp->pst_sym.pst_type == PST_RESDOM;
		rsdmp = rsdmp->pst_left)
	opv_norm_resdom(gstate->opa_global, rsdmp, FALSE);
...
**          Increase tuple size project.
**	15-dec-2009 (dougi)
**	    If RESDOM owns a PST_VAR that maps to a view or derived table, 
**	    Flag the RESDOM and set range var.
*/
static VOID
opa_presdom(
        OPA_RECURSION      *gstate,
...
**          Changes for range table expansion.
**	16-april-2009 (dougi)
**	    Save possible aggregate name for X100 name resolution.
*/
static VOID
opa_var(
        OPA_RECURSION         *gstate,
    ...
    */
        ...
        )
    {
        ...
        opa_sview(global, agg_qnode);   /* view has been found so
                                    ...
                                    ** list item */
        if (gstate->opa_joinid > PST_NOJOIN)
...
**	12-Jan-2011 (smeke01) b124837
**	    Constant folding of a bop can result in it becoming a single
**	    node Boolean constant (ie TRUE or FALSE). As such its pst_left
**	    and pst_right will be NULL. Add code to prevent de-reference
**	    of NULL.
*/
static bool
opa_bop(
        ...
        gstate->opa_gflatten)
    {
	    ...
	    ** join operators will check cardinality where needed */
        ...
        {   /* convert the ALL into a non-existence aggregate ANY */
            ...
            if (q->pst_right
                &&
                (q->pst_right->pst_sym.pst_dataval.db_datatype < 0)
                &&
...
#endif
            {
                ...
                {   /* if local then b34433 shows that a PST_VAR Node in the expression
                    ...
                    ** which fix this IS NULL expression problem  FIXME */
                    PST_QNODE       *qnode;
                    if (q->pst_left && (q->pst_left->pst_sym.pst_dataval.db_datatype < 0))
                    {
                    ...
                    else if (q->pst_right && (q->pst_right->pst_sym.pst_dataval.db_datatype < 0))
...

/*{
** Name: opa_byhead_check - check under AGHEAD for BYHEAD with nothing but
**	aggregates. They are effectively constants and the BYHEAD can be
**	dropped
**
** Description:
**
        ...
        PST_QNODE	**agg_qnode)
{
    ...
    /* There's not much to this one. Just check for BYHEAD under AGHEAD, 
    ** then loop down RESDOMs. If they're all constants or agheads, drop
    ** the BYHEAD. */

    for (nodepp = &(*agg_qnode)->pst_left; (nodep = (*nodepp)) &&
	nodep->pst_sym.pst_type == PST_RESDOM; nodepp = &(*nodepp)->pst_left)
     if (nodep->pst_right && nodep->pst_right->pst_sym.pst_type == PST_AGHEAD)
     {
	/* Replace by constant. */
	nodep->pst_right = opv_i1const(global, 1);
	STRUCT_ASSIGN_MACRO(nodep->pst_right->pst_sym.pst_dataval, 
...
**	14-apr-2011 (dougi) m1891
**	    byhead_check comes after opa_rnode() call, not before.
*/
static VOID
opa_generate(
        OPA_RECURSION       *gstate,
                                        ...
                                        */ )
{
    ...
    ** problems for long resdom lists etc. */
    for(;*agg_qnode; agg_qnode = &(*agg_qnode)->pst_left)
    {
	    ...
	    /* Tiny heuristic - if BYHEAD addresses an AGHEAD, bylist is
	    ** effectively constant and can be dropped. */
	    if ((*agg_qnode)->pst_left->pst_sym.pst_type == PST_BYHEAD &&
		(gstate->opa_global->ops_gmask & OPS_X100))
		opa_byhead_check(gstate->opa_global, &(*agg_qnode)->pst_left);
...
**	29-nov-2010 (dougi)
**	    Fine tune flattening decisions (for IVW).
**	16-Feb-2011 (kiria01)
**	    Ensure flattening includes derived tables, views and with elements.
**	4-May-2011 (kschendel)
**	    Drop the subsel-oj flag.  We're not getting gotone back reliably
**	    for some reason, and there's a better fix in opn anyway.
*/
VOID
opa_1pass(
        OPS_STATE          *global)
    ...
    */
    ...
    }
    {
        ...
        if (!(global->ops_cb->ops_smask & OPS_MDISTRIBUTED) /* unless
            ...
            (
                (   global->ops_cb->ops_check 
                    ...
                    opt_strace( global->ops_cb, OPT_F004_FLATTEN) /* or
                                            ...
                                            ** turned ON */
		    &&
		    !(global->ops_gmask & OPS_X100) /* but not for X100 */
                )
                ||
			...
			||
		     !(global->ops_gmask & OPS_X100_ONLY))
                )
                ||
                (
...
# endif
    ...
    opt_pt_entry(global, global->ops_qheader, global->ops_qheader->pst_qtree, 
                            "after notex transform", FALSE, FALSE, FALSE);

    /* If this is an X100 query, check for aggregates that need to be
    ** transformed (any() and the stat aggs for now). */
    if (global->ops_gmask & OPS_X100)
    {
	PST_QTREE *qtree = global->ops_qheader;
	i4 i;
	for (i = 0; i < qtree->pst_rngvar_count; i++)
	{
	    PST_RNGENTRY *rng = qtree->pst_rangetab[i];
	    if (rng &&
		rng->pst_rgroot && (
	    ...
	    {
		opa_x100_xform(global, &rng->pst_rgroot);
	    }
	}
	opa_x100_xform(global, &qtree->pst_qtree);

	if (global->ops_cb->ops_check && opt_strace(global->ops_cb, 
			OPT_F042_PTREEDUMP))    
	    opt_pt_entry(global, qtree, qtree->pst_qtree,
			"after X100 transform", FALSE, FALSE, FALSE);
    }
