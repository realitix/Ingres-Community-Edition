	...
	** !(newmap & usedmap)". */
                                        ...
                                        ** those variables entirely */
	{
	    ...
	    ** if it exists (and if it is not the main query).  This
	    ...
	    /* Before checking OJs, see if they're all weird ones which
	    ** don't need it. */
	    if (outer->ops_global->ops_qheader->pst_numjoins > 0)
	    {
		oddojs = 0;
		if (outer->ops_global->ops_goj.opl_antijmap)
		    oddojs = BTcount((char *)outer->ops_global->ops_goj.
			opl_antijmap, outer->ops_global->ops_goj.opl_glv+1);
		if (outer->ops_global->ops_goj.opl_semijmap)
		    oddojs += BTcount((char *)outer->ops_global->ops_goj.
			opl_semijmap, outer->ops_global->ops_goj.opl_glv+1);
		if (outer->ops_global->ops_goj.opl_subqjmap)
		    oddojs += BTcount((char *)outer->ops_global->ops_goj.
			opl_subqjmap, outer->ops_global->ops_goj.opl_glv+1);
		if (outer->ops_global->ops_qheader->pst_numjoins > oddojs)
		    checkojs = TRUE;
	    }
