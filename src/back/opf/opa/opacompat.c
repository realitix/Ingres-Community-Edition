...
**	29-Jun-2011 (kschendel) SIR 125253
**	    Add a couple WINDOW tests (codev merge).
...
**	9-apr-86 (seputis)
**          initial creation
**	03-May-2011 (smeke01) b125025
**          Combined improvements to trace point op147.
[@history_line@]...
*/
static OPS_SUBQUERY *
opa_sameagg(
...
# ifdef OPT_F019_AGGCOMPAT
		...
		TRformat(opt_scc,
		    ...
		    "\nAggregate in subquery %d (0x%p,%d,%d) on the concurrent list matches candidate aggregate\n\n",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry);
	    }
# endif
	    return (subquery);
	}
...
# ifdef OPT_F019_AGGCOMPAT
	if (opt_strace( global->ops_cb, OPT_F019_AGGCOMPAT ) )
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nNo match found for candidate aggregate to any aggregate in a subquery already on the concurrent list.\n");
# endif
...
# ifdef OPT_F019_AGGCOMPAT
	if (opt_strace( global->ops_cb, OPT_F019_AGGCOMPAT ) )
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSimple aggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) are IDENTICAL\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
    }
    else
    {
...
# ifdef OPT_F019_AGGCOMPAT
	if (opt_strace( global->ops_cb, OPT_F019_AGGCOMPAT) )
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSimple aggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) may be run together, adding to aggregate list\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
        {
...
# ifdef OPT_F019_AGGCOMPAT
	if (global->ops_cb->ops_check 
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nFunction aggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) are IDENTICAL\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
...
# ifdef OPT_F019_AGGCOMPAT
	if (global->ops_cb->ops_check
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nAdd unmatched function aggregate in subquery %d (0x%p,%d,%d) to list for concurrent processing.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry);
# endif
    }
...
# ifdef OPT_F019_AGGCOMPAT
	...
	TRformat(opt_scc,
	    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    "\nSubselects in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) are IDENTICAL\n",
	    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
	    header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
    subquery->ops_agg.opa_redundant = TRUE; /* set flag indicating that
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
	{
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nCandidate aggregate in subquery %d (0x%p,%d,%d) contains nested aggregates, so exclude it.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry);
	}
# endif
	return(FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSubqueries %d (0x%p,%d,%d) and %d (0x%p,%d,%d) are of types that cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE); 
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nCorrelation variables in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) do not match so cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE); 
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nFrom lists of subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) do not match so cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE); 
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nOne or both subqueries %d (0x%p,%d,%d) and %d (0x%p,%d,%d) are unions so cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE); 
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nDuplicate handling for subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) does not match, so cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE); 
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nAggregate combining switched off: so subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) cannot be combined.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
           return(FALSE); 
...
/*Profound check if distinct and non distinct aggregates are together*/
        ...
        if(subquery->ops_agg.opa_aop->pst_sym.pst_value.pst_s_op.pst_distinct == PST_DISTINCT)
	{
...
# ifdef OPT_F019_AGGCOMPAT
		...
		TRformat(opt_scc,
		    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		    (i4)sizeof(global->ops_trace.opt_trformat),
		    "\nAggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) cannot combine due to PRIME/NONPRIME rules.\n",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		    header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
...
# ifdef OPT_F019_AGGCOMPAT
		...
		TRformat(opt_scc,
		    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		    (i4)sizeof(global->ops_trace.opt_trformat),
		    "\nAggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different projection semantics and so cannot combine.\n",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		    header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
...
# ifdef OPT_F019_AGGCOMPAT
		...
		TRformat(opt_scc,
		    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		    (i4)sizeof(global->ops_trace.opt_trformat),
		    "\nSubquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) do not range over same variables, so cannot combine.\n",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		    header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	    return(FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
		...
		TRformat(opt_scc,
		    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		    (i4)sizeof(global->ops_trace.opt_trformat),
		    "\nSubquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different qualification lists, so cannot combine.\n",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		    header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	    return(FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nAggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different by lists, so cannot combine.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSubselects in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different target lists, so cannot combine.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	return(FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nAggregates in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different duplicate handling, so cannot combine.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
		return (FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSubquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) are counting different things, so cannot combine.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
	    return (FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (trace)
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\nSubselects in subquery %d (0x%p,%d,%d) and subquery %d (0x%p,%d,%d) have different oj ids, so cannot combine.\n",
		subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry,
		header->ops_tsubquery, header, header->ops_sqtype, header->ops_gentry);
# endif
       return (FALSE);
...
# ifdef OPT_F019_AGGCOMPAT
	if (opt_strace( global->ops_cb, OPT_F019_AGGCOMPAT ) )
	{
		...
		(i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		"\n\nCandidate identical %s in subquery %d (0x%p,%d,%d) is ...\n\n",
		(*previous)->ops_sqtype == OPS_SELECT?"subselect":"aggregate",
		(*previous)->ops_tsubquery, *previous, (*previous)->ops_sqtype, (*previous)->ops_gentry);
...
**	03-May-2011 (smeke01) b125025
**          Combined improvements to trace point op147.
**          Remove unused OPT_F003_AGGREGATE and OPT_AGLOOP.
*/
VOID
opa_compat(
	OPS_STATE          *global)
...
*/
	...
	if ((subquery->ops_sqtype != OPS_VIEW)
	    ...
	    &&
	    (subquery->ops_sqtype != OPS_WINDOW)
	    )
	{
# ifdef OPT_F019_AGGCOMPAT
	    if (opt_strace( global->ops_cb, OPT_F019_AGGCOMPAT ) )
		...
		TRformat(opt_scc,
		    (i4 *)global, (char *)&global->ops_trace.opt_trformat[0],
		    (i4)sizeof(global->ops_trace.opt_trformat),
		    "\n\nLooking for matches for %s in subquery %d (0x%p,%d,%d)\n\n",
		    subquery->ops_sqtype == OPS_SELECT?"subselect":"aggregate",
		    subquery->ops_tsubquery, subquery, subquery->ops_sqtype, subquery->ops_gentry);
