...
**	    Updated call of to opa_resolve reflecting pst_resolve change.
**/
...
**	21-dec-2009 (dougi)
**	    Remove dups from BY-list (they are troublesome for X100).
**	17-mar-2011 (dougi)
**	    Look for group by on constants in X100 queries.
**	14-apr-2011 (dougi) m1880
**	    Forgot to test above for X100 only.
*/
static bool
opa_resdom(
	OPS_SUBQUERY       *subquery,
...
{   /* place all the resdom nodes associated with the aggregate 
	...
	** also skip over any byhead nodes or aop nodes */
	...
	for (resdompp = &subquery->ops_root->pst_left; *resdompp;)
	{
		...
		if (x100 && byexpr_found && 
		    (constp = (*resdompp)->pst_right)->pst_sym.pst_type == 
				PST_CONST &&
		    constp->pst_sym.pst_value.pst_s_cnst.pst_tparmtype == 
				PST_RQPARAMNO)
		{
		    (*resdompp)->pst_right = opv_i1const(subquery->
						ops_global, 1);
		    STRUCT_ASSIGN_MACRO((*resdompp)->pst_right->pst_sym.
			    pst_dataval, (*resdompp)->pst_sym.pst_dataval);
		}
		resdompp = &(*resdompp)->pst_left;
		break;
		...
		/* There was code here to check for dup entries in the BY-list
		** in X100 queries. This is now done in the cross compoiler. */
...
*/
	...
	/* A tiny bug can result in failure of X100 queries here because
	** the name of an aggregate result in a view or derived table gets
	** lost in the woodwork. If this is an X100 query and if this 
	** subquery represents a derived table or view, try to fix it up. */
    }
...
**	4-jan-2010 (dougi)
**	    Produce distinct TID rsname.
**	18-Mar-2010 (kiria01) b123438
**	    Initialise .db_data otherwise can break op170 output.
**	9-dec-2010 (dougi)
**	    Dang. Forgot to copy distinct TID name to corresponding VAR node.
*/
VOID
opa_tnode(
	OPS_STATE          *global,
	...
	PST_QNODE          *root)
{
    ...
    resdomp->pst_sym.pst_value.pst_s_rsdm.pst_rsno =  rsno; /* assign
				...
				** element */
    resdomp->pst_sym.pst_value.pst_s_rsdm.pst_rsflags |= PST_RS_TID;
    resdomp->pst_left = root->pst_left;
    root->pst_left = resdomp;   /* attach non printing resdom
				** to the target list */
    MEcopy(tidname, sizeof(resdomp->pst_sym.pst_value.pst_s_rsdm.pst_rsname),
	(char *)&resdomp->pst_right->pst_sym.pst_value.pst_s_var.pst_atname);
}

/*{
...
**	23-feb-2010 (dougi)
**	    No "tid" for inners of anti/semi-joins.
**	19-nov-2010 (dougi)
**	    No "tid" for inners of subq-join, either.
**	25-feb-2011 (dougi)
**	    Check if TID is there before adding it.
*/
VOID
opa_stid(
	OPS_SUBQUERY       *subquery)
...
*/
    ...
    ** aggregation, so we'll get rid of it here. */
    ...
    {	/* FIXME - check if tid is not already in target list */
        OPV_GRV		*gvarp;
	PST_RNGENTRY	*rngp;
	PST_J_ID	antisemi_id;
	bool		skipit;

	/* If there are semi-, anti- or subq-joins, make sure this table isn't
	** inner to one, otherwise we don't want TID in RESDOM list. */
	if (global->ops_goj.opl_antijmap || global->ops_goj.opl_semijmap ||
		global->ops_goj.opl_subqjmap)
	{
	    rngp = global->ops_qheader->pst_rangetab[gvar];
	    for (antisemi_id = -1, skipit = FALSE; 
		!skipit && (antisemi_id = BTnext((i4)antisemi_id,
		(char *)&rngp->pst_inner_rel, sizeof(PST_J_MASK))) >= 0; )
	     if (global->ops_goj.opl_antijmap && 
		BTtest((i4)antisemi_id, (char *)global->ops_goj.opl_antijmap) ||
		global->ops_goj.opl_semijmap &&
		BTtest((i4)antisemi_id, (char *)global->ops_goj.opl_semijmap) ||
		global->ops_goj.opl_subqjmap &&
		BTtest((i4)antisemi_id, (char *)global->ops_goj.opl_subqjmap))
		skipit = TRUE;
	...
	/* The tid might already be in the RESDOM list - check first. */
	for (rsdmp = subquery->ops_root->pst_left, found = FALSE;
		!found && rsdmp && rsdmp->pst_sym.pst_type == PST_RESDOM;
		rsdmp = rsdmp->pst_left)
	 if (rsdmp->pst_right && 
	     rsdmp->pst_right->pst_sym.pst_type == PST_VAR &&
	     rsdmp->pst_right->pst_sym.pst_value.pst_s_var.pst_vno == gvar &&
	     rsdmp->pst_right->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id == DB_IMTID)
	    found = TRUE;
	...
	PST_QNODE          *countp)
{
    ...
    *qualpp = andp;
    ...
    opa_resolve(subquery, &andp->pst_right); /*equalp*/
}

/*{
...
**	24-dec-2009 (dougi)
**	    Remove previous change - it subverts some required dups elimination.
*/
static VOID
opa_recorelated(
	OPS_SUBQUERY       *subquery)
	...
	** so include all aggregate expressions in the map */
	...
	{   /* found var to substitute so search for non-corelated match */
		...
		{   /* found equi-join, so substitute variable */
                    STRUCT_ASSIGN_MACRO (
                        qnodep2->pst_left->pst_right->pst_sym.pst_value.pst_s_var,
                        qnode->pst_right->pst_sym.pst_value.pst_s_var);
		    if (cui_compare(sizeof(DB_ATT_NAME),
				qnode->pst_right->pst_sym.pst_value.pst_s_var.pst_atname.db_att_name,
				1, " ") == 0)
			MEcopy((char *)&qnode->pst_right->pst_sym.pst_value.
			    pst_s_var.pst_atname, sizeof(DB_ATT_NAME), 
			    (char *)&qnode->pst_sym.pst_value.pst_s_rsdm.
						pst_rsname);
                    /* bug 47541 */
                    STRUCT_ASSIGN_MACRO (
...
**	    to RFAGG.
**	11-dec-2008 (dougi)
**	    Another convertible FAGG to RFAGG.
**	21-dec-2009 (dougi)
**	    Remove duplicate BY-list entries.
*/
VOID
opa_final(
	OPS_STATE          *global)
...
/* 	    subquery->ops_duplicates = opa_duplicate(subquery);*/
 ...
 set; check b32738 
	    ...
	    /* Look for specific situations in which FAGG can be changed
	    ** to RFAGG (more efficient). conditions are:
	    ** - this is 1st sq or this is 2nd sq and 1st is a SAGG,
	    ** - next sq is MAIN or bext is SAGG and the following one
	    ** is MAIN,
	    ** - this is not projection and is a virtual table.
	    */
	    if ((subquery == global->ops_subquery || 
		(global->ops_subquery->ops_sqtype == OPS_SAGG &&
		subquery == global->ops_subquery->ops_next)) &&
			subquery->ops_next && 
		...
		** bylist values - add sort list for evaluation FIXME */
		...
		/* Check for X100 - PROJ flattening isn't allowed, so this
		** is effectively a failed SE join. */
