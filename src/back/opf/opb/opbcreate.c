...
**	    (Try to) fix more bugs with is null, ifnull, and nvl2 and union
**	    views.  Delete opb_cbf stub;  constant folding is done in the
**	    parser now, so it's easy to recognize degenerate true or false
**	    where-clauses here.
...
**          (b119881)
**	2-Mar-2011 (kschendel for dougi)
**	    Repair zhi's change to use ops-next, not ops-onext.  The former
**	    is the real "next" chain, the latter is something bogus which
**	    apparently is mainly for set qep.
**	    Tag IFNULL fragments as well as ISNULL fragments.
**	    Add some comments as to what's up with MAPACTIVE.
*/
static VOID
opl_bfnull(
	OPS_SUBQUERY	*subquery, 
...
{   /* this subtree has operators which depend on NULL
    ...
    ** operations */
    ...
    if (subquery->ops_sqtype == OPS_UNION &&
        subquery->ops_oj.opl_smask & (OPL_ISNULL|OPL_IFNULL) )
    {
       uv_p = subquery->ops_next; 
    ...
    if (union_view)
    {
	    ...
	    **
	    ** [kschendel] Note that if the var has no special-eqc, as
	    ** is likely in the union situation, no ojmask bits are set
	    ** and ojid may stay -1.  In that case, the code in opb-gbfbm
	    ** (in opnsm2) says "can be evaluated in a subtree", because
	    ** MAPACTIVE is set but ojmap (being empty) is always a subset
	    ** of lojevalmap.  Thus the bf gets pushed down the tree until
	    ** a leaf is reached, and then opnsm1 says "no MAPACTIVE's
	    ** can be evaluated at a leaf".  So, the isnull will be
	    ** suppressed entirely, seemingly by accident!
	    ** I strongly suspect that for the no-special-eqc case, we
	    ** could simply dump the BF here, but I'm not about to try
	    ** it without a lot more understanding and testing!
            */
...
**		nodep->ADI_F32768_IFNULL
**	3-Mar-2011 (kschendel) b125146
**	    Make sure that the ifnull flag is set for the analysis of the
**	    first ifnull-like function arg.  For MOP's, that's the RIGHT
**	    arg, not the left one.  Also, test global OJ-ness for setting
**	    opl-ifnull, not local OJ-ness.  The latter isn't good enough
**	    for union views as an OJ inner.
**	9-Mar-2011 (kschendel) m1720
**	    Correction to above, and/or has no fidesc!
*/
static VOID
opb_isconst(
	OPS_SUBQUERY       *subquery,
	...
	bool		   not_first)
{
    ...
    {
	if (subquery->ops_oj.opl_smask & (OPL_ISNULL | OPL_IFNULL) )
	...
	*novars = FALSE;		/* var is found so indicate that this
    ...
    {
	bool real_op_node;
	PST_QNODE **firstp, **restp;

	/* IS_A_PST_OP_NODE includes AND and OR since they use the pst_s_op
	** variant;  exclude and/or from fidesc testing!
	*/
	real_op_node = FALSE;
	if (IS_A_PST_OP_NODE(root->pst_sym.pst_type)
	  && root->pst_sym.pst_type != PST_AND
	  && root->pst_sym.pst_type != PST_OR)
	    real_op_node = TRUE;
        if (real_op_node
	...
	/* Analyze first arg first:  if it's an ifnull-like function, the
	** first arg is the one being tested for null-ness and it's the one
	** with special OJ implications.
	** If it's not an op at all, just recurse down the tree.
	*/
	firstp = &root->pst_left;
	restp = &root->pst_right;
	if (*firstp)
	    opb_isconst(subquery, bfp, firstp, novars, TRUE);
	if (ifnulltest)
	    subquery->ops_oj.opl_smask &= (~OPL_IFNULL);
	if (*restp)
	    opb_isconst(subquery, bfp, restp, novars, TRUE);
    }
}

/*{
...

/*{
** Name: opb_saggck	- check predicate parse tree for presence of 
**	SAGG const parms
**
** Description:
**      For X100 queries, check for presence of SAGG results referenced
**	from within predicate. Cross compiler needs them to NOT be treated
**	as constant BFs.
**
** Inputs:
**      node				pre to parse tree fragment
**	numparm				global->ops_qheader->pst_numparm
**
** Outputs:
**	Returns:
**	    TRUE	- if SAGG reference is present
**	    FALSE	- otherwise
**	Exceptions:
**	    none
**
...
**      31-mar-2010 (dougi)
**	    Written for IVW.
**	15-Mar-2011 (kschendel) m1541
**	    Add "not really a repeat param" test.
*/
static bool
opb_saggck(
	PST_QNODE	*node,
...

{
    ...
    /* Simply recurse in search of PST_CONST node with pst_tparmtype = 
    ** PST_RQPARAMNO and parm-no larger than any parsed param number. */

    for ( ; ; )
    {
	if (node->pst_sym.pst_type == PST_CONST &&
	    node->pst_sym.pst_value.pst_s_cnst.pst_tparmtype == PST_RQPARAMNO &&
	    node->pst_sym.pst_value.pst_s_cnst.pst_parm_no > numparm)
	    return(TRUE);

	if (node->pst_right && opb_saggck(node->pst_right, numparm))
	    return(TRUE);
...
**	3-Mar-2011 (kschendel) b125146
**	    Don't leave OPL_ISNULL set if we turned it on for the bf.  Probably
**	    doesn't matter, but be tidy.  Don't allow isnull to be "turned
**	    on" if it's already on, would get turned off too soon.  (Original
**	    code checked this, but case addition didn't.)  Deal with MOP
**	    properly (mostly for NVL2).
	...
	bool		   *complex)
{
	...
	** check for outer join ID in all PST_OP_NODEs */
	...
	case PST_BOP:
	case PST_UOP:
	    /* See if we need to turn on ISNULL checking.  (Don't need to if
	    ** it's already on for this fragment from higher in the tree.)
	    ** IFNULL-like functions are detected in opb-isconst.
	    */
	    if ((subquery->ops_oj.opl_smask & OPL_ISNULL) == 0)
	    {
		if (
	...
	*complex = TRUE;		    /* nested AND ==> complex disj. */
	...
	opno1 = root->pst_sym.pst_value.pst_s_op.pst_opno; /* operator id of
					    ** the node */
	isnull =(opno1 == global->ops_cb->ops_server->opg_isnull);
	if (isnull)
	...
	/* Prevent X100 SAGG compares from being constants. */
	if ((global->ops_gmask & OPS_X100) &&
	    opb_saggck(root, global->ops_qheader->pst_numparm))
	{
	    bfp->opb_mask |= OPB_NOTCONST;
	    break;
	}
	...
	*/
	...
	if (opno != global->ops_cb->ops_server->opg_eq)
	...
	*/
	...
	if ( (complement != ADI_NOFI)  /* first check if this
             ...
             &&
             !((varbopconst_or_constbopvar) &&
               (opno == global->ops_cb->ops_server->opg_ne) &&
               (constant->pst_sym.pst_value.pst_s_cnst.pst_pmspec != PST_PMNOTUSED)
              )
             &&
             (opno != global->ops_cb->ops_server->opg_notlike)
           ...
           )
	{
	    ...
	    ** key build but will help execution. */
		...
		while (*p)
		{
		    ...
		    {
			i4 cmp;
			if (adc_compare(global->ops_adfcb,
...
notsorted:	/* Reset constant in case list rearranged */
		...
		** that might otherwise fail through resource needs. */
		if (global->ops_cb->ops_alter.ops_inlist_thresh &&
			cnt > global->ops_cb->ops_alter.ops_inlist_thresh)
        ...
        */
        ...
        if (!(global->ops_cb->ops_check
            &&
            opt_strace(global->ops_cb, OPT_F044_NO_RTREE)))
                                /* No Rtree trace pt isn't on */
        {
            ...
            {
                status = adi_opid(global->ops_adfcb,
                        "inside", &inside_op);
                ...
                if (infix) spatf = root;
                    ...
                    if (opno != global->ops_cb->ops_server->
	...
	/* Check for pseudo-constants, flag them.  I don't think any
	** MOP's are pseudo-constants but whatever.
	*/
	if (root->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags 
		& ADI_F32_NOTVIRGIN)
	    bfp->opb_mask |= OPB_NOTCONST;

	/* Traverse args to set up various bit maps, flags */
	node = root;
	do
	{
	    opb_isconst(subquery, bfp, &node->pst_left, &dontcare, FALSE);
	    node = node->pst_right;
	} while (node != NULL);
	call_isconst = FALSE;
	/* If this MOP is ifnull-like, it's not null eliminating */
	if (root->pst_sym.pst_value.pst_s_op.pst_fdesc->adi_fiflags
		& ADI_F32768_IFNULL)
            subquery->ops_bfs.opb_ncheck = FALSE;
...
**	3-Mar-2011 (kschendel) m1475 b125146
**	    Don't run the constant checking if ojmapactive but no ojmap bits
**	    are set.  This is the special case caught by opl-bfnull where
**	    a bf was pushed into a union but (I think?!) will not actually
**	    be evaluated there because of outer join considerations.
**	    (Ideally the bf shouldn't have been pushed at all!)
*/
static VOID
opb_rtminmax(
	OPS_SUBQUERY	   *subquery,
	...
	OPB_IBF            bfi)
{
    ...
    /* Don't try to fold or evaluate a BF that was pushed into a union but
    ** probably shouldn't have been (ie can't be evaluated there due to OJ):
    */
    if (bfp->opb_mask & OPB_OJMAPACTIVE
      && BTcount((char *)&bfp->opb_ojmap, subquery->ops_oj.opl_lv) == 0)
	return;
    ...
    ** "c1 is null and c1 is not null". */
    ...
    if (updatebfinfo && updatebfinfo->opb_sargtype == ADC_KNOKEY)
	return;				/* quick exit - it's a "<>" */
...

/*{
** Name: opb_varcheck	- search ORed list for common PST_VAR nodes
**
** Description:
**	This routine coordinates a search through a series of OR'd 
**	subexpressions, looking for PST_UOP/BOP predicates that involve
**	a particular column. If each OR has a restriction on the column,
**	the restrictions can be split into a new factor that will
**	reduce the cardinality of its table.
**
** Inputs:
**	varp				ptr to PST_VAR node of target column
**	orp				ptr to chain of ORs to search
**	bador				ptr to flag to return indication
**					of bad OR structure (ORs under ANDs)
**
** Outputs:
**
**	Returns:
**	    TRUE	when restrictions on column found in all ORs
**	    FALSE	otherwise
**	Exceptions:
**	    none
**
...
{
    PST_QNODE		*orp, *andp, *predp, *var1p;
    PST_TYPE		type;
    bool		lastor, lastand, found;


    /* First assure we haven't already processed this column because 
    ** it was restrictied more than once in the 1st OR. */
    for (andp = nodep->pst_right, predp = andp->pst_right;
	    andp->pst_sym.pst_type == PST_AND;
	    andp = andp->pst_left, predp = andp->pst_right)
    {
	if ((type = predp->pst_sym.pst_type) == PST_UOP  &&
	    (var1p = predp->pst_left)->pst_sym.pst_type == PST_VAR ||
	    type == PST_BOP && 
	    ((var1p = predp->pst_left)->pst_sym.pst_type == PST_VAR &&
	    predp->pst_right->pst_sym.pst_type == PST_CONST ||
	    (var1p = predp->pst_right)->pst_sym.pst_type == PST_VAR &&
	    predp->pst_left->pst_sym.pst_type == PST_CONST) &&
	    varp == var1p)
	    break;			/* found ours first */

	if (var1p->pst_sym.pst_type == PST_VAR &&
	    ...
	    return(FALSE);		/* already done this VAR */
    }
		
    /* Loop over the other ORs looking for at least one PST_VAR matching the 
    ** target in each one. This requires nested loops, one to pass the ORs and
    ** one to pass the contained ANDs. Since the last OR and last AND have 
    ** something other than a OR/AND on the left of the last node, the loops
    ** are a bit ugly. */
    for (orp = nodep->pst_left, lastor = (orp->pst_sym.pst_type != PST_OR);
	orp != (PST_QNODE *) NULL; 
	orp = orp->pst_left, lastor = (orp == (PST_QNODE *) NULL || 
					orp->pst_sym.pst_type != PST_OR))
    {
	if (lastor && orp->pst_sym.pst_type != PST_AND)
	{
	    *bador = TRUE;
	    return(FALSE);
	}

	/* Inner loop goes over AND looking for our VAR. */
	for (andp = (lastor) ? orp : orp->pst_right, predp = andp->pst_right,
		lastand = (andp->pst_sym.pst_type != PST_AND), found = FALSE;
	    andp != (PST_QNODE *) NULL;
	    andp = andp->pst_left, predp = andp->pst_right, lastand = 
		(andp == (PST_QNODE *) NULL || andp->pst_sym.pst_type != PST_AND))
	{
	    if (lastand && (type = (predp = andp)->pst_sym.pst_type) != 
			PST_BOP && type != PST_UOP)
	    {
		*bador = TRUE;
		return(FALSE);
	    }
	...
	if (lastor)
	    return(TRUE);			/* must be ok if we're here */
    }
}

/*{
** Name: opb_varsplit	- construct new factor using existing OR list
**
** Description:
**	This routine constructs a factor containing ORs of (possibly) ANDs
**	of restrictions on a single column - all extracted from a larger,
**	more complex OR/AND factor.
**
** Inputs:
...

{
    ...
    OPB_BOOLFACT	*bfp = (OPB_BOOLFACT *) NULL;
    PST_QNODE		*orp, *andp, *predp, *var1p, *and1p, *workp, *rootp;
    PST_QNODE		*or1p = (PST_QNODE *) NULL;
    PST_TYPE		type;
    bool		lastor, lastand, sargable, cmpx;

    /* Loop over the ORs looking for at least one PST_VAR matching the 
    ** target in each one. This requires nested loops, one to pass the ORs and
    ** one to pass the contained ANDs. Since the last OR and last AND have 
    ** something other than a OR/AND on the left of the last node, the loops
    ** are a bit ugly. 
    **
    ** For each OR, a set of ANDed restrictions is constructed for all 
    ** restrictions on the column. They are then all ORed together. */

    for (orp = nodep, lastor = (orp->pst_sym.pst_type != PST_OR);
	orp != (PST_QNODE *) NULL; 
	orp = orp->pst_left, lastor = (orp == (PST_QNODE *) NULL || 
					orp->pst_sym.pst_type != PST_OR))
    {
	workp = opv_opnode(global, PST_OR, 
		(ADI_OP_ID)global->ops_cb->ops_server->opg_eq, OPL_NOOUTER);
	workp->pst_left = workp->pst_right = (PST_QNODE *) NULL;
	if (or1p == (PST_QNODE *) NULL)
	    rootp = or1p = workp;
	else
	{
	    or1p->pst_left = workp;
	...
	/* Inner loop goes over AND looking for our VAR. */
	for (andp = (lastor) ? orp : orp->pst_right, predp = andp->pst_right,
		lastand = (andp->pst_sym.pst_type != PST_AND);
	    andp != (PST_QNODE *) NULL;
	    andp = andp->pst_left, predp = andp->pst_right, lastand = 
		(andp == (PST_QNODE *) NULL || andp->pst_sym.pst_type != PST_AND))
	{
		...
		varp->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id ==
		    var1p->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id)
	    {
    		workp = opv_opnode(global, PST_AND,
		    (ADI_OP_ID)global->ops_cb->ops_server->opg_eq, OPL_NOOUTER);
		if (or1p->pst_right == (PST_QNODE *) NULL)
		    or1p->pst_right = workp;
		else and1p->pst_left = workp;
		and1p = workp;
		and1p->pst_left = (PST_QNODE *) NULL;
		and1p->pst_right = predp;
		opv_copytree(global, &and1p->pst_right);
	    }

	    if (lastand)
	    {
		and1p->pst_left = opv_qlend(global);
		break;				/* exit inner loop */
	    }
	}
    ...
    /* One more loop to suck AND root of last OR into parent OR. And while
    ** we're looking at the tree, remove PST_ANDs when there is only one 
    ** predicate per OR. */
    for (orp = rootp; orp != (PST_QNODE *) NULL; orp = orp->pst_left)
    {
	if (orp->pst_right->pst_left->pst_sym.pst_type == PST_QLEND)
	    orp->pst_right = orp->pst_right->pst_right;	/* just one AND */

	if (orp->pst_left->pst_left == (PST_QNODE *) NULL)
	{
	    orp->pst_left = orp->pst_left->pst_right;
	    break;
	}
    }

    /* New predicate fragment has now been built. Allocate a OPB_BOOLFACT
    ** and fill it in. */
    bfp = opb_bfget(subquery, bfp);
    bfp->opb_qnode = rootp;
    subquery->ops_oj.opl_smask &= (~OPL_ISNULL);	/* ??? */
    sargable = opb_bfinit(subquery, bfp, rootp, &cmpx);

    /* There is room here for further improvement when multiple ranges
    ** are ORed. They could be collapsed into a single all inclusive range.
    ** That might allow more rows, but would eliminate the ORs. opb_rtminmax()
    ** is involved in that process, but can't currently be called with 
    ** bfp->opb_keys as NULL, as is the case with ANDs under ORs. This
    ** would be a difficult extension for relatively little benefit (IMO). */
}

/*{
** Name: opb_orsplit	- search Boolean factor for factors that can be split
**
** Description:
**	This routine coordinates a search through a series of OR'd 
**	subexpressions. If restrictions on a column are found in every
**	OR'd subexpression, they are split out, OR'd and turned into 
**	an additional Boolean factor. That will allow restrictive
**	subexpressions from the OR to be applied on tables.
**
** Inputs:
**      subquery                        ptr to subquery being analyzed
**	bfp				ptr to Boolean factor with ORs 
**					to analyze
**
** Outputs:
**      subquery->ops_bfs               boolean factor structure possibly
**					augmented with BFs split from OR 
**	Returns:
**	    VOID
**	Exceptions:
**	    none
...
{
    PST_QNODE		*nodep = bfp->opb_qnode, *andp, *predp, *varp;
    PST_TYPE		type;
    OPS_STATE		*global = subquery->ops_global;
    bool		proceed, bador = FALSE;


    /* Quick check to assure 1st OR satisfies our layout conditions (ANDs
    ** down right side, BOPs or UOPs down left side, except at bottom right
    ** which may also be BOP or UOP). 
    ** Note: checks on layout of other ORs will be done during subsequent
    ** analysis. */
    for (andp = nodep->pst_right, predp = andp->pst_right;
	andp != (PST_QNODE *) NULL && andp->pst_sym.pst_type == PST_AND &&
				predp != (PST_QNODE *) NULL;
	andp = andp->pst_left, (andp != (PST_QNODE *) NULL) ? 
				predp = andp->pst_right : (PST_QNODE *) NULL)
     if ((type = predp->pst_sym.pst_type) != PST_BOP &&
		type != PST_UOP)
	return;			/* BOP/UOP only down left */

    /* After loop exit, verify that andp is either QLEND, BOP or UOP. */
    if (andp != (PST_QNODE *) NULL && 
	(type = andp->pst_sym.pst_type) != PST_AND && type != PST_QLEND && 
	type != PST_BOP && type != PST_UOP)
	return;

    /* Now we can search for restrictions on common columns in all the
    ** ORs. We can turn those into separate factors, increasing 
    ** table-level selectivity. */

    /* Loop over ANDs of 1st OR looking for PST_VARs (in BOP/UOPs). */
    for (andp = nodep->pst_right, predp = andp->pst_right;
		andp != (PST_QNODE *) NULL; /* iteration done below */ )
    {
	if ((type = predp->pst_sym.pst_type) == PST_UOP &&
		(varp = predp->pst_left)->pst_sym.pst_type == PST_VAR ||
	    type == PST_BOP && 
	    ((varp = predp->pst_left)->pst_sym.pst_type == PST_VAR &&
	    predp->pst_right->pst_sym.pst_type == PST_CONST ||
	    (varp = predp->pst_right)->pst_sym.pst_type == PST_VAR &&
	    predp->pst_left->pst_sym.pst_type == PST_CONST))
	{
	    proceed = opb_varcheck(varp, nodep, &bador);
	    if (bador)
		return;			/* lower ORs must not be right */

	    if (proceed)
		opb_varsplit(subquery, varp, nodep);
	}

	/* Iteration done here. The last AND sits atop 2 BOP/UOPs, hence
	** the slightly tricky logic. */
	if (andp->pst_sym.pst_type == PST_AND)
	{
	    andp = andp->pst_left;
	    predp = andp->pst_right;
	}
	else if (andp != predp)
	    predp = andp;			/* lhs of last AND */
	else andp = (PST_QNODE *) NULL;		/* break the loop */
...
**	9-sep-2009 (dougi)
**	    Exclude non-correlated simple aggregate subqueries in X100
**	    queries from opb_bfconstants.
**	25-sep-2009 (dougi)
**	    Split sets of single column restrictions from complex OR
**	    under nonCNF expansions to be applied at table access level.
**	    Improves TPC H query 19 & applies to both X100 & native Ingres.
**	18-dec-2009 (dougi)
**	    Skip "x = x" predicates which can appear in some "exists"
**	    queries with aggs and grouping because of a opa bug.
**	31-mar-2010 (dougi)
**	    Remove IVW opb_bfconstant exclusion logic to opb_saggck().
**	3-Mar-2011 (kschendel) b125146
**	    Drop old constant-qual stub code, look for ii_true, ii_false.
*/
static void
opb_bfsinit(
	OPS_SUBQUERY       *subquery)
{
	...
	/* Check for "x = x" and skip to next. */
	if ((bop = and_node->pst_left)->pst_sym.pst_type == PST_BOP &&
	    bop->pst_sym.pst_value.pst_s_op.pst_opno == 
			subquery->ops_global->ops_cb->ops_server->opg_eq &&
	    (var1p = bop->pst_left)->pst_sym.pst_type == PST_VAR &&
	    (var2p = bop->pst_right)->pst_sym.pst_type == PST_VAR &&
	    var1p->pst_sym.pst_value.pst_s_var.pst_vno ==
	  ...
	  && !(bfp->opb_mask & OPB_NOTCONST) )
	{
	    ...
	    **
	    ** Parser does constant folding now, all we need to do here is
	    ** check for folded operators II_TRUE or II_FALSE.  Anything
	    ** else, we'll eval at runtime.
	    */

	    ADI_OP_ID opno;

	    opno = -1;
	    if (bfp->opb_qnode->pst_sym.pst_type == PST_COP)
		opno = bfp->opb_qnode->pst_sym.pst_value.pst_s_op.pst_opno;
	    if (opno == ADI_IIFALSE_OP)
	    {
		/* where-clause is FALSE, say so, skip the others. */
		subquery->ops_sunion.opu_mask |= OPU_NOROWS;
		subquery->ops_bfs.opb_qfalse = TRUE;
		break;
	    }
	    else if (opno == ADI_IITRUE_OP)
	    {
	    ...
	    */
	    ...
	    and_node = previous;
	    continue;			/* swallow this BF, keep going */
	}
	...
	/* If this is an OR and we are in "noncnf" mode, see if we can
	** split out disjuncts on the same column. */
	if ((subquery->ops_mask & OPS_NONCNF) && bfp && bfp->opb_qnode &&
		bfp->opb_qnode->pst_sym.pst_type == PST_OR &&
		bfp->opb_eqcls == OPB_BFMULTI)
	    opb_orsplit(subquery, bfp);
