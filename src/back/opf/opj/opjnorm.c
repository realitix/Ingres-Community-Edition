...
**
** Inputs:
**      qual                            Address of pointer to the qualification tree
**
** Outputs:
**      qual                            All AND nodes moved to right edge (other than
**					those under OR nodes etc.)
**	Returns:
...
**	24-Feb-2011 (kiria01) m1556
**	    Switch algorithm to use indirection simplifying it and catering
**	    for problems like two ANDs in a row with NULL lhs which could
**	    slip through to SEGV and also the multiple presense of QLEND
**	    from flattening with anti-joins. Also the algorithm is happy
**	    for the QLEND to be missing.
**      23-jun-2011 (huazh01)
**          sort the AND list based on join id. This allow OPF to create
**          eqcls for columns/joins referenced in the where clause first,
**          then create eqcls for columns/joins referenced in outer join.
**          Otherwise, outer join multi-eqcls may cause columns/joins in
**          the where clause be put into different eqcls, causing a
**          cart-prod. (b125292)
*/
VOID
opj_mvands(
	...
	PST_QNODE	**qual)
{
    PST_QNODE **orig, *temp, *end, *current, *next; 
    register PST_QNODE *q;

    orig = qual; 

    while ((q = *qual) && q->pst_sym.pst_type == PST_AND)
    {
	register PST_QNODE *l = q->pst_left;
	if (l == NULL || l->pst_sym.pst_type == PST_QLEND)
	    /* No left or is QLEND - drop current AND */
	    *qual = q->pst_right;
	else if (l->pst_sym.pst_type == PST_AND)
	{
	    *qual = l;
	    q->pst_left = l->pst_right;
	    l->pst_right = q;
	}
	else
	{
    ...
    if (!current || current->pst_sym.pst_type == PST_QLEND)
       return;

    /* b125292, sort the AND list based on joinid */
    while (current->pst_right != end)
    {
       while (current->pst_right != end)
       {
          next = current->pst_right; 

          if (current->pst_left->pst_sym.pst_value.pst_s_op.pst_joinid > 
              next->pst_left->pst_sym.pst_value.pst_s_op.pst_joinid)
          {
              temp = current->pst_left; 
              current->pst_left = next->pst_left;
              next->pst_left = temp; 
          }
          current = current->pst_right;
       }
       end = current; 
       current = *orig; 
    }
...
**          tree. (b124212)
**      13-jan-2011 (huazh01)
**          Modify the fix to b124212. We now put the first element
**          in the constant linked list into const_array[], and put the 
**          rest of the constants into in_array[]. (b124483)
[@history_template@]...
*/
static bool
opj_bopscan(
	...
	** because it means a where with "a = x ... and a = y" in same OR) */
	...
	opj_orstatp->array_size++;		/* increment count */
            ...
            cnst->pst_left->pst_sym.pst_type == PST_CONST)
        {
            ...
            opj_orstatp->in_count++;
            /* b124483: disconnect from the rest of constants because
            ** they are now in the in_array[]. They will be put into
            ** const_array[] in opa_inreplace().
            */
            cnst->pst_left = NULL; 
        }
	...
	{	/* found a matching VAR */
		...
		cnst->pst_sym.pst_dataval.db_length) != 0)
		  BTclear((i4)i, (PTR)&opj_orstatp->alldups);
            ...
            /* queue the current constant into const_array[i].
            */
...
**      13-jan-2011 (huazh01)
**          modify the fix to b124212. Constants in the IN-list (except
**          the first one) will now be put into const_array[] in 
**          opj_inreplace().  (b124483)
[@history_template@]...
*/
static VOID
opj_inreplace(
	...
	** const_array. */
	...
	for (j = 0; j < opj_orstatp->array_size; j++)
	{
	    ...
	    if (j == invar)
	    {
                ...
                /* b124483: put constants in in_array[] into
                ** const_array[], and link them sideways, or 
                ** 'pst_right' field. Those values in the side
                ** column will be cloned and relinked in the next 
                ** iteration of this inner for-loop.
                */
                this->pst_left = opj_orstatp->const_array[j]; 
                opj_orstatp->const_array[j] = this; 
...

{
    ...
    ** where clause, and DMT_ATT_ENTRYs to define "column". */
	...
	attrp->att_collID = dataval->db_collID;
...
**	11-jan-2011 (dougi)
**	    No MS Access transform for IVW queries.
**      13-jan-2011 (huazh01)
**          opj_topscan() could modify the query tree and return a
**          failsure status. preserve a copy of the tree and restore it
**          if opj_topscan() fails. (b124483).
*/
VOID
opj_normalize(
	OPS_SUBQUERY	*subquery,
