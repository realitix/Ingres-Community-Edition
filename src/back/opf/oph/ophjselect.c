...
**	14-Dec-2010 (frima01) SIR 124685
**	    Move OPN_JTSPECIAL enum declaration to beginning of file.
...
static OPH_HISTOGRAM *oph_ojsvar(
	...
	OPE_IEQCLS eqcls,
	OPO_TUPLES tuples);
static OPE_IEQCLS oph_refjoin_check(
	OPS_SUBQUERY *subquery,
	OPE_BMEQCLS *jbmeqcls,
	OPN_JTREE *np,
	OPV_IVARS *refed,
	bool *leftrefing);
...

/*{
** Name: OPH_REFJOIN_CHECK	- look for referential relationship match
**	in join ordering
**
** Description:
**	Search jordering for an equivalence class representing the
**	mapping of a referential relationship. That greatly simplifies the
**	join cardinality estimation.
**
** Inputs:
**      subquery                        ptr to subquery being analyzed
**	jbmeqcls			ptr to map of join EQCs
**	np				ptr to join
**	refed				ptr to referenced table varno
**	leftrefing			ptr to flag indicating whether 
**					referencing table is left or right
**
** Outputs:
**	Returns:
**		eqc for relationship, or OPE_NOEQCLS
**	Exceptions:
**	    none
**
** Side Effects:
...
**	6-apr-2010 (dougi)
**	    Re-written to include joins mapping referential relationships 
**	    without underlying join indexes.
*/
static OPE_IEQCLS
oph_refjoin_check(
	OPS_SUBQUERY    *subquery,
...

{
    OPE_IEQCLS		maxeqcls = subquery->ops_eclass.ope_ev, 
			eqcno, eqcno1, jixeqc;
    OPE_EQCLIST		*eqclp, *eqclp1, *jixeqclp;
    OPV_IVARS		varno;
    i4			eqccnt, wrkcnt;

    /* First loop looks for JOINIX EQC in join ordering. If there is more than
    ** one, pick the one covering the most EQCs. */
    for (eqcno = jixeqc = OPE_NOEQCLS, eqccnt = 0;
	(eqcno = BTnext((i4)eqcno, (char *)jbmeqcls, (i4)maxeqcls)) >= 0; )
    {
	eqclp = subquery->ops_eclass.ope_base->ope_eqclist[eqcno];
	if (eqclp->ope_mask & OPE_JOINIX)
	 if ((wrkcnt = BTcount((char *)eqclp->ope_joinrep, (i4)maxeqcls)) >
					eqccnt)
	 {
	    eqccnt = wrkcnt;
	    jixeqc = eqcno;
    ...
    /* If no result from first loop, second loop examines all EQCs in sq
    ** looking for a OPE_JOINIX whose EQCs are contained in the join order.
    ** Again, we want the one covering the most EQCs. */
    if (jixeqc == OPE_NOEQCLS)
    {
	for (eqcno = 0; eqcno < subquery->ops_eclass.ope_ev; eqcno++)
	{
	    eqclp = subquery->ops_eclass.ope_base->ope_eqclist[eqcno];
	    if (eqclp == (OPE_EQCLIST *) NULL)
		continue;			/* skip empty entries */

	    if ((eqclp->ope_mask & OPE_JOINIX) && 
		BTsubset((char *)eqclp->ope_joinrep, (char *)jbmeqcls, 
					(i4)maxeqcls))
	     if ((wrkcnt = BTcount((char *)eqclp->ope_joinrep, (i4)maxeqcls)) >
					eqccnt)
	     {
	    ...
	    if (eqclp->ope_mask & OPE_REPJOINIX)
	     for (eqcno1 = OPE_NOEQCLS; (eqcno1 = BTnext((i4)eqcno1,
		(char *)eqclp->ope_repjoin, (i4)maxeqcls)) >= 0; )
	     {
		eqclp1 = subquery->ops_eclass.ope_base->ope_eqclist[eqcno1];
		if (BTsubset((char *)eqclp1->ope_joinrep, (char *)jbmeqcls,
					(i4)maxeqcls) &&
		    (wrkcnt = BTcount((char *)eqclp1->ope_joinrep, 
					(i4)maxeqcls)) > eqccnt)
	 	{
		    eqccnt = wrkcnt;
		    jixeqc = eqcno;
    ...
    *leftrefing = FALSE;			/* assume right */
    for (varno = -1; (varno = BTnext((i4)varno,
	(char *)jixeqclp->ope_rjrefing, subquery->ops_vars.opv_prv)) >= 0;)
     if (BTtest((i4)varno, (char *)&np->opn_child[0]->opn_rlmap))
     {
	*leftrefing = TRUE;
	break;
     }

    for (varno = -1, *refed = -1; (varno = BTnext((i4)varno,
	(char *)jixeqclp->ope_rjrefed, subquery->ops_vars.opv_prv)) >= 0;)
     if (BTtest((i4)varno, (char *)&np->opn_child[0]->opn_rlmap) ||
	BTtest((i4)varno, (char *)&np->opn_child[1]->opn_rlmap))
     {
	*refed = varno;
	break;
     }
    ...
    if (*refed == -1)
	jixeqc = OPE_NOEQCLS;		/* after all that, refed isn't avail */
...
**          subsequent joins.
**	18-feb-2009 (dougi)
**	    Add support for X100 - recognize joins that map referential 
**	    relationships.
**	5-apr-2010 (dougi)
**	    Broaden treatment of joins that map referential relationships.
*/
static OPH_HISTOGRAM *
opn_jtuples(
	OPS_SUBQUERY       *subquery,
	...
	OPN_PERCENT	   *jrelsel)
{
    ...
    OPE_IEQCLS		eqcls1, refjoineqc = OPE_NOEQCLS;
    OPV_IVARS		refed;
    bool		refjoin_leftrefing;
	...
	*tuples = cartprod;		/* number of tuples for cartesian prod*/
    ...
    if (jbmeqcls)
	MEcopy((char *)jbmeqcls, sizeof(jeqcmap), (char *)&jeqcmap);
    else
    {
	MEfill(sizeof(jeqcmap), 0, (char *)&jeqcmap);
	BTset((i4)eqcls, (char *)&jeqcmap);
	eqcls1 = eqcls;
    }

    eqcls = BTnext((i4)-1, (char *)&jeqcmap, (i4)maxeqcls);
    ...
    *jcomplete = FALSE;
    ...
    /* Before the EQC loop, see if there's a referential join. */
    if ((refjoineqc = oph_refjoin_check(subquery, &jeqcmap, np,
					&refed, &refjoin_leftrefing)) >= 0)
    {
	/* Determine join cardinality based on referential relationship. */
	reftups = subquery->ops_vars.opv_base->opv_rt[refed]->opv_tcop->
					opo_cost.opo_tups;

	if (reftups < 1.0)
	    reftups = 1.0;

	if (refjoin_leftrefing)
	    reftups = ltuples * (rtuples / reftups);
	else reftups = rtuples * (ltuples / reftups);

	if (reftups < 1.0)
	    reftups = 1.0;
    }

    /* Loop for each attribute in the join */
    for (eqcls = OPE_NOEQCLS; (eqcls = BTnext((i4)eqcls, (char *)&jeqcmap,
			(i4)maxeqcls)) >= 0; )
    {	
	...
	OPO_TUPLES	       jtups2; /* plan B tuple estimates */
	OPE_EQCLIST		*eqclp;
	...
	if (histpp != (OPH_HISTOGRAM **) NULL)
	    lhistp = *histpp;
	else if (eqcls != refjoineqc)
	    lhistp = oph_ojsvar(subquery, lrlmp, eqcls, ltuples);
					/* may not find hist for oj SVARs */
	else lhistp = (OPH_HISTOGRAM *) NULL;
	...
	else rhistp = (OPH_HISTOGRAM *) NULL;

	if (lhistp == (OPH_HISTOGRAM *) NULL)
	    lhistp = rhistp;
	else if (rhistp == (OPH_HISTOGRAM *) NULL)
	    rhistp = lhistp;
	...
	** histogram. */
    ...
    }	/* end of loop for multi-attribute join */
...
**	24-feb-2010 (dougi)
**	    Add histos for nonjoinix EQCs, too.
**	25-Feb-2011 (kschendel)
**	    Watch out for composite histos in the above code.
*/
VOID
oph_jselect(
	OPS_SUBQUERY       *subquery,
	...
	OPL_OJHISTOGRAM    *ojhistp)
{
	...
	jrelsel = OPN_D100PC;    	    /* init selectivity to 1.0 */
	newtrl->opn_histogram = opn_jtuples (subquery, np, jordering, 
		jxordering, lrlmp, rrlmp, jeqh, &junique, &jcomplete, &jtups, 
		&tidvarno, ojhistp, &jrelsel); 
					    /* if jeqh has 
	...
	*/
	...
	/* Add non-joinix EQCs mapped by EQCs in jeqh. */
	for (eqcls = -1; 
	    (eqcls = BTnext((i4)eqcls, (char *)&orig_jeqh, (i4)maxeqcls)) >= 0; )
	 if (subquery->ops_eclass.ope_base->ope_eqclist[eqcls]->ope_mask &
				OPE_REFNOJOINIX)
	    BTset((i4)eqcls, (char *)&histomap);
	for (eqcls = -1; 
	    (eqcls = BTnext((i4)eqcls, (char *)jeqh, (i4)maxeqcls)) >= 0; )
	{
	    eqclp = subquery->ops_eclass.ope_base->ope_eqclist[eqcls];
	    if (eqclp->ope_repjoin == (OPE_BMEQCLS *) NULL)
		continue;

	    for (eqcls1 = -1; 
		(eqcls1 = BTnext((i4)eqcls1, (char *)eqclp->ope_repjoin, 
				(i4)maxeqcls)) >= 0; )
	     if (subquery->ops_eclass.ope_base->ope_eqclist[eqcls1]->
				ope_mask & OPE_REFNOJOINIX)
		BTset((i4)eqcls1, (char *)&histomap);
	}
	for (histp = lrlmp->opn_histogram; histp; histp = histp->oph_next)
	{
	    if (histp->oph_attribute < 0)
		continue;	/* Skip composites */
	    attrp = abase->opz_attnums[histp->oph_attribute];
	    if (BTcount((char *)&attrp->opz_eqcmap, subquery->ops_attrs.opz_av)
				<= 1)
		continue;

	    for (eqcls = -1; (eqcls = BTnext((i4)eqcls, 
			(char *)&attrp->opz_eqcmap, maxeqcls)) >= 0; )
	     if (subquery->ops_eclass.ope_base->ope_eqclist[eqcls]->ope_mask &
				OPE_JOINIX)
		BTset((i4)eqcls, (char *)&histomap);
	}
