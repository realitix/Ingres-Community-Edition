/*
** Copyright (c) 1986, 2001, 2004, 2010 Actian Corporation
**
**
*/
	...
	OPB_BOOLFACT       *bp)
{
    ...
    ** exactly the columns in this predicate. */
    if (rdrcp != NULL)
     ...
     {
	colcp = &rdrcp->col_comp_array[i];
	if (colcp->cc_count != 1)
	    continue;
	else if (sametab && colcp->cc_tabid1.db_tab_base !=
		colcp->cc_tabid2.db_tab_base && 
			colcp->cc_tabid2.db_tab_base != 0)
	    continue;
	else if (!sametab && !(colcp->cc_tabid1.db_tab_base == tabbase1 &&
			colcp->cc_tabid2.db_tab_base == tabbase2 ||
		colcp->cc_tabid1.db_tab_base == tabbase2 &&
			colcp->cc_tabid2.db_tab_base == tabbase1))
	    continue;
	else if (colcp->cc_tabid1.db_tab_index != 
			colcp->cc_tabid2.db_tab_index)
	    continue;
	else if (colcp->cc_col1[0] != attr1 && colcp->cc_col2[0] != attr1 ||
...
**	    class asks for it
**	12-march-2009 (dougi)
**	    No faked join selectivity for X100.
**	10-jul-2009 (huazh01)
	...
	{   /* create histogram lists of all single equilvalence class boolean
		...
		** correct */
		...
		if (sebfp->opb_torcount > 0)
		{
			...
			subquery->ops_global->ops_cb->ops_alter.ops_range * 0.5;
