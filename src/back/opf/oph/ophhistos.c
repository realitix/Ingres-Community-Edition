...
**	7-july-2010 (dougi)
**	    Add logic to substitute fk histogram for join index (which has
**	    no histogram) for IVW tables.
*/
static bool
oph_catalog(
	OPS_SUBQUERY       *subquery,
...
involved
	...
	*/
	...
	if (attrp != (OPZ_ATTS *) NULL &&
		(attrp->opz_mask & OPZ_X100JOINIX) &&
		grv_ptr->opv_relation->rdr_refrel)
	{
	    /* This is a join index with no histogram. If the corresponding 
	    ** foreign key has a histogram, use it instead. */
	    rrarrayp = grv_ptr->opv_relation->rdr_refrel;
	    if (rrarrayp->refrel_cnt <= 0 ||
		rrarrayp->refrel_array[0].dbrr_refingtabid.db_tab_base !=
		    rdf_info->rdr_rel->tbl_id.db_tab_base)
		status = E_DB_WARN;		/* one more consistency check */
	    else for (i = 0; i < rrarrayp->refrel_cnt; i++)
	     if ((rrp = &rrarrayp->refrel_array[i])->dbrr_refingixcol ==
				dmfattr.db_att_id)
	      if (rrp->dbrr_colcount == 1 && (rdf_info->rdr_histogram)[rrp->
			dbrr_refingcols[0]] != NULL)
	      {
		/* Single foreign key column and it has histogram. Use
		** it instead. */
		dmfattr.db_att_id = rrp->dbrr_refingcols[0];
		status = E_RD0000_OK;
		break;
	      }
	}
...
**	24-Feb-2011 (kiria01)
**	    Initialise maxtid.tid_tid.tid_line.
[@history_line@]...
*/
static VOID
oph_uniform(
