	...
	ADI_OP_ID	   *op_ptr)
{
    ...
    */
    ...
    adfstatus = adi_opid( &adf_scb, func_name, op_ptr);
...
# endif
    ...
    if (opg_getfunc(opf_cb, "!=", DB_QUEL, &opg_cbp->opg_ne) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "+", DB_QUEL, &opg_cbp->opg_add) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "-", DB_QUEL, &opg_cbp->opg_sub) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "*", DB_QUEL, &opg_cbp->opg_mul) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "/", DB_QUEL, &opg_cbp->opg_div) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "abs", DB_QUEL, &opg_cbp->opg_abs) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "sqrt", DB_QUEL, &opg_cbp->opg_sqrt) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "power", DB_QUEL, &opg_cbp->opg_pow) != E_DB_OK)
        return(E_DB_FATAL);
    ...
    if (opg_getfunc(opf_cb, "count(*)", DB_SQL, &opg_cbp->opg_scount) != E_DB_OK)
        return(E_DB_FATAL);
    if (opg_getfunc(opf_cb, "float8", DB_SQL, &opg_cbp->opg_f8) != E_DB_OK)
        return(E_DB_FATAL);
...
# endif
    ...
    opg_cbp->opg_alter.ops_reuse = TRUE; /* default is to do reuse (for X100) */
    ...
    if (opf_cb->opf_smask & OPF_HASH_AGG) /* default taken from CBF parm */
	opg_cbp->opg_alter.ops_amask |= OPS_AHASHAGG;
    else
	opg_cbp->opg_alter.ops_amask &= (~OPS_AHASHAGG);
    if (opf_cb->opf_smask & OPF_HASH_JOIN) /* default taken from CBF parm */
	opg_cbp->opg_alter.ops_amask |= OPS_AHASHJOIN;
    else
	opg_cbp->opg_alter.ops_amask &= (~OPS_AHASHJOIN);
