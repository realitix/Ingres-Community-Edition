...
**		    icop->opo_cost.opo_tpb, &nocache, &cpu);
	...
	** project restrict is larger than the in memory hold file can support */
	...
	/* Just for fun (and for X100), let's assume no cost for this. 
...
**	8-apr-2010 (dougi)
**	    If some GB entries have histos, use them to produce an estimate
**	    (currently bails & uses default if at least one entry is an
**	    expression). Also identify DERIVED columns for exclusion from
...

{
	...
	*/
	...
	/* Identify DERIVED columns in X100 queries. */
	if (subquery->ops_global->ops_gmask & OPS_X100)
	    opc_x100_derck(subquery, subquery->ops_byexpr, TRUE);
	...
	for (zcnt = 0, found = 0, byresp = subquery->ops_byexpr; byresp && 
	    byresp->pst_sym.pst_type == PST_RESDOM; byresp = byresp->pst_left)
	{
	    if (byresp->pst_sym.pst_value.pst_s_rsdm.pst_rsflags & 
							PST_RS_DERIVED)
	    {
		byresp->pst_sym.pst_value.pst_s_rsdm.pst_rsflags &= 
			~PST_RS_DERIVED;	/* unset to help Xcompile */
	...
	*/
	...
	{
	    if (found != zcnt)
		groupresult = MHpow(groupresult, (f4)zcnt/(f4)found);
	    if (groupresult > 0.5 * gcostp->opo_tups)
...
**	    subquery.
**	26-apr-2010 (dougi)
**	    Add support of opo_jixeqcs/njixeqcs for IVW.
	...
	OPN_RLS		   *rlp)
{
    ...
    */
    ...
    {
	/* Copy refrel/join index info. */
	if (sjp->jixeqcs && BTcount((char *)sjp->jixeqcs, (i4)maxeqcls) > 0)
	{
	    newcop->opo_jixeqcs = (OPE_BMEQCLS *)opu_memory(global, 
						sizeof(OPE_BMEQCLS));	
	    MEcopy((char *)sjp->jixeqcs, sizeof(OPE_BMEQCLS), 
						(char *)newcop->opo_jixeqcs);
	}
	if (sjp->njixeqcs && BTcount((char *)sjp->njixeqcs, (i4)maxeqcls) > 0)
	{
	    newcop->opo_njixeqcs = (OPE_BMEQCLS *)opu_memory(global, 
						sizeof(OPE_BMEQCLS));	
	    MEcopy((char *)sjp->njixeqcs, sizeof(OPE_BMEQCLS), 
						(char *)newcop->opo_njixeqcs);
	}
    }
...
**	15-mar-2010 (dougi)
**	    Permit X100 queries to cost even with IMTID join cols.
**	26-apr-2010 (dougi)
**	    Add joinix, non-joinix EQC maps to parm list.
*/
OPN_STATUS
opn_calcost(
	OPS_SUBQUERY       *subquery,
	...
	OPE_BMEQCLS	   *njixeqcs)
{
    ...
    sjstate.np = np;
    sjstate.jixeqcs = jixeqcs;
    sjstate.njixeqcs = njixeqcs;
	...
	** product */
	...
	if ((iattr >= 0) && (oattr >= 0))
	{
	    ...
	    sjstate.notimptid = ((global->ops_gmask & OPS_X100) != 0 || 
		DB_IMTID != abase->opz_attnums[iattr]->opz_attnm.db_att_id);
	    ...
	    ** 
	    ** X100 queries are excepted from this because of refrel joins
