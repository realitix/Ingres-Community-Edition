...
OPO_CO             *cop;
{
    ...
    basep = cop->opo_variant.opo_scost;	/* May be meaningless, but save */

    memset(cop, 0, sizeof(OPO_CO));	/* Zero it all */
    ...
    cop->opo_x100clid = OPV_NOGVAR;
    if (subquery->ops_global->ops_cb->ops_smask & OPS_MDISTRIBUTED)
    {
	OPD_ISITE	site;

	cop->opo_variant.opo_scost = basep;	/* Put this back, it's real */
	for (site = subquery->ops_global->ops_gdist.opd_dv; --site >= 0;)
