...
**          (b124582)
**	3-feb-2011 (dougi)
**	    An additional check for ANTI/SEMI/SUBQ joins.
**	07-Feb-2010 (kiria01) -b123744 -124582 m1261
**	    Reverted prior two changes as these can drop viable combinations
**	    that may be the only potential candidates. These fixes also don't
**	    seem to address the original problem any longer.
*/
bool
opl_ojverify(
	OPS_SUBQUERY	    *subquery,
	...
	OPL_OUTER	    *outerp)
{
    ...
    /* First check for ANTI/SEMI/SUBQJOIN. Their inner & outer vars must 
    ** be covered by vmap. */
    if (outerp->opl_mask & (OPL_ANTIJOIN | OPL_SEMIJOIN | OPL_SUBQJOIN) &&
	(!BTsubset((char *)outerp->opl_ivmap, (char *)vmap, maxvar) ||
	 !BTsubset((char *)outerp->opl_ovmap, (char *)vmap, maxvar)))
	    return(FALSE);
