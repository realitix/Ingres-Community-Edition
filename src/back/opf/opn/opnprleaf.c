...
**	20-jan-2008 (dougi)
**	    Recognize DB_X100CL_STORE storage type for X100 support.
...
**	22-jan-2010 (dougi)
**	    For now, DB_X100IX_STORE is sort of the same as X100CL.
	...
	bool               keyed_only)
{
	...
	if (dmfptr->tbl_storage_type > DB_STDING_STORE_MAX)
	    x100tab = TRUE;
...
# endif
        ...
        else
	{
	  if (varp->opv_grv->opv_relation->rdr_rel->tbl_storage_type == 
					DB_BTRE_STORE ||
	    varp->opv_grv->opv_relation->rdr_rel->tbl_storage_type ==
		DB_X100CL_STORE ||
	    varp->opv_grv->opv_relation->rdr_rel->tbl_storage_type ==
		DB_X100IX_STORE)
	...
	||
    	(cop->opo_storage == DB_ISAM_STORE) /* if a isam, then ordering
                                            ...
                                            ** a full sort merge */
	||
    	(cop->opo_storage == DB_X100CL_STORE) /* clustered X100 */
	||
    	(cop->opo_storage == DB_X100IX_STORE) /* and indexed X100 */
	||
