	...
	OPN_STATUS	   *sigstat)
{
	...
	*rlmp = (OPN_RLS *) NULL;
	...
	OPE_BMEQCLS	reqr;		/* equivalence classes required from
                                        ** the right subtree
                                        */
	OPE_BMEQCLS	jixeqcs;	/* EQCs of refrels with join indexes
					** at current node (IVW only). */
	OPE_BMEQCLS	njixeqcs;	/* EQCs of refrels without join indexes
					** at current node (IVW only). */
	...
	opn_sm2 (subquery, np, &eqh, &bfm, &jeqh, &byeqcmap, &leqr, &reqr, 
		 eqr, &jixeqcs, &njixeqcs, &jxcnt, &jordering, &jxordering,
	...
	*sigstat = opn_calcost (subquery, jordering, rkeyjoin, lsubtp, 
		...
		reqclp, np, np->opn_ojid, lkjonly, &jixeqcs, &njixeqcs); 
	...
	*sigstat = opn_calcost (subquery, jordering, lkeyjoin, rsubtp, 
		...
		leqclp, np, np->opn_ojid, rkjonly, &jixeqcs, &njixeqcs); 
					/* calculate possible cost
