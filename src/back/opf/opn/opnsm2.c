...

/*{
** Name: OPN_NOJOINIXCK	- check for referential join with no join index
**
** Description:
**      This routine checks a refjoin EQC for the presence of a join index.
**
** Inputs:
**	eqclsp			ptr to join eqclass structure
**	lvmap			ptr to var map of left side of join
**	rvmap			ptr to var map of right side of join
**	maxvar			no of variables in subquery
**
** Outputs:
**	Returns:
**	    bool		TRUE if join has no join index, otherwise FALSE
** History:
**      24-feb-2010 (dougi)
**	    Created for X100.
*/
...
{
    OPV_BMVARS		tmap1, tmap2;

    /* Determine referencing tables for EQC that DO have join indexes. */
    MEcopy((char *)eqclp->ope_rjrefing, sizeof(OPV_BMVARS), (char *)&tmap1);
    MEcopy((char *)eqclp->ope_nojoinix, sizeof(OPV_BMVARS), (char *)&tmap2);
    BTnot((i4)maxvar, (char *)&tmap2);
    BTand((i4)maxvar, (char *)&tmap1, (char *)&tmap2);
    if (BTcount((char *)&tmap2, (i4)maxvar) == 0)
	return(TRUE);				/* none other than NOJOINIX */

    /* Check for refing in left, refed in right, & vice versa. */
    MEcopy((char *)&tmap2, sizeof(OPV_BMVARS), (char *)&tmap1);
						/* keep a copy */
    BTand((i4)maxvar, (char *)lvmap, (char *)&tmap2);
    if (BTcount((char *)&tmap2, (i4)maxvar) > 0)
    {
	/* A refing is in left. Check for refed in right. */
	MEcopy((char *)eqclp->ope_rjrefed, sizeof(OPV_BMVARS), (char *)&tmap1);
	BTand((i4)maxvar, (char *)rvmap, (char *)&tmap1);
	if (BTcount((char *)&tmap1, (i4)maxvar) > 0)
	    return(FALSE);
	else return(TRUE);
    }
    ...
    if (BTcount((char *)&tmap1, (i4)maxvar) > 0)
    {
	/* A refing is in right. Check for refed in left. */
	MEcopy((char *)eqclp->ope_rjrefed, sizeof(OPV_BMVARS), (char *)&tmap2);
	BTand((i4)maxvar, (char *)lvmap, (char *)&tmap2);
	if (BTcount((char *)&tmap2, (i4)maxvar) > 0)
	    return(FALSE);
	else return(TRUE);
	    ...
	    ((lbase->opl_ojt[bp->opb_ojid]->opl_mask & (OPL_OJCARTPROD | OPL_BOJCARTPROD)) && !(bp->opb_mask & OPB_BFUSED))))
	{
	    ...
	    ** need. Fixes bug 117655. */
	    ...
	    for (bfeqcls = -1; (bfeqcls = BTnext((i4)bfeqcls, 
					...
					** subtree */
	    {
		if (((eqclsp = subquery->ops_eclass.ope_base->
			ope_eqclist[bfeqcls])->ope_mask & OPE_REFNOJOINIX) &&
		    opn_nojoinixck(eqclsp, lvmap, rvmap, maxvar))
			continue;	/* ignore these guys */
		...
		{   /* this equivalence class is in the subtree */
			...
			&&
			!(subquery->ops_eclass.
			ope_base->ope_eqclist[bfeqcls]->ope_mask & OPE_JOINIX))
					/* and no join indices */
		    {
...
**	jixeqcs				Join eqclasses that map to IVW
**					referential relationships that have 
**					corresponding join indexes
**	njixeqcs			Join eqclasses that map to IVW
**					referential relationships that don't
**					have join indexes
**	Returns:
**	    VOID
**	Exceptions:
...
**	    Add X100 logic.
**	27-jan-2010 (dougi)
**	    Small changes for X100 refrels but no join index.
**	23-apr-2010 (dougi)
**	    Changes in IVW join index processing.
*/
VOID
opn_sm2(
	OPS_SUBQUERY       *subquery,
	...
	OPO_ISORT          *rkeyjoin)
{
    ...
    OPE_BMEQCLS         faeqcmap;           /* map of function attributes
                                            ...
                                            ** check */
    OPE_IEQCLS		joinixeqc = OPE_NOEQCLS; /* X100 join index EQC
					    ** (if applicable) */
    ...
    MEfill(sizeof(*jeqh), (u_char)0, (PTR)jeqh ); /* init 0 - same 
                                            ...
                                            ** for this node also */
    MEfill(sizeof(*jixeqcs), (u_char)0, (PTR)jixeqcs);
    MEfill(sizeof(*njixeqcs), (u_char)0, (PTR)njixeqcs);
	...
	*jxcnt = OPN_NJEQCLS;		    /* - init to no joining equivalence
					    ...
					    ** subtrees) */
	{
	    ...
	    /* For X100 queries, check for joins that can be replaced by
	    ** join indices. They do NOT include joins involving virtual
	    ** tables. */
	    virtual = FALSE;
	    if (eqclsp->ope_mask & OPE_JOINIX)
	    {
		BTset((i4)eqcls, (char *)jxeqh);
		BTset((i4)eqcls, (char *)jeqh);

		if ((eqclsp->ope_mask & OPE_REFNOJOINIX) &&
			opn_nojoinixck(eqclsp, &np->opn_child[0]->opn_rlmap,
				&np->opn_child[1]->opn_rlmap, maxvar))
		{
		    /* There's a refrel join, but no index. It conveys
		    ** info but is not otherwise useful. */
		    BTclear((i4)eqcls, (char *)leqr);
		    BTclear((i4)eqcls, (char *)eqr);
		    BTclear((i4)eqcls, (char *)reqr);
		    BTset((i4)eqcls, (char *)njixeqcs);
		    continue;
		}
		BTset((i4)eqcls, (char *)jixeqcs);
		if (joinixeqc == OPE_NOEQCLS ||
			BTcount((char *)eqclsp->ope_joinrep, maxeqcls) >
			BTcount((char *)subquery->ops_eclass.ope_base->
				ope_eqclist[joinixeqc]->ope_joinrep, maxeqcls))
		    joinixeqc = eqcls;
		continue;
	    }
	    if (subquery->ops_global->ops_gmask & OPS_X100)
	    {
		if (lvarp && lvarp->opv_grv->opv_relation == (RDR_INFO *) NULL || 
		    rvarp && rvarp->opv_grv->opv_relation == (RDR_INFO *) NULL)
		    virtual = TRUE;
		else for (attno = -1; !virtual && (attno = BTnext((i4)attno,
			(char *)&eqclsp->ope_attrmap, 
			subquery->ops_attrs.opz_av)) >= 0; )
		 if (subquery->ops_vars.opv_base->opv_rt[subquery->ops_attrs.
			opz_base->opz_attnums[attno]->opz_varnm]->opv_grv->
			opv_relation == (RDR_INFO *) NULL)
		    virtual = TRUE;

		/* If EQC is replaced by an X100 join index, save it & loop. */
		if ((eqclsp->ope_mask & OPE_REPJOINIX) &&
			eqclsp->ope_repjoin != (OPE_BMEQCLS *) NULL)
		{
		    /* Verify that EQC has JOINIX that covers both sides of 
 		    ** the join. Multi-attr joins don't always do that. Also, 
		    ** don't use joinix if it doesn't exist (i.e., if eqcls 
		    ** is in joinixeqc's ope_nojoinix map). */
		    for (workeqc = joinixeqc = OPE_NOEQCLS;
			(workeqc = BTnext((i4)workeqc, (char *)eqclsp->
			    ope_repjoin, subquery->ops_eclass.ope_ev)) >= 0; )
		    {
			jixeqcp = subquery->ops_eclass.ope_base->
						ope_eqclist[workeqc];
			left = right = FALSE;
			if (!(jixeqcp->ope_mask & OPE_REFNOJOINIX))
			 for (attno = -1; (attno = BTnext((i4)attno,
			    (char *)&jixeqcp->ope_attrmap, 
			    subquery->ops_attrs.opz_av)) >= 0; )
		 	 {
			    attrp = subquery->ops_attrs.opz_base->
							opz_attnums[attno];
			    if (BTtest(attrp->opz_varnm, (char *)&np->
						opn_child[0]->opn_rlmap))
		 		left = TRUE;
			    else if (BTtest(attrp->opz_varnm, (char *)&np->
						opn_child[1]->opn_rlmap))
				right = TRUE;
			    if (left && right)
			    {
				if (joinixeqc == OPE_NOEQCLS ||
				    (subquery->ops_eclass.ope_base->
					ope_eqclist[joinixeqc]->ope_mask & 
						OPE_REFNOJOINIX))
				    joinixeqc = workeqc;
				BTset((i4)workeqc, (char *)jixeqcs);
				BTset((i4)workeqc, (char *)jxeqh);
				break;
			    }
			 }
			else if (FALSE && jixeqcp->ope_rjrefed != (OPV_BMVARS *) NULL &&
			    jixeqcp->ope_rjrefing != (OPV_BMVARS *) NULL)
			{
			    OPV_BMVARS	varmap;
			    bool	leftrefed, rightrefed, leftrefing, 
					rightrefing;
			    /* If this is a "no join index" EQC, check if 
			    ** refed and refing vars are in left/right or 
			    ** right/left. */

			    /* Check referenced table coverage. */
			    MEcopy((char *)jixeqcp->ope_rjrefed, 
				sizeof(OPV_BMVARS), (char *)&varmap);
			    BTand(subquery->ops_vars.opv_rv, 
				(char *)&np->opn_child[0]->opn_rlmap,
				(char *)&varmap);
			    if (BTcount((char *)&varmap, 
					subquery->ops_vars.opv_rv) > 0)
				leftrefed = TRUE;
			    else leftrefed = FALSE;
			    MEcopy((char *)jixeqcp->ope_rjrefed, 
				sizeof(OPV_BMVARS), (char *)&varmap);
			    BTand(subquery->ops_vars.opv_rv, 
				(char *)&np->opn_child[1]->opn_rlmap,
				(char *)&varmap);
			    if (BTcount((char *)&varmap, 
					subquery->ops_vars.opv_rv) > 0)
				rightrefed = TRUE;
			    else rightrefed = FALSE;

			    /* Check referencing table coverage. */
			    MEcopy((char *)jixeqcp->ope_rjrefing, 
				sizeof(OPV_BMVARS), (char *)&varmap);
			    BTand(subquery->ops_vars.opv_rv, 
				(char *)&np->opn_child[0]->opn_rlmap,
				(char *)&varmap);
			    ...
			    if (BTcount((char *)&varmap, 
					subquery->ops_vars.opv_rv) > 0)
				rightrefing = TRUE;
			    else rightrefing = FALSE;

			    /* After all that, do we have a match?? */
			    if (leftrefed && rightrefing || 
				leftrefing && rightrefed)
			    {
				if (joinixeqc == OPE_NOEQCLS)
				    joinixeqc = workeqc;
				BTset((i4)workeqc, (char *)njixeqcs);
				BTset((i4)workeqc, (char *)jxeqh);
			    }
			}
		     }
...
*/
	...
	else if (!(subquery->ops_global->ops_gmask & OPS_X100))
	...
	else *lkeyjoin = *rkeyjoin = OPE_NOEQCLS;
					    /* no key joins in X100 */
	...
	** evaluating this join */
	...
	if (joinixeqc > OPE_NOEQCLS)
	    *jordering = joinixeqc;	    /* X100 join index */
	else if (lvarp 
	    && 
	    lvarp->opv_subselect 
	    && 
