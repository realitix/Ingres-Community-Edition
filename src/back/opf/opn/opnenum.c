...
**	9-apr-2010 (dougi)
**	    Defer dealloction of memory 'til later in opj_enum() to permit
**	    execution of opn_modcost() there, too.
**	2-Mar-2011 (kschendel) m1475
**	    op153 call to rall now suppresses bf's but prints atts, eqcs.
**	31-Mar-03 (wanfr01) b119039
**	    Backed out above fix with a rewrite.
**	    Greedy enumeration is fine if the star configuration doesn't have
**	    an extreme number of sort variables.
*/
VOID
opn_enum(
	OPS_SUBQUERY       *subquery)
...
onceMorewithFeeling:
	...
	opt_rall(FALSE,FALSE);	    	/* load subquery for opt_rall() and
	...
	*/
	...
	{
	    /* Used to free enumeration memory here, but that's now done
	    ** in opj_enum() to permit call to opn_modcost() without SEGVs. */
