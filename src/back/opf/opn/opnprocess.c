...

/*{
** Name: opn_clustperm	- record current permutation in OPN_STLEAVES array
**
** Description:
**      Recurse down current join tree, recording leaf node permutation
**	for cluster check.
**
** Inputs:
...
**      jtree                           ptr to current tree node
**					being analyzed 
**	x100perm			leaf node permutation array
**	nextelm				ptr to index to next array element
**
** Outputs:
**
**	Returns:
...
opn_clustperm(
	OPS_SUBQUERY	*subquery,
	OPN_JTREE	*jtree,
	OPN_STLEAVES	x100perm,
	i4		*nextelm)
{
    i4		i;

    for (i = 0; i < jtree->opn_nchild; i++)
    {
	if (jtree->opn_child[i]->opn_nleaves < 2)
	    x100perm[(*nextelm)++] = jtree->opn_child[i]->opn_prb[0];
	else opn_clustperm(subquery, jtree->opn_child[i], x100perm, nextelm);
    }

}

/*{
** Name: opn_clustck	- check X100 query for valid cluster permutation
**
** Description:
**      Verify that current permutation has no interleaving of X100
**	clusters.
**
** Inputs:
**	subquery			ptr to current subquery
**      jtree                           ptr to current tree node
**					being analyzed 
**	numleaves			number of leaf nodes in current tree
**
** Outputs:
**
**	Returns:
**	    TRUE	- if permutation is acceptible
**	    FALSE	- otherwise
**	Exceptions:
**	    none
**
...

{
    ...
    /* Call recursive function to load x100perm with current permutation
    ** of leaf nodes. */
    opn_clustperm(subquery, jtree, x100perm, &elmcount);

    /* Init. vector of bools, then loop over perm looking for interleave. */
    for (i = 0; i < numleaves; i++)
	clustmap.opv_bool[i] = FALSE;

    for (i = 0, prevvar = vbase->opv_rt[x100perm[0]]->opv_x100clid, 
	    clustmap.opv_bool[prevvar] = TRUE; i < numleaves; i++)
    {
	if (vbase->opv_rt[x100perm[i]]->opv_x100clid == prevvar)
	    continue;

	if (clustmap.opv_bool[vbase->opv_rt[x100perm[i]]->opv_x100clid])
	    return(FALSE);		/* we've already seen this one */

	prevvar = vbase->opv_rt[x100perm[i]]->opv_x100clid;
	clustmap.opv_bool[prevvar] = TRUE;
    }
...
**	13-april-2009 (dougi)
**	    Add cluster permutation check for X100 queries.
*/
OPN_STATUS
opn_process(
	OPS_SUBQUERY	   *subquery,
	i4                numleaves)
{
    ...
    */
					    ...
					    ** attach relations to leaves */
	{
	    ...
	    /* If this is an X100 query with some clustering, verify
	    ** that this permutation is valid (clusters don't interleave). */
	    if (FALSE && (subquery->ops_mask2 & OPS_HINTPASS) &&
		(global->ops_gmask & OPS_X100) && 
		    subquery->ops_x100clcnt > 0 &&
		    !opn_clustck(subquery, root, numleaves))
		continue;		    /* next perm if this one fails */
