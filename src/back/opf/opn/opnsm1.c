	...
	OPV_BMVARS         *vmap)
{
    ...
    bool		joinop = (eqh == (OPE_BMEQCLS *) NULL);
					/* hack flag for distinguishing 
					** opnncost.c call from opjjoinop.c */
	...
	OPB_IBF             bfno1;	/* boolean factor - index into
	    ...
	    if (bp->opb_mask & (OPB_OJJOIN|OPB_OJMAPACTIVE|OPB_BFPUSED))
		continue;		/* equi joins are not interesting at
					...
					** leaf of a qep
					** - also skip bfs in VW queries that 
					** have SAGG parms and have already 
					** been added to another CO-node */
            if (!bp->opb_virtual
                &&
		...
		if (joinop && bp->opb_parms && 
			subquery->ops_global->ops_gmask & OPS_X100)
		    bp->opb_mask |= OPB_BFPUSED;
					/* identify BFs in X100 queries with
					** SAGG parms */
	    }
	}
    }
    ...
    ** since only secondary indexes will be joined via a TID join */
	...
	{   /* for each equivalence class which is available from the subtree */
	    ...
	    {
		BTset((i4) eqcls, (char *) eqh); /* keep histos for these -
