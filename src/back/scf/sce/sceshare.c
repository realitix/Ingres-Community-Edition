...
**         Update evc_errf calls inline with sc0e_putAsFcn changes.
**      13-Jul-2011 (hanal04) SIR 125458
...
sce_einitialize( SCEV_CONTEXT *evc )
{
    ...
    */
    ...
    {
	(*evc->evc_errf)(E_SC0289_XSEV_ALLOC, (DB_ERROR *)NULL, &clerror, 1,
			 sizeof(pages), &pages);
	goto err_exit;
    }
    ev_created = TRUE;
    ...
    */
    ...
    if (clstat != OK)
    {
	(*evc->evc_errf)(E_SC0292_XSEV_SEM_INIT, (DB_ERROR *)NULL, NULL, 1,
...
err_exit:
    ...
    */
	...
	if (clstat != OK)
	{
	    (*evc->evc_errf)(E_SC0288_XSEV_SM_DESTROY, (DB_ERROR *)NULL, 
                             &clerror, 1, STlength(smnm), smnm);
	}
    }
    evc->evc_root = NULL;
    return (E_DB_FATAL);
...
sce_econnect( SCEV_CONTEXT *evc, i4  *evactive )
{
    ...
    *evactive = FALSE; 		/* Assume EV is not active */
	...
	if (clstat != OK)		
	{
	    ...
	    if ( !CXconfig_settings(CX_HAS_CSP_ROLE) )
	        (*evc->evc_errf)(E_SC028B_XSEV_ATTACH, (DB_ERROR *)NULL, 
                             &clerror, 1, STlength(smnm), smnm);
    ...
    */
	...
	ecb->evs_scount--;
	sce_munlock(&ecb->evs_gsemaphore);
	(*evc->evc_errf)(E_SC0293_XSEV_VERSION, (DB_ERROR *)NULL, NULL, 3,
	...
	(*evc->evc_errf)(E_SC028C_XSEV_EALLOCATE, (DB_ERROR *)NULL, NULL, 3,
...
sce_eregister( SCEV_CONTEXT *evc, SCEV_EVENT  *event, DB_ERROR  *error )
{
    ...
    */
	...
	{
	    sce_munlock(eunlock);
	    (*evc->evc_errf)(E_SC028D_XSEV_REGISTERED, (DB_ERROR *)NULL, 
                             NULL, 1,
	...
	else			/* No one's registered for this event yet */
	{
		...
		(*evc->evc_errf)(E_SC028C_XSEV_EALLOCATE, (DB_ERROR *)NULL, 
                             NULL, 3, sizeof("EV_REVENT")-1, "EV_REVENT",
			     sizeof(allocerr), &allocerr,
			     sizeof(ecb->evs_favailable), &ecb->evs_favailable);
...
sce_ederegister( SCEV_CONTEXT  *evc, SCEV_EVENT	*event, DB_ERROR  *error )
{
    ...
    */
	...
	{
	    sce_munlock(eunlock);
	    (*evc->evc_errf)(E_SC028E_XSEV_NOT_REGISTERED, (DB_ERROR *)NULL, 
                             NULL, 1,
			     sce_ename(event->ev_lname, event->ev_name, evname),
			     evname);
...
#   define		ESIG_MAX_ERR 10	/* Maximum number of errors in here.
	...
	*/
	...
	{
	    status = E_DB_ERROR;    /*@FIX_ME@ - should this abort? */
	    (*evc->evc_errf)(E_SC028F_XSEV_NO_SERVER, (DB_ERROR *)NULL, NULL, 1,
			     sizeof(PID), &orphan_err[err]);
	}
	for (err = 0; err < lk_errcnt; err++)
	{
	    /* Note -- don't set status, as this will abort the stmt */
	    (*evc->evc_errf)(lk_errmsg[err], (DB_ERROR *)NULL, &lkclerror, 2,
	   ...
	   DB_ERROR *error )
{
    ...
    *end_of_events = FALSE;				/* Assume more events */
	...
	{
	    sce_munlock(sunlock);
	    (*evc->evc_errf)(E_SC0291_XSEV_NOT_CONNECT, (DB_ERROR *)NULL, NULL,
                             2, sizeof(evc->evc_pid), &evc->evc_pid,
...
sce_edisconnect( SCEV_CONTEXT	*evc )
{
    ...
    */
	...
	if (lkerr)
	{
	    (*evc->evc_errf)(E_SC0290_XSEV_SIGNAL, (DB_ERROR *)NULL, &clerror, 
                             2, sizeof(PID), &evc->evc_pid,
...
sce_ewinit( SCEV_CONTEXT  *evc )
{
    ...
    if (clstatus != OK)
    {
	(*evc->evc_errf)(E_SC028A_XSEV_CRLOCKLIST, (DB_ERROR *)NULL, 
                         &clerror, 0);
	return(E_DB_ERROR);
    }
...
#define EVC_NO_SERVER "sce_ewinit"
        (*evc->evc_errf)(E_SC0291_XSEV_NOT_CONNECT, (DB_ERROR *)NULL, NULL, 2,
    ...
    if (clstatus != OK)
    {
        (*evc->evc_errf)(E_SC029F_XSEV_EVCONNECT, (DB_ERROR *)NULL, 
                         &clerror, 0);
	clstatus = LKrelease(LK_ALL, evc->evc_locklist,
			     (LK_LKID *)NULL, (LK_LOCK_KEY *)NULL,
			     (LK_VALUE *)NULL, &clerror);
