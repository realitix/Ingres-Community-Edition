/*
** Copyright (c) 1986, 2006 Actian Corporation
*/
...
**	    Drop AIC enable/disable functions, never used.
...
**      15-Sep-2011 (whiro01) b125701
**          Take out call to scf_cut_test() which was in "scftest.c"
**          which was deleted as part of change 512494.
**/
...
**          Added the SCC_BAT_CPY_ERR operation.
*/
DB_STATUS
...
SCF_CB             *cb;
{
	...
	{
	    sc0ePut(E_SC0023_INTERNAL_MEMORY_ERROR, &cb->scf_error, NULL, 0);
	    sc0ePut(internal_status, (DB_ERROR *)NULL, NULL, 0);
	    if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_EXDECL, NULL, NULL))
	    {
		if (ex_declared)
		    EXdelete();
...
scf_trace(DB_DEBUG_CB  *trace_cb)
{
	...
	case	SCS_TNOTIFY:
		...
		{
		    sc0e_uput(E_SC0228_INVALID_TRACE, (DB_ERROR *)NULL, 0, 0,
			      0, (PTR)0,
			      0, (PTR)0,
			      0, (PTR)0,
			      0, (PTR)0,
	...
	case	930:
		...
		switch (v1)
		{
		  ...
		  case 2:
		    ...
		    break;
		  case 3:
		    v2=(trace_cb->db_value_count==1 ? SC930_DEFAULT_BLOBSEGS
				    :trace_cb->db_vals[1]);
		    trace_val=ult_always_trace();
		    trace_val |= SC930_TRACE;
                    ult_set_always_trace(trace_val,Sc_main_cb->sc_pid);
		    ult_set_trace_blobseg(v2);
		    break;
		  default:
	...
	case	1000:
	    ...
	    sc0e_trace("\t   3  N \t- output only N segments (~2k) of blobs, 0=ALL");
	    break;
