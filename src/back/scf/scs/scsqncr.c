...
**      26-Jan-2011 (horda03) b124926
**          Once a sessions GCA connections are disposed of, the session must terminate.
**          Leaving the NEXT_OP state as CS_INPUT will result in a E_GC0005 error when
**          the session tries to set up a GC_RECEIVE.
**	30-Jan-2011 (kiria01) SIR 124690
**	    Add missing collID propagation
**      22-Jun-2011 (horda03) b125466
**          scs_cursor_remove may remove QSO objects using the sessions scb->scb_sscb.sscb_qsccb.
**          The sequencer however may be using the handle already stored in the
**          scb->scb_sscb.sscb_qsccb. If this is the case, the sequencer on exit will attempt
**          to destory the (QSO) handle, but this is now set to the last handle destroyed in
**          scs_cursor_remove, so an E_QS000F_BAD_HANDLE error occurs.
**      15-Jun-2011 (smeke01) b125445
**          Use PSQ_SPARALLEL so that we can give better error messages
**          for SET PARALLEL.
...
**          Initialise the qsf_feobj_id.qso_lname before QSO_CREATE calls.
**      13-Jul-2011 (hanal04) SIR 125458
**          Update cs_elog calls inline with sc0e_putAsFcn changes.
**	27-Aug-2011 (kschendel) b125701
**	    Change sscb state to enum for easier debugging.
...
static DB_STATUS scs_do_insert_to_copy(
	SCD_SCB *scb,
	PSQ_CB *psq_cb,
	SCC_GCMSG **msgpp);
...
**	    X100 drop-table ended up not being used, delete it.
*/
...
/* PSQ_X100_DGTT */		{SQ_PSF_MASK, 0},
/* PSQ_X100_CRCONST */		{SQ_PSF_MASK | SQ_OPF_MASK, 0},
/* PSQ_X100_CRINDEX */		{SQ_PSF_MASK, 0},
/* 197_notused */		{0, 0},
/* PSQ_X100_DRCONST */		{SQ_PSF_MASK, 0},
/* PSQ_X100_DRINDEX */		{SQ_PSF_MASK, 0},
/* 200 notused */		{SQ_PSF_MASK, 0},
/* 201 notused */		{SQ_PSF_MASK, 0},
/* PSQ_IVWCALL */  		{SQ_PSF_MASK | SQ_PSY_MASK, PSY_IVWCALL},
/* PSQ_X100_ALTABLE */		{SQ_PSF_MASK, 0},
/* 204 notused */		{SQ_PSF_MASK, 0},
/* 205 notused */		{SQ_PSF_MASK, 0},
/* PSQ_X100_DGTT_AS */		{SQ_PSF_MASK | SQ_OPF_MASK, 0},
	{0,0}
};
...
**      28-Sep-2009 (gefei01)
**          Enable CREATE INDEX on X100 table.
**	1-Oct-09 (gordy)
...
**	9-july-2010 (dougi)
**	    Add suport for "ALTER TABLE" and "DECLARE GLOBAL TEMPORARY TABLE"
**	    for IVW.
...
**          When performing batch inserts using the copy optimization
**          set qeu_msg_ptr to the first GCA_RE_DATA message. This will
**          allow COPY operation in QEF to call scc_error() with the new
**          SCC_BAT_CPY_ERR operation, in order to set error codes for
**          failed, error free inserts like duplicate key.
**       8-Feb-2011 (hanal04) Bug 124876
**          Batch inserts with copy optimization was allocating the
**          final inserts response message after the COPY operation.
**          if the last row hit a SCC_BAT_CPY_ERR failure scc_error
**          would either error or corrupt memory updating a response
**          that had not been allocated. For BCO (batch copy optim)
**          cases pre-allocate the final response message before
**          we perform the COPY.
**	24-Mar-2011 (kschendel) b125204 m1782
**	    Simplify batch insert-to-copy flags;  all we need from the
**	    parser is "was the last stmt an optimized insert execution"
**	    and we can figure out the rest (ie batch finish).  Make
**	    sure global psq-cb things are cleaned when batch ends.
**	    Chop insert-to-copy actions out of mainline, into a routine.
**	20-May-2011 (kschendel) m1959
**	    Make sure we shut down COPY if an interrupt arrives during
**	    insert-to-copy operations.
**	27-May-2011 (kschendel) m1260
**	    Set qmode in the copy fast-path so that the above code knows
**	    that it's copy, if we get there.  (Without qmode set, we
**	    didn't know to call end-copy.)
*/
DB_STATUS
...
#define			    SCS_RETRY_LIMIT	    10
    ...
    i4			i;
    i4			csi_no;
    PSQ_MODE		qmode = 0, qmode_saved = 0;
    i4			qsf_object = 0;
    i4			repeat_redef = 0;
    i4			recreate = 0;
    i4			abort_action = 0;
    ...
    SCC_GCMSG		*bco_prealloc_msg = (SCC_GCMSG *)NULL;
	...
	*/
	...
	if (DB_SUCCESS_MACRO(ret_val))
	    ret_val = E_DB_OK;
	qmode = PSQ_COPY;		/* for interrupt cleanup to see */
	...
	case CS_INPUT:
		...
		*next_op = CS_TERMINATE;
		...
		if (ret_val)
		{
		    sc0e_put(E_SC0240_DBMS_TASK_INITIATE, (DB_ERROR *)NULL, 
                             0, 1, sizeof(*sscb->sscb_ics.ics_eusername),
		...
		*/
		...
		{
		    i4 session_count;
		    sc0e_put(E_SC0241_VITAL_TASK_FAILURE, (DB_ERROR *)NULL, 
                             0, 1, sizeof(*sscb->sscb_ics.ics_eusername),
			     (PTR)sscb->sscb_ics.ics_eusername,
			     0, (PTR)0,
			     0, (PTR)0,
		...
		*/
		...
		if (status)
		{
		    ...
		    if (ret_val != E_US0010_NO_SUCH_DB )
                    {
		        sc0e_0_put(ret_val, (DB_ERROR *)NULL, 0);
                    }
		    else
		    {
			sc0e_put(ret_val, (DB_ERROR *)NULL, 0, 1,
		    ...
		    (VOID) scs_fire_db_alarms(scb, DB_CONNECT|DB_ALMSUCCESS);
                    sc0e_0_put(E_SC0123_SESSION_INITIATE, (DB_ERROR *)NULL, 0);
                    ...
                    if (status)
                    {
                        sc0e_0_put(status, (DB_ERROR *)NULL, 0 );
                        sc0e_0_uput(status, (DB_ERROR *)NULL, 0 );
		...
		*/
		...
		status = IIGCa_call(GCA_RQRESP,
                ...
                if (status != OK)
                {
                    ...
                    */
                    sc0e_0_put(gca_status, (DB_ERROR *)NULL, 0);
                    *next_op = CS_TERMINATE;
                ...
                {
                    sc0e_0_put(rrparms.gca_status, (DB_ERROR *)NULL, 
                               &rrparms.gca_os_status);
                    *next_op = CS_TERMINATE;
		...
		*next_op = CS_EXCHANGE;
		...
		TRdisplay(">>>>>>>>20C error 1: sscb_state = %x (%<%d.)\n",
			    sscb->sscb_state);
		sc0e_0_put(E_SC020C_INPUT_OUT_OF_SEQUENCE, (DB_ERROR *)NULL, 0);
		return(E_DB_ERROR);
	    }
	...
	case CS_TERMINATE:
	    ...
	    */
	    ...
	    {
		i4 session_count;
		sc0e_put(E_SC0241_VITAL_TASK_FAILURE, (DB_ERROR *)NULL, 0, 1,
	...
	default:
	    sc0e_0_put(E_SC0024_INTERNAL_ERROR, (DB_ERROR *)NULL, 0);
	    sc0e_put(E_SC0205_BAD_SCS_OPERATION, (DB_ERROR *)NULL, 0, 1,
		     ...
		     0, (PTR)0,
		     0, (PTR)0);
	    sc0e_0_put(E_SC021F_BAD_SQNCR_CALL, (DB_ERROR *)NULL, 0);
	    scd_note(E_DB_SEVERE, DB_SCF_ID);
	    break;
    }
...
#else
        sc0e_0_put(E_SC0024_INTERNAL_ERROR, (DB_ERROR *)NULL, 0);
        sc0e_put(E_SC0393_RAAT_NOT_SUPPORTED, (DB_ERROR *)NULL, 0, 0,
            0, (PTR)0,
            0, (PTR)0,
            0, (PTR)0,
            0, (PTR)0,
...

	...
	*/
	      ...
	      (sscb->sscb_qmode == PSQ_SECURE))
	{
	    ...
	    sc0e_uput(E_US13FE_NO_STAR_SUPP, (DB_ERROR *)NULL, 0, 1,
	...
	*/
	   ...
	   )
	{
		...
		*/
	    ...
	    if (ret_val = status)
	    {
		sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
		sc0e_0_put(0, &qeu.error, 0);
		qry_status = GCA_FAIL_MASK;
	    ...
	    */
	    ...
	    /* Clear flags relevant to statement about to be parsed.
	    ** alias-set: gets set when calling to recreate a dbproc, and not
	    ** clearing it may result in users being able to execute dbprocs
	    ** for which they have no permissions.
	    */
	    ps_ccb->psq_flag &= ~(PSQ_ALLDELUPD | PSQ_ALIAS_SET | PSQ_INSERT_TO_COPY);
	    ps_ccb->psq_flag2 &= ~(PSQ_BATCH | PSQ_LAST_BATCH);
	    ...
	    */
		...
		{
		    /*
		    ** Regular query, but no query text.
		    ** This indicates a batch-mode "repeat last parsed query",
		    ** not to be confused with traditional "repeated" queries!
		    ** Check to make sure that the client side (ie API) has
		    ** set the repeat-last-query flag in the GCA info, because
		    ** otherwise this is just a screw-up.
		    */
			...
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
			sc0e_0_put(E_SC02A0_NO_QUERY_TEXT, (DB_ERROR *)NULL, 0);
			qry_status = GCA_FAIL_MASK;
			break;
		    }
		    /* Take the most recent (current) query and duplicate it
		    ** into the QTEXT object allocated by scs-input.
		    */
		    ...
		    */
			...
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
			sc0e_put(E_SC02A1_INCORRECT_QUERY_PARMS, 
                                 (DB_ERROR *)NULL, 0, 2,
		...
		if (notext_copy_optim)
		{
		    ...
		    ps_ccb->psq_flag |= PSQ_INSERT_TO_COPY;  /* by definition */
		}
		else
		    status = psq_call(PSQ_PARSEQRY, ps_ccb, sscb->sscb_psscb);

		/* If stmt just parsed was copy-optimizable, remember for
		** next stmt incase it's a "repeat last query"
		*/
		if (ps_ccb->psq_flag & PSQ_INSERT_TO_COPY)
		...
		switch (ps_ccb->psq_error.err_code)
		{
			...
			sc0e_0_put(0, &ps_ccb->psq_error, 0);
			sc0e_0_uput(ps_ccb->psq_error.err_code, 
                                    (DB_ERROR *)NULL, 0);
			sc0e_0_uput(4700, (DB_ERROR *)NULL, 0);
			deadlock_occurred = 1;
			qry_status = GCA_FAIL_MASK;
			if (cscb->cscb_batch_count > 0)
			    cscb->cscb_eog_error = TRUE;
			...
			sc0e_0_uput(ps_ccb->psq_error.err_code, 
                                    (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, &ps_ccb->psq_error, 0);
			sc0e_0_uput(E_SC022A_SERVER_LOAD, (DB_ERROR *)NULL, 0);
			if (cscb->cscb_batch_count > 0)
			    cscb->cscb_eog_error = TRUE;
			qry_status = GCA_FAIL_MASK;
			break;
			...
			*/
		    ...
		    default:
			sc0e_0_put(E_SC0207_UNEXPECTED_ERROR, 
                                   (DB_ERROR *)NULL, 0);
			...
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, &ps_ccb->psq_error, 0);
			sc0e_0_put(E_SC0215_PSF_ERROR, (DB_ERROR *)NULL, 0);
		...
		sc0e_0_put(E_SC020C_INPUT_OUT_OF_SEQUENCE, (DB_ERROR *)NULL, 0);
		sc0e_0_uput(E_SC020C_INPUT_OUT_OF_SEQUENCE, 
                            (DB_ERROR *)NULL, 0);
		if (cscb->cscb_batch_count > 0)
		    cscb->cscb_eog_error = TRUE;
		qry_status = GCA_FAIL_MASK;
	    ...
	    /* Paranoia check that we don't try copy business if copy
	    ** control block pointer not set...don't want to see stale flags.
	    */
	    if (ps_ccb->psq_cp_qefrcb == NULL)
		ps_ccb->psq_flag &= ~PSQ_INSERT_TO_COPY;

	    /* Handle insert-to-copy optimization stuff if we just pushed
	    ** another row, or if batch is over and we need to finish up.
	    */
	    if (ps_ccb->psq_flag & PSQ_INSERT_TO_COPY
	      || (ps_ccb->psq_cp_qefrcb != NULL
		  && ps_ccb->psq_flag2 & PSQ_LAST_BATCH) )
	    {
		status = scs_do_insert_to_copy(scb, ps_ccb, &bco_prealloc_msg);
		if (status != E_DB_OK)
		{
		    /* Some sort of error */
		    qry_status = GCA_FAIL_MASK;
		...
		/* If optimized statement, we're done with it;  worker
		** routine cleaned up psf stuff, so just cycle around for
		** the next query.
		*/
		if (ps_ccb->psq_flag & PSQ_INSERT_TO_COPY)
		{
...

		...
		if (qmode == PSQ_REPCURS || qmode == PSQ_DELCURS)
		{
		    ...
		    {
			qry_status = GCA_FAIL_MASK;
			sc0e_0_uput(E_SC021E_CURSOR_NOT_FOUND, 
                                    (DB_ERROR *)NULL, 0);
			sscb->sscb_state = SCS_INPUT;
			*next_op = CS_EXCHANGE;
		...
		*/
		...
		if (DB_FAILURE_MACRO(status))
		{
			...
			*/
			...
			if (DB_FAILURE_MACRO(status))
			{
				sc0e_0_put(E_SC0215_PSF_ERROR, 
                                           (DB_ERROR *)NULL, 0);
				sc0e_0_put(0, &ps_ccb->psq_error, 0);
				sc0e_0_uput(0, &ps_ccb->psq_error, 0);
				scd_note(status, DB_PSF_ID);
			}
		    }
		    ...
		    {
			sc0e_0_put(0, &op_ccb->opf_errorblock, 0);
			sc0e_0_uput(0, &op_ccb->opf_errorblock, 0);
			sc0e_0_uput(4700, (DB_ERROR *)NULL, 0);
			deadlock_occurred = 1;
			if (cscb->cscb_batch_count > 0)
			    cscb->cscb_eog_error = TRUE;
		...
		|| (sscb->sscb_state == SCS_DELPROT))
	{
		...
		if (DB_FAILURE_MACRO(status))
		{
		    qry_status = GCA_FAIL_MASK;
		    sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
		...
		if (DB_FAILURE_MACRO(status))
		{
		    ...
		    {
			qry_status = GCA_FAIL_MASK;
			sc0e_0_put(E_SC0213_DMF_ERROR, (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, &dm_ccb->error, 0);
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
			scd_note(status, DB_DMF_ID);
			break;
	...
	else if (sscb->sscb_state == SCS_RPASSWORD)
	{
	    ...
	    */
	    ...
	    {
	        sc0e_0_put(cscb->cscb_gci.gca_status, (DB_ERROR *)NULL,
	    	    &cscb->cscb_gci.gca_os_status);
	        sc0e_0_put(E_SC022F_BAD_GCA_READ, (DB_ERROR *)NULL, 0);
	        sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
	        sscb->sscb_qmode = 0;
	        sscb->sscb_state = SCS_TERMINATE;
		sscb->sscb_cfac = DB_CLF_ID;
	    ...
	    */
		...
		/* Couldn't read the data, or not prompt reply */
	        sc0e_0_put(E_SC0225_BAD_GCA_INFO, (DB_ERROR *)NULL, 0);
	        sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
	        sscb->sscb_state = SCS_TERMINATE;
		sscb->sscb_cfac = DB_CLF_ID;
	        *next_op = CS_TERMINATE;
...

	...
	*/
	...
	{
	    qry_status = GCA_FAIL_MASK;
	    sc0e_0_uput(E_US126A_4714_ILLEGAL_STMT, (DB_ERROR *)NULL, 0);
	    sscb->sscb_state = SCS_INPUT;
	    *next_op = CS_EXCHANGE;
	    break;
	}
...

		...
		psf_qp_handle = 0;
		qe_ccb = sscb->sscb_qeccb;
		MEcopy((char *)&sscb->sscb_ics.ics_dbname, 
		    sizeof(DB_DB_NAME), (char *)&qe_ccb->qef_dbname);
...

		...
		if (qmode == PSQ_RETCURS)  /* Check, as may be fall through */
		{
			...
			MEcopy((char *)&scb->scb_sscb.sscb_ics.ics_dbname, 
			    sizeof(DB_DB_NAME), (char *)&qe_ccb->qef_dbname);
		    }
...

		...
		if (!(qef_wants_ruleqp || qef_wants_eiqp))
		{
		    ...
		    else qe_ccb->qef_stmt_type &= ~QEF_DGTT_AS_SELECT;
		}
		...
		*/
		...
		/* Set query (execution) start time to help detect long-
		** running queries.  A "query" is anything that doesn't look
		** like a sequencer return-back from QEF.
		** (Obviously we'll miss a lot of DDL, too, but oh well.)
		*/
		if ( ! (is_tproc_load_qp || qef_wants_ruleqp
			|| qef_wants_eiqp
			|| (qmode == PSQ_RETRIEVE && cquery->cur_state > CUR_FIRST_TIME)
		     ) )
		{
		    cquery->cur_start_secs = TMsecs();
		}
...

		...
		*/
		...
		{
		    qry_status = GCA_FAIL_MASK;
		    sc0e_0_uput(E_SC021E_CURSOR_NOT_FOUND, (DB_ERROR *)NULL, 0);
		    sscb->sscb_state = SCS_INPUT;
		    *next_op = CS_EXCHANGE;
		    break;
		}
...

		       ...
		       scan_scb = (SCD_SCB *)scan_scb->cs_scb.cs_next, cnt++)
		{
			...
			*/
			...
			sscb->sscb_flags &= ~(SCS_COPY_STARTED | SCS_INSERT_OPTIM);
			...
			if (DB_FAILURE_MACRO(status))
			{
				...
				sc0e_0_put(E_SC0215_PSF_ERROR, 
                                           (DB_ERROR *)NULL, 0);
				sc0e_0_put(0, &xa_ps_ccb->psq_error, 0);
				scd_note(status, DB_PSF_ID);
				ret_val = E_DB_ERROR;
			    }
			}
...

	/* QEF/DMF DBU operations are all executed in a similar manner */
	    ...
	    case PSQ_SSTATS:
            ...
            case PSQ_REG_INDEX:
            case PSQ_REG_IMPORT:
            case PSQ_X100_CRINDEX:
	    case PSQ_X100_DGTT:
            case PSQ_X100_ALTABLE:
		...
		*/
		...
		sscb->sscb_cfac = DB_QEF_ID;
                ...
                if (qmode == PSQ_X100_CRINDEX || qmode == PSQ_X100_ALTABLE ||
			qmode == PSQ_X100_DGTT)
                {
		    if (qmode == PSQ_X100_CRINDEX)
                	((QEU_CB *)qs_ccb->qsf_root)->qeu_d_op = 
							DMU_INDEX_TABLE;
		    else if (qmode == PSQ_X100_ALTABLE)
			((QEU_CB *)qs_ccb->qsf_root)->qeu_d_op = 
							DMU_ALTER_TABLE;
		    else if (qmode == PSQ_X100_DGTT)
			((QEU_CB *)qs_ccb->qsf_root)->qeu_d_op = 
							DMU_CREATE_TABLE;
		    status = qef_call(QEU_X100DBU,  qs_ccb->qsf_root);
                }
                else
		...
		*/
		...
		if  (qmode == PSQ_INDEX ||
                     qmode == PSQ_X100_CRINDEX ||
                     (qmode == PSQ_MODIFY && qef_wants_eiqp))
		{
		    i4 cont = 0;
		    /*
...

		...
		else
		{
			...
			*/
			...
			{
			    sc0e_0_uput(E_SC0030_CHANGESECAUDIT_IN_XACT, 
                                        (DB_ERROR *)NULL, 0);
			    qry_status = GCA_FAIL_MASK;
			    sscb->sscb_state = SCS_INPUT;
			    *next_op = CS_EXCHANGE;
			...
			*/
		        ...
		        {
			    sc0e_0_uput(E_SC035D_SET_ROLE_IN_XACT, 
                                        (DB_ERROR *)NULL, 0);
			    status=E_DB_ERROR;
		        ...
		        else if (Sc_main_cb->sc_secflags&SC_SEC_ROLE_NONE)
			{
			    ...
			    */
			    sc0e_put(E_US18E7_6375, (DB_ERROR *)NULL, 0, 1,
			...
			{
			    sc0e_0_put(E_SC0215_PSF_ERROR, (DB_ERROR *)NULL, 0);
			    sc0e_0_put(0, &psy_cb->psy_error, 0);
			    sc0e_0_put(E_SC0206_CANNOT_PROCESS, 
                                       (DB_ERROR *)NULL, 0);
			    sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                        (DB_ERROR *)NULL, 0);
			    scd_note(status, DB_PSF_ID);
...

		...
		*next_op = CS_EXCHANGE;
		break;
            ...
            case PSQ_SNUMOVFL:
                sscb->sscb_adscb->adf_exmathopt = ps_ccb->psq_numovfl;
                sscb->sscb_ics.ics_mathex = ps_ccb->psq_numovfl;
                sscb->sscb_state = SCS_INPUT;
                *next_op = CS_EXCHANGE;
                break;
...

		...
		if ((qmode == PSQ_SLOGGING) &&
		    (sscb->sscb_ics.ics_db_access_mode == DMC_A_READ))
		{
		    sc0e_0_uput(E_SC0381_SETLG_READONLY_DB, 
                                (DB_ERROR *)NULL, 0L);
		    qry_status = GCA_FAIL_MASK;
		    sscb->sscb_state = SCS_INPUT;
		    *next_op = CS_EXCHANGE;
		...
		** while in a transactions, but disallows others.
		...
		if (DB_FAILURE_MACRO(status))
		{
		    ...
		    {
			sc0e_0_uput(E_SC0380_SET_LOCKMODE_ROW, 
                                    (DB_ERROR *)NULL, 0);
			qry_status = GCA_FAIL_MASK;
		    }
		    else if (dm_ccb->error.err_code
		    ...
		    {
			sc0e_uput(E_SC0316_LOC_NOT_FOUND, (DB_ERROR *)NULL, 
                                  0, 1, dm_ccb->dmc_errlen_loc,
		    ...
		    {
			sc0e_uput(E_SC0317_LOC_NOT_AUTHORIZED, 
                                  (DB_ERROR *)NULL, 0, 1,
		    ...
		    {
			sc0e_0_uput(E_SC0300_EXDB_LOCK_REQ, 
                                    (DB_ERROR *)NULL, 0);
			qry_status = GCA_FAIL_MASK;
		    }
		    else if (dm_ccb->error.err_code
		    ...
		    {
			sc0e_0_uput(E_SC0301_LOC_EXISTS, (DB_ERROR *)NULL, 0);
			qry_status = GCA_FAIL_MASK;
		    }
                    else if (dm_ccb->error.err_code
                                    == E_DM0147_NOLG_BACKUP_ERROR)
                    {
                        sc0e_0_uput(E_DM0147_NOLG_BACKUP_ERROR, 
                                    (DB_ERROR *)NULL, 0);
                        qry_status = GCA_FAIL_MASK;
                    }
                    else if (dm_ccb->error.err_code
                    ...
                    {
                        sc0e_0_uput(E_DM0201_NOLG_MUSTLOG_ERROR, 
                                    (DB_ERROR *)NULL, 0);
                        qry_status = GCA_FAIL_MASK;
                    }
                    else if (dm_ccb->error.err_code
			...
			sc0e_0_uput(5922L, (DB_ERROR *)NULL, 0L);
			qry_status = GCA_FAIL_MASK;
                    }
		    else
		    {
			qry_status = GCA_FAIL_MASK;
			sc0e_0_put(E_SC0215_PSF_ERROR, (DB_ERROR *)NULL, 0);
			sc0e_0_put(dm_ccb->error.err_code, (DB_ERROR *)NULL, 0);
			sc0e_0_put(E_SC0206_CANNOT_PROCESS, 
                                   (DB_ERROR *)NULL, 0);
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
...

		...
		*/
		...
		{
		    qry_status = GCA_FAIL_MASK;
		    sc0e_0_uput(E_US1712_TRACE_PERM, (DB_ERROR *)NULL, 0);
		    ret_val = E_DB_ERROR;
		    sscb->sscb_state = SCS_INPUT;
		    *next_op = CS_EXCHANGE;
			...
			*/
			sc0e_0_put(E_SC023C_SXF_ERROR, (DB_ERROR *)NULL, 0);
		    }
		    break;
		}
		...
		*/
		...
		switch (qmode)
		{
			...
			STprintf(tp_info,"<unknown facility> %d",
				((DB_DEBUG_CB*)qs_ccb->qsf_root)->db_trace_point);
			sc0e_0_put(E_SC0024_INTERNAL_ERROR, 
                                   (DB_ERROR *)NULL, 0);
			ret_val = E_DB_FATAL;
		}
		if (DB_FAILURE_MACRO(status))
...

		...
		qry_status = GCA_FAIL_MASK;
		sc0e_0_uput(E_SC020D_NO_DISTRIBUTED, (DB_ERROR *)NULL, 0);
		sc0e_put(E_SC0209_PSF_REQ_DISTRIBUTED, (DB_ERROR *)NULL, 0, 1,
...

		...
		*/
		...
		{
		    sc0e_0_uput(E_SC002D_SET_SESS_AUTH_IN_XACT, 
                                (DB_ERROR *)NULL, 0);
		    qry_status = GCA_FAIL_MASK;
		    sscb->sscb_state = SCS_INPUT;
		    *next_op = CS_EXCHANGE;
			...
			*/
			for (save_status = E_DB_OK; ; save_status = status)
			{
			    ...
			    */
				...
				{
				    qry_status = GCA_FAIL_MASK;
				    sc0e_0_put(E_SC0216_QEF_ERROR, 
                                               (DB_ERROR *)NULL, 0);
				    sc0e_0_put(0, &qe_ccb->error, 0);
				    sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                                (DB_ERROR *)NULL, 0);
				    scd_note(status, DB_QEF_ID);
...

	    default:
		sc0e_put(E_SC020A_UNRECOGNIZED_QMODE, (DB_ERROR *)NULL, 0, 1,
			 sizeof(qmode), (PTR)&qmode,
			 0, (PTR)0,
			 0, (PTR)0,
			 0, (PTR)0,
		...
		if ((sscb->sscb_thandle) ||
		    ...
		    sc0e_put(E_SC020B_UNRECOGNIZED_TEXT, (DB_ERROR *)NULL, 0, 1,
			     ((PSQ_QDESC *) sscb->sscb_troot)->psq_qrysize,
		...
		sc0e_uput(E_SC020A_UNRECOGNIZED_QMODE, (DB_ERROR *)NULL, 0, 1,
			  sizeof(qmode), (PTR)&qmode,
			  0, (PTR)0,
			  0, (PTR)0,
			  0, (PTR)0,
		...
		if ((sscb->sscb_thandle) ||
			(qe_ccb->qef_qsf_handle.qso_handle))
		    sc0e_uput(E_SC020B_UNRECOGNIZED_TEXT, (DB_ERROR *)NULL, 
                        0, 1, ((PSQ_QDESC *) sscb->sscb_troot)->psq_qrysize,
			(PTR)((PSQ_QDESC *) sscb->sscb_troot)->psq_qrytext,
				  0, (PTR)0,
				  0, (PTR)0,
				  0, (PTR)0,
...

	...
	if( sscb->sscb_flags & SCS_DROP_PENDING )
	{
	    ...
	    */
	    ...
	    (void) scs_disassoc( cscb->cscb_assoc_id );
            ...
            /* Now that the session has been disassociated, ensure the
            ** next operation is to terminate the session.
            */
            *next_op = CS_TERMINATE;
	}
		    ...
		    || (sscb->sscb_force_abort == SCS_FAPENDING))
	{
	    ...
	    */
		...
		if (qe_copy != NULL && qe_copy->qeu_copy_ctl != NULL)
		{
		    ...
		    /* Clean up insert-to-copy stuff if in progress.  Note
		    ** that we'll set gca-fail-mask, which will also end the
		    ** batch, below;  so this is appropriate.
		    */
		    if (sscb->sscb_flags & SCS_COPY_STARTED)
		    {
			ps_ccb = sscb->sscb_psccb;  /* make sure */
			ps_ccb->psq_last_sname[0] = EOS;
			ps_ccb->psq_cp_qefrcb = NULL;
			sscb->sscb_flags &= ~(SCS_COPY_STARTED | SCS_INSERT_OPTIM);
		...
		*/
		...
		{
		    scs_cpinterrupt(scb);
		    sc0e_0_put(E_QE0024_TRANSACTION_ABORTED, 
                               (DB_ERROR *)NULL, 0);
		    sc0e_0_uput(E_QE0024_TRANSACTION_ABORTED, 
                                (DB_ERROR *)NULL, 0);
		...
		sc0e_0_put(E_QE0024_TRANSACTION_ABORTED, (DB_ERROR *)NULL, 0);
		sc0e_0_uput(E_QE0024_TRANSACTION_ABORTED, (DB_ERROR *)NULL, 0);
		sscb->sscb_force_abort = SCS_FACOMPLETE;
		*next_op = CS_INPUT;
		return(E_DB_OK);
	    }
	...
	*next_op = CS_EXCHANGE;
	...
	{
	    sc0e_0_uput(E_US1265_QEP_INVALID, (DB_ERROR *)NULL, 0);
	    sc0e_0_put(E_US1265_QEP_INVALID, (DB_ERROR *)NULL, 0);
	}
    }
		...
		*/
		...
		else
		{
		    ...
		    rdata = cscb->cscb_rdata;
                ...
                if (bco_prealloc_msg == (SCC_GCMSG *)NULL)
                {
		    message->scg_mtype = sscb->sscb_rsptype;
		...
		rdata->gca_rqstatus = qry_status;
                if ((bco_prealloc_msg == (SCC_GCMSG *)NULL) || (rdata->gca_rowcount != -1))
                {
		    rdata->gca_rowcount =
		        ((qry_status & GCA_FAIL_MASK)
		         ? -1
                ...
                bco_prealloc_msg = (SCC_GCMSG *)NULL;
		...
		i2 msg_name)
{
	...
	adu_sc930prtdataval(msg_name,parm_name,pcount,dbdv, 
			    scb->scb_sscb.sscb_adscb,f,bsegs);
	ult_close_tracefile(f);
    }
}
...
**       7-Jan-2011 (hanal04) Bug 124892
**          Check we have valid len, prec, scale for DBV values supplied
**          by a GCA partner. Invalid values can lead to all sorts of
**          nasty problems.
**      20-Jan-2011 (maspa05) SIR 124943
**          New SC930 record types QFETCH and QCLOSE for QUEL cursor fetch and
**          close, FETCH and CLOSE are now for SQL only. Remove confusing
**          ROWCOUNT - it's not really a rowcount and the information is in EQY
**          anyway
*/
DB_STATUS
scs_input(SCD_SCB *scb,
	  sequencer_state_enum *next_state )
...
#define	    GCA_DESC_SIZE   (sizeof(i4) + sizeof(i4) + sizeof(i4))
    ...
    *next_state = SCS_NOSTATE;
	...
	{
	    sc0e_0_put(gca_status, (DB_ERROR *)NULL, 
                       &cscb->cscb_gci.gca_os_status);
	    sc0e_0_put(E_SC022F_BAD_GCA_READ, (DB_ERROR *)NULL, 0);
	}
	...
	case GCA_CLOSE:
	{
		...
		** to GCA_FETCH with no pre-fetch. */
	    ...
	    if (ult_always_trace())
	    {
			...
			ltype=((sscb->sscb_ics.ics_qlang==DB_QUEL) ?
			         ((mode==GCA_CLOSE) ? SC930_LTYPE_QCLOSE:SC930_LTYPE_QFETCH) :
			          (mode==GCA_CLOSE) ? SC930_LTYPE_CLOSE:SC930_LTYPE_FETCH);

                        STprintf(tmp,"(ID=%d/%d)",
                                cquery->cur_qname.db_cursor_id[0],
                                cquery->cur_qname.db_cursor_id[1]);
                        ult_print_tracefile(f,ltype,
				tmp);
                        ult_close_tracefile(f);
                }
            }
...
# endif
			...
			if (dbdv.db_datatype != DB_QTXT_TYPE)
			{
			    ...
			    if (status)
			    {
				sc0e_0_put(0, &sscb->sscb_adscb->adf_errcb.ad_dberror, 0);
				sc0e_0_put( E_SC021C_SCS_INPUT_ERROR, 
                                            (DB_ERROR *)NULL, 0);
				sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                            (DB_ERROR *)NULL, 0);
				ret_val = E_DB_ERROR;
			...
			&& blob_input == FALSE)
		{
			...
			** creation of a new function. */
		    ...
		    qs_ccb->qsf_obj_id.qso_lname = 0;	/* object has no name */
                    qs_ccb->qsf_feobj_id.qso_lname = 0; /* an no alias        */
...
#endif
			sc930_dbdv.db_collID = DB_UNSET_COLL;
                        /* Check we have a valid len, prec, scale */
                        status = adc_lenchk(sscb->sscb_adscb, FALSE, 
                                            &sc930_dbdv,
                                            &temp_dbv);
                        if (status != E_DB_OK)
                        {
                            sc0e_0_put(0, &sscb->sscb_adscb->adf_errcb.ad_dberror, 0);
                            sc0e_put(E_SC0520_SCS_BAD_DBV, (DB_ERROR *)NULL, 
                                     0, 3, sizeof(sc930_dbdv.db_datatype),
                                     (PTR)&sc930_dbdv.db_datatype,
                                     sizeof(sc930_dbdv.db_length),
                                     (PTR)&sc930_dbdv.db_length,
                                     sizeof(sc930_dbdv.db_prec),
                                     (PTR)&sc930_dbdv.db_prec,
                                     0, (PTR)0,
                                     0, (PTR)0,
                                     0, (PTR)0);
                            sc0e_0_put( E_SC021C_SCS_INPUT_ERROR, 
                                        (DB_ERROR *)NULL, 0);
                            sc0e_0_uput(0, &sscb->sscb_adscb->adf_errcb.ad_dberror, 0);
                            ret_val = E_DB_ERROR;
                            break;
                        }
...
# endif
			qup->parm_dbv.db_collID = DB_UNSET_COLL;
...
# else
		...
		else
		{
			...
			qs_ccb->qsf_obj_id.qso_type = QSO_QTEXT_OBJ; /* saving the query text */
			qs_ccb->qsf_obj_id.qso_lname = 0;	/* object has no name */
                        qs_ccb->qsf_feobj_id.qso_lname = 0;     /* object has no alias */
			status = qsf_call(QSO_CREATE, qs_ccb);
			if (DB_FAILURE_MACRO(status))
			{
			    scs_qsf_error(status, qs_ccb->qsf_error.err_code,
...
# endif
			...
			{
			    blob_input = TRUE;
			    sc0e_0_uput(E_SC0132_LO_REPEAT_MISMATCH, 
                                        (DB_ERROR *)NULL, 0);
			    sc0e_0_put(E_SC0132_LO_REPEAT_MISMATCH, 
                                       (DB_ERROR *)NULL, 0);
		      	    ret_val = E_DB_ERROR;
			    break;
			}
...
# endif
			dbdv.db_collID = DB_UNSET_COLL;
                        ...
                        if (status != E_DB_OK)
                        {
                            sc0e_0_put(0, &sscb->sscb_adscb->adf_errcb.ad_dberror, 0);
                            sc0e_put(E_SC0520_SCS_BAD_DBV, (DB_ERROR *)NULL, 
                                     0, 3, sizeof(dbdv.db_datatype),
                                     (PTR)&dbdv.db_datatype,
                                     sizeof(dbdv.db_length),
                                     (PTR)&dbdv.db_length,
                                     sizeof(dbdv.db_prec),
                                     (PTR)&dbdv.db_prec,
                                     0, (PTR)0,
...

	...
	case GCA_QUERY:
	{
	    ...
	    */
		...
		for (;;)
		{
		    ...
		    qs_ccb->qsf_obj_id.qso_type = QSO_QTEXT_OBJ; /* saving the query text */
		    qs_ccb->qsf_obj_id.qso_lname = 0;	/* object has no name */
                    qs_ccb->qsf_feobj_id.qso_lname = 0; /* and no alias       */
		    status = qsf_call(QSO_CREATE, qs_ccb);
		    if (DB_FAILURE_MACRO(status))
		    {
			scs_qsf_error(status, qs_ccb->qsf_error.err_code,
	    ...
	    {
		sc0e_0_uput(E_SC0132_LO_REPEAT_MISMATCH, (DB_ERROR *)NULL, 0);
		sc0e_0_put(E_SC0132_LO_REPEAT_MISMATCH, (DB_ERROR *)NULL, 0);
		ret_val = E_DB_ERROR;
		break;
	    }
	...
	case GCA_CDATA:
	    sc0e_0_put(E_SC0250_COPY_OUT_OF_SEQUENCE, (DB_ERROR *)NULL, 0);
	    sc0e_0_uput(E_SC0250_COPY_OUT_OF_SEQUENCE, (DB_ERROR *)NULL, 0);
	    sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
	    ret_val = E_DB_ERROR;
	    break;
	...
	case GCA_MD_ASSOC:
	    ...
	    {
		sc0e_put(E_SC0230_INVALID_CHG_PROT_LEN, (DB_ERROR *)NULL, 0, 1,
			 ...
			 0, (PTR)0,
			 0, (PTR)0);
		sc0e_uput(E_SC0230_INVALID_CHG_PROT_LEN, (DB_ERROR *)NULL, 0, 1,
			  sizeof(buf->gca_l_user_data),
			  (PTR)&buf->gca_l_user_data,
			  0, (PTR)0,
			  0, (PTR)0,
			  0, (PTR)0,
			      0, (PTR)0,
			  0, (PTR)0);
		sc0e_0_put(E_SC022B_PROTOCOL_VIOLATION, (DB_ERROR *)NULL, 0);
		sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
		return(E_DB_SEVERE);
	    }
	    else if (buf->gca_user_data[0].gca_p_index != GCA_SUPSYS)
	    {
		sc0e_put(E_SC0231_INVALID_CHG_PROT_CODE, (DB_ERROR *)NULL, 0, 1,
			 ...
			 0, (PTR)0,
			     0, (PTR)0,
			 0, (PTR)0);
		sc0e_uput(E_SC0231_INVALID_CHG_PROT_CODE, (DB_ERROR *)NULL, 
                          0, 1,
			  sizeof(buf->gca_user_data[0].gca_p_index),
			  (PTR)&buf->gca_user_data[0].gca_p_index,
			  0, (PTR)0,
	...
	default:
	    sc0e_put(E_SC0226_BAD_GCA_TYPE, (DB_ERROR *)NULL, 0, 1,
		     ...
		     0, (PTR)0,
		     0, (PTR)0);
	    sc0e_uput(E_SC0226_BAD_GCA_TYPE, (DB_ERROR *)NULL, 0, 1,
		      sizeof(mode), (PTR)&mode,
		      0, (PTR)0,
		      0, (PTR)0,
		      0, (PTR)0,
	...
	*/
	...
	{
	    sc0e_put(E_SC022C_PREMATURE_MSG_END, (DB_ERROR *)NULL, 0, 2,
	      ...
	      SCD_SCB *scb )
{
	...
	*/
    ...
    {
	sc0e_0_put(err_code, (DB_ERROR *)NULL, 0);
	sc0e_0_uput(err_code, (DB_ERROR *)NULL, 0);
    }
	...
	sc0e_0_put(E_SC0024_INTERNAL_ERROR, (DB_ERROR *)NULL, 0);
	sc0e_0_put(err_code, (DB_ERROR *)NULL, 0);
	sc0e_0_put(error_blame, (DB_ERROR *)NULL, 0);
	sc0e_0_put(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
	sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
    }
	...
	*/
	...
	sc0e_0_put(E_SC0216_QEF_ERROR, (DB_ERROR *)NULL, 0);
	sc0e_0_put(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
        sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
    }
}
	      ...
	      i4 error_blame )
{
    ...
    {
	sc0e_0_put(E_SC020F_LOST_LOCK, (DB_ERROR *)NULL, 0);
	sc0e_0_put(err_code, (DB_ERROR *)NULL, 0);
	sc0e_0_put(error_blame, (DB_ERROR *)NULL, 0);
    ...
    {
	sc0e_0_put(E_SC0217_QSF_ERROR, (DB_ERROR *)NULL, 0);
	sc0e_0_put(err_code, (DB_ERROR *)NULL, 0);
	sc0e_0_put(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
                ...
                QEU_COPY *qeu_copy )
{
    ...
    if (sscb->sscb_flags & SCS_STAR)
    {
	sc0e_0_uput(E_SC0131_LO_STAR_MISMATCH, (DB_ERROR *)NULL, 0);
	sscb->sscb_req_flags |= SCS_EAT_INPUT_MASK;
	return(E_DB_WARN);
    }
...
/* ********* END OF #if 0 from way above */
	...
	segment_dv.db_collID = DB_UNSET_COLL;
	...
	sc0e_0_put(E_SC031A_LO_PARAM_ERROR, (DB_ERROR *)NULL, 0);
	sc0e_0_uput(E_SC031A_LO_PARAM_ERROR, (DB_ERROR *)NULL, 0);
...
# endif
	    ...
	    */
	    ...
	    if (status)
	    {
		sc0e_0_put(0, &scb->scb_sscb.sscb_adscb->adf_errcb.ad_dberror, 0);
		sc0e_0_put( E_SC021C_SCS_INPUT_ERROR, (DB_ERROR *)NULL, 0);
		sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
		ret_val = E_DB_ERROR;
		break;
	    }
...
# endif
	    ...
	    */
	    ...
	    {
		sc0e_0_uput(E_SC0130_LO_DBP_MISMATCH, (DB_ERROR *)NULL, 0);
	     ...
	     char	  **p_buf )
{
	...
	*p_buf += qry_size;
		...
		((PSQ_QDESC *)scb->scb_sscb.sscb_troot)->psq_qrysize + sizeof(SC0M_OBJECT))
	{
	    ...
	    {
		sc0e_0_put(ret_val, (DB_ERROR *)NULL, 0);
		sc0e_0_uput(ret_val, (DB_ERROR *)NULL, 0 );
		scd_note(E_DB_SEVERE, DB_SCF_ID);
		return(ret_val);
	    }
	}
...

{
    ...
    ** client, but I haven't found it. */
	...
	sc0e_uput(E_US10F0_4336, (DB_ERROR *)NULL, 0, 2,
...
static char     *resc_control="Resource Control";
    ...
    sc0e_uput(E_SC0313_NOT_LICENSED, (DB_ERROR *)NULL, 0, 2,
	      0, (PTR)stmt,
	      0, (PTR)package,
	      0, (PTR)0,
	      0, (PTR)0,
...
**      22-Jun-2011 (horda03) b125466
**          If QSO objects are destroyed, restore scb->scb_sscb.sscb_qsccb to its
**          original value, as it may be destroyed in the scs_sequencer.
*/
VOID
		  ...
		  SCS_CSI *cursor )
{
	    ...
	    */
		    ...
		    (c->csi_rep_parm_lk_id == c->csi_tlk_id))
		{
		    ...
		    c->csi_tlk_id = 0;
                ...
                {
                   saved_handle = TRUE;
                   lk_id = qs_ccb->qsf_lk_id;
                   handle = qs_ccb->qsf_obj_id.qso_handle;
                }
    ...
    if (saved_handle)
    {
       ...
       qs_ccb->qsf_lk_id = lk_id;
       qs_ccb->qsf_obj_id.qso_handle = handle;
    }
    return;
}

		 ...
		 PTR     	*lowkspp)
{
    ...
    lowksp->adw_fip.fip_work_dv.db_collID = DB_NOCOLLATION;
    *lowkspp = (PTR) lowksp;
    return(E_DB_OK);
}

	...
	** so the "if" has been removed. */
	/* if (non_rep_curs || opening_rep_curs) */
	{
		...
		*/
		...
		else
		{
		    ...
		    qs_ccb->qsf_obj_id.qso_lname = 0;
                    qs_ccb->qsf_feobj_id.qso_lname = 0;
		...
		** QEQ_CLOSE it
		...
		{
		    sc0e_0_uput(E_SC021D_TOO_MANY_CURSORS, 
                                (DB_ERROR *)NULL, 0);
		}
...
)
{
	...
	*/
	...
	if (DB_FAILURE_MACRO(status))
	{
		scs_qsf_error(status, qs_ccb->qsf_error.err_code,
		E_SC0210_SCS_SQNCR_ERROR);
		sc0e_0_put(E_SC033C_SET_SESSION_ERR, (DB_ERROR *)NULL, 0);
		break;
	}
	qsf_object = 1;
	    ...
	    */
		...
		{
	           sc0e_uput(E_SC039A_SET_SESSION_READWRITE_ERR, 
                    (DB_ERROR *)NULL, 0, 0,
	            0, (PTR)0,
	            0, (PTR)0,
	            0, (PTR)0,
                ...
                {
	           sc0e_uput(E_SC031C_ONERR_XCT_INPROG, (DB_ERROR *)NULL, 0, 0,
	            0, (PTR)0,
	            0, (PTR)0,
	            0, (PTR)0,
	            0, (PTR)0,
...
)
{
	...
	*/
	if((privs& scb->scb_sscb.sscb_ics.ics_maxustat) != privs)
	{
		...
		*/
	        sc0e_uput(E_US2471_9329_INVLD_SET_PRIV, (DB_ERROR *)NULL, 0, 0,
	         0, (PTR)0,
	         0, (PTR)0,
	         0, (PTR)0,
	         0, (PTR)0,
...
)
{
	...
	*/
	...
	{
		sc0e_uput(E_SC033F_INVLD_PRIORITY, (DB_ERROR *)NULL, 0, 1,
	...
	*/
	...
	if(clstat!=OK)
	{
		...
		*/
		sc0e_uput(E_SC033E_SET_SESSION_PRIORITY, (DB_ERROR *)NULL, 0, 2,
			 sizeof(req_priority), (PTR)&req_priority,
...
scs_mxsession(SCD_SCB *scb, PSY_CB *psy_cb)
{
	...
	*/
	if(psy_cb->psy_ctl_dbpriv & DBPR_IDLE_LIMIT)
	{
		...
		{
		    /* Idle limit invalid */
		    sc0e_uput(E_SC0342_IDLE_LIMIT_INVALID, (DB_ERROR *)NULL, 
                         0, 1,
	...
	if(psy_cb->psy_ctl_dbpriv & DBPR_CONNECT_LIMIT)
	{
			...
			scb->scb_sscb.sscb_ics.ics_connect_limit!= -1)
		{
		    ...
		    sc0e_uput(E_SC0343_CONNECT_LIMIT_INVALID, (DB_ERROR *)NULL, 
                         0, 1,
...
)
{
        ...
        if(ps_ccb->psq_ret_flag&PSQ_SET_NOROLE)
	{
	    ...
	    */
	    ...
	    {
		sc0e_uput(E_SC035F_SET_ROLE_NODBACCESS, (DB_ERROR *)NULL, 0,1,
			 sizeof("none")-1,
			 (PTR)"none",
			  0, (PTR)0,
			  0, (PTR)0,
	...
	else
	{
		...
		*/
		sc0e_0_uput(E_SC034D_ROLE_NOT_AUTHORIZED, (DB_ERROR *)NULL, 0);
		status=E_DB_ERROR;
		break;
	    }
	    else if (status!=E_DB_OK)
		...
		*/
		sc0e_0_uput(E_SC035B_ROLE_CHECK_ERROR, (DB_ERROR *)NULL, 0);
		sc0e_0_put(E_SC035B_ROLE_CHECK_ERROR, (DB_ERROR *)NULL, 0);
		break;
	    }
	    /*
	    ** OK to use this role
    ...
    */
	...
	if (tdesc)
	{
	    status = scs_compress(tdesc, scb->scb_cscb.cscb_tuples,
			    (i4)qe_ccb->qef_rowcount, &total_dif);
...

/*
** Name - scs_do_insert_to_copy
**
** Description:
**	After a query is parsed (or processed with PSQ_COPYBUF), if either:
**	- the query is an insert being optimized into copy for this batch, or
**	- the batch is over (just did the last query) and some insert-to-copy
**	  optimization occurred during the batch,
**
**	then we come here to do COPY things.  If the most recent query
**	was an execution of the insert-to-copy query, or if we're finishing
**	and have rows left to do, we may need to push buffered rows over
**	to COPY.  And, if we're finishing the batch, the copy needs to
**	be ended.
**
**	At present, only one (prepared) query can have insert-to-copy
**	optimization applied, so there's one pointer to the parser
**	constructed copy-control stuff, in psq-cp-qefrcb.  If the last
**	query was an execution of the optimized insert, PSQ_INSERT_TO_COPY
**	will be set.  If the batch is ending, PSQ_LAST_BATCH will be set.
**	If we finish the copy, we'll clean up some of the sequencer and
**	psq-cb stuff so it's clean for next time.
**
** Inputs:
**	scb			Sequencer control block
**	psq_cb			Query-parse control block for convenience
**	msgpp			An output
**
** Outputs:
**	msgpp			(SCC_GCMSG **) Preallocated message area
**	may set copy-started in sequencer flags;
**	Returns E_DB_OK or error status;
**	if error, scs_qef_error was called if needed
**
** History:
**	25-Mar-2011 (kschendel) b125204 m1782
**	    Extract from mainline as part of fixing / simplifying batch
**	    insert to copy optimization.
**	29-Mar-2011 (kschendel) more b125204
**	    Repair / improve 124876 fix (proper error reporting) by
**	    maintaining a sequence number, and tagging the response
**	    with the copy "id" and sequence within copy.
*/

static DB_STATUS
scs_do_insert_to_copy(SCD_SCB *scb, PSQ_CB *psq_cb, SCC_GCMSG **msgpp)
{
    bool send_buffer;
    DB_STATUS status;
    QEF_RCB *rcb;		/* Parser constructed copy QEF-RCB */
    QEU_CB qeu;			/* For transaction mgmt */
    QEU_COPY *qe_copy;		/* COPY control block */
    SCC_CSCB *cscb = &scb->scb_cscb;
    SCS_SSCB *sscb = &scb->scb_sscb;

    rcb = psq_cb->psq_cp_qefrcb;
    qe_copy = rcb->qeu_copy;

    /* If really an insert which was queued up for copy, preallocate a
    ** response message for the insert.
    */
    *msgpp = NULL;
    if (psq_cb->psq_flag & PSQ_INSERT_TO_COPY)
    {
	GCA_RE_DATA *rdata;
	SCC_GCMSG *message;

	/* Generate sequence number for row just queued */
	++ qe_copy->qeu_cp_scf_seq;
	...
	*msgpp = message;
	message->scg_marea = (PTR)message + sizeof(SCC_GCMSG);
	/* mark for deallocation after message send */
	message->scg_mask = SCG_QDONE_DEALLOC_MASK | SCG_NODEALLOC_MASK | SCG_BCO_RESP_MASK;
	message->scg_mtype = sscb->sscb_rsptype;
	if (message->scg_mtype == GCA_RETPROC)
	    message->scg_mtype = GCA_RESPONSE;
	message->scg_next = (SCC_GCMSG *) &cscb->cscb_mnext;
	...
	message->scg_msize = sizeof(GCA_RE_DATA);
	rdata = (GCA_RE_DATA *)message->scg_marea;
	rdata->gca_rqstatus = 0;
	rdata->gca_rowcount = 0;
	/* Id this response so that we can find it if a later copy call
	** generates a dup-key or dup-row error.
	*/
	message->scg_mdescriptor = (PTR) qe_copy;
	message->scg_sequence = qe_copy->qeu_cp_scf_seq;
    }

    /*
    ** check if we need to push a bufferload off to QEF. This can be:
    ** 1) we are finished with the batch and there are rows buffered, or:
    ...
    */
    send_buffer = ((psq_cb->psq_flag2 & PSQ_LAST_BATCH && qe_copy->qeu_cur_tups > 0)
      || qe_copy->qeu_load_buf
      || ((qe_copy->qeu_stat & CPY_SMALL) == 0 &&
		    qe_copy->qeu_cur_tups >= BEST_LOAD_TUPS ));

    /* If we're going to call QEF, end the internal transaction used to
    ** parse whatever the last query was.
    */
    if (send_buffer || psq_cb->psq_flag2 & PSQ_LAST_BATCH)
    {
	memset(&qeu, 0, sizeof(QEU_CB));
	qeu.qeu_db_id = (PTR) sscb->sscb_ics.ics_opendb_id;
	qeu.qeu_d_id = scb->cs_scb.cs_self;
	qeu.qeu_qmode = PSQ_COPY;
	...
	{
	    /* start copy */
	    rcb->qef_cb = sscb->sscb_qescb;
	    rcb->qef_sess_id = scb->cs_scb.cs_self;
	    rcb->qef_db_id = sscb->sscb_ics.ics_opendb_id;
	    rcb->qef_eflag = QEF_INTERNAL;
		...
		*/
	    ...
	    status = qef_call(QEU_B_COPY, rcb);
	    if (DB_FAILURE_MACRO(status))
	    {
		scs_qef_error(status, rcb->error.err_code,
				    E_SC0210_SCS_SQNCR_ERROR, scb);
		sscb->sscb_flags &= ~SCS_INSERT_OPTIM;
		psq_cb->psq_last_sname[0] = EOS;
	...
	/* load the data */
	qe_copy->qeu_stat |= CPY_INS_OPTIM;
	status = qef_call(QEU_R_COPY, rcb);
	if (DB_FAILURE_MACRO(status))
	{
	    qe_copy->qeu_stat |= CPY_FAIL;
	    /* close copy and rollback rows */
	    scs_copy_error(scb, rcb, 0);
	    sscb->sscb_flags &= ~(SCS_COPY_STARTED | SCS_INSERT_OPTIM);
	    psq_cb->psq_last_sname[0] = EOS;
	    psq_cb->psq_cp_qefrcb = NULL;
    ...
    sscb->sscb_cquery.cur_row_count = 1;
    if (psq_cb->psq_flag2 & PSQ_LAST_BATCH)
    {
	/* We're going to be done with this insert-copy group
	...
	*/
	...
	/* finish copy */
	qe_copy->qeu_stat = CPY_INS_OPTIM;
	status = qef_call(QEU_E_COPY, rcb);
	if (DB_FAILURE_MACRO(status))
	{
	    scs_qef_error(status, rcb->error.err_code,
			  E_SC0210_SCS_SQNCR_ERROR, scb);
	    sc0e_0_put(E_SC0251_COPY_CLOSE_ERROR, (DB_ERROR *)NULL, 0);
	    qe_copy->qeu_stat |= CPY_FAIL;
	    sscb->sscb_cfac = DB_CLF_ID;
	    return (E_DB_ERROR);
	}
    ...
    /* Clean up.  If last query was one of the inserts, it's done, clean
    ** up the PSF objects used to parse it.  If last query was something
    ** else, presumably we're finishing up, restart an internal transaction
    ** so that the statement can complete.  (COPY end will have committed
    ** if in autocommit mode;  in no-autocommit, we need to put the txn
    ** back into "internal transaction" mode.)
    */
    if (psq_cb->psq_flag & PSQ_INSERT_TO_COPY)
    {
	QSF_RCB *qs_ccb;
	...
	qs_ccb->qsf_obj_id.qso_handle = psq_cb->psq_result.qso_handle;
	qs_ccb->qsf_lk_id = psq_cb->psq_qso_lkid;
	status = qsf_call(QSO_DESTROY, qs_ccb);
	if (DB_FAILURE_MACRO(status))
	{
	    scs_qsf_error(status, qs_ccb->qsf_error.err_code,
	...
	qeu.qeu_eflag = QEF_EXTERNAL;
	qeu.qeu_flag = QEF_READONLY;
	(void) qef_call(QEU_BTRAN, &qeu);
    }
    return (E_DB_OK);

} /* scs_do_insert_to_copy */

/*
**  29-Sep-2004 (fanch01)
