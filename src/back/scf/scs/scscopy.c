...
**	    Minor change to error handling for vwloader pseudo-copy.
*/
...

{
    ...
    */
    ...
    MEcopy((char *)&sscb->sscb_ics.ics_dbname,
		sizeof(qe_ccb->qef_dbname), (char *)&qe_ccb->qef_dbname);
    ...
    */
    ...
    if (DB_FAILURE_MACRO(status))
    {
	/* Save info since the qe-copy will disappear */
	bool	vwload = (qe_copy->qeu_stat & CPY_VWLOAD) != 0;
	i4	qeu_b_copy_err;
	...
	*/
	...
	if (sscb->sscb_interrupt)
	    return (SEQUENCER_BREAK);
	/* Avoid the SC0210 if it's a VWLOAD and "already reported", since
	** the SC0210 is of no value to the vwloader program.
	*/
	if (!vwload || qeu_b_copy_err != E_QE0122_ALREADY_REPORTED)
...
**	    Hatchet COPY code out of sequencer mainline.
**	28-Jul-2011 (kschendel) SIR 125587
**	    Deal with vwloader pseudo-copy termination.
*/
...

{
	...
	*next_op = CS_EXCHANGE;
	...
	sc0e_put(E_SC022E_WRONG_BLOCK_TYPE, (DB_ERROR *)NULL, 0, 2,
	...
	*/
	...
	if (cpi_endcopy)
	{
	    ...
	    ** response to the Front End.  If it's a VWLOAD pseudo-copy,
	    ** the front-end will return the loaded rowcount in the
	    ** RESPONSE message, pass it for QEF to see.  (Normally,
	    ** the gca-rowcount is irrelevant.)
	    */
	    if (qe_copy->qeu_stat & CPY_VWLOAD)
	    {
		GCA_RE_DATA *rsp = (GCA_RE_DATA *) cpi_data;
		qe_copy->qeu_count = rsp->gca_rowcount;
		if (qe_copy->qeu_count < 0)
		    qe_copy->qeu_count = 0;	/* Paranoia */
	    }
...

{
	...
	*/
	if (qe_copy && qe_copy->qeu_error)
	{
		...
		*ret_val = E_DB_WARN;	/* So we get GCA_FAIL_MASK */
	    ...
	    {
		sc0e_0_uput(5842L /* user error */, (DB_ERROR *)NULL, 0);
	    }
	    else
	    {
		char	    buf[16];
		...
		**       successfully copied. */
		...
		sc0e_uput(5841L /* user error */, (DB_ERROR *)NULL, 0, 1,
	...
	*/
	...
	sc0e_0_put(E_SC021B_BAD_COPY_STATE, (DB_ERROR *)NULL, 0);
	sc0e_0_uput(E_SC0206_CANNOT_PROCESS, (DB_ERROR *)NULL, 0);
	scd_note(E_DB_ERROR, DB_SCF_ID);
	    ...
	    i++, cpdom_desc = cpdom_desc->cp_next)
	{
		...
		else
		{
		    ...
		    */
		    ...
		    if (DB_FAILURE_MACRO(status))
		    {
			sc0e_0_put(0, &scb->scb_sscb.sscb_adscb->adf_errcb.ad_dberror,
				 0);
			sc0e_0_uput(0, &scb->scb_sscb.sscb_adscb->adf_errcb.ad_dberror,
				 0);
			sc0e_0_uput(E_SC0206_CANNOT_PROCESS, 
                                    (DB_ERROR *)NULL, 0);
			return (E_DB_ERROR);
                ...
                i4	copy_done )
{
    ...
    */
	...
	if (DB_FAILURE_MACRO(status))
	{
		...
		sc0e_0_put(qe_ccb->error.err_code, (DB_ERROR *)NULL, 0);
		sc0e_0_put(E_SC0210_SCS_SQNCR_ERROR, (DB_ERROR *)NULL, 0);
		scs_cpsave_err(qe_copy, E_SC0251_COPY_CLOSE_ERROR);
	    }
	}
    }
    ...
    */
	...
	for (i = 0; i < 5; i++)
	{
		...
		sc0e_0_uput(qe_copy->qeu_uputerr[i], (DB_ERROR *)NULL, 0);
	}
...
scs_cpinterrupt(SCD_SCB *scb )
{
	...
	**	Write an error
	*/
	sc0e_0_put(status = cur_error, (DB_ERROR *)NULL, 0);
    ...
    if (status)
    {
	sc0e_0_put(status = error, (DB_ERROR *)NULL, 0);
    }
}

/*{
