...
**      to SCF users by the sequencer module.  A goodly portion of this
**	file is concerned with asynchonous interrupts sent here via the
**	"attention" dispatch from CS.  Such interrupts may be truly
**	asynchronous, being posted by some other thread;  or they may
**	be pseudo-asynchronous, posted by some sort of poll for a
**	message (typically an attention message from the front-end client).
**
**	As a historical note, originally this module made a distinction
**	between AIC's (Asynchronous Ingres Calls) and PAINE's (Priority
**	Asynchronous INgres Events).  The original intent for the distinction
**	is lost to the mists of time (and the multiplicity of owners of
**	the code line, no doubt).  PAINE's were apparently somewhat akin
**	to NMI's (non-maskable interrupts), while AIC's could be disabled
**	and enabled.  However, it seems that nothing ever used the
**	"cs-server-event" event code that asked for a PAINE instead of
**	and AIC;  and nobody ever used the disable/enable feature of
**	AIC's!
** 
...
**      took place.  Most handlers will return OK;  a FAIL (notOK)
**	return is a special DMF thing.
**
**          scs_declare() - declare an interrupt/attn handler
**          scs_clear() - delete an interrupt/attn
**	    scs_attn()	-   the attn delivered to scf, which calls all
**				others.
...
**	    After years of asking people what the difference between a
**	    PAINE and an AIC is, decide that the difference is that PAINE's
**	    never happen while AIC's do.  Also, delete the AIC disable/enable
**	    feature, which was also never invoked.
...
**
** Description:
**      This function allow the user to specify an interrupt (attn) handler
**      to SCF.  Upon receipt of any asynchronous event for the session, 
**      SCF will deliver the attn to the facility (via the handler) as
**	specified below.  By default, SCF will deliver attn's only if the
**      facility in question is running.  This sounds great, except that
**	it's the sequencer's notion of which facility is running that
**	we pay attention to!  which basically means that PSF (*), OPF,
**	and QEF can expect to only get attns if they are running, but
**	since the sequencer knows squat about DMF, GWF, or VWF, those
**	facilities had better ask for "always" notification via the
**	SCS_ALWAYS_MASK.
**
**	(*) PSF doesn't actually care about attns, nor does it register
**	an attn handler.  But if it did, it could ask to be notified
**	only when SCF thought that PSF was in control.
**
**      All asynchronous functions (or AFCNs) are expected to be declared 
**      as follows. 
** 
...
**      the address of the session control block for which the event 
**      took place.  Most handlers will want to return OK;  a notOK
**	return is a special DMF thing.
**
** Inputs:
...
**					    of facility activation
**          .scf_afcn                   address of attn function as described
**					    above
**
** Outputs:
**      scf_cb
...
scs_declare(SCF_CB *scf_cb, SCD_SCB *scb )
{
    ...
    if (scf_cb->scf_ptr_union.scf_afcn == NULL)
    {
	SETDBERR(&scf_cb->scf_error, 0, E_SC000E_AFCN_ADDR);
	return(E_DB_ERROR);
    }

    if (scf_cb->scf_nbr_union.scf_amask & ~(SCS_ALWAYS_MASK)
      || scf_cb->scf_facility < DB_MIN_FACILITY
      || scf_cb->scf_facility > DB_MAX_FACILITY)
    {
...

/*{
** Name: scs_clear	- clear the attn handler address
**
** Description:
**      This function is the opposite of the declare function 
**      above.  It removes the facility's handler from the registration.
**
** Inputs:
...
scs_clear(SCF_CB *scf_cb, SCD_SCB *scb )
{
    ...
    Sc_main_cb->sc_alist.sc_afcn[scf_cb->scf_facility].afcn_addr = NULL;
...
**	    happened.
**	20-May-2011 (kschendel) m1260
**	    Ditch PAINE's, they never happened.
**	    Add the "VW" (x100) facility to the list so that we pass
**	    attn's to the X100 interface.
*/
STATUS
scs_attn(i4 eid,
	 CS_SCB *csscb )
{
    ...
    {
	sc0ePut(E_SC0229_INVALID_ATTN_CALL, (DB_ERROR *)NULL, NULL, 0);
	return(FAIL);
    }
    else
    {
	...
	if ((scb->scb_sscb.sscb_interrupt == SCS_INTERRUPT)
	    ...
	    || (eid == CS_KILL_EVENT))
	{
	    /* Call attn handler for the "current" facility if it has
	    ** registered one, and also for all facilities asking for
	    ** attns always.
	    */
	    for (fac = DB_MIN_FACILITY;
		    ...
		    fac++)
	    {
		if (list->sc_afcn[fac].afcn_mode & SCS_ALWAYS_MASK
				...
				list->sc_afcn[fac].afcn_addr) )
		{
			...
			switch (fac)
			{
				...
				break;
			    case DB_VWF_ID:
				fac_scb = scb->scb_sscb.sscb_x100cb;
				break;
			    default:
				continue;
			}
...
**	29-Aug-2011 (kschendel) b125701
**	    Fix confusion over sscb-interrupt value vs sscb-state values.
*/
DB_STATUS
scs_i_interpret(SCD_SCB *scb )
{
    ...
    if (buf->gca_itype)
	scb->scb_sscb.sscb_interrupt = -SCS_INT_ENDRET;
    else
	scb->scb_sscb.sscb_interrupt = -SCS_INTERRUPT;
    return(E_DB_OK);
}
...
**	    Display session and query start times when showing formatted.
*/
STATUS
...

{
    ...
    char		datestr[50];	/* dd-mmm-yy hh:mm:ss */
	...
	*/
    ...
    datestr[0] = '\0';		/* Paranoia */
    (void) TMcvtsecs(scb->cs_scb.cs_connect, &datestr[0]);
    TRformat(sc0e_tput, &print_mask, buf, sizeof(buf) - 1,
	"    Session started at %s as user %#s",
	datestr, DB_OWN_MAXNAME, (char *) &scb->scb_sscb.sscb_ics.ics_iusername);
    TRformat(sc0e_tput, &print_mask, buf, sizeof(buf) - 1,
    	"    Terminal: %16s",
	...
	*/
	...
	/* If it looks like there might be a real query running, display its
	** query start time.  This is a bit iffy since SCF doesn't clear
	** the start time, so go by a) it's nonzero and b) the session isn't
	** in a BIOR wait.
	*/
	if (scb->scb_sscb.sscb_cquery.cur_start_secs > 0
	  && (scb->cs_scb.cs_state != CS_EVENT_WAIT
	      || (scb->cs_scb.cs_memory & CS_BIOR_MASK) != CS_BIOR_MASK) )
	{
	    datestr[0] = '\0';		/* Paranoia */
	    (void) TMcvtsecs(scb->scb_sscb.sscb_cquery.cur_start_secs, &datestr[0]);
	    TRformat(sc0e_tput, &print_mask, buf, sizeof(buf) - 1,
		"    Last recorded query start time: %s\n", datestr);
	}
