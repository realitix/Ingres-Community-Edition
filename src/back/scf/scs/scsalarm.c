...
scs_get_db_alarms(SCD_SCB *scb)
{
	...
	*/
	...
	if (status)
	{
		...
		sc0e_0_put(E_US2475_9333_IISECALARM_OPEN, (DB_ERROR *)NULL, 0);
	...
	*/
	...
	do {
	    ...
	    */
	    applies=FALSE;
		    ...
		    default:	    /* this should never happen */
		    {
	    		...
	    		sc0e_put(E_US2478_9336_BAD_ALARM_GRANTEE, 
                                 (DB_ERROR *)NULL, 0L, 2,
		...
		if(event_opened==FALSE)
		{
		    ...
		    */
		    ...
		    status = qef_call(QEU_OPEN, &ev_qeu);
	            if (status)
		    {
		         sc0e_0_put(E_SC034B_ALARM_QRY_IIEVENT, 
                                    (DB_ERROR *)NULL, 0);
		...
		*/
		...
		{
			sc0e_0_put(0, &xev_qeu.error, 0);
			sc0e_put(E_US247A_9338_MISSING_ALM_EVENT, 
                                (DB_ERROR *)NULL, 0,3,
	...
	{
	    sc0e_0_put(E_US2476_9334_IISECALARM_ERROR, (DB_ERROR *)NULL, 0);
	...
	{
		sc0e_0_put(0, &qeu.error, 0);
		sc0e_0_put(E_SC034B_ALARM_QRY_IIEVENT, (DB_ERROR *)NULL, 0);
		sc0e_0_put(E_US2476_9334_IISECALARM_ERROR, (DB_ERROR *)NULL, 0);
	}
	opened = FALSE;
    ...
    */
    if(status!=E_DB_OK)
    {
	sc0e_0_put(E_SC0349_GET_DB_ALARMS, (DB_ERROR *)NULL, 0);
    }
    return status;
}

...
scs_fire_db_alarms(SCD_SCB *scb, i4 alarm_spec)
{
	...
	*/
		...
		*/
		if(alarm->dba_flags&DBA_DBEVENT)
		{
		    ...
		    */
		    ...
		    if(status!=E_DB_OK)
		    {
			sc0e_0_put(E_US2477_9335_ALARM_EVENT_ERR, 
                                   (DB_ERROR *)NULL, 0);
			status=E_DB_SEVERE;
			break;
		    }
    ...
    if(status!=E_DB_OK)
    {
	sc0e_0_put(E_SC034A_FIRE_DB_ALARMS, (DB_ERROR *)NULL, 0);
    }
    return status;
}

