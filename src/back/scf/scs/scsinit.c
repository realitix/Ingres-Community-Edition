...
**      24-aug-2009 (stial01) 
**          scs_dbdb_info() check for charset conflict (b122505)
...
**	27-aug-2010 (dougi)
**	    Init. qef_dba.
**	28-Sep-2010 (kschendel)
**	    Revert IVW autocommit back to Ingres standard (OFF for SQL).
...
**          Or in PSQ_STRICT_ORDERBY when cbf dbms parm strict_orderby set.
**	26-Jul-2011 (wanfr01) Bug 125599
**	    SETDBERR must be used to set FILE/LINE information for errors 
**      10-Oct-2011 (smeke01) b125834
**          Handle cbf parameter int_math_i8_result via ADF_INTMATH_4ORLONGER
**          in adf_cb->adf_misc_flags.
*/
i4
scs_initiate(SCD_SCB *scb )
{
    ...
    */
	...
	{
	    sc0e_0_put(scb->scb_cscb.cscb_gci.gca_status, (DB_ERROR *)NULL,
	...
	{
	    sc0e_put(E_SC022D_1ST_BLOCK_NOT_MD_ASSOC, (DB_ERROR *)NULL, 0, 1,
	...
	*/
	    ...
	    )
	{
		...
		case    GCA_VERSION:
		    ...
		    {
			status = E_DB_ERROR;
			SETDBERR(&error, error.err_data, E_US0022_FLAG_FORMAT);
		    }
		    break;
		...
		case    GCA_APPLICATION:
			...
			*/
			status = E_DB_ERROR;
		        SETDBERR(&error, error.err_data, E_US0004_CANNOT_ACCESS_DB);
			break;
		    /*
		...
		case    GCA_XA_RESTART:
		{
	       ...
	       /* Parse the distributed transaction ID from the gca_value. */
		    ...
		    if (p_index == GCA_RESTART)
		    {
		       ...
		       {
			   status = E_DB_ERROR;
		           SETDBERR(&error, error.err_data, E_US0009_BAD_PARAM);
			   break;
		       }
			...
			*/
			...
			if (status)
			{
			    status = (status > E_DB_ERROR) ?
				      status : E_DB_ERROR;
			    SETDBERR(&error, error.err_data, E_SC0260_XENCODE_ERROR);
			    break;
			}
		    }
		...
		case GCA_GW_PARM:
		    ...
		    if (status)
		    {
			sc0e_0_put(E_SC0204_MEMORY_ALLOC, (DB_ERROR *)NULL, 0);
	...
	*/
	...
	{
	    status = E_DB_ERROR;
	    SETDBERR(&error, error.err_data, E_SC0327_RECOVERY_SERVER_NOCNCT);
	}
	else
	if (Sc_main_cb->sc_capabilities & SC_IOMASTER_SERVER)
	{
	    status = E_DB_ERROR;
	    SETDBERR(&error, error.err_data, E_SC0370_IOMASTER_SERVER_NOCNCT);
	}
    }
	...
	*/
	...
	if (scb->scb_sscb.sscb_flags & SCS_STAR)
	{
	    ...
	    */
		...
		if (status)
		{
		    SETDBERR(&error, error.err_data, E_SC0123_SESSION_INITIATE);
		    break;
		}
	    }
	}
	...
	*/
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_ADF_ID))
	{
	    ...
	    if(status != OK)
	    {
		SETDBERR(&error, error.err_data, status);
		status = E_DB_ERROR;
		sc0e_uput(E_SC033D_TZNAME_INVALID, (DB_ERROR *)NULL, 0, 3,
		...
		SETDBERR(&error, error.err_data, status);
		status = E_DB_ERROR;
		sc0e_uput(E_SC037B_BAD_DATE_CUTOFF, (DB_ERROR *)NULL, 0, 2,
	    ...
	    if (status)
	    {
		SETDBERR(&error, error.err_data, adf_cb->adf_errcb.ad_errcode);
	    ...
	    if (!Sc_main_cb->sc_adf_math_i8_res)
	    {
		/* Use ADI_4ORLONGER for integer maths */
		adf_cb->adf_misc_flags |= ADF_INTMATH_4ORLONGER;
	    }
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_DMF_ID))
	{
	    ...
	    */
	    ...
	    if (status)
	    {
		SETDBERR(&error, dmc->error.err_data, dmc->error.err_code);
		break;
	    }
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_QEF_ID))
	{
		...
		*/
	    ...
	    /* And the DBA. */
	    qef_cb->qef_dba = &scb->scb_sscb.sscb_ics.ics_dbowner;
	...
	*/
	if( scb->scb_sscb.sscb_stype == SCS_SMONITOR) 
	{
	    ...
	    {
		SETDBERR(&error, error.err_data, E_US0003_NO_DB_PERM);
		status = E_DB_ERROR;
	    }
	}
	/*
	...
	*/
		 ...
		 (scb->scb_sscb.sscb_stype == SCS_SNORMAL))
	{
	    ...
	    */
	    ...
	    {
		sc0e_put(E_SC034F_NO_FLAG_PRIV, (DB_ERROR *)NULL, 0, 3, 
			 ...
			 0, (PTR)0,
			 0, (PTR)0 );
		SETDBERR(&error, error.err_data, E_US0002_NO_FLAG_PERM);
		status=E_DB_ERROR;
		break;
	    }
	    /*
	...
	*/
	    ...
	    (scb->scb_sscb.sscb_ics.ics_uflags & SCS_USR_NOSUCHUSER) == 0 )
	{
		...
		SETDBERR(&error, error.err_data, E_US0061_USER_EXPIRED);
	    	status=E_DB_ERROR;
	    	break;
	    }
	}
	...
	*/
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_ADF_ID))
	    ...
	    */
		...
		{
		    SETDBERR(&error, error.err_data, E_DM00A0_UNKNOWN_COLLATION);
		    status = E_DB_ERROR;
		    break;
		}
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_PSF_ID))
	{
	    ...
	    */
	    ...
	    psq_cb->psq_cat_owner = &scb->scb_sscb.sscb_ics.ics_cat_owner;
            ...
            if(Sc_main_cb->sc_strict_orderby)
                psq_cb->psq_flag2 |= PSQ_STRICT_ORDERBY;
	...
	*/
	...
	if (scb->scb_sscb.sscb_ics.ics_dmf_flags & DMC_NLK_SESS)
	{
		...
		{
		    SETDBERR(&error, error.err_data, E_US1267_4711_DIS_TRAN_UNKNOWN);
		}
		else if (error.err_code == E_QE0055_DIS_TRAN_OWNER)
		{
		    SETDBERR(&error, error.err_data, E_US1268_4712_DIS_TRAN_OWNER);
		}
...
/* from INGRES 6.4 bug fix #62708 by angusm (harpa06) */
		...
		{
		    SETDBERR(&error, error.err_data, E_US126B_4715_DIS_DB_UNKNOWN);
		}
	...
	*/
	if( scb->scb_sscb.sscb_ics.ics_uflags&SCS_USR_RPASSWD)
	{
	    ...
	    */
	    ...
	    {
		SETDBERR(&error, error.err_data, E_US18FF_BAD_USER_ID);
		status=E_DB_ERROR;
		break;
	    }
	    /*
	...
	{
	    sc0e_put(E_SC0233_SESSION_START, (DB_ERROR *)NULL, 0, 5,
		     ((char *)
...
scs_terminate(SCD_SCB *scb )
{
    ...
    */
    ...
    for (mcb = scb->scb_sscb.sscb_mcb.mcb_sque.sque_next;
	    ...
	    mcb = new_mcb)
    {
	sc0e_put(E_SC0227_MEM_NOT_FREE, (DB_ERROR *)NULL, 0, 1, 
	...
	if (status)
	{
	    sc0e_0_put(E_SC0224_MEMORY_FREE, (DB_ERROR *)NULL, 0);
	    sc0e_0_put(status, (DB_ERROR *)NULL, 0);
	    return(E_DB_SEVERE);
	}
    }
    ...
    if (status)
    {
	sc0e_0_put(E_SC012B_SESSION_TERMINATE, (DB_ERROR *)NULL, 0);
    }
    if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_SESSION_LOG, NULL, NULL))
    {
	sc0e_0_put(E_SC0234_SESSION_END, (DB_ERROR *)NULL, 0);
    }
	...
	avg = (Sc_main_cb->sc_selcnt
		    ...
		    : 0);
	sc0e_put(E_SC0235_AVGROWS, (DB_ERROR *)NULL, 0, 2,
               ...
               DB_ERROR *error )
{
    ...
    */
	...
	for (;;)
	{
	    ...
	    */
		...
		else if (error->err_code == E_DM0113_DB_INCOMPATABLE)
		{
		    ...
		    {
			sc0e_0_put(E_US0008_CONVERT_DB, (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, error, 0);
			qeu.error.err_code = E_US0008_CONVERT_DB;
		    }
		    else
		    {
			sc0e_0_put(E_US0044_CONVERT_DBDB, (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, error, 0);
		...
		{
		    sc0e_0_put(E_US002B_DBDB_OPEN, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, error, 0);
	    ...
	    */
		...
		if(status!=E_DB_OK)
		{
		    sc0e_0_put(0, &sxf_cb->sxf_error, 0);
		...
		{
		    sc0e_0_put(E_US002C_DBDB_UNAVAILABLE, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
	    ...
	    */
		...
		if (status)
		{
		    sc0e_0_put(E_US000E_DATABASE_TABLE_GONE, 
                               (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
		...
		if (status)
		{
		    ...
		    {
			sc0e_0_put(E_US000F_DATABASE_TABLE, 
                                   (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, &qeu.error, 0);
		...
		*/
		...
		if (CMischarset_utf8())
		{
		    if (( dbt.du_dbservice & DU_UTYPES_ALLOWED ) == 0)
		    {
			/* II_CHARSET is utf8 but utypes not allowed */
			error->err_code = E_US0045_CHARSET_CONFLICT;
			return (E_DB_ERROR);
		    }
		}
	    ...
	    */
	    ...
	    if (status)
	    {
		sc0e_0_put(E_US000A_USER_TABLE_GONE, (DB_ERROR *)NULL, 0);
		sc0e_0_put(0, &qeu.error, 0);
	    ...
	    */
		...
		qeu.qeu_tab_id.db_tab_base = DM_B_GRPID_TAB_ID;
	        ...
	        if (status)
	        {
		    sc0e_0_put(E_US186A_IIUSERGROUP, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
	    ...
	    */
		...
		qeu.qeu_tab_id.db_tab_base = DM_B_DBPRIV_TAB_ID;
	        ...
	        if (status)
	        {
		    sc0e_0_put(E_US1888_IIDBPRIV, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
	...
	if (scb->scb_sscb.sscb_ics.ics_dbstat & DU_RDONLY)
	{
		...
		*/
		...
		if (status)
		{
		    sc0e_0_put(E_US0020_IILOCATIONS_OPEN, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
		...
		if ((status) || ((lct.du_status & DU_DBS_LOC) == 0))
		{
		    ...
		    {
			sc0e_0_put(E_US001D_IILOCATIONS_ERROR, 
                                   (DB_ERROR *)NULL, 0);
			sc0e_0_put(0, &qeu.error, 0);
		...
		if (status)
		{
		    sc0e_0_put(E_US001D_IILOCATIONS_ERROR, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, &qeu.error, 0);
		    qeu.error.err_code = E_US001D_IILOCATIONS_ERROR;
		    break;
		}
	    ...
	    */
	    ...
	    if (status)
	    {
		    sc0e_0_put(E_US002D_CLOSE_DB, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(0, error, 0);
    ...
    */
    ...
    if ( scb->scb_sscb.sscb_ics.ics_appl_code == DBA_DESTROYDB
        ...
        {
        sc0e_put(E_SC034F_NO_FLAG_PRIV, (DB_ERROR *)NULL, 0, 2,
                 sizeof (scb->scb_sscb.sscb_ics.ics_rusername),
                    (PTR)&scb->scb_sscb.sscb_ics.ics_rusername,
                 sizeof("-u flag")-1, "-u flag",
                 0, (PTR)0,
	...
	*/
	if( scb->scb_sscb.sscb_ics.ics_uflags&SCS_USR_REXTPASSWD)
	{
		...
		else if (status!=E_DB_OK)
		{
		     ...
		     ** Some kind of error
		     */
		     sc0e_0_put(E_SC035C_USER_CHECK_EXT_PWD, 
                                (DB_ERROR *)NULL, 0);
		     break;
	...
	*/
	    ...
	    != NULL)
	{
	    ...
	    if (status)
	    {
	        status = (status > E_DB_ERROR) ?
		      status : E_DB_ERROR;
		sc0e_0_put(E_SC0260_XENCODE_ERROR, (DB_ERROR *)NULL, 0);
	        break;
	    }
	}
	/*
...
**      17-Aug-2011 (hanal04) Bug 125663
**          SYSMOD requires access to iidbdb so the user must be trusted.
**          However, DU_UCREATEDB is arbitrary and the user may not have
**          this privilege. Extend the check to include DU_USECURITY,
**          DU_UUPSYSCAT and DBPR_DB_ADMIN, any of which will allow the
**          access to iidbdb that will in turn allow the lookup to see if
**          user is the DBA.
*/
static DB_STATUS
	...
	case DBA_SYSMOD:
	    ...
	    */
		...
		if (is_dbdb)
		{
		    ...
		    ** make sure this user is trusted at some level.
		    */
...
scs_propagate_privs(SCD_SCB *scb, i4 privs)
{
	...
	** always active if the session is allowed to use it..
	...
	if ( scb->scb_sscb.sscb_facility & (1 << DB_SXF_ID) )
	{
	    ...
	    if(status)
	    {
		    sc0e_0_put(0, &sxfrcb.sxf_error, 0);
		    privs=scb->scb_sscb.sscb_ics.ics_rustat;
		    err_pass++;
		    continue;
	    }
	...
	*/
    ...
    if(err_pass>0)
    {
	sc0e_0_put(E_SC034E_PRIV_PROPAGATE_ERROR, (DB_ERROR *)NULL, 0);
	return E_DB_FATAL;
    }
    else 
	return E_DB_OK;
