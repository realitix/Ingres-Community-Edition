...
)
{
	...
	*/
	...
	if (cl_stat != OK)
	{
	        sc0e_put(E_SC035A_EXTPWD_NO_AUTH_MECH, (DB_ERROR *)NULL, 0, 1,
		         scs_trimwhite(sizeof(*authname),
			      (char*)authname),
			(PTR)authname,
			0, (PTR)0,
    ...
    */
	...
	cl_stat = IIGCa_call(GCA_FASTSELECT,
        ...
        if (cl_stat != OK)
        {
	    sc0e_0_put(cl_stat, (DB_ERROR *)NULL, 0);
	    sc0e_0_put(E_SC0357_EXTPWD_GCA_FASTSELECT, (DB_ERROR *)NULL, 0);
	    status = E_DB_ERROR;
	    break;
        }
        else
        {
            ...
            {
	        sc0e_0_put(cl_stat, (DB_ERROR *)NULL, 0);
	        sc0e_0_put(E_SC0356_EXTPWD_GCA_CSSUSPEND, (DB_ERROR *)NULL, 0);
	        status = E_DB_ERROR;
		resume = 0;
		break;
            }
		...
		case E_GC0139_GCN_NO_DBMS:
		    ...
		    sc0e_put(E_SC0355_EXTPWD_GCA_NOPEER, (DB_ERROR *)NULL, 0, 2,
				 scs_trimwhite(sizeof(*authname),
				      (char*)authname),
				(PTR)authname,
				(i4)STlength(target),
		    ...
		    status = E_DB_ERROR;
                ...
                default:
	       	    ...
	       	    sc0e_0_put(cl_stat, (DB_ERROR *)NULL, 0);
		    sc0e_0_put(E_SC0354_EXTPWD_GCA_COMPLETION, 
                               (DB_ERROR *)NULL, 0);
		    status = E_DB_ERROR;
                    break;
                }
	...
	*/
	...
	if (error_status != 0)
	{
	   if (error_index == -1)
	   {
	       	sc0e_0_put(error_status, (DB_ERROR *)NULL, 0);
		sc0e_0_put(E_SC0359_EXTPWD_GCM_ERROR, (DB_ERROR *)NULL, 0);
	        status = E_DB_ERROR;
	   }
	   else
	   {
