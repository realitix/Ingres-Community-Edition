...
**      26-may-2011 (stial01) SIR 125253
**          Add copy_dmt_to_raat()
...
static void copy_dmt_to_raat(
				DMT_TBL_ENTRY *dmt_tbl,
				RAAT_TBL_ENTRY *raat_tbl);
	...
	case CS_INPUT:
	    ...
	    */
		...
		case RAAT_TABLE_OPEN:
		{
		    ...
		    {
			/*
			** This raat code assumes RAAT_ATT_ENTRY==DMT_ATT_ENTRY
			** We no longer require AAT_TBL_ENTRY==DMT_TBL_ENTRY
			** instead we use copy_dmt_to_raat() 
			*/
			TRdisplay("scs_raat_call() internal error ATT_ENTRY (%d %d)\n",
      ...
      defined(axp_lnx)
...
#endif
		    dmt_shw_cb.dmt_table.data_address = (char *)&dmt_tbl_entry;
		    ...
		    dmt_tbl = &dmt_tbl_entry;
		    raat_tbl = (RAAT_TBL_ENTRY *)(outbuf + buf_used);
		    buf_used += sizeof(RAAT_TBL_ENTRY);

		    /*
		    ** copy DMT_TBL_ENTRY to RAAT_TBL_ENTRY one field at a time
		    ** DON'T assume these structures are the same
		    */
		    copy_dmt_to_raat(&dmt_tbl_entry, raat_tbl);
		    ...
		    */
		    raat_tbl->tbl_qryid.db_qry_low_time = RAAT_VERSION_3;
      ...
      defined(axp_lnx)
		...
		case RAAT_BLOB_GET:
		{
		    ...
		    */
		    ...
		    coup.db_prec  = 0;
		    work.db_prec  = 0;
		    res.db_collID = DB_UNSET_COLL; 
		    coup.db_collID = DB_UNSET_COLL; 
		    work.db_collID = DB_UNSET_COLL; 
		...
		case RAAT_BLOB_PUT:
		{
		    ...
		    */
		    ...
		    workptr->adw_fip.fip_work_dv.db_collID = DB_NOCOLLATION;
...
static void
copy_dmt_to_raat(DMT_TBL_ENTRY *dmt_tbl, RAAT_TBL_ENTRY *raat_tbl)
{
    DMT_TBL_ENTRY *dtbl = dmt_tbl;
    RAAT_TBL_ENTRY *rtbl = raat_tbl;

    MEfill(sizeof(RAAT_TBL_ENTRY), 0, rtbl);
    rtbl->tbl_id.db_tab_base = dtbl->tbl_id.db_tab_base;
    rtbl->tbl_id.db_tab_index = dtbl->tbl_id.db_tab_index;
    MEmove(sizeof(dtbl->tbl_name.db_tab_name), dtbl->tbl_name.db_tab_name, ' ', 
	sizeof(rtbl->tbl_name), rtbl->tbl_name);
    MEmove(sizeof(dtbl->tbl_owner.db_own_name), dtbl->tbl_owner.db_own_name,' ',
	sizeof(rtbl->tbl_owner), rtbl->tbl_owner);
    rtbl->tbl_loc_count = dtbl->tbl_loc_count;
    MEmove(sizeof(dtbl->tbl_location.db_loc_name), dtbl->tbl_location.db_loc_name, ' ', 
	sizeof(rtbl->tbl_location), rtbl->tbl_location);
    MEmove(sizeof(dtbl->tbl_filename.db_loc_name), dtbl->tbl_filename.db_loc_name, ' ', 
	sizeof(rtbl->tbl_filename), rtbl->tbl_filename);
    rtbl->tbl_attr_count = dtbl->tbl_attr_count;
    rtbl->tbl_attr_nametot = dtbl->tbl_attr_nametot;
    rtbl->tbl_index_count = dtbl->tbl_index_count;
    rtbl->tbl_width = dtbl->tbl_width;
    rtbl->tbl_data_width = dtbl->tbl_data_width;
    rtbl->tbl_storage_type = dtbl->tbl_storage_type;
    rtbl->tbl_status_mask = dtbl->tbl_status_mask;
    rtbl->tbl_record_count = dtbl->tbl_record_count;
    rtbl->tbl_page_count = dtbl->tbl_page_count;
    rtbl->tbl_dpage_count = dtbl->tbl_dpage_count;
    rtbl->tbl_opage_count = dtbl->tbl_opage_count;
    rtbl->tbl_ipage_count = dtbl->tbl_ipage_count;
    rtbl->tbl_lpage_count = dtbl->tbl_lpage_count;
    rtbl->tbl_alloc_pg_count = dtbl->tbl_alloc_pg_count;
    rtbl->tbl_tperpage = dtbl->tbl_tperpage;
    rtbl->tbl_kperpage = dtbl->tbl_kperpage;
    rtbl->tbl_kperleaf = dtbl->tbl_kperleaf;
    rtbl->tbl_ixlevels = dtbl->tbl_ixlevels;
    rtbl->tbl_expiration = dtbl->tbl_expiration;
    rtbl->tbl_date_modified.db_tab_high_time = dtbl->tbl_date_modified.db_tab_high_time;
    rtbl->tbl_date_modified.db_tab_low_time = dtbl->tbl_date_modified.db_tab_low_time;
    rtbl->tbl_create_time = dtbl->tbl_create_time;
    rtbl->tbl_qryid.db_qry_high_time = dtbl->tbl_qryid.db_qry_high_time;
    rtbl->tbl_qryid.db_qry_low_time = dtbl->tbl_qryid.db_qry_low_time;
    rtbl->tbl_struct_mod_date = dtbl->tbl_struct_mod_date;
    rtbl->tbl_i_fill_factor = dtbl->tbl_i_fill_factor;
    rtbl->tbl_d_fill_factor = dtbl->tbl_d_fill_factor;
    rtbl->tbl_l_fill_factor = dtbl->tbl_l_fill_factor;
    rtbl->tbl_min_page = dtbl->tbl_min_page;
    rtbl->tbl_max_page = dtbl->tbl_max_page;
    rtbl->tbl_relgwid = dtbl->tbl_relgwid;
    rtbl->tbl_relgwother = dtbl->tbl_relgwother;
    rtbl->tbl_temporary = dtbl->tbl_temporary;
    rtbl->tbl_comp_type = dtbl->tbl_comp_type;
    rtbl->tbl_pg_type = dtbl->tbl_pg_type;
    rtbl->tbl_2_status_mask = dtbl->tbl_2_status_mask;
    rtbl->tbl_allocation = dtbl->tbl_allocation;
    rtbl->tbl_extend = dtbl->tbl_extend;
    rtbl->tbl_pgsize = dtbl->tbl_pgsize;
    rtbl->tbl_attr_high_colno = dtbl->tbl_attr_high_colno;
    rtbl->tbl_nparts = dtbl->tbl_nparts;
    rtbl->tbl_ndims = dtbl->tbl_ndims;
    rtbl->tbl_encflags = dtbl->tbl_encflags;
    rtbl->tbl_dimension = dtbl->tbl_dimension;
    rtbl->tbl_hilbertsize = dtbl->tbl_hilbertsize;
    rtbl->tbl_rangesize = dtbl->tbl_rangesize;
    rtbl->tbl_cachepri = dtbl->tbl_cachepri;
    rtbl->tbl_rangedt = dtbl->tbl_rangedt;
    MEmove(sizeof(dtbl->tbl_range), (PTR)&dtbl->tbl_range, 0, 
	sizeof(rtbl->tbl_range), (PTR)&rtbl->tbl_range);
