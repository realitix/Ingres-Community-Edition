...
sca_trace( i4  dispose_mask, i4  length, char *message )
{
    ...
    {
	sc0ePut(E_SC0318_SCA_USER_TRACE, (DB_ERROR *)NULL, 
                0, 1, length, message);
    }
    return(E_DB_OK);
}
		  ...
		  i4  *new_size )
{
    ...
    *new_svcb = adf_svcb;   /* Start out with no change */
	...
	{
	    sc0ePut(E_SC0263_SCA_RISK_CONSISTENCY, (DB_ERROR *)NULL, NULL, 0);
	}

	if (add_block->add_major_id <= 0)
	{
	    sc0ePut(E_SC0264_SCA_ILLEGAL_MAJOR, error, NULL, 1,
	...
	{
	    sc0ePut(E_SC0265_SCA_STATE, (DB_ERROR *)NULL, 0, 3,
				    sizeof(add_block->add_major_id),
				    &add_block->add_major_id,
	    ...
	    sc0ePut(E_SC024D_SCA_ADDING, (DB_ERROR *)NULL, 0, 4,
	...
	*new_svcb = new_adf_block;
	...
	if (old_adf_block && deallocate_flag)
	{
		...
		sc0ePut(E_SC024C_SCA_DEALLOCATE, &scf_cb.scf_error, NULL, 0);
	    }
	}
	old_size = cur_size;
	...
	sc0ePut(E_SC024E_SCA_NOT_ADDED, error, NULL, 0);
    }
	...
	{
	    sc0ePut(0, &scf_cb.scf_error, NULL, 0);
	    sc0ePut(E_SC024C_SCA_DEALLOCATE, (DB_ERROR *)NULL, NULL, 0);
	}
    }

    if (status && add_block && (add_block->add_trace & ADD_T_FAIL_MASK))
    {
	sc0ePut(E_SC0266_SCA_USER_SHUTDOWN, (DB_ERROR *)NULL, NULL, 0);
	status = E_DB_FATAL;
    }
    else
    {
...
sca_check(i4 major_id, i4 minor_id)
{
	...
	sc0ePut(E_SC0267_SCA_ID_UNKNOWN, (DB_ERROR *)NULL, 0, 0);
    ...
    {
    	if (Sc_main_cb->sc_capabilities & (SC_INGRES_SERVER | SC_STAR_SERVER))
	    sc0ePut(E_SC0268_SCA_RISK_INVOKED, (DB_ERROR *)NULL, 0, 0);
	status = 1;
    }
    return(status);
}
