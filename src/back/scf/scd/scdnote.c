...
scd_note( DB_STATUS status, i4   culprit )
{
    ...
    {
	sc0e_put(E_SC0223_BAD_SCDNOTE_FACILITY, (DB_ERROR *)NULL, 0, 2,
	...
	case E_DB_FATAL:
		...
		sc0e_put(E_SC0221_SERVER_ERROR_MAX, (DB_ERROR *)NULL, 0, 1,
			sizeof(i4), (PTR)&culprit,
			 0, (PTR)0,
			 0, (PTR)0,
			 0, (PTR)0,
	...
	default:
	    sc0e_put(E_SC0222_BAD_SCDNOTE_STATUS, (DB_ERROR *)NULL, 0, 2, 
		     sizeof(status), (PTR)&status,
		     sizeof (culprit), (PTR)&culprit,
		     0, (PTR)0,
		     0, (PTR)0,
