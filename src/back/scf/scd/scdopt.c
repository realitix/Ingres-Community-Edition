...
**          Add sc_strict_orderby flag, and bitmask SCO_STRICT_ORDERBY 
**          to catch any ambiguity in order-by clause.  
**	29-Mar-2011 (smeke01) SIR 125227
**	    Implement independent enabling/disabling of hash aggregation
**	    and hash join via new cbf parameter opf_hash_aggregate.
**       5-Jun-2011 (hanal04) SIR 125411
**          Add config support for sc924.
**      29-Jun-2011 (maspa05) bug 122380
**          SCO_AUTOSTRUCT now sets psq_cb flag rather than opf_cb one
**	01-Jul-2011 (kiria01) b125504
**	    Add trace to aid correctly setting default collations.
...
**	17-Jul-2011 (wanfr01) Bug 125563
**	    Add ima_memory.
**      10-Oct-2011 (smeke01) b125834
**          Handle new cbf parameter int_math_i8_result.
*/
...
#define         SCO_STRICT_ORDERBY          224 
#define		SCO_HASH_AGG		    225
#define         SCO_SC924_ERROR             226
#define         SCO_SC924_STACK             227
#define		SCO_IMA_MEMORY		    228
#define		SCO_ADF_MATH_I8_RES	    229
static SCD_OPT scd_opttab[] =
{
    ...
    SCO_HASH_AGG,		'z',	' ',	"!.opf_hash_aggregate",
    ...
    */
    ...
    SCO_STRICT_ORDERBY,         't',    ' ',    "!.strict_orderby", 
    SCO_SC924_ERROR,            'o',    ' ',    "!.sc924_error", 
    SCO_SC924_STACK,            't',    ' ',    "!.sc924_stack", 
    SCO_ADF_MATH_I8_RES,	'z',	' ',	"!.int_math_i8_result",
    0, 0, 0, 0
} ;
	...
	SCD_CB		*scd_cb )
{
	...
	for( scdopt = scd_opttab; scdopt->sco_name; scdopt++ )
	{
	    ...
	    */
	    ...
	    if( status )
	    {
		sc0e_put( status, (DB_ERROR *)NULL, (CL_ERR_DESC *)0, 1,
		...
		break;
            ...
            case SCO_SC924_ERROR:
		Sc_main_cb->sc_trace_errno = scd_value;
                break;

            case SCO_SC924_STACK:
                Sc_main_cb->sc_trace_stack = (i4)1;
                break;
	    ...
	    case SCO_ADF_MATH_I8_RES:
		Sc_main_cb->sc_adf_math_i8_res = FALSE;
		break;
...
#			undef _DEFINE
			...
			TRdisplay("scd_options: default uni collation %s -> %s (%d)\n",
					scd_svalue, collname_array[i+1], i);
		    }
		    else
		    {
			...
			TRdisplay("scd_options: default collation %s -> %s (%d)\n",
					scd_svalue, collname_array[i+1], i);
		    }
		    break;
		}
		...
		psq_cb->psq_flag2 |= PSQ_TABLE_AUTOSTRUCT;
		break;
		...
		{
		    sc0e_put(E_SC026B_INVALID_CURSOR_MODE, (DB_ERROR *)NULL, 
                             (CL_ERR_DESC *)0, 1,
	   ...
	   case SCO_STRICT_ORDERBY:
                Sc_main_cb->sc_strict_orderby = TRUE;
                break;
	    ...
	    case SCO_IMA_MEMORY:
		MO_mutex();
		MO_mem_limit = scd_value;
		MO_unmutex();
		break;
	    default:
		sc0e_put( E_SC0232_INVALID_STARTUP_CODE, (DB_ERROR *)NULL, 
                    (CL_ERR_DESC *)0, 2,
