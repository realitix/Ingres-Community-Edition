/*
** Copyright (c) 1987, 2009 Actian Corporation
**
**
NO_OPTIM=dr6_us5
*/
...
**          Initialise Sc_main_cb->sc_adf_math_i8_res to TRUE;
*/

/*
...
**	    PAINE gone, just attn, fix here.
**	    Pass x100 (session) cb ptr offset to x100.
**	29-Mar-2011 (smeke01) SIR 125227
**	    Implement independent enabling/disabling of hash aggregation
**	    and hash join.
**      29-Jun-2011 (maspa05) bug 122380
**          Remove setting of no longer used opf_autostruct
**	19-Jul-2011 (kiria01) b125564
**	    Correct the trimming of node to avoid buffer overrun.
*/
DB_STATUS
...
# if !defined(conf_CAS_ENABLED) || !defined(conf_CLUSTER_BUILD)
	...
	*/
	sc0ePut(E_SC0394_CLUSTER_NOT_SUPPORTED, (DB_ERROR *)NULL, NULL, 0);
...
# endif /* !int_lnx */
    ...
    */
    ...
    opf_cb.opf_smask |= OPF_HASH_JOIN;	    /* turn hash join on
					    ** by default */
    opf_cb.opf_smask |= OPF_HASH_AGG;	    /* turn hash aggregation on
					    ** by default */
    ...
    /* Integer arithmetic results default to ADI_FIXED 8 */
    Sc_main_cb->sc_adf_math_i8_res = TRUE;
    ...
    */
    ...
    if (status != E_DB_OK)
    {
        sc0ePut(E_SC0392_CUT_INIT, (DB_ERROR *)NULL, NULL, 0);
        return(E_SC0124_SERVER_INITIATE);
    }
    /*
    ...
    */
	...
	if (stat != OK)
	{
	    sc0ePut(stat, (DB_ERROR *)NULL, NULL, 0);
	    sc0ePut(E_SC0242_ALTER_MAX_SESSIONS, (DB_ERROR *)NULL, NULL, 2,
    ...
    */
	...
	if (status & E_SC_MASK)
	{
	    sc0ePut(E_SC0204_MEMORY_ALLOC, (DB_ERROR *)NULL, NULL, 0);
	    sc0ePut(status, (DB_ERROR *)NULL, NULL, 0);
	}
	else
	{
	    sc0ePut(E_SC0200_SEM_INIT, (DB_ERROR *)NULL, NULL, 0);
	    sc0ePut(0, &scf_cb.scf_error, NULL, 0);
	}
	return(E_SC0124_SERVER_INITIATE);
    }
...

	...
	** to distinguish it from standalone mode. */
	...
	{
	    sc0ePut(gca_cb.gca_status, (DB_ERROR *)NULL, 
                    &gca_cb.gca_os_status, 0);
	...
	*/
	...
	{
	    sc0ePut(gcr_cb.gca_status, (DB_ERROR *)NULL, 
                    &gcr_cb.gca_os_status, 0);
	...
	*/
	...
	if(stat != OK)
	{
		...
		*/
		...
		sc0ePut(E_SC033D_TZNAME_INVALID, (DB_ERROR *)NULL, NULL, 3,
	...
	*/
	{
	    ...
	    {
	    	sc0ePut(stat, (DB_ERROR *)NULL, NULL, 0);
		sc0ePut(E_SC037B_BAD_DATE_CUTOFF, (DB_ERROR *)NULL, NULL, 2,
				  STlength(SystemVarPrefix), SystemVarPrefix,
	...
	*/
	...
	if (status)
	{
	    sc0ePut(E_SC012C_ADF_ERROR, (DB_ERROR *)NULL, NULL, 0);
	    break;
	}
	...
	*/
	...
	if (status)
	{
	    sc0ePut(0, &gw_rcb.gwr_error, NULL, 0);
	    break;
	}
	...
	*/
	...
	{
	    sc0ePut(E_SC0024_INTERNAL_ERROR, (DB_ERROR *)NULL, NULL, 0);
	    return(E_SC0124_SERVER_INITIATE);
	}
	...
	if (status)
	{
	    sc0ePut(0, &dmc.error, NULL, 0);
	    break;
	}
	i = CL_OFFSETOF(SCD_SCB, scb_sscb) +
		CL_OFFSETOF(SCS_SSCB, sscb_x100cb);
	x100_set_cb_offset(i);
	/*
	...
	** Turn off its interrupt handlers.
	*/
	if (Sc_main_cb->sc_capabilities & SC_STAR_SERVER)	
	{
	    SCF_CB   tmp_scb;
	    ...
	    tmp_scb.scf_nbr_union.scf_amask = 0;
	    status = scf_call(SCS_CLEAR, &tmp_scb);
	    if (status)
	    {
		sc0ePut(0, &tmp_scb.scf_error, NULL, 0);
		break;
	    }
	}	
	...
	*/
	...
	if ((Sc_main_cb->sc_capabilities & SC_STAR_SERVER) == 0)
	{
	    ...
	    {
		sc0ePut(E_SC0269_SCA_INCOMPATIBLE, (DB_ERROR *)NULL, NULL, 2,
	...
	*/
	if (!(Sc_main_cb->sc_capabilities & SC_STAR_SERVER))
	{
	    ...
	    */
	    ...
	    if (status != E_DB_OK)
	    {
		sc0ePut(0, &sxf_rcb.sxf_error, NULL, 0);
		break;
	    }
	...
	if (status)
	{
	    sc0ePut(0, &rdf_ccb.rdf_error, NULL, 0);
	    break;
	}
	...
	*/
	...
	if (status)
	{
	    sc0ePut(0, &psq_cb.psq_error, NULL, 0);
	    break;
	}
	...
	if (status)
	{
	    sc0ePut(0, &opf_cb.opf_errorblock, NULL, 0);
	    break;
	}
	...
	if (status)
	{
	    sc0ePut(0, &qsf_rcb.qsf_error, NULL, 0);
	    break;
	}
...
# if 0
	...
	*/
	...
	{
	    sc0ePut(stat, (DB_ERROR *)NULL, NULL, 0);
	    sc0ePut(E_SC0245_SERVER_INIT_ADD, (DB_ERROR *)NULL, NULL, 0);
	    status = E_DB_ERROR;
	    break;
	}
# endif
	...
	*/
	if (!(Sc_main_cb->sc_capabilities & SC_STAR_SERVER))
	{
	    ...
	    */
	    ...
	    if (stat != OK)
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0237_FAST_COMMIT_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	if (Sc_main_cb->sc_events > 0)
	{
		...
		*/
		...
		{
		    sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		    sc0ePut(E_SC0272_EVENT_THREAD_ADD, (DB_ERROR *)NULL, 
                            NULL, 0);
	...
	*/
	{
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, NULL, 0);
		sc0ePut(E_SC0243_WRITE_BEHIND_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*/
	if (Sc_main_cb->sc_rep_qman)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC037D_REP_QMAN_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*/
	    ...
	    (Sc_main_cb->sc_nostar_recovr == FALSE))
	{
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0314_RECOVER_THREAD_ADD, (DB_ERROR *)NULL, NULL, 0);
		break;
	    }
	}
	...
	*/
	if ( secaudit_writer )
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0329_AUDIT_THREAD_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	if(term_thread)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0340_TERM_THREAD_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	if(secaudit_writer)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0360_SEC_WRITER_THREAD_ADD, (DB_ERROR *)NULL, 
                        NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	...
	*/
	if (Sc_main_cb->sc_writealong) 
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0367_WRITE_ALONG_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*/
	if (Sc_main_cb->sc_readahead)  
	{
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0368_READ_AHEAD_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	if (Sc_main_cb->sc_capabilities & SC_CSP_SERVER)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC02C0_CSP_MAIN_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC02C2_CSP_MSGMON_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC02C4_CSP_MSGTHR_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*tmpptr = '\0';
	...
	sc0ePut(E_SC051D_LOAD_CONFIG, (DB_ERROR *)NULL, NULL, 2,
		 STlength(server_flavor), server_flavor,
		 STlength(server_type), server_type);

	sc0ePut(E_SC0129_SERVER_UP, (DB_ERROR *)NULL, 0, 2,
...
scd_terminate(void)
{
    ...
    if (Sc_main_cb->sc_facilities & (1 << DB_GCF_ID))
    {
        ...
        if (status)
        {
            sc0ePut(gca_error, (DB_ERROR *)NULL, NULL, 0);
            error++;
        }
        else if (gca_cb.gca_status != E_GC0000_OK)
        {
...
Deadlock Detector,Sampler,Sort,Factotum", i,
	...
	avg = (Sc_main_cb->sc_selcnt
		    ...
		    : 0);
	sc0ePut(E_SC0235_AVGROWS, (DB_ERROR *)NULL, NULL, 2,
    ...
    {
        sc0ePut(E_SC0128_SERVER_DOWN, (DB_ERROR *)NULL, NULL, 0);
	return(E_DB_OK);
    }
}

...
i4	start_lkcback_thread)
{
	...
	*/
	if (recovery_server)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC031D_RECOVERY_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
		...
		*/
		...
		{
		    sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		    sc0ePut(E_SC032F_CP_TIMER_ADD, (DB_ERROR *)NULL, NULL, 0);
		    status = E_DB_ERROR;
		    break;
		}
	    }
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0371_DEADLOCK_THREAD_ADD, (DB_ERROR *)NULL, 
                        NULL, 0); 
		status = E_DB_ERROR;
		break;
	    }
		...
		*/
		...
		{
		    sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		    sc0ePut(E_SC0396_DIST_DEADLOCK_THR_ADD, (DB_ERROR *)NULL, 
                            NULL, 0); 
		    status = E_DB_ERROR;
		    break;
		}
	     ...
	     !CXconfig_settings( CX_HAS_DLM_GLC ) )
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0398_GLC_SUPPORT_THR_ADD, (DB_ERROR *)NULL, 
                        NULL, 0); 
		status = E_DB_ERROR;
		break;
	    }
	...
	*/
	...
	{
	    sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
	    sc0ePut(E_SC0321_CHECK_DEAD_ADD, (DB_ERROR *)NULL, NULL, 0);
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	{
	    sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
	    sc0ePut(E_SC0325_FORCE_ABORT_ADD, (DB_ERROR *)NULL, NULL, 0);
	    status = E_DB_ERROR;
	    break;
	}
	...
	if (start_gc_thread)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC0323_GROUP_COMMIT_ADD, (DB_ERROR *)NULL, NULL, 0);
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	while (num_logwriters > 0)
	{
	    ...
	    */
	    ...
	    {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
		sc0ePut(E_SC031F_LOGWRITER_ADD, (DB_ERROR *)NULL, 
                        NULL, 0); /* need new msg */
		status = E_DB_ERROR;
		break;
	    }
	...
	if (start_lkcback_thread)
	{
            ...
            */
            ...
            {
		sc0ePut(stat, (DB_ERROR *)NULL, &sys_err, 0);
                sc0ePut(E_SC037E_LK_CALLBACK_THREAD_ADD, (DB_ERROR *)NULL, 
                        NULL, 0); 
                status = E_DB_ERROR;
                break;
            }
...
)
{
	...
	if (PMget("II.$.SECURE.ROW_AUDIT_KEY", &pmvalue) == OK)
	{
		...
		{
		        sc0ePut(E_SC0339_SECURE_INVALID_PARM, (DB_ERROR *)NULL, 
                                NULL, 1,
	...
	if (PMget("II.$.SECURE.ROW_AUDIT_DEFAULT", &pmvalue) == OK)
	{
		...
		{
		       sc0ePut(E_SC0339_SECURE_INVALID_PARM, (DB_ERROR *)NULL, 
                                NULL, 1, sizeof("ROW_AUDIT_DEFAULT")-1,
				"ROW_AUDIT_DEFAULT");
			return(E_SC0124_SERVER_INITIATE);
		}
		if(echo)
