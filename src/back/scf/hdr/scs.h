...
**	    Drop disable/enable functions, never used.
**	27-Aug-2011 (kschendel) b125701
**	    Change sscb_state into an enum for ease of debugging.
*/

...
/* Define a state code used for sscb_state to tell the sequencer what
** state it's in.  The normal state progression, if one can even talk
** about "normal" in the sequencer, is simply SCS_INPUT -> SCS_CONTINUE
** and back.  There are a ton of additional states for special cases,
** though.
*/

typedef enum {
    SCS_NOSTATE,		/* No state assigned */
    SCS_INITIATE,		/* time to start up a session */
    SCS_INPUT,			/* need input from the user */
    SCS_CONTINUE,		/* Query is running, possibly sending data */
    SCS_TERMINATE,		/* session is ending */
    SCS_CP_INTO,		/* a copy into */
    SCS_CP_FROM,		/* A copy from */
    SCS_CP_ERROR,		/* COPY FROM has an error on the server end,
				** FE is streaming rows and server is waiting
				** for FE to poll to notice the error status.
				** Meanwhile, the server just tosses rows.
				*/
    SCS_ENDRET,			/* Mystery state ... apparently a state that
				** happens when ending a retrieve early.
				** Nothing actually checks for SCS_ENDRET, so
				** its distinction is probably that it is
				** neither INPUT nor CONTINUE. */
    SCS_ADDPROT,		/* add sys update capability */
    SCS_DELPROT,		/* Drop sys update capability */
    SCS_PROCESS,		/* Mystery state ...  apparently a "run the
				** query" state that is neither INPUT nor
				** CONTINUE.  Nothing ever checks specifically
				** for PROCESS.
				*/
    SCS_ACCEPT,			/* Accept connection and ask for
				** initiation parameters
				*/
    SCS_SPECIAL,		/* Non-user DBMS thread - run
				** specialized processing
				** depending on thread type
				*/
    SCS_DCONNECT,		/* Direct Connect mode.
				** Distributed server only.
				** User is connected directly to the LDB.
				*/
    SCS_IERROR,			/* Error found while processing input message */

    SCS_SPASSWORD,		/* Send password prompt */
    SCS_RPASSWORD		/* Receive password reply */
} sequencer_state_enum;
...
#define			CUR_COMPLETED	0x4	/* Not currently used */
    ...
    i4		    cur_start_secs;	/* Query start time (TMsecs), for IMA
...
**	    Drop sscb_cpy_qeccb.  Use the pointer in the psq-cb instead;
**	    it's always available when needed, and PSF has to determine
**	    the copy-optim memory block lifetime because they're allocated
**	    out of a PSF stream.  Without this proper lifetime knowledge,
**	    we end up using a stale pointer and poof.
**	24-May-2011 (kschendel) m1260
**	    Store ptr to X100SessionState a.k.a X100 CB here.
**	    SCF doesn't need it much, except for attn processing.
*/
...
#define			SCS_COPY_STARTED    0x400   /* session has started the copy
    ...
    i4		    sscb_cfac;		/* Facility SCF thinks is current */
    ...
    */
    ...
    sequencer_state_enum sscb_state;	/* what is going on in the world */
...
#define			SCS_ASIGNORE	4	/* Ignore GCA completion */
    ...
    struct X100SessionState *sscb_x100cb; /* X100 session CB */
    PTR		    sscb_dbparam;
...
FUNC_EXTERN DB_STATUS scs_input(SCD_SCB *scb, sequencer_state_enum *next_state);
