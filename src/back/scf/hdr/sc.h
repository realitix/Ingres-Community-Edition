...
**          Added E_SC051F_BATCPY_MSG_QUEUE.
**	27-Aug-2011 (kschendel) b125701
**	    Move state definitions to scs.h.
**/

/*
...
** The difference between INTERRUPT and INT_ENDRET is that INTERRUPT may have
** been asynchronous, like a control-c.  SCS_INT_ENDRET applies when an
** ATTENTION message has the GCA_ENDRET flag, implying that row retrievals
** are in progress and that rather than setting facility interrupt flags,
** the sequencer will get around to calling QEF with an end-retrieval function
** soon.  One significant difference in the final effect is that ENDRET
** simply terminates retrievals early, while a full INTERRUPT might act as
** a statement abort (which in turn might rollback the current transaction,
** if on-error rollback transaction is turned on).
** 
** (At present, only LIBQ-based clients make any distinction between
** full interrupt and end-of-retrieval interrupt.  The IIapi_abort function
** presented by the API always does a full interupt.)
*/

#define			SCS_INTERRUPT	0x1	/* interrupt the query */
#define			SCS_INT_ENDRET	0x2	/* Interrupt in the sequencer
						** but don't call facility attn
						** handlers
						*/
/*
...
#define                 E_SC051F_BATCPY_MSG_QUEUE           (E_SC_MASK + 0x051F)
