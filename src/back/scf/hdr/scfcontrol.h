...
**	    Get rid of PAINE's, never used;  AIC -> attn.
**      10-Oct-2011 (smeke01) b125834
**          Add sc_adf_math_i8_res.
**/

/*
...

/*}
** Name: SCD_AFCN - Description of a facility's attn handler
**
** Description:
**      This structure describes to SCF an attention handler for a facility.
...
**
**      Each facility's handler has a mode associated with it.  This mode
**      specifies whether the facility wishes to have the attn delivered
**      to it independent of whether the facility is active.  ("Active"
**	in this instance meaning "SCF knows it's active", which is not
**	quite what one might expect in general!)  Facilities such as DMF,
**	GWF, and VWF which are not called directly from the sequencer
**	had best ask for "call me always".
**
**	AIC's are expected to be functions of the form
**
**	    STATUS
**	    ruf_aic(reason, cb)		random user fac's aic
**	    i4 reason;			why am i getting this
**	    RUF_CB  *cb;		Random facility's session control block
**
** History:
**     17-Jan-86 (fred)
**          Created on Jupiter.
...
typedef struct _SCD_AFCN
{
    ...
    STATUS          (*afcn_addr)(i4, void *);     /* The address of the procedure */
}   SCD_AFCN;

/*}
...
#define		SC_SEC_OME_DISABLED	0x080L	/* OME disabled */
    ...
    i2              sc_strict_orderby;        /* Enforce strict order by namespacing */
...
#define         SC_DATE_NONE        0x04  /* Date is set to disallow "date" */
    ...
    SC0M_CB	    sc_sc0m;
    TIMER	    sc_timer;
    SCD_ALIST	    sc_alist;		/* attn handlers per facility */ 
...
#  define	SCE_HASH	i4
    ...
    bool	    sc_batch_copy_optim;    /* enable copy optimization 
					    ** for batched inserts */
    bool	    sc_adf_math_i8_res;	    /* Integer arithmetic results: ADI_FIXED 8 or ADI_4ORLONGER? */
