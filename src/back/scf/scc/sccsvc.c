...
**      13-Dec-2010 (horda03) b124691
**          ast_enabled is a boolean value.
...
**
** Inputs:
**      operation			SCC_ERROR, SCC_MESSAGE, SCC_WARNING,  
**					SCC_RSERROR or SCC_BAT_CPY_ERR
**		                           (op code to scf_call())
**      scf_cb
...
**          Added SCC_BAT_CPY_ERR operation. Used to log non-fatal user errors
**          that lead to rows not being inserted (e.g. duplicate key) during
**          batch inserts where copy optimization is in use. The caller
**          specifies the address of the GCA_RE_DATA message for the failed
**          tuple in scc_msg. We update the rowcount in the GCA_RE_DATA
**          and insert an error message prior to the GCA_RE_DATA message.
**          The caller is responsibe for advancing its scc_msg point to keep
**          the tuple PUT and response messages in synch.
**	29-Mar-2011 (kschendel) b125204
**	    Revise above so that response is found by matching copy ptr/id
**	    and copy sequence.  This eliminates any mutexing issues with
**	    copy child thread pointer chasing, plus any problems with
**	    non-batched-copy responses in the queue.
*/
DB_STATUS
scc_error( i4  operation, SCF_CB * scf_cb, SCD_SCB *scb )
{
	...
	if (!scf_cb->scf_ptr_union.scf_buffer)
	{
        ...
        else if (operation == SCC_BAT_CPY_ERR && scf_cb->scf_copy_ptr == NULL)
        {
            SETDBERR(&scf_cb->scf_error, 0, E_SC000D_BUFFER_ADDR);
            status = E_DB_ERROR;
        }
	else
	{
		...
		*/
		...
		else
		{
			...
			/* 
			** either we got an error while running an insert to
			** copy optimization or we got a group terminating
			** error in a batch. The assumption is that an error in
			** a copy that causes a user error to be sent to the
			** front-end will also terminate the copy (thus making
			** all previous rows evaporate). If a batch terminate
			** error occurs the work we did up to now will also
			** evaporate (due to rollback). What we will do in this
			** case is remove all previous responses from the
			** message queue and insert the error. When the code
			** drops back in to the sequencer this error will be
			** flagged as batch terminating which will cause us
			** to eat the rest of the input for the current batch.
			** The user will ultimately be sent one error message
			** for the current batch and must assume that the rest
			** of the batch failed.
			**
			** "single statement" errors, ie dup key, during an
			** insert to copy optimized batch is handled with
			** a special operation (SCC_BAT_CPY_ERR) since in that
			** case the copy is NOT aborted or rolled back.
			** That case is handled later in the code.
			*/
			...
			scb->scb_cscb.cscb_eog_error = TRUE;
                    ...
                    if(operation == SCC_BAT_CPY_ERR)
                    {
                        SCC_GCMSG   *msg;
                        GCA_RE_DATA *rdata;

			msg = scb->scb_cscb.cscb_mnext.scg_next;
			/* Search for the proper queued response */
			while (msg != (SCC_GCMSG *) &scb->scb_cscb.cscb_mnext)
			{
			    if (msg->scg_mask & SCG_BCO_RESP_MASK
			      && msg->scg_mdescriptor == scf_cb->scf_copy_ptr
			      && msg->scg_sequence == scf_cb->scf_copy_seq)
			    {
				/* Update insert status */
				rdata = (GCA_RE_DATA *)msg->scg_marea;
				rdata->gca_rowcount = -1;

				/* Insert error message before RESPONSE msg */
				ebuf->scg_next = msg;
				ebuf->scg_prev = msg->scg_prev;
				msg->scg_prev->scg_next = ebuf;
				msg->scg_prev = ebuf;
				break;
			    }
			    msg = msg->scg_next;
...
scc_send( CS_SCB *csscb, i4  sync )
{
    ...
    */
    ...
    {
	sc0ePut(last_error, (DB_ERROR *)NULL, 
                &scb->scb_cscb.cscb_gco.gca_os_status, 0);
        return(last_error);
    }
	...
	cmsg_next = cmsg->scg_next;
        ...
        if (cmsg_next == cmsg)
        {
            ...
            sc0ePut(E_SC023B_INCONSISTENT_MSG_QUEUE, (DB_ERROR *)NULL, NULL, 0);
	...
	*/
	...
	if (cmsg == (SCC_GCMSG *) &scb->scb_cscb.cscb_mnext)
	{
		...
		while ((SCC_GCQUE *) cmsg != &scb->scb_cscb.cscb_qdone)
		{
			...
			sc0ePut(E_SC023B_INCONSISTENT_MSG_QUEUE, 
    ...
    */
	...
	switch (cmsg->scg_mtype)
	{
		...
		*/
		sc0ePut(E_SC0255_INCOMPLETE_SEND, (DB_ERROR *)NULL, NULL, 2,
	...
	*/
    ...
    /* mdescriptor isn't a descriptor if BCO mask... */
    if ((cmsg->scg_mask & SCG_BCO_RESP_MASK) == 0)
	...
	**	Write an error
	*/
	sc0ePut(cur_error, (DB_ERROR *)NULL, NULL, 0);
	return cur_error;
    }
...
scc_recv( CS_SCB *csscb, i4  sync )
{
    ...
    */
    if (scb->scb_sscb.sscb_astate == SCS_ASIGNORE)
    {
	sc0ePut(E_SC0275_IGNORE_ASTATE, (DB_ERROR *)NULL, NULL, 0);
...
scc_fa_notify( SCD_SCB *scb )
{
	...
	*((i4 *)outbuf) = E_US1262_4706;
    ...
    {
	sc0e_uput(E_US1262_4706, (DB_ERROR *)NULL, NULL, 0);
