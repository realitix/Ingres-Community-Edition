...
**          Added macro IS_A_PST_OP_NODE.
**      25-Jan-2011 (coomi01) b124470
**          Add bitmask PSQ_STRICT_ORDERBY
**	02-Mar-2011 (kiria01 & smeke01) SIR 125040
**	    Add 'IF EXISTS' syntax to SQL DROP/REMOVE.
**	09-Dec-2010 (kiria01) b124821
**	    Added prototypes to allow other facilities access to
**	    DOT tracing code.
**	23-Mar-2011 (kschendel) b125204 m1782
**	    Simplify batch insert-to-copy flags.  All we really need is
**	    one saying that the last parsed stmt is the copy-optimized one.
**	    Sequencer flags handle the rest of it.
**      19-Apr-2011 (maspa05) 
**          Added E_PS1009_UNICODE_NOT_ALLOWED for disallowing unicode
**          functions in non-unicode databases
**	29-Mar-2011 (smeke01) SIR 125227
**          Add defines to support hints '[no]hashagg', '[no]hashjoin' and
**          '[no]hash'.  
**	17-may-2011 (dougi) m1954
**	    Added E_PS04F5 for self referencing constraints on VW tables.
**	08-Jun-2011 (kiria01) b125435
**	    Chain OPERANDS on right hand side.
**      15-Jun-2011 (smeke01) b125445
**          Define PSQ_SPARALLEL so that we can give better error messages
**          for SET PARALLEL.
**	27-Jun-2011 (kiria01) m2160
**	    Add PST_FWDCAST to support delayed datatype references in
**	    targetlist.
**	28-Jun-2011 (kiria01)
**	    Build dot trace code unconditionally to allow use on windows.
**      29-Jun-2011 (maspa05) b122380
**          Added PSQ_TABLE_AUTOSTRUCT 
**      13-Jul-2011 (horda03)
**          Include si.h to ensure FILE is defined. Needed for VMS builds.
**/

/*
**  Error definitions.
...
#define	E_PS04F1_X100_DROPIX		    (E_PS_ERRORS + 0x4F1)
#define E_PS04F2_X100_MIXED		    (E_PS_ERRORS + 0x4F2)
#define E_PS04F4_X100_CRINDEX		    (E_PS_ERRORS + 0x4F4)
#define E_PS04F5_X100_SELFREF		    (E_PS_ERRORS + 0x4F5)
/*
...
/* Error using nchar/nvarchar functions in a non-Unicode database */
# define E_PS1009_UNICODE_NOT_ALLOWED   (E_PS_MISC + 0x09L)
...

/*          
** PSF_DBPALIAS    a typedef for dbproc id to be used throughout PSF
**  
...
**    
** History:                 
**      12-Jul-2011 (horda03) b125497
**          defined from PSS_DBPALIAS.        
*/    
typedef char PSF_DBPALIAS[sizeof(DB_CURSOR_ID) + DB_OWN_MAXNAME + sizeof(i4)];

/*}
...
**	6-may-2009 (dougi)
**	    Add modes for X100 versions of various DDLs and PSQ_SX100DEF and
**	    PSQ_X100DEF mode & flag for "set [no]x100default".
**	4-mar-2010 (dougi)
**	    Add PSQ_IWVCALL for VW "syscall"s.
**      20-Aug-2009 (coomi01) b122237
...
**	15-sep-2010 (dougi)
**	    Add modes & flag for "set [no]vectorwise_row_default".
**	16-sep-2010 (dougi)
**	    Add PSQ_X100_DGTT_AS for IVW DGTT AS SELECT.
...
**	    X100 drop-table ended up not being used, remove it.
**	24-Mar-2011 (kschendel) b125204
**	    Move batch copy optim last-statement-name here, so sequencer
**	    can clear it when appropriate.  Simplify copy-optim flags.
**	29-Jul-2011 (kiria01) b125613
**	    Change psq_qnode to psq_qnodep with extra indirection to
**	    support pst_resolve interface change.
**	16-Jun-2012 (kschendel) SIR 126676
**	    Remove PSQ_MODE enum to dbdbms.h
*/
...
#define                 PSQ_C_C2SECURE  0x0001
    ...
    DB_TAB_NAME	    psq_tabname;	/* A generic table name.  Currently
					...
					** DELETE data to PSF.
					*/
    char	    psq_last_sname[DB_MAXNAME+1]; /* last dynamic statement. This
    ...
    i4              psq_numovfl;        /* Numeric overflow handling */
    ...
    /* A note on the below pointer:  insert-to-copy optimization takes a
    ** series of INSERTs executed from a prepared INSERT statement, and
    ** turns them into one COPY statement with the data coming from the
    ** inserted rows.  The control blocks needed for the COPY are created
    ** from the INSERT prototype stream.  Thus there is one set of COPY
    ** control blocks per batched prepared INSERT, which in turn implies
    ** that the pointer should be part of the prototype header.
    ** However, the sequencer needs to get at the control blocks too, since
    ** (at present) it's the one who issues the actual COPY calls to QEF.
    ** So, a copy of the pointer for the most recently seen optimized
    ** INSERT is kept here.  (A non-null pointer here does not imply that
    ** the last parsed statement was a optimized insert!  use the insert-
    ** to-copy flag in psq-flag for checking that!)
    ** At present, the sequencer has no ability to manage (finalize)
    ** multiple optimized inserts in the same batch, since it only sees
    ** this one most-recent pointer, and thus only knows to finalize the
    ** one copy at end of batch.
    */
    struct _QEF_RCB *psq_cp_qefrcb;	/* stashed QEF_RCB for insert to copy
					** optimization */
...
/* Flag set if statement just parsed is the one for which we're doing
** batch insert-to-copy optimization.  At present we only allow one
** such per batch (since sequencer only knows how to finish one).
*/
#define		PSQ_INSERT_TO_COPY	0x0002
...
#define		PSQ_DFLT_DIRECT_CRSR	    0x8000L
					...
					*/
#define		PSQ_NOFLATTEN_SINGLETON_SUBSEL	0x00010000L
					...
					** is a reused field.
					*/
#define		PSQ_OWNER_SET		    0x00020000L
					...
					** queries 
					*/
#define		PSQ_NO_CHECK_PERMS	    0x00040000L
					...
					*/
#define		PSQ_NO_JNL_DEFAULT	    0x00080000L

					/*
					** CBF says cache dynamic is ON
					*/
#define		PSQ_CACHEDYN		0x00100000L

					/*
					** Enable row security audit by
					** default
					*/
#define		PSQ_ROW_SEC_AUDIT	0x00200000L

					/*
					** Provide a row security key by default
					*/
#define		PSQ_ROW_SEC_KEY		0x00400000L
					...
					** behaviour)
					*/
#define		PSQ_NOCHK_SINGLETON_CARD 0x00800000L
...
#define		PSQ_INGRES_PRIV	        0x01000000L

#define		PSQ_RULE_DEL_PREFETCH	0x02000000L
...
#define		PSQ_ALLOW_TRACEFILE_CHANGE	0x04000000L

					/*
 					** CREATE TABLE defaults to X100
 					*/
#define		PSQ_X100_DEFAULT	0x08000000L

					/* No user passwords */

#define		PSQ_PASSWORD_NONE	0x10000000L

					/* No roles */

#define		PSQ_ROLE_NONE		0x20000000L

					/* Roles  need  passwords */

#define		PSQ_ROLE_NEED_PW	0x40000000L

					/* Session can select all */

#define		PSQ_SELECT_ALL		0x80000000L
...
#define	    PSQ_COPY_OPTIM		0x0010L /* copy optim is allowed */
...
#define     PSQ_STRICT_ORDERBY          0x0080L
#define     PSQ_TABLE_AUTOSTRUCT        0x0100L
                                        /* global table autostruct is on */
    ...
    */
    ...
    struct _PST_QNODE	**psq_qnodep;		/* QSF node details for */
...
**	    Added PST_10DISTINCT for "distinct" queries.
*/
...
#define		PST_9SINGLETON		    0x100
	/*
	** this is a DISTINCT [sub]select.
	*/
#define		PST_10DISTINCT		    0x200
...
**	11-may-2011 (dougi)
**	    Add flag to identify ops that are window functions.
*/
...
#define		PST_WINDOW_OP	512	/* this is a window function */
} PST_OP_NODE;

/*}
...
**         pst_print becomes PST_RS_PRINT bit in new field pst_rsflags
**	    to allow for extra flags in PST_RSDM_NODE
**	16-mar-2007 (dougi)
**	    Added PST_RS_KEYSET flag for keyset scrollable cursors.
**	1-oct-2008 (dougi)
**	    Added PST_RS_X100GB, PST_RS_DERIVED  flags for compiling X100 
**	    queries.
**	8-july-2009 (dougi)
**	    Added a few more flags for X100.
**	15-dec-2009 (dougi)
**	    Added PST_RS_FOLDED flag for IVW.
**	27-may-2011 (dougi)
**	    Added PST_RS_OBDESC & PST_RS_WINNOEMIT for window functions.
**	30-may-2011 (dougi) m1971
**	    Added PST_RS_CONST to help cross compiler with DERIVED cols.
*/
...
#define     PST_RS_USRCONST  (0x00000008) /* set iff user provided a constant
					...
					** both
					*/
#define     PST_RS_X100GB	(0x00000010) /* SET iff in a X100 query and this
					** is a GROUP BY column to be ignored
					** in future projections */
#define     PST_RS_DERIVED	(0x00000020) /* set if a GROUP BY column that
					** functionally depends on some other
					** gb column. Allows X100 queries to
					** mark it as DERIVED */
#define     PST_RS_RENAMED	(0x00000040) /* set if RESDOM incoprporates an
					** AS rename - used in X100 */
#define     PST_RS_NOQUAL	(0x00000080) /* set in opc for X100 queries to
					** indicate "as" RESDOM not needing 
					** table name qualification */
#define	    PST_RS_REUSEREN	(0x00000100) /* set in opc to identify X100
					** RESDOM names already pushed into
					** a "reuse" subquery */
#define     PST_RS_TID		(0x00000200) /* RESDOM is for TID - flag helps
					** X100 cross compiler with colname */
#define	    PST_RS_VTRENAMED	(0X00000400) /* RESDOM is in virtual table and
					** was renamed to distinctify X100 */
#define     PST_RS_PSEUDOTID	(0x00000800) /* RESDOM is for view pseudoTID -
					** used by X100 cross compiler */
#define	    PST_RS_OBDESC	(0x00001000) /* RESDOM is DESC ob column in
					** window definition */
#define	    PST_RS_WINNOEMIT	(0x00002000) /* window function not ready for
					** cross compiler to emit */
#define	    PST_RS_CONST	(0x00004000) /* RESDOM produces constant
					** (literal or constant function) */
...
**	27-apr-88 (stec)
**	    Modified for DB procedures.
**	15-Mar-2011 (kschendel)
**	    Use enum type for tparmtype.
*/
...

/*}
** Name: PST_WINDOW_NODE - Node to be used in window functions.
**
** Description:
**	This node defines the window structure of a window function.
**	In addition, its pst_left addresses the partitioning columns
**	(a chain of RESDOMs addressing the columns) and its pst_right 
**	addresses the ordering list.
**	
** History:
**	6-may-2011 (dougi)
**	    Written for window functions.
*/
typedef struct _PST_WINDOW_NODE
{
    DB_NAME	    pst_winname;	/* name of window (optional) */
    u_i2	    pst_units;		/* units of frame */
    i2		    pst_bound1;		/* start bound of frame */
    i2		    pst_bound2;		/* end bound of frame */
    i2		    pst_exclude;	/* frame exclusions */
    u_i2	    pst_pcount;		/* count of partitioning columns */
    u_i2	    pst_ocount;		/* count of ordering columns */
} PST_WINDOW_NODE;

/*}
...
**	6-may-2011 (dougi)
**	    Add WINDOW node for window functions.
*/
typedef union
{
...
**	  operand list runs through the right links, with the left ones
...
**	    to avoid portability problems. 
**	30-Dec-2010 (kiria01)
**	    Reduce the IS_A_PST_OP_NODE macro based on type closure with
**	    AND being 0 - removes compiler warning.
**	6-may-2011 (dougi)
**	    Add WINDOW node for window functions.
**	27-Jun-2011 (kiria01) m2160
**	    Add PST_FWDCAST to support delayed datatype references in
...
_DEFINE(FWDCAST,FWC, 30 /* nothing */)\
_DEFINE(WINDOW, WIN, 31 /* PST_WINDOW_NODE */)\
_DEFINEEND
...
#define	IS_A_PST_OP_NODE(pst_type) ((PST_TYPE)(pst_type) < PST_CONST || (PST_TYPE)(pst_type) == PST_MOP)
...
**	11-Mar-2011 (kschendel) b125173
**	    Add type strings for display.
*/
typedef enum pst_rgtype_enum {
#define PST_RGTYPES_MACRO \
_DEFINE(PST_UNUSED,	0, "UNUSED")\
_DEFINE(PST_TABLE,	1, "TABLE")	/* Base table */\
_DEFINE(PST_RTREE,	2, "RTREE")	/* View - pst_rgroot has the query tree */\
_DEFINE(PST_SETINPUT,	3, "SETINPUT")	/* Temporary table, used in set-input\
					** dbprocs; tab_id will be filled in, \
					** but will only be valid when \
					** RECREATEing the procedure */\
_DEFINE(PST_DRTREE,	4, "DRTREE")	/* Derived table in FROM clause */\
_DEFINE(PST_WETREE,	5, "WETREE")	/* WITH clause element "query name" */\
_DEFINE(PST_TPROC,	6, "TPROC")	/* Table procedure */\
_DEFINE(PST_TPROC_NOPARMS,7, "TPROC_NOPARM") /* Table proc with no parm list \
					** used internally in RDF */\
_ENDDEFINE
...
**	    and offset "n" values.
**	8-Mar-2011 (kschendel)
**	    Comment update re x100 flags.
*/
...
#define	PST_HINT_HASHAGG	0x0400	/* Allow use of hash aggregation */
#define	PST_HINT_NOHASHAGG	0x0800	/* Disable use of hash aggregation */
...
#define	PST_HINT_HASHJOIN	0x080000 /* Allow use of hash join */
#define	PST_HINT_NOHASHJOIN	0x100000 /* Disable use of hash join */
...
#define	    PST_RPTQRY				((i4) 0x8000)
						...
						*/
#define     PST_X100_TARGET			((i4)0x010000)
	/* Set for CTAS/insert/delete/update if target (result) table
	** is a Vectorwise table.  (For those statement types, if this
	** flag isn't set, one may assume that the target is an Ingres table.
	** See below (x100-query) for more on this.
	*/
...
#define		PST_PARM_OFFSETN		((i4)0x10000000)
	...
	** actual "n" value for repeat query/dbproc.
	*/
#define		PST_X100_QUERY			((i4)0x20000000)
	/*
	** this query references X100 tables - OPF will generate 
	** corresponding X100 query fragments mixed with native Ingres
	** query fragments.
	** A note on the three X100 flags:
	** X100-target refers to the X100-ness of the result variable,
	** if there is one:  insert/delete/update.
	** X100-query and X100-only-query refer to table references made
	** within the query, excluding the result variable;  unless the
	** result variable is also "part of the query".  What this means
	** in practice is that delete and update of an X100 table will set
	** X100-query, and possibly X100-only-query, as well as X100-target.
	** Insert-values into an X100 table sets X100-target, but NOT
	** either of the other two X100 flags (unless there are scalar
	** subqueries in the values list, which acts like insert-select).
	** Insert-select sets X100-target if the target is an X100 table,
	** and it sets X100-query / X100-only as dictated by the select
	** part of the query.
	*/
#define		PST_X100ONLY_QUERY			((i4)0x40000000)
	/*
	** this query references only X100 tables - OPF will generate 
	** a corresponding X100 query.
	*/
#define		PST_WINDOW_IN_QUERY			((i4)0x80000000)
	/*
 	** this query contains at least one window function. Triggers 
	** extra OPF code and helps psy_view().
	*/
...
#define			PSY_IVWCALL	37  /* X100 SYSCALL */
...
#define			PSY_MAXTAB_BYTES ((PSY_MAXTABS+7)/8)

/*}
...
**	13-may-2009 (dougi)
**	    Added flag vector to indicate X100 tables.
**	8-mar-2010 (dougi)
**	    Added psy_sysccmd, psy_syscargs for IVW syscall statement.
...
**	16-Jun-2012 (kschendel) SIR 126676
**	    Add per-object flag for DROP obj RESTRICT, obj RESTRICT, ...
**	    (Multi-object DROP doesn't use the PSY_DROP_CASCADE flag.)
*/
...
#define             PSY_APPOK   0x0008L	/* Append permitted */
    ...
    char	    psy_x100tab[PSY_MAXTAB_BYTES]; /* Bit set -> X100 table */
    char	    psy_restrict[PSY_MAXTAB_BYTES]; /* Bit set -> RESTRICT */
...
#define	    PSY_SYSTEM_GENERATED	    ((i4) 0x200)
					 ...
					 /*
					 ** statement qualified by IF EXISTS
					 */
#define	    PSY_IF_EXISTS		    ((i4) 0x400)
					/*
...
#define	    PSY_GR_REV_ROLE		((i4)0x8000)
    ...
    DB_TEXT_STRING *psy_sysccmd;	/*
					** command for IVW syscall
					*/
    DB_TEXT_STRING *psy_syscargs;	/*
					** (optional) argument string for IVW
					** syscall
					*/
...
**	    columns.
**	15-dec-2008 (dougi)
**	    Added flag to identify constraints on X100 tables.
**	28-Jul-2010 (kschendel) SIR 124104
**	    Add compression so that auto structure can maintain compression.
**	13-Oct-2010 (kschendel) SIR 124544
...
#define PST_CONS_TABLE_STRUCT	0x00040000
	...
	*/
#define	PST_CONS_X100		0x00080000
	/* This is a constraint involving X100 tables.
	*/
...
_DEFINE2(X100_CREATE_TABLE,10,	"X100_CREATE_TABLE_TYPE") /*PST_CREATE_TABLE*/ \
_DEFINE2(X100_CREATE_INTEGRITY,11,"X100_CREATE_INTEGRITY") /*PST_CREATE_INTEGRITY*/ \
_DEFINE2(X100_DROP_INTEGRITY,12,"X100_DROP_INTEGRITY") /*PST_DROP_INTEGRITY*/ \
_ENDDEFINE
...
**          Added pst_fe_lname and pst_fe_dbpid.
*/
...
#define	PST_REPEAT_DYNAMIC	0x0002 /* parse tree for repeat dynamic query */
	...
	** checksum as object identifier.
    ...
    DB_CURSOR_ID   pst_dbpid;
        ...
        /* pst_fe_dbpid is the FE full id given to this procedure.
        ** OPF uses this ID to set up an ALIAS if the QSO object is created
        ** by QSO_CREATE rather than QSO_TRANS_OR_DEFINE.
        ** pst_fe_lname is the number of characters in the name, 0 implies
        ** unnamed.
        */
    i4             pst_fe_lname;
    PSF_DBPALIAS   pst_fe_dbpid;
}   PST_PROCEDURE;

/*
...
/*
** PST_DOT_CTX call context for DOT tracing.
*/
typedef struct _PST_DOT_CTX {
	PSF_SESSCB_PTR cb;
	PST_QTREE *tree;
	ADF_CB adfcb;
	u_i4 idx;
	struct _PST_DOT_LINKS {
	    struct _PST_DOT_LINKS *next;
	    struct {
		void *f, *t;
#define PST_DOT_N_LINKS 1000
	    } list[PST_DOT_N_LINKS];
	} *links;
} PST_DOT_CTX;

typedef void (*PST_DOT_RTN)(PST_DOT_CTX*,FILE*,void*);
FUNC_EXTERN i4 pst_ptr_ok(
	FILE *fd,
	void *p);
FUNC_EXTERN void pst_dot_qnode(
	PST_DOT_CTX *ctx,
	FILE *fd,
	PST_QNODE *node);
FUNC_EXTERN void pst_qtree_dot1(
	PST_DOT_CTX *ctx,
	FILE *fd,
	PST_QTREE *tree);
FUNC_EXTERN void pst_dot_stmt(
	PST_DOT_CTX *ctx,
	FILE *fd,
	PST_STATEMENT *stmt);
FUNC_EXTERN i4 pst_dot_chk(
	PST_DOT_CTX *ctx,
	void *f,
	void *t);
FUNC_EXTERN void pst_dot(
	char *fname,
	PST_DOT_RTN rtn,
	void *args[2]);
FUNC_EXTERN void pst_qtree_dot(
	char *fname,
	PSF_SESSCB_PTR cb,
	PST_QNODE *node);
FUNC_EXTERN void pst_proc_dot1(
	PST_DOT_CTX *ctx,
	FILE *fd,
	PST_PROCEDURE *proc);
FUNC_EXTERN void pst_proc_dot(
	char *fname,
	PST_PROCEDURE *proc);
FUNC_EXTERN i4 pst_dot_chk(
	PST_DOT_CTX *ctx,
