...
**	    Add 'set [no]hashagg' and 'set [no]hashjoin' and equivalent cbf
**	    parameter opf_hash_aggregate.
**/

/*}
...
/* TRUE - hash joins enabled server startup flag */
...
#define			OPF_HASH_AGG		0x80000
/* TRUE - hash aggregation enabled server startup flag */

/*}
...
**          Removed opf_autostruct and table autostruct determination done
**          in parser now.
*/
...
*/
#define                 OPF_X100	26
/* SET X100 - no opf_value required
**  - enable the production of X100 algebra
*/
#define                 OPF_NOX100	27
/* SET NOX100 - no opf_value required
**  - disable the production of X100 algebra
*/
#define			OPF_REUSE	28
/* SET REUSE - no opf value required
**  - enable use of subquery reuse logic (X100 only for now)
*/
#define			OPF_NOREUSE	29
/* SET NOREUSE - no opf value required
**  - disable use of subquery reuse logic (X100 only for now)
*/
#define			OPF_HASHAGG	30
/* SET HASHAGG - no opf_value required
** - enable hash aggregation strategies
*/
#define			OPF_NOHASHAGG	31
/* SET NOHASHAGG - no opf_value required
** - disable hash aggregation strategies
*/
#define			OPF_HASHJOIN	32
/* SET HASHJOIN - no opf_value required
** - enable hash join strategies
*/
#define			OPF_NOHASHJOIN	33
/* SET NOHASHJOIN - no opf_value required
** - disable hash join strategies
*/
...

/*
** X100 cross compile errors
*/
/* X100 query - couldn't find SAGG to match "parameterized" CONST node */
#define		E_OP08C0_NOSAGG			((OPX_ERROR) (OPE_BASE+0X8C0))
/* X100 - unsupported combination of aggregate distinct operations. */
#define		E_OP08C1_BADAGGDIST		((OPX_ERROR) (OPE_BASE+0X8C1))
/* X100 - unsupported SE joins. */
#define		E_OP08C2_NOSEJOIN		((OPX_ERROR) (OPE_BASE+0X8C2))
/* X100 - unsupported coercion. */
#define		E_OP08C3_BADCOERCE		((OPX_ERROR) (OPE_BASE+0X8C3))
/* X100 - ADF consistency checks */
#define		E_OP08C4_BAD_ADC_TMLEN		((OPX_ERROR) (OPE_BASE+0X8C4))
/* X100 - ADF consistency checks */
#define		E_OP08C5_BAD_ADC_TMCVT		((OPX_ERROR) (OPE_BASE+0X8C5))
/* X100 - ADF consistency checks */
#define		E_OP08C6_BUFFER_OVERFLOW	((OPX_ERROR) (OPE_BASE+0X8C6))
/* X100 - ADF consistency checks */
#define		E_OP08C7_ADI_OPNAME		((OPX_ERROR) (OPE_BASE+0X8C7))
/* X100 - unsupported operator */
#define		E_OP08C8_VWOP_UNSUPP		((OPX_ERROR) (OPE_BASE+0X8C8))

/*
**  OPTIMIZEDB errors
*/
...
*/
#define		OPT_F079_X100_SYNTAX_DUMP		79
/* SET TRACE POINT OP207
** Display X100 syntax for IVW queries
*/
...
/* Some useful macros for testing whether features are on or not, globally and at query level */
/*
**	This macro determines whether hash join is enabled.  It checks ops_alter.ops_amask (which is set via the cbf
**	parameter opf_hash_join, and the SET [NO]HASH & SET [NO]HASHJOIN tracepoints). It then checks if this setting
**	is overridden (by WITH [NO]HASH & WITH [NO]HASHJOIN from the command line (aka 'hints')). 
*/
#define IS_HASHJOIN_ENABLED(global,override) \
 ( (global & OPS_AHASHJOIN) ? ( (override & PST_HINT_NOHASHJOIN) ? FALSE:TRUE ) : ( (override & PST_HINT_HASHJOIN) ? TRUE:FALSE ) )

/*
**	This macro determines whether hash aggregation is enabled.  It checks ops_alter.ops_amask (which is set via the cbf
**	parameter opf_hash_aggregation, and the SET [NO]HASH & SET [NO]HASHAGG tracepoints). It then checks if this setting
**	is overridden (by WITH [NO]HASH & WITH [NO]HASHAGG from the command line (aka 'hints')). 
*/
#define IS_HASHAGG_ENABLED(global,override) \
 ( (global & OPS_AHASHAGG) ? ( (override & PST_HINT_NOHASHAGG) ? FALSE:TRUE ) : ( (override & PST_HINT_HASHAGG) ? TRUE:FALSE ) )
