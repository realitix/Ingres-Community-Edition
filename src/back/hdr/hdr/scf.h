/*
** Copyright (c) 1985, 2011 Actian Corporation
**
*/
...
**       7-Jan-2010 (hanal04) Bug 124892
**          Added E_SC0520_SCS_BAD_DBV.
...
**          must set the new scc_msg in the SCF_CB to the value of the current 
**          tuple's GCA_RE_DATA message.
**	29-Mar-2011 (kschendel) b125204
**	    Replace scc-msg with scc-copy-ptr, scc-copy-seq.
**	20-May-2011 (kschendel) m1260
**	    Drop enable, disable functions;  drop PAINE vs AIC dregs.
*/

/*
...
#define 		SCU_SDESTROY    0x23L
	    ...
	    ** Once upon a time there was a distinction between AIC's and
	    ** PAINE's, but no longer.
	    */
#define                 SCS_DECLARE     0x30L
#define                 SCS_CLEAR       0x31L
	    /* the following are added in the scf_function field */

/* If set, ATTN is always sent to the facility's attn handler.
** Otherwise it's only sent to the handler if the "current facility"
** (as determined by SCF!) is that facility.
*/
#define			SCS_ALWAYS_MASK	0x40000L	/* Always deliver attn */
/* notused	was disable AIC		0x32 */
/* notused	was enable AIC		0x33 */
...
#define			SCS_ATFACTOT	0x60L	/* Create a Factotum thread */
            ...
            /*
            ** operations for Batch queries.
            */
#define                 SCC_BAT_CPY_ERR 0x61L   /* Used to log non-fatal
                                                ** insert failures (e.g.
                                                ** duplicate key) during
                                                ** batch insert with copy
                                                ** optimization.
                                                */
#define		    SCF_MAX_OPERATION	0x61L
...
#define                 E_SC0520_SCS_BAD_DBV        (E_SC_MASK + 0x520L)
...
**	29-Mar-2011 (kschendel) b125204
**	    Changed scc_msg to copy-ptr and copy-seq instead.
*/
struct _SCF_CB
{
    SCF_CB	    *scf_next;
...
#define		SCU_LT_MASK	    0x100   /* long term, can suspend */
	i4		scf_amask;	/* what type of attn request is this */
    ...
    /* The copy ptr and copy sequence are only needed by the SCC_BAT_CPY_ERR
    ** operation, and serve to identify the queued batch response message
    ** which should be modified to contain the error / warning status.
    ** The ptr and sequence should match what's in the SCC_GCMSG header.
    ** (Note that on the QEF copy side of things, all QEF can do is count
    ** tuples from the start.  That's why we need a sequence-within-copy
    ** plus some way to distinguish multiple copies per batch, assuming
    ** that such things are implemented!)
    */
    void		*scf_copy_ptr;	/* Some sort of unique cookie */
    u_i4		scf_copy_seq;	/* Tuple sequence for that copy */
};
