...
FUNC_EXTERN	VOID	qrinitsz( QRB * qrb );
...
GLOBALDEF	u_i8	globrownum;
GLOBALDEF	u_i4	globcolnum;
GLOBALDEF	bool	startofcol;
GLOBALDEF	bool	startofrow;
GLOBALREF	FMT	**formatarray;
...
QRIOB		qriob	= {
    ...
    */
	...
	for (i = 0; i < qrb->rd->rd_numcols; i++)  /* for each column */
	{
	    ...
	    /* Init flags to control vertical printing option */
	    startofcol = TRUE;
	    startofrow = (i == 0) ? TRUE : FALSE;
	    globcolnum = i;
		...
		default:
		    ...
		    ** separator if in silent mode
                    */
		    if (i == 0 && (TrulySilent || Tm_vertical))
			firstcol=TRUE;
		    else
			firstcol=FALSE;
