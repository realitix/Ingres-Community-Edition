...
**              Add commands \redirect, \noredirect, \vert, \novert
**              Report error if \sh <command> returns an error.
**      28-Jun-2011 (G Jones) SIR 125412
**		Add commands \colformat, \showcf
**              (and associated function colformat_assign).
**      12-Sep-2011 (horda03) SIR 125412
**              Initialisation of udflist is done where the variable is
**              defined, not here where it is referenced. Also include
**              me.h.
*/
...
GLOBALREF UDFORMAT *udflist; /* list of user defined column formats */

/* colformat_assign */
/* Accepts a string of args specified by the user following a "\colformat"
** command.
** 2 arguments are expected - a column name and a format string.
** Column and formats pairs are added to a linked list
** (or the format is replaced if the column name already exists).
** If only 1 argument is supplied - a column name - and that column exists
** in the linked list, then it will be removed from the list.
*/

i4
colformat_assign(char *format_args)
{
  UDFORMAT *currentptr;
  UDFORMAT *lastptr;
  UDFORMAT *newptr;
  ...
  /* Allocate memory for the new linked list entry */
  if ((newptr = (UDFORMAT *)MEreqmem((i2) 0,(u_i4)sizeof(UDFORMAT),TRUE,
                                (STATUS *)NULL)) == NULL)
               IIUGbmaBadMemoryAllocation(ERx("colformat_assign - newptr"));
  ...
  endchar = format_args + STlength(format_args);
  STgetwords(format_args, &words, &colname);

  /* Report an error if there's no column name */
  if (STlength(colname) == 0)
  {
    putprintf(ERx("invalid column format assignment\n"));
    MEfree(newptr);
    return -1;
  }

  colform = format_args + STlength(colname) + 1;
  if(colform > endchar)
      colform = endchar;

  /* ignore any leading white space from column format */ 
  while (*colform == ' ')
      colform++;

  STlcopy(colname, newptr->colname, sizeof(newptr->colname));
  STlcopy(colform, newptr->colformat, sizeof(newptr->colformat));
  colname = newptr->colname;
  colform = newptr->colformat;

  STtrmnwhite(colform, sizeof(newptr->colformat));

  /* Check whether a format for this column exists already */
  currentptr = udflist;
  lastptr = NULL;
  while (currentptr)
  {
    if (STscompare(currentptr->colname,STlength(currentptr->colname),
	colname, STlength(colname)) == 0)
    {
      /* Found an existing entry for this column name. */
      /* If only 1 arg, this is an unassignment rather than an assignment */
      /* (too lazy to add a \nocolformat) */
      if (STlength(colform) == 0)
      {
	/* Unassignment */
        if (lastptr)
          lastptr->ptr = currentptr->ptr;
        else
          udflist = currentptr->ptr;
        MEfree(currentptr);
      }
      else
      {
	/* Re-assignment */
        STprintf(currentptr->colformat,"%s",colform);
      }
      MEfree(newptr);
      return 0;
    }
  ...
  /* Ignore attempts to unassign a column format that hasn't been assigned */
  if (STlength(colform) == 0)
  {
    MEfree(newptr);
    return 0;
  ...
  /* Populate the new linked list element */
  newptr->ptr=NULL;

  /* Set up a pointer to the new element */
  if (udflist == NULL)
    udflist = newptr;
  else
    lastptr->ptr = newptr;
		...
		*/
		    ...
		    (!Moninquote || Tm_adfscb->adf_qlang == DB_QUEL))
		{
			...
			switch (controlno)
			{
			  ...
			  case C_QUIT:
				clrline(1);
				/* Free linked list of user defined formats */
				while (udflist)
				{
				  tempudflist = udflist->ptr;
				  MEfree(udflist);
				  udflist = tempudflist;
				}
				/*
			  ...
			  case C_REDIRECT:
				redir_file = getfilenm();
				Peekch = '\n';	/* Peekch is trashed in
						   getfilenm() */
				if (Redir != NULL)
				{
					/* Already defined */
					/* Close file, we'll open a new one */
					SIclose(Redir);
					Redir = NULL;
				}
				if (*redir_file == '\0')
					redir_file = ERget(F_MO0052_redir_ing);
				if (Nodayfile >= 0)
/*	The previous string is for the name of the script file. The default	*/
					putprintf(ERget(F_MO0053_Result_log_into_f), redir_file);

				STcopy(redir_file, locbuf);	
				LOfroms(PATH & FILENAME, locbuf, &loc);

				if (status = SIopen(&loc, ERx("w"), &Redir))
				{
					ERreport(status, errbuf);
					putprintf(ERget(F_MO0054_Cant_open_redirect_f),
						redir_file, errbuf);
			  ...
			  case C_COLFORMAT:
				format_arg = getfilenm();
				Peekch = '\n';
				status=colformat_assign(format_arg);
				continue;

			  case C_SHOWCF:
				tempudflist = udflist;
				while (tempudflist != NULL)
				{
				  putprintf(ERx("%s : %s\n"),
				    tempudflist->colname,
				    tempudflist->colformat);
				  tempudflist = tempudflist->ptr;
				}
				tempudflist = NULL;
				continue;
