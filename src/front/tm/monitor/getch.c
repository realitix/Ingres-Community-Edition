...
**              Output to file if Redir set.
**	15-aug-2011 (G Jones) SIR 125412
**		Don't redirect "help" output, new parameter qrb allows checking
*/


VOID
putresults(str, len, qrb)
char	*str;
i4	len;
QRB	*qrb;
{
...
# ifndef	FT3270
	...
	/* Redirect output if requested and if we are sending query output */
	/* (as opposed to an error or status message) */
	if (Redir == NULL || qrb->step < 0 || qrb->step > 4 
		|| qrb->help_stmt == TRUE)
	  SIwrite(outlen, IIMObf1_Buffer1, &dummy, stdout);
	else
	  SIwrite(outlen, IIMObf1_Buffer1, &dummy, Redir);
	if (Script != NULL)
	{
