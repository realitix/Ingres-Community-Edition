...
VOID WINAPI SpinStyleSet( HWND hWnd, DWORD dwStyle )
    ...
    lpSpin->dwStyle = (DWORD_PTR) GetWindowLongPtr( hWnd, GWL_STYLE );
    ...
    SetWindowLongPtr( hWnd, GWL_STYLE, (LONG)lpSpin->dwStyle );
...
DWORD_PTR WINAPI SpinStyleGet( HWND hWnd )
    {
    LPSPIN lpSpin = GETSPIN(hWnd);
...
LRESULT WINAPI SpinStrDataGet( HWND hWnd, int nIndex )
    {
    LPSPIN lpSpin = GETSPIN(hWnd);
    LRESULT   lRet;
