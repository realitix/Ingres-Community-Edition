		...
		else
		{
			...
			if (bSubstituteUser)
			{
				if (pInfo->IsIgnore(_T("user")))
				{
					...
					if (nPos != -1)
					{
						CString strR = m_strRight.Mid  (nPos + (int)_tcslen(_T("privileges.user.")));
						CString strL = m_strRight.Left (nPos + (int)_tcslen(_T("privileges.user.")));
						//
...
static void sWriteTo (FILE* pFile, LPCTSTR lpszName, LPCTSTR lpszValue)
{
	...
	int i, nLen = (int)_tcslen(tchszBuff);
	tchszBuff[nLen] = _T(':');
	...
	CTypedPtrList< CObList, CaCdaDifference* >& ldiff)
{
	...
	if (pParam->IsIgnore(_T("user")))
	{
		...
		if (nPos != -1)
		{
			CString strR = strName1.Mid (nPos + (int)_tcslen(_T("privileges.user.")));
			CString strL = strName1.Left(nPos + (int)_tcslen(_T("privileges.user.")));
		...
		if (nPos != -1)
		{
			CString strR = strName2.Mid (nPos + (int)_tcslen(_T("privileges.user.")));
			CString strL = strName2.Left(nPos + (int)_tcslen(_T("privileges.user.")));
...
BOOL VCDA_ConstructListRestoreParam (CaRestoreParam* pRestore)
{
	...
	*/
	...
	{
		int nNodeSize = 0;
		int nCount = (int)(pRestore->m_plvn1->GetCount() + pRestore->m_plvn2->GetCount());
		if (nCount == 0)
			nCount = 1;
...
static void RestoreVirtualNodes(CaRestoreParam* pRestore)
{
	...
	if (!pRestore->GetVirtualNode())
		return;
	int nNodeSize = 0;
	int nCount = (int)max (pRestore->m_plvn1->GetCount(), pRestore->m_plvn2->GetCount());
	if (nCount == 0)
		nCount = 1;
	UINT* pArray = new UINT[nCount];
	for (int i=0; i<nCount; i++)
					...
					for (int i=0; i<nItemSize; i++)
					{
						...
						if (pItem->GetHost().CompareNoCase(strNode) == 0)
						{
							...
							if (strTemplate.Find(_T("Attribute")) != -1) // attribute data item
							{
								...
								if (nPos != -1)
									strAttrName = strTemplate.Mid (nPos + (int)_tcslen (_T("Attribute ")));
								CaRestoreVNode* pNewDef = new CaRestoreVNode(ch);
...
{
	CString s;
	INT_PTR i, cSize = arrayFields.GetSize();
