...
** 25-Mar-2011 (drivi01)
**    Define VectorWise versions as different functionality
**    is supported in different versions.
**    Define new structure type VectorWise_row (aka PAX).
** 02-May-2011 (drivi01)
**    Add 1.6 VW version for consistency reasons.
**    It isn't currently being used, but might be if new
**    features are added in this release.
** 05-Jul-2011 (drivi01)
**    Add 3 filter types for tables: OT_INGRES, 
**    OT_VECTORWISE and OT_ALL to be used in 
**    ComboBoxFillTablesFiltered function.
** 28-Oct-2011 (drivi01)
**    Add VWVERS_20 for Vectorwise 2.0 version.
*/
...
// the VWVERS_ is used to store Ingres VectorWise version. This assertion is checked in dbaginfo.sc.
#define VWVERS_10	(OIVERS_NOTOI + 100)
#define VWVERS_15	(OIVERS_NOTOI + 150)
#define VWVERS_16	(OIVERS_NOTOI + 160)
#define VWVERS_20	(OIVERS_NOTOI + 200)

int GetOIVers ();   // Return the OpenIngres version.
int SetOIVers (int oivers);
int GetVWVers ();   //returns Ingres VectorWise version
...
#define IDX_VWROW	9	//Vectorwise_row (aka PAX) table structure
...
//Filters for sorting/displaying table types
#define OT_INGRES	0
#define OT_VECTORWISE 1
#define OT_ALL	2
...
typedef struct TableParams
{
   ...
   BOOL	 bCreateVectorWise;		  // This was created for IVW 1.0 most likely will become absolete.
   ...
   int nVWStructType; //This is mainly used for table types starting with VW 1.5. (i.e. HEAP, VW, VW_ROW)
...
{
 INT_PTR   dbNo;                             // Database Number
 UCHAR szVNode  [MAXOBJECTNAME];         // Virtual node name
 UCHAR szDBName [MAXOBJECTNAME];         // Database name on target machine
 INT_PTR   nTypeRepl;                        // replication type :1 Full Peer
...
{
  UCHAR DBName       [MAXOBJECTNAME];      // Database name (source)
  INT_PTR  collision_mode;                    // CDDS collision mode repl. 1.05
  INT_PTR  error_mode;                        // CDDS error mode     repl. 1.05
  int   iReplicType;
...
UCHAR *ConcatStr(UCHAR *Lp1, UCHAR *Lp2, size_t *freebytes);
