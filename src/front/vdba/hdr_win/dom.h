...
** 02-May-2011 (drivi01)
**    Add vwVer field to LPTOMDATA structure.
********************************************************************/
...
typedef struct  _DomData
{
  ...
  DWORD_PTR     currentRecId;   // record id for the next add item
  HWND      hwndTreeLb;
  ...
  int	    vwVer;		// version number for VectorWise - VWVERS_XX in dbaset.h	    
  ...
  DWORD_PTR dwSelBeforeRClick;      // store selection "before RClick"
...
typedef struct _tagComplimTree {
  LONG_PTR count_all;    // Total number of lines in the tree
  DWORD_PTR curRecId;     // Current selection
  DWORD_PTR FirstVis;     // Top item on the client area
...
extern LRESULT FAR PASCAL __export DomMDIChildWndProc (HWND,UINT,WPARAM,LPARAM);
...
VOID DomRegisterAsLink(HWND hwndCurDom, LPDOMDATA lpCurDomData, BOOL bDragDrop, HWND hwndDestDom, LPDOMDATA lpDestDomData, DWORD_PTR ItemUnderMouse);
VOID DomRefreshLink(HWND hwndCurDom, LPDOMDATA lpCurDomData, BOOL bDragDrop, HWND hwndDestDom, LPDOMDATA lpDestDomData, DWORD_PTR ItemUnderMouse);
