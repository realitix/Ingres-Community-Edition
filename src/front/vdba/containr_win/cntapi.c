...
BOOL WINAPI CntFldTtlSet( HWND hWnd, LPFIELDINFO lpFld, LPSTR lpszColTitle, size_t wTitleLen )
    {
    LPCONTAINER  lpCnt = GETCNT(hWnd);
...
VOID WINAPI CntStyleSet( HWND hWnd, DWORD dwStyleMod )
    ...
    dwStyle = (DWORD_PTR) GetWindowLongPtr( lpCnt->hWndFrame, GWL_STYLE );
    ...
    SetWindowLongPtr( lpCnt->hWndFrame, GWL_STYLE, (LONG_PTR)dwStyle );
    ...
    if( dwStyleMod )
      {
      dwStyle = (DWORD_PTR) GetWindowLongPtr( lpCnt->hWnd1stChild, GWL_STYLE );

      dwStyle |= dwStyleMod;

      lpCnt->dwStyle = dwStyle;

      SetWindowLongPtr( lpCnt->hWnd1stChild, GWL_STYLE, (LONG_PTR)lpCnt->dwStyle );
      ...
      if( lpCnt->bIsSplit )
        ...
        if( lpCntFrame->SplitBar.hWndLeft )
          ...
          SetWindowLongPtr( lpCntFrame->SplitBar.hWndLeft, GWL_STYLE, (LONG_PTR)lpCntLeft->dwStyle );
...
DWORD_PTR WINAPI CntStyleGet( HWND hWnd )
    {
    LPCONTAINER  lpCnt = GETCNT(hWnd);
...
/*OutputDebugString(szDebug);*/
      ...
      {
      wCurNotify = lpCnt->lpCI->wCurNotify;
      }
	return wCurNotify;
    }
...
HWND WINAPI CntCNChildWndGet( HWND hCntWnd, LPARAM lParam )
    ...
    if( (lpCnt->dwStyle & CTS_ASYNCNOTIFY) && hEvent )
      ...
      {
      hWndSender = lpCnt->lpCI->hWndCurCNSender;
      }
	return hWndSender;
    }
...
UINT WINAPI CntCNCharGet( HWND hCntWnd, LPARAM lParam )
    ...
    if( (lpCnt->dwStyle & CTS_ASYNCNOTIFY) && hEvent )
      ...
      {
      wCurAnsiChar = lpCnt->lpCI->wCurAnsiChar;
      }
	return wCurAnsiChar;
    }
...
BOOL WINAPI CntCNShiftKeyGet( HWND hCntWnd, LPARAM lParam )
    ...
    if( (lpCnt->dwStyle & CTS_ASYNCNOTIFY) && hEvent )
      ...
      {
      bShiftKey = lpCnt->lpCI->bCurCNShiftKey;
      }
	  return bShiftKey;
    }
...
BOOL WINAPI CntCNCtrlKeyGet( HWND hCntWnd, LPARAM lParam )
    ...
    if( (lpCnt->dwStyle & CTS_ASYNCNOTIFY) && hEvent )
      ...
      {
      bCtrlKey = lpCnt->lpCI->bCurCNCtrlKey;
      }
	return bCtrlKey;
    }
    ...
    if( lpFld->wColType == CFT_STRING || lpFld->wColType == CFT_LPSTRING )
      ...
      while( lpRec )
        ...
        if( uMethod == AS_AVGCHAR )
          ...
          if( nStrLen )
            {
            GetTextExtentPoint32( hDC, lpStrCell, nStrLen, &Size );
