...
/*    lpCntNew = (LPCONTAINER) GlobalAllocPtr( GHND, LEN_CONTAINER );*/
    ...
    SetWindowLongPtr( hWnd, GWL_CONTAINERHMEM, (LONG_PTR)lpCntNew );

    // User should never use the window scrollbar styles.
    dwStyle  = (DWORD_PTR) GetWindowLongPtr( hWnd, GWL_STYLE );
...
BOOL Frm_OnCreate( HWND hWnd, LPCREATESTRUCT lpCreateStruct )
    ...
    *-------------------------------------------------------------------*/
    ...
    lpCnt->hWnd1stChild = CreateWindow( CNTCHILD_CLASS, "", (DWORD)lpCnt->dwStyle,
                          0, 0, 0, 0,
    ...
    SetWindowLongPtr( lpCnt->hWnd1stChild, GWL_SHOWVSCROLL, TRUE );
...
/*      GlobalFreePtr( lpCnt );*/
      ...
      SetWindowLongPtr( hWnd, GWL_CONTAINERHMEM, (LONG_PTR)lpCnt );
      }
    else
      {
      // Free global memory for the CONTAINER struct.
