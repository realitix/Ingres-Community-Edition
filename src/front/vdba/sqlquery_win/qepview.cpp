...
static BOOL SetIconAndText(CWnd* pWnd, UINT nIcon, CString strText)
{
	...
	SetLastError(0);
	LONG_PTR lOld = SetWindowLongPtr(pWnd->m_hWnd, GWLP_USERDATA, (LONG_PTR)nIcon);
...
LRESULT CvQueryExecutionPlanView::OnRedrawLinks (WPARAM wParam, LPARAM lParam)
{
	Invalidate();
	return 0;
}
...
LRESULT CvQueryExecutionPlanView::OnOpenPopupInfo (WPARAM wParam, LPARAM lParam)
{
	m_pPopupInfoWnd = NULL;
	CdQueryExecutionPlanDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
LRESULT CvQueryExecutionPlanView::OnClosePopupInfo(WPARAM wParam, LPARAM lParam)
{
	if (m_pPopupInfoWnd)
