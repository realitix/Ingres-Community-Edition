...
static BOOL NextWord (const CString& strText, int index, LPCTSTR lpszWord)
{
	...
	int nWLen = (int)_tcslen (lpszWord);
	if (index > 0)
	{
...
static int Find (const CString& strText, TCHAR chFind, int nStart, BOOL bIgnoreInsideBeginEnd)
{
	...
	for ( i=nStart; i<nLen; i+= (int)_tclen((const TCHAR *)strText + i) )
	{
		if (nCommentStart > 0)
		{
			...
			if ((nCommentStart == 1 && nCommentEnd == 1) || (nCommentStart == 2 && nCommentEnd == 2))
			{
				...
				i+= (int)_tclen((const TCHAR *)strText + i);
				nCommentStart = 0;
				nCommentEnd   = 0;
			}
			continue;
		...
		if (cChar == _T('\'') && !bDQuote)
		{
			...
			int iNext = i + (int)_tclen((const TCHAR *)strText + i);
		...
		if (bIgnoreInsideBeginEnd)
		{
			if (!bDeclare && (cChar == _T('D') || cChar == _T('d')))
			{
				if (NextWord (strText, i, _T("DECLARE")))
				{
					...
					// be generated !!!.
					bDeclare = TRUE;
					i += (int)_tcslen (_T("DECLAR")); // i += strlen (_T("DECLARE")) -1;
					continue;
				}
			}
			...
			if (cChar == _T('B') || cChar == _T('b'))
			{
				if (NextWord (strText, i, _T("BEGIN")))
				{
					bDeclare = FALSE;
					nBeginCount++;
					i += (int)_tcslen (_T("BEGI")); // i += strlen (_T("BEGIN")) -1;
					continue;
				}
			}
			...
			if (cChar == _T('E') || cChar == _T('e'))
			{
				...
				{
					nEndCount++;
					i += (int)_tcslen (_T("EN")); // i += strlen (_T("END")) -1;
				}
				//
			...
			if (cChar == _T('C') || cChar == _T('c'))
			{
				...
				{
					nCaseCount++;
					i += (int)_tcslen (_T("CAS")); // i += strlen (_T("CASE")) -1;
				}
				//
...
BOOL CfSqlQueryFrame::ParseStatement (int nExec, CString& strRawStatement)
{
	...
	while (pos != -1 || !strText.IsEmpty())
	{
		...
		{
			nStart = nEnd;
			nStart+= (int)_tclen((const TCHAR *)strRawStatement + nStart);
			continue;
		}
