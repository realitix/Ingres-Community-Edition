...
LRESULT CuDlgSqlQueryResult::OnSQLStatementChange (WPARAM wParam, LPARAM lParam)
{
	TCITEM item;
	memset (&item, 0, sizeof (item));
	item.mask       = TCIF_PARAM;
...
// 1: qep.
// 2: generate xml.
LRESULT CuDlgSqlQueryResult::OnExecuteSQLStatement (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgSqlQueryResult::LoadDocument()
{
	...
	if (pDoc->IsLoadedDoc())
	{
		...
		pDoc->m_nResultCounter = pDoc->m_nResultCounter - (int)pDoc->m_listPageResult.GetCount();
