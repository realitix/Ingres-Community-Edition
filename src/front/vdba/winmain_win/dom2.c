...
** 11-Apr-2011 (drivi01)
**    Fix routines that bring up Create Index dialog
**    to determine table type for which index is being created
**    and display appropriate index structures in the dialog.
************************************************************************/
...
VOID DomAddObject (HWND hwndMdi, LPDOMDATA lpDomData, WPARAM wParam, LPARAM lParam)
{
   ...
   // JFS End
       ...
       */
       ...
       case OT_INDEX:
       {
           /* If the DOM item selected is an index, we need to get a parent (static Index)
           ** and move onto the next case statement.
           */
           dwParent = (DWORD)SendMessage(lpDomData->hwndTreeLb, LM_GETPARENT, 0, dwCurSel);
           if (!dwParent)
				return;
           lpRecord = (LPTREERECORD)SendMessage(lpDomData->hwndTreeLb, 
			   LM_GETITEMDATA, 0, (LPARAM)dwParent);
       ...
       {
		   /* Get a parent table here and figure out if it's VectorWise table.
		   ** Load table type into DMLCREATESTRUCT for CreateIndex dialog to
		   ** make sure proper dialog with proper index is displayed.
		   */
                dwParent = (DWORD)SendMessage(lpDomData->hwndTreeLb, LM_GETPARENT, 0, dwParent ? dwParent : dwCurSel);
                if (!dwParent)
					return;
                lpRecord = (LPTREERECORD)SendMessage(lpDomData->hwndTreeLb, 
					LM_GETITEMDATA, 0, (LPARAM)dwParent);
...
VOID DomCreateIndex(HWND hwndMdi, LPDOMDATA lpDomData, WPARAM wParam, LPARAM lParam)
{
	...
	switch (objType)
	{
	    ...
	    case OT_TABLE:
	    {
		if (IsVW() && (tblType = getint(lpRecord->szComplim + STEPSMALLOBJ + STEPSMALLOBJ)) > 0)
		{
			if (GetOIVers == OIVERS_NOTOI)
			{
				INDEXPARAMS idx;
...
VOID DomRegisterAsLink(HWND hwndCurDom, LPDOMDATA lpCurDomData, BOOL bDragDrop, HWND hwndDestDom, LPDOMDATA lpDestDomData, DWORD_PTR ItemUnderMouse)
{
  ...
  int   objType = -1;
  int   level   =  -1;
  INT_PTR   iret = IDCANCEL;    // compare to IDOK
