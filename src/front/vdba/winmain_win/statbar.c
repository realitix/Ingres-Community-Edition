...
static void SB_SetField( HWND hwnd, WPARAM wID, LPSTR lpText );
static void DestroyStatBar( HWND hwnd );
static void SB_SetMode( HWND hwnd, WPARAM wMode, LONG lParam );
static void PASCAL SB_SetBarColor( HWND hwnd, COLORREF lColor );
static void PASCAL SB_SetFieldColor( HWND hwnd, WPARAM wID, COLORREF lColor, WORD wWhat );
static void PASCAL SB_ShowField( HWND hwnd, WPARAM wID, LONG lShowHide );
static void PASCAL SB_SizeField( HWND hwnd, WPARAM wID, LPRECT lpRect );
...
HWND NewStatusBar( HANDLE hInst, HWND hwndParent, LPSB_DEFINITION lpStatBarDef )
{
  ...
  if (hwndStatBar)
  {
    ...
    SetWindowLongPtr(  hwndStatBar, GWLP_USERDATA, (LPARAM)hStatBar );
  }
					...
					WPARAM wParam, LPARAM lParam )
{
  switch (message)
  {
    ...
    case SBM_SETFIELD:
      SB_SetField( hwnd, (WPARAM)wParam, (LPSTR)lParam );
      break;

    case SBM_SETMODE:
      SB_SetMode( hwnd, (WPARAM)wParam, 0L );
      break;

    case SBM_SHOWFIELD:
      SB_ShowField( hwnd, (WPARAM)wParam, (LONG)lParam );
      break;

    case SBM_SIZEFIELD:
      SB_SizeField( hwnd, (WPARAM)wParam, (LPRECT)lParam );
      break;

    case SBM_SETBARBKGCOLOR:
      SB_SetBarColor( hwnd, (COLORREF)lParam );
      return 0;

    case SBM_SETFIELDFACECOLOR:
      /* wParam == 0xffff change all fields' color */
      SB_SetFieldColor( hwnd, (WPARAM)wParam, (COLORREF)lParam, 0 );
      return 0;

    case SBM_SETFIELDTEXTCOLOR:
      /* wParam == 0xffff change all fields' color */
      SB_SetFieldColor( hwnd, (WPARAM)wParam, (COLORREF)lParam, 1 );
      return 0;
  }
...
static void SB_EraseBackground( HWND hwnd, HDC hdc )
{
  ...
  hStatBar = (HWND)GetWindowLongPtr( hwnd, GWLP_USERDATA );
  if (hStatBar == NULL)
    return;
...
static void SB_SetField( HWND hwnd, DWORD_PTR wID, LPSTR lpText )
{
  HANDLE      hStatBar;
  LPSTATBAR   lpStatBar;
  LPSTATFIELD lpStatField;
...
static void SB_SetMode( HWND hwnd, DWORD_PTR wMode, LONG lParam )
{
  HANDLE      hStatBar;
  LPSTATBAR   lpStatBar;
  RECT        clientRect;
...
static void PASCAL SB_ShowField( HWND hwnd, DWORD_PTR wID, LONG lShowHide )
{
  UINT        i;
  HANDLE      hStatBar;
  LPSTATBAR   lpStatBar;
...
static void PASCAL SB_SizeField( HWND hwnd, DWORD_PTR wID, LPRECT lpRect )
{
  UINT        i;
  HANDLE      hStatBar;
  LPSTATBAR   lpStatBar;
...
static void PASCAL SB_SetFieldColor( HWND hwnd, DWORD_PTR wID, COLORREF lColor, WORD wWhat )
{
  UINT        i;
  HANDLE      hStatBar;
  LPSTATBAR   lpStatBar;
