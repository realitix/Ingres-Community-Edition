...
**  02-May-2010 (drivi01)
**     Add the VW version to DOM dialog, so every time dialog is
**     selected the version field is updated from lpDomData.
**     This will make sure that if VDBA is connected to several 
**     versions of VW at the same time, the functionality in each
**     window corresponds to the version.
*****************************************************************************/
...
{
  INT_PTR   domCreateMode;                // dom creation mode: tearout,...
...
extern void TS_MfcUpdateRightPane(HWND hwndDoc, LPDOMDATA lpDomData, BOOL bCurSelStatic, DWORD_PTR dwCurSel, int CurItemObjType, LPTREERECORD lpRecord, BOOL bHasProperties, BOOL bUpdate, int initialSel, BOOL bClear);
...
static BOOL NEAR DomFillTree(LPDOMDATA lpDomData, INT_PTR domCreateMode, HWND hwndMdiRef, FILEIDENT idFile);
...
//
static VOID NEAR  MakeDomCaption(char *sz, char *VirtNodeName, INT_PTR domCreateMode, HWND hwndMdiRef, BOOL bIncrementNum)
{
  ...
  if (hwndMdiRef)
    lpRefDomData = (LPDOMDATA)GetWindowLongPtr(hwndMdiRef, 0);
  else
    lpRefDomData = NULL;
...
LRESULT CALLBACK __export DomMDIChildWndProc (HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwnd, 0);
...
BOOL DomOnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
  LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwnd, 0);
  BOOL  retval;
...
static void DomOnDestroy(HWND hwnd)
{
    ...
    SetWindowLongPtr(hwnd, 0, 0L);   // Pure fix May 7, 97: lpDomData not valid anymore
...
void DomOnMdiActivate(HWND hwnd, BOOL fActive, HWND hwndActivate, HWND hwndDeactivate)
{
  ...
  if (fActive) {
	...
	if (lpDomData->bIsVectorWise) SetVWVers(lpDomData->vwVer);
  }
...
static BOOL NEAR DomWindowCreate(HWND hwndMdi, LPCREATESTRUCT lpCs)
{
  ...
  if (!lpDomData) {
    ...
    SetWindowLongPtr(hwndMdi, 0, 0L);
    return FALSE;
  }
  SetWindowLongPtr(hwndMdi, 0, (LONG_PTR) lpDomData);
  ...
  switch (lpDomCreateData->domCreateMode) {
    ...
    case DOM_CREATE_NEWWINDOW:
      // take rid and number according to cloned dom type
      lpRefDomData = (LPDOMDATA)GetWindowLongPtr(lpDomCreateData->hwndMdiRef, 0);
  ...
  if (IsVW()) lpDomData->vwVer = GetVWVers();
...
static BOOL NEAR DomWindowCommand(HWND hwndMdi, LPDOMDATA lpDomData, int id, HWND hwndCtl, UINT codeNotify)
{
  ...
  switch (id) {
    ...
    case IDM_REGISTERASLINK:
      DomRegisterAsLink(hwndMdi, lpDomData, FALSE, NULL, NULL, (DWORD_PTR)NULL);
      break;

    // Star management
    case IDM_REFRESHLINK:
      DomRefreshLink(hwndMdi, lpDomData, FALSE, NULL, NULL, (DWORD_PTR)NULL);
      break;
...
static BOOL NEAR DomFillTree(LPDOMDATA lpDomData, INT_PTR domCreateMode, HWND hwndMdiRef, FILEIDENT idFile)
{
  ...
  DWORD_PTR         tearedRecId;
  DWORD_PTR         recId;
  DWORD_PTR         lastCollapsedRecId;
  DWORD_PTR         refParentRecId;
  DWORD_PTR         stopLevel;
  DWORD_PTR         curLevel;
  ...
  switch (domCreateMode) {
    ...
    case DOM_CREATE_SCRATCHPAD:
      ...
      if (domCreateMode == DOM_CREATE_NEWWINDOW || domCreateMode == DOM_CREATE_TEAROUT)
        ...
        curSelRecId = -1; // will never be found
      ...
      if (domCreateMode == DOM_CREATE_NEWWINDOW) {
        ...
        stopLevel = (DWORD_PTR)(-1L);                 // will never be reached!
...
//
BOOL AddParentBranch(LPDOMDATA lpDomData, DWORD_PTR recId)
{
...
static DWORD_PTR MakeNewRecId(DWORD_PTR recId);
static DWORD_PTR GetNewRecIdFromOld(DWORD_PTR refParentRecId, BOOL bAdvance);
static void  FreeNewRecIds(void);
static DWORD_PTR GetNextNewRecId();

#define MAXIMORUM 200 // maximorum 200 * 16000 approximately...
static DWORD_PTR *ReseqListAnchor[MAXIMORUM];
...
static BOOL  AllocateNewRecIds(void)
{
  ...
  ReseqListAnchor[0] = (DWORD_PTR *)ESL_AllocMem(ReseqListSize);
...
static DWORD_PTR MakeNewRecId(DWORD_PTR recId)
{
  ...
  if (ReseqListIndex < ReseqListMaxItem) {
    ...
    if (ReseqListArrayNum < MAXIMORUM-1) {
      ...
      ReseqListIndex = 0;
      ReseqListTotal++;
      ReseqListAnchor[ReseqListArrayNum] = (DWORD_PTR *)ESL_AllocMem(ReseqListSize);
...
BOOL ResequenceTree(HWND hwndMdi, LPDOMDATA lpDomData)
{
  ...
  DWORD_PTR         recId;              // record Id in original tree
  DWORD_PTR         newRecId;           // record Id in resequenced tree
  DWORD_PTR         refParentRecId;
  LPTREERECORD  lpRefData, lpData;
  LINK_RECORD   lr;
  LPSTR         lpString;
  DWORD_PTR         dwTop, dwSel;       // selection and top item
...
BOOL DomIsSystemChecked(HWND hWnd)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndMdi, 0);
...
static BOOL  NEAR RefreshTrees(HWND hwndMdi, LPDOMDATA lpDomData)
{
  ...
  DWORD_PTR dwCurSel = GetCurSel(lpDomData);
  ...
  if (bHasProperties) {
    ...
    DWORD_PTR dwNewCurSel = GetCurSel(lpDomData);
...
LRESULT FAR PASCAL __export BaseOwnerProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
static VOID NEAR DeleteNanBarSpecialObjects(LPDOMDATA lpDomData)
{
    SetWindowLongPtr(lpDomData->hwndBaseOwner, GWLP_WNDPROC,
  ...
  SetWindowLongPtr(lpDomData->hwndOtherOwner, GWLP_WNDPROC,
  ...
  SetWindowLongPtr(lpDomData->hwndSystem, GWLP_WNDPROC,
...
static void SubclassCombo(HWND hwndCombo, LPDOMDATA lpDomData)
{
  if (hwndCombo) {
    ...
    if (hwndCombo == lpDomData->hwndBaseOwner) {
        ...
        lpDomData->lpOldBaseOwnerProc = (WNDPROC)GetWindowLongPtr(hwndCombo, GWLP_WNDPROC);
        SetWindowLongPtr(hwndCombo, GWLP_WNDPROC, (LONG_PTR)lpDomData->lpNewBaseOwnerProc);
    }
    else {
      lpDomData->lpNewOtherOwnerProc= (WNDPROC)MakeProcInstance((FARPROC)OtherOwnerProc, hInst);
      lpDomData->lpOldOtherOwnerProc = (WNDPROC)GetWindowLongPtr(hwndCombo, GWLP_WNDPROC);
      SetWindowLongPtr(hwndCombo, GWLP_WNDPROC, (LONG_PTR)lpDomData->lpNewOtherOwnerProc);
    }
    // store lpDomData
    SetWindowLongPtr(hwndCombo, GWLP_USERDATA, (LONG_PTR)lpDomData);
  }
}
...
static void SubclassCheckbox(HWND hwndCheck, LPDOMDATA lpDomData)
{
  if (hwndCheck) {
    ...
    lpDomData->lpOldSystemProc = (WNDPROC)GetWindowLongPtr(hwndCheck, GWLP_WNDPROC);
    SetWindowLongPtr(hwndCheck, GWLP_WNDPROC, (LONG_PTR)lpDomData->lpNewSystemProc);
    // store lpDomData
    SetWindowLongPtr(hwndCheck, GWLP_USERDATA, (LONG_PTR)lpDomData);
  }
}
...
static void NEAR SizeTreeStatusBarWnd(HWND hwndParent)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndParent, 0);
...
static int NEAR GetTreeStatusBarHeight(HWND hwnd)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(GetParent(hwnd), 0);
...
//  int           resultLevel;
  ...
  // tree line
  DWORD_PTR         idChildObj;       // child object id
//  DWORD         idChild2Obj;      // sub-child object id
  DWORD_PTR         idLast;           // last in the tree, for insert after
//  int           iret;
  LPTREERECORD  lpRecord;
...
LRESULT CommonTreeSubclassDefWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(GetParent(hwnd), 0);
    if (!lpDomData)
      return 0;
