...
** 25-Mar-2011 (drivi01)
**    Add a new structure type called VectorWise_row (aka PAX).
*/
...
BOOL FillErrorStructure()
{
  ...
  long lCode;
  size_t iNbChar,iNbBytesAllocated,iPos,iRetlineLength,iSeperatelineLength,iErrorStatementLength,iErrorTextLength;
    ...
    if (hFile != HFILE_ERROR) {
      if (_llseek(hFile, 0, 2) != HFILE_ERROR) {
        ...
        while (p) {
          p2 = x_strchr(p, '\n');
          if (p2) {
            _lwrite(hFile, p, (UINT)(p2-p));
            _lwrite(hFile, "\r\n", 2);
            p = ++p2;
          }
          else {
            _lwrite(hFile, p, (UINT)x_strlen(p));
            _lwrite(hFile, "\r\n", 2);
            break;
          }
        }
...
/*
**  History:
**	08-Aug-2011 (wanfr01) b122362
**	    Select from IICONSTRAINTS needs to be ordered by TEXT_SEQUENCE
**	    since the memory is initialized when TEXT_SEQUENCE = 1
*/
int VDBA2xGetTableCheckInfo  (LPTABLEPARAMS lpTS)
{
    int iret, m;
