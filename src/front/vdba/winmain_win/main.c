...
**    by removing deprecated POSIX function names.
** 13-Dec-2010 (drivi01)
...
static LONG  NEAR CommandHandler (HWND hwnd, int id, HWND hwndCtl, UINT codeNotify, DWORD_PTR dwData);
...
static INT_PTR   NEAR QueryCurDocType(HWND hwnd, HWND *pHwndDoc);
...
static INT_PTR  NEAR DOMLoadPassword(FILEIDENT idFile, BOOL *pbStopHere);
...
void        MainOnTimer(HWND hwnd, UINT_PTR id);
...
static BOOL VdbaWinHelp(HWND hWndMain, LPCTSTR lpszHelp, UINT uCommand, DWORD_PTR dwData)
{
	UINT nHelpID = 0;
	...
	case HELP_CONTEXT:
		...
		break;
	case HELP_FINDER:
		HtmlHelp(hWndMain, lpszHelp, HH_DISPLAY_SEARCH, (DWORD_PTR)&q);
		break;
...
static INT_PTR   storedActiveDocType  = 0;
...
INT_PTR WINAPI VdbaDialogBox(HINSTANCE hInstance, LPCSTR lpCstr, HWND hWnd, DLGPROC dlgProc)
{
  INT_PTR retval;
  vdbaDialog++;
...
INT_PTR WINAPI VdbaDialogBoxParam(HINSTANCE hInstance, LPCSTR lpCstr, HWND hWnd, DLGPROC dlgProc, LPARAM lParam)
{
  INT_PTR retval;
  vdbaDialog++;
   ...
   switch (nCode)
   {
       case MSGF_DIALOGBOX:
       {
           ...
           if (lpHelpStack && (lpHelpStack->Id != OFFSETHELPMENU) )
             ...
             if (lpHelpStack)
               VdbaWinHelp (hwndFrame, VDBA_GetHelpFileName(), HELP_PARTIALKEY, (DWORD_PTR)"");   // for .cnt file
             else
...
static VOID NEAR CloseOpenedCombobox()
{
  ...
  switch (curDocType) {
    case TYPEDOC_DOM:
      ...
      lpDomData = (LPDOMDATA)GetWindowLongPtr(GetVdbaDocumentHandle(curDocHwnd), 0);
...
static LONG NEAR CommandHandler (HWND hwnd, int id, HWND hwndCtl, UINT codeNotify, DWORD_PTR dwData)
{
  ...
  switch (id) {
    ...
    case IDM_HELPSEARCH:
      VdbaWinHelp (hwndFrame, VDBA_GetHelpFileName(), HELP_FINDER, (DWORD_PTR)0L);   // for .cnt file
      break;
...
//
static INT_PTR NEAR QueryCurDocType(HWND hwndFrame, HWND *pHwndDoc)
{
  HWND hwndDoc;
...
// This function MUST be updated for every new MDI document Class
//
LRESULT QueryDocType(HWND hwndDoc)
{
  INT_PTR *p;
  ...
  if(hDocHandle)
    p = (INT_PTR *)GetWindowLongPtr(hDocHandle, 0);
  else
    p = 0;
  if (!p) {
  ...
  while (hwndCurDoc) {
    ...
    switch (docType) {
      case TYPEDOC_DOM:
        ...
        sComplim.count_all = (LONG_PTR)SendMessage(hTree, LM_GETCOUNT, COUNT_ALL, 0L);
        sComplim.curRecId  = GetCurSel(lpDomData);
        sComplim.FirstVis  = (DWORD_PTR)SendMessage(hTree, LM_GETTOPITEM, 0, 0L);
        ...
        while (curRecId) {
          ...
          sComplimLine.parentRecId = (DWORD_PTR)SendMessage(hTree, LM_GETPARENT,
                            0, (LPARAM)curRecId);
          ...
          curRecId = (LPARAM)SendMessage(lpDomData->hwndTreeLb, LM_GETNEXT, 0, 0L);
        }
        break;
...
VOID UpdateGlobalStatus()
{
  ...
  INT_PTR     docType;        // current document type
  ...
  if (docType != TYPEDOC_UNKNOWN && docType != TYPEDOC_NONE) {
    ...
    HWND hDocHandle = GetVdbaDocumentHandle(hwndDoc);
    if(hDocHandle)
      lpVoidDocData = (LPVOID)GetWindowLongPtr(hDocHandle, 0);
    else
      lpVoidDocData = NULL;
  }
  else
