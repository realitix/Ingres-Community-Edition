...
static int GetValidDataType( UINT_PTR ColType , LPGATEWAYCAPABILITIESINFO pGWInfo)
{
    switch (ColType) {
...
static int DuplicateTableParamsStruct( LPTABLEPARAMS desttbl, LPTABLEPARAMS lpTS )
{
    ...
    if (lpTS->lpColumnHeader)   {
        size_t iSqlLen = x_strlen(lpTS->lpColumnHeader)+1;
    ...
    if (lpTS->lpSelectStatement)    {
        size_t iSqlLen = x_strlen(lpTS->lpSelectStatement)+1;
