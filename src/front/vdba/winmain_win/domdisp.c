...
** 01-Apr-2011 (drivi01)
**    Add Index item to the tree for IVW 1.5 release.
**    Index in IVW 1.5 is more like traditional Ingres index.
** 10-Aug-2011 (frima01) Bug 125297
**    In DOMUpdateDisplayData2 for OT_GRANTEE_SOLVE pass on grantee
**    type to DOMGetFirstObject if specified at grant time.
**    Also when determining grants for users pass on grantee type
**    to DOMGetFirstRelObject.
...
// for star management
static DWORD_PTR AddStaticSubItem(LPDOMDATA lpDomData, DWORD_PTR idParent, UCHAR *bufParent1, UCHAR *bufParent2, UCHAR *bufParent3, int parentDbType, UCHAR *bufOwner, UCHAR *bufComplim, UCHAR *tableOwner, int otStatic, UINT idRsString, BOOL bWithDummy, BOOL bSpeedFlag);
...
DWORD_PTR GetAnyAnchorId(LPDOMDATA lpDomData, int iobjecttype, int level,
                     LPUCHAR *parentstrings, DWORD_PTR previousId,
                     LPUCHAR bufPar1, LPUCHAR bufPar2, LPUCHAR bufPar3,
                     int iAction, DWORD_PTR idItem, int *piresultobjecttype,
...
static DWORD_PTR *DumListAnchor[MAXIMORUM];
...
static BOOL NEAR DumListInit()
{
  ...
  DumListAnchor[0] = (DWORD_PTR *)ESL_AllocMem(DumListSize);
  if (!DumListAnchor[0])
    return FALSE;
...
//
static BOOL NEAR DumListAdd(DWORD_PTR idChildObj)
{
  ...
  if (DumListIndex < DumListMaxItem) {
    ...
    if (DumListArrayNum < MAXIMORUM-1) {
      ...
      DumListIndex = 0;
      DumListTotal++;
      DumListAnchor[DumListArrayNum] = (DWORD_PTR *)ESL_AllocMem(DumListSize);
...
//
static BOOL NEAR DumListRemove(DWORD_PTR idChildObj)
{
  long  lcpt;
  int   arrayNum;
  long  index;
...
int DOMDisableDisplay(int hnodestruct, HWND hwndDOMDoc)
{
  ...
  if (!hwndDOMDoc) {
    ...
    while (hwndCurDoc) {
      ...
      if (QueryDocType(hwndCurDoc) == TYPEDOC_DOM) {
        lpDomData = (LPDOMDATA)GetWindowLongPtr(GetVdbaDocumentHandle(hwndCurDoc), 0);
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndDOMDoc, 0);
                           ...
                           HWND     hwndDOMDoc)     // handle on DOM MDI document
{
  ...
  DWORD_PTR dwOldCurSel = 0;
  DWORD_PTR dwNewCurSel = 0;
...
{
  DWORD_PTR dwFirstItem,dwNextItem,dwChildObj,recId;
  int iRet, objType;
...
//
//
int DOMUpdateDisplayData2 (int hnodestruct, int iobjecttype, int level, LPUCHAR *parentstrings, BOOL bWithChildren, int iAction, DWORD_PTR idItem, HWND hwndDOMDoc, int forceRefreshMode)
{
  int           iret;
  int           iretGlob;
  DWORD_PTR         idAnchorObj;      // id of the anchor object in the lbtree
  DWORD_PTR         idChildObj;       // id of the child object in the lbtree
  DWORD_PTR         idChild2Obj;      // id of the child objects of second level
  DWORD_PTR         idChild3Obj;      // id of child objects of depper level
  DWORD_PTR         idChild4Obj;      // id of child objects of ultra-deep level
  DWORD_PTR         idFirstOldChildObj; // id of the child in the branch
                                    // FindImmediateSubItem will start of
  DWORD_PTR         idLastOldChildObj;  // id of the child in the branch
                                    // FindImmediateSubItem will stop on
  DWORD_PTR         idInsertAfter;    // child we will insert after, for sort
  DWORD_PTR         idInsertBefore;   // child we will insert before, for sort
  DWORD_PTR         parentitem;		  // parent of tree item
  DWORD_PTR         parentitem2;	  // another parent item 
  UCHAR         buf[BUFSIZE];
  UCHAR         buf2[BUFSIZE];
  UCHAR         buf3[BUFSIZE];
  UCHAR         buf4[BUFSIZE];
  ...
  if (!hwndDOMDoc) {
    ...
    while (hwndCurDoc) {
      ...
      if (QueryDocType(hwndCurDoc) == TYPEDOC_DOM) {
        ...
        lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndMfcDoc, 0);
        if (lpDomData->psDomNode->nodeHandle == hnodestruct) {
  ...
  switch (iobjecttype) {
    ...
    case OT_GRANTEE_SOLVE:
      ...
      // pass on grantee type if present, else 0 is passed
      // we don't want to associate grants to non-existing
      // groups/roles with existing users of the same name
      resultType = 0;
...
//            return MergeResStatus(iretGlob, RES_ERR);   // Fatal
	    ...
	    idChild2Obj = AddStaticSubItem(lpDomData, idAnchorObj,
          ...
          // Add sub-subitem INDEX with sub dummy collapsed
          if (tblType == 0 || (tblType !=0 && GetVWVers() >= VWVERS_15))
          {
	    idChild2Obj = AddStaticSubItem(lpDomData, idAnchorObj,
                        aparentsResult[0],
                        buf2,       // schema - instead of aparentsResult[1]
      ...
      while (1) {
        ...
        */
        ...
        if (IsRelatedType(iobjecttype)) {
		  ...
		  // in order to determine grants for a grantee retrieve the grantee type first 
		  // and pass via bufComplim to DOMGetFirstRelObject
		  // get grantee node in tree - usually 3rd parent node (2nd parent node for
		  // granted roles)

		  if ( ( (iobjecttype >= OTR_GRANTEE_SEL_TABLE) && (iobjecttype <= OTR_GRANTEE_ROLE) ) ||
				(iobjecttype == OTR_GRANTEE_NEXT_SEQU) )
		  {
			  parentitem = SendMessage(lpDomData->hwndTreeLb,
													LM_GETPARENT,
													0, (LPARAM)idAnchorObj);

			  parentitem2 = SendMessage(lpDomData->hwndTreeLb,
													LM_GETPARENT,
													0, (LPARAM)parentitem);

	  		  if ( iobjecttype == OTR_GRANTEE_ROLE ) 
				lpRecord2=(LPTREERECORD)SendMessage(lpDomData->hwndTreeLb,
													LM_GETITEMDATA,
													0, (LPARAM)parentitem2);
			  else
			  {
				parentitem = SendMessage(lpDomData->hwndTreeLb,
													LM_GETPARENT,
			  ...
			  if ( (lpRecord2->recType == OT_USER) || (lpRecord2->recType == OT_ROLE) || 
				  (lpRecord2->recType == OT_GROUP) )
						storeint(bufComplim,lpRecord2->recType);
			  else {
				  switch (lpRecord2->displayType){
					  case OT_GRANTEE_SOLVED_USER:	storeint(bufComplim,OT_USER);
													break;
					  case OT_GRANTEE_SOLVED_GROUP:	storeint(bufComplim,OT_GROUP);
													break;
					  case OT_GRANTEE_SOLVED_ROLE:	storeint(bufComplim,OT_ROLE);
													break;
				  }
			  }
		  }
...
static BOOL NEAR RecordFits(LPDOMDATA lpDomData, int iobjecttype, int iAction, LPUCHAR *parentstrings, DWORD_PTR id, LPTREERECORD lpRecord, int *presultType, BOOL *pbRootItem);
