...
**    Add ParsePermitUserType to avoid confusing grants to non-existing
**    roles with grants of existing users of the same name.
********************************************************************/
...
UCHAR *ParseRuleText(LPUCHAR PUout)
{
   ...
   ***************************************/
   ...
   maxlen = pEnd-p ;
   slen=(maxlen > (size_t)(pEnd-p)) ? (pEnd-p+1) : maxlen; // take lower length
...
int ParsePermitUserType(extradata)
UCHAR *extradata;

{
   static struct UserTypes {
         char * type;
         int Value;
   } Put[] = {
		{" to user",     OT_USER},
   ...
   int j=0;
   while(Put[j].type) {
      if (x_strstr(extradata,Put[j].type)) 
         return Put[j].Value;    
      j++;
   }
   return 0; 
