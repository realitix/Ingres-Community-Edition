...
static DWORD_PTR    DraggedItem;        // item from which the drag/drop started
static HWND     hwndDomDest;        // handle of destination DOM
static DWORD_PTR    ItemUnderMouse;     // tree item id in the destination tree
...
static BOOL IsCursorOnMDIClient(HWND hwndActiveDoc, LPARAM lParam)
{
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndActiveDoc, 0);
...
static DWORD_PTR GetTreeItemUnderMouse(HWND hwndDom, HWND hwndRefDom, LPARAM lParam)
{
  LPDOMDATA lpDomData;
  DWORD_PTR     item;
  ...
  // ask for mouse item
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndDom, 0);
  item = SendMessage(lpDomData->hwndTreeLb, LM_GETMOUSEITEM, 0, lParam);
  return item;
}

// checks if we could drop here.
static BOOL NEAR CompatibleForDrop(HWND hwndDomDest, DWORD_PTR dwItemUnderMouse)
{
  BOOL          bStatic;
  ...
  lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndDomDest, 0);
...
//
static BOOL NEAR ManagePopupMenu(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwClickedItem)
{
  POINT ptCur;
...
BOOL DomTreeNotifMsg(HWND hwndMdi, UINT wMsg, LPDOMDATA lpDomData, WPARAM wParam, LPARAM lParam, LONG *plRet)
{
  ...
  DWORD_PTR dwOldCurSel  = -1;
  DWORD_PTR dwNewCurSel  = -1;
...
BOOL RPaneDragDropMousemove(HWND hwndMdi, int x, int y)
{
  ...
  RECT refRc;
  LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndMdi, 0); // GetVdbaDocumentHandle not needed
  GetWindowRect(lpDomData->hwndTreeLb, &refRc);
  x -= refRc.left;
  y -= refRc.top;
...
static BOOL DragDropMousemove(HWND hwndMdi, WPARAM wParam, LPARAM lParam, BOOL bRightPane)
{
  ...
  if (bOnMDIClient) {
    ...
    if (hwndDomDest) {
      lpDestDomData = (LPDOMDATA)GetWindowLongPtr(hwndDomDest, 0);
