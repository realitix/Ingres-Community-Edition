...
**   In UpdateDOMDataDetail store GranteeUGRType for grantees on tables,
**   view, procedures, events and sequences. If not specified at grant time
**   it will be 0.  We don't want to associate grants to non-existing
            ...
            for (iobj=0;iobj<pdata1->nbdb;
               ...
               for (i=0;iret1!=RES_ENDOFDATA; i++,pRawGranteedatanew=
                  ...
                  pRawGranteedatanew->GrantType  =
                              getint(extradata+MAXOBJECTNAME+STEPSMALLOBJ);
                  pRawGranteedatanew->GranteeUGRType  =
                              getint(extradata+MAXOBJECTNAME+2*STEPSMALLOBJ);
                  iret1 = DBAGetNextObject(buf,buffilter,extradata);
               }
