...
**    25-Mar-2011 (drivi01)
**	Add code for building "create table" statements with new
**	structure types: default, heap, vectorwise, vectorwise_row.
**    05-Jul-2011 (drivi01)
**      lpTS->bCreateVectorWise was used for IVW 1.0 release to
**      store result of a checkbox on "create table" dialog.
**      lpTS->nVWStructType will be used to store structure
**		type chosen in release 1.5 and on.
**      Fix "create table... with structure=heap" in IVW databases
**      by taking out "=" from GetVWVers() in VDBA20xWithClauseSyntax.
*******************************************************************************/
#include <assert.h>
#include "dba.h"
#include "dbaset.h"
...
/*        return NULL;    // Use the default option */
    ...
    x=ConcateStrings (&lpStructure, "structure=", (LPTSTR)0);
    if (!x)
    {
        if (m) *m = 0;
        return NULL;
    }
	/* Concatenate "with structure=" for VectorWise table
	** structures only.  If it's a heap then we could have
	** more "with clause" statements so it needs special handling.
	** Mark the table a bVectorWise to force structure statement
	** to be appended.
	*/
    if (IsVW() && GetVWVers() >= VWVERS_15 && lpTS->nVWStructType && lpTS->nVWStructType != IDX_HEAP)
    {
		switch(lpTS->nVWStructType)
		{
			case IDX_VW: //Vectorwise structure
				LoadString(hResource, IDS_STRUCT_VW, (LPTSTR)buff, sizeof(buff));
				x = ConcateStrings(&lpStructure, buff, (LPTSTR)0);
				break;
			case IDX_VWROW: //Vectorwise_row structure
				LoadString(hResource, IDS_STRUCT_VWROW, (LPTSTR)buff, sizeof(buff));
				x = ConcateStrings(&lpStructure, buff, (LPTSTR)0);
				break;
			case IDX_STORAGE_STRUCT_UNKNOWN: //Default structure
			default:
				if (lpStructure)
				{
					ESL_FreeMem (lpStructure);
					lpStructure = NULL;
					ESL_FreeMem (lpString);
					lpString = NULL;
				}
		...
		if (lpStructure)
		{
			x = ConcateStrings (&lpString, lpStructure, (LPTSTR)0);
			ESL_FreeMem (lpStructure);
		}
	if (!x)
	{
    ...
    else if (lpTS->bCreateVectorWise && (GetVWVers() < VWVERS_15))
    {
	LoadString(hResource, IDS_STRUCT_VW, (LPTSTR)buff, sizeof(buff));
	x = ConcateStrings(&lpStructure, buff, (LPTSTR)0);
	x = ConcateStrings (&lpString, lpStructure, (LPTSTR)0);
    ...
    {
		/* For VectorWise tables, we can create Ingres table
		** by specifying "with structure=heap".
		** this case here is meant to handle creation
		** of Ingres table.
		*/
		if (lpTS->nVWStructType == IDX_HEAP)
		{
				LoadString(hResource, IDS_STRUCT_HEAP, (LPTSTR)buff, sizeof(buff));
				x = ConcateStrings(&lpStructure, buff, (LPTSTR)0);
				x = ConcateStrings (&lpString, lpStructure, (LPTSTR)0);
				ESL_FreeMem (lpStructure);
				bOneClause = FALSE;
