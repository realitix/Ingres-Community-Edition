...
** 25-Mar-2011 (drivi01)
**    Add new VectorWise structure type: VectorWise_row.
**    Add VectorWise versioning into the code to make sure
**    we're still backwards compatible with older versions.
** 02-May-2011 (drivi01)
**    Add some routines to strip out the version # from a version
**    string, convert it to float and compare it against latest hardcoded
**    version.  The hardcoded version should be updated every time new
**    features are introduced and a new "else if" block should be added for
**    previous version.
**    This code is meant to keep the VDBA from breaking when version 
**    string is updated and intenral version of VDBA is not.
** 10-Aug-2011 (frima01) Bug 125297
**    In DBAOrgGetFirstObject store grantee type in pcur->ExtraData
**    if specified at grant time.
**    We don't want to associate grants to non-existing
**    groups/roles with existing users of the same name.
** 28-Oct-2011 (drivi01)
**    Update the versioning for Vectorwise product so that new
**    supported features can be enabled only for certain versions of
**    the product.
** 01-Nov-2011 (drivi01)
**    Add missing variable defines for versioning.
******************************************************************************/
    ...
    if (bCheckingConnection) {
		...
		bIsVectorWise=TRUE;
         ...
         if (bIsIngres && bIsVectorWise)
         {
               ...
               if ((v = strstr(buf, "1.5")) && (v == buf || v[-1] == ' '))
                  ivwversion = VWVERS_15;
               else if ((v = strstr(buf, "1.6")) && (v == buf || v[-1] == ' '))
				  ivwversion = VWVERS_16;
               else if ((v = strstr(buf, "2.0")) && (v == buf || v[-1] == ' '))
				  ivwversion = VWVERS_20;
               else
               {
		   strcpy(buf2, buf);
		   ptr1 = buf2 + 3; //skip VW in the version string
		   v = strstr(buf2, " (");
		   *v = '\0';
		   vers = atof(ptr1);
		   if (vers >= 2.0) //Set this number to latest defined version
			   ivwversion = VWVERS_20; //set ivwversion to latest defined version
               }
         }	
         else if (bIsIngres)
         {
  ...
  switch (iobjecttype) {
    case OTLL_GRANTEE:
        ...
        exec sql begin;
          ...
          storeint(pcur->ExtraData+MAXOBJECTNAME+2*STEPSMALLOBJ,
                   ParsePermitUserType(extradata));
          pcur=GetNextLLData(pcur);
          if (!pcur) {
            iresselect = RES_ERR;
            exec sql endselect;
