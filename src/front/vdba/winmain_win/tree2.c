...
** 25-Mar-2011 (drivi01)
**   Add a new table structure type called VectorWise_row (aka PAX).
** 11-Apr-2011 (drivi01)
**   Add GetParentAtLevel function to retrieve a
**   an item which is a parent item of the selected item
**   at a level specified.
**   Add IsVWTreeItem function which retrieves
**   a parent at level 3 (table level) of the selected item
**   and determines if the item is a VectorWise item based
**   on the parent table.
** 10-Aug-2011 (frima01) Bug 125297
**   In SolveType() pass grantee type to DOMGetObject. If no
**   grantee of the type has been found set lpRecord->objName
**   to empty string.
**   We don't want to associate grants to non-existing
**   groups/roles with existing users of the same name.
********************************************************************/
...
extern void FreeDomPageInfo(HWND hwndDom, DWORD_PTR m_pPageInfo);   // in mainvi2.cpp
...
static DWORD_PTR NEAR GetCurrentRecId(LPDOMDATA lpDomData);
...
#elif WIN64
#define TOP_SEUIL (0xFFFFFFFFFFFFFFFF - 100000000000000000)
#endif
// for debug - static DWORD_PTR TOP_SEUIL = 100;
...
//
DWORD_PTR TreeAddRecord(LPDOMDATA lpDomData, LPSTR lpString, DWORD_PTR parentId, DWORD_PTR insertAfter, DWORD_PTR insertBefore, LPVOID lpData)
{
  LINK_RECORD lr;
  BOOL        bRet;
  DWORD_PTR       recId;
...
//  Record destruction
//
BOOL TreeDeleteRecord(LPDOMDATA lpDomData, DWORD_PTR recordId)
{
...
//
DWORD_PTR AddDummySubItem(LPDOMDATA lpDomData, DWORD_PTR idParent)
{
  DWORD_PTR         dret;
...
//
DWORD_PTR AddProblemSubItem(LPDOMDATA lpDomData, DWORD_PTR idParent, int errortype, int iobjecttype, LPTREERECORD lpRefRecord)
{
  DWORD_PTR         dret;
  UCHAR         buf[MAXOBJECTNAME];
  LPTREERECORD  lpRecord;
...
//
DWORD_PTR GetAnchorId(LPDOMDATA lpDomData, int iobjecttype, int level, LPUCHAR *parentstrings, DWORD_PTR previousId, LPUCHAR bufPar1, LPUCHAR bufPar2, LPUCHAR bufPar3, int iAction, DWORD_PTR idItem, BOOL bTryCurrent, BOOL *pbRootItem)
{
  DWORD_PTR         id;
  LPTREERECORD  lpRecord;
  int           recType;
...
//
UCHAR *GetObjName(LPDOMDATA lpDomData, DWORD_PTR dwItem)
{
  LPTREERECORD  lpRecord;
...
//
DWORD_PTR GetCurSel(LPDOMDATA lpDomData)
{
  return (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETSEL, 0, 0L);
}
...
int GetCurSelObjType(LPDOMDATA lpDomData)
{
  ...
  dwCurSel=(DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETSEL, 0, 0L);
  if (dwCurSel)
...
//
int GetItemObjType(LPDOMDATA lpDomData, DWORD_PTR dwItem)
{
  LPTREERECORD  lpRecord;
...
//
int GetItemDisplayObjType(LPDOMDATA lpDomData, DWORD_PTR dwItem)
{
  LPTREERECORD  lpRecord;
...
//  returns FALSE if error (empty tree, record not valid)
//
BOOL IsItemObjStatic(LPDOMDATA lpDomData, DWORD_PTR dwItem)
{
  int objType;
...
//
DWORD_PTR GetPreviousSibling(LPDOMDATA lpDomData, DWORD_PTR dwReq)
{
...
//
DWORD_PTR GetNextSibling(LPDOMDATA lpDomData, DWORD_PTR dwReq)
{
#ifdef WIN95_CONTROLS
  return SendMessage(lpDomData->hwndTreeLb, LM_GETNEXTSIBLING,
                     (WPARAM)0, (LPARAM)dwReq);
#else   // WIN95_CONTROLS
...
//
DWORD_PTR GetFirstImmediateChild(LPDOMDATA lpDomData, DWORD_PTR idParentReq, DWORD *pParentLevel)
{
  if (pParentLevel)
#ifdef WIN95_CONTROLS
...
#endif  // WIN95_CONTROLS
  return (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETFIRSTCHILD,
                            0, (LPARAM)idParentReq);
}

//
//  returns a parent of a selected item at the level specified
//
//  returns current item if parent can not be found
//
//
//  11-Apr-2011 (drivi01)
//     This function returns a parent at level specified to
//     the selected parent.
//
DWORD_PTR GetParentAtLevel(LPDOMDATA lpDomData, DWORD_PTR dwCurSel, DWORD pParentLevel)
{

	DWORD pLevel = 0;
	DWORD_PTR dwParent = dwCurSel;
	pLevel = SendMessage(lpDomData->hwndTreeLb, LM_GETLEVEL,
							0, (LPARAM)dwCurSel);

	while ( pLevel > pParentLevel )
	{
	     dwParent = SendMessage(lpDomData->hwndTreeLb, LM_GETPARENT,
                              0, (LPARAM)dwParent);
		 pLevel = SendMessage(lpDomData->hwndTreeLb, LM_GETLEVEL,
							0, (LPARAM)dwParent);
	}
...
//
//  returns TRUE or FALSE if the item is VectorWise item
//
//  returns TRUE if item is a child of a VectorWise table
//          FALSE if the item is not a child of a VectorWise table
//
//
//  11-Apr-2011 (drivi01)
//     Created.
//
BOOL IsVWTreeItem(LPDOMDATA lpDomData, DWORD_PTR dwCurSel)
{
	DWORD pLevel;
	LPTREERECORD lpRecord;
	DWORD_PTR dwParent = dwCurSel;

	//if not connected to VW installation, return
	if (!IsVW())
		return FALSE;

	/* Retrieve a level of the item in the tree.  If it's
	** higher than table level, then at this stage we don't
	** know if it's a Vectorwise Item, return FALSE.
	*/
	pLevel = SendMessage(lpDomData->hwndTreeLb, LM_GETLEVEL,
							0, (LPARAM)dwCurSel);
	if (pLevel < 3)
	...
	/* If the level of the item is below a table level, 
	** find out which table it belongs to and determine if
	** a table is VW table.  If it is, then the item should
	** be treated as VW item.
	*/
	if (pLevel > 3)
		dwParent = GetParentAtLevel(lpDomData, dwCurSel, 3);
	lpRecord = (LPTREERECORD)SendMessage(lpDomData->hwndTreeLb,
								LM_GETITEMDATA, 0, (LPARAM)dwParent);
	if (lpRecord && lpRecord->recType == OT_TABLE &&
		getint(lpRecord->szComplim + STEPSMALLOBJ + STEPSMALLOBJ) > 0)
		return TRUE;
	else
		return FALSE;
}
...
//
DWORD_PTR GetNextImmediateChild(LPDOMDATA lpDomData, DWORD_PTR idParentReq, DWORD_PTR idCurChild, DWORD ParentLevel)
{
#ifdef WIN95_CONTROLS
  ...
  while (idChild) {
    ...
    if (ChildLevel == ParentLevel + 1) {
      ...
      idParent = (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETPARENT,
                                    0, (LPARAM)idChild);
...
//
DWORD_PTR GetFirstLevel0Item(LPDOMDATA lpDomData)
{
  return (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETFIRST, 0, 0L);
}
...
// must have been called prior to calling this function
//
DWORD_PTR GetNextLevel0Item(LPDOMDATA lpDomData, DWORD_PTR idCurItem)
{
  DWORD_PTR idChild;
  DWORD_PTR ChildLevel;
...
//
DWORD_PTR FindImmediateSubItem(LPDOMDATA lpDomData, 
                           DWORD_PTR idParentReq, 
                           DWORD_PTR idFirstChildObj, 
                           DWORD_PTR idLastChildObj, 
                           int iobjecttype, 
                           LPUCHAR objName, 
                           LPUCHAR objOwnerName,
                           LPUCHAR parentName, 
                           LPUCHAR bufComplim, 
                           DWORD_PTR *pidInsertAfter, 
                           DWORD_PTR *pidInsertBefore)
{
  DWORD_PTR     idCurChild;   // current child
  DWORD_PTR         idPrevChild;  // previous child, for sort management
  DWORD_PTR         dwParent;     // Parent id.
  LPUCHAR       curName;
  LPUCHAR       curParent0;
  ...
  *pidInsertBefore  = 0L;   // default value ---> insertAfter will be used
  ...
  if (idFirstChildObj) {
   dwParent = (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, LM_GETPARENT,
...
//  returns void
//
VOID TreeDeleteAllChildren(LPDOMDATA lpDomData, DWORD_PTR idParent)
{
  DWORD_PTR idChildObj;
...
//  returns void
//
extern VOID TreeDeleteAllMarkedChildren(LPDOMDATA lpDomData, DWORD_PTR idParent, DWORD_PTR idLastChildObj)
{
  DWORD_PTR         idChild;    // immediate child
  DWORD_PTR         idChild2;   // buffered child id
  LPTREERECORD  lpRecord;
  DWORD         dwParentLevel;
...
int SolveType(LPDOMDATA lpDomData, LPTREERECORD lpRecord, BOOL bUpdateRec)
{
  ...
  /* pass on grantee user type if present in query text */
  resultType = 0;
  if (lpRecord->szComplim)
	resultType = getint(lpRecord->szComplim);
  ...
  if (iret == RES_ENDOFDATA){
	/* indicate that there is no such object and thus no properties */
	lpRecord->objName[0] = '\0';
...
long GetNumberOfObjects(LPDOMDATA lpDomData, BOOL bForce)
{
  ...
  static        DWORD_PTR     lastCurSel;       // can be static
...
//
DWORD_PTR GetLastRecordId(LPDOMDATA lpDomData)
{
  DWORD_PTR curRecordId;
  DWORD_PTR prevRecordId;
