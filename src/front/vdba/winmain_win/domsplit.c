...
** 01-Apr-2011 (drivi01)
**    DOM displays index now, make sure index popup menu 
**    displays only supported menu items.
**    Update routines for popup menu to use IsVWTreeItem
**    function to take out some of the complexity out of
**    if statements used to determine which items in the
**    menu should be enabled or disabled.
**    Decided to leave the "Create Index" menu item in the
**    menu which was added for 1.0 release.
** 01-Nov-2011 (frima01) b125894
**    Enable ckpdb / rollforwarddb for Ingres VectorWise dbs > version
**    1.5, the commmands are functioning from Vectorwise 1.6 on.
**    
*/
...
// defaults to FALSE
BOOL CanRegisterAsLink( INT_PTR hnodestruct, LPTREERECORD lpRecord)
{
  int iobjecttype;
...
// defaults to FALSE
BOOL CanRefreshLink( INT_PTR hnodestruct, LPTREERECORD lpRecord)
{
  int iobjecttype;
...
int GetUnsolvedItemObjType(LPDOMDATA lpDomData, DWORD_PTR dwItem)
{
  LPTREERECORD  lpRecord;
...
VOID DomUpdateMenu(HWND hwndMdi, LPDOMDATA lpDomData, BOOL bActivate)
{
  HMENU         hMenu;
  BOOL          bCurSelStatic;
  DWORD_PTR         dwCurSel, dwParent = 0;
  int           CurItemObjType;
  LPTREERECORD  lpRecord, lpRecordParent;
  DWORD_PTR         dwFirstChild;
  int           potAddType = 0;     // Potential add type
  //char			vw_struct[MAXOBJECTNAME];
  ...
  //if (IsVW())
  //	  LoadString(hResource, IDS_STRUCT_VWIX, vw_struct, sizeof(vw_struct));
  ...
  if (hMenu)
    if (bActivate) {
      ...
      //Get the parent item in case we hit a static tree
      //record and we need to check a record from a parent
      dwParent = (DWORD_PTR)SendMessage(lpDomData->hwndTreeLb, 
...
//JFS End
      ...
      // Alter - Drop object
	  // Drop object only disabled for IVW indexes
      if (bCurSelStatic) {
        ...
        if (lpRecord==0 || IsNoItem(CurItemObjType, lpRecord->objName))
          ...
          if (CanObjectBeDeleted(CurItemObjType, lpRecord->objName,
                                                lpRecord->ownerName) &&
            (!IsVW() || 
            ((IsVWTreeItem(lpDomData, dwCurSel) &&
            CurItemObjType != OT_INDEX && CurItemObjType != OT_STATIC_INDEX)) ||
            (!IsVWTreeItem(lpDomData, dwCurSel))))
        ...
        if (lpRecord==0 || IsNoItem(CurItemObjType, lpRecord->objName))
          ...
          && (!IsVW() || !IsVWTreeItem(lpDomData, dwCurSel)))
      ...
      if (lpDomData->bIsVectorWise && (CurItemObjType == OT_TABLE ||
		...
		CurItemObjType == OT_INDEX ||
		CurItemObjType == OT_STATIC_INDEX))
      {
	    char vw_struct[MAXOBJECTNAME];
		LoadString(hResource, IDS_STRUCT_VWIX, vw_struct, sizeof(vw_struct));
		...
		/* in VW installs disable backup / restore for VW tables */
		if ( ((CurItemObjType == OT_TABLE) && IsVWTreeItem(lpDomData, dwCurSel))
		/* and for all tree items in VW version 1.5 except ingres tables */
		|| ( (GetVWVers() <= VWVERS_15) && !((CurItemObjType == OT_TABLE) && !IsVWTreeItem(lpDomData, dwCurSel))) )
		{
		...
		/* the following items will show up for everything except VW tables */
		if (!IsVWTreeItem(lpDomData, dwCurSel))
		{
      ...
      // audit, genstat... : only on database direct child items
        ...
        /* in VW installs disable for ... */
        if (IsVW() && (
        /* OT_STATIC_TABLE */
        (CurItemObjType == OT_STATIC_TABLE)
        /* VW tables */
        || ( (CurItemObjType == OT_TABLE) && IsVWTreeItem(lpDomData, dwCurSel))
        /* all tree items in VW version 1.5 except ingres tables */
        || ( (GetVWVers() <= VWVERS_15) && !((CurItemObjType == OT_TABLE) && !IsVWTreeItem(lpDomData, dwCurSel))) ) )	
	{
		EnableItemAndBtn(hMenu, lpDomData, IDM_CHECKPOINT ,MF_GRAYED);
		EnableItemAndBtn(hMenu, lpDomData, IDM_ROLLFORWARD,MF_GRAYED);
	...
	else
	{
	  ...
	  //This was created for version 1.0 but still works in 1.5, leaving it for now
	  //to disable uncomment statement below.
      if (IsVW() /*&& GetVWVers() < VWVERS_15*/)
      {
         if (lpRecord && (getint(lpRecord->szComplim + STEPSMALLOBJ + STEPSMALLOBJ) > 0) &&
			 CurItemObjType != OT_INDEX && CurItemObjType != OT_STATIC_INDEX)
...
static void DOM_PrintHeader(HDC hDCPrinter,LPSTR lpTitle,POINT ptPageSize,POINT ptFontSize,UINT uiPage)
{
  ...
  if (GetTextExtentPoint32(hDCPrinter, (LPCTSTR)work, (int)x_strlen (work), &xSize))
  {
    // UKS modifies March 4' 1996 >>
    int xWidth; 
...
int ManageExpandBranch(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  return Task_ManageExpandBranchInterruptible(hwndMdi, lpDomData, dwItem, hWndTaskDlg);
}
  
int ManageExpandBranch_MT(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  DWORD_PTR dwCurItem;
  int   objType;
  int   childType;
  DWORD_PTR dwFirstItem;
  DWORD_PTR dwNextItem;
  DWORD dwParentLevel;
...
int ManageForceRefreshCursub(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  return Task_ManageForceRefreshCursubInterruptible(hwndMdi, lpDomData, dwItem, hWndTaskDlg);
}

int ManageForceRefreshCursub_MT(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  DWORD_PTR         dwCurItem;
  int           objType;
  DWORD_PTR         dwFirstItem;
