...
**   In DOMGetNextObject return grantee type in lpresultextrastring if 
**   specified at grant time.
**   In DOMGetFirstRelObject compare grantee type passed in lpresultextradata 
**   to grantee type for each grant if present.
**   We don't want to associate grants to non-existing
**   groups/roles with existing users of the same name.
**   In DOMGetObject if grantee type has been passed in presulttype use it
**   to find the grantee rather then determinig the type on the fly.
******************************************************************************/
         ...
         case OT_SEQUGRANT_NEXT_USER :
            {
               ...
               if (lpresultextrastring)
				   storeint(lpresultextrastring , pRawGrantee->GranteeUGRType);
...

{
   ...
   if (lpresultextradata)
	   userobjtype = getint(lpresultextradata);
   nbobj=0;
               ...
               if (dbtype!=DBTYPE_DISTRIBUTED) {
                  ...
                  while (ires1==RES_SUCCESS) {
                     ...
                     while (ires2==RES_SUCCESS) {
                        if (!x_strcmp(parentstrings[0],buf)) {
                          /* check that user type matches retrieved object */
                          int extraint = 0;
                          if (extradata)
                            extraint = getint(extradata);
                          if ( userobjtype == 0 || extraint == 0 || extraint == userobjtype  )
                          {
                           fstrncpy(pLocalCurInfos->ObjectName,resultobjname,
                              MAXOBJECTNAME);
                           fstrncpy(pLocalCurInfos->ObjectOwner,resultobjowner,
      ...
      case OT_GRANTEE:
         {
            ...
            if ((*presulttype == OT_USER) || (*presulttype == OT_GROUP) || (*presulttype == OT_ROLE)) {
              ires=DOMGetObject(hnodestruct,lpobjectname,lpobjectowner,*presulttype,level,
