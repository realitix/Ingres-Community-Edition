...
** 25-Feb-2011 (drivi01) Bug 125106
**   Fixed a hang in WaitForBuffer function due to
**   signed/unsigned type mismatch.  lbufferSize in
**   WaitForBuffer is expected to be positive and 
**   negative, however SIZE_TYPE is of type size_t.
**   When lSize variable is negative being passed
**   to function that only accepts unsigned size_t
**   the negative number turned into a really big
**   positive number.
**   This change corrects signed/unsigned typing
**   all over dataldr to make sure there's no mismatches.
*/
...
typedef struct tagFILEBUFFER
{
	...
	size_t   nS;    /* Writting size of buffer, 0: empty */
	size_t   nR;    /* Reading  size of buffer, nS-nR = 0: no more data to read */
...
typedef struct tagLOBFILEMGT
{
	...
	SIZE_TYPE nReadCount;        /* The amount of bytes read from the buffer file 'pFileBuffer'*/
	SIZE_TYPE nWriteCount;       /* The amount of bytes written to the buffer file 'pFileBuffer'*/
...
static SIZE_TYPE nCbCopyBlock = 0;
...
int CIRCULARBUFFER_Read (CIRCULARBUFFER* pObj, SSIZE_TYPE nSize, SIZE_TYPE* lRead, char* pBuffer)
{
	SSIZE_TYPE v, n2Read = 0;
	SSIZE_TYPE k = nSize;
	char* pSource = NULL;
	char* pDest = pBuffer;
	FILEBUFFER* pFirst = pObj->pHead;
...
void CIRCULARBUFFER_Write (CIRCULARBUFFER* pObj, SSIZE_TYPE nSize, char* pBuffer)
{
	SSIZE_TYPE v, n2Write = 0;
	SSIZE_TYPE k = nSize;
	char* pDest = NULL;
...
int LOBFILEMGT_Read  (LOBFILEMGT* pLobMgt, SSIZE_TYPE nCount, SIZE_TYPE* lRead, char* pBuffer)
{
	int lRes = CIRCULARBUFFER_Read (&(pLobMgt->circularBuff), nCount, lRead, pBuffer);
	return lRes;
}

void LOBFILEMGT_Write (LOBFILEMGT* pLobMgt, SIZE_TYPE nCount, char* pBuffer, i4* dummy)
{
	CIRCULARBUFFER_Write (&(pLobMgt->circularBuff), nCount, pBuffer);
}

void LOBFILEMGT_SIfseek(LOBFILEMGT* pLobMgt, SIZE_TYPE nOffset, i4 nMode)
{
}
...
*/
SSIZE_TYPE LOBFILEMGT_FillBuffer (LOBFILEMGT* pLobMgt, char* lpRow, SSIZE_TYPE nlpRowBufferSize)
{
	STATUS   st;
	LOCATION lf;
	int  nLenSpec, nExit;
...
#endif
	i4 dummy = 0;
	SSIZE_TYPE lByteWindow = nlpRowBufferSize;
	FIELDSTRUCT* pFieldStruct = &(g_dataLoaderStruct.fields);
...
void DTSTRUCT_SetFieldTerminator  (DTSTRUCT* pDtStructure, char* strTerminator)
{
	...
	size_t nLen = STlength(strTerminator);
	if (nLen == 2)
	{
		if (strTerminator[0] == '\\')
		{
			...
			if (nQuotLen && pch && *pch==strQuoting[0]) { /* quoted string */
				...
				*pInField=EOS;
				...
				else
				{
					...
					{
						if (STbcompare(strTeminator,(i4)STlen(strTeminator),pch,0,0)==0) {
							for (i=0; i<nSepLen; i++)
								CMnext (pch);
							*pCur = pch;
						}
		...
		{ /* column has started with quoting chars */
			while (1)
			{
				...
				if (*p == strQuoting[0]) {
					...
					if (stricmp (strTeminator, "WHITESPACE") != 0) /* normal (non whitespace) separator */
					{
						if (STbcompare(strTeminator,(i4)STlen(strTeminator),p,0,0)==0) {
							for (i=0; i<nSepLen; i++)
								CMnext (p);
							break;
						}
...
{
	char szField[DT_SI_MAX_TXT_REC];
	SIZE_TYPE  nSize = STlength(szRecBuffer) + nFieldCount*5 + 2 + GetBufferSize4Sequence(SeqCount);
				...
				*/
				...
				else
				{
					...
					*/
					...
					size_t nLenFillerCol = STlength(listField->szColumnName);
					size_t nLenFillerVal = STlength(szField);
...
*/
		...
		if (pFiller)
		{
			...
			*/
			size_t i, nFillerSize = STlength(pFiller);
...
STATUS CachedSIgetrec(char *  p, SSIZE_TYPE n, FILE *  f)
{
	i4 lRead; 
	STATUS sr;
	i4 lToRead;
...
STATUS SIgetrec0(char *  p, SSIZE_TYPE n, FILE *  f, int* nError, char *pNullSubstChar, i4* pnNullSubst)
{
...
static STATUS mySIgetrec (int iCanHaveMultipleLines, DTSTRUCT*pDataStructure, char * szRecBuffer, SSIZE_TYPE buflen, SSIZE_TYPE n, FILE *f, int* nError, i4* pnNullSubst)
{
	STATUS status ;
	int error = 0;
	size_t ilen;
...
static void ftThreadReadInputFile(DTSTRUCT* pDataStructure)
{
	...
	*/
	...
	if (pInfile)
	{
		...
		SSIZE_TYPE n = DT_SI_MAX_TXT_REC;
...
*/
SSIZE_TYPE GetRecordLine (SSIZE_TYPE* rowLength, char* lpRow, SSIZE_TYPE* returnByte)
{
	int   nAddedNL  = 0;
	int   nFinished = 0;
	char* strRecord = NULL;
	...
	*/
	...
	{
		SSIZE_TYPE lRequest = *rowLength;
		SSIZE_TYPE lReturnBytes = LOBFILEMGT_FillBuffer (pLobfileManageMent, lpRow, lRequest);
		*returnByte = 0;
		if (lReturnBytes == -1)
		{
			g_nCriticalError = 1;
...
void Write2Log (FILE* f, char* strLine)
{
	...
	{
		SIwrite((i4)len, strLine, &count, f);
