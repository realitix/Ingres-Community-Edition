...
static BOOL OnCreate (HWND hwnd, LPCREATESTRUCT lpCreate)
{
	...
	lpList = (LPCATOLIST)GetWindowLongPtr(hwnd, GWL_LPLIST);
...
static BOOL OnNCCreate(HWND hwnd, LPCREATESTRUCT lpCreate)
{
	...
	SetWindowLongPtr(hwnd, GWL_LPLIST, (LONG)lpList);
...
static void OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	LPCATOLIST lpList = (LPCATOLIST)GetWindowLongPtr(hwnd, GWL_LPLIST);
	UINT uNotify;
		...
		case VK_SPACE:
		{
         ...
         if (nCount != LB_ERR)
         {
            ...
            HWND hwndCtl = GetParent(hwnd);
         	...
         	lpList = (LPCATOLIST)GetWindowLongPtr(hwndCtl, GWL_LPLIST);
            ASSERT(lpList);
...
static void LBOnLButtonDown(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
   ...
   {
      HWND hwndCtl = GetParent(hwnd);
   	LPCATOLIST lpList = (LPCATOLIST)GetWindowLongPtr(hwndCtl, GWL_LPLIST);
      ASSERT(lpList);
   	ForwardCALBNotification(lpList->hWndAssociate, hwndCtl, CALBN_CHECKCHANGE);
   }
}
...
LRESULT WINAPI __export LBDefWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	LPCATOLIST lpList = (LPCATOLIST)GetWindowLongPtr(GetParent(hwnd), GWL_LPLIST);
