...
static BOOL NextWord (const CString& strText, size_t index, LPCTSTR lpszWord)
{
	TCHAR* tchString = (TCHAR*)(LPCTSTR)strText;
	int nLen  = strText.GetLength();
	size_t nWLen = _tcslen (lpszWord);
	if (index > 0)
	{
	...
	if ((index + nWLen) <= nLen)
	{
		CString strExtract = strText.Mid ((int)index, (int)nWLen);
		if (strExtract.CompareNoCase (lpszWord) == 0)
		{
			...
			{
				TCHAR tchszC = strText.GetAt ((int)(index + nWLen));
				switch (tchszC)
				{
				case _T(' '):
				case _T(';'):
...
// Caller must ensure that the position i is always the position of a charactter
// ie leading byte.
static void MarkCommentStart (const CString& strText, size_t i, int& nCommentStart)
{
...
// Caller must ensure that the position i is always the position of a charactter
// ie leading byte.
static void MarkCommentEnd (const CString& strText, size_t i, int& nCommentEnd)
{
	int nBytes = sizeof (TCHAR);
	TCHAR* tchString = (TCHAR*)(LPCTSTR)strText;
	if ((i+nBytes) < strText.GetLength())
...
static int Find (const CString& strText, TCHAR chFind, int nStart, BOOL bIgnoreInsideBeginEnd)
{
	...
	if (strText.IsEmpty())
		return -1;
	size_t i, nLen = strText.GetLength();
	...
	for ( i=nStart; i<nLen; i+= _tclen((const TCHAR *)strText + i) )
	{
		...
		if (cChar == _T('\'') && !bDQuote)
		{
			...
			size_t iNext = i + _tclen((const TCHAR *)strText + i);
			if (iNext < nLen && strText.GetAt ((int)iNext) != _T('\''))
			{
				bSQuote = !bSQuote;
				continue;
			}
