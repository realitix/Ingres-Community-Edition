...
**  24-Feb-2010 (kiria01)
**	Clean down any stale data from prior runs.
...
#define SET_COM_PATH_CMD 4	/* (cmd to be modified before use) */
    ...
    */
    ...
    exec sql delete from remotecmdin rci
	      where exists(select * from remotecmd rc
	       where rci.handle=rc.handle AND
		rc.status = :status1);
    exec sql delete from remotecmdout rco
	      where exists(select * from remotecmd rc
	       where rco.handle=rc.handle AND
		rc.status = :status1);
    exec sql delete from remotecmd rc
	       where rc.status = :status1;
