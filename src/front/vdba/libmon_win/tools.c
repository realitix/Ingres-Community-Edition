...
UCHAR *pu;
{
	size_t iprevlen = STlength(pu);
	size_t inewlen  = STtrmwhite(pu);
	if  (inewlen<iprevlen)
		return TRUE;
	else
		return FALSE;
...
char * x_strncat( char *pdest, const char *psrc, size_t n )
{
...
{
   size_t len=x_strlen(lpString2Find);
   while(*lpstring !=EOS) {
...
int x_strnicmp( const char *str1, const char *str2, size_t len)
{
	int ires;
	while ( len > 1 ) {
		if (*str1 == EOS) {
...
int x_strncmp ( const char *str1, const char *str2, size_t len)
{
	int ires;
	while ( len > 1 ) {
		if (*str1 == EOS) {
