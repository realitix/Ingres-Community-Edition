...
BOOL Vdba_WriteBufferInFile(HFILE hdl,char *buftmp,size_t buflen)
{
	UINT uiresult;
	if ((VOID *)hdl == NULL || buflen == 0)
		return FALSE;
	
	//check that the buflen doesn't exceed the maximum UINT length
	if (buflen > (UINT)buflen)
		return FALSE;
	
	uiresult =_lwrite(hdl, buftmp, (UINT)buflen);
