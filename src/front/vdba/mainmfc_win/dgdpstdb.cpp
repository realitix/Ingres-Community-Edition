...
void CuDlgDomPropStaticDb::AddDatabase (CuNameWithOwner *pDatabase, int dbType)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pDatabase);
}
...
LRESULT CuDlgDomPropStaticDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropStaticDb::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropStaticDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropStaticDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropStaticDb::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaStaticDb.GetCount();
