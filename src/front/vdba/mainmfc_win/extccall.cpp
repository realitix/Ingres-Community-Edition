...
extern "C" INT_PTR VDBA_CreateIndex(HWND hwndParent, DMLCREATESTRUCT* pCr)
{
...
INT_PTR Notify_MT_Action(WPARAM wParam, LPARAM lParam)
{
...
LRESULT ManageBaseListAction(WPARAM wParam, LPARAM lParam, BOOL *bActionComplete)
{
	LRESULT lResult = 0L;
	DBAThreadEnableCurrentSession();
	switch ((int)wParam) {
		case ACTION_DOMUPDATEDISPLAYDATA:
			{
				...
				lResult = (LRESULT) DOMUpdateDisplayData_MT (
									pdata->hnodestruct,
									pdata->iobjecttype,
									pdata->level,
									pdata->parentstrings,
...
//UKS				lResult = (LRESULT) RefreshPropWindows_MT(pdata->bOnLoad,pdata->refreshtime);
			}
			break;
		case ACTION_MESSAGEBOX:
			{
				MESSAGEBOXPARAMS * pdata = (MESSAGEBOXPARAMS *) lParam;
				lResult = (LRESULT) ::MessageBox(pdata->hWnd,pdata->pTxt,pdata->pTitle,pdata->uType);
			}
			break;
		case ACTION_GETFOCUS:
			lResult = (LRESULT)GetFocus();
			break;
...
// Expand branch
//
int Task_ManageExpandBranchInterruptible(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  //
  // If "NOT USE ANIMATION" or "ANIMATION IS ALREADY RUNNING":
  if (!theApp.IsAnimateEnabled() || theApp.IsDlgWaitRunning())
...
//
int Task_ManageForceRefreshCursubInterruptible(HWND hwndMdi, LPDOMDATA lpDomData, DWORD_PTR dwItem, HWND hWndTaskDlg)
{
  //
  // If "NOT USE ANIMATION" or "ANIMATION IS ALREADY RUNNING":
  if (!theApp.IsAnimateEnabled() || theApp.IsDlgWaitRunning())
...
extern "C" int VDBA_ChooseIndex(HWND hwndParent, LPCTSTR lpszDatabase, LPCHECKEDOBJECTS* lpListObjects)
{
	...
	try
	{
		...
		nSize = (int)arraySelectedObj.GetSize();
		for (i=0; i<nSize; i++)
		{
