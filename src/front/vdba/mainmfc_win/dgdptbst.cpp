...
LRESULT CuDlgDomPropTableStatistic::OnUpdateQuery (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropTableStatistic::OnChangeProperty (WPARAM wParam, LPARAM lParam)
{
	if ((int)lParam == 0)
	{
		//
...
BOOL CuDlgDomPropTableStatistic::OnInitDialog() 
{
	...
	LONG_PTR style = GetWindowLongPtr (m_cListColumn.m_hWnd, GWL_STYLE);
	style |= LVS_SHOWSELALWAYS;
	SetWindowLongPtr (m_cListColumn.m_hWnd, GWL_STYLE, style);
...
LRESULT CuDlgDomPropTableStatistic::OnQueryType(WPARAM wParam, LPARAM lParam)
{
	ASSERT (wParam == 0);
	ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropTableStatistic::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropTableStatistic::OnLoad (WPARAM wParam, LPARAM lParam)
{
	int nCount = 0;
	try
	{
		...
		while (!m_statisticData.m_listColumn.IsEmpty())
		{
			...
			{
				m_cListColumn.SetItemData (nCount, (DWORD_PTR)pColumn);
				nCount++;
			}
		}
		UpdateDisplayList1();
...
LRESULT CuDlgDomPropTableStatistic::OnGetData (WPARAM wParam, LPARAM lParam)
{
