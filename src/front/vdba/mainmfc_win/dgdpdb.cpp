...
void CuDlgDomPropDb::AddAltLoc (CuAltLoc* pAltLoc)
{
  ...
  m_cListAlternateLocs.SetItemData(index, (DWORD_PTR)pAltLoc);
}
...
LRESULT CuDlgDomPropDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropDb::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDb::RefreshDisplay()
{
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    DWORD_PTR dwtype = m_CheckFrontEndList.GetItemData(i);
  ...
  int size = (int)m_Data.m_uaAltLocs.GetCount();
  ASSERT (size > 0);
