...
**  19-Mar-2009 (drivi01)
**    In effort to port to Visual Studio 2008, replace depricated POSIX 
**    ltoa with ISO C++ _ltoa. 
** 13-Dec-2010 (drivi01) 
...
LRESULT CuDlgDomPropTblPages::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropTblPages::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropTblPages::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropTblPages::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropTblPages::OnSelchangeComboRatio() 
{
  ...
  ASSERT (index != CB_ERR);
  long lCurPgPerCell = (long)m_cbRatio.GetItemData(index);
...
//			pData->AddOccupant (_T("B4:"), _T("S06"), v2);
		...
		m_strDataBucket        = _ltoa(pPageInfo->lbuckets_no_ovf        ,buffer,10);
		m_strEmptyBucket       = _ltoa(pPageInfo->lemptybuckets_no_ovf   ,buffer,10);
		m_strDataWithOverflow  = _ltoa(pPageInfo->lbuck_with_ovf         ,buffer,10);
		m_strEmptyWithOverflow = _ltoa(pPageInfo->lemptybuckets_with_ovf ,buffer,10);
		m_strOverflow          = _ltoa(pPageInfo->loverflow_not_empty    ,buffer,10);
		m_strEmptyOverflow     = _ltoa(pPageInfo->loverflow_empty        ,buffer,10);
		m_strNeverUsed         = _ltoa(pPageInfo->lneverused             ,buffer,10);
		m_strFree              = _ltoa(pPageInfo->lfreed                 ,buffer,10);
		m_strOthers            = _ltoa(lothers                           ,buffer,10);
		if (pPageInfo->llongest_ovf_chain<0L)
			m_strLongestOverflowChain.LoadString (IDS_NOT_AVAILABLE);//_T("N/A")
		else
			m_strLongestOverflowChain = _ltoa(pPageInfo->llongest_ovf_chain  ,buffer,10);
		...
		m_strTotal =_ltoa(pPageInfo->ltotal,buffer,10);
	}
	else
	{
		if (m_pPieCtrlHashTable && IsWindow (m_pPieCtrlHashTable->m_hWnd))
