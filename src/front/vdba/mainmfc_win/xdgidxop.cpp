...
BOOL CxDlgIndexOption::OnInitDialog() 
{
	...
	while (pos != NULL)
	{
		...
		if (idx != CB_ERR)
			m_cComboStructure.SetItemData (idx, (DWORD_PTR)pStruct);
	}
	if (!pIndex->bAlter)
	{
		int nItemData = 0;
	...
	if (!pTable->primaryKey.bAlter && StringList_Count (pTable->primaryKey.pListKey) > 0)
	{
		    ...
		    pTable->primaryKey.constraintWithClause.tchszIndexName[0] != _T('$'))
		{
			...
			if (nIdx == CB_ERR)
			{
				...
				if (nIdx != CB_ERR)
					m_cComboIndexName.SetItemData (nIdx, (DWORD_PTR)&(pTable->primaryKey.constraintWithClause));
			}
		}
	}
	//
		...
		if (StringList_Count (ls->lpcolumnlist) > 0 && !ls->bAlter)
		{
			...
			if (ls->constraintWithClause.tchszIndexName[0] && ls->constraintWithClause.tchszIndexName[0] != _T('$'))
			{
				...
				if (nIdx == CB_ERR)
				{
					...
					if (nIdx != CB_ERR)
						m_cComboIndexName.SetItemData (nIdx, (DWORD_PTR)&(ls->constraintWithClause));
				}
			}
		}
		ls = ls->next;
		...
		if (!lpRef->bAlter)
		{
			if (lpRef->constraintWithClause.tchszIndexName[0] && lpRef->constraintWithClause.tchszIndexName[0] != _T('$'))
			{
				...
				if (nIdx == CB_ERR)
				{
					...
					if (nIdx != CB_ERR)
						m_cComboIndexName.SetItemData (nIdx, (DWORD_PTR)&(lpRef->constraintWithClause));
				}
			}
		}
...
void CxDlgIndexOption::EnableOKButton()
{
	...
	{
		DWORD_PTR nData = m_cComboIndex.GetItemData (nSel);
		switch (nData)
		{
		case INDEXOPTION_USEDEFAULT:
		case INDEXOPTION_NOINDEX:
