...
**   In IsSimilar() return FALSE if Grantee type of the 2 objects
**   is different. We don't want to associate grants to non-existing
...
BOOL CuGrantee::IsSimilar(CuMultFlag* pSearchedObject)
{
  ...
  if (GetStrName() == pSearchedGrantee->GetStrName()) {
    if (GetGranteeType() == pSearchedGrantee->GetGranteeType())
      return TRUE;
	else 
      return FALSE;
  }
