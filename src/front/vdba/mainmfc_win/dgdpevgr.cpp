...
**   If specified at grant time grantee types are now already retrieved
**   into bufComplim during DOMGetFirstObject. If not specified they 
**   will be solved on the fly via DOMGetObjectLimitedInfo.
*/
...
void CuDlgDomPropDbEventGrantees::AddDbEventGrantee (CuDbEventGrantee* pDbEventGrantee)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pDbEventGrantee);
}
...
LRESULT CuDlgDomPropDbEventGrantees::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropDbEventGrantees::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
    ...
    iret =  DOMGetFirstObject(nNodeHandle,
     ...
     if (iret != RES_SUCCESS && iret != RES_ENDOFDATA) {
      ...
      while (iret == RES_SUCCESS) {
		...
		/* solve grantee type on the fly unless already present because
		it has been specified in the grant's text segment */
		int granteeType=getint(bufComplim);
	    if ((granteeType == OT_USER) || (granteeType == OT_ROLE) ||
			(granteeType == OT_GROUP))
		{
...
LRESULT CuDlgDomPropDbEventGrantees::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDbEventGrantees::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDbEventGrantees::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaDbEventGrantees.GetCount();
  ASSERT (size > 0);
