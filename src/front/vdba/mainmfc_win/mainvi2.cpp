...
** 01-Apr-2011 (drivi01)
**    For IVW release 1.5, add support for index, which
**    is more like traditional Ingres index.
**    Add Index Property Page to table properties, and
**    update properties for VectorWise Index to display only
**    2 tabs: detail and statistics.
*/
...
void CMainMfcView2::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
  ...
  if (m_pDomTabDialog) {
    ...
    ASSERT (hwndDoc);
    LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndDoc, 0);
    ASSERT (lpDomData);
...
extern "C" void FreeDomPageInfo(HWND hwndDom, DWORD_PTR m_pPageInfo)
{
  CWnd *pWnd;
...
CuDomPageInformation* GetDomItemPageInfo(int iobjecttype, LPTREERECORD pItemData, int ingresVer, int hDomNode)
{
  if (pItemData->bPageInfoCreated) {
    ...
    if (pPageInfo)
      pItemData->m_pPageInfo = (DWORD_PTR)pPageInfo;
...
static CuDomPageInformation* AllocateIVWTablePageInfo(BOOL bRowsNotDisplayable)
{
  ...
  if (bRowsNotDisplayable) {
    nTabID[2] = IDS_DOMPROP_TABLE_ROWSNA;
    nDlgID[2] = IDD_DOMPROP_TABLE_ROWSNA;
    pPageInfo = new CuDomPageInformation ((LPCTSTR)"CuDomVWTableNoRows", 6, nTabID, nDlgID, nIDTitle);
  }
  else
    pPageInfo = new CuDomPageInformation ((LPCTSTR)"CuDomVWTable", 6, nTabID, nDlgID, nIDTitle);
  return pPageInfo;
}
...
static CuDomPageInformation* AllocateIVWIndexPageInfo()
{
  CuDomPageInformation *pPageInfo = NULL;

  UINT nTabID [2] = {IDS_DOMPROP_INDEX, IDS_DOMPROP_TABLE_STATISTIC};
  // IDD_DOMPROP_INDEX_PAGES becomes obsolete
  // The index uses the same IDD_DOMPROP_TABLE_STATISTIC
  UINT nDlgID [2] = {IDD_DOMPROP_INDEX, IDD_DOMPROP_TABLE_STATISTIC};

  UINT nIDTitle = IDS_DOMPROP_INDEX_TITLE;
  pPageInfo = new CuDomPageInformation (_T("CuDomIndex"), 2, nTabID, nDlgID, nIDTitle);
  return pPageInfo;
}
...
static CuDomPageInformation* AllocateIndexPageInfo(LPTREERECORD pItemData, int ingresVer)
{
  ...
  else if ((vw_struct.LoadString(IDS_STRUCT_VWIX) && !stricmp((char *)pItemData->szComplim,  vw_struct.GetBuffer())) 
	  || (vw_struct.LoadString(IDS_STRUCT_VW) && !stricmp((char *)pItemData->szComplim, vw_struct.GetBuffer()))
	  || (vw_struct.LoadString(IDS_STRUCT_VWROW) && !stricmp((char *)pItemData->szComplim, vw_struct.GetBuffer())))
	  return AllocateIVWIndexPageInfo();
  else
...
LRESULT CMainMfcView2::OnQueryOpenCursor(WPARAM wParam, LPARAM lParam)
{
	BOOL bExit = FALSE;
	if (!m_pDomTabDialog)
		return bExit;
