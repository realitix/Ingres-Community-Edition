...
static void NotifyUpdateDBEvent (INT_PTR nHnode, LPCTSTR lpszDBName = NULL);
...
static BOOL AutomatedDomDoc(CTreeItemNodes* pSelectedItem, CuWindowDesc* pDesc)
{
  ...
  ASSERT (pActiveMDI);
  INT_PTR type = QueryDocType(pActiveMDI->m_hWnd);
...
static void NotifyUpdateDBEvent (INT_PTR nHnode, LPCTSTR lpszDBName)
{
    CDocument*   pDoc;
