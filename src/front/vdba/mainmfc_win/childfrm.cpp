...
**  11-Jan-2011 (shust01)
**    An exception occurs when opening DOM scratchpad. Bug 124912.
** 28-Jan-2011 (drivi01) 
**    Port the solution to x64. Clean up the warnings.
**    Clean up datatypes. Port function calls to x64.
** 11-Apr-2011 (drivi01)
**    Update File level menus to use IsVWTreeItem to determine
**    which menus should be enabled or disabled for VectorWise
**    DOM tree items.
** 01-Nov-2011 (frima01) b125894
**    Enable ckpdb / rollforwarddb for Ingres VectorWise > version
...
extern "C" void TS_MfcUpdateRightPane(HWND hwndDoc, LPDOMDATA lpDomData, BOOL bCurSelStatic, DWORD_PTR dwCurSel, int CurItemObjType, LPTREERECORD lpRecord, BOOL bHasProperties, BOOL bUpdate, int initialSel, BOOL bClear)
{
...
extern "C" BOOL RefreshPropWindows_MT(BOOL bOnLoad, time_t refreshtime)
{
  ...
  while (hwndCurDoc) {
    ...
    if (QueryDocType(hwndCurDoc) == TYPEDOC_DOM) {
      ...
      LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndMfcDoc, 0);
      ASSERT (lpDomData);
...
LRESULT CChildFrame::OnHasToolbar(WPARAM wParam, LPARAM lParam)
{
  return (LRESULT)TRUE;
}

LRESULT CChildFrame::OnIsToolbarVisible(WPARAM wParam, LPARAM lParam)
{
  return (LRESULT)m_wndToolBar.IsVisible();
}

LRESULT CChildFrame::OnSetToolbarState(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CChildFrame::OnSetToolbarCaption(WPARAM wParam, LPARAM lParam)
{
...
LPDOMDATA GetLPDomData(CWnd* pWnd)
{
  ...
  ASSERT (hwndMdi);
  LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndMdi, 0);
  ASSERT (lpDomData);
  return lpDomData;
}
...
void CChildFrame::OnUpdateButtonAddobject(CCmdUI* pCmdUI) 
{
  ...
  if (dwCurSel==0 || CurItemObjType == OT_STATIC_R_GRANT
    ...
    if (bCurSelStatic) {
      DWORD_PTR dwFirstChild = GetFirstImmediateChild(lpDomData, dwCurSel, 0);
...
void CChildFrame::OnUpdateButtonDropobject(CCmdUI* pCmdUI) 
{
  ...
  if (lpRecord && lpRecord->parentDbType != DBTYPE_COORDINATOR &&
	  IsVW() && IsVWTreeItem(lpDomData, dwCurSel) && CurItemObjType == OT_INDEX)
	  bEnable = FALSE;
...
void CChildFrame::OnUpdateButtonCheckpoint(CCmdUI* pCmdUI) 
{
  ...
  DWORD_PTR dwCurSel = GetCurSel(lpDomData);
  int   CurItemObjType = GetItemObjType(lpDomData, dwCurSel);
  BOOL   IsVWTrItem = IsVWTreeItem(lpDomData, dwCurSel);
  ...
  if (IsVW() && (
	    ...
	    /* VW tables */
		|| ( (CurItemObjType == OT_TABLE) && IsVWTrItem)
		/* all tree items in VW version 1.5 except ingres tables */
		|| ( (GetVWVers() <= VWVERS_15) && !((CurItemObjType == OT_TABLE) && !IsVWTrItem)) ) )
	  bEnable = FALSE;
...
DWORD_PTR CChildFrame::GetTreeCurSel()
{
  LPDOMDATA lpDomData = GetLPDomData(this);
  ASSERT (lpDomData);
  DWORD_PTR dwCurSel = GetCurSel(lpDomData);
...
LRESULT CChildFrame::FindSearchedItem(CuDomTreeFastItem* pFastItem, DWORD_PTR dwZeSel)
{
...
void CChildFrame::SelectItem(DWORD_PTR dwZeSel)
{
  LPDOMDATA lpDomData = GetLPDomData(this);
  ASSERT (lpDomData);
...
void TS_DOMRightPaneUpdateBitmap(HWND hwndMdi, LPVOID lpDomData, DWORD_PTR dwCurSel, LPVOID lpRecord)
{
