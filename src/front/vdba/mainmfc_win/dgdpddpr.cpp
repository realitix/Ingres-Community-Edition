...
void CuDlgDomPropDDbProc::AddProc (CuNameWithOwner *pDDbProc)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pDDbProc);
}
...
LRESULT CuDlgDomPropDDbProc::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropDDbProc::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDDbProc::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDDbProc::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDDbProc::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaDbProc.GetCount();
