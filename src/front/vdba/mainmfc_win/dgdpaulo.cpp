...
LRESULT CuDlgDomPropInstallationLevelAuditLog::OnQueryType(WPARAM wParam, LPARAM lParam)
{
	ASSERT (wParam == 0);
	ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropInstallationLevelAuditLog::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
	CString strMsg;
...
LRESULT CuDlgDomPropInstallationLevelAuditLog::OnLoad (WPARAM wParam, LPARAM lParam)
{
	int nIdx;
	CString strItem;
	...
	try
	{
		...
		while (!pData->m_listObjectName.IsEmpty())
		{
			...
			if (nIdx != CB_ERR)
				m_cComboObjectName.SetItemData (nIdx, (DWORD_PTR)pObj);
		}
		//
		// User:
...
LRESULT CuDlgDomPropInstallationLevelAuditLog::OnGetData (WPARAM wParam, LPARAM lParam)
{
	int i, nCount;
	CString strItem;
...
BOOL CuDlgDomPropInstallationLevelAuditLog::ComboObjectName_Requery(LPCTSTR lpszDatabase, int nObjectType)
{
	...
	while (ires == RES_SUCCESS)
	{
		...
		if (idx != CB_ERR && tchszOwner[0])
		{
			...
			m_cComboObjectName.SetItemData (idx, (DWORD_PTR)pObj);
		}
		tchszBuf[0] = 0;
		tchszOwner[0] = 0;
		tchszBufComplim[0] = 0;
