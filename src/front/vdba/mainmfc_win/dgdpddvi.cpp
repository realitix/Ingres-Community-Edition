...
void CuDlgDomPropDDbView::AddView (CuNameWithOwner *pDbView)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pDbView);
}
...
LRESULT CuDlgDomPropDDbView::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropDDbView::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDDbView::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropDDbView::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropDDbView::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaDbView.GetCount();
