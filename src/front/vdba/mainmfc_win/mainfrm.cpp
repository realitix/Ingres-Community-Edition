...
** 15-Mar-2011 (drivi01)
**    Add pragma to turn off optimization to fix
**    SEGV in release mode.
**/
...
#else	
		  //OpenDocumentFile is not working on x64, the SEGV is in the system code
		  //on SetTitle.  This workaround achieves the same thing without
		  //problems.
		  pNewDoc = pTemplate->CreateNewDocument();
		  if (pNewDoc) 
		  {
			...
			{
				CString strDocName;
				if (pTemplate->GetDocString(strDocName, CDocTemplate::docName) &&
					!strDocName.IsEmpty())
					pNewDoc->SetTitle(strDocName);
				pNewDoc->OnNewDocument();
				pTemplate->InitialUpdateFrame(pFrame, pNewDoc, TRUE);
			}
		  }
#endif
	  }
...
void CMainFrame::OnUpdateButtonPrint(CCmdUI* pCmdUI) 
{
  ...
  if (!pMdi)
    ...
    INT_PTR type = QueryDocType(pMdi->m_hWnd);
    switch (type) {
...
LRESULT CMainFrame::OnUpdateOpenwin(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CMainFrame::OnActionThreadAnimate(WPARAM wParam, LPARAM lParam)
{
	BOOL bActionComplete;
	LRESULT lresult = ManageBaseListAction(wParam,lParam,&bActionComplete);
	if (bActionComplete)
		return lresult;
