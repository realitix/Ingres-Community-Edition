...
void CuDlgDomTabCtrl::DisplayPage (CuDomPageInformation* pPageInfo, BOOL bUpdate, int initialSel)
{
    ...
    ASSERT (hWndDomDoc);
    LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hWndDomDoc, 0L);
    ASSERT (lpDomData);
    INT_PTR hDomNode = lpDomData->psDomNode->nodeHandle;
    ASSERT (hDomNode != -1);
...
LRESULT CuDlgDomTabCtrl::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  //
...
BOOL CuDlgDomTabCtrl::OnInitDialog() 
{
    ...
    try
    {
        ...
        lpRecord->m_pPageInfo = (DWORD_PTR)pPageInfo;
    }
    catch (CMemoryException* e)
    {
        VDBA_OutOfMemoryMessage ();
...
LRESULT CuDlgDomTabCtrl::OnQueryOpenCursor(WPARAM wParam, LPARAM lParam)
{
	BOOL bExit = FALSE;
	//
