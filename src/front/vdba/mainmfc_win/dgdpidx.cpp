...
void CuDlgDomPropIndex::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_LOCATIONS, m_stLocations);
	DDX_Control(pDX, IDC_STATIC_ALLOCATION, m_stAlloc);
	DDX_Control(pDX, IDC_STATIC_EXTEND, m_stExtend);
	DDX_Control(pDX, IDC_STATIC_FILLFACTOR, m_stFillFact);
	DDX_Control(pDX, IDC_STATIC_ALLOCATEDPAGES, m_stAllocatedPages);
	//}}AFX_DATA_MAP
}
...
LRESULT CuDlgDomPropIndex::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropIndex::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  switch(indexparams.nStructure) {
    ...
    case IDX_RTREE:
      ...
      break;
    case IDX_VWIX:
      m_Data.m_csStorage = "Vectorwise_ix";
      m_Data.m_bIVW = TRUE;
      break;
...
LRESULT CuDlgDomPropIndex::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropIndex::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropIndex::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_acsIdxColumns.GetSize();
  ...
  if (m_Data.m_bEnterprise)
    ...
    size = (int)m_Data.m_acsLocations.GetSize();
    for (cnt = 0; cnt < size; cnt++)
  ...
  if (m_Data.m_bEnterprise) {
    ...
    m_chkCompress.ShowWindow(SW_HIDE);
	  ...
	  m_edAlloc.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_stAlloc.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_edExtend.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_stExtend.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_edAllocatedPages.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_stAllocatedPages.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_edFillFact.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_stFillFact.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_stLocations.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
	  m_lbLocations.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
    ...
    if (m_Data.m_csPgSize == "n/a") {
      ...
      m_chkPersist.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
      m_stPersist.ShowWindow(m_Data.m_bIVW ? SW_HIDE : SW_SHOW);
    }
  }
}
