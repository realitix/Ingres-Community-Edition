...
**   can then avoid confusion with sequences granted to non-existing 
...
void CuDlgDomPropUserXGrantedSeq::AddUserXGrantedSeq (CuGrantedSeq* pGrantedSeq)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pGrantedSeq);
}
...
LRESULT CuDlgDomPropUserXGrantedSeq::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropUserXGrantedSeq::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropUserXGrantedSeq::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropUserXGrantedSeq::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropUserXGrantedSeq::RefreshDisplay()
{
  ...
  m_cListCtrl.DeleteAllItems();
  size = (int)m_Data.m_uaUserXGrantedSeq.GetCount();
  ASSERT (size > 0);
