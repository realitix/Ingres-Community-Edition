   ...
   *pc='\0';
   ...
   switch (mycallbackstate) {
      case 0 :
         ...
         break;
      case 1:
         if (!x_strncmp(CurLine,szMore4Hash,(int)x_strlen(szMore4Hash))) {
            mycallbackstate = 2;
        ...
        //    bcallbackregistered = FALSE;
         ...
         if (!x_strncmp(CurLine,szMore4HashNew_begin,(int)x_strlen(szMore4HashNew_begin)) &&
      ...
      case 3:  // 2 is reserved for "old hash" (not managed), 3 is for Hash for Ingres 2.5 or more 
		if (!x_strncmp(CurLine,szthereare,(int)x_strlen(szthereare))) {
		...
		if (!x_strncmp(CurLine,szlongestovfch,(int)x_strlen(szlongestovfch))) {
		...
		if (!x_strncmp(CurLine,szavgovfchain,(int)x_strlen(szavgovfchain))) {
			favg_ovf_chain = atof(CurLine+x_strlen(szavgovfchain));
			break;
		}

		if (!x_strncmp(CurLine,szDummy1,(int)x_strlen(szDummy1)) ||
			!x_strncmp(CurLine,szDummy2,(int)x_strlen(szDummy2)) ||
			!x_strncmp(CurLine,szDummy3,(int)x_strlen(szDummy3)) ||
			!x_strncmp(CurLine,szDummy4,(int)x_strlen(szDummy4))    ) 
			break;
...
static BOOL Table_GetStatistics1 (CaTableStatisticColumn* pColumn, CaTableStatistic& ts)
{
   ...
   if (!CurLine)
      return FALSE;
   while (CurLine && x_strnicmp(CurLine,szdate,(int)x_strlen(szdate))) 
      CurLine = GetNextSignificantTraceLine();
   if (!CurLine)
      return FALSE;
   ...
   while (CurLine && x_strnicmp(CurLine,szrepet,(int)x_strlen(szrepet))) 
      CurLine = GetNextSignificantTraceLine();
   if (!CurLine)
      return FALSE;
   ...
   *pc1='\0';
   ...
   while ((CurLine = GetNextSignificantTraceLine())!=NULL) {
      ...
      if (x_strnicmp(CurLine,szcell,(int)x_strlen(szcell)) )  
         continue;
