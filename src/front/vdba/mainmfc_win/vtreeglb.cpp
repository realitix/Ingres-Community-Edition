...
void CTreeGlobalData::Serialize (CArchive& ar)
{
  ...
  if (ar.IsStoring()) {
    ...
    ar << (DWORD_PTR)m_hSelectedItem;
    ar << (DWORD_PTR)m_hFirstVisibleItem;
  }
  else {
    DWORD_PTR dw;
    ar >> dw;
...
void CTreeGlobalData::FillTreeFromSerialList()
{
  ...
  while (pos) {
    ...
    if (lineHParentItem) {
      ...
      if (!hTreeItemMap.Lookup((DWORD)lineHParentItem, (DWORD)dwHNewParentItem))   // key, ref. on value
    ...
    m_pTree->SetItemData(hNewItem, (DWORD_PTR)linePItem);
  ...
  DWORD    dwHItem = 0;
  if (m_hSelectedItem) {
    if (!hTreeItemMap.Lookup((DWORD)m_hSelectedItem, (DWORD)dwHItem))
      ASSERT (FALSE);
  ...
  //this may need revisiting HTREEITEM being converted to 4-byte DWORD
  if (!hTreeItemMap.Lookup((DWORD)m_hFirstVisibleItem, (DWORD)dwHItem)) 
    ASSERT (FALSE);
  HTREEITEM hItem = (HTREEITEM)dwHItem;
