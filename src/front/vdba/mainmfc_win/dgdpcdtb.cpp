...
void CuDlgDomPropCddsTables::AddCddsTableCol (CuCddsTableCol* pCddsTableCol)
{
  ...
  m_cListTableCol.SetItemData(index, (DWORD_PTR)pCddsTableCol);
}
...
void CuDlgDomPropCddsTables::AddCddsTable (CuCddsTable* pCddsTable)
{
  ...
  // Attach pointer for future sort purposes
  m_cListTable.SetItemData(index, (DWORD_PTR)pCddsTable);
}
...
LRESULT CuDlgDomPropCddsTables::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
...
LRESULT CuDlgDomPropCddsTables::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
  ...
  int dummySesHndl;
  INT_PTR iResult = GetDetailInfo ((LPUCHAR)GetVirtNodeName(nNodeHandle),
                             replicCddsTableType,
                             &cddsparams,
                             FALSE,
                             &dummySesHndl);
...
LRESULT CuDlgDomPropCddsTables::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropCddsTables::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropCddsTables::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaCddsTables.GetCount();
  ASSERT (size > 0);
