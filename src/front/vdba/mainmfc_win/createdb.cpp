...
void CxDlgCreateAlterDatabase::OnOK() 
{
	...
	else
	{
		...
		if ( nIndex != LB_ERR )
		{
			...
			lCurPgSize = (long)m_pDlgcreateDBAdvanced->m_CtrlCatalogsPageSize.GetItemData(nIndex);
	...
	if ( nCount != 0 )
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			if (m_pDlgcreateDBAdvanced->m_CheckFrontEndList.GetCheck (i))
			{
				...
				DWORD_PTR dwtype = m_pDlgcreateDBAdvanced->m_CheckFrontEndList.GetItemData(i);
