...
void CuDlgDomPropGroupXGrantedDb::AddGroupXGrantedDb (CuGrantedDb* pGrantedDb)
{
  ...
  m_cListCtrl.SetItemData(index, (DWORD_PTR)pGrantedDb);
}
...
LRESULT CuDlgDomPropGroupXGrantedDb::OnQueryType(WPARAM wParam, LPARAM lParam)
{
  ASSERT (wParam == 0);
  ASSERT (lParam == 0);
  return DOMPAGE_LIST;
...
LRESULT CuDlgDomPropGroupXGrantedDb::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropGroupXGrantedDb::OnLoad (WPARAM wParam, LPARAM lParam)
{
...
LRESULT CuDlgDomPropGroupXGrantedDb::OnGetData (WPARAM wParam, LPARAM lParam)
{
...
void CuDlgDomPropGroupXGrantedDb::RefreshDisplay()
{
  ...
  size = (int)m_Data.m_uaGroupXGrantedDb.GetCount();
  ASSERT (size > 0);
