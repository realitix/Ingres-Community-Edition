...
void CuListCtrl::OnInsertitem(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	try
	{
		if (m_bActivateColorItem)
		{
			...
			CListCtrl::SetItemData (pNMListView->iItem, (DWORD_PTR)pData);
		}
	}
	catch (...)
	{
...
BOOL CuListCtrl::SetItemData(int nItem, DWORD_PTR dwData)
{
	if (!m_bActivateColorItem)
...
DWORD_PTR CuListCtrl::GetItemData(int nItem ) const
{
	if (!m_bActivateColorItem)
