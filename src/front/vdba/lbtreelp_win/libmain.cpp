...
#define  MESS(mess) {LinkedListWindow * ll=(LinkedListWindow *)GetWindowLongPtr(hWnd,0); if (ll) return(ll->ON_##mess(wParam,lParam));}
...
extern "C" LRESULT CALLBACK __export WLLWndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
  switch (message)
    ...
    case WM_CREATE:
      ...
      if (ll)
        {
        SetWindowLongPtr(hWnd,0,(LPARAM) (LinkedListWindow FAR *) ll);
    ...
    case WM_DESTROY:
        {
        // UKS adds this line
        LinkedListWindow * ll=(LinkedListWindow *)GetWindowLongPtr(hWnd,0); 
...
extern "C" LBTREE_LIBAPI HWND CALLBACK __export LRCreateWindowPrint (HWND hOwner,LPRECT rcLocation,DWORD dwStyle,BOOL bVODrawStyle)
{
  ...
  hWnd=CreateWindow(pClassName,
                    ...
                    0,
                    ghDllInstance,
                    (void FAR*) (DWORD_PTR) bVODrawStyle);
  return hWnd;
}
