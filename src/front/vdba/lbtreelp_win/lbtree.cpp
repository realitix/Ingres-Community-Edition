...
LinkNonResolved::LinkNonResolved (ULONG_PTR ul,Link * prev,char * p,ULONG_PTR ulParent,void * lpItemData) : Link(ul,0,prev,0,p,lpItemData) 
{ 
  lparentitemdata=ulParent;
  bDeleted=0;
}

Link::Link(ULONG_PTR ID ,Link *pParent,Link * insertAfter,Link * insertBefore,char * pString,void * lpItemData)
{
  parent=pParent;
  litemdata=ID;
  expanded=1;
...
Link *  Linked_List::Add(ULONG_PTR ID,ULONG_PTR parentID,char *pStr, void * lpItemData, ULONG_PTR uiInsertAfter,ULONG_PTR uiInsertBefore)
{
...
void Linked_List::ResolveLink(ULONG_PTR ID)
{
...
Link * Linked_List::Find(ULONG_PTR ID)
{
  Link *lcur;
  Link *l;
...
Link * Linked_List::Find(ULONG_PTR ID,int bSearchNonResolved)
{
  #ifdef DEBUGEMB
  lFind2DebugEmbCount++;
  #endif
