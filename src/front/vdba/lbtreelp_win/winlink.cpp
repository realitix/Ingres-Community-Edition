...
{
  int w1=(int) (DWORD_PTR) _tcsupr(pc);
  while(*p)
    {
    int w2=(int) (DWORD_PTR) _tcsupr(p);
    if (w1==w2)
      return p;
    CMnext(p);
    }
...
static int StrCmpExt(char *pSrc,char *pDst)
{
  while (*pDst)
    ...
    switch(*pSrc)
      ...
      default:
        {
        int w1=(int) (DWORD_PTR) _tcsupr(pSrc);
        int w2=(int) (DWORD_PTR) _tcsupr(pDst);
        if (w1!=w2)
          return (w1-w2);
        }
        break;
...
void LinkedListWindow::VerifyScroll()
{
  ...
  if (GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_AUTOVSCROLL)
...
void LinkedListWindow:: VerifyScrollH()
{
  ...
  if (bEnable || !(GetWindowLongPtr(hWnd, GWL_STYLE) & LMS_AUTOHSCROLL) )
    {
  ...
  if (GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_AUTOHSCROLL)
    {
        ...
        if (!work->strExtent)
          ...
          if (p)
            ...
            if (lpTab && (VODrawStyle || iTabStop))
              ...
              if (VODrawStyle)
                {
                work->strExtent=LOWORD(GetTextExtent(hDC,lpTab,_tcslen(lpTab)))+(size_t)(sizeIndent*3);
                }
              else
                {
                work->strExtent=LOWORD(GetTextExtent(hDC,lpTab,_tcslen(lpTab)))+(size_t)iTabStop;
                }
              }
            else
              {
...
void PaintItem(LPLMDRAWITEMSTRUCT lm,DWORD_PTR ulFlags,HICON hIcon1,HICON hIcon2,int iIconsMargin)
{
  RECT    rcInter;
  ...
  if (!ulFlags&LMDRAWSTRING) 
    ...
    if (lm->lpString)
      {
      int iStrLen=(int)_tcslen(lm->lpString)*sizeof(TCHAR);
//
//RPB - Removed in leiu of the SETICONSPACING message
//      int iIconsMargin = (GetWindowLongPtr(lm->hWndItem,GWL_STYLE)&LMS_WITHICONS) ?  POSITION_TEXT : 0;
...
//              DrawIcon(lm->hDC,rc.left+POSITION_SECOND_ICON,iVertPos,hIcon2);
        ...
        if (lm->iTabStopSize && lpTab)
          ...
          if (IntersectRect(&rcInter,&newRC,&lm->rcPaint))
            ExtTextOut(lm->hDC,newRC.left,newRC.top,ETO_OPAQUE|ETO_CLIPPED,&newRC,lpTab,(int)_tcslen(lpTab),NULL);
          }
...
static void __inline VODrawTabString(HDC hDC,LPRECT lpRC,int iTxtPos,LPSTR lpStr,LPSTR lpTab,BOOL bFocus,int indentSize)
{
  ...
  ExtTextOut(hDC,iTxtPos,rc.top,ETO_OPAQUE|ETO_CLIPPED,&rc,lpStr,(int)(lpTab-lpStr),NULL);
  oldBKMode=SetBkMode(hDC,TRANSPARENT);
  SetTextColor(hDC,bFocus ?  RGB(255,255,255) : RGB(0,0,0));
  ExtTextOut(hDC,iTxtPos+1,rc.top+1,ETO_CLIPPED,lpRC,lpStr,(int)(lpTab-lpStr),NULL);
  ...
  rc.right=lpRC->right;
  CMnext(lpTab);
  ExtTextOut(hDC,rc.left,rc.top,ETO_OPAQUE|ETO_CLIPPED,&rc,lpTab,(int)_tcslen(lpTab),NULL);

}

void PaintVODrawStyle(LPLMDRAWITEMSTRUCT lm,DWORD_PTR ulFlags,HICON hIcon1,HICON hIcon2,int iIconsMargin)
{
  DWORD_PTR dwStyle=GetWindowLongPtr(lm->hWndItem,GWL_STYLE);
//  int iIconsMargin = (GetWindowLongPtr(lm->hWndItem,GWL_STYLE)&LMS_WITHICONS) ?  POSITION_TEXT : 0;
...
//          ExtTextOut(lm->hDC,rcText.left+lm->uiIndentSize+1+iIconsMargin,rcText.top,ETO_OPAQUE|ETO_CLIPPED,&rcText,lm->lpString,_tcslen(lm->lpString),NULL);
          ExtTextOut(lm->hDC,rcText.left+lm->uiIndentSize+1+(2*iIconsMargin)+(iIconsMargin ? POSITION_TEXT : 0),rcText.top,ETO_OPAQUE|ETO_CLIPPED,&rcText,lm->lpString,(int)_tcslen(lm->lpString),NULL);
...
LONG_PTR LinkedListWindow::FindFromString(Link * From)
{
  if (pStrToSearch)
    {
...
LONG_PTR LinkedListWindow::FindFromString(Link * From,char * pStr,BOOL bMatch)
{
  bSearchMatch = bMatch;
  if (pStrToSearch)
    delete [] pStrToSearch;
