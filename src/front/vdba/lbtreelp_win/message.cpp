...
static BOOL Cmp (LPSTR p, INT_PTR c)
{
  LPSTR lp =_tcschr(p, '\t');
  if (lp)
    CMnext(lp);
    ...
    lp=p;
  return  (_tcsupr(lp)==_tcsupr((LPSTR) (DWORD_PTR) c));
}
...
static LRESULT LLCallProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
  ...
  if (hWndParent)
    {
    LinkedListWindow * ll=(LinkedListWindow *)GetWindowLongPtr(hWndParent,0); 
    if (ll) 
      {
...
{
  if (!hWnd || !hWndOwner ||  (GetWindowLongPtr(hWnd,0)!=(LONG_PTR)this))
    return -1;
  if (lParam)
    VODrawStyle= ((((LPCREATESTRUCT)lParam)->lpCreateParams)!=0);
  ownerDraw=((GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_OWNERDRAW)!=0);
  epilogDraw=((GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_EPILOGDRAW)!=0);
  if (GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_CANEDIT)
    {
    ...
    if (hWndEdit)
      {
      lpOldEditProc = SetWindowLongPtr(hWndEdit,GWLP_WNDPROC,(LONG_PTR) EditSubWndProc);
...
LRESULT LinkedListWindow::ON_DESTROY (WPARAM wParam,LPARAM lParam)
{
  ...
  if (hWndEdit && lpOldEditProc)
    SetWindowLongPtr(hWndEdit,GWLP_WNDPROC,lpOldEditProc);
...
LRESULT LinkedListWindow::ON_MOUSEMOVE   (WPARAM wParam,LPARAM lParam)
{
  ...
  if (behindTheMouse && !inDragDrop)
    {
    if ((curMessage!=behindTheMouse)||(GetWindowLongPtr(hWnd,GWL_STYLE)&LMS_ALLONITEMMESSAGES))
      {
...
LRESULT LinkedListWindow::ON_FINDNEXT(WPARAM wParam,LPARAM lParam)
{
  if (cursel && cursel->Next())
    {
    return lParam ? FindFromString(cursel->Next(),(LPSTR) lParam,(BOOL)wParam) : FindFromString(cursel->Next());
    }
  return 0;
}
...
{
  return (first&&lParam) ? FindFromString(first,(LPSTR) lParam,(BOOL)wParam) : 0;
}
...
{
  DWORD_PTR wOldStyle=VODrawStyle;

  VODrawStyle=(BOOL)wParam;
