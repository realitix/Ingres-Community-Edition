...
{
    SIZE size;
    GetTextExtentPoint32(hdc, lpstr, (int)cb, &size);
...
extern void PaintItem(LPLMDRAWITEMSTRUCT lm,DWORD_PTR ulFlags,HICON hIcon1,HICON hIcon2,int iIconsMargin);
extern void PaintVODrawStyle(LPLMDRAWITEMSTRUCT lm,DWORD_PTR ulFlags,HICON hIcon1,HICON hIcon2,int iIconsMargin);


typedef struct _LINK_RECORD
{
  ULONG_PTR  ulRecID;
  ULONG_PTR  ulParentID;
  ULONG_PTR  ulInsertAfter;
  LPSTR  lpString;
  LPVOID lpItemData;
...
class LinkedListWindow : public Linked_List
{
  ...
  public:
    ...
    LONG_PTR                GetEditProc() {return lpOldEditProc;}
    ...
    LONG_PTR                FindFromString(Link * From);
    LONG_PTR                FindFromString(Link * From,char * pStr,BOOL bMatch);
