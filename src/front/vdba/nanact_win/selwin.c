...
static void PASCAL FillListBox(HWND hDlg,HWND hMdi)
{
  ...
  lpEnum=MakeProcInstance ((WNDENUMPROC)EnumMDIChild,GetWindowInstance(hDlg));
...
INT_PTR EXPORT FAR PASCAL SelWinDlgProc (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
    {
...
static void SubClassMDIBegin (LPOLDGLOBALS lp)
{
 ...
 lp->lpfnOldFarProc = (WNDPROC) SetWindowLongPtr (lp->hWnd,GWLP_WNDPROC,(LONG_PTR) lpfnNewFarProc);
}
...
void SubClassMDIEnd(LPOLDGLOBALS lp)
{
  ...
  if (lp->lpfnOldFarProc)
    {
    lpfnNewFarProc= (FARPROC) SetWindowLongPtr (lp->hWnd,GWLP_WNDPROC,(LONG_PTR) lp->lpfnOldFarProc);
