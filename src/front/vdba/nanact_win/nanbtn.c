...
NANACT_LIBAPI HWND FAR PASCAL CreateNanBarWnd(HWND hwndParent, HANDLE hInstance, UINT * lpumsg)
{
  ...
  *lpumsg = (UINT)NULL;
  if (hwndChild)
  {
     LPNANBARDATA pNanBar = (LPNANBARDATA) GetWindowLongPtr(hwndChild, GWL_USERDATA);
...
static LRESULT NEAR PASCAL HandleNanUserMessages(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  LPNANBARDATA pNanBar = (LPNANBARDATA) GetWindowLongPtr(hwnd, GWL_USERDATA);
...
static BOOL NanOnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
    ...
    SetWindowLongPtr(hwnd, GWL_USERDATA, (LONG_PTR) pNanBar);
...
static LRESULT HandleStatusUserMessages(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case STATUS_SETMSG:
        {
            PSTATUSWNDDATA pStatusWndData = (PSTATUSWNDDATA) GetWindowLongPtr(hwnd, GWL_USERDATA);
...
static BOOL StatusOnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (pStatusWndData)
  {
    ...
    SetWindowLongPtr(hwnd, GWL_USERDATA, (LONG_PTR) pStatusWndData);
    return TRUE;
  }
  return FALSE;
}
...
static void StatusOnPaint(HWND hwnd)
{
  PAINTSTRUCT paintStruct;
  BeginPaint(hwnd, &paintStruct);
  StatusMsgPaint(hwnd, paintStruct.hdc, (PSTATUSWNDDATA) GetWindowLongPtr(hwnd, GWL_USERDATA));
  EndPaint(hwnd, &paintStruct);
}
