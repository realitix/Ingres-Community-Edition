...
LRESULT CuDlgPageDifferentList::OnUpdateData(WPARAM wParam, LPARAM lParam)
{
	CString strCount;
...
void CuDlgPageDifferentList::ConstructList4Sorting(CObArray& T)
{
	...
	for (i=0; i<nCount; i++)
	{
		...
		DWORD_PTR dwData = m_cListCtrl.GetItemData(i);
	...
	try
	{
		...
		else
		{
			...
			for (i=0; i<nCount; i++)
			{
				...
				int nHeader = (int)pItem->m_arrayItem.GetSize();
				for (k=0; k<nHeader; k++)
				{
...
void CuDlgPageDifferentList::PopupDetail(int nSelectedDiffItem)
{
	...
	if (nSelectedDiffItem >= 0)
	{
		...
		DWORD_PTR dwItemData = m_cListCtrl.GetItemData (nSelectedDiffItem);
