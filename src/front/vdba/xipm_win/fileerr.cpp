	...
	if (h != INVALID_HANDLE_VALUE)
	{
		...
		if (dwRes != 0xFFFFFFFF )
		{
			...
			while (p && *p != _T('\0')) {
				p2 = _tcschr(p, _T('\n'));
				if (p2) {
					WriteFile(h,p, (DWORD)(p2-p),&dwNbBytes,NULL);
					WriteFile(h, csEndLine, csEndLine.GetLength(),&dwNbBytes,NULL);
					p = ++p2;
				}
				else {
					WriteFile(h, p, (DWORD)_tcslen(p),&dwNbBytes,NULL);
					WriteFile(h, csEndLine, (DWORD)csEndLine.GetLength(),&dwNbBytes,NULL);
					break;
				}
			}
			...
			tcSeparate[70] = _T('\0');
			WriteFile(h, tcSeparate, (DWORD)_tcslen(tcSeparate),&dwNbBytes,NULL);
			WriteFile(h, csEndLine, (DWORD)csEndLine.GetLength(),&dwNbBytes,NULL);
		}
...
int CaFileError::ReadSqlErrorFromFile(CTypedPtrList<CPtrList, CaSqlErrorInfo*>* lstSqlError)
{
	...
	size_t iNbChar,iRetlineLength,iSeperatelineLength,iErrorStatementLength,iErrorTextLength;
