 ...
 * drivi01  28-May-2009 During 64-bit port encountered errors concerning 
 ...
 * drivi01  04-Nov-2010 Port the solution to x64. Clean up the warnings.
...
WNDPROC SubClassEditCntl( HWND hDlg, int nDlgItem, WORD wType)
{
	...
	if (hMem)
	{
		...
		if (lpInfo)
		{
			...
			lpInfo->lpFn= (WNDPROC)SetWindowLongPtr(hEdit,
				GWLP_WNDPROC,
				(LONG_PTR)SubEditCntlProc);
			lpOldProc=lpInfo->lpFn;
			GlobalUnlock(hMem);
		}
	}
...
void ResetSubClassEditCntl(HWND hDlg, int nDlgItem)
{
	...
	if (hMem)
	{
		...
		if (lpInfo)
		{
    		...
    		lpProc=(WNDPROC)SetWindowLongPtr(hEdit, GWLP_WNDPROC,
				(DWORD) lpInfo->lpFn);
