...
** 05-Jul-2011 (drivi01)
**    Add routines for determining which tables should
**    be displayed for referential constraints.
**    Use nVWStructType to determine which structure
**	  is used and then filter accordingly. If heap
**    then displays Ingres tables only, if VectorWise
**    or VectorWise_Row then display VectorWise tables only.
**    For 1.0 release existing function should still work,
**    since bCreateVectorWise is a bool and false (0) means
**    OT_INGRES, and true (1) means OT_VECTORWISE.
**/
...
static BOOL OnInitDialog (HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
	...
	if (IsVW() &&  GetVWVers() >= VWVERS_15 && lpTable->bCreate)
	{
		switch(lpTable->nVWStructType)
		{
		case IDX_HEAP:
			ComboBoxFillTablesFiltered (hwndReferencedTable, (LPUCHAR)lpTable->DBName, OT_INGRES);
			break;
		case IDX_VW:
		case IDX_VWROW:
			ComboBoxFillTablesFiltered (hwndReferencedTable, (LPUCHAR)lpTable->DBName, OT_VECTORWISE);
			break;
		}
