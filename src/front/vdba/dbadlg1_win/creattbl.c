...
** 25-Mar-2011 (drivi01)
**   Add support for "create table" structure types
**   of vectorwise, vectorwise_row, and heap.
**   Also update the code to handle default vectorwise
**   structure.
**   Added combo box with the list of table structures
**   to the "create table" dialog.
** 05-Jul-2011 (drivi01)
**   Fix "create table..." to use nVWStructType to store
**   storage structure instead of bCreateVectorWise which
**   will still be used to support 1.0 version.
*****************************************************************************/
...
static CTLDATA tblStruct[] = 
{
IDX_STORAGE_STRUCT_UNKNOWN, 	IDS_STRUCT_DEFAULT,
IDX_VW,		IDS_STRUCT_VW,
IDX_VWROW,	IDS_STRUCT_VWROW,
...
static void OnSelectStructure		(HWND hwnd);
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    /* For VectorWise release 1.0 the only structure type possible
    ** for "create table" was VectorWise which is why check box
    ** was sufficient.
    ** In 1.5 release, there's 3 structures: Heap, VectorWise and
    ** VectorWise_row.  The structure type options are put into
    ** a combo box as a list.
    */
    if (IsVW() && GetVWVers() >= VWVERS_15)
    {
	EnableWindow (GetDlgItem (hwnd, IDC_INGCHECKVW_COMBO), TRUE);
	EnableWindow (GetDlgItem (hwnd, IDC_STATIC_INGCHECKVW), TRUE);
	ShowWindow (GetDlgItem (hwnd, IDC_INGCHECKVW), SW_HIDE);
	OccupyComboBoxControl (GetDlgItem (hwnd, IDC_INGCHECKVW_COMBO), tblStruct); 

	//default table structure is VectorWise. Disable "Options"
	//and "Check" buttons as VectorWise doesn't support those
	//options.
    ...
    {
	EnableWindow (GetDlgItem (hwnd, IDC_INGCHECKVW_COMBO), FALSE);
	EnableWindow (GetDlgItem (hwnd, IDC_STATIC_INGCHECKVW), FALSE);
	ShowWindow (GetDlgItem (hwnd, IDC_INGCHECKVW), SW_SHOW);
...
static void OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    case IDC_INGCHECKVW:
      ...
      break;
    case IDC_INGCHECKVW_COMBO:
      OnSelectStructure(hwnd);
      break;
    }
}
...
static void OnSelectStructure(HWND hwnd)
{
    /* Retrieve the selected item in the table structure 
    ** control.  If it's a heap, enable "Options" and "Check"
    ** buttons.  If it's some other sturcture in the list,
    ** disable those 2 buttons.
    */
    LPTABLEPARAMS lptbl = (LPTABLEPARAMS)GetDlgProp(hwnd);
    HWND hwndVWCombo	= GetDlgItem(hwnd, IDC_INGCHECKVW_COMBO);
    int index = ComboBox_GetCurSel( hwndVWCombo );
    lptbl->nVWStructType = (int)ComboBox_GetItemData( hwndVWCombo, index );
    if (lptbl->nVWStructType == IDX_HEAP)
    {
      EnableWindow(GetDlgItem(hwnd, IDC_STRUCTURE), TRUE);
      EnableWindow(GetDlgItem(hwnd, IDC_TLCBUTTON_CHECK), TRUE);
    }
...
static void OnOK (HWND hwnd)
{
	...
	if (IsVW() && GetVWVers() >= VWVERS_15)
	{
		// Create Table in 1.5 release can have a structure of Heap, VectorWise, or VectorWise_row
		HWND hwndVWCombo	= GetDlgItem(hwnd, IDC_INGCHECKVW_COMBO);
		// lptbl->bCreateVectorWise should already be set by OnSelectStructure function, but
		// in case if something went wrong, just double check here and set it.
		if (!lptbl->nVWStructType && (lptbl->bCreateVectorWise = IsWindowEnabled(hwndVWCombo)) && ComboBox_GetCurSel( hwndVWCombo) > 0)
		{
			index = ComboBox_GetCurSel( hwndVWCombo );
			lptbl->nVWStructType = (int)ComboBox_GetItemData( hwndVWCombo, index );
		}
	}
	else
	{
		//
		// Create Table as VectorWise in 1.0 release.
    ...
    if (lptbl->bCreateVectorWise || lptbl->nVWStructType)
    {
	lptbl->lpCheck = NULL;
    }
...
static void OnReference   (HWND hwnd)
{
    ...
    ts.primaryKey   = lptbl->primaryKey;
    ts.bCreateVectorWise = lptbl->bCreateVectorWise;
    ts.nVWStructType = lptbl->nVWStructType;
    MakeColumnsList   (hwnd, &ts);
...
static LPTSTR InsertString (LPTSTR *lpDestination, LPTSTR lpCurrent,LPTSTR lpStrAdd)
{
    ...
    size_t nbAlloc,iStartInsertPosition,nbLenAddStr,nbLenOldStr;
                                ...
                                LPTSTR lpTblName, LPTSTR lpHeader)
{
    ...
    size_t nMemoryAllocated, nMemAlloc,iParenthesisLevel;
