...
//       When retrieving grantees also retrieve the grantee type (user/
//       group/role) when present to avoid confusion with grants on non-
//       existing objects with existing ones of the same name.
********************************************************************/
...
static LPCHECKEDOBJECTS GetGrantee (LPREVOKEPARAMS lprevoke, int objectToFind, int grantee)
{
   ...
   /* retrieve grantee type into bufusertype - if none is set determine via DBA_GetObjectType */ 
   ires = DOMGetFirstObject (hdl, grant_user, 2, parentstrings, bwsystem, NULL, buf, NULL, bufusertype);
   while (ires == RES_SUCCESS)
   {
	   usertype = getint(bufusertype);
	   switch (usertype)
	   {
		    case OT_USER:
		    case OT_ROLE:
       ...
       ires  = DOMGetNextObject (buf, buffilter, bufusertype);
   }
   return list;
}
