...
**    05-Jul-2011 (drivi01)
**      Fix "create synonym..." to fetch all table types
**      using the new ComboBoxFillTablesFiltered with filterType
**      set to OT_ALL.
*****************************************************************************/
...
static BOOL FillComboBoxWithObject (HWND hwndCtl, int ObjectType, LPSYNONYMPARAMS lpsynonymparams)
{
   ...
   switch (ObjectType)
   {
       case OT_TABLE:
		   ...
		   if (IsVW())
			   ComboBoxFillTablesFiltered(hwndCtl, lpsynonymparams->DBName, OT_ALL);
		   else
