...
**    05-Jul-2011 (drivi01)
**      Fix ComboBoxFillTablesFiltered to take a filterType parameter.
**	    filterTypes supported are OT_INGRES, OT_VECTORWISE and OT_ALL.
**      OT_INGRES filters Ingres only tables.
**      OT_VECTORWISE filters VectorWise tables only.
**      OT_ALL displays all tables.
********************************************************************/
...
BOOL GetEditCtrlMinMaxValue (HWND hwnd, HWND hwndCtl, LPEDITMINMAX lpLimits, DWORD_PTR FAR * lpdwMin, DWORD_PTR FAR * lpdwMax)
/*
	Function:
...
void my_itoa (INT_PTR value, char FAR * lpszBuffer, int radix)
{
...
void my_dwtoa (DWORD_PTR value, char FAR * lpszBuffer, int radix)
{
	static char szBuf[20];
	...
	_ultoa((unsigned long)value, szBuf, radix);
	lstrcpy(lpszBuffer, szBuf);
}
...
LPOBJECTLIST AddListObject(LPOBJECTLIST lpList, size_t cbObjSize)
/*
   Function:
...
LPOBJECTLIST AddListObjectTail(LPOBJECTLIST FAR * lplpList, size_t cbObjSize)
/*
   Function:
...
/*
** filterType can be the following:
** OT_INGRES - Will fill Ingres only type objects
** OT_VECTORWISE - Will fill Vectorwise only type objects
** OT_ALL - Will fill both types, Ingres and Vectorwise type objects
*/
BOOL ComboBoxFillTablesFiltered (HWND hwndCtl, LPUCHAR DatabaseName, int filterType)
{
	...
	while (ires == RES_SUCCESS)
	{
		if ((filterType == OT_INGRES && getint(szExtra+STEPSMALLOBJ+STEPSMALLOBJ) == 0)
			|| (filterType == OT_VECTORWISE && getint(szExtra+STEPSMALLOBJ+STEPSMALLOBJ) > 0)
			|| filterType == OT_ALL)
			idx = ComboBox_AddString	(hwndCtl, buf);
		buffowner = ESL_AllocMem (x_strlen (szOwner) +1);
		if (!buffowner)
