...
**  07-Jun-2010 (drivi01)
**    During 64-bit port encountered errors concerning 
**    undeclared identifier DWL_USER and GWL_WNDPROC.
**    The functions that pass these values as parameters
**    are SetWindowLong and GetWindowLong have been superseeded
**    by SetWindowLongPtr and GetWindowLongPtr which pass
**    DWLP_USER and GWLP_WNDPROC as parameters instead.
**    Updating functions and parameters as explained above
...
extern BOOL MfcAddDatabaseCdds( LPREPCDDS lpcdds ,INT_PTR nHandle );
...
static INT_PTR GetIndexForInsertString(HWND hwndCtl,DWORD ItemValue)
/*
// Function:
...
*/
{
   ...
   _itoa((int)lpconn->dbNo,szKey,10);
  ...
  //    lpconn->nServerNo=1;
   ...
   _itoa(lpconn->nServerNo,szKey,10);
...
{
   LPREPCDDS lpold = (LPREPCDDS)GetWindowLongPtr(hwnd,DWLP_USER);
   REPCDDS r2;
   REPCDDS r3;
...
static void InitializeContainers(HWND hwnd)
{
    ...
    cntColData.fp = (WNDPROC) SetWindowLongPtr(GetWindow(hCntCol,GW_CHILD),GWLP_WNDPROC,(LONG) ColCntWndProcNew);
    ...
    cntPathData.fp = (WNDPROC) SetWindowLongPtr(GetWindow(hCntPath,GW_CHILD),GWLP_WNDPROC,(LONG) PathCntWndProcNew);
...
static BOOL OnInitDialog (HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SetWindowLongPtr(hwnd,DWLP_USER,(LONG) lpnew);
...
static void OnDestroy (HWND hwnd)
{
    ...
    LPREPCDDS lp1 = (LPREPCDDS)GetWindowLongPtr(hwnd,DWLP_USER);
    ...
    if (lpcntcoldata)
        SetWindowLongPtr(GetWindow(hCntCol,GW_CHILD),GWLP_WNDPROC,(LONG) lpcntcoldata->fp);

    lpcntPathData=(LPCNTPATHDATA)CntUserDataGet(hCntPath);
    if (lpcntPathData)
        SetWindowLongPtr(GetWindow(hCntPath,GW_CHILD),GWLP_WNDPROC,(LONG) lpcntPathData->fp);
