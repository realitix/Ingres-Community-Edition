...
** 11-Apr-2011 (drivi01)
**    Disable VectorWise table structure combo box on
**    alter table, as alter table isn't supported by VW
**    so if the dialog is displayed it's an Ingres dialog.
*******************************************************************************/
...
{
    UINT_PTR  uDataType;                // A number identifying Data type
...
static int  GetMinReqLength         (UINT_PTR nId);
static long GetMaxReqLength         (UINT_PTR nId);
static void MakeTypeLengthString    (UINT_PTR uType, LPCNTDATA lpdata, LPSTR lpszNumber);
static void GetTypeLength           (UINT_PTR uType, LPCNTDATA lpdata, LPSTR lpszNumber);
...
static int GetDataTypeStringId      (UINT_PTR nId);
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    lStyle  = GetWindowLongPtr (GetDlgItem (hwnd, IDC_TABLE), GWL_STYLE);
    lStyle |= ES_READONLY;
    SetWindowLongPtr (GetDlgItem (hwnd, IDC_TABLE), GWL_STYLE, lStyle);
    //
    ...
    ShowWindow   (GetDlgItem (hwnd, IDC_INGCHECKVW_COMBO), SW_HIDE);
    ShowWindow   (GetDlgItem (hwnd, IDC_STATIC_INGCHECKVW), SW_HIDE);
