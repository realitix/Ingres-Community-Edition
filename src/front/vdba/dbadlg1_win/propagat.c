...
static BOOL HasParentStaticReplicator(LPDOMDATA lpDomData, DWORD_PTR dwCurSel, char pReplicDbName[]);
...
static void RefreshBranchesOnTargetNode( LPPROPAGATE lppropadb)
{
   ...
   while (hwndCurDoc) {
       ...
       INT_PTR docType = QueryDocType(hwndCurDoc);
       ...
       if (bMustRefresh) {
         if (QueryDocType(hwndCurDoc) == TYPEDOC_DOM) {
           LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(GetVdbaDocumentHandle(hwndCurDoc), 0);
...
static void RefreshReplicRightPaneOnTargetNode(LPPROPAGATE lppropadb)
{
   ...
   while (hwndCurDoc) {
       ...
       if (docType == TYPEDOC_DOM) {
           ...
           LPDOMDATA lpDomData = (LPDOMDATA)GetWindowLongPtr(hwndDomDoc, 0);
...
static BOOL HasParentStaticReplicator(LPDOMDATA lpDomData, DWORD_PTR dwCurrentSel, char pReplicDbName[])
{
    INT_PTR level;
    DWORD_PTR dwSel;
