...
static BOOL     TamponOnExpand       (HWND hwnd, DWORD_PTR identifier);
static BOOL     TamponOnCollapse     (HWND hwnd, DWORD_PTR identifier);
static BOOL     TamponOnDelete       (HWND hwnd, DWORD_PTR identifier);
static BOOL     TamponOnDeleteAll    (HWND hwnd);
static DWORD_PTR    TamponOnGetItemData  (HWND hwnd, DWORD_PTR identifier);
static DWORD_PTR    TamponOnGetParent    (HWND hwnd, DWORD_PTR identifier);
static DWORD_PTR    TamponOnGetCurSel    (HWND hwnd);
static BOOL     TamponOnSetSel       (HWND hwnd, DWORD_PTR identifier);
static BOOL     TamponOnSetString    (HWND hwnd, LPSETSTRING lpst);
static LPCTSTR  TamponOnGetItemText  (HWND hwnd, DWORD_PTR identifier);
static DWORD    TamponOnGetItemLevel (HWND hwnd, DWORD_PTR identifier);

static BOOL     TamponOnGetItemState (HWND hwnd, DWORD_PTR identifier); // TRUE:Expanded
static BOOL     TamponOnSetTopItem   (HWND hwnd, DWORD_PTR identifier); 
static BOOL     TamponOnAddBefore    (HWND hwnd, LPLINK_RECORD lpRec); 
static DWORD_PTR    TamponOnGetFirstChild(HWND hwnd, DWORD_PTR identifier); 
static DWORD_PTR    TamponOnGetFirst     (HWND hwnd); 
static DWORD_PTR    TamponOnGetNext      (HWND hwnd, DWORD_PTR identifier); 
static DWORD_PTR    TamponOnGetPrev      (HWND hwnd, DWORD_PTR identifier); 
static DWORD_PTR    TamponOnGetTopItem   (HWND hwnd); 
static DWORD    TamponOnGetCount     (HWND hwnd);
static DWORD_PTR    TamponOnGetMouseItem (HWND hwnd, POINTS ptShort);

static DWORD    TamponOnSetFont      (HWND hwnd, HFONT hFont); 
static BOOL     TamponOnCollapseOne  (HWND hwnd, DWORD_PTR identifier);
...
static DWORD    TamponOnGetVisState  (HWND hwnd, DWORD_PTR identifier);
static DWORD_PTR    TamponOnGetNextSibling  (HWND hwnd, DWORD_PTR identifier);
static DWORD_PTR    TamponOnGetPrevSibling  (HWND hwnd, DWORD_PTR identifier);
...
protected:
  ...
  CMap< DWORD_PTR, DWORD_PTR, DWORD_PTR, DWORD_PTR > m_DWORDxHTREEITEM;
  CImageList iconList;
...
protected:
  ...
  DWORD_PTR     GetHTreeItem_LastDwId;        // received parameter
  ...
  DWORD_PTR     GetItemIdentifier_LastDwId;       // returned value
  ...
  CMap < DWORD_PTR, DWORD_PTR, DWORD_PTR, DWORD_PTR > m_HandleToDwIndex;
...
public:
  ...
  HTREEITEM GetHTreeItem    (DWORD_PTR dwId);
  DWORD_PTR     GetItemIdentifier (HTREEITEM hItem);
  void SetAssociate (DWORD_PTR identifier, HTREEITEM hItem);
  void DelAssociate (DWORD_PTR identifier, HTREEITEM hItem);
  void DelAssociate ();
...
static void TamponOnDestroy(HWND hwnd)
{
  ...
  DbaTree * pTree = (DbaTree *)GetWindowLongPtr(hwnd, 0);
  if (pTree)
  {
...
{
  DWORD_PTR     vdbaRecId;  // record id from the vdba point of view
  HTREEITEM hItem;      // tree item handle
  LRESULT   lResult;
  DbaTree * pTree = NULL;
  
  pTree = (DbaTree *)GetWindowLongPtr(hwnd, 0);
...
static BOOL TamponOnAddRecord(HWND hwnd, LPLINK_RECORD lpLr)
{
   ...
   bRet = pTree->SetItemData(hItem, (DWORD_PTR)lpLr->lpItemData);
   if (bRet) {
      TV_ITEM item;
...
static BOOL     TamponOnGetItemState (HWND hwnd, DWORD_PTR identifier)
{
    DbaTree * pTree = (DbaTree *)GetWindowLongPtr(hwnd, 0);
    HTREEITEM hItem;
...
static BOOL     TamponOnAddBefore    (HWND hwnd, LPLINK_RECORD lpRec)
{
    ...
    bRet = pTree->SetItemData (hItem, (DWORD_PTR)lpRec->lpItemData);
    if (bRet) 
    {
        TV_ITEM item;
        memset(&item, 0, sizeof(item));
...
LRESULT CALLBACK /* __export */ TamponWndProc ( HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
{
  switch (wMsg) {
    ...
    case LM_EXPAND:
      return TamponOnExpand(hwnd, (DWORD_PTR)lParam);
    case LM_COLLAPSE:
      return TamponOnCollapse(hwnd, (DWORD_PTR)lParam);
    case LM_DELETERECORD:
      return TamponOnDelete(hwnd, (DWORD_PTR)lParam);
    case LM_DELETEALL:
      return TamponOnDeleteAll(hwnd);
    case LM_GETITEMDATA:
      return TamponOnGetItemData(hwnd, (DWORD_PTR)lParam);
    case LM_GETPARENT:
      return TamponOnGetParent(hwnd, (DWORD_PTR)lParam);
    case LM_GETSEL:
      return TamponOnGetCurSel(hwnd);
    case LM_SETSEL:
      return TamponOnSetSel(hwnd, (DWORD_PTR)lParam);
    case LM_SETSTRING:
      return TamponOnSetString(hwnd, (LPSETSTRING)lParam);
    case LM_GETSTRING:
      return (LRESULT)TamponOnGetItemText(hwnd, (DWORD_PTR)lParam);

    case LM_GETLEVEL:
      return TamponOnGetItemLevel (hwnd, (DWORD_PTR)lParam);
    case LM_GETSTATE:
      return TamponOnGetItemState (hwnd, (DWORD_PTR)lParam);
    case LM_SETTOPITEM:
      return TamponOnSetTopItem   (hwnd, (DWORD_PTR)lParam); 
    case LM_ADDRECORDBEFORE:
      return TamponOnAddBefore    (hwnd, (LPLINK_RECORD)lParam); 
    case LM_GETFIRSTCHILD:
      return TamponOnGetFirstChild(hwnd, (DWORD_PTR)lParam); 
    case LM_GETFIRST:
      return TamponOnGetFirst     (hwnd); 
    case LM_GETNEXT:
      return TamponOnGetNext      (hwnd, (DWORD_PTR)lParam); 
    case LM_GETPREV:
      return TamponOnGetPrev      (hwnd, (DWORD_PTR)lParam); 
    case LM_GETTOPITEM:
    ...
    case LM_COLLAPSEBRANCH:
      return TamponOnCollapseOne  (hwnd, (DWORD_PTR)lParam);
    case LM_COLLAPSEALL:
    ...
    case LM_GETVISSTATE:
      return TamponOnGetVisState  (hwnd, (DWORD_PTR)lParam);

    // speed optimization
    case LM_GETNEXTSIBLING:
      return TamponOnGetNextSibling (hwnd, (DWORD_PTR)lParam);
    case LM_GETPREVSIBLING:
      return TamponOnGetPrevSibling (hwnd, (DWORD_PTR)lParam);
...
extern "C" HWND FAR PASCAL EXPORT LRCreateWindow(HWND hOwner,LPRECT rcLocation,DWORD dwStyle,BOOL bVODrawStyle)
{
  ...
  ASSERT (sizeof(HTREEITEM) == sizeof(DWORD_PTR) );
  ...
  TRY {
    ...
    if (bRet)
      if (pTree->AssociateIconList())
        ...
        SetWindowLongPtr(hwndTampon, 0, (LONG_PTR)pTree);
...
HTREEITEM DbaTree::GetHTreeItem (DWORD_PTR dwId)
{
  BOOL found;
  DWORD_PTR     dwHItem;  // placeholder for HTREEITEM
...
DWORD_PTR DbaTree::GetItemIdentifier (HTREEITEM hItem)
{
  BOOL found;
  DWORD_PTR dwIndex;
  ...
  found = m_HandleToDwIndex.Lookup ((DWORD_PTR)hItem, dwIndex);
  if (found) {
...
void DbaTree::SetAssociate (DWORD_PTR identifier, HTREEITEM hItem)
{
  m_DWORDxHTREEITEM.SetAt (identifier, (DWORD_PTR)hItem);
  // added for second table
  m_HandleToDwIndex.SetAt ((DWORD_PTR)(hItem), identifier);
}

void DbaTree::DelAssociate (DWORD_PTR identifier, HTREEITEM hItem)
{
    m_DWORDxHTREEITEM.RemoveKey (identifier);
    m_HandleToDwIndex.RemoveKey ((DWORD_PTR)hItem);

    // reset for HTREEITEM GetHTreeItem(DWORD_PTR dwId)
    if ( identifier == GetHTreeItem_LastDwId) {
      GetHTreeItem_LastDwId = (DWORD_PTR)-1;
      GetHTreeItem_LastHtreeItem = NULL;
    }

    // reset for DWORD_PTR GetItemIdentifier(HTREEITEM hItem)
    if (hItem == GetItemIdentifier_LastHtreeItem) {
      GetItemIdentifier_LastHtreeItem = NULL;
      GetItemIdentifier_LastDwId = (DWORD_PTR)-1;
    }
}
...
{  
  ...
  if (!m_bRClickNotified) {
    ...
    DWORD_PTR vdbaRecId = GetItemIdentifier(hItem);
...
static void LBTreePopulate (HWND hlbtree, HWND hwnd /*TamponWindow*/)
{
    ...
    UINT_PTR first = TamponOnGetFirst (hwnd);
