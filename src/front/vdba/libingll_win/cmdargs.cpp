...
int CaArgumentLine::Parse ()
{
	...
	INT_PTR i = 0, nSize = arrayToken.GetSize();
#if defined (_DEBUG)
	for (i=0; i<nSize; i++)
	{
...
BOOL CaArgumentLine::AnalyseNonKey(CStringArray& arryToken, INT_PTR& nCurrentIndex)
{
	INT_PTR nSize = arryToken.GetSize();
...
//
// The command line is: -k
BOOL CaArgumentLine::AnalyseKey(CStringArray& arryToken, INT_PTR& nCurrentIndex, CaKeyValueArg* pKey)
{
	INT_PTR nSize = arryToken.GetSize();
	CString strToken = arryToken.GetAt(nCurrentIndex);
	int nLen = strToken.GetLength();
	...
	CString strKey = pKey->GetKey();
	int nKeyLen = (int)_tcslen(strKey);
...
BOOL CaArgumentLine::AnalyseKeyValue(CStringArray& arryToken, INT_PTR& nCurrentIndex, CaKeyValueArg* pKey)
{
	INT_PTR nSize = arryToken.GetSize();
	CString strToken = arryToken.GetAt(nCurrentIndex);
	int nLen = strToken.GetLength();
...
BOOL CaArgumentLine::AnalyseKeyEqValue(CStringArray& arryToken, INT_PTR& nCurrentIndex, CaKeyValueArg* pKey)
{
