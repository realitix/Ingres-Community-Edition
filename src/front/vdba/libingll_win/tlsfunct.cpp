...
TCHAR* fstrncpy(TCHAR* pu1, TCHAR* pu2, size_t n)
{
...
char* _fstrncpy(char* pu1, char* pu2, size_t n)
{
	// don't use STlcopy (may split the last char)
	char* bufret = pu1;
	while ( n >= CMbytecnt(pu2) +1 ) { // copy char only if it fits including the trailing \0
...
{
	int nLength = item.GetLength();
	int i;
	for (i=0; i<nLength; i+= (int)_tclen((const TCHAR *)item + i))
	{
