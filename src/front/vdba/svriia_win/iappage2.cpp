	...
	try
	{
		...
		if (pWnd)
		{
			...
			if (nKindOf == FMT_FIXEDWIDTH)
			{
				...
				int  i, nDividerCount = (int)pWnd->SendMessage(WMUSRMSG_GETITEMDATA, 3, 0);
				INT_PTR nSize;
				...
				{
					int nTableColumns = (int)dataPage1.GetTableColumns().GetCount();
	...
	for (i=0; i<nCount; i++)
	{
		...
		if (nKindOf == FMT_FIXEDWIDTH)
		{
			...
			if (bMatch)
			{
				...
				INT_PTR k, nPosCount = arrayDivider.GetSize();
