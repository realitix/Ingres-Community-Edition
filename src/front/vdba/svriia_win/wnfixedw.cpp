...
INT_PTR CuWndFixedWidth::GetColumnDividerCount()
{
	CuStaticRuler* pRuler =(CuStaticRuler*)&m_ruler;
	if (!pRuler)
		return 0;
	INT_PTR nCount = pRuler->GetCount();
	return nCount;
}
...
void CuWndFixedWidth::GetColumnPositions(int*& pArrayPosition, INT_PTR& nSize)
{
	pArrayPosition = NULL;
...
void CuWndFixedWidth::Display(int nPos)
{
	if (!m_pArrayLine)
		return;
	INT_PTR i, nSize = m_pArrayLine->GetSize();
	m_vertScrollBar.SetScrollRange (0, (int)nSize-m_nStartIndex-m_nItemsPerPage);
	m_listbox.ResetContent();
...
void CuWndFixedWidth::DrawThumbTrackInfo (int nPos, BOOL bErase)
{
	...
	INT_PTR nSize = m_pArrayLine->GetSize();
	CString strSize;
