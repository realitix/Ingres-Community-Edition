...
**    generate statistics.
** 08-Apr-2010 (drivi01)
**    Remove dll.h header as it's causing build problems on x64.
** 13-Dec-2010 (drivi01) 
...
BOOL CuPPage4::OnSetActive() 
{
	...
	try
	{
		...
		INT_PTR i, nSize = commonItemData.m_arrayColumnOrder.GetSize();
		nHeader = 0;
		for (i=1; i<nSize; i++)
		{
...
BOOL CuPPage4::ImportByCopy(CaIIAInfo* pData)
{
	...
	INT_PTR i, nSize = arrayOrderColumn.GetSize();
	...
	else
	{
		...
		if (dataPage2.GetCurrentSeparatorType() == FMT_FIXEDWIDTH)
		{
			...
			INT_PTR nSize = arrayDividerPos.GetSize();
			int i;
	...
	if (dataPage2.GetCurrentSeparatorType() == FMT_DBF)
		...
		nMaxSizeToRead = (dataPage1.GetKBToScan() > 0)? 1024*dataPage1.GetFileSize(): (int)dataPage1.GetArrayTextLine().GetSize();
	...
	CTypedPtrList< CObList, CaDBObject* >& listColumn)
{
	...
	{
		CArray <int, int>& arrayDividerPos = dataPage2.GetArrayDividerPos();
		INT_PTR i, nSize = arrayDividerPos.GetSize();
		int nMenLen = dataPage1.GetMinRecordLength();
		for (i=0; i<nSize; i++)
		{
