...
LRESULT CuDlgPageCsv::OnUpdateData (WPARAM wParam, LPARAM lParam)
{
	...
	int i, nSize = (int)arrayRecord.GetSize();
	for (i=0; i<nSize; i++)
	{
...
LRESULT CuDlgPageCsv::OnCleanData (WPARAM wParam, LPARAM lParam)
{
	ASSERT (m_pData);
	CaDataPage1& dataPage1 = m_pData->m_dataPage1;
	CaDataPage2& dataPage2 = m_pData->m_dataPage2;
...
LRESULT CuDlgPageCsv::OnGetData (WPARAM wParam, LPARAM lParam)
{
	int nMode = (int)wParam;
	switch (nMode)
	{
...
void CuDlgPageCsv::OnCheckConfirmChoice() 
{
	...
	if (pParent && pParent->IsKindOf(RUNTIME_CLASS(CTabCtrl)))
	{
		...
		LRESULT nCheck;
		int i, nCount = pTab->GetItemCount();
		for (i=0; i<nCount; i++)
		{
