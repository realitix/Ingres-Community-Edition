...
{
	int j, nSize = (int)m_arrayColumnOrder.GetSize();
	CString strItem;
	CaColumn* pCol = NULL;
...
void CaIngresHeaderRowItemData::EditValue (CuListCtrlDoubleUpper* pListCtrl, int iItem, int iSubItem, CRect rcCell)
{
	...
	if (!m_bAllowChange)
		return;
	int j, nSize = (int)m_arrayColumn.GetSize();
	CString strItem;
	CaColumn* pCol = NULL;
...
BOOL CaIngresHeaderRowItemData::UpdateValue (CuListCtrlDoubleUpper* pListCtrl, int iItem, int iSubItem, CString& strNewValue)
{
	...
	int i, nSize = (int)m_arrayColumn.GetSize();
...
{
	int i, nSize = (int)m_arrayFieldSizeInfo.GetSize();
	for (i=0; i<nSize; i++)
	{
...
int& CaDataPage2::GetFieldSizeMax(INT_PTR nIndex)
{
...
int& CaDataPage2::GetFieldSizeEffectiveMax(INT_PTR nIndex)
{
	ASSERT(nIndex>=0 && nIndex<m_arrayFieldSizeInfo.GetSize());
	CaColumnSize* pNewSize = (CaColumnSize*)m_arrayFieldSizeInfo.GetAt(nIndex);
...
BOOL& CaDataPage2::IsTrailingSpace(INT_PTR nIndex)
{
	ASSERT(nIndex>=0 && nIndex<m_arrayFieldSizeInfo.GetSize());
	CaColumnSize* pNewSize = (CaColumnSize*)m_arrayFieldSizeInfo.GetAt(nIndex);
...
void CaDataForLoadSave::Cleanup()
{
	...
	// Page 3:
	// *******
	INT_PTR i, nSize = m_arrayColumnOrder.GetSize();
	for (i=0; i<nSize; i++)
	{
...
void CaDataForLoadSave::LoadData (CaIIAInfo* pIIAData, CString& strFile)
{
	try
	{
		...
		INT_PTR i, nSize = m_arrayTreeItemPath.GetSize();
		for (i=0; i<nSize; i++)
		{
...
void CaDataForLoadSave::SaveData (CaIIAInfo* pIIAData, CString& strFile)
{
	try
	{
		...
		CStringArray& arrayTreePath = dataPage1.GetTreeItemPath();
		INT_PTR i, nSize = arrayTreePath.GetSize();
		for (i=0; i<nSize; i++)
		{
		...
		if (m_nSeparatorType == FMT_FIXEDWIDTH)
		{
			...
			INT_PTR i, nSize = arrDividerPos.GetSize();
			for (i=0; i<nSize; i++)
...
void AutoDetectFormatCsv (CaIIAInfo* pData)
{
	...
	if (dataPage1.IsExistingTable() && dataPage1.GetFileMatchTable() == 1)
	{
		//
		// YES:
		nTableColumns = (int)dataPage1.GetTableColumns().GetCount();
	}
