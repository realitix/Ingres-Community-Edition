...
public:
	...
	void Cleanup()
	{
		...
		INT_PTR i, nSize = m_arrayRecord.GetSize();
		for (i=0; i<nSize; i++)
		{
	...
	INT_PTR GetFieldSizeInfoCount(){return m_arrayFieldSizeInfo.GetSize();}
	//
	...
	void SetFieldSizeInfo (INT_PTR nNewSize);
	int& GetFieldSizeMax(INT_PTR nIndex);
	int& GetFieldSizeEffectiveMax(INT_PTR nIndex);
	BOOL& IsTrailingSpace(INT_PTR nIndex);
protected:
	//
