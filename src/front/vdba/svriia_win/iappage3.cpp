...
** 05-Jun-2010 (martinb) 125220
**    Add ISO4T and ISO4TC
** 14-Apr-2011 (frima01) 125220
**    Increase cbsize to also hold ISO4T and ISO4TC.
**/
...
static void CALLBACK DisplayListCtrlLower(void* pItemData, CuListCtrl* pListCtrl, int nItem, BOOL bUpdate, void* pInfo)
{
	...
	int i;
	INT_PTR nSize = arrayFields.GetSize();
	int nIdx = nItem;
...
void CuPPage3::Loading()
{
	...
	if (!dataPage1.IsExistingTable())
	{
		...
		{
			INT_PTR i, nSize = arrayColumnOrder.GetSize();
			for (i=1; i<nSize; i++)
			{
	...
	else
	{
		...
		for (i=1; i<nSize; i++)
		{
			...
			{
				BOOL bExit = FALSE;
				INT_PTR k, nSize2 = commonItemData.m_arrayColumnOrder.GetSize();
				for (k=0; k<nSize2; k++)
				{
