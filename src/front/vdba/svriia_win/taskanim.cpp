...
extern "C" long GetRowCSV (long* rowLength, char* lpRow, size_t* returnByte)
{
	USES_CONVERSION;
	...
	{
		INT_PTR nMax = dataPage1.GetArrayTextLine().GetSize();
	...
	if (dataPage1.GetKBToScan() == 0 || dataPage1.GetStopScanning() == FALSE) // The Total file has been scanned:
	{
		...
		else
		{
			...
			if (pRecord)
			{
				...
				int nFields = (int)arrayFields.GetSize();
	...
	else // The partial file has been scanned (or interrupted, in this case the records are not all in memory):
	{
		...
		int nLen = 0;
		size_t& nCurrent = (size_t&)pUserData4CopyHander->GetCurrentRecord();
		...
		while ( !feof(pDataFile) )
		{
			...
			if (nLen <= 3500)
			{
				...
				else
				{
					...
					INT_PTR nFields = arrayFields.GetSize();
					//
...
//     1: failed
//  9900: (ENDFILE). End of data reached. This should be accompanied with a returnByte value of 0
extern "C" long GetRowDBF (long* rowLength, char* lpRow, size_t* returnByte)
{
	USES_CONVERSION;
	CaUserData4GetRow* pUserData4CopyHander = theApp.GetUserData4GetRow();
	ASSERT(pUserData4CopyHander);
	...
	if (dataPage1.GetKBToScan() == 0 || dataPage1.GetStopScanning() == FALSE)
	{
		INT_PTR nMax = dataPage2.GetArrayRecord().GetSize();
		if (pUserData4CopyHander->GetCurrentRecord() >= nMax)
			bEnd = TRUE;
	}
	else
...
int CaExecParamQueryRecords::Run(HWND hWndTaskDlg)
{
	...
	for (i=0; i<nSize; i++)
	{
		...
		nField = (int)arrayFields.GetSize();
		if (nCoumnCount == -1)
		{
			nCoumnCount = nField;
