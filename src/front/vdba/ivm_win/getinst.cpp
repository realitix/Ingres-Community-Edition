...
**        to be skipped.
** 27-Jun-2011 (drivi01) Mantis 1867, b12556
**     "logstat -headers -processes" command returns ACTIVE servers 
**     in a slightly different format now. 
**     Update the code to recognize new format as well as
**     use a port number to search for active archiver.
** 27-Jun-2011 (drivi01) Mantis 1979, b125526
**     Update IVM_GetFileSize function and routines around it
**     with newer APIs to ensure the change in log size is 
**     detected as soon as it happens.  _stat function used
**     does not appear to detect the changes immediately,
**     I suspect something related to permissions.
**     Also, the new routines can accomodate a larger log file.
*/
		...
		case COMP_TYPE_NAME         :
			{
				...
				while (pos != NULL)
				{
					...
					if (pInstance->m_csConfigName.CompareNoCase((LPCTSTR)pComponent->m_strComponentName)==0 &&
						...
						else if(isInstanceInIINamuOutput( (char *)(LPCTSTR) iigcn_id))
						{
							pItem = new CaTreeComponentInstanceItemData();
							pItem->m_strComponentName = pComponent->m_strComponentName;
							pItem->m_strComponentType = pComponent->m_strComponentType;
							pItem->m_nComponentType   = pComponent->m_nComponentType;
							pItem->m_strComponentInstance = iigcn_id;
							listInstance.AddTail (pItem);
						}
					}
				}
...
find_procs( )
{
	...
	// get list of "virtual" instances:
	{
		...
		char * szActive		     = "ACTIVE,A";
		char * szArchiver		 = "ARCHIV ";
		int	   pid		         = 0;
		/* logstat will fail in the scenario where ingres is started without service.
		** PCget_PidFromName will fail tryng to access any proccess that is run
		** by System or other user than ivm.exe.
		** PCget_PidFromName should be able to get the status of dmfacp when
		** ingres is not started as a service, for the initial installation scenarios.
		** When ingres is started as a service, search logstat output for active
		** servers.
		** NOTE: PCget_PidFromName will detect stray dmfacp processes
		** and may incorrectly report the server is up while GCN may 
		** not be able to see it.
		*/
		if ((pid = PCget_PidFromName("dmfacp")) != 0)
					bArchive	   = TRUE;
		else
		{
			...
			if (pc1) 
			{
				if (_tcsstr(pc1,szArchiver) || _tcsstr(pc1, szActive))
					bArchive	   = TRUE;
			}
		...
		/* This is for special cases when service is not running and 
		** logstat is unable to execute.  If system error 2 is returned
		** but rcp and acp are both up, logging and locking should be
		** up in most cases.  Report those as up.
		*/
		if (!bLockingOn && !bLoggingOn)
		{
			char * logstat_msg = "system error was 2";
			if (bArchive && iirecovery_count>0 && _tcsstr(pc, logstat_msg))
			{
				bLockingOn = TRUE;
				bLoggingOn = TRUE;
			}
...
static BOOL IVM_FillBufferFromFile(LONGLONG startpos, char * filename,char *buffer,int buflen, int * pretlen, BOOL bAppend0)
{
#ifdef MAINWIN
   int ihdl;
   int inumread;
...
*/
   ...
   *pretlen= inumread;
   ...
   _close(ihdl);
#else
   	BOOL dwRet = 0;
	LARGE_INTEGER start_ptr;
	LARGE_INTEGER new_ptr;
	HANDLE handle;

	start_ptr.QuadPart = startpos;
	new_ptr.QuadPart = static_cast<LONGLONG>(0);
	...
	dwRet = SetFilePointerEx(handle, start_ptr, &new_ptr, FILE_BEGIN);
	if (dwRet == 0)
	{
		CloseHandle(handle);
		return FALSE;
	...
	dwRet = ReadFile(handle, buffer, buflen, &inumread, NULL);
	if (inumread < 0)
		bResult = FALSE;
	
	*pretlen = inumread;

	for (int i=0; i<(int)inumread; i++)
	{
		if (buffer[i]=='\0')
			buffer[i]=' ';
	}
...
FileContentBuffer::FileContentBuffer(char * filename, long lmaxbufsize, BOOL bEndOnlyIfTooBig)
{
	int cbread;
	m_pbuffer = NULL;
	LONGLONG lstartpos = 0LL;
	strcpy(m_filename,filename);
	LONGLONG lBytes2Read = IVM_GetFileSize(filename);
	if (lBytes2Read<0LL) 
		return;
	...
	/* can typecast lBytes2Read to int here b/c the size is checked and if it's too big, 
	** broken up/truncated 
	*/
	BOOL bRes =IVM_FillBufferFromFile(lstartpos, filename,pbuffer2Read,(int)lBytes2Read+1, &cbread, TRUE);
