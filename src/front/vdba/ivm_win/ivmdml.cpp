...
** 28-Jun-2011 (drivi01) Mantis 1979, b125526
**    Update the functions that read/write from/to files to use
**    proper datatypes for file sizes to clear up the warnings
**    due to changes in IVM_GetFileSize.
**    Fix SerializeLogFileCheck to shift archive by correct amount
**    of bytes, otherwise the information isn't retrieved properly.
** 30-Aug-2011 (drivi01) Bug 125703
**    Fix SerializeLogFileCheck function to avoid error 
**    IDS_W_LOGFILE_CHGD_ALL_UNREAD.  lposcheckbuffer must
**    be of the same type as lpos1.  Fix everything else
**    accordingly.
*/
...
static ULONGLONG lbufferstartpos = 0LL;
static ULONGLONG buf0or1         = 0LL;
static ULONGLONG lnextpos2read = 0LL;
static ULONGLONG lnextpos2handle = 0LL;
...
static long lID0 = -1l;
static ULONGLONG lprevFileSize = 0LL;
...
static LONGLONG lposcheckbuffer;
...
static long ReadErrLogBuffer( char * buffer, LONGLONG lstartpos, long ilen)
{
	char filename[256];
#ifdef MAINWIN
	long lres;
	...
	if (bCheckPreviousFileState && bcheckinfoready)
	{
		...
		start_ptr.QuadPart = lposcheckbuffer;
		dwRet = SetFilePointerEx(handle, start_ptr, &new_ptr, FILE_BEGIN );
		if (dwRet == 0)
		{
			CloseHandle(handle);
			return READERRLOGINCONSISTENT;
		}

		dwRet = ReadFile(handle, bufloc, FILECHANGE_CHECK_BYTES, &inumread, NULL);
		if (inumread != FILECHANGE_CHECK_BYTES)
		{
			CloseHandle(handle);
			return READERRLOGINCONSISTENT;
	...
	dwRet = ReadFile(handle, buffer, ilen, &inumread, NULL);
	CloseHandle(handle);

	if (!bcheckinfoready && inumread > 0 && inumread > FILECHANGE_CHECK_BYTES)
	{
		lposcheckbuffer = lstartpos;
		memcpy(checkbuffer, buffer, FILECHANGE_CHECK_BYTES);
		bcheckinfoready = TRUE;
...
BOOL SerializeLogFileCheck(CArchive& ar)
{
	if (ar.IsStoring()) {
		...
		BOOL bShouldTest;
		LONGLONG lpos1; //The size here should match the size of lpscheckbuffer in if statement above
		...
		while (TRUE) {
			...
			lres =_lseeki64(ihdl, lpos1, SEEK_SET);
...
static char *fstrncpy(char *pu1, char *pu2, size_t n)
...
int IVM_llGetNewEventsStatus()
{
	...
	else
	{
		if (bMoreInMemory)
			return RET_NEW_EVENTS;
		ULONGLONG lnewlogfilesize;
		lnewlogfilesize = IVM_GetFileSize((char *) (LPCTSTR) theApp.m_strLoggedEventFile);

		if (lnewlogfilesize == -1LL) /* _the only case where IVM_GetFileSize() returns -1 is assumed to be if the file */
			lnewlogfilesize = 0LL;   /* doesn't exist, in which case we consider it (in terms of Ingres messages) as empty*/
	...
	try
	{
		...
		else
		{
			...
			while (TRUE) {
				...
				ULONGLONG lnextpos2readNEW = buf0or1*EACHBUFFER+lr;
