...
**    Add RefreshComponentData call when a new event is detected.
**    This will ensure the component tree is refreshed quicker
**    and IVM reflects the status of the servers quicker.
**/
...
BOOL CappIvmApplication::RefreshEventData(BOOL bBlocking)
{
	...
	try
	{
		...
		/* When new event is detected, refresh the component tree to detect any changes
		** in the status of any of the servers.
		** This will ensure the changes are reflected fairly quickly.
		** I am concerned about constant log changes which would result
		** in constant refreshes of component data and may cause a lag
		** on the machine.  If that occurs, this can be taken out.
		*/
...
UINT CappIvmApplication::ThreadProcControlEvents (LPVOID pParam)
{
	...
	while (1)
	{
		...
		if (nPitStop >= nEllapse || bReinit)
		{
			...
			{
				/* Change INFINITE to a number, INFINITE creates a hang in this thread
				** and the events never get reinitialized.
				*/
...
{
	ShellExecute(0, _T("open"), "http://www.actian.com/support.php", 0, 0, SW_SHOWNORMAL);
}
