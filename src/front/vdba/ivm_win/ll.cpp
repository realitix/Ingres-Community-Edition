...
static ULONGLONG		lconfiglog = 0LL;
...
{
	ULONGLONG ltemp =  IVM_GetFileSize( bufconfiglog);
...
ULONGLONG IVM_GetFileSize( char *FileName)
{
#ifdef MAINWIN
	struct _stat buffer;
	...
	if (result !=0 )
		return -1LL;
	return (ULONGLONG)buffer.st_size ;
#else
	HANDLE handle = NULL;
	LARGE_INTEGER FileSize;
	BOOL dwRet = 0;

	FileSize.QuadPart = static_cast<LONGLONG>(0);
	...
	{
		dwRet = GetFileSizeEx(handle, &FileSize);
		if (!dwRet)
			return -1LL;
