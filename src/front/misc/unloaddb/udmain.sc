...
**      16-Jun-2012 (Geraint Jones) SIR 126683
**          Add -relpath to unloaddb and prevent -add_drop case from
**          generating a truncated copy.in file.
**      17-Jun-2012 (pickr01) b126591
**          Added csv/ssv option to unloaddb as well
**/
    ...
    */
	...
	if (FEutaget(ERx("parallel"), 0, FARG_FAIL, &rarg, &pos) == OK)
        ...
        if (FEutaget(ERx("on_logfull"), 0, FARG_FAIL, &rarg, &pos) == OK)
        {
            ...
            {
                IIUGerr(E_UD0018_On_Logfull_Invalid, UG_ERR_FATAL, 0);
                PCexit(FAIL);
            }
        }
	...
	if (FEutaget(ERx("permitsonly"), 0, FARG_FAIL, &rarg, &pos) == OK)
        ...
        if (output_flags & (XF_DROP_INCLUDE | XF_RELATIVE_PATH))
        {
          if (!(output_flags & (XF_VIEW_ONLY | XF_PROCEDURE_ONLY |
            XF_RULES_ONLY | XF_SEQUENCE_ONLY | XF_INTEG_ONLY |
            XF_PERMITS_ONLY)))
        ...
        if (FEutaget(ERx("portable_ssv"), 0, FARG_FAIL, &rarg, &pos) == OK)
        {
            ...
            {
                IIUGerr(E_UD0019_CSV_SSV, UG_ERR_FATAL, 0);
                PCexit(FAIL);
            }
