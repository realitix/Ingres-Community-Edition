...
**	x100ixes	flag indicating whether VectorWise indexes are wanted
**			or not. Either just VW indexes or no VW indexes are
**			returned - not both.
**
** Outputs:
**
...
**	21-sep-2010 (dougi)
**	    Separate IVW indexes from regular Ingres indexes.
*/
XF_TABINFO *
xffillindex(i4 constr, XF_TABINFO *basetable, i4 x100ixes)
{
EXEC SQL BEGIN DECLARE SECTION;
    XF_TABINFO	*ip;
    short	nullind;
...
r.rng_ll4, r.rng_ur1, r.rng_ur2, r.rng_ur3, r.rng_ur4, t.table_reltid, \
t.table_reltidx FROM ((iitables t JOIN iiindexes i ON i.index_name=t.table_name) \
...
AND (i.index_owner = '%s' OR '' = '%s')"), Owner, Owner);
	...
	/* add X100 qualifier or filter defpending on parm */
	if (x100ixes)
	    STcat(stmtbuf, " AND i.storage_structure LIKE 'VECTORWISE%'");
	else
	    STcat(stmtbuf, " AND i.storage_structure NOT LIKE 'VECTORWISE%'");
	...
	EXEC SQL EXECUTE IMMEDIATE :stmtbuf INTO
		...
		:ip->table_reltid, :ip->table_reltidx;
	EXEC SQL BEGIN;
	{
	    if (fillindex(ip))
	    {
