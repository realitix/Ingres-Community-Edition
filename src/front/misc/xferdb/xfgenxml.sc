...
**      12-jan-2011 (stial01) (b124922)
**          Use DB_MAXNAME for column names, Init rdv.db_datatype
**      14-mar-2011 (stial01) m1604
**          Changes for i8 num_rows
**      06-may-2011 (stial01)
...
xmltabinfo(XF_TABINFO *tp)
{
   ...
   */
   ...
   /* row_nr */
   CVla8(tp->num_rows, num_rows_buf);
   STprintf( tbuf, ERx("\t\t row_nr=\"%s\"\n"), num_rows_buf);
   xfwrite(Xf_out, tbuf);
...
xmlprintresultset(XF_TABINFO *tp)
{
    ...
    /* row_end */
    STprintf( tbuf, ERx("\t\t row_end=\"%s\"\n"), num_rows_buf);
    xfwrite(Xf_out, tbuf);

    /* subset_size */
    STprintf( tbuf, ERx("\t\t subset_size=\"%s\"\n"), num_rows_buf);
    xfwrite(Xf_out, tbuf);

    /* row_count */
    STprintf( tbuf, ERx("\t\t row_count=\"%s\">\n"), num_rows_buf);
    xfwrite(Xf_out, tbuf);
}
...
xmlprintindex(XF_TABINFO *tp)
{
    ...
    if ((ilist = xffillindex(0, tp, 0)) == NULL)
	return;
