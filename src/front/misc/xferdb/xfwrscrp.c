...
**	    to FALSE.
**	10-dec-2009 (dougi)
**	    Generalize the X100 support a bit.
**	14-dec-2009 (dougi)
**	    Reset authID for VW table copy's.
...
**	23-sep-2010 (dougi)
**	    Augment IVW index logic to support indexes newly defined in
**	    the catalogs (i.e., by iirelation rows).
**	11-Apr-2011 (kschendel) SIR 125253
**	    Delete VW join-index column logic, no longer needed.
**      12-Apr-2011 (hanal04) SIR 125256
**          Add -on_logfull option. If in use generate a SET statement.
**      11-May-2011 (hanal04) SIR 125274
**          In UTF8 Portable mode (XF_NVARCHAR) force II_MONEY_FORMAT to
**          'L: ' to avoid any transliteration issues. Also set II_DATE_FORMAT 
**          to SWEDEN which is compatible with ANSI dates.
**      21-Jun-2011 (frima01) Bug 125449
**          Change logic of dropping sequences and views to be the same as when 
**          creating them, plus a check for -add_drop. Unloaddb was dropping
**          sequences but not recreating them when -add_drop was specified.
*/
void
	    ...
	    i4 output_flags, i4 output_flags2)
{
    ...
    */
    ...
    if(Portable)
    {
        ...
        {
            char	*nvarchar_money_format = "L: ";
            char	*nvarchar_date_format = "SWEDEN";

            /* set date_format II_DATE_FORMAT */
            STprintf(tbuf, ERx("set date_format '%s'"), nvarchar_date_format);
            xfwrite(Xf_both, tbuf);
            xfwrite(Xf_both, GO_STMT);

            /* set money_format II_MONEY_FORMAT */
            STprintf(tbuf, ERx("set money_format '%s'"), nvarchar_money_format);
            xfwrite(Xf_both, tbuf);
            xfwrite(Xf_both, GO_STMT);
        }
        else
    ...
    **
    */
    if(Xf_on_logfull != NULL)
    {
    	/* add set session with on_logfull=x */
    	xfwrite(Xf_in, ERx("set session with on_logfull="));
        xfwrite(Xf_in, Xf_on_logfull);
    	xfwrite(Xf_in, GO_STMT);
    }
    ...
    */
    if ( ! With_distrib )
    {
      ...
      */
      xftables(&foundcount, output_flags, output_flags2,
               &tlist_ptr);
    }
	...
	*/
    ...
    if (ivw_tabs)
    {
	XF_TABINFO	*ivwilist, *ip;
	XF_COLINFO	*cp1, *cp2;
	bool		match;

	/* Look for IVW indexes on the catalog (early versions didn't
	** record indexes in iirelation rows). */
	ivwilist = xffillindex(0, 0, 1);	/* X100 indexes? */

	if (ivwilist != NULL)
	{
	    /* Loop over indexes looking for matching tables. */
	    for (ip = ivwilist; ip; ip = ip->tab_next)
	    {
		/* Loop over tables looking for match. */
		for (tp = tlist_ptr, match = FALSE; tp; tp = tp->tab_next)
		 if (tp->is_x100 && tp->table_reltid == ip->table_reltid &&
			tp->table_reltidx == 0)
		 {
		    match = TRUE;
		    break;
		...
		/* Now loop over indexed columns, looking for match in
		** corresponding base table. */
		for (cp1 = ip->column_list; cp1; cp1 = cp1->col_next)
		{
		    /* Loop over base table looking for match. */
		    for (cp2 = tp->column_list, match = FALSE;
				cp2; cp2 = cp2->col_next)
		     if (STcompare(cp1->column_name, cp2->column_name) == 0)
		     {
			/* Got the sucker. Update table to show sequencing
			** on the column. */
			match = TRUE;
			cp2->key_seq = cp1->key_seq;
			MEcopy((char *)&ip->name, sizeof(ip->name),
				(char *)&tp->x100_ixname);
			tp->ixname_x100 = TRUE;
			break;
		     }
	...
	/* Special X100 index processing. */
	if (ivw_ixes)
	    xfivwixes(tlist_ptr, &tpvec, &tcount);

	/* Generate COPY statements for X100 tables. */
	xfwritehdr(VWTABS_COMMENT);

	if (ivw_ixes)
	 for (i = 0; i < tcount; i++)
	 {
	    xfsetauth(Xf_both, tpvec[i]->owner);
	    writecopy(tpvec[i], output_flags, output_flags2, NULL, TRUE);
	 }
	else
	 for (tp = tlist_ptr; tp; tp = tp->tab_next)
	  if (tp->is_x100)
	  {
	    xfsetauth(Xf_both, tp->owner);
	    writecopy(tp, output_flags, output_flags2, NULL, TRUE);
	  }
    }
}

