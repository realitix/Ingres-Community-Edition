...
**          Use gcn.local_vnode from config.dat rather than II_GCNxx_LCL_VNODE
*/
DUT_STATUS
...
DUT_ERRCB	*dut_errcb;
{
    ...
    if (dut_u16_vnode_p == NULL || *dut_u16_vnode_p == EOS)
    {
        ...
        if (PMload( (LOCATION *) NULL,  (PM_ERR_FUNC *) NULL) == OK)
        {
           ...
           {
               if (PMget("ii.$.gcn.local_vnode", &dut_u16_vnode_p) != OK)
               {
                   PMget("ii.$.gcn.lcl_vnode", &dut_u16_vnode_p);
               }
           }
        }
