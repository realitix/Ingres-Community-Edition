/*
** Copyright (c) 1998, 2011 Actian Corporation
*/
...
**	    Add m_ingresdate to track exactly what setting was chosen by
**	    the user.  This will help determine the best settings for
**	    date_alias in fresh installs, modifies, and upgrades.
**	15-Nov-2010 (drivi01)
**	    Add new variable to set VectorWise database location.
**	    Add new variables for Ingres VectorWise parameters.
**	    Add new functions to convert Ingres text to IVW
**	    for each dialog. Function name ConvertTextToIVW.
**	31-Aug-2011 (drivi01)
**	    Add m_JavaHome variable to store JDK or JRE path
**	    with intention to add it to the path.
**	    Add InstallManagementService to install the service.
**	    Add StartManagement to start the new service.
**	14-Oct-2011 (drivi01)
**	    Add m_isVista variable to contain a setting for 
**	    newer versions of Windows such as Vista, Windows 7 and 2003 server.
**	
**
*/
...
public:
	...
	BOOL	m_DBATools;
	BOOL	m_IngresVW;
	CString	m_DataBlockSize;
	CString m_ProcMemory;
	CString m_BuffPoolMem;
	CString	m_DataBlockGroupSize;
	CString	m_MaxParallelismLevel;
	CString m_VWConfigPath;
	CString m_JavaHome;
...
public:
	...
	int SetupCMConfig();
	int SetupTXNConfig();
	void ConvertTextToIVW(CPropertyPage *dialog);
