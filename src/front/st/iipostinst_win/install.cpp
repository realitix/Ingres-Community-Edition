...
**  15-Nov-2010 (drivi01)
**      Define II_VWDATA variable which will point to Ingres VectorWise
**      databases only.
**      Add new routines to handle custom configuration for Ingres
**      VectorWise.
**      Verify proper icons are displayed for both Ingres and IVW.
**      Add routines to create VectorWise specific directories, copy
**      vectorwise.conf to II_SYSTEM\ingres\data\vectorwise and
**      add VectorWise specific configurations to vectorwise.conf.
**      Update BI configurations to slightly modify the parameters
**      set to fit IVW. 
**      Add ConvertTextToIVW().
**  26-Jan-2011 (drivi01)
**      Update Transactional configuration with serializable 
**      isolation level and system_lock_level of mvcc.
**  23-Feb-2011 (frima01) Bug 125083
**      Only upgrade demodb when upgrading the installation.
**  29-Mar-2011 (smeke01) SIR 125227
**      Add iiinitres for new config parameter opf_hash_aggregate.
**  19-May-2011 (drivi01)
**      Add code to update ii_charset in config.dat. After charset 
**      is set, set default_collation based on that charset.
**  24-May-2011 (drivi01)
**	Add II_SYSTEM\\ingres\\ckp\\vectorwise creation to prepare for
**	vectorwise backups.
**  08-July-2011 (frima01) SIR 125503
**      Rewrite date alias setting to be available in dba tools and client only
**      install. Use new variable m_ingresdate to determine upgrade or modify.
**      Check for non-existance of config.dat to determine fresh net / dba tools
**      install and set customer value for date alias - else keep present value.
**  01-Aug-2011 (drivi01)
**      Rename "Ingres VectorWise" to just "VectorWise".
**      Fix AppendComment function to align "Done" and "Failed"
**      in the dialog.
**  28-Sep-2011 (drivi01)
**	Include gl.h header to have access to SYSTEM_SERVICE_NAME constant.
**	Use that constant to generate the service name on Windows.
**	This helps to access service for VW installations after the service
**	is renamed.
**	Add VectorWise specific routine to remove "Ingres" service before
**	installing VectorWise.  In the scenario where "Ingres" service doesn't
**	exist, nothing will happen.  
**  31-Aug-2011 (drivi01)
**	Installer should set JavaHome in the registry for the product
**	if JRE or JDK of correction version is installed.
**	Retrieve the value from the registry and add it to the PATH if it's 
**	not part of PATH already.
**  11-Oct-2011 (drivi01) m2424, b125833
**	Fix deprecated parameters for netsh.exe to add iigcc and iigcd 
**	servers to the firewall allowed program list.
**	Add new command to add newly added iimgmtsvr.exe to the
**	firewall as well.
**  19-Oct-2011 (drivi01)
**	Add a missing end quote to default_collation execution line.
**  10-Oct-2011 (smeke01) b125834
**      Add iiinitres for new config parameter int_math_i8_result.
**  25-Oct-2011 (drivi01)
**	For VectorWise installations for upgrade from 1.6 to 2.0 versions,
**	added special handling for service removal and installation.
**	Before removing Ingres service, query the information such as
**	auto start and username, and then use that information to
**	add VectorWise service.
**  31-Oct-2011 (drivi01)
**	DBA Tools comments dialog was not aligned, this change aligns it
**	by changing algorithm.
*/
...
#define SEPARATOR ";"
#define MGMTSVC "Manager_Service"
...
CInstallation::Init()
{
    ...
    */
	...
	if (m_IngresVW)
		theIcon = AfxGetApp()->LoadIcon(IDR_VWICON);
    ...
    if (schSCManager)
    {
	sprintf(szServiceName, "%s_Database_%s", SYSTEM_SERVICE_NAME, m_installationcode);
...
//    m_checkpointdatabases = GetRegValueBOOL("checkpointdatabases", FALSE);
    ...
    m_JavaHome = GetRegValue("JavaHome");
...
**          Remove references to II_GCNxx_LCL_VNODE
**	05-Oct-2011 (drivi01) 
**	    Add II_MTS_JAVA_HOME to symbol table.
*/
BOOL
CInstallation::SetSymbolTbl()
{
    ...
    {
	s.Format("II_MTS_JAVA_HOME \"%s\"", (LPCSTR)m_JavaHome);
	if (Exec("ingsetenv.exe", s))
	    error = TRUE;
    }
...
CInstallation::SetServerSymbolTbl()
{
    ...
    s.Format("II_VWDATA \"%s\"", (LPCSTR)m_vwdata);
    if (Exec("ingsetenv.exe", s))
   	error = TRUE;
...
BOOL
CInstallation::SetVWConfigurations()
{
	BOOL bRet = FALSE;

	AppendComment(IDS_CONFIGVW);
	AppendToLog(IDS_CONFIGVW);
	m_DataBlockSize = GetRegValue("vwdatablocksize");
	m_ProcMemory = GetRegValue("vwprocmemory");
	m_BuffPoolMem = GetRegValue("vwbuffpoolmem");
	m_DataBlockGroupSize = GetRegValue("vwdatablockgroupsize");
	m_MaxParallelismLevel = GetRegValue("vwmaxparallelthreads");

	bRet = WritePrivateProfileString("memory", "max_memory_size", (LPCSTR)m_ProcMemory, (LPCSTR)m_VWConfigPath);
	if (bRet) bRet = WritePrivateProfileString("cbm", "bufferpool_size", (LPCSTR)m_BuffPoolMem, (LPCSTR)m_VWConfigPath);
	if (bRet) bRet = WritePrivateProfileString("cbm", "block_size", (LPCSTR)m_DataBlockSize, (LPCSTR)m_VWConfigPath);
	if (bRet) bRet = WritePrivateProfileString("cbm", "group_size", (LPCSTR)m_DataBlockGroupSize, (LPCSTR)m_VWConfigPath);
	if (bRet) bRet = WritePrivateProfileString("system", "num_cores", (LPCSTR)m_MaxParallelismLevel, (LPCSTR)m_VWConfigPath);
	if (bRet) bRet = WritePrivateProfileString("engine", "max_parallelism_level", (LPCSTR)m_MaxParallelismLevel, (LPCSTR)m_VWConfigPath);

	DeleteRegValue("vwdatablocksize");
	DeleteRegValue("vwprocmemory");
	DeleteRegValue("vwbuffpoolmem");
	DeleteRegValue("vwdatablockgroupsize");
	DeleteRegValue("vwmaxparallelthreads");

	AppendComment(!bRet ? IDS_FAILED : IDS_DONE);
	if (!bRet) 
	{
		CString error;
		error.Format(IDS_SYSERROR, GetLastError());
		AppendToLog(error);
	}
...
CInstallation::CreateDatabases()
{
	...
	// upgrade demodb when no user dbs are to be upgraded and this is an upgrade
	if (DatabaseExists("demodb") && !theInstall.m_upgradedatabases && m_DBMSupgrade)
	{
		AppendToLog(IDS_UPGRADEDEMODB);
		if (bret && Exec("upgradedb", "demodb") )
			bret = FALSE;
	}
...
CInstallation::AppendComment(UINT uiStrID)
{
	...
	if (theInstall.m_DBATools || theInstall.m_IngresVW)
	{
		if (theInstall.m_DBATools)
			s.Replace("Ingres", "DBA Tools");
		else if (theInstall.m_IngresVW)
			s.Replace("Ingres", "VectorWise");

		//make sure output appears correct, align DONE|FAILED column
		if (uiStrID!=IDS_DONE && uiStrID!=IDS_FAILED )
		{
			/* DBA Tools with VW splash screens has the same spacing on the comments
		 	** dialog as Ingres version of DBA Tools.  It has to fall into "else"
			** statement here. */
			if (theInstall.m_IngresVW && !theInstall.m_DBATools)
			{
				/* To make sure DONE/FAILED appear in the proper place and are aligned,
				** for all strings not containing "VectorWise" add additional tab to make sure
				** everything aligns.  For strings containing "VectorWise" and "Installing"
				** or "Updating" and for strings less than 25 characters, add an extra tab
				** to force alignment.  Note, the reason why strings containing 
				** "Installing" and "Updating" words require additional tab is b/c the font
				** used uses different amount of pixels for each letter as a result
				** the 2 words above are narrower than other words with the same # of characters.
				** This results in requirement of different # of tabs for strings with identical
				** number of characters to align.
				*/
				if ((s.Find("VectorWise")) < 0 || (((s.Find("Installing")) >= 0 ||
					s.Find("Updating") >= 0) && s.Find("VectorWise") > 0) || s.GetLength() <= 25 )
					s.AppendChar('\t');
			}
			else
			{
				/* Update the calculation of tabs for DBA Tools post isntallation.
				** Remove a tab for all strings containing "DBA Tools" unless they're
				** between 30 and 34 characters long.
				*/
				int length = s.GetLength();
				if ((s.Find("DBA Tools")) >= 0 && (length > 34 || length < 30))
				{
					int index = s.ReverseFind('\t');
					if (index >=0) s.SetAt(index, '\0');
				}
			}
		}
	}
...
void CInstallation::SetEnvironmentVariables()
{
    ...
    pJava = getenv("PATH");
    if (!m_JavaHome.IsEmpty() && pJava && (strstr(pJava, m_JavaHome.GetBuffer()) == NULL))
    {
	s.Format("%s\\bin;", (LPCSTR)m_JavaHome);
...
{
    BOOL	error = FALSE, bConfigDatExists = FALSE;
    ...
    HANDLE	File;
    CComponent	*dbms = theInstall.GetDBMS();
    CComponent	*net = theInstall.GetNet();
	...
	*/
	...
	cmd.Format("qsf_guideline \"%s\\ingres\\files\\dbms.rfm",
		   (LPCSTR)m_installPath);
	Exec(m_installPath + "\\ingres\\utility\\iiinitres.exe", cmd, FALSE);
	cmd.Format("opf_hash_aggregate \"%s\\ingres\\files\\dbms.rfm",
		   (LPCSTR)m_installPath);
	Exec(m_installPath + "\\ingres\\utility\\iiinitres.exe", cmd, FALSE);
	cmd.Format("int_math_i8_result \"%s\\ingres\\files\\dbms.rfm",
		   (LPCSTR)m_installPath);
	Exec(m_installPath + "\\ingres\\utility\\iiinitres.exe", cmd, FALSE);
	...
	*/
	...
	bConfigDatExists = FileExists(sConfigDat);
	...
	else if (dbms && dbms->m_selected)
	{
	   ...
	   /* during dbms modify or fresh install set date alias according to customer */
	   /* selection, during upgrade neither ingresdate nor ansidate will be set 
	   /* by installer and the parameter will not be changed */
	   if (m_ansidate) 
	   {			
		cmd.Format("ii.%s.config.date_alias ansidate", m_computerName);
		Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd, FALSE);
	   }
	   else if (m_ingresdate)
	   {
		cmd.Format("ii.%s.config.date_alias ingresdate", m_computerName);
		Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd, FALSE);	
	   }
	}
	/* during client install, upgrade and modify only set date alias if */
	/* no value is present in config.dat, else preserve present value */	
	else if (net && net->m_selected)
	{
		CString configdate;
		...
		{
			cmd.Format("-keep date_alias \"%s\\ingres\\files\\default.rfm",
	   	   			(LPCSTR)m_installPath);
	   		Exec(m_installPath + "\\ingres\\utility\\iiinitres.exe", cmd, FALSE);	    	
		}
	}
	...
	/*
	** DBMS specific parameter settings that apply to new installations
	** and upgrades should go in here.
	*/
	if (dbms && dbms->m_selected)
	{
	    /*
	    ** Set ii_charset in config.dat to the charset selected.
	    ** Currently no way to set ii_charset using crs files.
	    ** Update collation based on the charset being set.
	    ** This code should execute for dbms component.
	    ** It will execute for new installs and upgrades.*/
	    cmd.Format( "ii.*.setup.ii_charset %s", (LPCSTR)m_charset);
	    Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd, FALSE);
	    cmd.Format("default_collation \"%s\\ingres\\files\\dbms.rfm\"",
		   (LPCSTR)m_installPath);
	    Exec(m_installPath + "\\ingres\\utility\\iiinitres.exe", cmd, FALSE);
	}
...
CInstallation::SetupBIConfig()
{
	...
	{
		cmd.Format("ii.%s.rcp.lock.per_tx_limit 7500", (LPCSTR)m_computerName);
		bret = Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
		if (bret) goto end;

		cmd.Format("ii.%s.dbms.*.psf_memory 20000000", (LPCSTR)m_computerName);
		bret = Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
		if (bret) goto end;
	}
...
CInstallation::SetupTXNConfig()
{
	...
	cmd.Format("ii.%s.dbms.*.system_isolation serializable", (LPCSTR)m_computerName);
	bret = Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
	if (bret) goto end;

	cmd.Format("ii.%s.dbms.*.system_lock_level mvcc", (LPCSTR)m_computerName);
	bret = Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
	if (bret) goto end;
...
CInstallation::SetTemp()
{
	...
	if (m_DBATools && strcmp(m_installationcode, "VT")==0)
		...
		sprintf(product_name, "VectorWise%s", (LPCSTR)m_installationcode);
	else 
...
BOOL
CInstallation::StartManagement()
{
    BOOL	error = FALSE;
    char 	szServiceName[MAX_PATH];
    SERVICE_STATUS	ServiceStatus;

    AppendToLog(IDS_MGMT_START);
    AppendComment(IDS_MGMT_START);

    sprintf(szServiceName, "%s_%s", MGMTSVC, m_installationcode);    

    SC_HANDLE schSCManager = OpenSCManager(NULL, NULL, SC_MANAGER_CONNECT);
    if (schSCManager)
    {
	SC_HANDLE schService = OpenService(schSCManager, szServiceName, SERVICE_START | SERVICE_QUERY_STATUS);
	if (schService != NULL)
	{
	     if (!StartService(schService, 0, NULL) && 
		QueryServiceStatus(schService, &ServiceStatus) && ServiceStatus.dwCurrentState != SERVICE_RUNNING)
			error = TRUE;
...
**	    Remove ice, leave the code.
**  14-Oct-2011 (drivi01)
**	    On older opertation systems like XP or Win 2003, preserve the
**	    old syntax for netsh.exe.
**	    
*/
DWORD
CInstallation::ThreadPostInstallation()
{
	...
	if (m_isVista)
	{
    	    cmd.Format("advfirewall firewall add rule name=\"iigcc\" dir=in action=allow program=\"%s\\ingres\\bin\\iigcc.exe\" enable=yes", (LPCSTR)m_installPath); 
    	    cmd2.Format("advfirewall firewall add rule name=\"iigcd\" dir=in action=allow program=\"%s\\ingres\\bin\\iigcd.exe\" enable=yes", (LPCSTR)m_installPath);
	}
	else
	{
	    cmd.Format("firewall add allowedprogram \"%s\\ingres\\bin\\iigcc.exe\" iigcc enable", (LPCSTR)m_installPath);
	    cmd2.Format("firewall add allowedprogram \"%s\\ingres\\bin\\iigcd.exe\" iigcd enable", (LPCSTR)m_installPath);
	}
	Exec("netsh.exe", cmd, TRUE, FALSE);
	Exec("netsh.exe", cmd2, TRUE, FALSE);
    }
	...
	*/
	...
	DeleteRegValue("ingresdate");
	DeleteRegValue("waitforpostinstall");
	DeleteRegValue("JavaHome");
    }
    ...
    */
    ...
    if (!m_JavaHome.IsEmpty() && ((dbms && dbms->m_selected) || (net && net->m_selected)))
	StartManagement();
...
CInstallation::ServerPostInstallation()
{
    ...
    if (m_IngresVW)
    {
    	if (!Local_NMgtIngAt("II_VWDATA", m_vwdata))
	     m_vwdata = GetRegValue("vwdatadir");
    	if (m_vwdata.IsEmpty())
	     m_vwdata = m_installPath;
    }
...
BOOL
CInstallation::CreateVWDATA()
{
    CString dir;
    BOOL    bret = TRUE;

    dir.Format("%s", (LPCSTR)m_vwdata);
    if (GetFileAttributes(dir) == -1)
	bret = CreateDirectoryRecursively(dir.GetBuffer(dir.GetLength()+1));

    if (bret)
    {
	dir.Format("%s\\ingres", (LPCSTR)m_vwdata);
	if (GetFileAttributes(dir) == -1)
	    bret = CreateDirectory(dir, NULL);
    }
    if (bret)
    {
	dir.Format("%s\\ingres\\data", (LPCSTR)m_vwdata);
	if (GetFileAttributes(dir) == -1)
	    bret = CreateDirectory(dir, NULL);
    }
	// copy vectorwise template file to vectorwife.conf under
	//II_VWDATA location
    if (bret && m_IngresVW)
    {
	dir.Format("%s\\ingres\\data\\vectorwise", (LPCSTR)m_vwdata);
	if (GetFileAttributes(dir) == -1)
	    bret = CreateDirectory(dir, NULL);
	CString srcDir, dstDir;
	srcDir.Format("%s\\ingres\\files\\vectorwise.conf_TMPL", (LPCSTR)m_installPath);
	dstDir.Format("%s\\vectorwise.conf", (LPCSTR)dir.GetBuffer());
	if (bret)
	{
		CopyFile(srcDir, dstDir, FALSE);
		m_VWConfigPath = dstDir;
	}
    }
...
CInstallation::CreateIICHECKPOINT()
{
    ...
    if (bret && m_IngresVW)
    {
	dir.Format("%s\\ingres\\ckp\\vectorwise", (LPCSTR)m_iicheckpoint);
	if (GetFileAttributes(dir) == -1)
	    bret = CreateDirectory(dir, NULL);
    }
...
#ifdef conf_IVW
		/* In VW release 2.0, the service was renamed from Ingres_Database_XX to VectorWise_Database_XX.
		** For upgrade scenarios, the old service has to be removed, and the new one added, however
		** b/c it's an upgrade, the registry doesn't contain any configuration information for the
		** service.  If the VW service is not detected in the code above, it could be the special
		** case being described in this statement.  Query the service configuration information
		** from the old Ingres service before removing it and use it to configure the new service.
		** Note, that password can not be retrieved from the service as it is a security risk,
		** therefore, if service is owned by an installation owner, users will have to edit 
		** service and fix the password before starting ingres as a service.
		** If service is owned by the system, service will be fully configured by the code below.
		*/
		if (!schService)
		{
			char	szServiceNameOld[MAX_PATH];
			sprintf(szServiceNameOld, "Ingres_Database_%s", m_installationcode);
			SC_HANDLE schService=OpenService(schSCManager, szServiceNameOld, SERVICE_QUERY_STATUS|SERVICE_QUERY_CONFIG);
			if (schService)
			{
				LPQUERY_SERVICE_CONFIG pqsc;
				DWORD dwSize = 0;
				DWORD dwBytesNeeded = 0;
				dwSize = sizeof(QUERY_SERVICE_CONFIG);
				QueryServiceConfig(schService, NULL, 0, &dwSize);
				if (dwSize && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
				{
					pqsc = (LPQUERY_SERVICE_CONFIG)malloc(dwSize);
					if (QueryServiceConfig(schService, pqsc, dwSize, &dwBytesNeeded))
					{
						if (pqsc->dwStartType == SERVICE_AUTO_START)
							m_serviceAuto = TRUE;
						if (pqsc->lpServiceStartName)
							theInstall.m_serviceloginid = pqsc->lpServiceStartName;
						if (theInstall.m_serviceloginid.Compare("LocalSystem") != 0)
							Message(IDS_NOPASSWORD_SERVICE, "");
					}
					if (pqsc)
						free(pqsc);
				} /* if (dwSize && (GetLastError() == ERROR_INSUFFICIENT_BUFFER)) */
...
#ifdef conf_IVW
	/* Vectorwise product service is being renamed, remove old Ingres service */
	Exec("opingsvc.exe", "remove Ingres", FALSE);
#endif
    }
    if ((dbms && dbms->m_selected))
...
BOOL 
CInstallation::InstallManagementService()
{
	CComponent *dbms = GetDBMS();
	CComponent *net = GetNet();
	char szServiceName[MAX_PATH];
	SERVICE_STATUS ServiceStatus;
	BOOL nret = FALSE;
	BOOL bServiceStopped = TRUE;

	sprintf(szServiceName, "%s_%s", MGMTSVC, m_installationcode);

	if ((dbms && dbms->m_selected) || (net && net->m_selected))
	{
	     SC_HANDLE schSCManager = OpenSCManager(NULL, NULL, SC_MANAGER_CONNECT);
	     if (schSCManager)
	     {
		SC_HANDLE schService = OpenService(schSCManager, szServiceName, SERVICE_QUERY_STATUS | SERVICE_USER_DEFINED_CONTROL | SERVICE_STOP);
		if (schService != NULL)
		{
			...
			if (ServiceStatus.dwCurrentState == SERVICE_RUNNING)
				...
				QueryServiceStatus(schService, &ServiceStatus) && ServiceStatus.dwCurrentState != SERVICE_STOPPED)
	     ...
	     */ 
	     if (bServiceStopped)
	     {
	     AppendComment(IDS_MGMT_SERVICE);
	     AppendToLog(IDS_MGMT_SERVICE);
	     Exec("iimgmtinstallsvc.exe", "remove", FALSE);
	     nret = (Exec("iimgmtinstallsvc.exe", "install", TRUE) == 0);
	     }
		...
		SetServiceAuto(szServiceName, TRUE);
	    	cmd.Format("advfirewall firewall add rule name=\"iimgmtsvr\" dir=in action=allow program=\"%s\\ingres\\bin\\iimgmtsvr.exe\" enable=yes", (LPCSTR)m_installPath); 
		Exec("netsh.exe", cmd, TRUE, FALSE);
	     }

	     if (nret) AppendComment(IDS_DONE);
	     else
		AppendComment(IDS_FAILED);
...
/*
**  Name:UpdateIngresBatchFiles (obsolete from the comments)
**
**  History:
**	18-mar-2002 (penga03)
...
void 
CInstallation::ConvertTextToIVW(CPropertyPage *dialog)
{
	if (theInstall.m_IngresVW)
	{
		// Fix the Welcome dialog to be Ingres VectorWise friendly 
		CString text, ingres, vectorwise;
		ingres.Format(IDS_INGRES);
		vectorwise.Format(IDS_INGRESVW);
		CWnd *ctrlTxt = NULL, *ctrlNext = NULL;
		int ctrlID = 0, oldID = 0;
	
		//retrieves each control in the dialog and replaces
		//"Ingres" with "VectorWise" and a few other
		//IVW specific strings.
