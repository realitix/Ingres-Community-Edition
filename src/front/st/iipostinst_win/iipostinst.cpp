...
**  09-Dec-2010 (drivi01)
**	    Add the title for Ingres VectorWise post installation.
**	    Update the title on message dialogs when Ingres VectorWise post
**	    installation is being performed.
**  30-Sep-2011 (drivi01)
**	    Copy Chris Clark's change for using environment variable to trigger
**	    DebugBreak().
**  07-Oct-2011 (drivi01)
**	    Added isVistaAndAbove to figure out if we're on Vista version of 
**	    Windows or above.
*/
...
MyMessageBox(UINT uiStrID, UINT uiFlags, LPCSTR param)
{
	...
	title.Format(IDS_TITLE_IVW, theInstall.m_installationcode);
    else
...
{
    BOOL    silent;
    char    *debug_strp = NULL;

    /* if debug... */
    debug_strp = getenv("II_DEBUG_INSTALLER");
    if (debug_strp != NULL)
    {
	UINT uiFlags = MB_OK | MB_ICONEXCLAMATION;
	HWND hwnd = 0;

	if (!hwnd)
	     uiFlags |= MB_APPLMODAL;

	if (strcmp("break", debug_strp) == 0)
	     DebugBreak();
    }
	...
	*/
	...
	s.Format(IDS_TITLE_IVW, theInstall.m_installationcode);
    else
