...
**	01-Jun-2010 (hanje04)
**	    Mantis 741
**	    Define prodname to point to "Ingres" or VectorWise" depending
...
**	09-Dec-2010 (hanje04)
**	    Add support for II_CONFIG_TYPE=IVW for VectorWise installs
**	31-Jan-2011 (hanje04)
**	    Mantis 1315
**	    Correct dirname for II_VWDATA location. Use 'data' rather than
**	    'data/vectorwise' as it's only used for removal and both dirs
**	     need to be removed.
**	07-Feb-2010 (hanje04)
**	    Wrap locations and parameter description strings in N_() so 
**	    that they can be extracted using xgettext for translation.
**	    Include gipsup.h to enable gettext libraries.
**	10-Feb-2011 (hanje04)
**	    Mantis 1303, 1348 & 1434
**	    Add sanity checking and upper limits to VW config parameters
**	    Initialize memory values to 0 for IVW config.
**	    Correct block size defaults.
**	    Move structure defns for _date_type_alias and_ing_config_type
**	    to gipdata.h as typedefs as they're needed elsewhere.
**	18-May-2011 (hanje04)
**	    Define DEBDefaultII_SYSTEM as location is fixed and different from
**	    other installs
**	04-Aug-2011 (hanje04)
**	    Define package names in a product dependent manor
**	24-Aug-2011 (hanje04)
**	    Remove "Ingres" from "Ingres VectorWise"
**	31-Aug-2011 (hanje04)
**	    Add prod_label_list to store labelname:string pairs so that the
**	    product name (Ingres|VectorWise) and be substituted in as required.
**	03-Oct-2011 (hanje04)
**	    Bug 125688
**	    Add javainfo structure for JRE location info required for
**	    remote manager service (iimgmtsvr)
*/
...
**
**  vw_cfg structure defined in gipdata.h
**
...
static vw_cfg ivw_block_size = {
	...
	N_("Block size"),
	"ivw_cfg_blksz",
	RFAPI_DEFAULT_VWCFG_BLOCK_SIZE_KB_INT,
	RFAPI_DEFAULT_VWCFG_BLOCK_SIZE_KB_INT,
	VWKB,
	VWKB,
};
...
static vw_cfg ivw_max_parallelism = {
	GIP_VWCFG_MAX_PARALLELISM,
	II_VWCFG_MAX_PARALLELISM,
	N_("Maximum parallel threads"),
	"ivw_cfg_max_para",
	1,
	1,
	VWNOUNIT, /* no unit for group size */
...
/* date type alias */
GLOBALDEF  _date_type_alias date_type_alias = {
		II_DATE_TYPE_ALIAS,
		{ "ansidate", "ingresdate" },
		INGRES
	};
...
/* Ingres config type */
GLOBALDEF _ing_config_type ing_config_type = {
		II_CONFIG_TYPE,
		{ "TXN", "BI", "CM", "IVW", "TRAD" },
...
GLOBALDEF const char *prodname = INGPRODNAME;
...
static stage_info install_stages[] = {
	...
	{ NI_IVWPKGSEL, "prog_pkgsel", ST_INIT },
...
GLOBALDEF stage_info *adv_ivw_stages[] = { 
	...
	&install_stages[NI_EXP], /* experience level (CHOOSE_INSTALL_TYPE)*/
	&install_stages[NI_INSTID], /* advanced config (ADVANCED_INSTALL) */
	&install_stages[NI_IVWPKGSEL], /* basic config (BASIC_INSTALL) */
	&install_stages[NI_DBLOC],  /* advanced config (ADVANCED_INSTALL) */
...
GLOBALDEF stage_info *ivw_nodbms_stages[] = { 
	&install_stages[NI_START], /* start screen (WELCOME_TO_INSTALL) */
	&install_stages[NI_LIC], /* license agreement (WELCOME_TO_INSTALL) */
	&install_stages[NI_EXP], /* experience level (CHOOSE_INSTALL_TYPE)*/
	&install_stages[NI_INSTID], /* advanced config (ADVANCED_INSTALL) */
...
GLOBALDEF _javainfo	javainfo = { "", "", 0 };

/* Upgrade modify strings */
GLOBALDEF const char	*ugactionstrlc;
GLOBALDEF const char	*ugactionstrlced;
GLOBALDEF const char	*ugactionstrcc;
GLOBALDEF const char	*ugactionstr_cc;

/* Product name dependent strings/labels from GUI */
GLOBALDEF prod_label prod_label_list[] = {
	{"label473", N_("This wizard will guide you through upgrading or removing an\nexisting %s instance, or installing a new one.\n\nClick Next to begin.") },
	{"label474", N_("<b>Welcome to the %s Installation Wizard</b>") },
	{"label476", N_("Another %s instance of the same version has been detected on \nyour machine\n\nSelect an option to proceed") },
	{"label480", N_("An older version of %s has been detected on your machine.\n\nSelect an option to proceed") },
	{"label484", N_("Multiple instances of %s have been detected on your machine.\n\nSelect an option to proceed") },
	{"label488", N_("%s supports multiple instances installed on the same server. Each\ninstance is uniquely identified by an instance name.") },
	{"label490", N_("<b>Existing %s Instances</b>") },
	{"label491", N_("<b>%s Instances</b>") },
	{"label692", N_("This wizard will guide you through creating an %s installer response file\n\nClick 'Next' to begin.") },
	{"label681", N_("<b>Welcome to the %s Response File Generator</b>") },
	{"label689", N_("A response file will be created for deploying %s\non Linux") },
	{"label688", N_("A response file will be created for deploying %s\non Windows") },
	{"label278", N_("This wizard will guide you through installing and setting up %s. \n\nClick 'Next' to begin.") },
	{"label372", N_("<b>Welcome to the %s Community!</b>") },
	{"label782", N_("<b>Ingres Configuration Type</b>") },
	{"label374", N_("%s supports multiple instances installed on the same machine. Each\ninstance is uniquely identified by an instance name. ") },
	{"label338", N_("You are now ready to install %s. Review your settings below\nand click <b>Install</b> to proceed with the installation.") },
	{"label337", N_("<b>Ready to Install %s</b>") },
	{"label324", N_("<b>Please wait while %s is installed</b>") },
	{"label326", N_("%s has been successfully installed.\n\nTo exit click \"<b>Finish</b>\"") },
	NULL
};
