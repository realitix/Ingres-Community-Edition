...
**	31-Jan-2011 (hanje04)
**	    Mantis 1315
**	    When listing II_VWDATA for removal, add the extra "vectorwise"
**	    directory even though we actually remove the parent
**	01-Feb-2011 (hanje04)
...
**	    Verify processing memory and buffer memory don't exceed 90%
**	    of physical memory. Popup warning if they do.
**	    Limit block size to GIP_VWCFG_MAX_BLK_SIZE_MB (64Mb) and
...
**	18-Mar-2011 (hanje04)
**	    Mantis 1767
**	    Change on_ivw_cfg_prcmem_val_change &
**	    on_ivw_cfg_buffpoolmem_val_change
**	    to be attached to GtkEditable signal "changed" instead of the
**	    GtkSpinButton signal "value_changed". This is to work around what
**	    appears to be a GTK bug, where by if the values in the corresponding
**	    fields are changed by hand to a figure which would trigger the
**	    memory over allocation warning and "Next" is clicked before
**	    leaving the field, the GUI crashes with:
**	    Gtk-WARNING **: GtkEntry - did not receive focus-out-event
**	    Also disable "Next" button if either field is empty.
**	17-May-2011 (hanje04)
**	    Add support for DEBs installs (i.e. don't update II_SYSTEM).
**	    Replace STprintf() with snprintf() where appropriate to reduce
**	    the risk of buffer overrun.
**	    Correct formatting for displaying "size types" to quiet compiler
**	    warnings.
**	22-May-2011 (hanje04)
**	    Mantis 1832
**	    Limit IVW_MAX_PARALLELISM to > 0
*/
                                        ...
                                        gpointer         user_data)
{
    ...
    widget_ptr=lookup_widget(IngresInstall, "tr_adv_config");
    gtk_widget_hide(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "tr_instance_name_box");
    gtk_widget_hide(widget_ptr);

    /* handle IVW/Ingres difference */
    comp_sel_box=lookup_widget(IngresInstall, "tr_comp_sel_box");
    summary_box=lookup_widget(IngresInstall, "tr_summary_box");
	...
	gtk_widget_hide(comp_sel_box);
	gtk_widget_hide(summary_box);
                                        ...
                                        gpointer         user_data)
{
    ...
    /* show the advanced progress items */
    widget_ptr=lookup_widget(IngresInstall, "tr_adv_config");
    gtk_widget_show(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "tr_instance_name_box");
    gtk_widget_show(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "tr_comp_sel_box");
    gtk_widget_show(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "tr_summary_box");
    gtk_widget_show(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "next_button");
    gtk_widget_show(widget_ptr);
    widget_ptr=lookup_widget(IngresInstall, "install_button");
    gtk_widget_hide(widget_ptr);

    /* hide/show DBMS config panes in train */
    widget_ptr = lookup_widget( IngresInstall, "tr_adv_db_config" );
                                        ...
                                        gpointer         user_data)
{
    ...
    if ( instmode & IVW_INSTALL )
	pkgs_to_install=VECTORWISE_CLIENT;
    else
                                        ...
                                        gpointer         user_data)
{
	...
	if ( instmode & IVW_INSTALL)
	{
	    /* skip IVW config panes if DBMS not selected */
	    stage_names = pkgs_to_install & PKG_DBMS ? 
                           adv_ivw_stages : ivw_nodbms_stages ;
	    adv_db_config = lookup_widget( IngresInstall, "tr_ivw_cfg_box" );
	}
	else
	{
	    ...
	    adv_db_config = lookup_widget( IngresInstall, "tr_log_loc_box" );
    ...
    */
	...
	if ( PCcmdline( NULL, cmdline, TRUE, NULL, NULL ) == OK )
		...
		if ( PCcmdline( NULL, cmdline, TRUE, NULL, NULL ) != OK )
		{
		    ...
		    STprintf( tmpbuf, SHUTDOWN_FAILED, prodname,
			selected_instance->instance_ID );
                                        ...
                                        gpointer         user_data)
{
	...
	*/
	...
	switch ( stage_names[current_stage]->stage )
	{
		...
		/* Correct train and strings for next stage */
		GIPsetUgAction();
		break;
	    case UG_SSAME:
		...
		DBG_PRINT("In UG_SELECT, going to %s\n",ug_mode&UM_MOD ?
						"UM_MOD" : "UG_SUMMARY" );
		if ( ~instmode & IVW_INSTALL && ug_mode&UM_MOD )
                              ...
                              gpointer         user_data)
{
    ...
    GtkWidget	*basic_install_button;
                                  ...
                                  gpointer         user_data)
{
    ...
    /* set upgrade modify strings and train */
    GIPsetUgAction();
                                        ...
                                        gpointer         user_data)
{
    ...
    /*
    ** If II_SYSTEM hasn't been changed by the user, and it's not a
    ** DEB install,  update that too
    */
    if ( new_pkgs_info.pkgfmt != PKGDEB &&
	 ! strncmp(iisystem.path, default_ii_system, pathsize ) )
    {
	GtkWidget *ii_system_entry;
					...
					gpointer data)
{
	...
	/*
	** For IVW, if we're not upgrading we're removing so
	** select instance for removal
	*/
	if ( instmode & IVW_INSTALL && ug_mode&UM_MOD )
	{
	    widget_ptr = lookup_widget( IngresInstall, "remove_inst" );
	    gtk_button_clicked( GTK_BUTTON(widget_ptr) );
	    widget_ptr = lookup_widget( IngresInstall,
						"ivw_rmv_all_files_check" );
                                        ...
                                        gpointer         user_data)
{
	...
	*/
	...
	while ( locptr != NULL )
	{
	    if ( dblocations[locptr->idx]->rfapi_name == II_RF_VWDATA )
	        STprintf( tmpbuf, "\t%s/ingres/%s/vectorwise\n",
			locptr->loc,
	...
	gtk_window_set_title( GTK_WINDOW( warning ), WARN_DIALOG_TITLE );
                                        ...
                                        gpointer         user_data)
{
    ...
    {
	if ( val < vw_cfg_info[i]->value && val > vw_cfg_info[i]->value/2)
	    val=((val + 1) >> 1); /* drop value by at least 1/2 */
    ...
    /* upper limit is 64Mb */
    if ( vw_cfg_info[i]->unit == VWMB &&
	    vw_cfg_info[i]->value > GIP_VWCFG_MAX_BLK_SIZE_MB )
	vw_cfg_info[i]->value = GIP_VWCFG_MAX_BLK_SIZE_MB;
                                        ...
                                        gpointer         user_data)
{
    ...
    if ( unit == VWMB && vw_cfg_info[i]->value > GIP_VWCFG_MAX_BLK_SIZE_MB )
    {
	char	field_buf[100];

        STprintf(field_buf, "%s_val", vw_cfg_info[i]->field_prefix);
        widget_ptr=lookup_widget(IngresInstall, field_buf);
        DBG_PRINT("Setting %s to %d\n", field_buf, GIP_VWCFG_MAX_BLK_SIZE_MB);
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(widget_ptr),
						GIP_VWCFG_MAX_BLK_SIZE_MB);
    }
...
void
on_ivw_cfg_procmem_val_change          (GtkEditable     *editable,
                                        gpointer         user_data)
{
    vw_cfg	*param;
    i4		i=0;
    ...
    /* if field empty, grey out next/back buttons and return */
    next_button=lookup_widget(IngresInstall, "next_button");
    if ( STlen(gtk_entry_get_text( GTK_ENTRY(editable) )) == 0 )
    {
	gtk_widget_set_sensitive( next_button, FALSE );
	return;
    }
    ...
    vw_cfg_info[i]->value = gtk_spin_button_get_value(
					GTK_SPIN_BUTTON(editable));

    DBG_PRINT("%s_value set to = %ld\n", vw_cfg_info[i]->field_prefix,
					vw_cfg_info[i]->value);

    if ( gipCheckMemReq() != OK )
	popup_warning_box(GIP_MEM_REQ_EXCEED_PHYSICAL);
...
void
on_ivw_cfg_buffpoolmem_val_change      (GtkEditable     *editable,
                                        gpointer         user_data)
{
    vw_cfg	*param;
    i4		i=0;
...
void
on_ivw_max_para_toggled                (GtkToggleButton *togglebutton,
                                        gpointer         user_data)
{
    GtkWidget    *widget_ptr;

    widget_ptr=lookup_widget(IngresInstall, "ivw_cfg_max_para_box");
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(togglebutton)))
        gtk_widget_set_sensitive(widget_ptr, TRUE);
    else
    {
	set_ivw_cfg_default(GIP_VWCFG_MAX_PARALLELISM);
        gtk_widget_set_sensitive(widget_ptr, FALSE);
    }
...
void
on_ivw_cfg_max_para_val_change         (GtkSpinButton   *spinbutton,
                                        gpointer         user_data)
{
    vw_cfg	*param;
    i4		i=0;
    ...
    while(vw_cfg_info[i]->bit != GIP_VWCFG_MAX_PARALLELISM)
	i++;

    newval = gtk_spin_button_get_value(spinbutton);

    /* limit to > 0 & <= # cpus on the machine */
    if ( newval < 1 )
	gtk_spin_button_set_value(spinbutton, 1);
    else if ( (gipGetSysCpu(&maxval) == OK) && (newval > maxval) )
	gtk_spin_button_set_value(spinbutton, maxval);
    else
    {
	vw_cfg_info[i]->value = newval;
	DBG_PRINT("%s_value set to = %ld\n",
		vw_cfg_info[i]->field_prefix,
		vw_cfg_info[i]->value);
    }
