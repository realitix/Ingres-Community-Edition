/*
** Copyright 2006, 2010 Actian Corporation. All rights reserved 
*/
...
**	09-Dec-2010 (hanje04)
**	    Mantis 1244
**	    Correct ordering of parameters in UG_SUM_LABEL strings
**	09-Dec-2010 (hanje04)
**	    Mantis 1258
**	    Make sure "install" button is always hidden when entering license
**	    dialog.
**	09-Dec-2010 (hanje04)
**	    Mantis 1263
**	    Do nothing if GIPsetUgAction() called in new install mode
**	14-Dec-2010 (hanje04)
**	    For IVW use II_CONFIG_TYPE=IVW as default config
**	    Update message box to handle "info", "warning" and "error"
**	01-Feb-2011 (hanje04)
...
**	15-Mar-2011 (hanje04)
**	    Mantis 1739
**	    Correct "upgrade string" used for ug_inst_label4 and ug_inst_label6
**	17-Mar-2011 (hanje04)
**	    Remove obsolete function GIPValidatePkgName()
**	17-May-2011 (hanje04)
**	    Mantis 1942
...
**	04-Oct-2011 (hanje04)
**	    Bug 125688
**	    Add II_MTS_JAVA_HOME to response file to pass location of JRE
**	    for remote mananger service (iimgmtsvr)
**	26-Oct-2011 (hanje04)
**	    Correct typo in DBG_PRINT() to display javainfo.home when
**	    generating the response file. %n causes the pointer to be 
**	    written too! Should be %s
**	28-Oct-2011 (hanje04)
**	    Mantis 2830
**	    Add II_MTS_JAVA_HOME to upgrade response file too. MGMTSVC not 
**	    being setup because JRE info is missing.
**	
*/

const char instance_name_descrip[] = N_("Ingres supports multiple instances installed on the same machine.\nEach instance is uniquely identified by an instance name.\n ");
...
write_installation_summary(GtkTextBuffer *buffer)
{
    ...
    */
    ...
    gtk_text_buffer_insert_with_tags_by_name(buffer,
				&iter, 
				_("Installation Location (II_SYSTEM):\n\t"),
				-1,
				"bold",
				NULL);
    gtk_text_buffer_insert_with_tags_by_name(buffer,
	...
	gtk_text_buffer_insert(buffer,
				&iter,
				_("The installation log file location:\n\t"),
				-1);
	...
	if ( pkgs_to_install & PKG_DBMS )
	{
		...
		while(vw_cfg_info[i])
		{
		    ...
		    STprintf( tmpbuf, "%s: ", _(param->descrip));
	    ...
	    gtk_text_buffer_insert_with_tags_by_name(buffer,
						...
						"bold",
						NULL);	
	    gtk_text_buffer_insert(buffer, &iter, _("Log file size: "), -1);
	    STprintf( tmpbuf, "%dMB\n", txlog_info.size );
	    gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter,
						tmpbuf,
						...
						"monospace",
						NULL);
	    STprintf( tmpbuf, "%s: ", _(txlog_info.log_loc.name) );
	    gtk_text_buffer_insert(buffer, &iter, tmpbuf, -1);
	    gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter,
		...
		gtk_text_buffer_insert(buffer,
					&iter,
					_("Dual logging enabled\n"),
					-1 );
		STprintf( tmpbuf, "%s: ", _(txlog_info.dual_loc.name) );
		gtk_text_buffer_insert(buffer, &iter, tmpbuf, -1);
		gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
	    ...
	    if ( ~instmode & IVW_INSTALL)
	    {
	        ...
	        gtk_text_buffer_insert(buffer,
				&iter,
				_("Strict SQL-92 Compliance: "),
				-1);
	        gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter,
						misc_ops & GIP_OP_SQL92 ?
						GIP_STR_YES: GIP_STR_NO,
						-1,
						"monospace",
						NULL);
	        gtk_text_buffer_insert(buffer, &iter, "\n", -1);
	...
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						...
						"bold",
						NULL);	
	gtk_text_buffer_insert(buffer, &iter, _("Timezone: "), -1);
	gtk_text_buffer_insert_with_tags_by_name(buffer,
							&iter, 
	...
	gtk_text_buffer_insert(buffer, &iter, "\n", -1);
	/* character set */
	gtk_text_buffer_insert(buffer, &iter, _("Character Set: "), -1);
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
						locale_info.charset,
						-1,
	...
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						...
						"bold",
						NULL);	
	gtk_text_buffer_insert(buffer, &iter, _("Username: "), -1);
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
						userid,
						-1,
						...
						NULL);
	gtk_text_buffer_insert(buffer, &iter, "\n", -1);
	gtk_text_buffer_insert(buffer, &iter, _("User Group: "), -1);
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
						groupid,
						-1,
	...
	gtk_text_buffer_insert(buffer,
				&iter,
				_("Start instance with computer: "),
				-1);
	gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
...
# ifdef CREATE_DT_FOLDER
	    gtk_text_buffer_insert(buffer,
				&iter,
				_("Create Ingres folder on Desktop: "),
				-1);
	    gtk_text_buffer_insert_with_tags_by_name(buffer,
						&iter, 
...
gen_install_response_file( II_RFAPI_STRING rflocstring )
{
    ...
    /* Config Type */
    DBG_PRINT("ing_config_type.val_idx is set to %d\n", ing_config_type.val_idx );
...
# ifdef conf_BUILD_MGMTSVC
    /* Management tools JRE */
    DBG_PRINT("javainfo.home is set to %s\n", javainfo.home );
    param.name = II_MTS_JAVA_HOME;
    param.value = 
	    (II_RFAPI_STRING)javainfo.home;
    param.vlen = STlen(param.value);
    rfstat = IIrfapi_addParam( &rfhandle, &param );
    if ( rfstat != II_RF_ST_OK )
    {
	DBG_PRINT(
	  "Failed adding II_MTS_JAVA_HOME to response file with error:\n %s\n",
			IIrfapi_errString( rfstat ) );
	if ( rfhandle != NULL )
	    IIrfapi_cleanup( &rfhandle );
        return( rfstat );
...
increment_wizard_progress(void)
{
	...
	switch ( stage_names[current_stage]->stage )
	{
		...
		*/
		...
		STprintf(tmpbuf, UG_SUM_LABEL1_TXT, ugactionstrcc );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_sum_label2");
		STprintf(tmpbuf, UG_SUM_LABEL2_TXT, ugactionstrcc );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_sum_label3");
		STprintf(tmpbuf, UG_SUM_LABEL3_TXT, ugactionstrlc, prodname );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_inst_label4");
		STprintf(tmpbuf, UG_SUM_LABEL4_TXT, prodname, ugactionstrlced );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_inst_label5");
		STprintf(tmpbuf, UG_SUM_LABEL5_TXT, ugactionstrcc );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_inst_label6");
		STprintf(tmpbuf, UG_SUM_LABEL6_TXT, prodname, ugactionstrlced );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall, "ug_inst_label7");
		STprintf(tmpbuf, UG_SUM_LABEL7_TXT, ugactionstrcc );
		gtk_label_set_markup(GTK_LABEL(widget_ptr), tmpbuf);
		widget_ptr=lookup_widget(IngresInstall,
						 "upgrade_install_label");
		gtk_label_set_text(GTK_LABEL(widget_ptr), ugactionstrcc );
	...
	switch ( stage_names[current_stage]->stage )
	{
		...
		gtk_widget_set_sensitive(back_button, TRUE);
		if ( instmode & IVW_INSTALL && instmode & BASIC_INSTALL) 
		{
		    gtk_widget_show(next_button);
		    gtk_widget_hide(install_button);
		}
...
write_upgrade_summary(GtkTextBuffer *buffer)
{
    ...
    /* Title */
    STprintf(tmpbuf, _("%s Summary"), ugactionstrcc );
    gtk_text_buffer_insert_with_tags_by_name( buffer, &iter, tmpbuf,
						-1, "heading", NULL);
    gtk_text_buffer_insert( buffer, &iter, "\n\n", -1 );
    
    /* instance info */
    STprintf(tmpbuf, _("Instance %s:\n\t"), ugactionstrlced );
    gtk_text_buffer_insert_with_tags_by_name( buffer, &iter, tmpbuf,
					    		-1, "bold", NULL );
	...
	/* upgrade info */
	gtk_text_buffer_insert_with_tags_by_name(buffer, &iter,
						_("Upgrade user databases: "),
						-1, "bold", NULL);
	gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, 
					misc_ops & GIP_OP_UPGRADE_USER_DB ?
						GIP_STR_YES : GIP_STR_NO, -1,
						"monospace", NULL);
	gtk_text_buffer_insert(buffer, &iter, "\n\n", -1);
    }
    else
	...
	{
	    gtk_text_buffer_insert_with_tags_by_name(buffer, &iter,
						_("Components removed:\n"),
						-1, "bold", NULL);
	    gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, _("\tNONE"),
							-1, "monospace", NULL);
	    gtk_text_buffer_insert(buffer, &iter, "\n", -1);
	}
	else
...
add_instance_columns(GtkTreeView *treeview)
{
    ...
    column=gtk_tree_view_column_new_with_attributes( _("Instance Name"),
							renderer,
							"text",
							COL_NAME,
							NULL);
					  ...
					  GtkTreeIter *dflt_iter )
{
    ...
    gtk_window_set_title( GTK_WINDOW( dialog ), _("Select a value") );
...
GIPvalidateLocs( char *iisystem )
{
    ...
    */
    ...
    /* vectorwise data */
    if ( STcompare( dblocations[INST_II_DATABASE]->path,
...
** Description:
**
**	Display the correct items in the train and the correct action names
**	(upgrade, modify, remove) in the text.
**
** Inputs:
**	
**	None
...
**	    Created.
**	09-Dec-2010 (hanje04)
**	    Do nothing if called in new install mode
*/
void
GIPsetUgAction()
{
    ...
    if ( ug_mode & UM_INST )
	return; /* don't do anything for new install mode */
    ...
    if ( ug_mode & UM_UPG )
    {
	ugactionstrlc=_("upgrade");
	ugactionstrlced=_("upgraded");
	ugactionstrcc=_("Upgrade");
	ugactionstr_cc=_("_Upgrade");

	gtk_widget_show(ug_install_table);
	gtk_widget_hide(ug_mod_table);
	gtk_widget_show(inst_sel_table);
    }
    else if ( ug_mode & UM_RMV )
    {
	ugactionstrlc=_("remove");
	ugactionstrlced=_("removed");
	ugactionstrcc=_("Remove");
	ugactionstr_cc=_("_Remove");

	gtk_widget_show(ug_install_table);
	gtk_widget_show(ug_mod_table);
	gtk_widget_show(inst_sel_table);

	/* no modify for IVW, just remove */
	if ( instmode & IVW_INSTALL )
	    gtk_widget_hide(ug_mod_table);
	else
	    gtk_widget_show(ug_mod_table);
    ...
    {
	ugactionstrlc=_("modify");
	ugactionstrlced=_("modified");
	ugactionstrcc=_("Modify");
	ugactionstr_cc=_("_Modify");
    ...
    gtk_label_set_text(GTK_LABEL(upgrade_install_label), ugactionstrcc);
}
