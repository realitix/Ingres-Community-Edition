...
**	22-Nov-2010 (hanje04)
**	    Add prototype for GIPsetUgAction()
**	10-Dec-2010 (hanje04)
**	    Add "info" popup box.
**	16-Mar-2011 (hanje04)
**	    Remove *_DIALOG_TITLE defines, moved to gipmsg.h
**	16-May-2011 (hanje04)
**	    II_SYSTEM is different for DEBs, set appropriately for DEBs
**	    support
*/
...
# define SET_II_SYSTEM(p)	{ \
		...
		STcopy( "/opt/Ingres/"SYSTEM_PRODUCT_NAME, DEBDefaultII_SYSTEM ); \
		STcopy( RFAPI_WIN_DEFAULT_INST_LOC, WinDefaultII_SYSTEM ); \
		p = (new_pkgs_info.pkgfmt == PKGDEB) ? \
			DEBDefaultII_SYSTEM : \
			LnxDefaultII_SYSTEM ; }
# define TMPDIR "/tmp"
...
# define popup_info_box(msg)		popup_message_box( INFO_DIALOG_TITLE, msg)
