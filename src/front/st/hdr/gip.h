/*
** Copyright (c) 2006, 2010 Actian Corporation. All rights reserved
*/
...
**	01-Jun-2010 (hanje04)
**	    Mantis 741
**	    Update UG_SUM messages take an argument for product name
**	    rather than hard coded 'Ingres'.
**	14-Jul-2010 (hanje04)
**	    BUG 124081
...
**	09-Dec-2010 (hanje04)
**	    Mantis 1244
**	    Correct parameters in UG_SUM_LABEL strings
**	28-Jan-2011 (hanje04)
**	    Mantis 1303, 1348 & 1434
**	    Add santiy checking and upper limits to VW config parameters
**	    Move typedefs to gipdata.h, where they really belong. Had to 
**	    move some stuff around fix compilation issues with the new
**	    gipsysconf.c module.
**	07-Feb-2011 (hanje04)
**	    Wrap UG_SUM messages in _() for native language support.
**	16-Mar-2011 (hanje04)
**	    Mantis 1726 & 1739
**	    Swap UG_SUM_LABEL5_TXT & 6 after natvite language support changes.
**	    Messages are messed up in the installer
**	12-May-2011 (hanje04)
**	    Replace format with pkgfmt and pkgdir in saveset to add support
**	    for DEBs installs
**	20-Jun-2011 (hanje04)
**          Mantis 2135 & 2138
**	    Allow installation of signed saveset even if it can't be verified.
**	    Add "checksig" to saveset structure and RPM_ALLOW_UNAUTH
**	22-Jun-2011 (hanje04)
**	    Mantis 2146 & 2017
**	    Use apt-get "install" to perform debs upgrade to prevent
**	    a full system update.
**	06-Oct-2011 (hanje04)
**	    Bug 125688
**	    Define MGMT*_CMD for starting, stopping and configuring 
...
#define INSTALL_LOGFILE_NAME "/ingres/files/install.log"
...
#define RPM_ALLOW_UNAUTH "--nosignature "
#define DEB_ALLOW_UNAUTH "--allow-unauthenticated "
#define DEB_INSTALL_CMD "apt-get install -V -y -q --no-upgrade "DEB_ALLOW_UNAUTH
#define DEB_UPGRADE_CMD "apt-get install -V -y -q "DEB_ALLOW_UNAUTH
#define DEB_REMOVE_CMD "apt-get remove -V -y -q "
#define DEB_HIDE_PROGRESS "| sed -u -e '/Reading database/d' "
#define RM_CMD "rm -rf"
...
#define MGMTSTART_CMD "/etc/init.d/iimgmtsvc%s start"
#define MGMTSTOP_CMD "/etc/init.d/iimgmtsvc%s stop > /dev/null 2>&1"
#define MGMTSTATUS_CMD "/etc/init.d/iimgmtsvc%s status > /dev/null 2>&1"
#define MGMTCONFIG_CMD "/etc/init.d/iimgmtsvc%s configure %s"
#define OUTPUT_READSIZE 5
#define IVWPRODNAME "VectorWise"
#define INGPRODNAME "Ingres"
...
#define UG_MOD_UPG_TXT _("The following instances have been detected, please select the instance\nyou wish to %s")
#define UG_SUM_LABEL1_TXT _("<b>Ready to %s</b>")
#define UG_SUM_LABEL2_TXT _("Click <b>%s</b> to begin the installation")
#define UG_SUM_LABEL3_TXT _("You are now ready to %s your %s instance.")
#define UG_SUM_LABEL4_TXT _("<b>Please wait while the %s instance is %s</b>")
#define UG_SUM_LABEL5_TXT _("<b>%s Complete</b>")
#define UG_SUM_LABEL6_TXT _("%s has been successfully %s")
#define UG_SUM_LABEL7_TXT _("<b>%s Failed</b>")
...
#define VECTORWISE_SERVER (PKG_CORE|PKG_DBMS|PKG_NET)
#define VECTORWISE_CLIENT (PKG_CORE|PKG_NET)
...
/* package formats for saveset.pkgfmt */
typedef enum _PKGFMT {
	PKGRPM,
	PKGDEB,
	PKGUDEF
} PKGFMT ;
