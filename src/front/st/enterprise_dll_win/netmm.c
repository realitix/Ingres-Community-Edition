...
/*
**  Name:ingres_confirm_noUTF8
**
**  Description:
**	Pops up a message to the user confirming they want to choose 
**	character set other than UTF8.  If UTF8 is not chosen for VW
**	installation, the client will not be able to connect to VectorWise
**	installation.
**
**  Inputs:
**	hInstall	Handle to the installation.
...
*/
UINT __stdcall
ingres_confirm_noUTF8(MSIHANDLE hInstall)
{
	char buffer[1000];
	BOOL bAnswer = FALSE;
	char prop[10]="YES";

	sprintf(buffer, "If this client will be connecting to a UTF8 installation (such as VectorWise), the character set must be set to UTF8.\r\n\r\nContinue anyway?");
	
	bAnswer = AskUserYN(hInstall, buffer);
	if (bAnswer)
		sprintf(prop, "YES");
	else
		sprintf(prop, "NO");
	MsiSetProperty(hInstall, "INGRES_CONTINUE", prop);	
