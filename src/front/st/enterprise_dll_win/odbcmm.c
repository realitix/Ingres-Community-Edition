...
**  20-May-2011 (drivi01)
**		Optimize this function to install ODBC driver for DBA Tools package.
*/
...
ingres_set_odbc_reg_entries (MSIHANDLE ihnd)
{
    ...
    char szCode[3], szComponent[39], szProduct[39], szBuf[MAX_PATH];
    ...
    dwSize = sizeof(tchValue);
    if ((MsiGetProperty (ihnd, "DBATools", tchValue, &dwSize) == ERROR_SUCCESS) && strcmp(tchValue, "1")==0)
	isDBATools = TRUE;

    if ((!isDBATools && !MsiGetComponentState(ihnd, "Bin.70DC58B6_2D77_11D5_BDFC_00B0D0AD4485", &iInstalled, &iAction)) ||
	(isDBATools && !MsiGetComponentState(ihnd, "ODBCBin", &iInstalled, &iAction)))
    {
	...
	*/
	...
	else
	{
	    ...
	    sprintf(szBuf, "%s_Database_%s", SYSTEM_SERVICE_NAME, szCode);
