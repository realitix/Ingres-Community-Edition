...
**	14-Nov-2010 (drivi01)
**	    Add VectorWise data directory registry setting.
**	    Add ingresvw_get_cpus to retrieve number of CPUs on the machine,
**	    ingresvw_get_memory to get total memory on the machine to set
**	    process memory and buffer pool memory parameters for IVW.
**	    Also, add ingresvw_update_config_property to put the
**	    data and memory values and units in the format expected
**	    by vectorwise.conf.
**	    Retrieve new IVW values from msi and add them to the registry.
**	    The values include: "vectorwise", "vwdatadir", "vwdatablocksize",
**	    "vwdatablockgroupsize", "vwprocmemory", "vwbuffpoolmem", 
**	    "vwmaxparallelthreads".
*/
...
ingres_set_dbms_reg_entries (MSIHANDLE ihnd)
{
    ...
    */
    ...
    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "VW_DATADIR", tchValue,
			&dwSize ) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
	...
	status = RegSetValueEx( hKey, "vwdatadir", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }
    ...
    dwSize = sizeof(tchValue);
    if (status || MsiGetProperty(ihnd, "VW_DATABLOCK_SIZE", tchValue, 
				&dwSize) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
    ...
    {
	status = RegSetValueEx( hKey, "vwdatablocksize", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status || MsiGetProperty(ihnd, "VW_DATABLOCK_GROUP_COMBO", tchValue, 
				&dwSize) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
    ...
    {
	status = RegSetValueEx( hKey, "vwdatablockgroupsize", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status || MsiGetProperty(ihnd, "VW_PROC_MEMORY_JOIN", tchValue, 
				&dwSize) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
    ...
    {
	status = RegSetValueEx( hKey, "vwprocmemory", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status || MsiGetProperty(ihnd, "VW_BUFFPOOL_MEMORY_JOIN", tchValue, 
				&dwSize) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
    ...
    {
	status = RegSetValueEx( hKey, "vwbuffpoolmem", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }

    dwSize = sizeof(tchValue);
    if (status || MsiGetProperty(ihnd, "VW_MAX_PARALLELISM", tchValue, 
				&dwSize) != ERROR_SUCCESS)
    {
	return (ERROR_INSTALL_FAILURE);
    }
    ...
    {
	status = RegSetValueEx( hKey, "vwmaxparallelthreads", 0, REG_SZ, tchValue,
				strlen(tchValue) + 1 );
    }
...
ingres_rollback_dbms(MSIHANDLE hInstall)
{
	...
	RegDeleteValue(hkRegKey, "vectorwise");
	RegDeleteValue(hkRegKey, "vwdatadir");
	RegDeleteValue(hkRegKey, "vwdatablocksize");
	RegDeleteValue(hkRegKey, "vwdatablockgroupsize");
	RegDeleteValue(hkRegKey, "vwprocmemory");
	RegDeleteValue(hkRegKey, "vwbuffpoolmem");
	RegDeleteValue(hkRegKey, "vwmaxparallelthreads");
...
/*
**  ingresvw_update_config_property
**
**  Description:
**	For Ingres VectorWise specific configurations that include units in the
**	combo boxes, this custom action will concatenate the size and unit together.
**
**  Inputs:
**	hInstall	Handle to the installation.
**
...
*/
UINT __stdcall
ingresvw_update_config_property(MSIHANDLE hInstall)
{
	char block_size[MAX_PATH];
	char block_size_unit[MAX_PATH];
	char proc_mem[MAX_PATH];
	char proc_mem_unit[MAX_PATH];
	char buff_pool_mem[MAX_PATH];
	char buff_pool_mem_unit[MAX_PATH];
	DWORD dwSize = 0, dwSize2 = 0;
	BOOL bRet = ERROR_SUCCESS;

	dwSize = sizeof(block_size);
	dwSize2 = sizeof(block_size_unit);
	if (MsiGetProperty(hInstall, "VW_DATABLOCK_SIZE_COMBO", &block_size, &dwSize) == ERROR_SUCCESS &&
		MsiGetProperty(hInstall, "VW_DATABLOCK_SIZE_UNIT", &block_size_unit, &dwSize2) == 
ERROR_SUCCESS)
	{
		strcat(block_size, block_size_unit);
		MsiSetProperty(hInstall, "VW_DATABLOCK_SIZE", block_size);
	}

	dwSize = sizeof(proc_mem);
	dwSize2 = sizeof(proc_mem_unit);
	if (MsiGetProperty(hInstall, "VW_PROC_MEMORY", &proc_mem, &dwSize) == ERROR_SUCCESS &&
		MsiGetProperty(hInstall, "VW_PROC_MEMORY_UNIT", &proc_mem_unit, &dwSize2) == ERROR_SUCCESS)
	{
		strcat(proc_mem, proc_mem_unit);
		MsiSetProperty(hInstall, "VW_PROC_MEMORY_JOIN", proc_mem);
	}

	dwSize = sizeof(buff_pool_mem);
	dwSize2 = sizeof(buff_pool_mem_unit);
	if (MsiGetProperty(hInstall, "VW_BUFFPOOL_MEMORY", &buff_pool_mem, &dwSize) == ERROR_SUCCESS &&
		MsiGetProperty(hInstall, "VW_BUFFPOOL_MEMORY_UNIT", &buff_pool_mem_unit, &dwSize2) == 
ERROR_SUCCESS)
	{
		strcat(buff_pool_mem, buff_pool_mem_unit);
		MsiSetProperty(hInstall, "VW_BUFFPOOL_MEMORY_JOIN", buff_pool_mem);
	}
...
**
**  Description:
**	For Ingres VectorWise specific configurations, retrieves system
**      memory to calculate 50% for process memory configuration and
**      25% for buffer pool memory.  Contains routines for calculating
**      units to represent memory.
**
**  Inputs:
**	hInstall	Handle to the installation.
**
...
*/
UINT __stdcall
ingresvw_get_memory(MSIHANDLE hInstall)
{
# define	DIV	1024
# define	HALF	0.5
# define	QUARTER	0.25
	...
	DWORDLONG physMem = 0;
	double procMem = 0;
	DWORDLONG lProcMem = 0;
	char sProcMem[MAX_PATH];
	double buffPool = 0;
	DWORDLONG lBuffPool = 0;
	char sBuffPool[MAX_PATH];
	char *mem_units[] = {"K", "M", "G", 0};
	BOOL bRet = ERROR_INSTALL_FAILURE;
	DWORD dwRet = 0;
	int i = 0;
	...
	{
		//Processing memory should default to 50% of total memory
		procMem = (double)memstat.ullTotalPhys*HALF;
		procMem /= DIV; //convert memory to KB right away
		while ((procMem/DIV) >= 1 && mem_units[i+1])
		{
			procMem /= DIV;
			i++;
		}
		sprintf(sProcMem, "%.0f", procMem);
		dwRet = MsiSetProperty(hInstall, "VW_PROC_MEMORY", sProcMem);
		if (dwRet)
			dwRet = GetLastError();

		MsiSetProperty(hInstall, "VW_PROC_MEMORY_UNIT", mem_units[i]);
		strcat(sProcMem, mem_units[i]);
		MsiSetProperty(hInstall, "VW_PROC_MEMORY_JOIN", sProcMem);
		
		//reset units and counter
		i=0;

		//Buffer pool memory should default to 25% of total memory
		buffPool = (double)memstat.ullTotalPhys*QUARTER;
		buffPool /= DIV; //convert memory to KB right away
		while (( buffPool / DIV) >= 1 && mem_units[i+1])
		{
			buffPool /= DIV;
			i++;
		}
		sprintf(sBuffPool, "%.0f", buffPool);
		MsiSetProperty(hInstall, "VW_BUFFPOOL_MEMORY", sBuffPool );
		MsiSetProperty(hInstall, "VW_BUFFPOOL_MEMORY_UNIT", mem_units[i]);
		strcat( sBuffPool, mem_units[i]);
		MsiSetProperty(hInstall, "VW_BUFFPOOL_MEMORY_JOIN", sBuffPool);
...
**
**  Description:
**	For Ingres VectorWise specific configurations, retrieves number CPUs 
**      on the machine for parallelism setting in vectorwise.conf. 
**
**  Inputs:
**	hInstall	Handle to the installation.
**
...
*/
UINT __stdcall
ingresvw_get_cpus(MSIHANDLE hInstall)
{
	SYSTEM_INFO sysinfo;
	char cpus[MAX_PATH];
	...
	if (sysinfo.dwNumberOfProcessors && 
		MsiSetProperty(hInstall, "VW_MAX_PARALLELISM", _itoa(sysinfo.dwNumberOfProcessors, cpus, 10)) == ERROR_SUCCESS)
	{
		dwRet = ERROR_SUCCESS;
	}
