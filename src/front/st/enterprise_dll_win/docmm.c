...
**	25-Feb-2011 (drivi01)
**	    Adopt this function for VectorWise Documentation
**	    installer.  Only 1 guide is shipped right now,
**	    it isn't a Bookshelf.  Update the function to check
**	    for LAUNCH_USERGUIDE_NAME property first to get the
**	    guide name.  If property doesn't exist then default 
**	    to Bookshelf.
**	03-Mar-2011 (drivi01)
**	    To bring up bookshelf or user guide using Adobe,
**	    the registry check for Adobe should check 32-bit
**	    registry location on 64-bit Windows.
*/
UINT __stdcall
ingres_start_bookshelf(MSIHANDLE ihnd)
{
	...
	dwSize = sizeof(szPdf);
	MsiGetProperty(ihnd, "LAUNCH_USERGUIDE_NAME", szPdf, &dwSize);
	...
	if (_access(szPath, 00) == 0)
	{
		...
		{	
			...
			if (rc == ERROR_SUCCESS)
			{
				...
				sprintf(szBuf2, "SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\%s", subKey);
				rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szBuf,  0, KEY_ALL_ACCESS, &hKey );
				if (rc == ERROR_FILE_NOT_FOUND)
				{
					if (hKey)
						RegCloseKey(hKey);
					rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szBuf2,  0, KEY_ALL_ACCESS, &hKey );
				}
				if (rc == ERROR_SUCCESS)
				{
			...
			{
				if (*szPath)
					sprintf(szBuf, "\"%s\\%s\" \"%s\\%s\"", acrord, subKey, szPath, szPdf);
				else
