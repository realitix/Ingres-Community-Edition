/*
**  Copyright (c) 2001-2009 Actian Corporation.
*/
...
**          file. bug 124503.
**	10-Jun-2011 (drivi01)
**	    Rename Ingres Documentation component to Documentation component
**	    to accomodate summary dialog for both Ingres and VectorWise
**	    products.
**	29-Sep-2011 (drivi01)
**	    Generate Windows service name using SYSTEM_SERVICE_NAME variable defined
**	    in commonmm.h instead of hard-coding the service name.
**	04-Oct-2011 (drivi01)
**	    Add ingres_check_JDK function to check the minimum version of JRE
**	    is installed on the machine and to set JAVA_HOME to that version.
**	    If correction version (min. JRE 1.6.0_14) is not installed then
**	    bring up download page if user chooses to download now or continue
**	    without JRE just don't attempt to start the new Management service.
*/
#include <windows.h>
#include <msi.h>
#include <msiquery.h>
...
**	    Created.
**  10-Dec-2010 (drivi01)
**	    Add VW_DATADIR for Ingres VectorWise data
**	    to be verified here.	  
*/
UINT __stdcall 
ingres_verify_paths(MSIHANDLE hInstall)
{
	...
	nret = sizeof(buf);
	if ((MsiGetProperty(hInstall, "IVW", buf, &nret) == ERROR_SUCCESS) && buf[0])
		isIVW = TRUE;
    ...
    if (!isIVW) i=1; //skip VW_DATADIR if this isn't IVW install
...
**  10-Dec-2010 (drivi01)
**	    Add II_VWDATA parameter to be processed from the response file.
**	    Also add routines to handle new VectorWise Configuration section with
**	    five new values: "II_VWCFG_MAX_MEMORY", "II_VWCFG_BUFFERPOOL", 
**	    "II_VWCFG_BLOCK_SIZE", "II_VWCFG_GROUP_SIZE", "II_VWCFG_MAX_PARALLELISM".
**	    
*/
UINT __stdcall 
...
#ifdef conf_IVW
	sharedDiskCount=0;
	GetPrivateProfileString("Ingres Locations", "II_VWDATA", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if(strlen(lpReturnedString)==0)
		GetPrivateProfileString("User Defined Properties", "II_VWDATA", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if(lpReturnedString[0] && VerifyPath(lpReturnedString))
	{
		MsiSetProperty(hInstall, "VW_DATADIR", lpReturnedString);
		...
		MsiSetProperty(hInstall, "VW_DATADIR", strInstallPath);
#endif /* conf_IVW */
...
#ifdef conf_IVW
	/*IVW Configurations */
	GetPrivateProfileString("Vectorwise Configuration", "II_VWCFG_MAX_MEMORY", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (strlen(lpReturnedString) == 0)
		GetPrivateProfileString("User Defined Properties", "II_VWCFG_MAX_MEMORY", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (lpReturnedString[0])
		MsiSetProperty(hInstall, "VW_PROC_MEMORY_JOIN", lpReturnedString);

	GetPrivateProfileString("Vectorwise Configuration", "II_VWCFG_BUFFERPOOL", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (strlen(lpReturnedString) == 0)
		GetPrivateProfileString("User Defined Properties", "II_VWCFG_BUFFERPOOL","", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (lpReturnedString[0])
		MsiSetProperty(hInstall, "VW_BUFFPOOL_MEMORY_JOIN",lpReturnedString);

	GetPrivateProfileString("Vectorwise Configuration", "II_VWCFG_BLOCK_SIZE", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (strlen(lpReturnedString) == 0)
		GetPrivateProfileString("User Defined Properties", "II_VWCFG_BLOCK_SIZE", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (lpReturnedString[0])
		MsiSetProperty(hInstall, "VW_DATABLOCK_SIZE", lpReturnedString);	

	GetPrivateProfileString("Vectorwise Configuration", "II_VWCFG_GROUP_SIZE", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (strlen(lpReturnedString) == 0)
		GetPrivateProfileString("User Defined Properties", "II_VWCFG_GROUP_SIZE", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (lpReturnedString[0])
		MsiSetProperty(hInstall, "VW_DATABLOCK_GROUP_COMBO", lpReturnedString);

	GetPrivateProfileString("Vectorwise Configuration", "II_VWCFG_MAX_PARALLELISM", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (strlen(lpReturnedString) == 0)
		GetPrivateProfileString("User Defined Properties", "II_VWCFG_MAX_PARALLELISM", "", lpReturnedString, sizeof(lpReturnedString), strRSPFile);
	if (lpReturnedString[0])
		MsiSetProperty(hInstall, "VW_MAX_PARALLELISM", lpReturnedString);
#endif /* if conf_IVW */	
...
**	    Created.
**	Nov-11-2010 (drivi01)
**		Retrieve "IVW" property here to be able to set
**	    custom path if needed for Ingres VectorWise installation.
**	    Set INSTALLDIR property here as well.
**
*/
UINT __stdcall 
ingres_set_targetpath(MSIHANDLE hInstall) 
{
    ...
    dwSize = sizeof(szBuf);
    if (MsiGetProperty(hInstall, "IVW", szBuf, &dwSize) == ERROR_SUCCESS && szBuf[0])
	isIVW = TRUE;
    ...
    MsiSetTargetPath(hInstall, "INSTALLDIR", szII_SYSTEM);
...
ingres_start_ingres(MSIHANDLE hInstall)
{
    ...
    if (schSCManager)
    {
	sprintf(szBuf, "%s_Database_%s", SYSTEM_SERVICE_NAME, szId);
...
/*
**  Name: replace_config_token
**
**  Description: This function is a helper function for ingres_addsummary
**					function and is meant to search default string in the RTF
**					file and replace it with the new configuration value.
**					This function is here to help reduce a number of repeated 
**					lines in ingres_addsummary.
**
**
**  History:
**	  10-Dec-2010 (drivi01)
...
*/
void
replace_config_token(char *pathBuf, char *cfg_param, char *default_val, char *txt_buf)
{
	char *txt_buf_head = NULL, *txt_buf_tail = NULL, *txt_buf_tmp = NULL;
	u_char *point, *point2;
	unsigned int length;
	size_t szBuf = 0;
	
	szBuf = strlen(txt_buf);
	...
	if (strcmp(pathBuf, default_val) !=0 )
	{
			if ((point=strstr(txt_buf, cfg_param)) != NULL)
			{
				point = point+strlen(cfg_param)+1;
				point2 = strstr(point, default_val);
				if (point2 != NULL)
				{
				...
				point=point+strlen(default_val);
				strncpy(txt_buf_tail, point, _msize(txt_buf)-length-strlen(default_val)+1);
				if (strlen(pathBuf) > strlen(default_val))
				{
					length = strlen(txt_buf)+strlen(pathBuf)-strlen(default_val);
	...
	if (txt_buf_tail)
		free(txt_buf_tail);
	txt_buf_head = txt_buf_tail = NULL;
...
**	15-Nov-2010 (drivi01)
**	    Update Ingres VectorWise Data Location in the summary.
**	    Take some of the repeatative routines into replace_config_token
**	    function to make code easier to read.
**	    Add summary manipulations for Ingres VectorWise new parameters.
**		The five parameters are:
**			IVW Data Block Size
**			IVW Data Block Group Size
**			IVW Processing Memory
**			IVW Buffer Pool Memory
**			IVW Maximum Parallel Threads
**	01-Aug-2011 (drivi01)
**	    Change "Include Ingres in PATH..." for VectorWise to say
**	    "Include VectorWise in PATH...".
*/
UINT __stdcall
ingres_addsummary(MSIHANDLE hInstall)
{
	...
	u_char *point, *point2;
	HANDLE	handle;
	char	IngresDBMS[] = {"Database Server and Tools"};
	...
	char	*component_names[] = {"Database Server and Tools", "Ingres Client", "Ingres Replicator", 
					"Ingres .NET Data Provider", "Ingres Querying and Reporting Tools",
					"Ingres Star", "Documentation", "\0"};
	...
	char	vwdatalocation[] = {"VectorWise Data Location:"};
	...
	char	data_block_size[] = {"IVW Data Block Size:"};
	char	data_block_group_size[] = {"IVW Data Block Group Size:"};
	char	proc_mem[] = {"IVW Processing Memory:"};
	char	buff_pool_mem[] = {"IVW Buffer Pool Memory:"};
	char	max_parallelism[] = {"IVW Maximum Parallel Threads:"};
	...
	char	vwingresinpath_string[] = {"Include VectorWise in PATH, INCLUDE and LIB:"};
	...
	char	default_log_size[] = {"512"};
	char	default_data_block_size[] = {"512KB"};
	char	default_data_block_group_size[] = {"8"};
	char	default_proc_mem[] = {"1G"};
	char	default_buff_pool_mem[] = {"512M"};
	char	default_max_parallelism[] = {"2"};
	...
	dwSize = sizeof(buff);
	if (MsiGetProperty(hInstall, "IVW", &buff, &dwSize) == ERROR_SUCCESS && buff[0])
		isIVW = TRUE;
	...
	*/
	...
	if (isIVW)
	{
	//Update VectorWise Data Location
	dwSize=sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_DATADIR", pathBuf, &dwSize);
	path = duplicate_slashes(pathBuf);
	strncpy(pathBufD, path, strlen(path)+1);
	path=(char *)&pathBufD;
	length = szBuf;
	...
	replace_location_string(path, vwdatalocation, &txt_buf, txt_buf_head, txt_buf_tail, length);
	}
	...
	if (strcmp(pathBuf, default_instance) != 0)
	{
		if ((point=strstr(txt_buf, default_instance)) != NULL)
		{
			...
			if (isIVW)
			{
				length = (point+2) - txt_buf;
				strncpy(txt_buf_head, txt_buf, length);
				txt_buf_head[length] = '\0';
				strncpy(txt_buf_tail, point+sizeof(default_instance)-1, _msize(txt_buf)-length-sizeof(default_instance)+1);
				sprintf(txt_buf, "%s%s%c", txt_buf_head, txt_buf_tail, '\0');
			}
		}			
	}
	...
	replace_config_token(pathBuf, logsize_string, default_log_size, txt_buf);

	if (isIVW)
	{
	//Ingres VectorWise specific configurations
	//Update Data Block Size
	dwSize=sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_DATABLOCK_SIZE", pathBuf, &dwSize);
	replace_config_token(pathBuf, data_block_size, default_data_block_size, txt_buf);

	//Update Data Block Group Size
	dwSize = sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_DATABLOCK_GROUP_COMBO", pathBuf, &dwSize);
	replace_config_token(pathBuf, data_block_group_size, default_data_block_group_size, txt_buf);

	//Update Processing Memory
	dwSize = sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_PROC_MEMORY_JOIN", pathBuf, &dwSize);
	replace_config_token(pathBuf, proc_mem, default_proc_mem, txt_buf);

	//Update Buffer Pool Memory
	dwSize = sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_BUFFPOOL_MEMORY_JOIN",pathBuf, &dwSize);
	replace_config_token(pathBuf, buff_pool_mem, default_buff_pool_mem, txt_buf);

	//Update Maximum Parallelism 
	dwSize = sizeof(pathBuf);
	MsiGetProperty(hInstall, "VW_MAX_PARALLELISM",pathBuf, &dwSize);
	replace_config_token(pathBuf, max_parallelism, default_max_parallelism, txt_buf);
	}

	//Update Time Zone
	dwSize=sizeof(pathBuf);
	MsiGetProperty(hInstall, "INGRES_TIMEZONE", pathBuf, &dwSize);
	replace_config_token(pathBuf, timezone_string, default_timezone, txt_buf);
	
	
	//Update Character Set
	dwSize=sizeof(pathBuf);
	MsiGetProperty(hInstall, "INGRES_CHARSET", pathBuf, &dwSize);
	replace_config_token(pathBuf, charset_string, default_charset, txt_buf);
	...
	if (isIVW)
	{
	//Update "Include VectorWise in PATH, INCLUDE and LIB" parameter
	dwSize=sizeof(pathBuf);
	MsiGetProperty(hInstall, "INGRES_INCLUDEINPATH", pathBuf, &dwSize);
	if (strcmp(pathBuf, "1") !=0 )
	{
			if ((point=strstr(txt_buf, vwingresinpath_string)) != NULL)
			{
				char *point2;
				point = point+sizeof(vwingresinpath_string);
...
**     28-Feb-2011 (drivi01)
**         On 64-bit Windows, registry check will not pick
**         up documentation installation in the registry
**         if it is installed. Added a check in 32-bit
**         location of the registry for documentation
**         install.
*/
UINT __stdcall 
ingres_install_documentation(MSIHANDLE hInstall)
{
	...
	int idx;
	char guid[MAX_PATH], buf[MAX_PATH+1], cmd[MAX_PATH+1], cmd2[MAX_PATH+1], cmd3[MAX_PATH+1], installdir[MAX_PATH+1];
	HKEY hRegKey;
	WIN32_FIND_DATA	wfd;
	HANDLE handle;
	...
	*buf=0; dwSize=sizeof(buf);
	...
	sprintf(cmd3, "SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\%s", guid);
	...
	if (!RegOpenKeyEx(HKEY_LOCAL_MACHINE, cmd2, 0, KEY_QUERY_VALUE, &hRegKey) ||
		!RegOpenKeyEx(HKEY_LOCAL_MACHINE, cmd3, 0, KEY_QUERY_VALUE, &hRegKey))
	{
	     bUpgrade=1;
	     RegCloseKey(hRegKey);
	}
...
**	18-Feb-2011 (drivi01)
**	    On 64-bit machines when checking if .NET Data Provider
**	    already installed on the machine, check both 32-bit
**	    and 64-bit locations on Windows.
**	    On 64-bit machine, the registry location for .NET Data
**	    Provider will be different than on 32-bit machine.
*/
UINT __stdcall 
ingres_install_dotnet(MSIHANDLE hInstall)
{
	...
	DWORD dwSize;
	int idx;
	char guid[MAX_PATH], buf[MAX_PATH+1], cmd[MAX_PATH+1], cmd2[MAX_PATH+1], cmd3[MAX_PATH+1], installdir[MAX_PATH];
	HKEY hRegKey;
	WIN32_FIND_DATA	wfd;
	HANDLE handle;
	int bUpgrade=0;
	...
	*buf=0; dwSize=sizeof(buf);
	...
	if (!bUpgrade && !RegOpenKeyEx(HKEY_LOCAL_MACHINE, cmd3, 0, KEY_QUERY_VALUE, &hRegKey))
	{
	    bUpgrade=1;
	    RegCloseKey(hRegKey);
	}
...
/*
**  Name:ingresvw_verify_vwparams
**
**  Description:
**	Verify memory, buffer pool, and parallelism parameters for Ingres
**  VectorWise entered by the user are integers to avoid writing
**	bogus information to vectorwise.conf.
**
**  Inputs:
**	hInstall	Handle to the installation.
**
...
*/
UINT __stdcall
ingresvw_verify_vwparams(MSIHANDLE hInstall)
{
	char proc_mem[MAX_PATH], buffpool_mem[MAX_PATH], threads[MAX_PATH];
	DWORD dwSize = 0, dwSize2 = 0;
	int i = 0;
	BOOL bError = FALSE;

	dwSize = sizeof(proc_mem);
	if (MsiGetProperty(hInstall, "VW_PROC_MEMORY", &proc_mem, &dwSize) == ERROR_SUCCESS )
	{
		for (i=0; i < strlen(proc_mem); i++)
			if (!isdigit(proc_mem[i]))
			{
				bError = TRUE;
				break;
			}
	...
	dwSize = sizeof(buffpool_mem);
	if (!bError && MsiGetProperty(hInstall, "VW_BUFFPOOL_MEMORY", &buffpool_mem, &dwSize) == ERROR_SUCCESS )
	{
		for (i=0; i < strlen(buffpool_mem); i++)
			if (!isdigit(buffpool_mem[i]))
			{
				bError = TRUE;
				break;
			}
	...
	dwSize = sizeof(threads);
	if (!bError && MsiGetProperty(hInstall, "VW_MAX_PARALLELISM", &threads, &dwSize) == ERROR_SUCCESS )
	{
...
/*
**  Name:IsMinimumJREVersion
**
**  Description:
**	Called from ingres_check_JDK.  Determines if the version of the JDK
**	passed to this function is at least the minimum version we're looking
**	for.
**
**  Inputs:
**	hInstall	Handle to the installation.
...
**	None.
**	Returns:
**	    TRUE	if version is higher or equal to 1.6.0_14
**	    FALSE	if version is lower than 1.6.0_14
**
**  Side Effects:
**	None.
**
...
*/
BOOL 
IsMinimumJREVersion(char *vers)
{
	const int minmaj=1;
	const int minmin=6;
	const int minbuild=14;  //Minimum JRE version is 1.6.0_14
	BOOL hasValidJRE = FALSE;

	if (vers && *vers)
	{
		int maj = 0, min = 0, build = 0;
		if (vers) sscanf(vers, "%d", &maj);
		if (vers) vers = strchr(vers, '.');
		if (vers) sscanf(++vers, "%d", &min);
		if (vers) vers = strchr(vers, '.');
		if (vers) vers = strchr(vers, '_');
		if (vers) sscanf(++vers, "%d", &build);
		if (maj > minmaj)
			hasValidJRE = TRUE;
		else if (maj == minmaj)
		{
...
/*
**  Name:ingres_check_JDK
**
**  Description:
**	Find minimum version of JRE and set JAVA_HOME property to the
**	correct version.  If correct version is not found then the user gets
**	a message box stating that, they're given a choice to download the
**	latest JRE or continue without one and install at a later time.
**	post installer will not start a Remote Manager Service wtihout JRE. 
**
**  Inputs:
**	hInstall	Handle to the installation.
**
...
**	    Created.
**	17-Oct-2011 (drivi01)
**	    Update the error message about missing JRE to specify if it needs
**	    64-bit or 32-bit version of JRE.  Current message is creation 
**	    confusion if user does have a later version of JRE but it isn't
**	    being picked up.  The new clarification will help with this 
**	    confusion.
*/
UINT __stdcall
ingres_check_JDK(MSIHANDLE hInstall)
{
	HKEY	hKey;
	char	KeyName[MAX_PATH], KeyName2[MAX_PATH];
	DWORD	dwSize, dwType;
	TCHAR	tchValue[MAX_PATH], versJRE[MAX_PATH], JavaHome[MAX_PATH];
	TCHAR	pathBit[MAX_PATH], pathBit2[MAX_PATH], strRSPFile[MAX_PATH];
	char	*varJRE;
	int	hasValidJRE = FALSE;
	int	bSilent = FALSE;
	const int minmaj=1;
	const int minmin=6;
	const int minbuild=14;  //Minimum JRE version is 1.6.0_14
	...
	*KeyName = *KeyName2 = '\0'; 
	...
	dwSize=sizeof(strRSPFile);
	MsiGetProperty(hInstall, "INGRES_RSP_LOC", strRSPFile, &dwSize);
	if(strcmp(strRSPFile, "0"))
		bSilent = TRUE;

	/* First check if JAVA_HOME is set, points to a valid location,
	** and is at least a minimum version of JRE required.
	*/
	varJRE = getenv("JAVA_HOME");
	if (varJRE != NULL)
	{
		char *ptr;
		_strlwr(varJRE);
		if ((ptr = strstr(varJRE, "jdk")) != NULL)
		{
			ptr=ptr+3;
			if (IsMinimumJREVersion(ptr))
			{
#ifdef WIN64
				sprintf(KeyName, "SOFTWARE\\JavaSoft\\Java Development Kit");
#else
				sprintf(KeyName, "SOFTWARE\\Wow6432Node\\JavaSoft\\Java Development Kit");
				sprintf(KeyName2, "SOFTWARE\\JavaSoft\\Java Development Kit");
#endif /* WIN64 */			
				/* Verify that the JavaHome path of JDK matches the 
				** path of the same version from registry to avoid
				** 32-bit, 64-bit versions mix up. */
				sprintf(KeyName, "%s\\%s", KeyName, ptr);
				if (*KeyName2) sprintf(KeyName2, "%s\\%s", KeyName2, ptr);

				if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName, 0, KEY_ALL_ACCESS, &hKey) != ERROR_SUCCESS &&
					*KeyName2 && hKey==NULL)
					RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName2, 0, KEY_ALL_ACCESS, &hKey);

				if (hKey)
				{
					dwSize = sizeof(tchValue);
					dwType = REG_SZ;
					if (RegQueryValueEx(hKey, "JavaHome", NULL, &dwType, &tchValue, &dwSize) == ERROR_SUCCESS && *tchValue)
						if (stricmp(tchValue, varJRE) == 0)
						{
							hasValidJRE=TRUE;
							strcpy(JavaHome, varJRE);
						}
					RegCloseKey(hKey);
				}
			}
...
#ifdef WIN64
	sprintf(KeyName, "SOFTWARE\\JavaSoft\\Java Runtime Environment");
#else
	sprintf(KeyName, "SOFTWARE\\Wow6432Node\\JavaSoft\\Java Runtime Environment");
	sprintf(KeyName2, "SOFTWARE\\JavaSoft\\Java Runtime Environment");
#endif /* WIN64 */

	if (!hasValidJRE)
	{
	/* Now check whether we have acceptable version of JRE installed by quering 
	** the registry. */
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName, 0, KEY_ALL_ACCESS, &hKey) != ERROR_SUCCESS &&
		*KeyName2 && hKey==NULL)
		RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName2, 0, KEY_ALL_ACCESS, &hKey);
	...
	if (hKey)
	{
		int index = 0;
		DWORD dwErr = 0;
		while ((dwErr = RegEnumKey(hKey, index, tchValue, sizeof(tchValue))) != ERROR_NO_MORE_ITEMS
			&& dwErr == ERROR_SUCCESS)
		{
			index++;
			/* minimum required version of JRE is 1.6.0_14, scan the key retrieved
			** into maj, min, build to separate the version and compare to minimum
			** required version. */
			strcpy(versJRE, tchValue);
			hasValidJRE = IsMinimumJREVersion(tchValue);
			if (hasValidJRE) break;

		}
		RegCloseKey(hKey);
	}
	/*
	** If Acceptable version of JRE is found in the registry, 
	** then append it to the  original registry key and query the 
	** path to JRE home.
	*/
	if (hasValidJRE)
	{
		sprintf(KeyName, "%s\\%s", KeyName, versJRE);
		if (*KeyName2) sprintf(KeyName2, "%s\\%s", KeyName2, versJRE);
	...
	else
	{
		int bAnswer;
		char web[MAX_PATH];
		sprintf(web, "http://www.oracle.com/technetwork/java/javase/downloads/index.html");
		if (!bSilent)
		{
			char messageBuf[MAX_PATH*10];
#ifdef WIN64
			sprintf(messageBuf, "For the Remote Manager service to run, 64-bit Java Runtime\nEnvironment 1.6.0_14 or later must be installed. This version\nwas not detected on your machine. The Remote Manager service\nwill not work without a compatible version of JRE installed.\n\nClick Yes to install or update JRE now and continue Remote\nManager service installation.\nClick No to continue Remote Manager service installation without\ninstalling or updating JRE. If you choose No, you must install or\nupdate JRE manually before the Remote Manager service will work.\n");
#else
			sprintf(messageBuf, "For the Remote Manager service to run, 32-bit Java Runtime\nEnvironment 1.6.0_14 or later must be installed. This version\nwas not detected on your machine. The Remote Manager service\nwill not work without a compatible version of JRE installed.\n\nClick Yes to install or update JRE now and continue Remote\nManager service installation.\nClick No to continue Remote Manager service installation without\ninstalling or updating JRE. If you choose No, you must install or\nupdate JRE manually before the Remote Manager service will work.\n");
#endif /* WIN64 */
			bAnswer = AskUserYN(hInstall, messageBuf);
			if (bAnswer)
			{
				MsiSetProperty(hInstall, "INGRES_CONTINUE", "NO");
				ExecuteShellEx(web);
	...
	if (hasValidJRE && *JavaHome)
	{
		MsiSetProperty(hInstall, "JAVA_HOME", JavaHome);
	}	
