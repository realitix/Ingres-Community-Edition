/*
**  Copyright (c) 2001-2011 Actian Corporation.
*/
...
**	    file. bug 124503.
**	30-Jun-2011 (drivi01) SIR 125503
**	    Add registry entry for ingresdate.  This is needed for post
**	    installer to be able to tell if we're in modify or upgrade
**	    mode.  Otherwise m_ansidate could be "0" b/c user chose
**	    ingresdate and also because we are upgrading.
**	    Adding the second value will help us determine if we're in
**	    upgrade or modify mode in post installer.
**	25-Aug-2011 (drivi01)
**	    Set a start directory for command prompts to II_SYSTEM.
**	29-Sep-2011 (drivi01)
...
**	04-Oct-2011 (drivi01)
**	    Add StopManagementService function, to stop the Remote Manager
**	    service for Management Studio during installation.  
**	    (for upgrade and modify scenarios)
**	    Add JAVA_HOME to ingres_set_dbmsnet_reg_entries custom action,
**	    it will set JavaHome in the registry for post-installer.
*/
...
/*
**  Name: StopManagementService
**
**  Description:
**	Check if Remote Manager Service under the specified id is running.
**	Shut it down if it is.
**
**  Returns:
**	 0	Shut down failed
**	 1	Shut down was successful
**
**  History:
**	30-Sep-2011 (drivi01)
**	    Created.
*/
int
StopManagementService(char *ii_id)
{
	...
	sprintf(szServiceName, "%s_%s", MGMTSVC, ii_id);
...
**	15-Mar-2011 (drivi01)
**	    During upgrade of 32-bit to 64-bit installations, remove
**	    32-bit HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\IngresCorporation\Installation_XX
**	    key manually.
**	28-Sep-2011 (drivi01)
**	    Set vectorwise=1 in the registry for client installations as 
**	    well.  This is now needed as the service name is different
**	    for Ingres and VectorWise.
*/
...
ingres_set_dbmsnet_reg_entries (MSIHANDLE ihnd)
{
    ...
    char		tchValue[MAX_PATH], tchValue2[MAX_PATH], tchII_INSTALLATION[3];
...
#ifdef WIN64
	/*  if this is 64-bit installer and an upgrade, we might be upgrading from 32-bit installation.
	/** Remove 32-bit registry entries if 64-bit registries were already created.
	*/
	if (bIngresUpgrade)
	{
	    sprintf(oldKeyName, "Software\\Wow6432Node\\IngresCorporation\\Ingres\\%s_Installation",
		tchII_INSTALLATION);
	    if (!RegOpenKeyEx(HKEY_LOCAL_MACHINE, oldKeyName, 0, KEY_ALL_ACCESS, &hKey2) &&
		!RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName, 0, KEY_ALL_ACCESS, &hKey3))
	    {
		RegDeleteKey(HKEY_LOCAL_MACHINE, oldKeyName);
		RegCloseKey(hKey2);
		RegCloseKey(hKey3);
	    }
	}
    ...
    /* Do not return failed return code if JAVA_HOME is not
    ** set.  Let it through let post installer deal with it.
    */ 
    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "JAVA_HOME", tchValue,
		   &dwSize ) == ERROR_SUCCESS && (*tchValue))
    {
	    status = RegSetValueEx( hKey, "JavaHome", 0, REG_SZ,
				    tchValue, strlen(tchValue) + 1 );
    }
    ...
    {
      /* add both ingres and ansidate in registry - mutually exculsive of course
	   none will be set for upgrade in order to distinguish beetween modify and upgrade */
	  if (!bIngresUpgrade)
	  {
	     if (strcmp(tchValue, "ANSI") == 0)
	     {
	         status = RegSetValueEx( hKey, "ansidate", 0, REG_SZ,
				"1", strlen(tchValue) + 1 );
	         status = RegSetValueEx( hKey, "ingresdate", 0, REG_SZ,
				"0", strlen(tchValue) + 1 );
	     }
	     else
	     {
    ...
    dwSize = sizeof(tchValue);
    if (status ||
	MsiGetProperty( ihnd, "IVW", tchValue, 
			&dwSize ) != ERROR_SUCCESS || !tchValue[0])
    {
    }
    else
    {
	status = RegSetValueEx( hKey, "vectorwise", 0, REG_SZ, 
				(CONST BYTE *)&tchValue,
				strlen(tchValue) + 1 );
    }
	...
	else
	{
	    ...
	    sprintf(szBuf, "%_Database_%s", SYSTEM_SERVICE_NAME, tchII_INSTALLATION);
...
**          something.
**	10-Dec-2010 (drivi01)
**	    Add routines to detect Ingres VectorWise installations by retrieving
**	    IVW property.
**	    Update the expected title of ingconfig.exe for this installation.
*/
...
ingres_start_ingres_service (MSIHANDLE ihnd)
{
	...
	if ((MsiGetProperty( ihnd, "DBATools", tchValue, &dwSize)==ERROR_SUCCESS) && tchValue[0])
		isDBATools=TRUE;
	dwSize = sizeof(tchValue);
	if ((MsiGetProperty( ihnd, "IVW", tchValue, &dwSize) == ERROR_SUCCESS) && tchValue[0])
		isIVW = TRUE;
	...
	if(!bServiceStartsIipost)
	{
		...
		if (CreateProcess(NULL, szBuf, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi))
		{
			...
			if (isIVW)
				sprintf(szBuf, "Ingres VectorWise %s Configuration", tchII_INSTALLATION);
	...
	if ((schSCManager = OpenSCManager(NULL, NULL, SC_MANAGER_CONNECT)) != NULL)
	{
		if (isDBATools)
		     ...
		     sprintf(ServiceName, "%_Database_%s", SYSTEM_SERVICE_NAME, tchII_INSTALLATION);
...
ingres_delete_file_associations(MSIHANDLE hInstall)
{
	...
	*p=0;
	...
	sprintf(szKey, "%s_Database_%s\\shell", SYSTEM_SERVICE_NAME, cur_code);
	RegDeleteKey(HKEY_CLASSES_ROOT, szKey);
	if (isDBATools)
	sprintf(szKey, "Ingres_DBATools_%s", cur_code);
	else
	sprintf(szKey, "%s_Database_%s", SYSTEM_SERVICE_NAME, cur_code);
	RegDeleteKey(HKEY_CLASSES_ROOT, szKey);
...
**	    before stop Ingres.
**	10-Dec-2010 (drivi01)
**	    Retrieve "IVW" property to be able to display custom
**	    message for Ingres VectorWise installations when
**	    prompting user to shut down the installation.
*/
UINT __stdcall
ingres_stop_ingres(MSIHANDLE hInstall)
{
	...
	/* shut down Remote Manager Service for Management Studio */
	StopManagementService(ii_id);
	...
	if (isIVW)
		sprintf(szBuf, "Your Ingres VectorWise instance is currently running. Would you like to shut it down now?");
	else
		sprintf(szBuf, "Your Ingres instance is currently running. Would you like to shut it down now?");
	if (bSilent ||
		AskUserYN(hInstall, szBuf))
	{
		SetEnvironmentVariable("II_SYSTEM", ii_system);
...
ingres_create_vista_shortcut(MSIHANDLE hInstall)
{
    ...
    char szSystemFolder[MAX_PATH];
    int isDBATools = FALSE, isIVW = FALSE;
    ...
    dwSize = sizeof(szBuf);
    if ((MsiGetProperty(hInstall, "IVW", &szBuf, &dwSize) == ERROR_SUCCESS) &&
	szBuf[0])
	isIVW = TRUE;
    ...
    dwSize = sizeof(szBuf);
    if (MsiGetProperty(hInstall, "ProductName", &szBuf, &dwSize) == ERROR_SUCCESS && szBuf[0])
	sprintf(szFolder, "Ingres\\%s", szBuf);
...
**	15-Nov-2010 (drivi01)
**	    Add routines for generating II_VWDATA response file option
**	    when VW_DATADIR is set.
**	10-Dec-2010 (drivi01)
**	    Add new section to the generated response file
**	    called VectorWise Configurations with 5 new parameters.
**	17-Aug-2011 (drivi01)
**	    Build response file custom to the product.
**	    Take out absolete features such as Replicator, Vision,
**	    Star, and Spatials from IVW response file.
*/
UINT __stdcall
ingres_create_rspfile(MSIHANDLE hInstall)
{
    ...
    cchValue = sizeof(szVal);
    if ((MsiGetProperty(hInstall, "IVW", &szVal, &cchValue) == ERROR_SUCCESS) &&
	szVal[0])
	isIVW = TRUE;
    ...
    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_DATADIR"), szBuf, &cchValue);
    fprintf(fp, "II_VWDATA=\"%s\"\n", szBuf);
    }
    ...
    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_PROC_MEMORY_JOIN"), szBuf, &cchValue);
    fprintf(fp, "II_VWCFG_MAX_MEMORY =\"%s\"\n", szBuf);

    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_BUFFPOOL_MEMORY_JOIN"), szBuf, &cchValue);
    fprintf(fp, "II_VWCFG_BUFFERPOOL =\"%s\"\n", szBuf);     

    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_DATABLOCK_SIZE"), szBuf, &cchValue);
    fprintf(fp, "II_VWCFG_BLOCK_SIZE =\"%s\"\n", szBuf);

    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_DATABLOCK_GROUP_COMBO"), szBuf, &cchValue);
    fprintf(fp, "II_VWCFG_GROUP_SIZE =\"%s\"\n", szBuf);    

    cchValue = sizeof(szBuf)/sizeof(TCHAR);
    ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
    MsiGetProperty(hInstall, TEXT("VW_MAX_PARALLELISM"), szBuf, &cchValue);
    fprintf(fp, "II_VWCFG_MAX_PARALLELISM=\"%s\"\n", szBuf);
    }
...
**	    before invoking "perfmon" with the given parameter.
**	05-Oct-2011 (drivi01)
**	    Set JAVA_HOME in the setingenvs.bat file.
*/
UINT __stdcall
ingres_installfinalize(MSIHANDLE hInstall)
{
    char szBuf[1024], szII_SYSTEM[1024], szBuf2[1024];
    char szSystemFolder[MAX_PATH], szJAVA_HOME[MAX_PATH];
    FILE *fp;
    int total, i;
    HKEY hKey;
    ...
    dwSize=sizeof(szJAVA_HOME);
    if (MsiGetProperty(hInstall, "JAVA_HOME", szJAVA_HOME, &dwSize))
	return (ERROR_SUCCESS);
	...
	if (isDBATools)
	     ...
	     sprintf(szServiceName, "%s_Database_%s", SYSTEM_SERVICE_NAME, szId);
