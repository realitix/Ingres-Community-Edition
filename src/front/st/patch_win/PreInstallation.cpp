...
/*
**  History:
**	11-Aug-2011 (wanfr01) SIR 125652
**	    Store patch log location in PATCHLOG
*/
BOOL
CPreInstallation::LaunchPatchInstaller()
{
	...
	*/ 
	...
	/* update patch log location in msi*/
	sprintf(patchlog, "%s\\ingres\\files\\patch.log", ii_system);
	sprintf(view, "UPDATE Property SET Value = '%s' WHERE Property = 'PATCHLOG'", patchlog);
	sprintf(logBuf, "Patch Log location is set to %s\r\n", patchlog);
	AppendToLog(logBuf);
	if (!setupii_vexe(msih, view))
	{
		AppendToLog("Error updating PATCHLOG in Property table of the MSI.\r\n");
		if (!bSilent)
			BoxErr4("Error updating PATCHLOG in Property table of the MSI.\r\n", "");
		return_code = 1;
		return 1;
	}
