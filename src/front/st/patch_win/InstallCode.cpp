...
**  05-Oct-2011 (drivi01)
**	Update AskUserYN(IDS_INGRESRUNNING) to take a installation id parameter
**	so that the error message specifies the installation that is being
**	patched.
*/
...
BOOL CInstallCode::OnWizardFinish() 
{
	...
	while( rc ==1)
	{
		...
		if(!flag && AskUserYN(IDS_INGRESRUNNING, szInstallCode))
		{
			CWaitCursor wait;
