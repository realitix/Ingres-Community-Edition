...
**  10-Dec-2010 (drivi01)
**	Add custom title and subtitle for Ingres VectorWise.
**	Do not display "Ingres ??" for IVW when asking the user whether
**	to upgrade user databases on upgrade.
**	Update filter routines for the table of installations to display
**	only Ingres installations for Ingres, IVW for IVW and DBA Tools
**	for DBA Tools. Consolidate the code to rely on filter only 
**	instead of repeating the code several times for each product
**	installation.
**  14-Feb-2011 (drivi01)
**	Add some routines to block upgrade of 32-bit Ingres to 64-bit if
**      OpenROAD and Enterprise Access installations are detected in
**      II_SYSTEM.
**  06-Apr-2011 (drivi01)
**	Instance IDs for Ingres and VectorWise are slightly different.
**	This change fixes the problem with selected instance id
**	for VectorWise by just accepting the whole Instance ID field 
**	as Instance identifier instead of parsing like it does
**	Ingres instance name.
**	Also, installer is designed to block UTF8 upgrades of
**	installations before 9.2 release. Since VectorWise version
**	is smaller than 9.2, the installation was being blocked.
**	Added a change to ignore this check for VectorWise
**	upgrades.      
**  
*/
	...
	: CPropertyPage(InstanceList::IDD)
{
	...
	*/ 
	...
	{
		m_psp.pszHeaderTitle = MAKEINTRESOURCE(IDS_INSTLISTTITLEVW);
		m_psp.pszHeaderSubTitle = MAKEINTRESOURCE(IDS_INSTLISTSUBTITLEVW);
	}
}
...
InstanceList::OnSetActive()
{
	...
	*/
	...
	if (thePreInstall.m_InstallType == 0)
	{
		...
		if (thePreInstall.m_Installations.GetSize() > 1 && !thePreInstall.m_IngresVW)
			m_autoid.SetWindowText("Ingres \?\?");
		else
			m_autoid.SetWindowText("");
	...
	*/
	...
	else if (thePreInstall.m_IngresVW && !bReplaced)
	{
...
InstanceList::SetList()
{
	...
	if (thePreInstall.m_IngresVW)
		m_list.InsertColumn(0, "Instance ID", LVCFMT_LEFT, rect.Width() * 1/4, 0);
	else
	...
	for(int i=0; i<thePreInstall.m_Installations.GetSize(); i++)
	{
		...
		if (thePreInstall.m_DBATools)
			...
			install_id = inst->m_id;
		else
			install_id = "Ingres "+inst->m_id;
		if(inst /*&& !thePreInstall.m_DBATools && !thePreInstall.m_IngresVW*/ && 
			((itype == 0 && (ver == 2)) || ( itype == 1 && (ver == 0 || ver == 3))) && 
			((!thePreInstall.m_DBATools && !thePreInstall.m_IngresVW && !inst->m_isDBATools && !inst->m_isIVW) ||
			(thePreInstall.m_DBATools == inst->m_isDBATools 
			&& thePreInstall.m_IngresVW == inst->m_isIVW)))
		{
			/* 
...
InstanceList::OnWizardFinish()
{
...
#ifdef WIN64
			/* If this code routine is executed, that means setup.exe
			** is a 64-bit binary.
			** We're running an upgrade of 32-bit installation to 64-bit.
			** If EA or OR are installed into this installation, upgrade
			** is blocked as netierh EA or OR will work after the upgrade
			** b/c there's no 32-bit iilibcompat.dll after upgrade completes.
			*/
			char vers[MAX_PATH];
			BOOL bFail = FALSE;
			if (inst->m_ReleaseVer.Find("int.w32"))
			{
				sprintf(vers, "%s\\%s",inst->m_path, "ingres\\eaversion.rel");
				if (GetFileAttributes(vers) != INVALID_FILE_ATTRIBUTES)
					bFail = TRUE;
				if (!bFail)
				{
					sprintf(vers, "%s\\%s",inst->m_path, "ingres\\w4glrun.rel");
					if (GetFileAttributes(vers) != INVALID_FILE_ATTRIBUTES)
						bFail = TRUE;
				}
				if (bFail)
				{
					Error(IDS_BLOCKUPGRADE_EAOR, inst->m_path);
					return FALSE;
				}
	...
	if(IngresAlreadyRunning()==0)
	{
		...
		{
			CString charset;
			//UTF8 charset check doesn't apply to VectorWise installations
			//b/c the VectorWise installation is at Ingres 10.0 level
			if (IsPre92Release(inst->m_ReleaseVer) && !thePreInstall.m_IngresVW)
			{
