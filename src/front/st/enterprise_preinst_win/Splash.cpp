/*
**  Copyright (C) 1998, 2002 Actian Corporation
*/
...
**	10-Dec-2010 (drivi01)
**	    Customize the location of the copyright notice to be centered
**	    at the buttom of the splash screen.
**	    Add routines to load custom splash screen for Ingres and
**	    Ingres VectorWise dialogs.
*/
...
#define CP_Y		 353 /* Y coordination of coyright box */
...
CSplashWnd::Create(CWnd* pParentWnd)
{
	...
	bLoad = m_bitmap.LoadBitmap(IDB_SPLASH256_VW);
    else
	bLoad = m_bitmap.LoadBitmap(IDB_SPLASH256);
    if (!bLoad)
		return FALSE;
...
CSplashWnd::OnPaint()
{
    ...
    HRSRC hrsrc = NULL;
    if (thePreInstall.m_IngresVW)
	hrsrc = FindResource(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_SPLASH256_VW) ,RT_BITMAP);
    else
	hrsrc = FindResource(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_SPLASH256) ,RT_BITMAP);
    if (hrsrc)
    {
