...
**  10-Dec-2010 (drivi01)
**      Use custom title for Ingres VectorWise install.
**      Add routines for figuring out which of the buttons should
**      display (new install/modify/upgrade), figure out how many
**      upgradable and modifyable installations are there for
**      the product that's being installed.
**      Add a call to ConvertTextToIVW().
**  14-Feb-2011 (drivi01)
**      Cleanup the warnings.
**  02-Mar-2011 (drivi01)
**	Delay long typecast for proper calculations of control placement.
**	Typecast needed to cleanup warnings.
**  01-Jun-2011 (drivi01)
...
InstallType::OnSetActive()
{
	...
	for(int i=0; i<thePreInstall.m_Installations.GetSize(); i++)
	{
		...
		if (inst)
		{
			if ((!thePreInstall.m_IngresVW && !thePreInstall.m_DBATools && !inst->m_isDBATools && !inst->m_isIVW) ||
				(thePreInstall.m_IngresVW == inst->m_isIVW && thePreInstall.m_DBATools == inst->m_isDBATools))
			{
	...
	if (is_upgrade == 0)
	{
		...
		rect.top = (long)(urect.top + ((urect.bottom - urect.top) * .15));
		rect.bottom = rect.top+rect.Height();
		pic_m.top = rect.top;
		pic_m.bottom = pic_m.top + height;
		irect.top = (long)(urect.bottom - ((urect.bottom-urect.top) * .35));
		irect.bottom = irect.top + irect.Height();
		pic_i.top = (long)(irect.top - (height * .5));
	...
	if (is_modify == 0)
	{
		...
		urect.top = (long)(rect.top + ((rect.bottom - rect.top) *.25));
		urect.bottom = urect.top+height;
		pic_u.top = (long)(urect.top-(img_height * .5));
		pic_u.bottom = pic_u.top + img_height;
		height = irect.Height();
		irect.top = (long)(rect.bottom - ((rect.bottom - rect.top) *.35));
		irect.bottom = irect.top +height;
		pic_i.top = (long)(irect.top-(img_height * .5));
