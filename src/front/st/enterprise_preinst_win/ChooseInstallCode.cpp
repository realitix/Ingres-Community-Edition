...
**     b/c it is a bug.
**  10-Dec-2010 (drivi01)
**     Added custom title and subtitle for this dialog for Ingres
**     and Ingres VectorWise products.
**  14-Feb-2011 (drivi01)
**      Cleanup the warnings.
**  02-Aug-2011 (drivi01)
**     Disable "Ingres" prefix of Instance Name, for VectorWise we
**     will only display Instance ID.
*/
	...
	: CPropertyPage(ChooseInstallCode::IDD)
{
	...
	{
		m_psp.pszHeaderTitle = MAKEINTRESOURCE(IDS_INSTNAMETITLEVW);
		m_psp.pszHeaderSubTitle = MAKEINTRESOURCE(IDS_INSTNAMESUBTITLEVW);
	}
	else
	{
...
BOOL ChooseInstallCode::OnSetActive()
{
	...
	if (thePreInstall.m_IngresVW)
	{
		CWnd *wnd = GetDlgItem(IDC_STATIC_INGRES);
		CWnd *wnd2 = GetDlgItem(IDC_STATIC_INGRES2);
		wnd->ShowWindow(FALSE);
		wnd2->ShowWindow(FALSE);
	}
