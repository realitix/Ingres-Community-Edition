...
**	8-Apr-2011 (wanfr01) b124861
**	    ii_temploc should not point point to global space or it
**	    can get corrupted by future LO calls.
**	8-Jul-2011 (kschendel) SIR 125253
**	    Merge in codev changes to stop x100 servers.  The RCP is supposed
**	    to stop them, but leave in the code here, as a backup.
**	08-Jun-2011 (drivi01)
**	    Moved stop_x100server() to stop_rcp function, all the way 
**	    at the buttom.  The x100 servers should be down by the
**	    time the function is executed, but call it just in case.
**	    With the resent changes, a lot of the code for x100 in
**	    here is not needed and should probably be removed soon.
*/
...
static BOOL (FAR WINAPI *pfnQueryFullProcessImageName)(
			HANDLE hProcess,
			DWORD dwFlags,
			LPTSTR lpExeName,
			PDWORD lpdwSize) = NULL;
...
static int  get_x100server_count(void);
static void stop_x100server( void );
static void stop_x100( DWORD );
FUNC_EXTERN DWORD PCget_AllPidsFromName(void *pszProcessName, DWORD *pPidBuf, 
                                        DWORD ID_BUF_LEN );
...
# define ID_BUF_LEN         512
# define EVENT_NAME_LEN     MAX_PATH
# define SERVER_KILL_TIMEOUT  2000
...
static char iivw[GL_MAXNAME]            = "x100";
#if defined(conf_BUILD_ICE)
...
static char x100_server[GL_MAXNAME]      = "x100_server";

char        *ii_system;
char        *ii_install;
char        *ii_temp;
LOCATION     ii_temploc;
char         ii_tempbuf[MAX_LOC+1];
...
BOOL        shut_gcn     = FALSE;
BOOL        shut_vw      = FALSE;
DWORD       x100pids_list[ID_BUF_LEN];
int         minutes      = 0;
...
main( int argc, char *argv[] )
{
    ...
    MEfill(sizeof(x100pids_list), '\0', x100pids_list);
    ...
    */
    ...
    LOcopy(&ii_temploc, ii_tempbuf, &ii_temploc);
	...
	*/
    ...
    if (!Individual && iidbms_count == 0 && vw_count > 0)
	if (vw_count == 1)
	{
	    F_PRINT( ERget( S_ST06C4_one_x100 ), iivw);
	}
	else
	    F_PRINT2( ERget( S_ST06C5_many_x100 ), vw_count, iivw);
...
skip_printfs:
	...
	*/
	...
	if (vw_count)
		PRINT( ERget( S_ST06C7_vwstop_failed ) );
    }
/*
**  Cleanup temp files
*/
...
find_procs( )
{
    ...
    //Get x100 server count for this installation
    //This function returns right away
    //if iigcn is not up. Get x100 count before
    //this function returns b/c x100 server doesn't
    //report to iigcn.
    vw_count = get_x100server_count();
...
/*
** Name: stop_rcp - stop the rcp processes
**
** Description:
** 
** Inputs:
**     immediate - whether rcpconfig should use -imm_shutdown or not
**
** Outputs:
**     none
**
...
** History:
**	8-Apr-2011 (wanfr01) b124861
**	    Only run rcpconfig if it exists in $II_SYSTEM/ingres/bin
**
*/
static
void
stop_rcp( BOOL immediate )
{
	...
	*/
	...
	rcp_up = FALSE; /* No recovery servers found */
	    ...
	    {
		NMloc(BIN, FILENAME, ERx( "rcpconfig.exe" ), &binloc);
		LOtos(&binloc, &path);

		if (SIopen(&binloc, "r", &rcpdesc) == E_DB_OK)
		{
                    SIclose(rcpdesc);
                    STprintf( cmdbuf, ERx( "%s %s -silent" ), path, cmdopt);
                    system( cmdbuf );
		}
	    }
        }
...
static int
get_x100server_count()
{
	HANDLE hProcess = NULL;
	HMODULE hDll = NULL;
	DWORD   dwSize = 0, dwRet = 0;
	DWORD   x100pids_buf[ID_BUF_LEN];
	int     i, count = 0, srv_count = 0;
	char    szfullpathname[MAX_PATH];

	strlwr(ii_system);
    	count = PCget_AllPidsFromName( x100_server, x100pids_buf,  
                                 sizeof(x100pids_buf) );  

	if (count > 0)
	{
		/* QueryFullProcessImageName is only available on Vista version
		** of Windows and above.
		** On Windows XP and 2003 Server we have to use GetModuleFileName.
		** On later versions of Windows due to security changes, ingstop
		** doesn't have privileges to execute GetModuleFileName if
		** x100 server was started by service and is running in session 0.
		*/
		...
		for ( i = 0; i < count ; i++ )
		{
			dwSize = sizeof(szfullpathname);
			if (pfnQueryFullProcessImageName != NULL)
			{
				hProcess = OpenProcess( PROCESS_QUERY_LIMITED_INFORMATION, FALSE, x100pids_buf[i] );  
				if (hProcess)
					dwRet = (*pfnQueryFullProcessImageName)(hProcess, 0, szfullpathname, &dwSize);
			}
			else
			{
				hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, x100pids_buf[i]);
				if (hProcess)
					dwRet = GetModuleFileNameEx( hProcess, NULL, szfullpathname, MAX_PATH );
			}
			if (dwRet && (strstr( strlwr(szfullpathname), ii_system ) != NULL ))
				x100pids_list[srv_count++]=x100pids_buf[i];
			if (hProcess)
				CloseHandle(hProcess);
		}
	}
...
{
  DWORD   x100pids_buf[ID_BUF_LEN];
  DWORD   i, count = 0, dwSize = 0, dwRet = 0 ;
  HANDLE  hProcess = NULL, hProcess2 = NULL;
  HMODULE hDll = NULL;
  char    szfullpathname[MAX_PATH];
  int     bHasCount = FALSE;

  /* The RCP should have shut down the X100 servers now.  The x100-pids-list
  ** is out of date, don't use it.  Instead, check for any x100 servers
  ** running, there shouldn't be any (at least, none from this installation).
  */
  vw_count = 0;
  strlwr(ii_system);
  count = PCget_AllPidsFromName( x100_server, x100pids_buf,  
  ...
  if ( count > 0 )
	PRINT( ERget(S_ST06C6_stop_vw) ); 

  if (count && (hDll = LoadLibrary( TEXT("kernel32.dll") )) != NULL)
  {
	pfnQueryFullProcessImageName = (BOOL (FAR WINAPI *)(HANDLE hProcess,
		DWORD dwFlags,
		LPCSTR lpExeName,
  ...
  for ( i = 0; i < count ; i++ )
  {
	  ...
	  { 
		  ...
		  if (dwRet && strstr( strlwr(szfullpathname), ii_system ) != NULL ) 
		  { 
			  if (PCis_alive(x100pids_buf[i]))
				  stop_x100(x100pids_buf[i]);
		  }
		  CloseHandle(hProcess);
	  }
  }
...
stop_x100(DWORD x100pid)
{
	...
	sprintf_s( eventname, EVENT_NAME_LEN, "%sx100_server_%d", ObjectPrefix, x100pid);
	hEvent = OpenEvent( EVENT_MODIFY_STATE, FALSE, eventname);
	hProcess= OpenProcess( PROCESS_TERMINATE | SYNCHRONIZE, FALSE, x100pid );
	if ( hEvent && hProcess)
	{
		SetEvent( hEvent );
		if (WaitForSingleObject(hProcess, SERVER_KILL_TIMEOUT) == WAIT_OBJECT_0)
			bStopped = TRUE;
	}
	
	if (hProcess && !bStopped)
	...
	if ( !bStopped )
	{
		F_PRINT("Error during the termination of x100_server (pid=%d)  \n\n", 
                      x100pid );
	}
}
