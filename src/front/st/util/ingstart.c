...
**	02-Oct-2011 (hanje04)
**	    Bug 125688
**	    Add startup for remote manager server (iimgmtsvr)
**
*/ 
...
/*
**  Start MGMT Tools server
*/

# if defined(UNIX) && defined(conf_BUILD_MGMTSVC)
STATUS
start_mgmtsvr()
{
char*   ingstart_mgmtsvr;

    if( PMget( ERx( "ii.$.ingstart.*.mgmtsvr" ),
        &ingstart_mgmtsvr ) != OK )
    {
        return( OK );
    }
    
    PRINT( ERget( S_ST06CA_mgmtsvr_start ) );

    STprintf( cmd, ERx("%smgmtsvrstart%s"), SystemAltExecName, Affine_Some );
...
# if defined(UNIX) && defined(conf_BUILD_MGMTSVC)
		    if( STbcompare( argp, 0, ERx( "iimgmtsvr" ), 0, TRUE ) == 0 &&
			p2 == NULL )
		    {
			startup_mask = PCM_MGMTSVR;
			continue;
		    }
# endif
# ifdef NT_GENERIC
...
# ifdef NT_GENERIC
	if (GVvista() && !as_service && argc==0)
		F_PRINT( ERget( S_ST069E_not_recommended), SystemProductName );
# endif
...
#endif
#if defined(UNIX) && defined(conf_BUILD_MGMTSVC)
		else if( startup_mask & PCM_MGMTSVR )
                {
                    if( start_mgmtsvr() != OK )
                        ERROR( ERget( S_ST06CC_mgmtfail_start ) );
                }
#endif
