...
**      In test_connection(), display the errors returned from SQLGetDiatRec()
**      up to 4 times.
...
# endif /* VMS */
			    ...
			    STcopy(fullPath,attrList[j]->value);
                    ...
                    if (attrList[j]->id == DSN_ATTR_VENDOR)
                        STcopy(DSN_STR_ACTIAN_CORP,attrList[j]->value);
...
**	    Compiler caught routine trying to return address of a local, fix.
*/
char *get_driver_version(char *path)
{
    ...
    static char version[20];	/* Returned string */
...
VOID test_connection( char *dsnname )
{
    ...
    {
        i = 1;
        while (SQL_SUCCEEDED(SQLGetDiagRec(handleType, handle, i,
