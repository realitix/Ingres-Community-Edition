/*
** Copyright (c) 2007 Actian Corporation. All rights reserved.
*/
# include <compat.h>
# include <si.h>
# include <st.h>
...
**	01-Apr-2011 (hanje04)
**	    Fix up for VectorWise
**	18-May-2011 (hanje04)
**	    As debs can only be installed in a fixed location and don't
**	    support multiple installs, use product name as install location
**	04-Aug-2011 (hanje04)
**	    Correct default install location after merge wiith codev
*/
...
char	*pkgInstLoc = "/opt/Ingres/"SYSTEM_PRODUCT_NAME;
...
**	    Created.
**	01-Apr-2011 (hanje04)
**	    Correct build dir to match package name
*/
static char *
gen_deb_dir_name( char *buff, char *directory, SPECS *specs )
{
    STprintf(buff, "%s/%s_%s/%s/DEBIAN/",
	    directory,
	    subsBaseName->value,
	    subsVersion->value,
