/*
** Copyright (c) 2004, 2010 Actian Corporation. All rights reserved.
*/
# include <compat.h>
# include <si.h>
# include <st.h>
...
**	09-Dec-2010 (hanje04)
**	    Only include $II_SYSTEM/ingres as a dir, not it's entire contents.
**	    Stops all the duplicate file warnings building the RPMS.
**	23-Mar-2011 (hanje04)
**	    Mantis 1811
**	    Use default (root) user for files in non-LSB builds. Users can
**	    install as any user and 'ingres' user may not exist.
*/
...
# ifndef conf_LSB_BUILD
    ...
    {
	SIfprintf(fileSpecs, ERx("%%dir %%attr (%o,-,-)  %s/ingres\n"),
			  0755,
			  subsPrefix->value);

	SIfprintf(fileSpecs, ERx("%%attr (%o,-,-) %%verify (%s) %s\n"),
			  0644,
