...
**
**	-l gpl|com|beta|pre|eval Overide license type defined in rpmconfig (rpm only)
**
...
**	15-Mar-2011 (bonro01)
**	    Add Pre Evaluation license type.
**	04-Aug-2011 (hanje04)
**	    To allow building as both Ingres and IVW II_RPM_PREFIX is
**	    no longer set in the build environment by default. Set default
**	    value here instead if it's not set.
**	19-Aug-2011 (frima01) BUG 125253
**	    Change setenv call to IPCLsetEnv to avoid link errors.
*/
...
usage()
{
	...
	SIfprintf( stderr,
		ERx( "\n    -u -- Unhide i18n, i18n64, and documentation packages\n\n" ) );
	SIfprintf( stderr,
		ERx( "\n    -l gpl|com|beta|pre|eval -- Override default license file\n\n" ) );
	SIfprintf( stderr,
	...
	for( argvx = 1 ; argvx < argc ; argvx++ )
	{
		...
		switch( argv[ argvx ][ 1 ] )
		{
			...
			case 'l':
				...
				else if (STcompare(argv[argvx], pre_lic) == 0)
				{
				    license_type=pre_lic;
				    break;
				}
				else
				    license_type=NULL;
...
# endif
	if( rpm || srpm )
	{
		...
		{
# ifdef conf_IVW
		    rpmPrefix="/opt/Ingres/IngresVW";
# else
		    rpmPrefix="/opt/Ingres/IngresII";
# endif
            SIfprintf(stderr, 
                ERx("\nII_RPM_PREFIX not defined. Using: %s\n"), rpmPrefix);
		    IPCLsetEnv("II_RPM_PREFIX", rpmPrefix, 1);
		}
		audit=TRUE;
