...
##	20-Jan-2011 (hanje04)
##	   Correct remove_all regex as it's still not quite right for VectoWise.
##	   Correct typo in invalid package message and clean some others.
##	25-Jul-2011 (hanje04)
...
where:
	package ID 	- Two character ID embedded in the names of 
			  ...
			  names and NOT the value of II_INSTALLATION for
			  the instance to be removed.
		-a	- Remove ALL (PROD1NAME) RPM packages for ALL
			  (PROD1NAME) installations.
			  CANNOT be used if specifying a package ID
		-y	- Answer 'yes' to all prompts (do not prompt)
		-c	- Remove the \$II_SYSTEM/ingres directory after
			  the uninstall has completed.
...
remove_all()
{
	...
	cat << EOF
WARNING!
You are about to remove ALL (PROD1NAME) RPM installations from
$host
...
EOF
   ...
   [1-9]\.[0-9]\.[0-9])
			...
			echo "$pkg is not an (PROD1NAME) RPM package, skipping..."
			continue
			;;
	esac
    done
