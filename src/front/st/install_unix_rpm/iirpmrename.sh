:
## 
## Copyright (c) 2003, 2010 Actian Corporation. All rights reserved. 
## 
## Name:
##	iirpmbuild.sh
##
...
##	25-Mar-2011 (hanje04)
##	    Mantis 1806
##	    Prevent renaming to embed default installation IDs for Ingres
##	    and IVW
##	18-May-2011 (hanje04)
##	    argsInit before printing $0 else we don't pick up the quiet flag
##	21-Jun-2011 (hanje04)
##	    Mantis 2135 & 2138
##	    Don't check sigs when rebuilding RPMs. If they're signed, it
##	    could fail
##
...
usage:
	...
	installation_id   = Installation identifier to be embedded into
			    rpm_package (Cannot be default IDs II or VW)
...
# check for proper number of command line arguments
	...
	case "$1" in
	    II|VW)
		# Renaming to default IDs not allowed
		cat << !
Cannot rename to embed "$1", default ID...
...
# stick rpm contents in RPMFILES
    ...
    RPMFILES=`rpm --nosignature -qlp "$1"`
    rc=$?
    if (($rc)); then
    ...
    licdep=`rpm --nosignature -qp --requires "$1" | grep ${PRODNAME}-license |\
		cut -d' ' -f1`
    rc=$?
    if (($rc)); then
