:
## Copyright (c) 2007, 2011 Actian Corporation, all rights reserved.
## Name:
##	iidebbuild.sh
##
## Usage:
...
##	21-Apr-2011 (hanje04)
##	    Fix up build script to be more Ubuntu-centric.
##	    Correct file name format
##	12-May-2011 (hanje04)
##	    Bring up to date for adding DEB gui support.
##	    Use II_DEB_PREFIX and DEBs install to:
##		 /opt/Ingres/SYSTEM_PRODUCT_NAME
##	04-Aug-2011 (hanje04)
##	    II_DEB_PREFIX no longer always set in build environment, default
##	    to /opt/Ingres/(PROD1NAME) to handle VW and Ingres builds
##
...
case $vers in
    #IA32/64 Linux | AMD64 LINUX | S390 Linux
    int_lnx|\
   i64_lnx|\
   a64_lnx|\
   ppc_lnx|\
...
# check environment for proper variables
    ...
    if [ -z "$II_DEB_PREFIX" ] ; then
	II_DEB_PREFIX="/opt/Ingres/(PROD1NAME)"
    fi
...
unpack_archives()
{
    ...
    [ -d "$II_DEB_BUILDROOT/${deb_basename}_${deb_version}/$1" ] ||
    {
	error "Build area for $1 package doesn't exist:

	$II_DEB_BUILDROOT/${deb_basename}_${deb_version}/$1
...
Has \"buildrel -d\" been run?"
	...
	builddir=$II_DEB_BUILDROOT/${deb_basename}_${deb_version}/$1/${deb_doc_prefix}/${deb_basename}_${deb_version}
    else
	builddir=$II_DEB_BUILDROOT/${deb_basename}_${deb_version}/$1/${II_DEB_PREFIX}
    fi
...
generate_md5sums()
{
    ...
    debloc="$II_DEB_BUILDROOT/${deb_basename}_${deb_version}/$1"
...
build_deb()
{
    ...
    pkg=$1
    if [ "${pkg}" = "core" ] ; then
	deb=${deb_basename}_${deb_version}
    else
	deb=${deb_basename}-${pkg}_${deb_version}
    fi
    bldloc="$II_DEB_BUILDROOT/${deb_basename}_${deb_version}"
    outloc="$II_DEB_BUILDROOT/${ARCH}"
...
"
    ...
    echo "Moving to ${outloc}/${deb}_${ARCH}.deb..."
    mv ${pkg}.deb ${outloc}/${deb}_${ARCH}.deb
...
create_pkg_file()
{
    ...
    [ ! -d ${outloc} ] &&
    {
	error "Cannot locate DEB directory: ${outloc}"
	return 1
    }

    cd ${outloc}
    echo "Creating DEB packages file: Packages.gz"
    ( ${DEBSCAN} ./ | gzip - > Packages.gz ) ||
    {
