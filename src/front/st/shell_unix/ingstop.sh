...
##	15-Dec-2010 (bonro01)
##	    Fix syntax error in syntax message.
##	23-Mar-2011 (hanje04)
##	    Mantis 1810
##	    Add check for Linux services subsystem entry created when
##	    starting using /etc/init.d/ingresXX and warn user about
##	    false positives for a running installation until it's
##	    shutdown using the same method.
##	25-Apr-2011 (hanje04)
##	    Mantis 1810
##	    Update warning message about shutting down service as per doc
##	    group
##	8-Jul-2011 (kschendel) SIR 125253
##	    Integrate X100 server stoppage from codev line.
##
...
# For Linux, check for entry in service subsystem and warn if instance was
# originally started with service script, /etc/init.d/ingresXX
if [ -f /var/lock/subsys/ingres${(PROG3PRFX)_INSTALLATION} ]
then
    warning_box 76 << EOF | wmsg
(PROD1NAME) installation $(PROG3PRFX)_INSTALLATION on $node was started as a service and  may still appear as "running" at the OS level after being shut down by this process. To shut down the $(PROG3PRFX)_INSTALLATION installation service, run:

    /etc/init.d/ingres${(PROG3PRFX)_INSTALLATION} stop
...
if [ -x $II_SYSTEM/ingres/bin/x100_server ] ; then
    wmsg ""
    wmsg "Shutting down VectorWise servers..."
    ii_vwdata=`ingprenv II_VWDATA`
    if [ -z "$ii_vwdata" ] ; then
	ii_vwdata=`ingprenv II_DATABASE`
    fi
    $PSCMD > /tmp/ps.$$
    x100_1=`grep 'iix100' /tmp/ps.$$ | grep -v '/ingres/data/vectorwise' | awk '{print $2}'`
    x100_2=`grep 'iix100' /tmp/ps.$$ | grep "$ii_vwdata/ingres/data/vectorwise" | awk '{print $2}'`
    for i in $x100_1 $x100_2
    do
	    kill $i >/dev/null 2>/dev/null
    done
    rm -f /tmp/ps.$$
    wmsg ""
fi
