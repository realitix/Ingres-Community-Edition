:
# Copyright (c) 1993, 2010 Actian Corporation
#
#
#
...
##      18-may-2010 (hweho01)
##          Add code to setup the VW database location.
##      28-may-2010 (hweho01)
##          Set the default II_VWDATA to II_DATABASE, also set the default
##          installation ID to VW.
##      01-Nov-2010 (hweho01)
##          Add plstform specific codes in get_num_processors to retrieve 
##          processor number for Linux and HP/Itanium.
##      01-Nov-2010 (hweho01)
##          Fix the typo in the previous change.
##      06-Dec-2010 (maspa05) bug 124628
##              Check for available diskspace under LOCATION/ingres if it
##              exists in case it's on a different device to LOCATION. 
##              Added find_free() and find_dev() for this purpose
##      08-Dec-2010 (maspa05) bug 124628
##              typo in above change - "duel_dev" should be "dual_dev"
##	25-Feb-2011 (bonro01) Mantis 1331.
##	    Update II_NUM_OF_PROCESSORS to prompt for single or multi processor
##	    instead of number of CPUs. 
##      26-May-2011 (maspa05) 
##          Added get_set_charset as a common routine for setting charset for
##          iisuapi, iisunet and iisudbms   
##	11-jun-2011 (bonro01) m2025
##	   confg_vw_location() Create the vectorwise checkpoint location
...
##      06-July-2011 (maspa05)
##          Added unsetting of II_CHARSETxx where II_CHARSETxx is an empty
##          string to get_set_charset. This was previously part of the charset
##          setting code in iisudbms but was missed from the merged version
##          when get_set_charset was created.
##
...
#
# find_dev() - finds device that requires space either $1 or $1/ingres if 
#               it exists
#
find_dev()
{
  if [ -d $1/ingres ]
  then
      df -k $1/ingres  | tail -1 | awk '{print $NF}'
  else
      df -k $1 | tail -1 | awk '{print $NF}'
  fi

}

#
# find_free() - finds free space on device - check sub-directory ingres if
#               it exists
#
find_free()
{
...
#
# If RELEASE_NAME is VW and location is II_CHECKPOINT, create the vectorwise
# checkpoint directory. For now, the default vectorwise directory is
# II_CHECKPOINT
#
config_location()
{
...
!
   ...
   if $DATALOC && $DIR_OK ; then
      ...
      if $VW_CHECKPOINT ; then
         check_directory $BASE/(PROD2NAME)/$DIR/vectorwise 700 || VW_DIR_OK=false 
      fi
   fi

   # Write final messages

   if $DIR_OK && $VW_DIR_OK ; then
   case "$LOCATION_SYMBOL" in
...
!
   ...
   [ -z "$DEFAULT" -a "$RELEASE_NAME" = "VW" ] && DEFAULT="VW"
...
You must now specify whether this machine is a multi-processor or not.
This value affects processing in the DBMS when waiting for mutually exclusive
resources.  A value larger than 1 causes a more compute intensive strategy
that may result in getting the resource faster.
Specify 1 for a single processor machine, and for a
multi-processor machine, enter a value of 2.

The value you specify will be used to set the (PROD1NAME) variable
(PROG3PRFX)_NUM_OF_PROCESSORS
...
## axp_osf and usl_us5 check for multiple processors on machine ##
		...
		*_lnx)
			DEF_PROCS=1		
			[ -r /proc/cpuinfo ] && \
			DEF_PROCS=`grep -c processor /proc/cpuinfo`
                	;;

		i64_hpu)
			DEF_PROCS=1		
			[ -x /usr/sbin/ioscan ] && \
			DEF_PROCS=`/usr/sbin/ioscan -kf | grep -c processor`
                	;;
		*)
			DEF_PROCS=1
                	;;
	...
	[ $DEF_CPUS -gt 1 ] && DEF_CPUS=2

	NOTDONE=true
	while $NOTDONE ; do
		(PROG1PRFX)echonn "Please enter 1 for a single or 2 for a multi-processor machine [$DEF_CPUS] "
...
You have entered an invalid value. Please enter a numeric
...
iisys_dev=`find_dev $II_SYSTEM`
[ "$II_DATABASE" ] || II_DATABASE=`ingprenv II_DATABASE`
[ "$II_DATABASE" ] && data_dev=`find_dev $II_DATABASE`

[ "$II_CHECKPOINT" ] || II_CHECKPOINT=`ingprenv II_CHECKPOINT`
[ "$II_CHECKPOINT" ] && ckp_dev=`find_dev $II_CHECKPOINT`

[ "$II_LOG_FILE" ] || II_LOG_FILE=`ingprenv II_LOG_FILE`
[ "$II_LOG_FILE" ] && log_dev=`find_dev $II_LOG_FILE`

[ "$II_DUAL_LOG" ] || II_DUAL_LOG=`ingprenv II_DUAL_LOG`
[ "$II_DUAL_LOG" ] && dual_dev=`find_dev $II_DUAL_LOG`
...
#
# config_vw_location() - configure a VectorWise location variable
#
#	returns 0 if all is well
#
config_vw_location()
{
   ...
   if $BATCH ; then
      ...
      if [ "$READ_RESPONSE" = "true" ] ; then
         location_value=`iiread_response "${LOCATION_SYMBOL}" $RESINFILE` 
         if [ ! -z "$location_value" ] ; then
             BASE=$location_value
         fi
      fi
...
!
   ...
   if $DATALOC && $DIR_OK ; then
      check_directory $BASE/(PROD2NAME)/$DIR/vectorwise 700 || DIR_OK=false 
   fi
   ...
   if $DIR_OK ; then
      if [ "$WRITE_RESPONSE" = "true" ] ; then 
	 check_response_write II_VWDATA $BASE replace
      fi
     (PROG2PRFX)setenv $LOCATION_SYMBOL $BASE
   else
      error_box << !
...
#
# get_set_charset() - sets the II_CHARSETxx and ii.host.ii_charset if not 
#                     already set
#
get_set_charset()
{
...
!
       ...
       if env | grep  "^II_CHARSET$II_INSTALLATION" > /dev/null 
       then
...
# equates to iisetres ii.$CONFIG_HOST.setup.ii_charset - PROG4PRFX used for
# consistency
       $DOIT (PROG1PRFX)setres (PROG4PRFX).$CONFIG_HOST.setup.(PROG4PRFX)_charset $VALUE
