...
##	05-Oct-2011 (hanje04)
##	    Bug 125688
## 	    Setup and install remote manager service
##
...
do_setup()
{
	...
	for s in ingres iimgmtsvc
	do
	    mkrc -s ${s} -r
	    [ -f "$II_SYSTEM/ingres/files/rcfiles/${s}${inst_id}" ] &&
	        rm -f "$II_SYSTEM/ingres/files/rcfiles/${s}${inst_id}"
	    ( $runuser -m -c "mkrc -s ${s}" $II_USERID ) && mkrc -s ${s} -i
	    ( [ "$II_START_ON_BOOT" = "NO" ] || [ "$START_ON_BOOT" = "NO" ] ) &&
	    {
	        if [ -x /usr/sbin/update-rc.d ] ; then
		    /usr/sbin/update-rc.d -f ${s}${inst_id} remove > \
							/dev/null 2>&1
	        else
		    /sbin/chkconfig --del ${s}${inst_id} > /dev/null 2>&1
	        fi
	    }
	done
	# create state dir
