...
##	    keep going.
##	08-Apr-2011 (hanje04)
##	    Remove "sh" when calling *postinst scripts. Not needed and causes
##	    problems on Ubuntu as it doesn't search the path for the script.
...
setup_dbms()
{
    ...
    # then run the setup
    iidbmspreinst -r ingres -v ${versno}-${bldno} &&
	iigenpostinst -r ingres -v ${versno}-${bldno} -p dbms -s iisudbms
    rc=$?
    if [ $rc = 0 ] ; then
	purge_todo iisudbms
    fi
...
setup_net()
{
    # run pre setup to check for potential errors
    # then run the setup
    iigenpostinst -r ingres -v ${versno}-${bldno} -p net -s iisunet
    rc=$?
    if [ $rc = 0 ] ; then
	purge_todo iisunet
    fi
...
EOF
        ...
        iigenpostinst -r ingres -v ${versno}-${bldno} -s $scr 
        rc=$?
        if [ $rc = 0 ] ; then
	    purge_todo $scr
        else
