:
#
# Copyright (c) 1992, 2011 Actian Corporation
#
#
#
...
##	31-Mar-2010 (hweho01)
##	    Ingres Vectorwise release only supports ansidate format, so 
##	    set the II_DATE_TYPE_ALIAS accordingly. 
##	05-Apr-2010 (hanje04)
##	    SIR 123296
...
##	09-may-2010 (hweho01)
##	    For Vectorwise install, copy x100 configuration into
##	    $II_DATABASE/data/vectorwise directory.  
##	17-may-2010 (hweho01)
##	    The x100 configuration file is renamed to vectorwise.conf.
##	18-may-2010 (hweho01)
##	    Add code to setup the VW configuration options.
##	28-may-2010 (hweho01)
##	    More changes to handle the VW configuration options:
##	    1) Change the default installation ID to VW
##	    2) Round up the block_size and group_size to the next
##	       power of two.
##	    3) Set the default for max_memory_size and bufferpool_size
##             if they are not specified in the response file.
##	11-Jun-2010 (hweho01) mantis issue #769
##	    Force POSIX locale to always get the intended numerice   
##	    results for VectorWise configuration settings.
##	23-Jun-2010 (frima01) SIR 123296
##	    Move defining createdb to the init section so it will be present
##	    everywhere in this script.
##	07-Jul-2010 (hweho01) Mantis issue #846
##	    Retrieved physical memory information for HPUX/Itanium.   
##	08-Jul-2010 (hanje04)
##	    SD 144213
##	    BUG 124055
##	    Make II_CONFIG_TYPE=TRAD a no-op as it just refers to the default
##	    Ingres configuration and causes an error in install.log
##	12-Jul-2010 (hweho01) Mantis issue #843
##	    Preserve the vectorwise configuration selections if the
##	    setup is run on the top of an existing installation, do
##	    not prompt for the settings.
##	16-Jul-2010 (hweho01) Mantis issue #846
##	    To obtain the memory size on HPUX/Itanium, add more checking, 
##	    cover the various display formats from machinfo command.
...
##	16-Aug-2010 (hweho01) Mantis issue #830
##	    Check if x100_server can be started. 
...
##	01-Nov-2010 (hweho01)
##	    Add code to setup the VW configuration option for 
##	    max_parallelism_level. 
##	01-Nov-2010 (hweho01)
##	    Remove the prompt for VW option columnspace_size, it was
##	    dropped off after 1.0 GA.  
##	    Correct the expression checking for numeric input.  
##	08-Nov-2010 (hweho01)
##	    Setup the VW configuration option num_cores.  
##	19-Nov-2010 (kiria01) SIR 124690
##	    Default to UCS_BASIC collation if UTF8 chosen
##	23-Nov-2010 (hweho01)
##	    Update the parallelism option name for response file, it   
##	    is II_VWCFG_MAX_PARALLELISM.
##	9-Dec-2010 (kschendel)
##	    Define VW memory stuff for osx.
##	04-Feb-2011 (bolke01/bonro01)
##	    Added cktmpl_ivw as part of the checkpoint of IVW
##      10-Feb-2011 (maspa05) bug 125022
##          Ensure ING_EDIT is set to something that actually exists
##	25-Feb-2011 (bonro01) Mantis 1331
##	   Update max_parallelism_level prompt to be consistent 
##	   with the windows installer
##	04-Mar-2011 (bonro01) Mantis 1570
##	    Fix syntax error caused by pre Ingres 9.2 ungrade check.
##	16-mar-2011 (bonro01) Mantis 1742.
##	   Previous change caused upgrade to prompt for and override
##	   configuration values.
##	17-Mar-2011 (bonro01) Mantis 1510.
##	   Upgrade fails to create cktmpl_ivw, so create the file
##	   if it does not exist.
##	17-May-2011 (hanje04)
##	   Mantis 1832
##	   Ensure II_VWCFG_MAX_PARALLELISM > 0.
##      29-Mar-2011 (smeke01) SIR 125227
##          Add iiinitres for new config parameter opf_hash_aggregate.
##      26-May-2011 (maspa05)
##          Replaced code to get/set charset with get_set_charset() from
##          iishlib
##	01-jun-2011 (stial01) m1871
##	   Create the vectorwise checkpoint location
##	   Remove cktmpl_ivw processing, x100 checkpoint/restore done by dmfjsp
##	   If upgrade, ingunset II_CKTMPL_FILE if it points to cktmpl_ivw.def
##	16-jun-2011 (stial01) m2059
##         The 'vectorwise' ckp directory will be created in config_location
##         right after the 'default' ckp directory. (if installing vectorwise)
##	8-Jul-2011  (kschendel) SIR 125253
##	   Merge Vectorwise iisudbms, wrap VW setup in giant if, make some
##	   attempt at fixing missing indentation in VW setup code.
##	08-Jul-2011 (hanje04)
##	   Mantis 2173
##	   Correct regex used to validate II_VWCFG_MAX_PARALLELISM.
##	30-Sep-2011 (bonro01)
##	    Configure demodb only for Ingres not Vectorwise.
##      10-Oct-2011 (smeke01) b125834
##          Add iiinitres for new config parameter int_math_i8_result.
##	    
#----------------------------------------------------------------------------
. iisysdep
. iishlib
...
!
	...
	$DOIT iiinitres opf_hash_aggregate $II_CONFIG/dbms.rfm
	$DOIT iiinitres int_math_i8_result $II_CONFIG/dbms.rfm
...
# Determine the release name, it is II or VW ?  
RELEASE_NAME=`head -1 $II_SYSTEM/ingres/version.rel | cut -d' ' -f1`

# Upgrade restriction for non VectorWise installation
if [ "$RELEASE_NAME" = "II" ]; then
...
!
   ...
   if $BATCH ; then
	 ...
	 II_INSTALLATION="$2"
      ...
      [ -z "$II_INSTALLATION" -a "$RELEASE_NAME" = "VW" ] && II_INSTALLATION="VW"
...
# Ingres Vectorwise release only supports ansidate as of mar/31/2010. 
if [ "$RELEASE_NAME" = "VW" ] ; then 
...
# Big section for Vectorwise setup (only)

if [ "$RELEASE_NAME" = 'VW' ] ; then
  #
  # Setup the VectorWise data location II_VWDATA. 
  #
  if $INITIAL_SETUP ; then
     echo ""
     cat << !
The VectorWise Database Location
--------------------------------
The Ingres VectorWise variable II_VWDATA stores the default  
location used to store the VectorWise database files.

!
     $BATCH || pause
     LOCATION="II_VWDATA"  
     LOCATION_TEXT="VectorWise Database Files" 
     
     config_vw_location "$LOCATION" "$LOCATION_TEXT" 

  else
    # ingunset II_CKTMPL_FILE if it points to II_SYSTEM/ingres/files/cktmpl_ivw.def
    CKTMPL=`ingprenv II_CKTMPL_FILE`
    DEPRECATED_CKTMPL="$II_SYSTEM/ingres/files/cktmpl_ivw.def"
    if [ "$CKTMPL" = "$DEPRECATED_CKTMPL" ] ; then 
      ingunset II_CKTMPL_FILE
    fi
  fi  # if INITIAL_SETUP

  #
  # START of the section for setting up the VectorWise configuration options
  #
  DEFAULT_TEMPLATE="$II_SYSTEM"/ingres/files/vectorwise.conf_TMPL
  UPDATE_STMT=""
  #
  # Setup the VectorWise configuration option : max_memory_size 
  # 1) Set the default to be 50% of the total physical memory. If the  
  #    system memory can't be obtained, set the default to be 1GB. 
  # 2) Prompt user to enter selection. 
  #
  if $INITIAL_SETUP ; then
    OPT_NAME="max_memory_size"
    DEFAULT_MBYTES=""
    case $VERS in 
     *_lnx)
        [ -r /proc/meminfo ] && \
        DEFAULT_MBYTES=`grep MemTotal /proc/meminfo | LC_ALL=C \
                awk '{if($3~/[Kk][Bb]/) print ($2 / 1024 / 2 ); \
                      else if($3~/[Mm][Bb]/) print (($2 / 2) ); \
                      else { exit 1 } }'`
        [ $? != 0 -o "$DEFAULT_MBYTES" = "" ] && DEFAULT_MBYTES="1024"
        ;; 
     i64_hpu)
        [ -x /usr/contrib/bin/machinfo ] && \
        DEFAULT_MBYTES=`/usr/contrib/bin/machinfo | grep -i Memory | LC_ALL=C \
          awk '{if($1~/Memory:/ && $3~/[Mm][Bb]/) print ($2 / 2 ); \
                else if($1~/Memory:/ && $3~/[Kk][Bb]/) print (($2 / 1024 / 2) ); \
                else if($2~/=/ && $4~/[Mm][Bb]/) print (($3 / 2) ); \
                else if($2~/=/ && $4~/[Kk][Bb]/) print (($3 / 1024 / 2) ); \
                else { exit 1 } }'`
        if [ $? != 0 -o "$DEFAULT_MBYTES" = "" ]; then
          DEFAULT_MBYTES="1024"
          echo "Unable to retrieve the system physical memory information," 
          echo "set $OPT_NAME to the default size: $DEFAULT_MBYTES MB. " 
        fi 
        ;; 
      int_osx)
	DEFAULT_MBYTES=`sysctl hw.memsize 2>/dev/null | cut -d' ' -f2`
	if [ -n "$DEFAULT_MBYTES" ] ; then
	    # bytes -> mbytes, then / 2
	    DEFAULT_MBYTES=`expr $DEFAULT_MBYTES / 2097152`
	fi
	if [ -z "$DEFAULT_MBYTES" ] ; then
	    # Just pick something
	    DEFAULT_MBYTES=1024
	fi
	;;
     *)
       echo "Need to define platform specific information for max_memory_size."
       exit 1
       ;;
    esac

    if $BATCH ; then
      NEWVALUE="$DEFAULT_MBYTES""M"
      if $READ_RESPONSE ; then
	USER_CHANGE=`iiread_response II_VWCFG_MAX_MEMORY $RESINFILE`
        [ ! -z "$USER_CHANGE" ] && NEWVALUE=$USER_CHANGE
      fi #end read response
    else
      cat << !

The VectorWise Processing memory
--------------------------------
The configuration option max_memory_size is used to specify the 
amount of total memory available for query execution. The default 
is 50% of the system physical memory, it is computed to be :
$DEFAULT_MBYTES M bytes 

!
      NOT_DONE=true
      NEWVALUE=$DEFAULT_MBYTES
      if prompt "Is this default size you want to use?" y
	then
	NOT_DONE=false
      else
	while $NOT_DONE
	do  
	  iiechonn "Please enter the desired max memory size (in Megabytes) [$DEFAULT_MBYTES] "
	  read NEWVALUE junk
	  echo ""
	  [ -z "$NEWVALUE" ] && NEWVALUE=$DEFAULT_MBYTES
	  if echo $NEWVALUE |egrep -s '^[0-9]+\.?[0-9]*$' >/dev/null 2>&1
	  then 
	    NOT_DONE=false
	  else
	    iiechonn "Please enter a numeric number only." 
	    echo ""  
	  fi
	done
      fi
    ...
    VWDEFAULT=`grep "^#$OPT_NAME" $DEFAULT_TEMPLATE` 
    NEWSETTING="$OPT_NAME="$NEWVALUE
    UPDATE_STMT="$UPDATE_STMT"" -e 's/"$VWDEFAULT"/"$NEWSETTING"/'"
    cat << !

VectorWise option $OPT_NAME is set to $NEWVALUE Bytes
!
    if $WRITE_RESPONSE ; then
      check_response_write II_VWCFG_MAX_MEMORY $NEWVALUE replace
    fi
  fi  # if INITIAL_SETUP
  #
  # Setup the VectorWise configuration parameter : bufferpool_size 
  # 1) Set the default to be 25% of the total physical memory. If the  
  #    system memory can't be obtained, set the default to be 2GB. 
  # 2) Prompt user to enter selection.
  #
  if $INITIAL_SETUP ; then
    OPT_NAME="bufferpool_size"
    DEFAULT_MBYTES=""
    case $VERS in 
     *_lnx)
        [ -r /proc/meminfo ] && \
        DEFAULT_MBYTES=`grep MemTotal /proc/meminfo | LC_ALL=C \
                awk '{if($3~/[Kk][Bb]/) print ($2 / 1024 / 4 ); \
                      else if($3~/[Mm][Bb]/) print (($2 / 4) ); \
                      else { exit 1 } }'`
        [ $? != 0 -o "$DEFAULT_MBYTES" = "" ] && DEFAULT_MBYTES="2048"
        ;; 
     i64_hpu)
        [ -x /usr/contrib/bin/machinfo ] && \
        DEFAULT_MBYTES=`/usr/contrib/bin/machinfo | grep -i Memory | LC_ALL=C \
          awk '{if($1~/Memory:/ && $3~/[Mm][Bb]/) print ($2 / 4 ); \
                else if($1~/Memory:/ && $3~/[Kk][Bb]/) print (($2 / 1024 / 4)); \
                else if($2~/=/ && $4~/[Mm][Bb]/) print (($3 / 4)); \
                else if($2~/=/ && $4~/[Kk][Bb]/) print (($3 / 1024 / 4)); \
                else { exit 1 } }'`
        if [ $? != 0 -o "$DEFAULT_MBYTES" = "" ]; then
          DEFAULT_MBYTES="2048"
          echo "Unable to retrieve the system physical memory information," 
	...
	if [ -n "$DEFAULT_MBYTES" ] ; then
	    # bytes -> mbytes, then / 4
	    DEFAULT_MBYTES=`expr $DEFAULT_MBYTES / 4194304`
	fi
	if [ -z "$DEFAULT_MBYTES" ] ; then
	    # Just pick something
	    DEFAULT_MBYTES=512
	...
	;;
     *)
       echo "Need to define platform specific information for bufferpool_size."
       exit 1
       ;;
    esac
    ...
    if $BATCH ; then
      NEWVALUE="$DEFAULT_MBYTES""M"
      if $READ_RESPONSE ; then
	USER_CHANGE=`iiread_response II_VWCFG_BUFFERPOOL $RESINFILE`
        [ ! -z "$USER_CHANGE" ] && NEWVALUE=$USER_CHANGE
      fi #end read response
    else
      cat << !

The VectorWise Buffer Pool Memory
---------------------------------
The configuration option bufferpool_size is used for the buffer  
pool memory, the default is 25% of the system physical memory,   
...
!
      NOT_DONE=true
      NEWVALUE=$DEFAULT_MBYTES
      if prompt "Is this the default size you want to use?" y
	then
	NOT_DONE=false
      else
	while $NOT_DONE
	do  
	  iiechonn "Please enter the desired buffer pool size (in Megabytes) [$DEFAULT_MBYTES] "
	  read NEWVALUE junk
	  echo ""
	  [ -z "$NEWVALUE" ] && NEWVALUE=$DEFAULT_MBYTE
	  if echo $NEWVALUE |egrep -s '^[0-9]+\.?[0-9]*$' >/dev/null 2>&1
	  then 
	     NOT_DONE=false
	  else
...
!
    if $WRITE_RESPONSE ; then
      check_response_write II_VWCFG_BUFFERPOOL $NEWVALUE replace
    fi
  fi  # if INITIAL_SETUP

  #
  # Setup the VectorWise configuration parameter : block_size 
  ...
  if $INITIAL_SETUP ; then
    ...
    if $BATCH ; then
      if $READ_RESPONSE ; then
	  USER_CHANGE=`iiread_response II_VWCFG_BLOCK_SIZE $RESINFILE`
	  if [ ! -z "$USER_CHANGE" ] ; then
	    NEWVALUE=$USER_CHANGE
	    UPDATE_YES=true
	  fi
      fi #end read response
    else
...
The VectorWise Data Block Size
------------------------------
The configuration option block_size is used to specify the  
minimum I/O granularity, the default is 512KB.  
...
!
	...
	while $NOT_DONE
	do  
	  iiechonn "If the selection is not in the power of 2, it will be rounded up."
	  echo ""
	  iiechonn "Please enter the desired data block size (in Kilobytes) [$DEFAULT_KBYTES] "
	  read NEWVALUE junk
	  echo ""
	  [ -z "$NEWVALUE" ] && NEWVALUE=$DEFAULT_KBYTES
	  if echo $NEWVALUE |egrep -s '^[0-9]+\.?[0-9]*$' >/dev/null 2>&1
	  then 
	     #if input isn't in the power of 2, round up to the next power of 2
	     num_bytes=`expr $NEWVALUE \* 1024`
	     num_bytes=`echo $num_bytes | LC_ALL=C awk '{ n=( log($1)/log(2)); \
		       powern=((m=int(n))< n)?(m=m+1):n ; print (2^powern)}'`
	     NEWVALUE=`expr $num_bytes / 1024` 
	     NOT_DONE=false
	  else
	     iiechonn "Please enter a numeric value only." 
	     echo ""
	  fi
	done
	if [ $DEFAULT_KBYTES != $NEWVALUE ]; then 
	  UPDATE_YES=true
	  NEWVALUE="$NEWVALUE""K"
	fi
      fi
...
!
      if $WRITE_RESPONSE ; then
	check_response_write II_VWCFG_BLOCK_SIZE $NEWVALUE replace
      fi
    fi
  fi  # if INITIAL_SETUP

  #
  # Setup the VectorWise configuration parameter : group_size 
  # The default is 8   
  #
  if $INITIAL_SETUP ; then
    OPT_NAME="group_size"
    ...
    if $BATCH ; then
      if $READ_RESPONSE ; then
	USER_CHANGE=`iiread_response II_VWCFG_GROUP_SIZE $RESINFILE`
	if [ ! -z "$USER_CHANGE" ] ; then
	  NEWVALUE=$USER_CHANGE
	  UPDATE_YES=true
	fi
...
The VectorWise Data Block Group Size
------------------------------------
The configuration option group_size is used to specify the number 
of blocks to handle in a disk request, the default is 8.
...
!
	...
	do  
	  iiechonn "If the selection is not in the power of 2, it will be rounded up."
	  echo ""
	  iiechonn "Please enter the desired data block group size [$DEFAULT_NUM] "
	  read NEWVALUE junk
	  echo ""
	  [ -z "$NEWVALUE" ] && NEWVALUE=$DEFAULT_NUM
	  if echo $NEWVALUE |grep -s '[^0-9]' >/dev/null 2>&1 
	  then 
	    iiechonn "Please enter a numeric value only." 
	    echo ""
	  else
	    #if input isn't in the power of 2, round up to the next power of 2
	    NEWVALUE=`echo $NEWVALUE | LC_ALL=C awk '{ n=( log($1)/log(2)); \
		       powern=((m=int(n))< n)?(m=m+1):n ; print (2^powern)}'`
	    NOT_DONE=false
	  fi
	done
	[ "$DEFAULT_NUM" != "$NEWVALUE" ] && UPDATE_YES=true 
      fi # BATCH
    fi
    if $UPDATE_YES ; then     
      VWDEFAULT=`grep "^#$OPT_NAME" $DEFAULT_TEMPLATE` 
...
VectorWise option $OPT_NAME is set to $NEWVALUE . 
!
      if $WRITE_RESPONSE ; then
	check_response_write II_VWCFG_GROUP_SIZE $NEWVALUE replace
      fi
    fi
  fi  # if INITIAL_SETUP

  #
  # Setup the VectorWise configuration parameters : max_parallelism_level
  #					          num_cores	
  #
  if $INITIAL_SETUP ; then
    OPT_NAME="max_parallelism_level"
    OPT_NAME2="num_cores"
    DEFAULT_LEVEL=""
    case $VERS in 
     *_lnx)
        [ -r /proc/cpuinfo ] && \
        DEFAULT_LEVEL=`grep -c processor /proc/cpuinfo` 
        [ "$DEFAULT_LEVEL" = "" -o "$DEFAULT_LEVEL" = "0" ] && DEFAULT_LEVEL="1"
        ;; 
     i64_hpu)
        [ -x /usr/sbin/ioscan ] && \
        DEFAULT_LEVEL=`/usr/sbin/ioscan -kf | grep -c processor`
        [ "$DEFAULT_LEVEL" = "" -o "$DEFAULT_LEVEL" = "0" ] && DEFAULT_LEVEL="1"
        ;; 
     int_osx)
	DEFAULT_LEVEL=`sysctl hw.activecpu 2>/dev/null | cut -d' ' -f2`
	if [ -z "$DEFAULT_LEVEL" ] ; then
	    DEFAULT_LEVEL=1
	fi
	if [ $DEFAULT_LEVEL -le 0 ] ; then
	    DEFAULT_LEVEL=1
	fi
	;;
     *)
       echo "Need to define platform specific information for max_parallelism_level."
       exit 1
       ;;
    esac

    # Limit the max_parallelism_level to 8
    if [ $DEFAULT_LEVEL -gt 8 ]; then
      DEFAULT_LEVEL=8
    fi

    if $BATCH ; then
      NEWVALUE="$DEFAULT_LEVEL"
      if $READ_RESPONSE ; then
	USER_CHANGE=`iiread_response II_VWCFG_MAX_PARALLELISM $RESINFILE`
        ( echo "$USER_CHANGE" |grep -s '^[1-9]' >/dev/null 2>&1 ) &&
        	NEWVALUE=$USER_CHANGE
	[ $NEWVALUE -lt 1 ] &&
	{
	    echo "II_VWCFG_MAX_PARALLELISM must be > 0, using 1"
	    NEWVALUE=1
	}
...
The VectorWise Maximum Parallel Threads
----------------------------------------
The configuration option max_parallelism_level is used to define
the maximum number of threads used by a single query. A value larger
than 1 enables  multi-core parallelism.
The default value is $DEFAULT_LEVEL

!
      NOT_DONE=true
      NEWVALUE=$DEFAULT_LEVEL
      if prompt "Is this the default value you want to use?" y
      then
	NOT_DONE=false
      else
	while $NOT_DONE
	do  
	  iiechonn "Please enter the number for max parallelism level [$DEFAULT_LEVEL] "
	  read NEWVALUE junk
	  echo ""
	  [ -z "$NEWVALUE" ] && NEWVALUE=$DEFAULT_LEVEL
	  if echo $NEWVALUE |grep -s '^[1-9]' >/dev/null 2>&1
	  then 
	    iiechonn "Please enter a numeric value greater than 1" 
	    echo ""
	  else
	    NOT_DONE=false
	  fi
...
The Vectorwise Number Of Cores
------------------------------
The configuration option num_cores is used to calculate the amount of memory
available for each transaction. num_cores is a database-wide limit on the
number of concurrent threads, but it is a soft limit.  After num_cores is
exceeded, new queries will always get one core.  The value is not modifiable
here but may be updated in the vectorwise.conf file.
The default value is the same as max parallelism level which is $NEWVALUE
!
    fi # BATCH
    ...
    VWDEFAULT=`grep "^#$OPT_NAME2" $DEFAULT_TEMPLATE` 
    NEWSETTING="$OPT_NAME2="$NEWVALUE
    UPDATE_STMT="$UPDATE_STMT"" -e 's/"$VWDEFAULT"/"$NEWSETTING"/'"

    cat << !

VectorWise option $OPT_NAME is set to $NEWVALUE
VectorWise option $OPT_NAME2 is set to $NEWVALUE
!
    if $WRITE_RESPONSE ; then
      check_response_write II_VWCFG_MAX_PARALLELISM $NEWVALUE replace
    fi
  fi  # if INITIAL_SETUP

  #
  # Update the VectorWise configuration file with the local modifications, 
  # then copy it into the VW data file directory.  
  #
  if $INITIAL_SETUP ; then
    II_VWDATA=`ingprenv II_VWDATA`
    SUFFIX=(PROD2NAME)/data/vectorwise
    VW_CFG_LOC="$II_VWDATA"/"$SUFFIX"
    if [ -z "$UPDATE_STMT" ] ; then
      $DOIT cp $DEFAULT_TEMPLATE $VW_CFG_LOC/vectorwise.conf
    else
      SED_UPDATE_STMT="sed $UPDATE_STMT"
      eval $SED_UPDATE_STMT $DEFAULT_TEMPLATE > $VW_CFG_LOC/vectorwise.conf   
    fi
    cat << !

The VectorWise configuration file is setup in the installation as : 
$VW_CFG_LOC/vectorwise.conf

!
  fi  # if INITIAL_SETUP

fi # end of if RELEASE_NAME is VW

# 
# END of the section for setting up the VectorWise configuration options 
#
...
Contact Actian Corporation Technical Support for assistance.
!
		$DOIT ingstop >/dev/null 2>&1
                if [ $STAR_SERVERS -ne 0 ] ; then
	    	  $DOIT iisetres ii.$CONFIG_HOST.ingstart."*".star $STAR_SERVERS
...
You should contact Actian Corporation Technical Support to resolve this problem. 
!
		NONFATAL_ERROR=Y
	    } 
...
# Check if the system has the shared libraries required by x100_server
## FIXME:  a) this fiddling with LD_LIBRARY_PATH is exceedingly undesirable,
## should be set in the environment, or via rpath, or not at all;
## b) ldd works on linux but not (e.g.) on OS/X which wants otool
if [ "$RELEASE_NAME" = "VW" ] ; then
  LD_LIBRARY_PATH=$II_SYSTEM/ingres/lib:${LD_LIBRARY_PATH:-"/lib:/usr/lib"}
  export LD_LIBRARY_PATH
  X100_SERVER=$II_SYSTEM/ingres/bin/x100_server 
  $DOIT $X100_SERVER --version >/dev/null 2>&1 ||
    {
      cat << !

Command "x100_server --version" failed to run on the system. 
The problem is probably caused by a missing dependency, the 
following command can be used to verify if the system has all
the shared libraries that are required by the x100_server :
     ldd $X100_SERVER

!
     $BATCH || pause
   }
fi  #if [ "$RELEASE_NAME" = "VW" ]
