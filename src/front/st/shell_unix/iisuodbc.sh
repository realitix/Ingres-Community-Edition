...
##      15-Jun-2011 (Ralph Loen) Bug 125446
##          Changed (PROG1PRFX)odbcddmn to (PROG1PRFX)odbcadmin.
##	8-Jul-2011 (kschendel) SIR 125253
##	    Change name of installation guide for Vectorwise: use just
##	    Ingres Installation Guide, not (PROD1NAME).
##          
##
...
$BATCH ||
{
   ...
   Ingres drivers as residing in II_SYSTEM/ingres/lib.

!

    express_opt=false
    prompt "Do you want the EXPRESS option?" y  && express_opt=true
    if [ "$express_opt" = "true" ]; then
        BATCH=true
        NOTBATCH=false
    fi
}
...
!
           exit 0
            ...
            echo "Successfully wrote ODBC configuration files to $cfgloc"     
        fi
    else
        echo "Successfully wrote ODBC configuration files to /usr/local/etc"     
    fi
    else
...
iguide="(PROD1NAME)"
if expr "$iguide" : '.*Vector' >/dev/null 2>&1 ; then
    iguide='Ingres'
fi
$BATCH || cat << !

An odbcinst.ini file has been created in the directory "$ALTPATH".  
You may use the utility (PROG1PRFX)odbcadmin to create and manage
ODBC data sources.

See the $iguide Installation Guide for more information.
