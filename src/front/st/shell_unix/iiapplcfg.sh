...
##	09-Dec-2010 (hanje04)
##	    Add values for VectorWise
##	26-Jan-2011 (hanje04)
##	    BUG 124959
##	    Change trasactional defaults to use mvcc and serializable
##	    isolation level.
##	10-Feb-2011 (hanje04)
##	    Mantis 1454
##	    Bump opf_memory to 80Mb for IVW following Microstratergy issue.

basename=`basename $0`
validcfg="TXN BI CM IVW"
cfgtype=""
revert=false
...
apply_config_txn()
{
    ...
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.system_isolation"  serializable &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.system_lock_level"  mvcc &&
...
apply_config_ivw()
{
    rc=0
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.blob_etab_page_size"  16384 &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.cache_dynamic"  OFF &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.connect_limit"  64  &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.cursor_limit"  32  &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.dmf_group_size"  32  &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.max_tuple_length"  0 &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.opf_joinop_timeout"  100 &&
    iisetres -v +p "ii.${CONFIG_HOST}.dbms.*.opf_memory"  80000000 &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.opf_new_enum"  ON &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.opf_timeout_factor"  1 &&
    iisetres -v +p "ii.${CONFIG_HOST}.dbms.*.psf_memory"  20000000 &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.private.*.dmf_group_size"  32  &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.private.*.p8k.dmf_separate" ON &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.private.*.cache.p16k_status" ON &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.private.*.p16k.dmf_separate" ON &&
    ...
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.system_maxlocks"  500 &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.system_readlock"  nolock &&
    iisetres -v "ii.${CONFIG_HOST}.dbms.*.table_auto_structure"  OFF &&
    iisetres -v "ii.${CONFIG_HOST}.rcp.lock.per_tx_limit"  7500 &&
    iisetres -v "ii.${CONFIG_HOST}.rcp.log.buffer_count"  50
    rc=$?
...
apply_config()
{
	...
	CM)
	    ...
	    ;;
	IVW)
	    echo_cmd "Applying Ingres VectorWise Configuration..."
	    apply_config_ivw
	    rc=$?
	    ;;
	*)
