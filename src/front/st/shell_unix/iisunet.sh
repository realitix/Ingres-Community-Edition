...
##      16-Dec-2010 (hanal04) Bug 124830
##         If local_vnode holds an upper case host name we could compare
##         it's value to II_HOSTNAME and get a mismatch because iinethost
##         will lowercase the II_HOSTNAME. For consistency lower case the
##         SERVER_HOST and CONFIG_SERVER_HOST values. This avoids incorrect
##         hostname mismatches.
##      26-May-2011 (maspa05)
##          Replaced code to get/set charset with get_set_charset() from
##          iishlib
##	8-Jul-2011 (kschendel) SIR 125253
##	    Integrate Vectorwise changes to set RELEASE_NAME, different names
##	    for some of the manuals.
##	   
##
...
# Generalize for slightly different manual names for Vectorwise:
if [ "$RELEASE_NAME" = 'VW' ] ; then
    iguide="(PROD1NAME) User Guide"
    cguide="Ingres Connectivity Guide"
else
    iguide="(PROD1NAME) Installation Guide"
    cguide="(PROD1NAME) Connectivity Guide"
fi
...
#
CONFIG_SERVER_HOST=`(PROG1PRFX)getres (PROG4PRFX)."*".config.server_host | tr 'A-Z' 'a-z'`

SERVER_HOST=`(PROG1PRFX)getres (PROG4PRFX).$CONFIG_SERVER_HOST.gcn.local_vnode | tr 'A-Z' 'a-z'`
...
For more detail about setting up networking refer to the Installation Guide.
...
Refer to the $iguide for more information about
...
$cguide) to authorize remote users for access
to this server.
...
o Use "(PROG0PRFX)netutil" as described in the $cguide.
...
(PROD1NAME) Networking has been successfully set up in this installation.
Refer to the $cguide for instructions on how to use
