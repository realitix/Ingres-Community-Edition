...
**	    i4's are to be formatted with %d, not %ld.
**	17-Jun-2011 (smeke01) b122745
**	    In transaction_logs_form, when converting the total size in bytes of the
**	    logfile to the total size in megabytes, if the logfile has more than one
**	    partition make sure that we round up to the nearest megabyte.
**      29-Sep-2011 (horda03) b125797
**          Where full path names used to invoke utilities on VMS need to preceed the
**          path with the MC command.
*/
...
# endif /* UNIX */
	...
	exec frs begin;
		...
		*/
		...
		/*
		** When the log file size is first calculated in function
		** write_transaction_log, it is calculated as follows:
		**
		** 1). Take the value in kilobytes from $.$.rcp.file.kbytes. This
		** is the value as specified by the user in megabytes multiplied
		** by 1024.
		** 2). Divide this value by (LG_PAGE_SIZE / 1048) to arrive at
		** the number of LG_PAGE_SIZE pages (LG_PAGE_SIZE is in kilobytes).
		** 3). Divide this value by the number of log partitions.
		** 4). Create each partition with this value * LG_PAGE_SIZE.
		**
		** Step (3) means that for log files with more than 1 partition
		** the sum of the size of each partition can be less than the original
		** value supplied by the user in megabytes. This will show here as
		** the total bytes not being exactly divisible by 1048576 (bytes 
		** in a megabtyte). So if we have more than 1 partition, we add
		** 1048575 to make sure the megabyte result is rounded up.
		*/
		if (num_parts1 > 1)
		    lsize1 += 1048575L; /* 1 less than bytes in a megabyte */
		STprintf( size1, ERx( "%u" ), (i4)(lsize1 / 1048576L ));
		...
		*/
		if (num_parts2 > 1)
		    lsize2 += 1048575L; /* 1 less than bytes in a megabyte */
		STprintf( size2, ERx( "%u" ), (i4)(lsize2 / 1048576L ));
...
prompt_log_size:
			...
			*/
		    ...
		    STprintf( value, ERx( "%d" ), mbytes * 1024 );
...
# endif /* VMS */
    ...
    LOtos(&loc, &locstr);
    STprintf( cmd, ERx( "%s%s %s %s %s %s" ), lead_txt, locstr, Affinity_Arg,
