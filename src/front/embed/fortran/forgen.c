...
**      16-May-2011 (maspa05) b125361
**          Formerly embedded fortran output was either 'strict' or not
**          according to whether STRICT_F77_STD was define'd i.e. hard-coded
**          per-platform. 
**          Now this is a default which can be over-ridden with 
**          II_FORTRAN_FORMAT
**      23-May-2011 (maspa05) b125361
**          Included continuation line strings in the above change
**          Maximum line length (G_LINELEN) needs to correspond to output
**          format.
**      20-Jul-2011 (coomi01) Bug 125478
**          Allow user to specify max line length as final resort.
*/

i2  fortran_output_format;  /* fortran output format either
					 STANDARD (aka 'strict') or TAB */
i2  g_linelen;              /* maximum line length */

char cont_string[8];      /* continuation line start for string constant */
char cont_state[8];       /* continuation line start for statement */
...
gen_init()
{
    ...
    g_indent[0] = '\0';
     ...
     /* Default Fortran output format according to STRICT_F77_STD */
 
 #ifdef STRICT_F77_STD
     fortran_output_format=FORTRAN_FMT_STANDARD;
 #else
     fortran_output_format=FORTRAN_FMT_TAB;
 #endif
 
     NMgtAt( ERx("II_FORTRAN_FORMAT"), &forfmt);
     if (forfmt)
     {
        if (STbcompare(forfmt,0,"TAB",0,TRUE)==0)
 	{
             fortran_output_format=FORTRAN_FMT_TAB;
 	}
	else if (STbcompare(forfmt,0,"STANDARD",0,TRUE)==0)
 	{
             fortran_output_format=FORTRAN_FMT_STANDARD;
 	}
     }
...
#ifdef UNIX
    if (fortran_output_format == FORTRAN_FMT_STANDARD)
    {
	    STcopy(CONT_STR_STD,cont_string);
	    STcopy(CONT_STATE_STD,cont_state);
	    g_linelen=G_LINELEN_STD;
    }
    else
#endif
    {
	    STcopy(CONT_STR,cont_string);
	    STcopy(CONT_STATE,cont_state);
	    g_linelen=G_LINELEN;
    }

    /* If the user specified a output record length 
    ** through the -olen switch, this overrides the above
    */
    if (0 < eq->eq_outrec_len)
    {
	g_linelen=eq->eq_outrec_len;
    }
}
...
register char	*obj;
{
    ...
    {
        if (fortran_output_format == FORTRAN_FMT_TAB)
	{
	    out_emit( ERx("\t") );
	    out_emit( g_indent );
	}
