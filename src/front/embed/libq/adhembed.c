...
**	20-sep-2011 (wanfr01) b125766
**	    Use the appropriate length value for NULL DB_DBV_VALUEs
**	    when setting the NULL bit.
**	    
*/
...
DB_DATA_VALUE		*adh_dv;
{
	...
	*/
	...
	/* Bug 125766 - NULL DB_DBV_TYPES need to retain data length to find
	** the null bit
	*/
	   
	if (adh_ev->ed_type == DB_DBV_TYPE
	   && adh_dv->db_data == ((DB_DATA_VALUE *)(adh_ev->ed_data))->db_data)
	{
	    adh_dv->db_length = ((DB_DATA_VALUE *)(adh_ev->ed_data))->db_length;
	}
...
**			values
** Output:
**	IIlbqcb->ii_lq_ucode_ctbl	The unicode coercion table 
**					is initiated.
** Returns:
**
...
i4	setnfc;
{
    ...
    { 
        if (status = aduucolinit(col,
                  (ADUUCETAB **)&IIlbqcb->ii_lq_ucode_ctbl, &sys_err))
        {
          status = FAIL;
        }
