...
**	    Changes to implement VWLOAD mechanism, a direct-to-X100 load
**	    hook.  COPY is used for authentication and access control, the
**	    caller's COPYHANDLER is called (once) to run the entire copy.
*/ 
...
**
**	One special case is the VectorWise fast-loader, which loads rows
**	directly to x100-server.  In order to validate the table and user
**	permissions, and get authorization to access the x100-server,
**	the vwloader will do a special pseudo-copy:
**
**	    COPY tablename() FROM VWLOAD WITH COPYHANDLER=func;
**
**	As far as COPY and the backend are concerned, this is a more or
**	less ordinary COPY, except that after the copy setup, instead of
**	fetching rows from a file and sending them, we call the user
**	function once (with connect info).  When it returns successfully,
**	the rows have been loaded, and we simply tell the DBMS server that
**	the thing is done.
**
**  Inputs:
...
i4	    (*user_routine)();		/* Function to get/put rows */
{
    ...
    */
    if (copy_blk.cp_program || copy_blk.cp_vwload)
    ...
    */
    ...
    /* If it's a vwloader pseudo copy, handle it and return. */
    if (copy_blk.cp_vwload)
    {
	STATUS sts;

	sts = (*copy_blk.cp_usr_row)(&copy_blk.cp_rowcount, (char *) copy_blk.cp_token_struct, 0);
	if (sts != OK)
	    copy_blk.cp_error = TRUE;	/* So DBMS knows it didn't work */
	IIcpendcopy(IIlbqcb, &copy_blk);
	EXdelete();
...
**	    Skip most of this if vwloader, since vwloader doesn't care.
*/

static STATUS
IIcpinit( II_LBQ_CB *IIlbqcb, II_CP_STRUCT *copy_blk, i4  msg_type )
{
    ...
    */
    ...
    copy_blk->cp_vwload = ((copy_blk->cp_status & CPY_VWLOAD) != 0);
...
# endif
    ...
    ** Note: for VWLOAD, this will actually be a port/token string, but
	...
	*/
    ...
    /* Now that the entire copy map message has been processed, if this is
    ** a vwload pseudo-copy, set up the struct for the copyhandler and
    ** skip all the rest of this.  The copyhandler in vwloader will do
    ** all of the work.
    */
    if (copy_blk->cp_vwload)
    {
	...
	if (loc_alloc(1, sizeof(II_CP_TOKEN_STRUCT), (PTR *) &copy_blk->cp_token_struct) != OK)
	    return (E_CO0037_MEM_INIT_ERR);
	memset(copy_blk->cp_token_struct, 0, sizeof(II_CP_TOKEN_STRUCT));
	/* Since it's a copy (), num domains == num columns */
	copy_blk->cp_token_struct->num_columns = copy_blk->cp_num_domains;
	p = copy_blk->cp_filename;	/* The "port token" string */
	while (*p != ' ' && *p != '\0')
	    ++p;
	if (*p == '\0')
	    return (E_CO0037_MEM_INIT_ERR);	/* Bogus, but shouldn't happen */
	*p = '\0';		/* for CVal */
	(void) CVal(copy_blk->cp_filename, &copy_blk->cp_token_struct->port);
	copy_blk->cp_token_struct->token = ++p;
	return (OK);
    }
...
**	    The change for SIR 122974 to test for null directly, tests
**	    for null by comparison to zero, whereas adc_isnull() tests for
**	    ADF_NVL_BIT. I have amended the change so that we test directly
**	    for ADF_NVL_BIT.
**      22-Sep-2011 (maspa05) b125430
**          Treat delimiter as unsigned char so non-7-bit ASCII chars can be
**          used
*/

static i4
	...
	*/
	...
	else
	{
		...
		*/
	    ...
	    {
		isnull = ADF_ISNULL_BL_MACRO(tup_ptr, cmap->cp_tuplen);
	    }
...
**      16-May-2011 (hanal04) SIR 125274
**          Convert LVCH to UTF8 when the copy coercion is long nvarchar
**          instead of the default long varchar.
*/
...

{
    ...
    DB_DATA_VALUE src_dv, rdv;	/* For UTF8 conversion */
    DB_STATUS	dbstat;
    i4		absttype;
    ...
    STATUS	sts;
    bool	utf8_conv = FALSE;

    absttype = abs(cmap->cp_tuptype);
    lo_type = DB_CHA_TYPE;
    if ((absttype == DB_LNVCHR_TYPE) || 
        ((absttype == DB_LVCH_TYPE) &&  
         (abs(cmap->cp_rowtype) == DB_LNVCHR_TYPE)))
    {
        if(absttype == DB_LNVCHR_TYPE)
        {
	...
	if (cblk->cp_convert)
	{
	    /* Set up UTF8 conversion */
            utf8_conv = TRUE;
	    rdv.db_datatype = DB_UTF8_TYPE;
            rdv.db_collID = DB_UNSET_COLL;
	    src_dv.db_datatype = lo_type;
            src_dv.db_collID = DB_UNSET_COLL;
	    /* prec uninteresting; length, addr set below */
	    ...
	    */
	    ...
	    {
                i2	factor = 3; /* Default for UCS2 source. See above */

                if(absttype == DB_LVCH_TYPE)
                {
                    /* char -> UCS2 -> UTF8 needed */
                    factor *= sizeof(UCS2);
                }

		cgroup->cpg_ucv_len = sizeof(i2) + MAX_SEG_INTO_FILE * factor;
	...
	if (utf8_conv)
	{
	    ...
	    src_dv.db_data = segbuf;
	    src_dv.db_length = segbytes;
	    ...
	    ((DB_TEXT_STRING *)rdv.db_data)->db_t_count = cgroup->cpg_ucv_len - sizeof(i2);
            if((lo_type == DB_CHA_TYPE))
            {
                dbstat = adu_nvchr_strutf8conv(lob_info->libq_cb->ii_lq_adf,
                                               &src_dv, &rdv);
            }
            else
            {
	        dbstat = adu_nvchr_toutf8(lob_info->libq_cb->ii_lq_adf, 
                                               &src_dv, &rdv);
            }
	    if (dbstat != E_DB_OK)
	    {
...
**      06-May-2010 (coomi01) b125340
**          Initialise num_written variable in case user
**          has not spotted change in documented interface
**          requirement and does not set it up as required.
*/
...
**      13-Jul-2011 (maspa05) b125382
**          For non-buffered COPY FROM PROGRAM return ENDFILE when cp_usr_row 
**          returns OK but has read 0 bytes.
*/
...
**	12-Jan-2011 (kschendel) b124887
**	    Empty input fields that are counted-strings are empty strings,
**	    not nulls!  Change a couple MEcopy's to memcpy's while I
**	    am in here.
*/
    ...
    register II_CP_STRUCT *copy_blk, II_CP_GROUP *cgroup, PTR tuple)
{
    ...
    */
    if (cgroup->cpg_tup_length > 0)
	memcpy(tuple, copy_blk->cp_zerotup + cgroup->cpg_tup_offset,
		cgroup->cpg_tup_length);
...
got_value:
	...
	*/
	...
	{
	    val_start = copy_blk->cp_valstart;
	    /* See if this might be a null or default:
	    ** - might know it's a null from a null-indicator byte;
	    ** - might match the null marker string;
	    ** - might be an empty value (but NOT if it's a counted string!
	    **   A zero-length counted string is an empty string.
	    **   A zero-length non-counted string is a default/null.)
	    ** Empty (non-counted) input is treated as default, which I don't
	    ** really approve of, but that's how it has always worked.
	    */
	    if (isnull
	      || (vallen == 0 && !cmap->cp_counted)
	      || (isnull = IIcp_isnull(cmap, val_start, vallen)) )
	    {
		/* If really NULL, make sure the column is nullable. */
		if (isnull && cmap->cp_tuptype > 0)
		{
		...
		*/
			...
			|| tup_ptr == (PTR) ME_ALIGN_MACRO(tup_ptr, DB_ALIGN_SZ) ) )
		{
		    ...
		    */
		    memcpy(aligned_tupval, tup_ptr, cmap->cp_adf_fnblk.adf_r_dv.db_length);
		}
		if (! (absrtype == DB_CHR_TYPE
			|| absttype == DB_BYTE_TYPE
			|| absrtype == DB_CHA_TYPE
		...
		{
		    memcpy(rowval_buf, val_start, vallen);
		...
		*/
		...
		{
		    memcpy(tup_ptr, aligned_tupval, cmap->cp_adf_fnblk.adf_r_dv.db_length);
		}
#endif
...
**      24-May-2011 (hanal04) Bug 125382
**          cp_program cases may return success with 0 bytes read rather than
**          returning ENDFILE. Ensure we return END file as we did before
**          the COPY rewrite.
*/
...
**	    Don't display improper COPY-FROM CO0028 counts if the DBMS server
**	    returns an error after we send our ending response.  This
**	    is relatively unlikely for Ingres copies, but can happen
**	    on a VectorWise copy-from if a constraint is violated.
**	11-Mar-2011 (kschendel) m1710
**	    More fine-tuning of copy ending:  unlike an Ingres table with
**	    unique key, if a VW copy-from violates a uniqueness constraint,
**	    VW loads NO rows of the dups (Ingres would load the first one).
**	    The CO0028 report is showing the number of rows sent, not
**	    the number of rows actually loaded.  Report the latter instead,
**	    so we don't report e.g. "2 rows copied" when none were.
**	24-Jul-2011 (kschendel) SIR 125587
**	    Handle vwload, must return rowcount and short-cut most of the
**	    other stuff that a normal copy does.
*/
static void
IIcpendcopy(II_LBQ_CB *IIlbqcb, II_CP_STRUCT *copy_blk)
{
    ...
    **	   the front end.
    **     No point in flushing if vwloader pseudo-copy.
    */
    if ((copy_blk->cp_direction == CPY_FROM)
      && ! copy_blk->cp_dbmserr && ! copy_blk->cp_vwload)
    {
    ...
    */
    ...
    if (read_resp && copy_blk->cp_vwload)
    {
	/* For vwloader, read DBMS response, but don't compare rowcounts */
    ...
    else if (read_resp && !copy_blk->cp_dbmserr)
    {
	...
	if (IIcgc_read_results(cgc, TRUE, GCA_RESPONSE) == 0)
	{
	    ...
	    */
	    else if ((cgc->cgc_resp.gca_rowcount == 0) &&
	    ...
	    /* if COPY FROM, DBMS rowcount is accurate # rows copied;
	    ** the DBMS knows how many rows were tossed for dup key, etc.
	    ** It also knows the difference in behavior between Ingres
	    ** and VW tables when things go wrong (eg constraint violation).
	    ** If COPY IN, keep our own rowcount which is the right one.
	    */
	    if (copy_blk->cp_direction == CPY_FROM)
	    {
		copy_blk->cp_rowcount = cgc->cgc_resp.gca_rowcount;
	...
	else if (copy_blk->cp_direction == CPY_FROM)
	{
	    /* Failure to get a normal RESPONSE after we sent our 
	    ** response on a copy from implies that something went poof
	    ** during copy end, and the DBMS sent back an error instead
	    ** a response.  Most likely, it's a VW constraint-check error,
	    ** or just possibly an error on the last row(s).
	    ** Show error status to avoid bogus N-rows-copied messages.
	    */
	    copy_blk->cp_error = TRUE;
	    copy_blk->cp_rowcount = 0;
	}
    ...
    */
    if (copy_blk->cp_direction == CPY_FROM && !copy_blk->cp_vwload &&
...
IIcpcleanup(II_CP_STRUCT *copy_blk)
{
    ...
    */
    if (!(copy_blk->cp_program || copy_blk->cp_vwload) && (copy_blk->cp_file_ptr))
    ...
    */
    ...
    if (copy_blk->cp_token_struct)
    {
	MEfree(copy_blk->cp_token_struct);
	copy_blk->cp_token_struct = NULL;
    }
...
**	    Use table macro to keep type-name table up to date.
*/

#define _DEFINEEND
#define _DEFINE(ty, v, dc, sc, q, qv, s, sv) "DB_" #ty "_TYPE",
static char *type_name_list[] = { DB_DT_ID_MACRO };
#undef _DEFINE
#undef _DEFINEEND
