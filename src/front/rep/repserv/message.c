...
**	12-Dec-2010 (coomi01) b124689
**	    Ensure generic errors are logged, and acted upon.
**/
...
static char	copyright_msg[] =
	ERx("%s Replicator %s - Copyright (c) 1993, %s Actian Corporation\n");
...
...)
{
	...
	if (errParm->ge_errorHandle)
	{
		...
		{
		    /*
		    ** Here we set the error type, but not the status.
		    ** ~ Specialising the status gives us oppurtunity to
		    **   write more appropriate error messages etc.
		    */
		    err_type = RS_ERR_DBMS;
		}
		else if (IIAPI_GE_ERROR == errParm->ge_type)
		{
		    /*
		    ** Bug 124689
		    ** A generic error, so mark it!
		    ** ~ We must ensure Replicator will not now exit
		    **   by not dealing with the problem at hand.
		    ** ~ We also want it to put out an error message,
		    **   below, so that failures are not silent.
		    */
		    status = err_type = RS_ERR_DBMS;
		}
	}
	...
	/* Bug b124689 
	** Where an error has occured, *especially* a generic one, 
	** we must log an error message, not fail and then exit silently.
	** ~ Which is why we sync'd status, and err_type above.
	*/
