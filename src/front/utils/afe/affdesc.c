...
**	11-Aug-2011 (kiria01) b125530
**	    Protect lookup from missing valid FI slots when skipping
**	    FIs that had the wrong number of operands.
*/
DB_STATUS
	...
	{ /* No coercions */
		...
		*/
			...
			ftab.adi_lntab > 1)
		{
		    ...
		    ** in operands. These will always be the first FI
		    ** of each compare op if present. */
		    best++;
		}
		for (; best < ftab.adi_tab_fi + ftab.adi_lntab; best++)
		{
		    /* 
		...
		if (best >= ftab.adi_tab_fi + ftab.adi_lntab)
		    return _error(cb, E_AF6006_OPTYPE_MISMATCH, opid, op[0], op[1]);
