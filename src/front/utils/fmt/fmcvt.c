...
**	    replace nat and longnat with i4
**	26-sep-2009 (zhayu01) Bug 121210
**	    Reduce the save_errmsgp string size from DB_MAXSTRING to 256
**	    on Pocket PC to avoid stack overflow there.
**/

/* # define's */

#ifdef WCE_PPC
#define	WCE_PPC_MAXSTRING	256
#endif
...
**	26-sep-2009 (zhayu01) SIR 121210
**	    Reduce the save_errmsgp string size from DB_MAXSTRING to 256
**	    on Pocket PC to avoid stack overflow there.
*/
...
i4		*sqlpat;
{
    ...
    i4			saved_emsglen;
#ifdef WCE_PPC
    char		saved_errmsgp[WCE_PPC_MAXSTRING];
#else
	...
	{   /* didn't pass template scan. try the generic conversion */
...
#ifdef WCE_PPC
	    STlcopy( cb->adf_errcb.ad_errmsgp, saved_errmsgp, WCE_PPC_MAXSTRING-1);
#else
		...
		status = saved_status;
#ifdef WCE_PPC
	        STlcopy( saved_errmsgp, cb->adf_errcb.ad_errmsgp, WCE_PPC_MAXSTRING-1);
#else
