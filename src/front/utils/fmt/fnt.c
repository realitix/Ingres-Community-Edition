...
**	29-Oct-2011 (kiria01) b125830
**	    Correct for buffer underrun from CMprev.
*/
	...
	while (t_ptr >= fmt->fmt_var.fmt_template)
	{
		...
		if (chkchar != '\0')
		{
			        ...
			        *t_ptr == FC_DECIMAL))
			{
				...
				{
					*r_ptr = fillchar;
					/* t_ptr might be invalid at this point as
					** CMprev can set it to be before start point
					** and if that is the case this loop would
					** terminate anyway and r_ptr needs not to be
					** updated. */
					if (t_ptr < fmt->fmt_var.fmt_template)
						r_ptr -= CMbytecnt(t_ptr);
				}
