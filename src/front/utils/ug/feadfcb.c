/*
** Copyright (c) 1989, 2010, 2011 Actian Corporation
**	All rights reserved.
*/
...
**          Pick up boolean II_DECIMAL_ROUND from the environment and
**          set ADF_CVTA_ROUND bitmask to adf_misc_flags for use in format
**          routine f_cvta. This will round rather than truncate decimal
**          values in fe-tools, eg report writer.
*/
...
FEadfcb ()
{
	...
	*/
	...
	NMgtAt(ERx("II_DECIMAL_ROUND"), &cp);
    	if ( cp != NULL && (0 == STcasecmp(cp,"TRUE")) )
    	{
	    cb.adf_misc_flags |= ADF_CVTA_ROUND;
    	}
	...
	** error TM_PMFILE_OPNERR, then lets report it now. */
	...
	if (CMischarset_utf8())
	{
          if (status = aduucolinit(col, &ucode_ctbl, &sys_err))
          {
    	    EXsignal(EXFEBUG, 1, ERx("FEadfcb(alloc)"));
          }
